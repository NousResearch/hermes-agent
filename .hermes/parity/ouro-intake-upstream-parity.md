...
| INT-005 | `src/ouroboros/bigbang/interview.py` prompt construction path | Question generation should use the upstream InterviewEngine contract: prompt-safe initial context, answer prefixes, brownfield intent/decision instruction, ambiguity snapshot, and perspective panel. | BO-062 stores `upstream_question_contract` from the vendored prompt builder on start/answer turns; gateway can consume an explicitly supplied Hermes-generated question override from that contract while provider calls remain disabled in this no-restart slice. | adapter_equivalent | `test_bo062_records_vendored_upstream_question_prompt_contract`, `test_bo062_updates_upstream_question_contract_after_answer`, `test_bo062_hermes_generated_question_overrides_gateway_fallback_on_start` |
