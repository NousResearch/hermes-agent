...
This checkout retains fork-only capabilities at the extension and operations
edges. Before an upstream merge or a change to fork-owned behavior, read
[`fork/AGENTS.md`](fork/AGENTS.md) and the applicable nested guide. The
repository-native policy in `scripts/merge_tools/` is the authoritative merge
