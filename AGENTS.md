...
**ALWAYS use `scripts/run_tests.sh`** — do not call `pytest` directly for PR/parity checks. The script enforces the same high-level environment controls CI relies on (credential vars blanked, `TZ=UTC`, `LANG=C.UTF-8`, deterministic hash seed, and the repo venv). It also runs tests through `scripts/run_tests_parallel.py`, which isolates test files in freshly spawned `python -m pytest <file>` subprocesses. There is no xdist layer in the wrapper.

```bash
scripts/run_tests.sh                            # full suite, CI-parity
scripts/run_tests.sh -j 4                       # cap per-file subprocess parallelism
scripts/run_tests.sh tests/gateway/             # one directory
scripts/run_tests.sh tests/tools/test_foo.py    # one file
scripts/run_tests.sh tests/foo.py -- --tb=long  # path + pytest args
scripts/run_tests.sh -- -v --tb=long            # pytest args only
```

Everything after a literal `--` is passed through to each per-file pytest invocation. Positional path arguments before `--` override the default discovery root (`tests/`). Avoid node-id examples in docs unless the runner explicitly supports them for the target path.

### Subprocess-per-file isolation

Each discovered test file runs in its own freshly spawned pytest subprocess via `scripts/run_tests_parallel.py`. This prevents module-level state from leaking between files while avoiding the old xdist/per-test isolation stack.

Implementation notes:

- The wrapper activates `.venv`, `venv`, or `$HOME/.hermes/hermes-agent/venv`.
- `-j/--jobs` controls how many file subprocesses run in parallel.
- Use `--` to pass pytest flags such as `-q`, `-v`, or `--tb=long`.
- The historic `--no-isolate` / `tests/_isolate_plugin.py` flow is not the current wrapper contract.
...
| HOME / `~/.hermes/` | Your real config+auth.json | Temp dir per test via `tests/conftest.py` |
| Timezone | Local TZ (PDT etc.) | UTC |
| Locale | Whatever is set | C.UTF-8 |
| Parallelism | Raw pytest / IDE behavior | Per-file subprocess runner controlled by `-j` |

`tests/conftest.py` also enforces the most important env isolation as an autouse fixture so IDE/raw pytest runs are safer than they used to be, but the wrapper remains the PR-parity path.

### Running without the wrapper (only if you must)

If you can't use the wrapper (e.g. inside an IDE that shells pytest directly), at minimum activate the venv and run an explicit pytest target:
...
Always run the wrapper before pushing changes.
