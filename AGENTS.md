...
Keep this file concise because Hermes auto-loads it into every repo session.
Long-form examples moved out of the always-loaded context live in
`docs/development/AGENTS-extended-reference.md`; load that file only when a
summarized section is directly relevant.
...
`hermes --tui` or `HERMES_TUI=1`.

- Process model: Node/Ink owns the screen (transcript, composer, prompts,
  tool activity) and speaks newline-delimited JSON-RPC over stdio to Python
  `tui_gateway`, which owns `AIAgent`, tools, sessions, and slash-command
  execution. See `tui_gateway/server.py` for the method/event catalog.
- Slash flow: local client commands (`/help`, `/quit`, `/clear`, `/resume`,
  `/copy`, `/paste`, etc.) are handled in `ui-tui/src/app.tsx`; everything
  else goes through `slash.exec` (`_SlashWorker`) with `command.dispatch`
  fallback.
- Dev loop: work in `ui-tui/`; use `npm run dev`, `npm run build`,
  `npm run typecheck`, `npm run lint`, and `npm test`.
- Dashboard `/chat`: embeds the real `hermes --tui` through
  `hermes_cli/pty_bridge.py` + `/api/pty` in `hermes_cli/web_server.py`.
  **Do not re-implement the primary chat experience in React.** Extend Ink for
  transcript/composer behavior; React sidebars/inspectors are fine only when
  they complement the PTY child and fail non-destructively.
- Electron desktop (`apps/desktop/`) is a separate Electron + React +
  nanostore chat surface using `tui_gateway` JSON-RPC, not an embedded TUI.
  Its slash-command curation lives in
  `apps/desktop/src/lib/desktop-slash-commands.ts`; keep skill and
  `quick_commands` extension commands discoverable (`isDesktopSlashExtensionCommand`)
  and executable. Tests: `apps/desktop/src/lib/desktop-slash-commands.test.ts`.

Expanded notes and examples: `docs/development/AGENTS-extended-reference.md`.
...
The skin engine (`hermes_cli/skin_engine.py`) is data-only visual customization.
Built-in skins live in `_BUILTIN_SKINS`; user skins are YAML files at
`~/.hermes/skins/<name>.yaml`. Missing values inherit from `default`.

Skins customize banner/response colors, spinner faces/verbs/wings, tool prefix,
per-tool emojis, and branding text. Activate with `/skin <name>` or
`display.skin: <name>` in `config.yaml`. Built-ins: `default`, `ares`, `mono`,
`slate`.

Expanded key table and YAML example: `docs/development/AGENTS-extended-reference.md`.

---

## Plugins

Hermes has multiple plugin surfaces. Repo-shipped plugins live under `plugins/`;
user plugins live under `~/.hermes/plugins/` or pip entry points.

- General plugins (`hermes_cli/plugins.py`, `plugins/<name>/`) expose
  `register(ctx)` for lifecycle hooks, tools, and CLI subcommands. Hooks run
  from `model_tools.py` / `run_agent.py`. Discovery is triggered by importing
  `model_tools.py`; code that reads plugin state earlier must call
  `discover_plugins()` explicitly.
- Plugins MUST NOT modify core files such as `run_agent.py`, `cli.py`,
  `gateway/run.py`, or `hermes_cli/main.py`. If a plugin needs more framework
  power, widen the generic plugin API instead of hardcoding plugin-specific
  core logic.
- Memory providers implement `agent/memory_provider.py::MemoryProvider` and are
  orchestrated by `agent/memory_manager.py`; only the currently active provider
  exposes CLI commands. The in-tree `plugins/memory/` provider set is closed —
  new memory backends ship as standalone plugins, not new directories here.
- Model providers live under `plugins/model-providers/<name>/` and register a
  `ProviderProfile`. Discovery is lazy (`providers/__init__.py`), separate from
  the general PluginManager, and user plugins override bundled names.
- Context-engine, image-gen, and dashboard companion plugins follow the same
  ABC + orchestrator pattern; example/companion plugins belong in the
  `hermes-example-plugins` repo unless they are core-shipped.

Expanded scan-order details: `docs/development/AGENTS-extended-reference.md`.
...
- `optional-skills/` — heavier or niche skills installed explicitly via
  `hermes skills install official/<category>/<skill>`; adapter:
  `tools/skills_hub.py::OptionalSkillSource`.

Heavy-dependency or niche skill PRs belong in `optional-skills/`. Standard
frontmatter fields: `name`, `description`, `version`, `author`, `license`,
`platforms`, `metadata.hermes.tags`, `metadata.hermes.category`,
`metadata.hermes.related_skills`, and optional `metadata.hermes.config`.
Top-level `tags:` / `category:` are accepted and mirrored.

Skill authoring standards are HARDLINE:

1. `description` is one sentence, ≤60 chars, ends with a period, and avoids
   marketing fluff or repeating the skill name.
2. SKILL.md prose names Hermes tools/MCP servers accurately. Prefer native
   tools (`read_file`, `search_files`, `patch`, `terminal`, `delegate_task`,
   etc.) over wrapped shell utilities (`cat`, `grep`, `sed`, `find`, `ls`).
3. Audit `platforms:` against actual scripts/imports; make skills
   cross-platform first, gate only when genuinely platform-bound.
4. Credit the human contributor first; "Hermes Agent" can be secondary.
5. Use the modern body order: title, intro, When to Use, Prerequisites, How to
   Run, Quick Reference, Procedure, Pitfalls, Verification.
6. Put scripts in `scripts/`, references in `references/`, templates in
   `templates/`; ship helpers for non-trivial parsing or automation.
7. Skill tests live at `tests/skills/test_<skill>_skill.py`, use stdlib +
   pytest + mocks, and avoid live network calls.
8. `.env.example` changes must stay inside a clearly delimited block.

Expanded checklist: `docs/development/AGENTS-extended-reference.md`.
...
**ALWAYS use `scripts/run_tests.sh`** instead of raw `pytest` for normal
verification. The wrapper enforces CI parity: credential env vars unset,
`HOME`/`HERMES_HOME` isolated, UTC timezone, `C.UTF-8` locale, xdist, and the
in-tree subprocess-isolation plugin.
...
scripts/run_tests.sh --no-isolate tests/foo/          # debug only
```

Subprocess isolation: every test runs through `tests/_isolate_plugin.py` using
`multiprocessing` spawn, so module state and ContextVars do not leak. The
`isolate_timeout` from `pyproject.toml` caps tests at 30s. Use `--no-isolate`
only for focused debugging or explicit state-leak checks.

If the wrapper is impossible (IDE/off-POSIX), activate a venv and run
`python -m pytest ...`; the isolation plugin still loads from `pyproject.toml`.
Always run the relevant wrapper command before claiming verification.

Do not write change-detector tests. Avoid snapshots of expected-to-change data
(model names, config-version literals, provider counts). Prefer behavior or
relationship invariants, e.g. catalog plumbing works, migrations update to the
current `DEFAULT_CONFIG["_config_version"]`, plan-only models don't leak into
legacy lists, and every catalog model has required metadata.

Expanded rationale and examples: `docs/development/AGENTS-extended-reference.md`.
