...
This root file contains universal invariants plus some project-wide reference material. Detailed
operational rules apply only when their stated area or trigger matches the task. Each area has its
own `AGENTS.md` (aim for ~8k chars; `agent/subdirectory_hints.py` delivers up to 32k and truncates
head/tail with a warning past that); use the **routing table** at the end to load the applicable
sub-area guide before editing there.
