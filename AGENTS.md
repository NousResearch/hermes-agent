...
### Scoped instructions and detailed references

Root `AGENTS.md` keeps the always-needed rules. More detailed guidance lives in:

- `docs/agent-context/*.md` — extracted long-form references.
- `agent/AGENTS.md` — core agent and prompt-building rules.
- `gateway/AGENTS.md` — messaging/gateway rules.
- `tools/AGENTS.md` — tool implementation rules.
- `tests/AGENTS.md` — test-suite rules.
- `plugins/AGENTS.md` — plugin policy.
- `ui-tui/AGENTS.md` — Ink TUI rules.
- `apps/desktop/AGENTS.md` — Electron desktop app rules.

Do not move load-bearing rules out of this root file unless a short version remains here.
...
## TUI, Dashboard Chat, and Desktop App

Detailed reference moved to `docs/agent-context/tui-and-desktop.md`.

Load-bearing rules:

- `hermes --tui` is an Ink/Node UI connected to Python `tui_gateway` over stdio JSON-RPC.
- Dashboard `/chat` embeds the real `hermes --tui` through a PTY bridge; do **not** rebuild the main transcript/composer in dashboard React.
- The Electron desktop app is a separate React/nanostore chat surface that talks to `tui_gateway` JSON-RPC; do **not** treat it as dashboard or TUI.
- Desktop slash-command curation lives in `apps/desktop/src/lib/desktop-slash-commands.ts`; skill and quick commands are extension commands and must remain discoverable/executable.
- For TUI changes run the relevant `ui-tui` npm checks; for desktop slash-command changes run the repo-root Vitest tests that cover `apps/desktop/src/lib/desktop-slash-commands.test.ts`.
...
Detailed reference moved to `docs/agent-context/skin-theme-system.md`.

Load-bearing rules:

- Skins are data-driven (`hermes_cli/skin_engine.py` plus user YAML under `$HERMES_HOME/skins/`).
- Missing skin values inherit from the default skin.
- Skin changes should not require core logic changes unless adding new skinable surfaces.
- Do not use `\033[K` in spinner/display code; use space padding instead.

## Plugins

Detailed reference moved to `docs/agent-context/plugins.md`.

Load-bearing rules:

- Plugins must not modify core files (`run_agent.py`, `cli.py`, `gateway/run.py`, `hermes_cli/main.py`, etc.) for plugin-specific behavior.
- If a plugin needs missing capability, widen the generic plugin surface or ABC; do not hardcode that plugin into core.
- General plugins register hooks/tools/CLI commands through `hermes_cli/plugins.py` and `ctx.register_*` APIs.
- Memory-provider plugins implement the `MemoryProvider` ABC and are orchestrated by `agent/memory_manager.py`.
- No new in-tree third-party-product plugins or memory providers; publish them as standalone plugin repos installed into `$HERMES_HOME/plugins/` or via entry points.
- Discovery timing pitfall: plugin discovery often happens as a side effect of importing `model_tools.py`; code that reads plugin state earlier must call discovery explicitly and idempotently.

## Skills

Detailed reference moved to `docs/agent-context/skills.md`.

Load-bearing rules:

- Skills are procedural memory for reusable workflows; keep project-wide always-on rules in AGENTS.md and put repeatable procedures in skills.
- Skill files must be complete enough to execute without hidden context: trigger conditions, exact commands, pitfalls, and verification.
- Do not add pagination/lazy-reading escape hatches to instructional tools that load skills; the agent must read the full procedure.
- If you use a skill and discover stale/wrong steps, patch the skill immediately.
- Agent-created skills are maintained by curator; bundled and hub-installed skills are not auto-deleted.
