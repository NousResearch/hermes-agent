...
If the user invokes `Use Act-As`, `Use Comply`, `Use Business Plan`, `Use Viber Structure`, `Use WOW Resource`, `Go to Sleep`, `Review Chat`, or an alias, read the registry and then the mapped prompt file before answering. Do not guess or summarize the shortcut from memory.
