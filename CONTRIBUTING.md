...
├── hermes_state.py           # SessionDB facade; SQLite operational/default storage lives in hermes_state_*.py siblings
├── conversation_store.py     # Provider-neutral contract for optional canonical conversation-history backends
...
| `~/.hermes/state.db` | Hermes operational/session database; also canonical message storage when `sessions.store: sqlite` (the default) |
| `~/.hermes/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written. |
...
- **Session persistence**: First-party callers use `SessionDB`. SQLite remains the zero-config default and owns Hermes operational/session-accounting state; `sessions.store` may delegate canonical conversation history to one external `ConversationStore`. External stores are single-authority: Hermes does not silently double-write or fall back to SQLite transcript payloads. Use `/save json` or `hermes sessions export` for an explicit export.
