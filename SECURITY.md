# HT AI Agent Security Policy

This document describes HT AI Agent's trust model, names the one
...
repository ([github.com/uaixo/awesome-hermes-agent/security/advisories](https://github.com/uaixo/awesome-hermes-agent/security/advisories)).
Do not open public issues for security vulnerabilities. **HT AI Agent
...
- Environment details (`ht version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted HT AI Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in HT AI Agent's documentation
...
HT AI Agent supports two OS-level isolation postures. They address
...
HT AI Agent supports this in two ways:

- **HT AI Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, HT AI Agent's in-process heuristics
...
HT AI Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in HT AI Agent
itself. Bugs in HT AI Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   HT AI Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, HT AI Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  HT AI Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not HT AI Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in HT AI Agent. Bugs in HT AI
...
- HT AI Agent includes supply-chain guards for MCP server
...
- **Channel:** the GitHub Security Advisory thread on
  [github.com/uaixo/awesome-hermes-agent](https://github.com/uaixo/awesome-hermes-agent/security/advisories).
