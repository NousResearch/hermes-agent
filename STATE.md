...
Last verification time: 2026-07-24 05:20 PDT (UTC-07:00)
...
- Branch: `main`
- Current HEAD: `c8baa0fc0`
- Upstream relation at this verification: 26 commits ahead of `origin/main`
...
- Serialized every audit-head migration precondition behind the SQLite
  `BEGIN IMMEDIATE` write transaction, so concurrent first opens cannot race a
  duplicate singleton insert while genuine legacy anchoring remains intact and
  a deleted post-migration checkpoint still fails closed.
...
- Verified the latest review fix with 57 control-plane tests, including a
  deterministic 24-caller concurrent first-open regression plus the existing
  legacy and deleted-head cases, repository-wide Ruff, touched-file Ruff
  formatting checks, `git diff --check`, the uv lockfile consistency check, and
  the relevant managed-node API/CLI suites.
