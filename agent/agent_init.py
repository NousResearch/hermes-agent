...
from agent.model_metadata import (
    MINIMUM_AGENT_CONTEXT_LENGTH, fetch_model_metadata, is_local_endpoint, query_ollama_num_ctx
)
...
def _enforce_minimum_context(agent):
    # Reject windows below the configured agent floor (MINIMUM_AGENT_CONTEXT_LENGTH,
    # 16K by default, overridable via HERMES_MINIMUM_AGENT_CONTEXT_LENGTH so small
    # self-hosted models can run against their real budget); an explicit positive
    # model.context_length on LM Studio is allowed below the floor.
    ...
    if _ctx and _ctx < MINIMUM_AGENT_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        floor_k = MINIMUM_AGENT_CONTEXT_LENGTH // 1000
        if agent.base_url and is_local_endpoint(agent.base_url):
            ...
            remedy = (
                ...
                f"{floor_k}K context (llama.cpp: -c {MINIMUM_AGENT_CONTEXT_LENGTH}; vLLM: --max-model-len; "
                f"Ollama: OLLAMA_CONTEXT_LENGTH={MINIMUM_AGENT_CONTEXT_LENGTH} or a Modelfile num_ctx), "
...
_CALLBACK_PARAMS = (
    ...
    "thinking_callback", "reasoning_callback", "clarify_callback", "clarify_form_callback",
...
def init_agent(
    ...
    clarify_callback: callable = None, clarify_form_callback: callable = None,
