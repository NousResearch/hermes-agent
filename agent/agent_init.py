...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        ...
        for provider_id, config in PROVIDER_REGISTRY.items():
            if (
                normalize_registry_provider(normalize_model_provider(provider_id))
                == provider
            ):
...
def _custom_provider_model_matches(agent_model: str, entry: Dict[str, Any]) -> bool:
    ...
    catalog = (
        [str(m).strip().lower() for m in models]
        if isinstance(models, (dict, list, tuple))
        else []
    )
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if is_actual_route(agent.provider, base_url):
        ...
        provider_name is None and host == "chatgpt.com" and "/backend-api/codex" in url
    ):
        agent.api_mode = "codex_responses"
        ...
        provider_name is None and host == "api.anthropic.com"
    ):
        ...
        host.startswith("bedrock-runtime.")
        and base_url_host_matches(url, "amazonaws.com")
    ):
...
def _init_turn_state(agent, run_budget_seconds):
    ...
    agent._credits_latch = (
        new_credits_latch()
    )  # threshold-notice latch (sticky keys + gates)
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "bedrock":
        ...
        if not agent.quiet_mode:
            print(
                f"🤖 AI Agent initialized with model: {agent.model} (AWS Bedrock + AnthropicBedrock SDK, {agent._bedrock_region})"
            )
        return
    ...
    effective_key = (
        api_key or (resolve_anthropic_token() if _is_native_anthropic else None) or ""
    )
...
def _explicit_client_kwargs(
    agent, api_key, base_url, _provider_timeout
) -> Dict[str, Any]:
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    if _routed_client is not None:
        ...
        effective_provider = getattr(
            _routed_client, "_hermes_aux_effective_provider", ""
        )
    ...
    for _fb in _fallback_entries(fallback_model):
        try:
            ...
            logger.debug(
                "Init-time fallback entry %s failed: %s", _fb.get("provider"), _fb_exc
            )
            continue
...
def _apply_openai_header_policy(agent, client_kwargs: Dict[str, Any]) -> None:
    ...
    if (
        ...
        if _TOOL_STREAMING_BETA not in existing_beta:
            headers["x-anthropic-beta"] = ",".join(
                filter(None, (existing_beta, _TOOL_STREAMING_BETA))
            )
    ...
    try:
        ...
        apply_custom_provider_tls_to_client_kwargs(
            client_kwargs, _cp_base_url, _cp_entries
        )
...
def _init_openai_client(agent, api_key, base_url, fallback_model, _provider_timeout):
    ...
    try:
        ...
        if agent.provider == "bedrock" and "bedrock-mantle." in str(
    ...
    try:
        ...
        if not agent.quiet_mode:
            ...
            _print_key_banner(
                client_kwargs.get("api_key", "none"), "API key", warn_missing=True
            )
...
def _lazy_headers(
    module: str, name: str, pass_key: bool = False, pass_base: bool = False
):
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    (
        ...
        _lazy_headers(
            "agent.auxiliary_client", "build_nvidia_nim_headers", pass_base=True
        ),
    ),
    (
        "api.routermint.com",
        _lazy_headers("agent.client_lifecycle", "_routermint_headers"),
    ),
    (
        "githubcopilot.com",
        _lazy_headers("hermes_cli.models", "copilot_default_headers"),
    ),
    ...
    (
        "chatgpt.com",
        _lazy_headers("agent.codex_headers", "codex_cloudflare_headers", pass_key=True),
    ),
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    ...
    if agent.tools:
        print(
            f"🛠️  Loaded {len(agent.tools)} tools: {', '.join(sorted(agent.valid_tool_names))}"
        )
        if enabled_toolsets:
...
def _apply_display_config(agent, _agent_cfg, platform):
    ...
    agent.show_commentary = bool(
        _cfg_dict(_agent_cfg, "display").get("show_commentary", True)
    )

    # Window (seconds) for the bounded /fast auto|cold modes (agent.fast_mode).
    agent.fast_auto_seconds = (_agent_cfg.get("agent") or {}).get(
    ...
    agent.lmstudio_load_mode = (
        _load_mode if _load_mode in {"explicit", "jit"} else "explicit"
    )
...
def _compression_threshold(agent, cfg: Dict[str, Any]) -> tuple[float, bool]:
    ...
    with suppress(Exception):
        ...
        threshold, agent._compression_threshold_autoraised = (
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    app_server_auto = str(
        cfg.get("codex_app_server_auto", "native") or "native"
    ).lower()
...
def _parse_compression_config(agent, _agent_cfg) -> CompressionSettings:
    ...
    app_server_auto, responses_native, compact_threshold = _compression_codex_settings(
        cfg
    )
    ...
    return CompressionSettings(
        ...
        min_tail_users=max(
            1, _parse_config_int(cfg.get("min_tail_user_messages", 1), 1)
        ),
        max_attempts=min(max_attempts, 10),
        # Opt-in proactive tool-result prune trigger (0 = disabled; negatives = disabled).
        proactive_prune_tokens=max(
            0, _parse_config_int(cfg.get("proactive_prune_tokens", 0), 0)
        ),
        ...
        proactive_prune_min_reclaim=max(
            0,
            _parse_config_int(
                cfg.get("proactive_prune_min_reclaim_tokens", 4096), 4096
            ),
        ),
        ...
        model_thresholds={
            str(k): float(v)
            for k, v in _cfg_dict(cfg, "model_thresholds").items()
        ...
        micro_compact_defrag_tokens=max(
            1,
            _parse_config_int(
                cfg.get("micro_compact_defrag_threshold_tokens", 2000), 2000
            ),
        ),
...
def _warn_invalid_config_int(
    ...
    _ra().logger.warning(
        "Invalid %s: %r — %s. Falling back to %s.",
        what,
        value,
        requirement,
        fallback,
    ...
    if isinstance(_user_providers, dict):
        ...
        for _key, _entry in _user_providers.items():
            if not isinstance(_entry, dict):
                continue
            _ids = _custom_provider_runtime_ids(_key) | _custom_provider_runtime_ids(
                _entry.get("name")
            )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        and _norm != "custom"
        and not _norm.startswith("custom:")
    ):
        with suppress(Exception):
...
def _active_route_url(agent, base_url) -> str:
    ...
    if "?" in _requested.split("#", 1)[0]:
        with suppress(TypeError, ValueError):
            ...
            if normalize_route_base_url(_without_query) == normalize_route_base_url(
                _active
            ):
                _active = _requested
...
def _scope_context_length_to_default_runtime(
    ...
    _configured_base_url = _configured_default_base_url(
        _agent_cfg, _model_cfg, _custom_providers
    )
...
def _warn_invalid_custom_provider_context_length(agent, _custom_providers) -> None:
    ...
    for _cp_entry in _custom_providers:
        ...
        _cp_model_cfg = (
            _cp_models.get(agent.model, {}) if isinstance(_cp_models, dict) else None
        )
        _cp_ctx = (
            _cp_model_cfg.get("context_length")
...
def _resolve_context_length(agent, _agent_cfg, base_url):
    ...
    try:
        agent._aux_compression_context_length_config = (
            int(_aux_ctx) if _aux_ctx is not None else None
        )
    except (TypeError, ValueError):
...
def _enforce_minimum_context(agent):
    ...
    if (
        ...
        and not (_is_jit or _allow_lmstudio_explicit_below_floor)
    ):
        raise ValueError(
...
def _emit_compression_summary(agent, cs):
    ...
    if not agent.quiet_mode:
        ...
        if cs.enabled:
            ...
            print(
                f"📊 Context limit: {_cc.context_length:,} tokens (compress at {int(_pct * 100)}% = {_cc.threshold_tokens:,}{_cap_note})"
            )
        else:
            print(
                f"📊 Context limit: {_cc.context_length:,} tokens (auto-compression disabled)"
            )
...
def init_agent(
    ...
    _config_context_length, _custom_providers, _effective_context_length, _model_cfg = (
        _resolve_context_length(agent, _agent_cfg, base_url)
    )
