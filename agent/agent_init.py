...
def init_agent(
    ...
    #
    # Codex gpt-5.5 autoraise (upstream): the Codex backend caps the window at
    # 272K, so the default 50% would compact at ~136K — half the usable
    # context. ``_compression_threshold_for_model`` raises it to 85% unless the
    # ``compression.codex_gpt55_autoraise`` opt-out flag is set; when it fires
    # we stash a one-time notification (replayed on the first turn).
    ...
    if _cfg_model_thresh is not None:
        ...
        try:
            ...
            if _model_cthresh is not None:
                ...
                # override is a long-standing silent default). Skip the notice
                # when the user's global threshold already meets/exceeds the
