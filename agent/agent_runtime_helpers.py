...
def recover_with_credential_pool(
    ...
    *,
    ...
    if effective_reason == FailoverReason.billing:
        ...
        next_entry = pool.mark_exhausted_and_rotate(status_code=rotate_status, error_context=error_context, api_key_hint=(agent.api_key if isinstance(getattr(agent, "api_key", None), str) else None))
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if (is_openrouter or is_nous_portal) and is_claude:
        return True, False
    # ZenMux (zenmux.ai) serves Claude models via an OpenAI-compatible
    # /v1/chat/completions endpoint that accepts Anthropic-style
    # cache_control markers in the OpenAI-wire envelope layout (the same
    # format OpenRouter uses). ZenMux's Anthropic models are in the
    # "explicit cache" tier — without cache_control breakpoints the
    # upstream returns 0% cache reads and re-bills the full prompt on
    # every turn. Official docs confirm OpenAI-wire cache_control support:
    # https://zenmux.ai/docs/zh/guide/advanced/prompt-cache.html
    is_zenmux = base_url_host_matches(eff_base_url, "zenmux.ai")
    if is_zenmux and is_claude:
        return True, False
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    # switch_model() rebuilds the client and _primary_runtime but historically
    # left ``_credential_pool`` untouched.  A session that started on a
    # provider WITHOUT a pool (e.g. deepseek) keeps ``_credential_pool=None``
    # after the user /model-switches to a pool-backed provider (e.g.
    # kimi-coding).  When that provider later returns 403/429,
    # ``recover_with_credential_pool()`` short-circuits at ``pool is None`` and
    # the agent falls through to ``fallback_providers`` instead of rotating to
    # the second account.  Reload the pool here so the new provider's
    # multi-account rotation works mid-session.
    try:
        from agent.credential_pool import load_pool
        # Defense-in-depth: normalize models.dev-style provider ids (e.g.
        # "kimi-for-coding" from resolve_provider_full) back to the
        # PROVIDER_REGISTRY canonical id before loading the pool.  The pool
        # is keyed on the registry id; loading under a models.dev id returns
        # an empty pool and disables mid-session credential rotation.
        _pool_provider = (new_provider or "").strip().lower()
        try:
            from hermes_cli.auth import PROVIDER_REGISTRY as _pr, resolve_provider as _rp
            if _pool_provider and _pool_provider not in _pr:
                _resolved = (_rp(_pool_provider) or "").strip().lower()
                if _resolved and _resolved in _pr:
                    _pool_provider = _resolved
        except Exception:  # noqa: BLE001
            pass
        _new_pool = load_pool(_pool_provider)
        if _new_pool and _new_pool.has_credentials():
            agent._credential_pool = _new_pool
        else:
            agent._credential_pool = None
    except Exception as _pool_err:  # noqa: BLE001
        logger.debug(
            "switch_model: credential pool reload for %s failed: %s",
            new_provider, _pool_err,
        )
        agent._credential_pool = None
