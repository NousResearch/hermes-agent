...
def _is_deepseek_anthropic_endpoint(base_url: str | None, model: str = "") -> bool:
    ...
    Detection order:
    1. Direct: base_url contains ``api.deepseek.com`` and path has ``/anthropic``
    2. Model-based: any DeepSeek V4 model via a custom relay (the relay forwards
       to DeepSeek and enforces the same thinking echo-back contract).
    See hermes-agent#16748, #17341.
    """
    # Direct DeepSeek endpoint
    if base_url_host_matches(base_url or "", "api.deepseek.com"):
        normalized = _normalize_base_url_text(base_url)
        if normalized and "/anthropic" in normalized.rstrip("/").lower():
            return True
    # Model-based detection: DeepSeek V4 models through custom relays
