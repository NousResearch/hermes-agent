...
def _sanitize_replay_block(b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    *input* schema forbids ("Extra inputs are not permitted"): text blocks get
    ...
    if btype == "text":
        ...
        # blocks where "text" is empty or whitespace-only. Type-safe:
        # captured blocks can carry text=None from an invalid upstream
        # payload, which a bare .strip() would crash on.
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # `content or ""` there would silently restore the invalid provider
    # payload this function exists to prevent.
    effective = blocks if blocks else [{"type": "text", "text": "(empty)"}]
