...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      3. ANTHROPIC_AUTH_TOKEN env var (alternative auth token)
    ...
    # 3. ANTHROPIC_AUTH_TOKEN (alternative auth token env var)
    auth_token = os.getenv("ANTHROPIC_AUTH_TOKEN", "").strip()
    if auth_token:
        preferred = _prefer_refreshable_claude_code_token(auth_token, creds)
        if preferred:
            return preferred
        return auth_token
