...
def _convert_user_message(content) -> Dict:
...
def _strip_orphaned_tool_blocks(result: List[Dict]) -> None:
...
def _merge_consecutive_roles(result: List[Dict]) -> List[Dict]:
...
def _manage_thinking_signatures(
    result: List[Dict], base_url: str | None, model: str | None
) -> None:
    """Strip or preserve thinking blocks based on endpoint type.

    Anthropic signs thinking blocks against the full turn content.
    Any upstream mutation invalidates the signature, causing HTTP 400.
    Signatures are Anthropic-proprietary — third-party endpoints can't
    validate them. Strategy depends on endpoint type.
    """
...
def _evict_old_screenshots(result: List[Dict]) -> None:
    """Keep only the most recent 3 computer-use screenshots to save context.

    Base64 images cost ~1,465 tokens each and accumulate across tool calls.
    Walk backward, keep the most recent, replace older ones with a placeholder.
    """
    _MAX_KEEP_IMAGES = 3
    _image_count = 0
