"""
AppState - Centralized state container for AIAgent.

This dataclass holds ALL configuration, runtime, and state attributes
that were previously scattered across run_agent.py.__init__ as self.xxx assignments.

All attributes are typed for clarity. Defaults are None or zero-values
where the attribute may be optionally set.
"""
...
class AppState:
    ...
    iteration_budget: Any = None  # IterationBudget instance
    ...
    _task_router: Any = None  # TaskRouter instance for intelligent routing
    ...
    # Current tool tracking (for activity reporting)
    _current_tool: str = None
    _api_call_count: int = 0

    # Anthropic image fallback cache
    ...
    # Budget caution/warning thresholds
    _budget_caution_threshold: float = 0.7
    _budget_warning_threshold: float = 0.9
    _budget_pressure_enabled: bool = True
    ...
    # Last content with tools
    _last_content_with_tools: Any = None

    # Invalid retry counters
    _invalid_tool_retries: int = 0
    _invalid_json_retries: int = 0
    _empty_content_retries: int = 0
    _incomplete_scratchpad_retries: int = 0
    ...
    # Executing tools flag
    _executing_tools: bool = False

    # Stream callback and break flag
    _stream_callback: Callable = None
    _stream_needs_break: bool = False
