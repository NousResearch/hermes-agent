...
The persistent overlay is only ever grown by an explicit, human-curated
promotion step (see ``learn(..., persist=True)``), never automatically from a
single run's denial.
...
# --------------------------------------------------------------------------- #
# LIVE in-session learning (process-scoped by default)                          #
#                                                                               #
...
# a NOVEL evasion the dictionary doesn't know, the judge may still deny it, but #
# the detector stays silent and the model can keep reusing that fresh phrasing  #
# for the rest of the SAME run. To close that hole, when a completion is denied #
# and the detector did NOT flag it, the caller can ``learn()`` the offending    #
# response: we extract its candidate clauses and add them to a PROCESS-SCOPED   #
# learned set the detector consults immediately, so the same novel dodge is     #
# caught on the model's very next turn — no human, no run-end.                   #
#                                                                               #
# IMPORTANT (review nits teknium1 @ deception.py:203, chaos-xxl): learned       #
# clauses are NOT persisted to the profile-global overlay by default. A single  #
# run's denial is not proof each extracted clause is deceptive, and a           #
# substring-matched clause like "the limitation is" could contaminate unrelated #
# future runs. So ``learn()`` defaults to ``persist=False`` (this run only).    #
# The ONLY promoter into the persistent overlay is the explicit, human-curated  #
# ``harvest`` path. Persistence is available but opt-in (``persist=True``).      #
# --------------------------------------------------------------------------- #
...
def learn(text: str, *, category: str = "learned_evasion", persist: bool = False) -> list[str]:
    """Learn novel deception phrasings from a response that was denied but the
    detector did NOT flag. Adds them to the PROCESS-SCOPED learned set (enforced
    immediately, this run only). Returns the phrases learned this call.

    ``persist`` is ``False`` by default (review nits teknium1 @ deception.py:203,
    chaos-xxl): a single run's denial is not proof each extracted clause is
    deceptive, so we do not silently write to the profile-global overlay where a
    broad substring like "the limitation is" could contaminate unrelated runs.
    The explicit, human-curated ``harvest`` path is the only promoter into the
    persistent overlay. Pass ``persist=True`` only from that curated path.
    """
...
def _append_to_overlay(category: str, phrases: list[str]) -> None:
    ...
    if not spec.get("note"):
        spec["note"] = (
            "Learned live from a denied completion the detector had not yet "
...
def scan(final_response: str, *, user_name: str = "") -> DeceptionSignal:
    ...
    # learned_evasion: novel phrasings captured live this run from denied
    # completions the dictionary hadn't seen. Enforced from the very next turn.
    if _has_any(t, learned_patterns()):
        sig.flags.append("learned_evasion")
        sig.notes.append(
            "You reused a phrasing that was already rejected earlier in this run. "
