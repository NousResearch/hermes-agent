...
def _try_qianfan() -> Tuple[Optional[Any], Optional[str]]:
    """Try Baidu Qianfan (custom_providers.qianfan) for auxiliary tasks.

    Returns (client, model) if qianfan is configured in custom_providers,
    otherwise (None, None).
    """
    try:
        from hermes_cli.config import load_config
    ...
    for cp in custom_providers:
        if isinstance(cp, dict) and cp.get("name") == "qianfan":
            qianfan_cfg = cp
            break
    if not qianfan_cfg:
        return None, None

    base_url = str(qianfan_cfg.get("base_url", "")).strip().rstrip("/")
    api_key = str(qianfan_cfg.get("api_key", "")).strip()
    if not base_url or not api_key:
        return None, None

    model = "kimi-k2.5"
    logger.debug("Auxiliary client: qianfan (%s)", model)
