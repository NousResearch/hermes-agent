...
def _is_invalid_aux_response_error(exc: Exception) -> bool:
    ...
    return (
        ...
        or "llm returned router timeout shim" in msg
    )
...
def _validate_llm_response(
    ...
    if is_router_timeout_shim(response):
        # HTTP-success router failure: keep it eligible for provider fallback.
        raise RuntimeError(f"Auxiliary {task or 'call'}: LLM returned router timeout shim")
