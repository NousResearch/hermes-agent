...
_MEMORY_REVIEW_PROMPT = (
    "Review the conversation above and classify whether anything belongs in persistent memory.\n\n"
    "Memory is ONLY for compact durable facts that should be available in every "
    "future turn: stable user preferences/personal facts, communication style, "
    "environment facts, tool quirks, and stable conventions.\n\n"
    "Do NOT save procedures, checklists, reusable workflow corrections, operational "
    "rules, project-scoped conventions, knowledge assets, task progress, completed "
    "work logs, or temporary state to memory. Those belong in skills, project docs, "
    "AI_Agent_Assets, Kanban/Quest state, or session_search.\n\n"
    "If a compact user/environment fact clearly belongs in memory, save it using "
    "the memory tool. If the learning belongs elsewhere, or nothing is worth "
    "injecting into every future turn, say 'Nothing to save.' and stop."
)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "style/format/workflow preference, classify the destination first. "
    "Compact stable user facts may also belong in memory, but workflow, "
    "format, and procedure corrections belong in the SKILL.md body or a "
    "governing instruction. When they complain about how you handled a task, "
    "the skill that governs that task needs to carry the lesson.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and update durable learning only in the right place.\n\n"
    "**Memory**: compact durable facts that should be injected into every future "
    "turn. Save only stable user preferences/personal facts, communication style, "
    "environment facts, tool quirks, or stable conventions. Do NOT save procedures, "
    "workflow rules, reusable corrections, project-scoped facts, knowledge assets, "
    "task progress, completed-work logs, or temporary state to memory.\n\n"
    ...
    "you handled a task, first classify the destination. A compact, stable "
    "preference may belong in memory, but workflow/format/procedure corrections "
    "belong in the skill or governing instruction that controls that class of "
    "work. Memory alone is not enough, and memory should not carry operational "
    "rules that a skill or project document should own.\n\n"
