...
_SKILL_REVIEW_PROMPT = (
    ...
    "'remember this' are FIRST-CLASS skill signals, not just memory signals, but a signal is not "
    "an automatic write. Persist a style or workflow preference only when the user explicitly "
    "corrects the behavior or the pattern repeats. When that gate is met, update the relevant "
    "skill(s) so the next session starts already knowing.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "is FIRST-CLASS evidence to assess, not an automatic write. Persist a style or workflow "
    "preference only when the user explicitly corrects the behavior or the pattern repeats. "
    "When that gate is met, embed the lesson in the skill that governs the task.\n"
