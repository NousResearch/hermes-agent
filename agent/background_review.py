...
_SKILL_REVIEW_PROMPT = (
    ...
    "play, so it's the right one to extend. Being loaded earlier is "
    "NOT permission to write: call skills_list() and confirm that "
    "skill is writable before you patch it. Most loaded skills belong "
    "to the user and are not writable by this pass.\n"
    ...
    "Which skills you may write (READ THIS BEFORE PICKING A TARGET):\n"
    "This pass owns only the skills the agent itself created. In this "
    "pass, skills_list() marks every entry with `writable` (true/false) "
    "and `blocked_because`. Pick your target from the writable=true "
    "entries. A write to a writable=false entry is refused every time, "
    "so do NOT retry one — retrying a blocked target is the single "
    "biggest waste of this pass.\n"
    "Blocked (writable=false) covers, per `blocked_because`:\n"
    "  • bundled — shipped with Hermes (e.g. 'hermes-agent').\n"
    "  • hub_installed — installed via 'hermes skills install'.\n"
    "  • protected_builtin — a load-bearing built-in.\n"
    "  • external — lives in a skills.external_dirs directory.\n"
    "  • pinned — marked via 'hermes curator pin'. Pin blocks autonomous "
    "edits too, not only deletion: there is no user here to consent to "
    "the change.\n"
    "  • not_agent_created — the user wrote it (created_by is not "
    "'agent'). Manually authored skills are not yours to change.\n"
    "MANDATORY fallback when the best-fitting skill is blocked: do not "
    "retry it, and do not drop the lesson. Write the lesson into a NEW "
    "agent-owned class-level skill (skill_manage action=create), or, "
    "when it is a fact about the user rather than a technique, save it "
    "with the memory tool. A blocked best-target is never a reason to "
    "end the pass with nothing saved.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "it's the right place. Being loaded earlier is NOT permission to "
    "write: call skills_list() and confirm that skill is writable "
    "before you patch it. Most loaded skills belong to the user and "
    "are not writable by this pass.\n"
