...
_SKILL_REVIEW_PROMPT = (
    ...
    "'references/', 'templates/', or 'scripts/'. A support file is a TWO-PART write: the file AND a "
    "one-line pointer in SKILL.md naming it. skill_view() lists supporting file names but never "
    "reads their content, so a file SKILL.md does not name is invisible to future sessions — "
    "skill_manage reports that as route_warning on the write; clear it before you finish.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "probes). The file and a one-line pointer in SKILL.md naming it are ONE write: skill_view() "
    "lists supporting file names but never reads their content, so a file SKILL.md does not name "
    "is invisible to future sessions (skill_manage reports it as route_warning on the write).\n"
