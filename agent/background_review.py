...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • The foreground agent hit max_iterations, repeated the same "
    "tool pattern without new evidence, or the user reported the same "
    "class of failure again. Capture the reusable lesson as a bounded "
    "workflow, and when appropriate state that the durable fix should "
    "be an executable route such as local_repair, a gateway known_ops_tasks "
    "entry, a cron/status probe, or a regression test. Do not only save "
    "a memory note for failures that need an executable path.\n\n"
