...
_SKILL_REVIEW_PROMPT = (
    ...
    "play, so it's the right one to extend. Before any skill_manage "
    "patch/edit/write_file/remove_file call, load the exact target in "
    "the current review turn: call skill_view(name) for SKILL.md or "
    "skill_view(name, file_path=...) for a support file, then write "
    "against the content you just loaded. If skill_manage returns "
    "_read_before_write_required, perform that matching skill_view call "
    "and retry once.\n"
    "  2. UPDATE AN EXISTING UMBRELLA (via skills_list + skill_view). "
    "If no loaded skill fits but an existing class-level skill does, "
    "patch it. Add a subsection, a pitfall, or broaden a trigger. The "
    "same read-before-write rule applies: skill_view the exact file in "
    "this turn immediately before mutating it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "it's the right place. Before any skill_manage patch/edit/write_file/"
    "remove_file call, load the exact target in this review turn with "
    "skill_view(name) for SKILL.md or skill_view(name, file_path=...) "
    "for a support file; if a write is refused with "
    "_read_before_write_required, do that matching read and retry once.\n"
    "  2. UPDATE AN EXISTING UMBRELLA (skills_list + skill_view to "
    "find the right one). Patch it, after skill_view'ing the exact file "
    "you are about to mutate in the current review turn.\n"
