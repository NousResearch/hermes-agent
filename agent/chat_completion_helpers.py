...
def _chat_summary_attempt(agent, api_messages: list, api_request_id: str):
    ...
    def _attempt(retry_count: int) -> str:
        ...
        if not agent._get_transport().validate_response(response):
            # Router timeout shim (or otherwise invalid payload): surface as an
            # empty summary so the retry loop tries once more instead of
            # presenting router noise as the final summary.
            return ""
