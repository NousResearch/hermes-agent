...
class ClientLifecycleMixin:
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        def _check_final_wire_route(final_kwargs: dict) -> None:
            check_route(current_routing_policy(), provider=str(getattr(self, "provider", "") or ""),
                        model=str(effective_wire_model(final_kwargs, getattr(self, "model", "")) or ""),
                        base_url=str(getattr(request_client, "base_url", "") or getattr(self, "base_url", "") or ""))
