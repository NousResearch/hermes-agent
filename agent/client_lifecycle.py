...
def _swap_fallback_clients(agent, fb_client, fb_provider: str, fb_model: str, fb_base_url: str, fb_api_mode: str) -> None:
    ...
    if fb_api_mode == "anthropic_messages":
        ...
        agent._is_anthropic_oauth = is_native_anthropic_oauth(effective_key, fb_base_url)
...
class ClientLifecycleMixin:
    ...
    def _anthropic_oauth_flag(self, token) -> bool:
        """OAuth flag only for the native Anthropic host; third-party Anthropic-protocol endpoints
        must not trip OAuth paths. A named custom provider pointing at api.anthropic.com keeps a
        key_cmd-sourced OAuth token's identity (#114967)."""
        from agent.anthropic_credentials import is_native_anthropic_oauth
        return is_native_anthropic_oauth(token, getattr(self, "_anthropic_base_url", None))
