...
def _codex_app_server_params(note: Any) -> Dict[str, Any]:
...
def _codex_app_server_text_parts(value: Any) -> List[str]:
    if value is None:
        return []
    ...
    for item in iterator:
        parts.extend(_codex_app_server_text_parts(item))
    return parts
...
def _make_codex_app_server_event_callback(agent):
    """Bridge codex app-server item notifications into Hermes stream callbacks.

    The app-server runtime bypasses the Responses SSE consumer, so without this
    bridge Oryn/other API clients get a final assistant message but no live
    ``hermes.reasoning.delta`` events while Codex is thinking.
    """

    def _on_event(note: dict) -> None:
        method = note.get("method", "") if isinstance(note, dict) else ""
        params = _codex_app_server_params(note)

        if method == "item/agentMessage/delta":
            text = "".join(_codex_app_server_text_parts(params.get("delta")))
            if text:
        ...
        if method == "item/reasoning/delta":
            text = "".join(_codex_app_server_text_parts(params.get("delta")))
            if text:
                agent._codex_app_server_reasoning_delta_seen = True
                agent._fire_reasoning_delta(text)
            return

        if method == "item/completed":
            item = params.get("item")
            text = _codex_app_server_completed_reasoning_text(item)
            if text and not getattr(
                agent, "_codex_app_server_reasoning_delta_seen", False
            ):
                agent._fire_reasoning_delta(text)
    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        agent._codex_session = CodexAppServerSession(
            ...
            on_event=_make_codex_app_server_event_callback(agent),
        )
