...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def _build_compression_call_kwargs(self, prompt: str) -> Dict[str, Any]:
    ...
    def _call_compression_llm(self, prompt: str) -> str:
        """Call the auxiliary compression LLM through the guarded summary path."""
        with aux_interrupt_protection():
            response = call_llm(**self._build_compression_call_kwargs(prompt))
        message = response.choices[0].message
        if isinstance(message, dict):
            content = message.get("content")
        else:
...
"""
    ...
    def _summarize_segment_recursive(
        ...
        try:
            prompt = self._build_segment_summary_prompt(
                segment_text, segment_index, segment_count, focus_topic
            )
            prompt += f"\nTarget ~{budget} tokens."
            return self._call_compression_llm(prompt)
        except Exception as exc:
...
{template_sections}"""
        ...
        final_summary = self._call_compression_llm(reduce_prompt)
