...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def _log_compaction_stats(
        ...
        """Write compaction statistics to ~/.hermes/logs/compaction.log as JSON lines."""
        try:
            log_dir = Path(get_hermes_home()) / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
            log_path = log_dir / "compaction.log"
            entry = {
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "session_id": getattr(self, "_session_id", None) or None,
                "before_messages": before_messages,
                "after_messages": after_messages,
                "messages_compressed": before_messages - after_messages,
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        self._log_compaction_stats(
            ...
            tokens_saved=max(0, saved_estimate),
        )
