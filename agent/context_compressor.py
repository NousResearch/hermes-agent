...
class ContextCompressor(SummaryDispatchMixin, MicroCompactionMixin, ContextEngine):
    ...
    def _derive_trigger(self, model: str, context_length: int, provider: str) -> tuple[float, float, int]:
        ...
        policy = resolve_compression_policy(
            model, context_length, self.model_thresholds, self.context_window_profiles,
            getattr(self, "_config_threshold_percent", self.threshold_percent),
            getattr(self, "_config_target_ratio", self.summary_target_ratio), provider,
        )
        base_percent = policy.threshold
        effective_percent = self._effective_threshold_percent(
            context_length, base_percent, policy.bypass_small_context_floor,
        )
