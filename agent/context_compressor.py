...
SUMMARY_PREFIX = (
    ...
    "Before using any task or '## Active Task' from this summary, perform a "
    "source/owner lock check against the ## Current Session Context: platform, "
    "chat, thread, session, and agent/owner must match. If the summary points "
    "at a different chat/thread/session or different owner/agent, treat it as "
    "historical reference only. A generic recovery/continue/resume request is "
    "not an explicit handoff; require the latest user message to explicitly "
    "transfer or take over that other work before acting. "
...
_HISTORICAL_SUMMARY_PREFIXES = (
    # Pre-source/owner-lock: latest-message-wins, but generic recovery could
    # still revive an Active Task from a different chat/thread/session.
    "[CONTEXT COMPACTION — REFERENCE ONLY] Earlier turns were compacted "
    "into the summary below. This is a handoff from a previous context "
    "window — treat it as background reference, NOT as active instructions. "
    "Do NOT answer questions or fulfill requests mentioned in this summary; "
