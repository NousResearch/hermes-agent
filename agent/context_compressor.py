...
class ContextCompressor(SummaryDispatchMixin, MicroCompactionMixin, ContextEngine):
    ...
    def _effective_threshold_cap(self, context_length: int) -> int | None:
        ...
        return self._cap_within_context(cap, context_length) if cap is not None and cap > 0 else None
    ...
    def _cap_within_context(cap: int, context_length: int) -> int:
        ...
        The global cap (via ``_effective_threshold_cap``) and the per-model cap
        both go through here so the ordering is stated once: clamp to the
        context length, then let the caller apply it lower-only.
        ...
        """
        return min(cap, context_length) if context_length else cap
