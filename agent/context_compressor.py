...
class ContextCompressor(ContextEngine):
    ...
    def _emit_init_summary_once(self) -> None:
        """Emit the resolved startup summary once, after lazy resolution."""
    ...
    def _resolve_context_length(self) -> int:
        """Resolve and cache the model context window on first access."""
    ...
    @property
    def context_length(self) -> int:  # pyright: ignore[reportIncompatibleVariableOverride]
    ...
    def context_length(self, value: int) -> None:
        ...
        base = getattr(self, "_base_threshold_percent", None)
        if base is not None:
            self.threshold_percent = self._effective_threshold_percent(value, base)
    ...
    @property
    def threshold_tokens(self) -> int:  # pyright: ignore[reportIncompatibleVariableOverride]
    ...
    def __init__(
        ...
        # Context-length resolution is intentionally deferred until first use.
        # The private lazy state is initialized below after the per-engine
        # configuration fields are ready.
        ...
        # The informative startup line reports resolved budgets. Emitting it
        # in __init__ would defeat lazy resolution, so the first property read
        # emits it once when quiet mode is disabled.
    ...
    def _emit_micro_compaction_telemetry(
        ...
        *,
        ...
        try:
            ...
            threshold = self._threshold_tokens if hasattr(self, "_threshold_tokens") else self.threshold_tokens
            context_limit = self._resolved_context_length if hasattr(self, "_resolved_context_length") else self.context_length
            occupancy = None
