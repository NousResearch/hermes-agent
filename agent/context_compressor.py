...
def _lean_recovery_stub(tool_name: str, content_len: int, session_id: str) -> str:
    ...
    # The public tool's query shape receives current_session_id from agent/tool_executor.py. Passing
    # session_id here would switch the tool to its bounded READ shape and ignore the query. Discovery
    # defaults to user/assistant, so demoted tool output needs an explicit tool filter.
    hint = " Recover with session_search(query=..., role_filter='tool', detail='full')" if session_id else ""
    return (
