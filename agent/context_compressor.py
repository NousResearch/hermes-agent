        ...
        for i in range(compress_start, compress_end):
            ...
            for tc in tool_calls:
                ...
                if result_pos is None:
                    # No result found anywhere — pending or external chain
                    has_pending_or_external_chain = True
                    break
                elif result_pos >= compress_end:
                    # Result is outside the compress range — chain must be protected
                    has_pending_or_external_chain = True
                    break

            if has_pending_or_external_chain:
                # Move compress_end past this assistant + all its consecutive tool results
                new_end = i + 1
                while new_end < n and messages[new_end].get("role") == "tool":
                    new_end += 1
                if not self.quiet_mode:
                    logger.info(
                        "Compression: protecting in-flight tool_call at position %d, "
                        "extending tail from %d to %d",
                        i, compress_end, new_end,
                    )
                compress_end = new_end
