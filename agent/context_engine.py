...
class ContextEngine(ABC):
    ...
    _pre_compact_hooks: List[Callable[[List[Dict[str, Any]]], None]] = []
    ...
    def register_pre_compact_hook(
        self,
        fn: Callable[[List[Dict[str, Any]]], None],
    ) -> None:
        """Register a hook that will run before compaction."""
        self._pre_compact_hooks.append(fn)

    def unregister_pre_compact_hook(self, fn: Callable) -> bool:
        """Unregister a pre-compaction hook. Returns True if found."""
        try:
            self._pre_compact_hooks.remove(fn)
            return True
        except ValueError:
            return False

    def pre_compact(self, messages: List[Dict[str, Any]]) -> None:
        """Run all registered pre-compaction hooks on a deep copy of messages.

        Each hook is called with a deep copy of the messages list so that
        hooks can inspect or mutate without affecting the original list.
        A failing hook is logged and skipped so that one broken hook does
        ...
        for hook in self._pre_compact_hooks:
            try:
                ...
                logger.info(
                    "pre_compact hook %s failed: %s", hook.__name__, exc, exc_info=True
                )
            else:
                logger.info("pre_compact hook %s completed", hook.__name__)
