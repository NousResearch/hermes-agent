...
def _save_pre_compaction_snapshot(session_id: str, messages: list) -> None:
    if not session_id:
        return
    snapshot_dir = Path(get_hermes_home()) / "sessions"
    snapshot_dir.mkdir(parents=True, exist_ok=True)
    snapshot_path = snapshot_dir / f"{session_id}_pre_compact.json"
    payload = {
    ...
    logger.info("Pre-compaction snapshot saved to %s", snapshot_path)
...
def compress_context(
    ...
    *,
    ...
    # Optional pre-compression warning
    _warn_before_compress = load_config().get("compression", {}).get("warn_before_compress", False)
    if _warn_before_compress:
        try:
            agent._emit_warning(
                "⚠️ Context is ~85% full. About to compress history. "
                "Critical data should be saved to your database before this happens."
            )
        except Exception:
            pass
    ...
    # Optional pre-compaction snapshot
    _snapshot_enabled = load_config().get("compression", {}).get("snapshot_before_compress", False)
    if _snapshot_enabled:
        try:
            _save_pre_compaction_snapshot(agent.session_id, messages)
        except Exception:
            logger.debug("pre-compaction snapshot failed", exc_info=True)
