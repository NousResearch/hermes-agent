...
class CredentialPool:
    ...
    def entry_for_api_key(self, api_key: Optional[str]) -> Optional[PooledCredential]:
        """Resolve the exact pool entry that supplied a runtime request token."""
        if not api_key:
            return None
        with self._lock:
            return next(
                (entry for entry in self._entries if entry.runtime_api_key == api_key),
                None,
            )
        ...
        *,
        ...
        with self._lock:
            ...
            if api_key_hint:
                ...
                if entry is None:
                    # A request-scoped hint is authoritative. Falling back to
                    # pool.current() here would attach an unknown token's 429 to
                    # an arbitrary healthy sibling. Fail closed on both mutation
                    # and rotation so the caller can continue to provider fallback.
                    logger.warning(
                        "credential pool: failed request token is not present in %s pool; "
                        "skipping status mutation and rotation",
                        self.provider,
                    )
                    return None
            else:
    ...
    def try_refresh_for_api_key(self, api_key_hint: Optional[str]) -> Optional[PooledCredential]:
        """Refresh only the credential that supplied the failed request token."""
        if not api_key_hint:
            return None
        with self._lock:
            entry = next(
            ...
            if entry is None:
                logger.warning(
                    "credential pool: auth-failed request token is not present in %s pool; "
                    "skipping refresh",
                    self.provider,
                )
                return None
