    ...
    def _commit_anthropic_rotation(
        ...
        try:
            ...
            if entry.source == "claude_code":
                ...
                return None
            # The singleton stamps its commit instant; the caller carries it onto
            # the entry so the seeding freshness gate has the same clock on both
            # sides (an unstamped entry makes every comparison None: adopt-always).
    ...
    for source_name, creds in sources:
        if creds and creds.get("accessToken"):
            seed.upsert(source_name, {
                ...
                # The singleton's refresh instant, under the pool's snake_case name,
                # so the freshness gate compares the same clock on both sides.
                # Absent on singletons written before the producer stamped it (and
                # always for claude_code, written by the Claude Code CLI): the gate
                # stays adopt-by-default for those.
