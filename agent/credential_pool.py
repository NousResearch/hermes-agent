...
def _normalize_pool_priorities(provider: str, entries: List[PooledCredential]) -> bool:
    ...
    # ``round_robin`` encodes its rotation *in the priority order* itself
    # (``select()`` moves the used entry to the end and renumbers). Rewriting
    # priorities from static source precedence on every ``load_pool()`` would
    # undo that rotation and pin the pool to one credential, so leave the
    # persisted order alone for this strategy.
