...
_RETRY_DELAY_PATTERNS: Tuple[Tuple[re.Pattern, Callable[[re.Match], float]], ...] = (
    ...
    # Note the shape: "retry" optionally followed by "after", never "retry IN".
    # Gemini's "Please retry in 52.9s" is deliberately NOT matched.  That value
    # is the per-minute RPM window, so on a drained free-tier DAILY quota it
    # bounds nothing: honouring it re-probes a bucket we have already proven
    # empty about once a minute.  Falling through to the status TTL instead
    # gives a rotating pool the full EXHAUSTED_TTL_429_SECONDS hour (a sole
    # credential still gets the deliberately short
    # EXHAUSTED_TTL_SOLE_CREDENTIAL_SECONDS, since benching the only key for an
    # hour is its own problem).  See the "re-probe during cooldown" PR closed
    # for this reason in AGENTS.md.  Widen this only for a window that actually
    # bounds the exhausted quota.
    (
