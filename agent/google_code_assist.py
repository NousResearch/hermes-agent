...
def resolve_project_context(
    ...
    *,
    ...
    # The available tiers ordered from lowest to highest.
    tier_ranks = {
        "": 0,
        FREE_TIER_ID: 1,
        LEGACY_TIER_ID: 2,
    ...
    def get_rank(t: str) -> int:
        return tier_ranks.get(t, 0)

    highest_tier = tier
    for t in info.allowed_tiers:
        if get_rank(t) > get_rank(highest_tier):
            highest_tier = t

    # Attempt to discover an elevated GCP project from Antigravity settings
    auto_project = ""
    try:
        import json
        ...
        if settings_file.exists():
            with open(settings_file) as f:
                sdata = json.load(f)
            for cfg in sdata.get("mcpServers", {}).values():
                p = cfg.get("env", {}).get("GOOGLE_CLOUD_PROJECT")
                if p:
                    auto_project = p
                    break
    except Exception:
    ...
    # If Google exposes free-tier for OAuth (standard-tier) but Ultra limits require a GCP project
    paid_tier = ""
    if isinstance(info.raw, dict):
        pt = info.raw.get("paidTier")
        if isinstance(pt, dict):
            paid_tier = str(pt.get("id") or "")

    if (not highest_tier or (paid_tier and get_rank(paid_tier) > get_rank(highest_tier))) and auto_project:
        logger.info(
            "Auto-discovered elevated tier companion project %s from Antigravity config. "
            "You can set HERMES_GEMINI_PROJECT_ID=<project> to force standard-tier.",
            auto_project
        )
        return ProjectContext(
    ...
    if highest_tier and highest_tier != tier:
        # Onboard to the highest allowed tier
        try:
            onboard_resp = onboard_user(
                access_token,
                tier_id=highest_tier,
                project_id=effective_project or auto_project,
            ...
            logger.warning("Failed to upgrade tier to %s: %s", highest_tier, e)
            if not tier:
                tier = FREE_TIER_ID
            source = "discovered"
    elif not tier:
