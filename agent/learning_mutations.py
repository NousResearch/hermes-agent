...
def _apply_memory_mutation(node_id: str, apply_change, done: str) -> dict[str, Any]:
    ...
    identical entries are two cards — and re-checked against the text the node id fingerprints,
    which is the card the user actually clicked rather than whatever that index names now. When the list shifted under the lock, a single occurrence of that text is
