...
def _parse_memory_id(node_id: str) -> tuple[str, int]:
    ...
    if parts[1] not in _MEMORY_FILES:
        # Provider-contributed nodes are read-only because their storage lives
        # outside the section-delimited memory files this module can rewrite.
        raise ValueError(
...
# ── Materialize a provider session and recall journey context ──────────────
