...
def get_configurable_minimum_context(config_floor: int | None = None) -> int:
    ...
    *config_floor* is ``None`` the default ``MINIMUM_CONTEXT_LENGTH`` is used.
    ...
    if config_floor < _MINIMUM_CONTEXT_FLOOR_HARD_LIMIT:
        logger.warning(
            "compression.minimum_context_floor=%d is below the %dK hard safety "
            "limit; clamping to %d.",
            config_floor,
            _MINIMUM_CONTEXT_FLOOR_HARD_LIMIT // 1000,
