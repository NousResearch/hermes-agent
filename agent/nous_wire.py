...
def apply_pending_wire_switch(agent: Any) -> bool:
    ...
    try:
        ...
        switch_model(
            agent, agent.model, "nous", api_key=getattr(agent, "api_key", "") or "",
            base_url=getattr(agent, "base_url", "") or "", api_mode=want,
            # This changes only the wire protocol, not the selected model. Preserve the exact
            # runtime rather than re-resolving a default as though it were a model selection.
            reasoning_config_override=getattr(agent, "reasoning_config", None),
        )
