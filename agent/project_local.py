...
def trusted_project_mcp_servers(
    start: str | os.PathLike[str] | None = None,
    *,
    project_state: ProjectLocalState | None = None,
) -> dict[str, dict]:
    """Return trusted MCP definitions for an already-captured project state.

    Callers that own a session should pass ``project_state``.  Re-resolving
    from a process-wide working directory at registration time can otherwise
    associate a session with a different project's trusted configuration.
    ``start`` remains for CLI/config callers that do not yet have session
    state.
    """
    state = project_state or resolve_project_local_state(start)
