...
KANBAN_GUIDANCE = (
    ...
    "metadata=..., evidence_paths=[...])`. `summary` is 1–3 human-readable "
    "sentences naming concrete artifacts. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). Downstream "
    "workers read both via their own `kanban_show`. Include an existing absolute "
    "evidence path (`evidence_paths`, `metadata.evidence_refs`, or `artifacts`) "
    "for every done claim. Never put secrets / "
    ...
    "`metadata` are NOT uploaded). Files must exist at completion and count as "
    "evidence when they are existing absolute local paths.\n"
