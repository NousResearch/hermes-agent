...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (mandatory)\n"
            "Before acting, scan the skills below. If a skill matches the task, load it and follow "
            "its instructions. Prefer skills_list(query=...) for routing, then load the smallest "
            "complete unit with skill_view(name, section=...) or skill_view(name, file_path=...). "
            "A skill already loaded in the current conversation remains active: do NOT call "
            "skill_view for the same unchanged skill/file/section again. The tool tracks content "
            "hashes and suppresses unchanged duplicate loads; use force_reload only when you "
            "specifically need the full bytes again. A new session or context-compression child "
            "session may load the skill again. Skills contain specialized APIs, workflows, user "
            "preferences, conventions, and quality standards that should override generic habits. "
            "Load relevant skills even when basic tools could technically complete the task, but "
            "do not load unrelated skills merely for possible context.\n"
            "Whenever the user asks you to configure, set up, install, enable, disable, modify, "
            "or troubleshoot Hermes Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `hermes-agent` skill "
            "first unless it is already present unchanged in this conversation. It has the actual "
            "commands (e.g. `hermes config set …`, `hermes tools`, `hermes setup`) so you don't "
