...
KANBAN_GUIDANCE = (
    ...
    "If your workspace is a git checkout, run `git status`, stage only the "
    "task files, commit with the task id in the message, and push if remote "
    "durability matters before `kanban_complete`. Never complete with dirty "
    "or untracked files. If the work needs human review (most coding tasks), "
    "put changed_files/tests_run/diff_path in a `kanban_comment` first, then "
