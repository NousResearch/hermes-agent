...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Define the exit predicate.** Before editing, state a checkable finish "
    "condition in `kanban_comment` (for example, the reported reproduction no "
    "longer fails and named verification passes). Keep returning to that "
    ...
    "5. **Own relevant, reversible discoveries.** Fix broken skills, verifier "
    "drift, closely related bugs, and tooling failures when they block or "
    "materially improve the assigned task and fit one coherent change. Record "
    "important accepted or discarded attempts in `kanban_comment`. For useful "
    "work outside that boundary, create an issue-linked child with "
    "`kanban_create` and return to the original predicate; do not mix the side "
    "fix into this task's change.\n"
    "6. **Block on genuine ambiguity or missing authority.** If you need a "
    "human decision you cannot infer (missing credentials, UX choice, "
    "paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you.\n"
    "Never merge or deploy changes, rotate secrets, or widen authentication, "
    "payment, or storage behavior unless the task explicitly grants that "
    "authority.\n"
    ...
    "eyes on it.\n"
    "Include the exit predicate and whether it passed, verification and "
    "iterations performed, changes kept or discarded, created side-fix cards, "
    "and the final blocker when it did not pass.\n"
