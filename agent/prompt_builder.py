...
KANBAN_GUIDANCE = (
    ...
    "Skip heartbeats for short tasks. **If your task may run longer than the "
    "configured stale heartbeat gap (default 1 hour), you MUST call "
    "`kanban_heartbeat` before that gap elapses** — the dispatcher reclaims "
    "tasks running past `kanban.dispatch_stale_timeout_seconds` (default 4 "
    "hours) when no heartbeat has arrived within "
    "`kanban.dispatch_stale_heartbeat_gap_seconds` (default 1 hour). A "
