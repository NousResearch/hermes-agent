...
KANBAN_GUIDANCE = (
    ...
    "counting as merged/done (most coding tasks), first check whether this "
    "task already has downstream review / QA / release child cards in "
    "`kanban_show()` parent/child context. If child review gates already "
    "exist (PMO / gated pipeline), write a `review-required` handoff comment "
    "with structured metadata (changed_files / tests_run / diff_path / PR URL), "
    "then `kanban_complete(summary=\"review-required: <one-line summary>\", "
    "metadata=...)` so the existing child review card can promote naturally. "
    "Do NOT block the parent implementation task in that pipeline; it leaves "
    "the parent ready under the active-PR respawn guard and delays downstream "
    "work. If there is NO downstream review child and this same task row is "
    "the manual review gate, then comment the structured metadata first and "
    "end with `kanban_block(reason=\"review-required: <one-line summary>\")` "
