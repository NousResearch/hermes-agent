...
KANBAN_GUIDANCE = (
    ...
    "Exception: use `kanban_block(reason=\"review-required: <one-line "
    "summary>\")` only when this task itself has no explicit downstream "
    "review/test task. If your role or task instructions require creating a "
    "child `tester`, `reviewer`, or other validation card, create it, verify "
    "its returned task id, then call `kanban_complete` on THIS parent. A "
    "blocked parent keeps its child in `todo` forever, so never block a "
    "parent after creating its validation child. Put structured metadata "
    "(changed_files / tests_run / diff_path) in `kanban_complete`; downstream "
    "workers receive it through `kanban_show`.\n"
