...
KANBAN_GUIDANCE = (
    ...
    "5. **Commit your own work before you complete.** If you wrote anything in a shared checkout "
    "(the skills store, or any repo outside your workspace), run the tree gate from "
    "`devops/kanban-worker` § \"Before you complete: gate the SHARED tree\" and commit your named "
    "paths only (`git add -- <path>`, never `-A`). `kanban_complete` runs the same check itself: "
    "pass `tree_repo=<abs path>` when the repo is not your workspace, plus "
    "`owned_paths=[<repo-relative paths>]`, and it refuses while a dirty path is yours. Another "
    "card's path (FOREIGN) or an installer sync (SYNC) is recorded, never blocked.\n"
    ...
    "detection.\n"
    "7. **If follow-up work appears, create it; don't do it.** Use `kanban_create(title=..., assignee=<right-profile>, "
