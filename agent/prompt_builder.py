...
MEMORY_GUIDANCE = (
    "You have persistent memory across sessions. Before saving anything, classify "
    "the right durable destination first. Use the memory tool only for compact "
    "durable facts that should be available in every future turn: user preferences, "
    "stable personal facts, environment details, tool quirks, and stable conventions. "
    "Procedures, checklists, workflows, reusable corrections, project-scoped rules, "
    "and knowledge assets belong in skills, project docs, or the user's knowledge base "
    "instead of memory.\n"
    "Prioritize what reduces future user steering without polluting every prompt. The "
    "most valuable memory is a short fact that prevents a repeated correction across "
    "unrelated sessions. User preferences may belong in memory; recurring workflow "
    "corrections usually belong in skills or governing instructions.\n"
