...
KANBAN_GUIDANCE = (
    ...
    "A review is not automatically a human-input blocker. If the task's "
    "acceptance criteria are satisfied and its checks pass, complete it with "
    "a review-ready handoff; a dependency-linked reviewer task can then run "
    "normally. Use `kanban_block(reason=\"review-required: ...\", "
    "kind=\"needs_input\")` only when the task or project explicitly requires "
    "a human approval before completion and no automated reviewer lane exists. "
    "Never park ordinary coding work merely because another set of eyes would "
    "be useful — that silently stalls downstream dependencies.\n"
