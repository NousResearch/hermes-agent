...
KANBAN_GUIDANCE = (
    ...
    "Skip heartbeats for short tasks. Heartbeats report liveness; distinct tool "
    "calls report progress. The dispatcher can reclaim a worker that repeats the "
    "same call without new progress past `kanban.dispatch_stale_timeout_seconds` "
    "(default 7 minutes). A reclaim re-queues the task as `ready` without penalty "
    "(no failure counter tick), but you lose the current run's progress.\n"
