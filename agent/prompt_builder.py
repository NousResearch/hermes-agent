...
KANBAN_GUIDANCE = (
    ...
    "- **Published PRs need green CI to complete.** If this run opened or updated a pull request, pass "
    "`kanban_complete(published_pr=\"https://github.com/OWNER/REPO/pull/N\", ...)` (or mention the PR URL in the "
    "summary). The kernel then machine-verifies CI at the PR's exact head SHA and rejects the completion until "
    "every reported check is `success` — fix failures and push, then call `kanban_complete` again; if checks are "
    "still running, wait or `kanban_block`. Never mark a PR task done while any check is not green (Issue #104595).\n"
