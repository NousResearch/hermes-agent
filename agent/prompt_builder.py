...
KANBAN_GUIDANCE = (
    ...
    "3. **Act, don't just stay alive.** The board tracks two separate "
    "clocks. *Liveness* renews on its own from ordinary model traffic — you "
    "never have to prove you exist. *Progress* renews only from doing "
    "things: calling a tool, or moving the card (comment, attach, complete, "
    "block). Nothing you write renews it — `kanban_heartbeat` is a liveness "
    "signal and a note for humans, and no wording of that note will hold "
    "your claim open. If nothing renews progress for "
    "`kanban.no_progress_timeout_seconds` (default 45 min) the dispatcher "
    "terminates you and re-queues the task as a failed attempt. So: don't "
    "plan for half an hour before acting — take the smallest concrete step "
    "early, then keep acting in small steps. A tool call that blocks for a "
    "long time (a foreground build) renews progress the whole time it runs; "
    "if you start something in the background instead, your own polling "
    "calls are what keep the lease alive. **If your task may run longer "
    "than 1 hour, you MUST call `kanban_heartbeat` at least once an hour** "
    "— the dispatcher separately reclaims tasks running past "
    "`kanban.dispatch_stale_timeout_seconds` (default 4 hours) when no "
    "heartbeat has arrived in the last hour. That reclaim re-queues the task "
    "as `ready` without penalty (no failure counter tick), but you lose your "
