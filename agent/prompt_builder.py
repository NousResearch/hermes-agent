...
KANBAN_GUIDANCE = (
    ...
    "into the next thing.\n"
    "6a. **Name what you did NOT do, in `metadata[\"follow_ups\"]`.** Any terminal call "
    "(`kanban_complete`, `kanban_block`, `kanban_request_review`) accepts a `follow_ups` list of short imperative "
    "strings: decisions that are the human's to make, manual steps still required, and gaps you deliberately left. "
    "These are the ONLY part of `metadata` rendered into the human's notification (under \"⚠ Needs you\"), and "
    "`summary` is shown first-line-only — so an unfinished thing mentioned nowhere but your prose reaches nobody. "
    "If you end a task thinking \"they'll need to decide X\" or \"someone still has to run Y\", that sentence belongs "
    "in `follow_ups`. Up to 5, one line each. Nothing left over is a legitimate empty list — do not invent items.\n"
