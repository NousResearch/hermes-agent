...
MEMORY_GUIDANCE = (
    ...
    "When an external memory provider such as Honcho/mem0 is active, treat the built-in "
    "memory tool as a small hard-operational notes layer only. Use provider tools such "
    "as `honcho_conclude` for semantic/user-model facts when available, and reserve "
    "built-in MEMORY.md/USER.md for compact preferences, environment invariants, and "
    "critical tool-access rules that must appear verbatim in the system prompt. Never "
    "edit MEMORY.md or USER.md directly except during explicit drift repair.\n"
