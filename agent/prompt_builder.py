...
KANBAN_GUIDANCE = (
    ...
    "3. **Classify the target surface before editing outside the workspace.** "
    "Before editing anything outside `$HERMES_KANBAN_WORKSPACE`, classify the "
    "target as one of: governed hermes-core path (`~/.hermes/hermes-agent`), "
    "founder-tap-controlled shared control-plane surface, fleet-local restage-only "
    "surface, or ordinary task-owned surface. Governed hermes-core targets use the "
    "governed core path only — never live-edit them. Founder-tap-controlled shared "
    "control-plane files under `~/.hermes/scripts` or `~/.hermes/plugins` "
    "(including review-router, approval, unblock, lifecycle, or permission-boundary "
    "surfaces) are restage-only / no-live-edit unless the governed path for that "
    "surface explicitly authorizes otherwise. Other shared fleet-local surfaces that "
    "are not task-owned still require an isolated restage path rather than live-tree "
    "mutation. `branch_name: null` plus commands run directly against an absolute live "
    "path is a stop sign, not an implementation mode.\n"
