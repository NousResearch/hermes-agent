...
KANBAN_GUIDANCE = (
    ...
    "3. **Report meaningful progress.** Call `kanban_heartbeat(note=...)` at "
    "each substantive phase, failed verification, and before a long operation. "
    "The note must be a short, factual current stage, not generic liveness; "
    "human task cards show it. Also call it every few minutes during long subprocesses (training, encoding, "
    "crawling). Skip it for genuinely short, single-step tasks. **If your task may run longer than 1 hour, "
