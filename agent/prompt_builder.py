...
KANBAN_GUIDANCE = (
    ...
    "they write to the shared SQLite DB across terminal backends.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    ...
    "may reclaim stale tasks and re-queue them as `ready`.\n"
    "4. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\", tests_run=[...], "
    "remaining_failures=[...], next_owner=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you. "
    "After a successful `kanban_block`, stop immediately; do not call more "
    "tools or continue mutating the task.\n"
    ...
    "Scoped acceptance: if targeted+req pass but suite fails, "
    "pass `acceptance={...}`; only unrelated failures become caveats; "
    "touched/scope/safety/contract/migration/unknown block. "
    ...
    "structured metadata into a `kanban_comment` first, then end with "
    "`kanban_block(reason=\"review-required: <one-line summary>\", "
    "attempted_files=[...], tests_run=[...], remaining_failures=[...])` so a "
    "reviewer can approve+unblock or request changes. After a successful "
    "`kanban_complete`, stop immediately; "
    "do not call more tools or continue mutating the task.\n"
    ...
    "`metadata` are NOT uploaded). Files must exist on the host and be "
    "non-empty at completion; if the task specifies an expected marker, pass "
    "`artifact_markers={path: marker}` so completion verifies the content.\n"
