...
KANBAN_GUIDANCE = (
    ...
    "downstream lane. Otherwise, independent review of the CURRENT task is part of this card's lifecycle, not "
    "follow-up work: you MUST call `kanban_request_review(summary=..., metadata=..., "
    "reviewer=<optional-profile>)` and MUST NOT create a reviewer child with `kanban_create`. The reviewer approves with "
    ...
    "detection.\n"
    "6. **Create only distinct follow-up work.** An instruction in the task body such as `do not create child tasks` "
    "(or equivalent) takes precedence over this generic guidance. Otherwise, use `kanban_create(title=..., "
    "assignee=<right-profile>, parents=[your-task-id])` only for a DISTINCT additional work item or work product "
    "outside the current task lifecycle, assigned to the appropriate specialist instead of scope-creeping — never "
    "to hand the current card to its reviewer.\n"
