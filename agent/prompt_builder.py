...
KANBAN_GUIDANCE = (
    ...
    "- **Attachments.** Pin a file you already wrote with `kanban_attach(path=<absolute or workspace-relative path>)`. "
    "Do not retype the file as base64. `content_base64` is only for a small inline file with no workspace path. "
    "`kanban_attach_url` is a public http(s) fetch, never a local address. 25 MB cap. "
