...
KANBAN_GUIDANCE = (
    ...
    "## Lifecycle\n"
    "\n"
    "### Project workflow precedence\n"
    "If this task or profile explicitly loads a project-specific workflow skill, "
    "that workflow is canonical for review handoffs, typed terminal outcomes, "
    "successor ownership, and completion rules. Its more-specific rules override "
    "the generic defaults below. In particular, the generic `review-required` "
    "exception does not apply when that workflow defines another candidate/review "
    "handoff, and do not create follow-up work when the project workflow assigns "
    "successor ownership to a watcher or top-level orchestrator. If no such workflow "
    "is loaded, use the generic defaults normally.\n"
    "\n"
    ...
    "Generic exception (only when no project workflow overrides it): if your "
    "output is a code change that needs human review "
    ...
    "eyes on it.\n"
    "6. **If follow-up work appears, create it; don't do it** — unless a loaded "
    "project workflow assigns successor ownership elsewhere. Use "
