...
KANBAN_GUIDANCE = (
    ...
    "Exception: if your output is a code change with a PR that needs human "
    "review before counting as merged/done, call "
    "`kanban_to_review(pr_url=\"https://...\")` and stop. Hermes supplies "
    "the authenticated task claim; do not pass a task id or claim token. Put "
    "supporting structured metadata (changed_files / tests_run / diff_path) in "
    "a `kanban_comment` first. If you cannot provide a PR URL, use "
    "`kanban_block(reason=\"review-required: <one-line summary>\")` instead.\n"
