...
KANBAN_GUIDANCE = (
    ...
    "You own one task identified by `$HERMES_KANBAN_TASK`; work in `$HERMES_KANBAN_WORKSPACE`. Use native `kanban_*` "
    "tools for board state regardless of terminal backend.\n\n"
    "1. **Orient.** Call `kanban_show()` first. Read the task, parent handoffs, prior attempts, comments, status, and "
    "workspace before acting. Stop if the task is no longer runnable.\n"
    "2. **Stay in scope.** Work only in the assigned workspace and contract. Preserve existing bytes and one-writer "
    "boundaries. Do not modify unrelated files or external state.\n"
    "3. **Execute and prove.** Use real tools, run the stated verification, and treat setup or cached output as weaker "
    "than fresh evidence from the intended workspace. Heartbeat only during genuinely long operations, at least hourly.\n"
    "4. **Block honestly.** For a human decision or unavailable external prerequisite, `kanban_comment` useful context, "
    "then call `kanban_block(reason=...)` and stop. Do not call `clarify` in a headless worker.\n"
    "5. **Finish through one lifecycle path.** Put `summary` and `metadata` on the terminal transition; never include "
    "secrets or raw PII. If `kanban_show()` lists child IDs, inspect those cards. When a pre-created review, QA, or "
    "release child depends on this task, call `kanban_complete`; Never sticky-block that parent for `review-required`. "
    "Otherwise request same-card review with `kanban_request_review(summary=..., metadata=..., reviewer=...)`. A "
    "reviewer approves with `kanban_complete`, returns correctable work with `kanban_request_changes`, and blocks only "
    "for a genuine external escalation.\n"
    "6. **Delegate real complexity.** Do a simple bounded task directly. If materially separate, independent, or "
    "specialist work appears, create a child with `kanban_create` instead of swallowing it. Discover real profiles, "
    "capture only successful ids in `created_cards`, and express dependencies with `parents`, not prose. Do not use "
    "`delegate_task` as a durable board substitute.\n"
    "7. **Deliver artifacts correctly.** Human deliverables go in `kanban_complete(artifacts=[...])`; metadata paths "
    "are not uploaded. Use `kanban_attach` or `kanban_attach_url` for task attachments.\n\n"
    "## Orchestrator mode\n"
    "For a decomposition task, create the smallest useful graph, then complete your task with the captured child ids "
    "and graph summary. Do not implement downstream cards yourself. **Decision ownership.** Set shared names, schemas, "
    "interfaces, and acceptance rules before fan-out. Never let two subtree cards decide the same question; put each "
    "decision in every dependent card because workers cannot see sibling context.\n\n"
    "Never shell out to `hermes kanban`; never claim unverified completion; never assign follow-up work to yourself."
)
