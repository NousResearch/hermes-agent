...
MEMORY_GUIDANCE = (
    ...
    "tool (MEMORY/USER): user preferences, environment details, tool quirks, and "
    "stable conventions — facts about who the user is and how they work that will "
    "still be true later. Memory is injected into every turn, so keep it compact "
    "and focused on those stable facts.\n"
    "Route by durability: stable user, profile, and environment facts → MEMORY/USER. "
    "Reusable executable workflows and procedure corrections → skill_manage "
    "(create or patch). Completed task progress and session history → session_search.\n"
    "Prioritize what reduces future user steering — the most valuable memory is one "
    "that prevents the user from having to correct or remind you again. "
    "User preferences and recurring preference corrections matter more than procedural task details.\n"
    "Autonomous routing: prefer session_search for task progress, session outcomes, "
    "completed-work logs, or temporary TODO state rather than writing those to memory. "
    "Honor explicit user requests to remember something — MemoryStore does not "
    "reject user-authored writes. Set user_requested=true when honoring an "
    "explicit remember-this request, and suggest relocating the fact to session "
    "history if it will go stale. Prefer not to invent a skill or a memory entry "
    "for a one-off completed task, or to record PR numbers, issue numbers, commit "
    "SHAs, 'fixed bug X', 'submitted PR Y', 'Phase N done', file counts, or any "
    "artifact that will be stale in 7 days.\n"
...
USER_PROFILE_GUIDANCE = (
    ...
    "environment details, and communication style. The profile is injected into every turn, "
...
SKILLS_GUIDANCE = (
    ...
    "skill_manage (create or patch). Durable user, profile, and environment "
    "facts belong in the memory tool (MEMORY/USER). Completed task progress "
    "and session history are recovered with session_search. Route those "
    "autonomously via session_search rather than inventing a skill or memory "
    "note; honor explicit user requests to store something.\n"
