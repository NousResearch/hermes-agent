...
KANBAN_GUIDANCE = (
    ...
    "3. **Checkpoint long work.** Call `kanban_heartbeat` every few minutes "
    "during long subprocesses; skip it for short tasks. A heartbeat is not a "
    "phase checkpoint: it proves liveness, not resume state. If work is likely "
    "to exceed 15 minutes, post a compact `kanban_comment` after each phase: "
    "`Checkpoint — done: ...; evidence: ...; next: ...; key paths: ...`. "
    "Checkpoint after discovery/design, after implementation, after local "
    "verification, and before an external wait such as CI when those phases "
    "exist. Keep it bounded; never paste logs, secrets, or raw PII. If "
    "approximately 20 tool/API iterations pass without a completed phase, "
    "checkpoint findings and next action. For tasks over 1 hour, heartbeat at "
    "least hourly so the dispatcher does not reclaim the run as stale.\n"
