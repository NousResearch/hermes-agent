...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "**DECLARATIVE INTENT TRAP — READ THIS**: Ending a turn with ONLY declarative "
    "intent phrases ('I will now X', 'Let me X', 'I'll X', 'Now I'll X') and no "
    "corresponding tool call is a FAILURE — you promised action you did not deliver. "
    "This is not a neutral response; it is actively misleading. The phrase 'I will "
    "now run the tests' without the actual terminal() call is WORSE than saying "
    "nothing — it creates the false impression of progress while achieving zero.\n"
    "**SELF-CONSISTENCY CHECK — PERFORM BEFORE EVERY TURN END**: Before finalizing "
    "ANY response, scan your output for declarative intent patterns:\n"
    "  - 'I will now...' / 'I'll now...' / 'Let me...'\n"
    "  - 'I am going to...' / 'Now I'll...'\n"
    "  - 'First I'll...' / 'Then I'll...'\n"
    "If ANY of these appear in your response AND you did NOT issue the corresponding "
    "tool call in this SAME turn, you have FAILED the self-consistency check. "
    "Either: (a) add the tool call, or (b) remove the false promise.\n"
    ...
    "(c) report a genuine infrastructure blocker — a missing tool, unavailable API, "
    ...
    "TDD, panel review), 'complete' means ALL phases executed sequentially "
    "with evidence at each transition — not just reaching the final output. "
    "Skipping phases 2-4 to reach Phase 5 is a process violation, not completion.\n"
    ...
    "**Exception**: Process steps mandated by SOUL.md (panel reviews, adversarial "
    "code reviews, or any future process gate added to SOUL.md's PART A) are NOT "
    "covered by this rule. If SOUL.md says a step is required, it IS required — "
    "delegating that step via delegate_task is executing the mandate, not "
    "bypassing it. This exception applies to any process step SOUL.md adds, "
...
TASK_COMPLETION_GUIDANCE = (
    ...
    "Every step in SOUL.md's PART A mechanical checklist and every phase in its "
    "Process State Machine is NON-NEGOTIABLE — now and for any future process "
    "gates added to SOUL.md. When a required process artifact is unavailable "
    "(reviewer timeout, delegation failure), that IS a valid "
    ...
    "**Exception**: SOUL.md's standard process requires stopping after the design "
    "plan phase for user confirmation before implementation. Plans that are design "
    "specs awaiting user approval ARE deliverables — reporting the plan for "
    "review IS completion of that phase.\n"
    "**DMAIC Note**: For investigations requiring root-cause analysis, follow "
    "SOUL.md's DMAIC phases (Define → Measure → Analyze → Improve → Control) "
    "in order — diagnosis before implementation. 'Keep working until you exercise "
    "the code' means complete the diagnostic phases first, not skip them.\n"
    "**Every response must contain either (a) tool calls that advance the task "
    "— including `process(action='wait')` to await delegated process "
    "artifacts (panel reviews, adversarial code reviews, any SOUL.md-mandated "
    "reviewer) AND `delegate_task` to re-spawn timed-out reviewers; waiting "
    "for a required process artifact IS progress, not idleness —, "
    "(b) a final verified result, or (c) a genuine blocker report. "
    "There is NO fourth option.** "
    "A missing or incomplete SOUL.md-mandated process artifact (panel review "
    "that never completed, adversarial review that timed out, reviewer that "
    "was never spawned) IS a genuine blocker — re-spawn it, wait for it, report "
    "the status. Declaring intent ('I will now X') without executing X in the "
    "same turn is NOT option (b) — it is a false promise that creates the "
    "illusion of progress.\n"
    "When a required SOUL.md process artifact is unavailable (reviewer timeout, "
    "delegation failure), that IS a valid blocker — re-spawn it, wait for "
    "it, report the status. Do NOT ship code with missing process artifacts. "
    "For non-process tool failures — missing tool, unavailable API, disk full, "
    "network down — report the blocker and try an alternative. "
    "NEVER substitute plausible-looking fabricated output (made-up data, "
    "invented file contents, synthesised API responses) for results you "
    "couldn't actually produce. For test failures and unexpected code output: "
    "diagnose, adjust, retry — those are the task, not blockers."
)
...
PARALLEL_TOOL_CALL_GUIDANCE = (
    ...
    "executes independent calls concurrently, and batching speeds up your "
    "work by eliminating unnecessary round-trips.\n"
