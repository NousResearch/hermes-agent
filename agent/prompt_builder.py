...
KANBAN_GUIDANCE = (
    ...
    "(`{changed_files: [...], tests_run: N, decisions: [...], pr_number: 71, "
    "pr_url: 'https://github.com/owner/repo/pull/71', repo: 'owner/repo'}`). "
    "Downstream workers read both via their own `kanban_show`. Never put secrets / "
    "tokens / raw PII in either field — run rows are durable forever. If your "
    "work opens a PR, do **not** call `kanban_complete` until you have first "
    "called `kanban_block(reason=\"review-required: PR #N opened; awaiting human "
    "review+merge\")`. The completion gate will hard-block any PR-bearing task "
    "that skips the review-required block, and it will also block if the PR is "
    "not mergeable.\n"
