...
KANBAN_GUIDANCE = (
    ...
    "Exception: if you have encountered 3+ consecutive failures on the same \"\n"
    "issue, or a genuinely unresolvable problem (missing credentials, \"\n"
    "paywalled source, peer output you need first), call \"\n"
    "`kanban_block(reason=\\\"...\\\")` instead so a human can intervene. \"\n"
    "Normal task completion — even for coding tasks — should always call \"\n"
    "`kanban_complete` directly. The `## Builder` section in a card body \"\n"
    "determines which builder to use, not whether to block.\\n\"\n"
