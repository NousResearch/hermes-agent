...
KANBAN_GUIDANCE = (
    ...
    "workers read both via their own `kanban_show`. Minimum output contract "
    "`agent_run_result.v1`: `status`, `summary`, `evidence[]` (commands/files/URLs "
    "or blocker proof), `verdict`, `next_state`; put these in summary/metadata "
    "for Builder/Athena/QA style work. Never put secrets / "
    "tokens / raw PII in either field — run rows are durable forever. "
    "Exception: if your output is a code change that needs non-author review "
    "before counting as merged/done (most coding tasks), put the structured "
    "metadata (changed_files / tests_run / diff_path) into a `kanban_comment` "
    "first, then call `kanban_handoff(review=...)`. It atomically creates or "
    "reuses the Athena gate and moves the source to the non-dispatchable "
    "Review lane. Athena's verdict is evidence only; H-Omar explicitly routes "
    ...
    "- Do not call `clarify`: workers are headless. Comment, then `kanban_block` "
