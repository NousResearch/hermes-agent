...
KANBAN_GUIDANCE = (
    ...
    "before counting as merged/done (most coding tasks), end with "
    "`kanban_review(summary=..., metadata=...)` instead of `kanban_complete`. "
    "Use `metadata` for structured facts such as changed_files / tests_run / "
    "diff_path. A reviewer can approve with `kanban_complete` or request "
    "changes by commenting and unblocking the task back to ready. Reviewing-"
    "then-completing is more honest than auto-completing work that still "
    "needs eyes on it.\n"
