...
MEMORY_GUIDANCE = (
    ...
    "Write memories as declarative facts with a compact provenance tail,\n"
    "not as bare facts or instructions to yourself.\n"
    "\n"
    "Every durable entry should carry a short [why: ...] tail naming the\n"
    "failure or observation it prevents, plus the date when known:\n"
    "\n"
    "  <fact>  [why: <concrete failure / observation> · <YYYY-MM-DD>]\n"
    "\n"
    "Examples:\n"
    "  \"resolver scans env vars without base_url\"\n"
    "    [why: hermetic conftest blanks env vars → RuntimeError · 2026-08-17]\n"
    "  \"always pass base_url in tests\"\n"
    "    [why: resolver.py:8 scans env vars without base_url; conftest\n"
    "     blanks them → RuntimeError · 2026-08-17]\n"
    "\n"
    "Reject vacuous tails at write time — before committing the entry:\n"
    "  ALLOW: [why: resolver.py:8 scans env vars without base_url →\n"
    "          RuntimeError · 2026-08-17]\n"
    "  REJECT: [why: useful], [why: best practice], [why: see above],\n"
    "          [why: to be safe], any tail that does not name a concrete\n"
    "          failure or observation. Reword the entry with a real premise\n"
    "          or don't write it.\n"
    "\n"
    "Where the date comes from:\n"
    "  dated events (bug report, incident, session, PR) → YYYY-MM-DD\n"
    "  undated facts (always-true convention, stable env detail) → omit\n"
    "    the date rather than invent one\n"
    "  never backfill a date you can't anchor — the tail's value is the\n"
    "    concrete premise, not the timestamp\n"
    "\n"
    "This tail is the provenance signal that makes later deletion\n"
    "decisions evidence-based (issue #86022, Phase 2). It is NOT a\n"
    "changelog entry for rollback — that's a separate concern\n"
    "(issue #81015, reason-attached change log for mutations).\n"
    "\n"
    "The tail lives in the existing §-delimited ledger format. It does\n"
    "not require a schema change, a new file layout, or front matter.\n"
    "It is the cheapest phase of the proposal and the only one in\n"
    "scope for this PR.\n"
    "\n"
    "When memory is full and you need to add a rationale-attached entry,\n"
    "use the batch 'operations' escape hatch (memory(action='apply_batch',\n"
    "operations=[...])) to remove or replace stale entries and add the new\n"
    "entry in a single call — the budget is checked against the final state,\n"
    "so you don't need a multi-turn consolidate-then-add dance (see #79414\n"
    "for docs on the operations escape hatch).\n"
)
