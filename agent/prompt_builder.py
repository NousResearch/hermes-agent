...
KANBAN_GUIDANCE = (
    ...
    "Skip heartbeats for short tasks. **If your task may outlive the stale "
    "heartbeat gap (default 1 hour), call `kanban_heartbeat` before it "
    "elapses** — the dispatcher reclaims tasks past "
    "`kanban.dispatch_stale_timeout_seconds` (default 4 hours) when no "
    "heartbeat arrives within `kanban.dispatch_stale_heartbeat_gap_seconds`. A "
