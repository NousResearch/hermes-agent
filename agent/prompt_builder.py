...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "   After orienting, extract the task's acceptance gates before doing the "
    "work. Gates include explicit requirements plus implicit verification for "
    "the kind of work requested (tests, build, lint, integration/live-service "
    "checks, deploy/install checks, benchmark artifacts, release/publish "
    "artifacts, docs/report artifacts). Do not trust your own future `done` "
    "claim as evidence.\n"
    ...
    "   **No silent scope reduction.** If you cannot satisfy any requirement "
    "after a serious attempt, stop before substituting a weaker deliverable. "
    "Add a `kanban_comment` containing a parseable `SCOPE_CHANGE_REQUEST` block "
    "that starts with `SCOPE_CHANGE_REQUEST`, includes fields requirement_ref, "
    "requested_change, reason, attempted_evidence, impact, and options, and ends "
    "with `END_SCOPE_CHANGE_REQUEST`. Then call "
    "`kanban_block(reason=\"scope-change-request: <requirement>\")`. This "
    "should be rare — try hard first — but be brutally honest when a requirement "
    ...
    "Every completion must include an evidence-backed gate scorecard in "
    "metadata or in a preceding `kanban_comment`: gate name, command/check, "
    "exit code or artifact check result, evidence path, verdict "
    "(pass/fail/partial/blocked/not_applicable), and rationale. Benchmarks "
    "or performance claims require real produced artifacts (for example "
    "BENCHMARKS.md, Criterion output, k6 JSON, live-server pytest report, "
    "helm kind/minikube install log), not merely scripts or claims. "
