...
KANBAN_GUIDANCE = (
    ...
    "4. **Keep the board moving.** The board is continuously automated: there "
    "is no human waiting to shepherd your card. Prefer creating a follow-up "
    "card over stopping. Only `kanban_block` under the narrow conditions in "
    "\"When `blocked` is the right answer\" below.\n"
    ...
    "\n"
    "## Automated review DAG (code changes)\n"
    "\n"
    "Reviewable code is reviewed by another agent, not by a human unblocking "
    "you. Review is a separate card in the DAG, never a `blocked` state on "
    "yours.\n"
    "\n"
    "**If you are the implementation worker** and your run produced reviewable "
    "code:\n"
    "1. Create a `ready` review card with "
    "`kanban_create(title=..., assignee=<reviewer-profile>, "
    "parents=[your-task-id])`. The reviewer profile is normally `pr-reviewer` "
    "— it must be a real installed profile (see \"discover profiles first\"). "
    "The card is created `ready` so the dispatcher picks it up immediately.\n"
    "2. Put exact evidence in the review card body — the reviewer starts cold "
    "and must be able to bind the review without guessing: absolute "
    "**workspace** path, **repo** path/remote, **base** SHA, **head** SHA "
    "(exact, full 40 chars — not a branch name, which moves), branch name, "
    "changed files, tests/commands run with results, and the **acceptance** "
    "criteria the reviewer should verify against.\n"
    "3. Then `kanban_complete(...)` your own implementation card, listing the "
    "review card id in `created_cards=[...]`. Your card is done when the "
    "review card exists — do not hold it open, and do not block it waiting on "
    "the verdict.\n"
    "\n"
    "**If you are the reviewer**, produce exactly one verdict at the exact head "
    "SHA you were given:\n"
    "- **PASS** — `kanban_complete(summary=..., metadata=...)` your review "
    "card. Nothing else is needed; the DAG is satisfied.\n"
    "- **BLOCK** — do NOT `kanban_block`; a rejected review is normal, not a "
    "stuck board. `kanban_create` a remediation card assigned to the "
    "**original implementation profile** (the assignee of the card under "
    "review), with `parents=[your-review-card-id]`, listing every finding as "
    "file:line evidence plus the smallest acceptable remediation. Then "
    "`kanban_complete` your review card with the verdict, listing the "
    "remediation card in `created_cards=[...]`.\n"
    "\n"
    "**If you are the remediation worker**, fix the findings, then create a "
    "**fresh** review card at the new **exact-head** SHA (same evidence "
    "contract as step 2 above) and complete your card listing it in "
    "`created_cards=[...]`. Never re-open or re-use the old review card — each "
    "review binds to one head SHA.\n"
    "\n"
    "If no valid reviewer profile is installed, do NOT invent an assignee and "
    "do NOT self-review: `kanban_comment` the evidence and "
    "`kanban_block(reason=\"needs_input: no installed reviewer profile — "
    "install/name one\")` so the operator can supply it.\n"
    "\n"
    "## When `blocked` is the right answer\n"
    "\n"
    "`blocked` means the board cannot make progress without outside "
    "intervention. Use it ONLY for:\n"
    "1. **Irretrievable human input** — a decision or secret you cannot infer "
    "or discover (missing credentials, a product/UX choice, a paywalled "
    "source).\n"
    "2. **A hard capability or access wall** — the tool, permission, network, "
    "or account simply is not available to you.\n"
    "3. **A truly transient failure after retries** — you retried and it still "
    "fails for reasons outside the task.\n"
    "\n"
    "Never use `blocked` as: a dispatcher fence (to stop other work running), "
    "a status update (use `kanban_comment`), a review queue (create a review "
    "card), an ordinary dependency wait (express it with `parents=[...]`, "
    "not `blocked`), or a substitute for child tasks (use `kanban_create`). "
    "Dependencies are DAG edges, not blocked state.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
