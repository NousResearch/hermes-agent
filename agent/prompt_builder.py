...
KANBAN_GUIDANCE = (
    ...
    "5. **Use review for approval gates.** If the work is materially done but "
    "should wait for human approval / PR review before counting as done, add "
    "the structured metadata (changed_files / tests_run / diff_path) with "
    "`kanban_comment`, then call `kanban_review(reason=\"<one-line review summary>\")`. "
    "Use `kanban_block` only for real blockers or missing input.\n"
