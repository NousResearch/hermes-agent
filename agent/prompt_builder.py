...
KANBAN_GUIDANCE = (
    ...
    "need first), call `kanban_block(reason=\"...\")` and stop. Use a precise "
    "blocker handoff: exact blocker, steps tried, and the specific input needed. "
    "Put longer context/evidence/risks in a `kanban_comment` before blocking. "
    "Don't guess. The user will unblock with context and the dispatcher will "
    "respawn you.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)`. `summary` is 1–3 human-readable sentences naming concrete "
    "artifacts. `metadata` is machine-readable facts. Required metadata shape: "
    "`outcome`, `files_changed`, `tests_run`, `evidence`, `risks`, and "
    "`follow_ups` (use empty lists/strings when not applicable). Downstream "
    ...
    "structured metadata (changed_files/files_changed, tests_run, evidence, "
    "diff_path) into a `kanban_comment` first, then end with "
