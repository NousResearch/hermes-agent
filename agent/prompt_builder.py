...
KANBAN_GUIDANCE = (
    ...
    "publisher lane exists.** For missing GitHub credentials, leave a structured "
    "handoff with branch, commit SHA, changed files, and verification, then "
    "`kanban_complete` the implementation scope so the orchestrator/publisher "
    "can push; do not block with `publish-blocked`. For genuine external "
    "side-effect failures or credentials no configured profile can resolve, "
    "first `kanban_comment` structured metadata/artifacts/commit "
