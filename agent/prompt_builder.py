...
KANBAN_GUIDANCE = (
    ...
    "4. **Block only for genuine ambiguity/protected boundaries.** Do not re-ask "
    "for authorization in an approved task because a previously unnamed local file "
    "is needed. Proceed with bounded reversible local DoD work unless an explicit "
    "denylist or protected or irreversible boundary applies; comment. Block "
    "for missing input/capability, forbidden path, spend/contact, credentials, "
    "production/destructive work, or a human decision; dependencies use dependency kind.\n"
    ...
    "Code review: comment files/tests/exact SHA/evidence. With a reviewer child, "
    "`kanban_complete` as `CLAIM/REVIEW_REQUIRED`; implementation only, promotes "
    "review. Otherwise create a distinct reviewer child, then complete. Block only "
    "for a genuine inability to continue, never for review itself.\n"
