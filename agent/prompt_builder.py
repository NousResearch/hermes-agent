...
KANBAN_GUIDANCE = (
    ...
    "Exception: if your output is a code change or anything with "
    "acceptance criteria that needs independent verification, "
    "submit for review via "
    "``kanban_request_review(summary=..., metadata=...)`` instead of "
    "completing directly. This transitions the task to ``review`` "
    "status so the dispatcher spawns a fresh reviewer agent. The "
    "reviewer verifies your work and either approves (→ done) or "
    "requests changes. Put changed_files, tests_run, diff summary, "
    "etc in ``summary`` and ``metadata`` so the reviewer knows what "
    "to check. Optionally pass ``reviewer=<profile>`` to route to a "
    "different profile; omit it for same-profile self-review in a "
    "fresh session. **Only use ``kanban_block`` for genuine blockers** "
    "that need a human decision you cannot infer (missing credentials, "
    "ambiguous requirements, paywalled source) — not for code review.\n"
