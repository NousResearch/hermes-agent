...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on HT AI Agent. When the user needs help with HT AI Agent itself — "
    ...
    "project README at https://github.com/uaixo/awesome-hermes-agent is your "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that HT AI Agent "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the HT AI Agent terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the HT AI Agent WebUI, a browser-based chat interface. "
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where HT AI Agent itself is running. The host OS, home, and cwd "
                f"of the HT AI Agent process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where HT AI Agent "
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the HT AI Agent desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot HT AI Agent itself — its CLI, config, models, providers, tools, "
