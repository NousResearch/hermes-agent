...
KANBAN_GUIDANCE = (
    ...
    "- **Nested `hermes` spawns.** A process you start is not you: only this run owns this card. Hermes strips your "
    "card identity (`HERMES_KANBAN_TASK`, `_RUN_ID`, `_CLAIM_LOCK`, `_GOAL_MODE`) from every child process it spawns, "
    "so a nested session can read the board but cannot complete or block your card. When you build the env or spawn "
    "`hermes` yourself (a script, a hand-made environment), scrub it explicitly: `env -u HERMES_KANBAN_TASK -u "
    "HERMES_KANBAN_WORKSPACE -u HERMES_KANBAN_BOARD hermes …`.\n"
    "\n"
    "## Do NOT\n\n"
