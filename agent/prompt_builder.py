...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "round-trip. For exact labels, digits, totals, dates, or identifiers "
        "on Windows, pass `ocr=true`; local OCR returns text and word boxes. "
        "Cross-check critical values against the screenshot and AX labels — "
        "OCR and vision are evidence, never perfect ground truth.\n"
        "5. To request a dedicated background app instance, use `action='launch_app', "
        "app='...', creates_new_application_instance=true`, then "
        "`action='list_windows'` and capture the returned exact `pid` + "
        "`window_id`. Check `new_process_confirmed`: Windows apps may reuse an "
        "existing process. Exact window targeting prevents cross-session mixups, "
        "but only a separately configured VM/sandbox is a security boundary.\n\n"
