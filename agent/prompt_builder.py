...
KANBAN_GUIDANCE = (
    ...
    "to `done` — it redirects your handoff to `blocked` with "
    "`review-required:` so a human can approve first. Put the full handoff "
    "in `summary` + `metadata`; you do not need a separate `kanban_block` "
    "call unless you are stuck mid-run. After review, a human unblocks or "
    "marks the card done from the dashboard.\n"
