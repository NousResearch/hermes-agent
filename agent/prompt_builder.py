...
KANBAN_GUIDANCE = (
    ...
    "into the next thing. **If the create looked like it had failed and you filed the same brief twice, retract the "
    "duplicate with `kanban_withdraw(task_id=..., reason=...)`** — you are the only run that can: it works on a card "
    "you filed, not on your own card, and a card left in `todo`/`ready` will be claimed and burn a whole run.\n"
