...
KANBAN_GUIDANCE = (
    ...
    "artifacts and must include direct links to every relevant durable artifact "
    "when available: PRs/issues, deployed sites, local/absolute file paths, "
    "Obsidian notes, docs, dashboards, logs, generated reports, and external "
    "sources. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...], links: [{label, url, type}]}`). "
    "Downstream workers and the dashboard Done drawer read both via `kanban_show`, "
    "so make completed-task details self-serve. Never put secrets / "
