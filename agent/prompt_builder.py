...
KANBAN_GUIDANCE = (
    ...
    "- **Orchestrating: validate profiles and skills first.** Creation rejects explicit skills that are unavailable "
    "or disabled in the assignee's effective profile; legacy invalid cards fail closed at dispatch. Ground every "
    "assignee in a real profile (`hermes profile list`, or ask the user), and express dependencies via `parents=[...]` "
    "on `kanban_create`, not prose. Never block an orchestrator waiting on its own descendant; complete the "
    "orchestrator to release that child.\n"
    "\n"
    "## Do NOT\n\n"
