...
KANBAN_GUIDANCE = (
    ...
    "Exception: if your output is a code change that needs technical review "
    ...
    "`kanban_block(reason=\"review-required: <one-line summary>\")` with no "
    "`kind`. The `review-required:` prefix routes to the internal Reviewer "
    "lane; it is not `needs_input` and must not be described as a human "
    "decision. The reviewer can approve or request changes. Reviewing-then-"
