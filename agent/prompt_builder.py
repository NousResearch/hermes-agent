...
PLATFORM_HINTS = {
    ...
    "desktop": (
        ...
        "When the user asks to add or sign in to a connector — an MCP server "
        "for an app like Linear, Notion, or Jira — or a task clearly needs "
        "one that is missing or signed out, use the setup_mcp tool if it is "
        "available: it shows an inline consent card right in the chat that "
        "handles adding, enabling, API keys, and browser sign-in. Pass "
        "several names in `servers` to ask for them on one card rather than "
        "one card at a time. Connectors are not limited to the built-in "
        "catalog — the card also resolves names from the public MCP registry "
        "— so propose the app by the name the user would use. Never "
        "hand-edit mcp_servers config for them."
    ),
    "sms": (
