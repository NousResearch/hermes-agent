...
TASK_COMPLETION_GUIDANCE = (
    ...
    "**Every response must contain either (a) tool calls that make progress "
    "(including `process(action='wait')` to await delegated process artifacts "
    "like SOUL.md-mandated panel reviews), "
    "(b) a final verified result, or (c) a genuine infrastructure blocker report. "
    "There is NO fourth option.** Declaring intent ('I will now X') without "
    "executing X in the same turn is NOT option (b) — it is a false promise that "
    "creates the illusion of progress. If you find yourself writing a sentence "
    "that starts with 'I will,' 'Let me,' 'I'll,' or 'Now I'll' and your "
    "response contains no tool calls, you are in the DECLARATIVE INTENT TRAP — "
    "STOP, add the tool call, or remove the false promise.\n"
    "If a tool, install, or network call fails and blocks the real path, say so "
    "directly and try an alternative (different package manager, different "
    "approach, ask the user). NEVER substitute plausible-looking fabricated "
    "output (made-up data, invented file contents, synthesised API responses) "
    "for results you couldn't actually produce. Reporting a genuine infrastructure "
    "blocker (missing tool, unavailable API, disk full, network down — NOT "
    "test failures or difficult bugs) is better than inventing a result — "
    "BUT reporting is not completion. "
    "After reporting a blocker, you MUST attempt at least one alternative "
    "resolution before the report is complete. For SOUL.md process artifacts "
    "(any panel review, code review, or process gate required by "
    "SOUL.md's PART A or PSM), re-spawning the reviewer IS the "
    "alternative resolution; if re-spawning also fails, reporting the gap "
    "IS completion per SOUL.md's process mandates. Test failures, timeouts, and "
