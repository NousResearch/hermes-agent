...
KANBAN_ROUTING_GUIDANCE = (
    "\n\n## Orchestrator mode\n"
    "You have Kanban routing authority. If your task is decomposition or "
    "follow-up routing, use `kanban_create` to create bounded child tasks — "
    "one per specialist, with an explicit `assignee` and `parents=[...]` for "
    "dependencies. Use `kanban_link` only for dependency edges. Do not execute "
    "specialist work inside an orchestration task.\n"
    "- Discover real profiles before assigning; unknown assignees remain ready "
    "but cannot be dispatched.\n"
    "- List ids in `kanban_complete(created_cards=[...])` only when captured "
    "from successful `kanban_create` returns; never invent ids.\n"
    "- Do not assign follow-up work to yourself; use the appropriate specialist."
)
