...
def redact_sensitive_text(
    ...
    *,
    ...
    # over the real one.
    text = _redact_registry_patterns(text, file_read=file_read)

    return text


_PATTERNS_CACHE: dict = {"mtime": None, "path": None, "lit_re": None,
                         "key_re": None, "lit_masks": {}, "default_mask": None,
                         "broken": False}
...
def _redact_registry_patterns(text: str, *, file_read: bool) -> str:
    ...
    if not (_PATTERNS_CACHE.get("mtime") == mtime
        ...
        try:
            ...
            _PATTERNS_CACHE["key_re"] = (
                re.compile(r"(?i)\b(" + "|".join(re.escape(k) for k in keys)
                           + r")(['\"]?)(\s*)([=:])(\s*)"
                           + r"(?:\"([^\"\n]*)\"|'([^'\n]*)'|([^\s\"',;}{}\])]+))")
                if keys else None)
    ...
    if _PATTERNS_CACHE["key_re"] is not None:
        def _k(m):
            ...
            return (m.group(1) + (m.group(2) or "") + m.group(3) + m.group(4)
                    + m.group(5) + q
                    + _apply_mask(val, _PATTERNS_CACHE["default_mask"], file_read)
                    + q)
