...
_STREAMING_ENV_CANDIDATE_RE = re.compile(
    rf"([A-Z0-9_]{{1,{_MAX_STREAMING_ENV_NAME}}})(\s*)"
    r"(?:(=)\s*['\"]?)?",
)
...
def _could_be_env_opener_at(text: str, start: int) -> bool:
    """Whether ``text[start:]`` can become a secret ENV assignment."""
    match = _STREAMING_ENV_CANDIDATE_RE.fullmatch(text, start)
    if match is None:
        return False
    name = match.group(1)
...
def _could_be_env_opener(candidate: str) -> bool:
    ...
    return _could_be_env_opener_at(candidate, 0)


def _partial_env_opener_start(
    text: str,
    *,
    embedded_prefixes: bool,
) -> int | None:
    """Return an ENV opener from its grammar-bounded trailing name region."""
    suffix = _STREAMING_ENV_SUFFIX_RE.search(text)
    if suffix is None:
        return None
    for start in range(suffix.start(1), suffix.end(1)):
        has_boundary = (
            start == 0
            or text[start - 1]
        ...
        if (
            (embedded_prefixes or has_boundary)
            and _could_be_env_opener_at(text, start)
        ):
            return start
    return None
    ...
    def _extend_incremental_candidate(self, text: str) -> bool:
        ...
        if env_stage == "env_quoted_value":
            ...
            if closing == len(text) - 1:
                ...
                return True
        if env_stage == "env_quoted_closed" and not text:
            return True
        return False
    ...
    def feed_with_metadata(
        ...
        *,
        ...
        if (
            not final
            and (
                self._pending_json_stage is not None
