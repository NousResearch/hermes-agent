...
def redact_sensitive_text(
    ...
    *,
    ...
    if not code_file:
        if "=" in text:
            # Git author/committer identity vars (GIT_AUTHOR_*/GIT_COMMITTER_*)
            # contain "AUTH" (in "AUTHOR") and so falsely match the secret-name
            # patterns below — including upstream's _CFG_ANCHORED_RE (matches the
            # bare "auth" substring). They are never secrets (public commit
            # metadata); redacting them corrupts `git filter-branch --env-filter`.
            # Exempt them here so the guard covers ALL three ENV passes
            # (_ENV_ASSIGN_RE, _CFG_DOTTED_RE, _CFG_ANCHORED_RE) uniformly.
            _GIT_IDENTITY_NAMES = (
            ...
            def _redact_env(m):
                ...
                # Strip an optional `export ` prefix before the allowlist check
                # so `export GIT_AUTHOR_EMAIL=...` is exempted too.
                _bare = name.split()[-1] if name else name
                if _bare in _GIT_IDENTITY_NAMES:
                    return m.group(0)
