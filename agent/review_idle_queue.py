...
class _PendingReview:
    ...
    kwargs: Dict[str, Any]
    enqueued_at: float
    # The enqueuing turn's context: its profile HERMES_HOME override and secret scope. The dispatcher
    # thread starts with an empty context, so without it the review writes the default profile.
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                if item is not None:
                    if not item.ctx.run(self._still_enabled, item):
                        logger.info(
                    ...
                    item.ctx.run(item.agent._spawn_background_review_now, **item.kwargs)
