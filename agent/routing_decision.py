...
def _persist_agent_decision(
    ...
    from agent.delegation_context import is_dispatcher_owned_worker_context
    dispatcher_task_id = _optional_text(os.environ.get("HERMES_KANBAN_TASK"))
    if (
        not is_dispatcher_owned_worker_context()
        or not dispatcher_task_id
        or _optional_text(stored.get("task_id")) != dispatcher_task_id
    ):
