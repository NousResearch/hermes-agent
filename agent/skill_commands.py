...
def _register_skill_command(
    *,
    ...
    if resolve_command(cmd_name) is not None:
        logger.warning(
            "Skill %r generates slash command '/%s' which collides with a core "
            "Hermes command; skipping auto-registration. Use '/skill %s' instead.",
            name,
            cmd_name,
            name,
        )
    ...
    if cmd_key in _skill_commands:
        logger.warning(
            "Skill %r maps to slash command %s already claimed by %r; keeping "
            "the first and skipping this one.",
            name,
            cmd_key,
