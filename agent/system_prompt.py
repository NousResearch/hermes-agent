...
def resolve_kanban_worker_guidance(valid_tool_names) -> str:
    ...
    try:
        ...
        # Lifecycle instructions can mutate durable board state; ambiguous
        # ownership must fail closed rather than borrow an ambient task.
        return ""
...
def _tool_guidance_block(agent: Any) -> Optional[str]:
    ...
    # Kanban lifecycle is task-bound, not merely tool-bound. Configured
    # orchestrators expose kanban_show without owning a current task.
    _kanban_guidance = getattr(agent, "_kanban_worker_guidance", None)
    if _kanban_guidance is None:
        # Compatibility fallback for lightweight agents that bypass _load_tools.
