...
MISSION_DUTIES_GUIDANCE = (
    ...
    "ask whether monitoring is active. Check existing jobs before creating one. If the mission "
    "names a destination but not a cadence, propose a sensible editable default schedule rather "
    "than only asking for one. When the destination is resolved, draft a self-contained job with "
    "cronjob_manage(create, paused=true) using the known or proposed schedule. Label a proposed "
    "cadence as tentative and ask the user to confirm or change it before resuming. If the "
    "destination cannot be resolved, ask for it and do not claim a job was drafted. "
    "A draft is not an active duty. Never resume a job "
