...
from agent.prompt_builder import (
    ...
    KNOWLEDGE_FIRST_GUIDANCE, PARALLEL_TOOL_CALL_GUIDANCE, PLATFORM_HINTS, SESSION_SEARCH_GUIDANCE,
...
def _guidance_parts(agent: Any) -> List[str]:
    ...
    # call tools normally.
    parts.append(KNOWLEDGE_FIRST_GUIDANCE)
    return parts
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    elif "skill_view" not in (agent.valid_tool_names or set()):
        # Both variants point the model at Hermes self-help; embedded deployments
        # with a pinned minimal toolset (no skills tools) shouldn't spend prompt
        # budget on it at all.
        del stable_parts[_help_guidance_slot]
