...
def _is_unsupported_response_format(exc: BaseException) -> bool:
    """True when the provider rejected the json_schema response_format itself.

    Some providers implement only ``json_object`` and refuse the request rather
    than ignoring the unknown field — DeepSeek answers HTTP 400 "This
    response_format type is unavailable now". That is a different failure from a
    provider that accepts the request and returns non-JSON: here no response
    exists to parse, so the loose-JSON fallback in ``_extract_title_text`` never
    gets a chance to run. Only a 400 naming the format earns the retry; a 400
    from a malformed request or bad auth must still surface.
    """
...
def generate_title(
    ...
    try:
        ...
        try:
            ...
            # A provider that implements only json_object refuses the request instead of
            # ignoring the unknown field, so there is no response and no prose for the
            # fallback parser to salvage. Retry unconstrained: the prompt itself demands
            # JSON-only output, which is all _extract_title_text needs.
            if not _is_unsupported_response_format(exc):
                raise
            logger.warning(
                "Title generation: provider rejected response_format (%s); retrying without it", exc,
            )
            response = call_llm(
                task="title_generation",
                messages=[{"role": "system", "content": prompt}, {"role": "user", "content": user_snippet}],
