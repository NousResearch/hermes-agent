...
def _is_unsupported_response_format(exc: BaseException) -> bool:
    ...
    Some providers implement only ``json_object`` (DeepSeek answers HTTP 400
    "This response_format type is unavailable now"). A 400 from a malformed
    request or bad auth must NOT trigger the retry, so the message text is
    gated too.
    """
...
def generate_title(
    ...
    try:
        try:
            ...
            # Providers that implement only json_object (e.g. DeepSeek) reject the
            # json_schema response_format outright with HTTP 400 ("This response_format
            # type is unavailable now"). Retry without it: the prompt already demands
            # JSON-only output and _extract_title_text parses loose JSON or first-line
            # prose, so the title still lands.
            if not _is_unsupported_response_format(e):
                raise
            logger.warning(
