    ...
    *,
    ...
    if _is_multimodal_tool_result(persisted_result):
        ...
        persisted_result = maybe_persist_tool_result(
            ...
            env=get_active_env(effective_task_id), task_id=effective_task_id,
            config=budget,
        )
...
def _finalize_tool_batch(agent, messages: list, effective_task_id: str, num_tools: int, budget: BudgetConfig) -> None:
    ...
    if num_tools <= 0:
        return
    enforce_turn_budget(messages[-num_tools:], env=get_active_env(effective_task_id), task_id=effective_task_id, config=budget)
