...
def _emit_tool_complete_and_risk(
    agent, ref: _ToolCallRef, result, risk_metadata, blocked: bool, *,
    is_error: bool = False, tool_duration: float = 0.0,
) -> None:
    """Fire tool_complete_callback (unless blocked), the tool.output_risk projection, and the
    fork's own tool.completed telemetry (structured event log + the /timeline ring buffer).

    Registry tools suppress model_tools' own post-hook (the executor owns it here), so this is
    the one place both dispatch paths (sequential, segmented/parallel) converge and can emit the
    fork telemetry exactly once per call.
    """
    ...
    _outcome = "blocked" if blocked else ("failed" if is_error else "succeeded")
    try:
        from hermes_logging import emit_event

        emit_event(
            "tool.completed", subsystem="tools", outcome=_outcome,
            action_id=ref.call_id, detail=f"{ref.name} ({tool_duration:.2f}s)",
        )
    except Exception:
        pass
    ...
    try:
        ...
        _timeline_record_end(
            getattr(agent, "session_id", None), ref.call_id, status=_outcome, duration=tool_duration,
        )
    except Exception:
        pass
...
def _append_batch_results(agent, messages: list, effective_task_id: str, batch: _ConcurrentBatch, budget: BudgetConfig) -> bool:
    ...
    for i, pc in enumerate(batch.parsed_calls):
        ...
        _emit_tool_complete_and_risk(
            agent, ref, display_function_result, risk_metadata, blocked,
            is_error=is_error, tool_duration=tool_duration)
    return True
...
def _publish_sequential_result(agent, messages: list, ref: _ToolCallRef, managed: _ManagedToolResult, *, tool_duration: float, index: int, budget: BudgetConfig) -> bool:
    ...
    _emit_tool_complete_and_risk(
        agent, ref, display_function_result, risk_metadata, managed.blocked,
        is_error=_is_error_result, tool_duration=tool_duration)
    if agent._should_emit_quiet_tool_messages():
        cute_msg = _get_cute_tool_message_impl(ref.name, ref.args, tool_duration, result=display_function_result)
        agent._safe_print(f"  {cute_msg}")
    elif _tool_progress_enabled(agent):
