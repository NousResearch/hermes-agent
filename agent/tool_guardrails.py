...
IDEMPOTENT_TOOL_NAMES = frozenset({
    ...
    "tool_search", "tool_describe", "browser_snapshot", "browser_console", "browser_get_images", "mcp_filesystem_read_file",
...
# The tool_search bridge (progressive tool disclosure) is the newest runaway-prone surface: on a
# tier-2 catalog (tool names hidden) a weak model can spray near-identical BM25 queries
# indefinitely, each returning success, so the repeated-failure detector never trips and the turn
# burns its whole iteration budget without a single real tool_call. Generous enough for genuine
# multi-tool discovery, low enough to stop the spiral.
...
_DECISION_MESSAGES: dict[str, str] = {
    ...
    "loop_tool_search_cap": (
        "Blocked tool_search: this turn has already searched the tool catalog {cap} times, the per-turn limit. "
        "This looks like a runaway discovery loop. Stop searching: if the matches so far include a tool that "
        "fits, load it with tool_describe and call it; otherwise the capability is likely not available — tell "
        "the user what is missing or ask them with clarify instead of searching again."
    ),
}
...
class ToolCallGuardrailController:
    ...
    def _check_loop_cap(
        ...
        increment = 1 if tool_name in ("web_search", "tool_search") else (_subagent_spawn_count(args) if cap else 0)
