...
#: HTTP-success payload some OpenAI-compatible gateways emit for a provider
#: timeout. Byte-exact match only: any whitespace, sibling choice, or tool
#: call is genuine model output, never the shim.
ROUTER_TIMEOUT_SHIM = "Connect timeout, please try again later."


def _has_positive_completion_tokens(usage: Any) -> bool:
    """Whether usage proves the provider generated output (not a shim)."""
...
def _responses_timeout_text(response: Any) -> tuple[bool, str | None]:
    """Raw Responses evidence as ``(recognized, sole_text)``.

    ``sole_text`` is set only when the response holds exactly one message
    item with exactly one text part (or only a top-level ``output_text``
    compatibility field). Any sibling item/content part proves this is not
    the router's one-text shim.
    """
...
def is_router_timeout_shim(response: Any) -> bool:
    """Whether a response is the router timeout shim.

    The shim is an HTTP-success payload emitted by some OpenAI-compatible
    gateways. It is only a shim when the *raw* response is exactly one text
    payload equal to the sentinel, has no tool-call evidence, and usage does
    not prove the model generated output. Whitespace, additional
    choices/items/content parts, and function calls are meaningful evidence.
    """
    if response is None:
        return False
    ...
    if choices:
        ...
        if content != ROUTER_TIMEOUT_SHIM:
            return False
        if _obj_get(message, "tool_calls"):
            return False
    else:
        recognized, responses_text = _responses_timeout_text(response)
        if not recognized:
            return False
        if responses_text != ROUTER_TIMEOUT_SHIM:
            return False
    return not _has_positive_completion_tokens(_obj_get(response, "usage"))
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def validate_response(self, response: Any) -> bool:
        ...
        Some OpenAI-compatible gateways answer a provider timeout with HTTP
        success carrying exactly ``"Connect timeout, please try again later."``
        as the sole choice content. Accepting it as a ChatCompletion surfaces
        router noise as the assistant reply and skips provider fallback, so
        reject it here (``validate_response_shape`` routes to fallback).
        """
        if response is None:
            return False
