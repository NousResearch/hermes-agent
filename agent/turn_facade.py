...
_COMPLETION_SILENCE_GUIDANCE = (
    "[System: This turn is an internal process completion notification. If all results "
    "have already been communicated or do not change the current conclusion, return "
    "exactly NO_REPLY. Do not send an acknowledgement that results were already accounted "
    "for. Report any new material result or failure, or use tools if further work is needed. "
    "An empty response is not intentional silence. This permission applies only to this "
    "notification turn, never to a subsequent human message.]"
)
