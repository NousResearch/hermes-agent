...
# A promoted reasoning blob that is ONLY a short fully-bracketed status note (see the
# placeholder guard in finish_text_response): the model emitting a system-status line
# as its entire reply. 4-60 chars inside the brackets keeps long scaffold brackets like
# "[System: ...]" (nudges) and real bracketed prose out.
_PROMOTED_PLACEHOLDER_RE = re.compile(r"\A\[[^\[\]]{4,60}\]\Z")
...
def finish_text_response(
    ...
    # Degenerate reasoning shapes: a reasoning parser can file a system-note STYLE placeholder
    # as the reasoning on a clean stop — a short fully-bracketed status line like
    # "[Empty reply content regenerated by debridizer]" or "[Invalid JSON was filtered from
    # this turn]" (observed 4x on qwen3.8-flash-next after long tool-heavy chains at 100k+
    # input). That is not chain-of-thought and not an answer: promoting it DELIVERS the
    # placeholder to the user as the reply. Leave it unpromoted so the text response reads as
    # empty and the empty-response ladder runs (bounded re-prompts / prefill instead).
    _placeholder_reasoning = bool(_PROMOTED_PLACEHOLDER_RE.match(_promoted)) if _promoted else False
    if _placeholder_reasoning:
        logger.warning(
            "Reasoning-only clean stop was a placeholder status note (%r) — not promoting; "
            "entering empty-response recovery (model=%s provider=%s)",
            _promoted, agent.model, agent.provider,
        )
        _promoted = None
