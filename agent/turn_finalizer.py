...
def finalize_turn(
    ...
    *,
    ...
    # --- Protocol violation auto-detection (issue #1000) ---
    # If this is a kanban worker and the model exited with a text response
    # (no tool call), it likely never called kanban_complete/kanban_block.
    # Auto-call _record_task_failure so the dispatcher sees a clean protocol
    # transition instead of a protocol_violation crash.
    _kanban_task = os.environ.get("HERMES_KANBAN_TASK")
    if _kanban_task and completed and normal_text_response:
        try:
            # Check if kanban_complete or kanban_block was called
            # during this turn by scanning messages for tool results
            _kanban_tool_called = False
                ...
                if _msg.get("role") == "assistant":
                    for tc in (_msg.get("tool_calls") or []):
                        if tc and isinstance(tc, dict):
                            fn = tc.get("function", {})
                            if fn.get("name") in ("kanban_complete", "kanban_block"):
                                _kanban_tool_called = True
                                break
                if _kanban_tool_called:
                    break
            if not _kanban_tool_called:
                from hermes_cli import kanban_db as _kb
                _conn = _kb.connect()
                try:
                    _kb._record_task_failure(
                        ...
                        _kanban_task,
                        error=(
                            "Worker exited with text response without calling "
                            "kanban_complete or kanban_block \u2014 auto-blocked by "
                            "turn_finalizer protocol violation detection"
                        ),
                        outcome="crashed",
                        failure_limit=1,
                        release_claim=True,
                        ...
                        event_payload_extra={
                            "auto_detected": True,
                            "exit_reason": str(_turn_exit_reason)[:200],
                        },
                    )
                    logger.warning(
                        "Protocol violation auto-detected for task %s: "
                        "text_response without kanban_complete/kanban_block",
                        _kanban_task,
                    )
                finally:
                    try:
            ...
            logger.warning(
                "Failed to auto-detect protocol violation for task %s",
                _kanban_task,
                exc_info=True,
            )
