...
def _maybe_close_kanban_task_from_final_response(
    ...
    *,
    ...
    """Close a still-running Kanban worker task from ordinary final text.

    Dispatcher-spawned workers are expected to end with ``kanban_complete`` or
    ``kanban_block``. If a model returns ordinary final text instead, this
    fail-safe converts that text into a terminal Kanban transition so the
    dispatcher does not treat the clean process exit as a protocol violation.
    The normal tool path remains authoritative: this helper first checks the DB
    and no-ops unless the env task is still ``running`` for the current run.
    """
