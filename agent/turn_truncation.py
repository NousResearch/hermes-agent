...
def _retry_truncated_tool_call(st: _Trunc, api_kwargs: Any) -> TruncationVerdict:
    ...
    execute incomplete arguments. When the prompt already filled the context window no
    max_tokens boost can help (there is no room for output), so refuse immediately
    instead of burning the retries — mirrors the _continue_text ceiling exit."""
    agent = st.agent
    filled = st.window_filled
    if st.truncated_tool_call_retries < 4 and filled is None:
    ...
    if filled is not None:
        agent._vprint(
            f"{agent.log_prefix}⚠️  Not retrying the truncated tool call — the prompt filled the "
            f"context window ({filled[0]:,}/{filled[1]:,} tokens); no max_tokens boost can help.",
            force=True,
        )
        _final_response = _CONTEXT_OVERFLOW_PARTIAL_FINAL
    elif st.is_stub:
        agent._vprint(
