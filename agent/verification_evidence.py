...
def _connect() -> sqlite3.Connection:
    ...
    settings = resolve_database_settings(_db_path())
    if settings.backend == "postgres":
        from hermes_cli.postgres_util import connect

        return connect(settings, "verification_evidence", initialize=_ensure_schema)
...
def _insert_evidence(evidence: VerificationEvidence) -> dict[str, Any]:
    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        params = (
            created_at, e.session_id, e.cwd, e.root, e.command, e.canonical_command,
            e.kind, e.scope, e.status, e.exit_code, e.output_summary,
        )
        if getattr(conn, "backend", "sqlite") == "postgres":
            event_id = int(conn.execute(statement + " RETURNING id", params).fetchone()[0])
        else:
