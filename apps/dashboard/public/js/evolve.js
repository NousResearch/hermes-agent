...
function proposalRow(p, redraw, actionable) {
  ...
  const source = p.source || p.payload?.source || "heuristic";
  const isModel = source === "model";
  const badge = h("span.evolve-source", {
    class: `evolve-source ${isModel ? "evolve-source-model" : "evolve-source-heuristic"}`,
    title: isModel
      ? "Written by the deep model reviewing the agent's telemetry & memory"
      : "Generated by a deterministic rule",
  }, isModel ? "✦ MODEL" : "⚙ RULE");

  return h("div.evolve-row", {},
    h("div.evolve-main", {},
      h("div.evolve-title-row", {}, h("div.evolve-title", {}, p.title), badge),
