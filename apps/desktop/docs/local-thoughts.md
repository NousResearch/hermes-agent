...
2. Type a fragment and choose **Save thought**. Saving works while the gateway
   is offline, once Desktop has identified the current connection and profile.
3. Open **Saved thoughts**, then **Use in input** to return to one. This does not
   send it anywhere. Enter sends through the existing chat path when connected.
...
receipt confirming delivery. Replace the text or save it when you return.
