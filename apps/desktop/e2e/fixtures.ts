...
export function writeMockProviderConfig(
  ...
  const display = options.language ? `display:\n  language: ${options.language}\n` : ''
 ...
 */
export async function setupMockBackend(options: { language?: string } = {}): Promise<MockBackendFixture> {
  // 1. Start mock server
  const mock = await startMockServer()

  // 2. Create sandbox + write config
  const sandbox = createSandbox('mock')
  writeMockProviderConfig(sandbox.hermesHome, mock.url, options)
