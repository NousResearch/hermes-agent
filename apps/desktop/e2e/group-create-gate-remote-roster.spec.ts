...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(hermes.bin, [...hermes.args, 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
    cwd: REPO_ROOT,
    detached: true,
    // HOME is pinned to the sandbox too: profile roots are HOME-anchored, and a
    // HERMES_HOME that happens to sit under the developer's ~/.hermes (a TMPDIR
    // there) would otherwise serve their real profiles as the remote's bots.
    env: { ...process.env, HOME: root, HERMES_HOME: home, HERMES_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN, PYTHONPATH: REPO_ROOT },
