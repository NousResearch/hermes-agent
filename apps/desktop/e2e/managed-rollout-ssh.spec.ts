...
test.describe('managed SSH rollout acceptance (provider-gated)', () => {
  test('preparation completes before a pinned rollout can be admitted', async ({}, testInfo) => {
    requireCredentialFreeProvider(testInfo)
  })

  test('recheck observes an unknown attempt without redispatching it', async ({}, testInfo) => {
    requireCredentialFreeProvider(testInfo)
  })
