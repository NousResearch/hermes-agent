...
async function gotoRoute(route: string): Promise<void> {
  ...
  // The router may normalise or redirect the hash (an unknown route, a settings
  // tab it rewrites), so wait for the navigation itself rather than an exact
  // hash — every in-page hop must keep the chosen scale either way.
  await page.waitForFunction(previous => window.location.hash !== previous, before)
}
