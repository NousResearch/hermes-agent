...
describe('artifact version identity', (): void => {
  ...
  it('reports baked channel source version independently of native sequence version', (): void => {
    const stamp: InstallStamp = {
      schemaVersion: 1, source: 'channel-build', payload: 'bundled', distribution: 'desktop-app',
      updateMechanism: 'electron-updater', tag: null, commit: 'a'.repeat(40), commitDate: null,
      branch: null, builtAt: null, dirty: false, baseVersion: '1.2.3', displayVersion: '0.0.7', distance: null,
      channelBuild: { schema: 1, buildId: 'b'.repeat(32), channel: 'new-name', sequence: 7,
        repository: 'example/hermes-agent', commit: 'a'.repeat(40), sourceVersion: '1.2.3', version: '0.0.7', windowsVersion: '0.0.7.0', publicBase: 'https://example.com', bundleEnv: {},
        identity: { token: '1234567890abcdef', displayName: 'Preview', appId: 'chat.preview', appNamePascal: 'Preview', artifactNamePascal: 'Preview', cliName: 'preview', windowsExecutableName: 'Preview.exe', msixAppIdWithOrg: 'Nous.Preview' } }
    }
    expect(appVersionInfo(stamp, '9.9.9-remote', '0.0.7')).toMatchObject({
      channel: 'new-name', baseVersion: '1.2.3', appVersion: '1.2.3 (new-name #7, aaaaaaaa)',
      sequence: 7, buildId: 'b'.repeat(32)
    })
  })
