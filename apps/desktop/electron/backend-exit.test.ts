...
test('backend exit waits through escalation and removes its listener', async () => {
  ...
  try {
    await waitForBackendExit(child, process => { escalated++; process.kill() }, 0)
    assert.equal(escalated, 1)
    assert.ok(child.exitCode !== null || child.signalCode !== null)
    assert.equal(child.listenerCount('exit'), before)
    await waitForBackendExit(child, () => { throw new Error('an exited process must not be killed again') })
  } finally {
...
test('backend exit refuses a live child rather than reporting a completed shutdown', async () => {
  ...
  try {
    await assert.rejects(waitForBackendExit(child, () => {}, 0), /did not exit/)
...
test('a failed spawn has no process to escalate', async () => {
  ...
  try {
    ...
    await waitForBackendExit(child, () => { throw new Error('a failed spawn must not be signalled') }, 0)
  } finally {
