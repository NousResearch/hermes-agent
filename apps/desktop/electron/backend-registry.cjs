...
// in isolation; main.cjs owns the impure read/write/verify/kill wiring.
 ...
 *
 * @param {string} text
 * @returns {{pid:number, command:string, startedAt:number}[]}
 */
function parseRegistry(text) {
  ...
  const list = Array.isArray(raw)
    ? raw
    : raw && Array.isArray(raw.backends)
      ? raw.backends
      : []
  const seen = new Set()
  const out = []
...
function stringifyRegistry(entries) {
...
function upsertEntry(entries, entry) {
  ...
  next.push({
    ...
    startedAt:
      entry && Number.isFinite(Number(entry.startedAt)) ? Number(entry.startedAt) : 0
  })
  return next
}

/** Drop entries whose pid appears in `pids`. Returns a NEW array. */
function removePids(entries, pids) {
  const base = Array.isArray(entries) ? entries : []
 ...
 * @returns {number[]}
 */
function reapablePids(entries, selfPid) {
  const base = Array.isArray(entries) ? entries : []
 ...
 * @returns {boolean}
 */
function backendCommandMatches(command) {
