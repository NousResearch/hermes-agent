...
test('parseRegistry accepts a bare array for back-compat', () => {
  ...
  assert.deepEqual(parseRegistry(text), [{ pid: 7, command: 'hermes dashboard --no-open', startedAt: 5 }])
})
...
test('reapablePids returns every recorded pid except our own', () => {
  const entries = [
    { pid: 10, command: '', startedAt: 0 },
    { pid: 20, command: '', startedAt: 0 },
    { pid: 30, command: '', startedAt: 0 }
  ]
...
test('reapablePids de-duplicates and skips invalid pids', () => {
  const entries = [
    { pid: 10, command: '', startedAt: 0 },
    { pid: 10, command: '', startedAt: 0 },
    { pid: 0, command: '', startedAt: 0 },
    { pid: -1, command: '', startedAt: 0 },
    { pid: 40, command: '', startedAt: 0 }
  ]
