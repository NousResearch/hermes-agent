  ...
  async function open(owner: number, plugin: string, raw: unknown): Promise<boolean> {
    ...
    if (!win || win.isDestroyed()) {
      win = registry.openOrFocus(key, () => {
        ...
        // Electron can lose ready-to-show on Wayland even after a successful load.
        const reveal = createWindowRevealController({
          isDestroyed: () => created.isDestroyed(),
          isVisible: () => created.isVisible(),
          show: () => created.showInactive()
        })

        created.once('ready-to-show', reveal.reveal)
        created.webContents.once('did-finish-load', reveal.scheduleFallback)
        created.on('closed', reveal.dispose)
