...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundCapacitySkip(error) || isBackgroundSlotRetryDeferred(error)) {
    return
  }
...
async function ensureBackend(
  profile,
  opts: { passive?: boolean; spawnPriority?: LocalBackendSpawnPriority; speculative?: boolean } = {}
) {
  localBackendLifecycle.assertCanStart()
...
async function ensureRegistryBackend(
  ...
  const primary = await reuseMatchingPrimarySshBackend({
    ...
    ensurePrimary: () => ensureBackend(profile, { passive, spawnPriority, speculative }),
    profile,
    registry,
    source
  })
  ...
  if (source.kind === 'local') {
    ...
    if (localRoute.delegate) {
      return ensureBackend(profile, { passive, spawnPriority, speculative })
    }
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (entry.speculative === true && spawnPriority === 'background') {
    ...
    // Queue subscriptions let the arbiter reclaim a slot for a foreground
    // request, including a promoted background request.
...
async function connectDesktopProfileRoute(
  ...
  try {
    connection = await runForegroundRetryingDialClaim(
      ...
      () =>
        route.connectionId
          ? ensureRegistryBackend(route.connectionId, route.profile, '', { spawnPriority, speculative })
          : ensureBackend(route.profile, { spawnPriority, speculative }),
