...
import { createLinkPreviewRuntime, registerLinkPreviewRuntime, TITLE_BYTE_BUDGET, TITLE_USER_AGENT } from './link-preview-runtime'
...
// Link title + link preview resolution live in link-preview-runtime.ts (the
// runtime owner: pinned one-hop curl transport, hidden title window, durable
// preview cache, and the IPC surface). main.ts composes it and keeps only the
// pieces other subsystems share.
...
registerLinkPreviewRuntime(
  ipcMain,
  createLinkPreviewRuntime({ app, BrowserWindow, session })
)
