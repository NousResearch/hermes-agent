...
//
// The avatar is stored at ~/.hermes/avatar.png and persists across updates.
...
ipcMain.handle('hermes:avatar:get', async () => {
  const avatarPath = path.join(app.getPath('home'), '.hermes', 'avatar.png')
...
ipcMain.handle('hermes:avatar:set', async (_event, dataUrl: string) => {
  ...
  const hermesDir = path.join(app.getPath('home'), '.hermes')
  ...
  const avatarPath = path.join(hermesDir, 'avatar.png')
