...
function buildRemoteBlock(remoteUrl, authMode, token, options: any = {}) {
  ...
  const block: { url: string; authMode: string; token: object; org?: string; computerUseBridge: boolean } = {
    url: normalizeRemoteBaseUrl(remoteUrl),
    authMode,
    token,
    computerUseBridge: options.computerUseBridge !== false
  }
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  if (key) {
    ...
    if (remoteLike) {
      profiles[key] = {
        mode,
        ...buildRemoteBlock(remoteUrl, authMode, nextToken, { org: cloudOrg, computerUseBridge })
      }
    } else {
      delete profiles[key]
    }
...
async function prepareProfileDeleteRequest(request) {
  const requestedProfile = profileNameFromDeleteRequest(request)

  const decision = decideProfileDeleteAction(requestedProfile, {
