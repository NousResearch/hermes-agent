...
export interface TrustedSourceReader {
  ...
  git(args: readonly string[], repositoryRoot: string, connectionId: string): Promise<string | Uint8Array>
...
export async function verifyReviewedGitSource(
  ...
  const query = async (...args: string[]): Promise<string> => text(await reader.git(args, source.repositoryRoot, expected.connectionId)).trim()
  ...
  try {
    await reader.git(['merge-base', '--is-ancestor', source.targetSha, ref], source.repositoryRoot, expected.connectionId)
  } catch {
  ...
  const protocol = await reader.git(['show', `${source.targetSha}:${PROTOCOL_RESOURCE_PATH}`], source.repositoryRoot, expected.connectionId)
