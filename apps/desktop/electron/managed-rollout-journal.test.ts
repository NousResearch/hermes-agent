...
test('ignores a crashed process stale marker only when an external process owner is supplied', () => {
  withTempDirectory(directory => {
    const modulePath = fileURLToPath(new URL('./managed-rollout-journal.ts', import.meta.url))

    const script = [
      "const { pathToFileURL } = require('node:url')",
      'import(pathToFileURL(process.argv[1]).href).then(({ ManagedRolloutJournal }) => {',
      '  new ManagedRolloutJournal({ directory: process.argv[2] }).acquireOwner()',
      '  process.exit(0)',
      '})'
    ].join('\n')

    const owner = spawnSync(process.execPath, ['--experimental-strip-types', '-e', script, modulePath, directory], {
      encoding: 'utf8',
      windowsHide: true
    })
    ...
    assert.throws(
      () => journal(directory).acquireOwner(),
      (error: unknown) => error instanceof JournalError && error.code === 'owner-unavailable'
    )
    const replacement = journal(directory, { processOwner: () => true }).acquireOwner()
    replacement.release()
  })
})

test('refuses writes after external process ownership is lost without advancing the journal', () => {
  withTempDirectory(directory => {
    let ownsProcess = true
    const instance = journal(directory, { processOwner: () => ownsProcess })
    instance.create(snapshot())
    ownsProcess = false
    ...
    assert.throws(
      () => instance.record({
        id: ROLLOUT_A,
        expectedRevision: 1,
        requestId: 'request-after-owner-loss',
        payload: { action: 'pause' },
        snapshot: snapshot(ROLLOUT_A, { phase: 'paused' }),
    ...
    const replacement = journal(directory, { processOwner: () => true })
    assert.equal(replacement.read(ROLLOUT_A).snapshot.revision, 1)
    assert.equal(replacement.hasUnresolvedInstall(INSTALL_A), false)
  })
})

test('refuses journal reconstruction before creating the directory when process ownership is absent', () => {
  withTempDirectory(parent => {
    const directory = path.join(parent, 'journal')

    assert.throws(
      () => journal(directory, { processOwner: () => false }),
      (error: unknown) => error instanceof JournalError && error.code === 'owner-unavailable'
    )
...
test('retains bounded rollout metadata across record updates and a fresh journal process', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'managed-rollout-metadata-'))
  const metadata = { schema: 1, queueGeneration: 4, planDigest: 'plan-digest' }

  try {
    const first = new ManagedRolloutJournal({ directory, clock: () => '2026-09-21T00:00:00.000Z' })
    first.create(snapshot(), { metadata })
    first.record({
      id: ROLLOUT_A,
      expectedRevision: 1,
      requestId: 'metadata-update',
      payload: { kind: 'metadata-update' },
      snapshot: snapshot(ROLLOUT_A, { phase: 'attention-required' }),
      metadata: { ...metadata, queueGeneration: 5 },
      events: [event('metadata-updated')]
    })

    const reopened = new ManagedRolloutJournal({ directory, clock: () => '2026-09-21T00:00:00.000Z' })
    assert.deepEqual(reopened.read(ROLLOUT_A).metadata, { ...metadata, queueGeneration: 5 })
  } finally {
    fs.rmSync(directory, { recursive: true, force: true })
  }
})
