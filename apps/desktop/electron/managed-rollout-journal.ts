...
// eslint-disable-next-line no-control-regex -- journal identifiers cannot contain NUL or line breaks
const REQUEST_CONTROL_RE = /[\x00\r\n]/
...
export interface JournalRecordOptions {
  ...
  archive?: ArchiveMetadata | null
  facts?: JournalEvidenceFact[]
  /** Bounded coordinator-owned recovery metadata; never exposed as IPC payload. */
  metadata?: Record<string, unknown>
}
...
export interface ManagedRolloutJournalOptions {
  ...
  directory: string
  /** Held by the Electron main process for the lifetime of every effectful rollout. */
...
function validateRequestId(requestId: unknown): string {
  if (
    ...
    REQUEST_CONTROL_RE.test(requestId)
  ) {
...
function validateIsoLike(value: unknown, label: string): string {
  if (typeof value !== 'string' || !value || value.length > 128 || REQUEST_CONTROL_RE.test(value)) {
...
function validateReason(value: unknown, label: string): string {
  if (typeof value !== 'string' || !value || value.length > MAX_REASON_LENGTH || REQUEST_CONTROL_RE.test(value)) {
...
function validateMetadata(value: unknown): Record<string, unknown> {
  if (!isPlainObject(value)) {throw new JournalCorruptionError('Journal metadata must be an object.')}
  const serialized = canonicalJson(value)

  if (Buffer.byteLength(serialized, 'utf8') > MAX_REQUEST_PAYLOAD_BYTES) {
    throw new JournalCorruptionError('Journal metadata is too large.')
  }
...
export class ManagedRolloutJournal {
  ...
  private readonly processOwner?: () => boolean
  ...
  constructor(options: ManagedRolloutJournalOptions) {
    ...
    this.assertProcessOwner()
    this.open()
  }

  private assertProcessOwner(): void {
    if (this.processOwner && !this.processOwner()) {
      throw new JournalError('owner-unavailable', 'This process does not own the managed rollout journal.')
    }
  }
  ...
  acquireOwner(): JournalOwnerLease {
    if (this.processOwner) {
      ...
      return { token: randomUUID(), release: () => undefined }
    }
  ...
  private assertRegular(pathname: string, label: string): void {
    const stat = this.fs.lstatSync(pathname)
  ...
  private parseRecord(raw: string, expectedId: string): JournalRecord {
    ...
    const metadata = parsed.metadata === undefined ? undefined : validateMetadata(parsed.metadata)
  ...
  private prepareAck(id: string, revision: number, eventSequences: number[]): JournalAck {
    ...
    if (!acceptanceId || REQUEST_CONTROL_RE.test(acceptanceId)) {
