...
test('captures a coherent inventory revision and routes Git through the inspected source', async () => {
  ...
  assert.equal(await adapters.sourceReader.git(['rev-parse', 'HEAD'], ROOT, CONNECTION_A), 'ok')
...
test('routes identical repository paths on distinct SSH sources to their selected installations', async () => {
  ...
  const adapters = createManagedRolloutProductionAdapters({
    ...
    git: async connectionId => {
      gitCalls.push(connectionId)
  ...
  assert.ok(snapshot)
  assert.equal(snapshot!.observations.length, 2)
  assert.equal(await adapters.sourceReader.git(['status', '--short'], ROOT, CONNECTION_A), CONNECTION_A)
  assert.equal(await adapters.sourceReader.git(['status', '--short'], ROOT, CONNECTION_B), CONNECTION_B)
  assert.deepEqual(gitCalls, [CONNECTION_A, CONNECTION_B])
  await assert.rejects(
    () => adapters.sourceReader.git(['status', '--short'], `${ROOT}-other`, CONNECTION_A),
    /reviewed-repository-source-unavailable/
  )
})
