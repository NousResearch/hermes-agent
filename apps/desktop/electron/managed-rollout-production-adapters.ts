...
export function createManagedRolloutProductionAdapters(
  ...
  // A remote path is meaningful only together with its authenticated source.
    ...
    capture: async (): Promise<TrustedInventorySnapshot | null> => {
      ...
      const consolidated = [...groups.values()].map(entries => {
        ...
        for (const entry of entries) {
          repositoryOwners.set(entry.source.id, primary.observation.codeRoot)
        }
  ...
  const sourceReader: TrustedSourceReader = {
    ...
    git: async (args, repositoryRoot, connectionId) => {
      const root = canonicalCodeRoot(repositoryRoot)

      if (!connectionId || repositoryOwners.get(connectionId) !== root) {
        await inventoryReader.capture()
      }

      if (!connectionId || repositoryOwners.get(connectionId) !== root) {
        throw new Error('reviewed-repository-source-unavailable')
      }

      return options.git(connectionId, args, root)
    }
  }
