...
import { installationFingerprint, sourceFingerprint, canonicalCodeRoot } from './managed-rollout-identity'
...
function readProfile(root: string, profile: string): { generation: number; requiredControlIds: readonly string[] } | null {
  ...
  try {
    ...
    if (!Array.isArray(ids) || ids.length === 0 || ids.some(id => typeof id !== 'string' || !id || /[\x00-\x1f\x7f]/.test(id))) return null
