...
import {
  ...
  type ManagedRolloutAttemptState,
...
type PersistedCoordinatorMetadata = {
  schema: 1
...
function coordinatorMetadata(runtime: Runtime, state: ManagedRolloutState): PersistedCoordinatorMetadata {
  return {
    schema: 1,
    ...
    plan: clone(runtime.plan)
  }
}

function readCoordinatorMetadata(value: unknown): PersistedCoordinatorMetadata | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {return null}
  const input = value as Record<string, unknown>

  const phases: CoordinatorPhase[] = [
    'queued', 'running', 'awaiting-promotion', 'paused', 'stopped',
    'attention-required', 'reconciling', 'completed', 'completed-with-exclusions'
  ]

  if (
    input.schema !== 1 ||
    !Number.isSafeInteger(input.queueGeneration) || Number(input.queueGeneration) < 0 ||
    !Number.isSafeInteger(input.currentWave) || Number(input.currentWave) < 0 ||
    (input.policy !== 'manual' && input.policy !== 'auto-after-canary') ||
    typeof input.phase !== 'string' || !phases.includes(input.phase as CoordinatorPhase) ||
    typeof input.canaryApproved !== 'boolean' ||
    typeof input.continuationRequired !== 'boolean' ||
    typeof input.stopRequested !== 'boolean'
  ) {return null}

  try {
    return {
      schema: 1,
      queueGeneration: input.queueGeneration as number,
      currentWave: input.currentWave as number,
      phase: input.phase as CoordinatorPhase,
      policy: input.policy as ManagedRolloutPolicy,
      canaryApproved: input.canaryApproved as boolean,
      continuationRequired: input.continuationRequired as boolean,
      stopRequested: input.stopRequested as boolean,
      plan: validateRolloutPlan(input.plan)
    }
  } catch {
    return null
  }
...
function restoredAttemptState(phase: RolloutSnapshot['attempts'][number]['phase']): ManagedRolloutAttemptState {
  switch (phase) {
...
function restoredState(
  snapshot: RolloutSnapshot,
  metadata: PersistedCoordinatorMetadata,
  targets: readonly ManagedRolloutTarget[]
): ManagedRolloutState {
  const snapshotAttempts = new Map(snapshot.attempts.map(attempt => [attempt.identity.installId, attempt]))

  const attempts = Object.fromEntries(targets.map(target => {
    const persisted = snapshotAttempts.get(target.installId)
    const state = restoredAttemptState(persisted?.phase ?? 'queued')

    return [target.installId, {
      ...target,
      state,
      reprobeCount: persisted?.reprobes ?? 0,
      reprobeCooldownUntilMono: null,
      ...(persisted?.reasons[0] ? { reason: persisted.reasons[0] } : {}),
      ...(persisted?.phase === 'skipped' ? { excluded: persisted.skipReason === 'operator-excluded' } : {})
    }]
  }))
...
export function createManagedRolloutProvider(
  ...
  const persistNow = (
    ...
    const ack = deps.journal.record({
      ...
      events,
      facts,
      metadata: coordinatorMetadata(runtime, state),
      unresolved
    })
  ...
  const persistAuthorization = async (runtime: Runtime, authorization: ManagedRolloutAuthorization): Promise<void> => {
    await enqueue(runtime, async () => {
      ...
      const ack = deps.journal.record({
        ...
        metadata: coordinatorMetadata(runtime, runtime.coordinator.snapshot),
  ...
  const makeRuntime = (
    id: string,
    plan: RolloutPlan,
    restored?: { snapshot: RolloutSnapshot; state: ManagedRolloutState }
  ): Runtime => {
    ...
    for (const [wave, installIds] of plan.waves.entries()) {
      for (const installId of installIds) {
        ...
        if (!row?.reviewedSource || !row.requiredScopeIds || !row.admittedHead) {throw new Error('rollout-plan-provenance-incomplete')}
        const persisted = restored?.snapshot.attempts.find(attempt => attempt.identity.installId === installId)
        targets.set(installId, {
          ...
          correlationId: persisted?.correlationId ?? randomUUID(),
          wave
        })
      }
    }
    ...
    const coordinatorPlan = {
      id,
      revision: 1,
      queueGeneration: restored?.state.queueGeneration ?? 0,
      policy: policy(plan.promotionPolicy),
      targets: [...targets.values()]
    }

    const initialState = restored?.state ?? createManagedRolloutState(coordinatorPlan)
    const restoredAttempts = restored?.snapshot.attempts ?? []
    ...
    const coordinatorDependencies: ManagedRolloutCoordinatorDependencies = {
      ...
      service: {
        ...
        launch: (authorization, capability) => {
          ...
          persistNow(
            runtime,
            `handoff:${authorization.installId}:${authorization.correlationId}`,
            { kind: 'handoff-accepted', installId: authorization.installId, correlationId: authorization.correlationId },
            runtime.coordinator.snapshot,
    ...
    runtime.coordinator = createManagedRolloutCoordinator(initialState, coordinatorDependencies)

    for (const attempt of Object.values(initialState.attempts)) {
      if (!['none', 'intent-recorded', 'skipped', 'cancelled-before-launch'].includes(attempt.state)) {
        runtime.authorizations.set(attempt.installId, {
          rolloutId: runtime.id,
          ...
          reviewedSource: clone(attempt.reviewedSource),
          correlationId: attempt.correlationId,
          queueGeneration: initialState.queueGeneration
        })
      }
    }
  ...
  const hydrateRuntimes = (): void => {
    for (const summary of deps.journal.history({ limit: MAX_PROVIDER_PAGE_SIZE }).items) {
      ...
      try {
        ...
        const metadata = readCoordinatorMetadata(record.metadata)

        if (!metadata) {continue}
        const snapshot = validateRolloutSnapshot(record.snapshot)

        if (
          JSON.stringify(metadata.plan.target) !== JSON.stringify(snapshot.target) ||
          metadata.plan.retryOf !== snapshot.retryOf ||
          metadata.plan.rows.length !== snapshot.attempts.length ||
          new Set(metadata.plan.rows.map(row => row.installId)).size !== new Set(snapshot.attempts.map(attempt => attempt.identity.installId)).size
        ) {
          throw new Error('rollout-recovery-plan-mismatch')
        }

        const targets: ManagedRolloutTarget[] = []

        for (const [wave, installIds] of metadata.plan.waves.entries()) {
          for (const installId of installIds) {
            const row = metadata.plan.rows.find(item => item.installId === installId)
            const persisted = snapshot.attempts.find(attempt => attempt.identity.installId === installId)

            if (!row?.reviewedSource || !row.requiredScopeIds || !row.admittedHead || !persisted) {
              throw new Error('rollout-recovery-provenance-incomplete')
            }
            ...
            targets.push({
              ...
              connectionId: row.connectionId,
              sourceFingerprint: row.sourceFingerprint,
              targetSha: metadata.plan.target.sha,
              reviewedSource: clone(row.reviewedSource),
  ...
  const validateAdmission = async (plan: RolloutPlan): Promise<{
    ...
    if (plan.retryOf) {
      let prior: JournalRecord

      try {
        prior = deps.journal.read(plan.retryOf)
      } catch {
        throw new Error('retry-source-unavailable')
      }

      if (prior.unresolved.length > 0) {throw new Error('retry-unresolved-fence')}
  ...
  const startInternal = async (request: { token: string; requestId: string }): Promise<unknown> => {
    ...
    const ack = deps.journal.create(snapshot as unknown as JournalSnapshot, {
      ...
      metadata: coordinatorMetadata(runtime, startedState),
  ...
  const command = async (raw: ManagedRolloutIpcCommand): Promise<unknown> => {
    ...
    return enqueue(runtime, async () => {
      ...
      switch (parsed.action) {
        case 'pause': transition = await runtime.coordinator.command({ kind: 'pause' });

 break

        case 'resume': transition = await runtime.coordinator.command({ kind: 'resume' });

 break

        case 'stop': transition = await runtime.coordinator.command({ kind: 'stop' });

 break

        case 'exclude': transition = await runtime.coordinator.command({ kind: 'exclude', installId: parsed.installId! });

 break

        case 'promote': transition = await runtime.coordinator.promote(false);

 break

        case 'reprobe': transition = await runtime.coordinator.reprobe(parsed.installId!);

 break

        case 'recover': transition = await runtime.coordinator.recover(parsed.installId!);
