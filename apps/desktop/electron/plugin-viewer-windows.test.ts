...
import { WINDOW_REVEAL_FALLBACK_MS } from './window-reveal'
...
it.each(['lost', 'ready-first', 'load-first', 'fallback-first'])(
  'reveals a loaded viewer exactly once without focus: %s ready-to-show',
  async order => {
    vi.useFakeTimers()
    ...
    const input = { id: 'watch', title: 'Viewer', url: 'http://127.0.0.1:9234/viewer' }
    expect(await viewers.open(1, 'demo', input)).toBe(true)
    vi.advanceTimersByTime(WINDOW_REVEAL_FALLBACK_MS)
    ...
    if (order === 'lost' || order === 'fallback-first') {
      vi.advanceTimersByTime(WINDOW_REVEAL_FALLBACK_MS - 1)
      expect(win.showInactive).not.toHaveBeenCalled()
    ...
    // Further loads and the losing reveal path cannot produce another show event.
    win.webContents.emit('did-finish-load')
    vi.advanceTimersByTime(WINDOW_REVEAL_FALLBACK_MS)
    expect(win.showInactive).toHaveBeenCalledOnce()
...
it.each(['closed', 'destroyed', 'plugin close', 'owner close', 'failed load'])(
  'retires a pending viewer reveal on %s, including late events',
  async retirement => {
    vi.useFakeTimers()
    const win = fakeWindow()
    const viewers = api.createPluginViewerWindows(() => win as unknown as BrowserWindow)
    ...
    const retire = {
      ...
      destroyed: () => win.destroy(),
      'plugin close': () => viewers.close(1, 'demo', 'watch'),
      'owner close': () => viewers.closeOwner(1),
      'failed load': async () => {
