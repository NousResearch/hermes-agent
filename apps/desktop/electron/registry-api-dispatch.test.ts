...
describe('registry REST dispatch', () => {
  it.each([undefined, 'background', 'foreground', 'unexpected'])(
    'preserves explicit dial priority: %s',
    async priority => {
      const backend = { baseUrl: 'https://gateway.example.test', sharedRemote: true }
      const ensureRegistryBackend = vi.fn(async () => backend)

      const dispatch = createRegistryApiDispatcher({
        backendDialClaims: new BackendDialClaims(),
        ensureRegistryBackend,
        fetchJsonForBackend: vi.fn(async () => ({ ok: true })),
        profileRouteOptions: () => ({})
      })

      await dispatch({ path: '/api/status', profile: 'work', priority }, 'remote-a')
      expect(ensureRegistryBackend).toHaveBeenCalledWith('remote-a', 'work', '', {
        spawnPriority: priority === 'foreground' ? 'foreground' : 'background'
      })
    }
  )

  it('preserves profile-preference bookkeeping for registry requests', async () => {
    const afterProfileRequest = vi.fn()
    const dispatch = createRegistryApiDispatcher({
      afterProfileRequest,
      backendDialClaims: new BackendDialClaims(),
      ensureRegistryBackend: vi.fn(async () => ({ baseUrl: 'https://gateway.example.test', mode: 'remote', sharedRemote: true })),
      fetchJsonForBackend: vi.fn(async () => ({ ok: true })),
      profileRouteOptions: () => ({})
    })
    const request = { path: '/api/profiles/renamed', method: 'PATCH' }

    await dispatch(request, 'remote-a')

    expect(afterProfileRequest).toHaveBeenCalledWith('remote-a', request, { ok: true }, 'remote')
  })
