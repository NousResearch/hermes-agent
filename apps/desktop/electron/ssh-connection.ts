...
function withRemoteTimeout(remoteCommand, timeoutSecs = REMOTE_PROBE_TIMEOUT_SECS) {
  ...
  return (
    // zsh exits a non-interactive shell when asked to enable job control without
    // a TTY. SSH exec commands use precisely that shape, so leave job control
    // disabled there and retain the direct-child kill below.
