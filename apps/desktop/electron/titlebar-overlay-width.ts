...
/**
 ...
 //     overlay, so it reserves nothing here.
 //   - Windows and WSLg paint the Electron overlay, so they reserve the
 //     fallback width.
 //   - Plain Linux now paints NO overlay (getTitleBarOverlayOptions() returns
 //     false there to honour a frameless/minimalist desktop), so it must reserve
 //     nothing either — otherwise a dead 144px gap is left on the right edge
 //     where the native controls used to sit.
 //
 // @param {{ isMac?: boolean, isLinux?: boolean }} opts
  */
 export function nativeOverlayWidth({ isWindows = false, isWsl = false, isMac = false, isLinux = false } = {}) {
   if (isMac || isLinux) {
     return 0
   }
