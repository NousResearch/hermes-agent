...
test('dev and opt-in perf production builds keep the real session-open perf fixture', () => {
  ...
  assert.match(sessionOpenPerfFixtureEntry(root, 'build', { VITE_PERF_PROBE: '1' }), /session-open-perf-fixture\.ts$/)
  assert.doesNotMatch(sessionOpenPerfFixtureEntry(root, 'build', { VITE_PERF_PROBE: '1' }), /\.noop\.ts$/)
})

test('Vite resolves the internal fixture alias to the production no-op', async () => {
  const config = await resolveConfig({ configFile: path.join(root, 'vite.config.ts'), mode: 'production' }, 'build')
