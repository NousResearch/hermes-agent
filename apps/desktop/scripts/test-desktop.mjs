...
const PRODUCT_NAME = PACKAGE_JSON.build.productName
...
function resolveDmgPath() {
  const prefix = `Team-Hermes-Desktop-${PACKAGE_JSON.version}`

  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `${prefix}-${ARCH}.dmg`)
  }
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `${prefix}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is
  // 'Team-Hermes-Desktop-${version}-${os}-${arch}.${ext}'.
