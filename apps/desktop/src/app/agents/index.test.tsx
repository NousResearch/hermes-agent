...
describe('AgentsView outcome status', () => {
  it('labels a partial result as partial instead of merely unverified', () => {
    ...
    expect(screen.getAllByLabelText(outcomeCopy.partial)).toHaveLength(2)
    expect(screen.queryByLabelText(outcomeCopy.verificationRequired)).toBeNull()
  })

  it('keeps an unverified result behind the verification-required state', () => {
    renderAgents('unverified')

    expect(screen.getAllByLabelText(outcomeCopy.verificationRequired)).toHaveLength(2)
    expect(screen.queryByLabelText(outcomeCopy.partial)).toBeNull()
  })
})
