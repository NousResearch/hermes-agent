...
import { AlertCircle, CheckCircle2, Clock, ClockPause } from '@/lib/icons'
...
/**
 * Toggle a running subagent's delegation.child_timeout_seconds cap on or off.
 *
 * Only rendered for rows the backend reports a configured cap for (see
 * `timeoutSeconds` in the store) — a child with no cap has nothing to
 * toggle. `active` starts true (a cap is in force from the moment the row
 * appears) and flips on each round trip from the `active` the gateway
 * reports back, so this stays correct even if a click races the subagent
 * finishing.
 */
function ExtendTimeoutButton({ subagentId }: { subagentId: string }) {
  const { t } = useI18n()
  ...
  const handleToggle = async (event: MouseEvent) => {
    ...
    try {
      const result = await requestGateway<{ active: boolean | null; applied: boolean }>(
        'subagent.extend_timeout',
        active ? { disable: true, subagent_id: subagentId } : { restore: true, subagent_id: subagentId }
      )
      ...
      notifyError(error, t.agents.extendTimeoutFailed)
    } finally {
      setPending(false)
    }
  }

  const label = active ? t.agents.extendTimeout : t.agents.extendTimeoutRestore
  ...
  return (
    <Tip label={label}>
      <Button
        aria-label={label}
        className={cn(
          'mt-0.5 shrink-0 text-muted-foreground/70 hover:text-foreground',
          !active && 'text-primary/85 hover:text-primary'
        )}
        disabled={pending}
        ...
        variant="ghost"
      >
        {active ? <ClockPause size={12} /> : <Clock size={12} />}
      </Button>
    </Tip>
  )
}
  ...
  return (
    <div className={cn('grid min-w-0 max-w-full gap-2', depth > 0 && 'pl-4')} data-slot="tool-block" ref={enterRef}>
      <div className="group flex w-full min-w-0 items-start gap-2.5">
        ...
        {running && node.timeoutSeconds ? <ExtendTimeoutButton subagentId={node.id} /> : null}
      </div>
