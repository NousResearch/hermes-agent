...
import { canvasRefreshPrompt, canvasRepairPrompt } from './canvas-policy'
...
export function CanvasesView({
  ...
  const previousWorkingIdsRef = useRef<string[]>(workingSessionIds)
  ...
  useEffect(() => {
    ...
    previousWorkingIdsRef.current = workingSessionIds

    if (previous.some(id => !workingSessionIds.includes(id))) {
      void refreshCanvases()
    }
  }, [refreshCanvases, workingSessionIds])

  useEffect(() => {
    const refreshOnFocus = () => void refreshCanvases()
  ...
  const requestCanvasAction = (prompt: string) => {
    if (onRequestRefresh) {
      onRequestRefresh(prompt)
    } else {
  ...
  const deleteSelectedCanvas = useCallback(async () => {
    ...
    try {
      ...
      notifyError(error, 'No se pudo eliminar el canvas')
    } finally {
      setIsDeleting(false)
    }
          ...
          <div className="mx-auto max-w-6xl">
            <div className="mb-5 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-1">
                ...
                <button
                  className="rounded-md px-2 py-1 text-xs text-(--ui-text-secondary) hover:bg-(--ui-control-hover-background) disabled:opacity-70"
                  disabled={isDeleting}
                  onClick={() => void deleteSelectedCanvas()}
                  type="button"
                >
            ...
            {loadError ? (
              <div className="rounded-lg border border-(--ui-stroke-tertiary) p-4 text-sm text-(--ui-text-secondary)">
                <p>{loadError}</p>
                <button
                  className="mt-3 rounded-md bg-(--ui-row-active-background) px-3 py-2 text-xs font-medium text-(--ui-text-primary) hover:bg-(--ui-control-hover-background)"
                  onClick={() => requestCanvasAction(canvasRepairPrompt(selectedCanvas.path))}
                  type="button"
                >
                  Repair with Hermes
                </button>
            ...
            {canvas ? (
              <div>
                <button
                  ...
                  onClick={() => {
                    requestCanvasAction(canvasRefreshPrompt(canvas, selectedCanvas.path))
                  }}
                  type="button"
                >
