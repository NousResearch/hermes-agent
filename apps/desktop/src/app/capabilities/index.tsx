...
import { UpdateSkillsButton } from './skills/update-skills-button'
...
export function CapabilitiesView({
  ...
  // Skills and Tools are fetched as a pair and wait for each other: until BOTH
  // lists are in, neither tab paints. Browse (the hub catalog) doesn't read the
  // installed list, and MCP/Plugins load their own data.
  const gated = mode === 'toolsets' || (mode === 'skills' && capabilityView === 'installed')
    ...
    >
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 overflow-hidden">{loadGate ?? tabContent[mode]()}</div>
        </div>
      </div>
    </PageSearchShell>
  )
