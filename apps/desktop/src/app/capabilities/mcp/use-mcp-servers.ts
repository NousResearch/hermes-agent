...
import { probeCache, probeKey, resolveMcpOwner } from '@/lib/mcp-probe-cache'
...
export function useMcpServers({ gateway, profile }: UseMcpServersOptions): McpServersController {
  ...
  const owner = resolveMcpOwner(profile, normalizeProfileKey(appProfile), getApiRequestConnection(), connection)
  const scopeProfileKey = owner?.key ?? `@unresolved::${profileScopeKey(profile)}`
