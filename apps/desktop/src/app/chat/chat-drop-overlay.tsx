...
const COPY: Record<'files' | 'session', { icon: string; labelKey: string }> = {
  files: { icon: 'cloud-upload', labelKey: 'chat.dropOverlay.attachFiles' },
  session: { icon: 'comment-discussion', labelKey: 'chat.dropOverlay.linkSession' }
}
...
export function ChatDropOverlay({ kind }: { kind: DragKind }) {
  ...
  const { icon, labelKey } = COPY[kind ?? lastKind.current]
