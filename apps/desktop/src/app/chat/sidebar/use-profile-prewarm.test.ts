...
describe('useProfilePrewarm (#100548 layout-only pointerenter)', () => {
  ...
  it('coalesces overlapping row visits onto the latest pointer intent', () => {
    const first = renderHook(() => useProfilePrewarm('first'))
    const second = renderHook(() => useProfilePrewarm('second'))

    act(() => {
      first.result.current.startPrewarm()
      first.result.current.notePointerMove()
      second.result.current.startPrewarm()
      second.result.current.notePointerMove()
      vi.advanceTimersByTime(DWELL_MS)
    })

    expect(prewarmProfileBackend).toHaveBeenCalledExactlyOnceWith('second')
  })
