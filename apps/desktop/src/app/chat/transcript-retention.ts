 ...
 * - Returns the input array by reference when nothing is released, so React
 *   trees are not re-rendered for a no-op.
 */
...
/**
 * Weight of already-paged-through history kept behind the live window.
 *
 * One window page: enough that the first "Show earlier" of a session pages
 * through memory (and the backfill it may then start is prefetched by the same
 * click), while the retained payload stays proportional to the window instead
 * of to the session's length.
 */
export const TRANSCRIPT_RETAIN_SLACK = TRANSCRIPT_WINDOW_BUDGET

export interface TranscriptRetention {
  /** Transcript to keep in the store; identical to the input when nothing was
   *  released. */
  messages: ChatMessage[]
  /** Rows released this pass. 0 means nothing was older than the retained
   *  window and slack. */
  releasedRows: number
  /** Persisted rows still retained, counted from the newest. This is the
   *  `transcript-tail` offset the next older REST page starts at — the backend
   *  pages by persisted rows, so it is counted in the same currency. */
...
export function boundRetainedTranscript(
  ...
  const keepEverything: TranscriptRetention = {
    messages: messages as ChatMessage[],
    releasedRows: 0,
    retainedPersistedRows: persistedRowCount(messages)
  }
  ...
  let boundary = anchor
  let slackLeft = TRANSCRIPT_RETAIN_SLACK
  ...
  return { messages: retained, releasedRows: boundary, retainedPersistedRows }
}
