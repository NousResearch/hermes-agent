...
const NAVIGATION_SEARCH_ENTRIES: readonly NavigationSearchEntry[] = [
  { id: 'nav-new-chat', route: NEW_CHAT_ROUTE, titleKey: 'chat.sidebar.newSession', detailKey: 'commandCenter.nav.newSessionDetail' },
  { id: 'nav-settings', route: SETTINGS_ROUTE, titleKey: 'shell.titlebar.openSettings', detailKey: 'commandCenter.nav.settingsDetail' },
  { id: 'nav-skills', route: SKILLS_ROUTE, titleKey: 'chat.sidebar.skills', detailKey: 'commandCenter.nav.skillsDetail' },
  {
    id: 'nav-messaging',
    route: MESSAGING_ROUTE,
    titleKey: 'chat.sidebar.messaging',
    detailKey: 'commandCenter.nav.messagingDetail'
  },
  { id: 'nav-artifacts', route: ARTIFACTS_ROUTE, titleKey: 'chat.sidebar.artifacts', detailKey: 'commandCenter.nav.artifactsDetail' }
]

const SECTION_SEARCH_ENTRIES: readonly SectionSearchEntry[] = [
  { id: 'section-sessions', section: 'sessions', titleKey: 'commandCenter.search.sessionsPanel', detailKey: 'commandCenter.search.sessionsPanelDetail' },
  { id: 'section-system', section: 'system', titleKey: 'commandCenter.search.systemPanel', detailKey: 'commandCenter.search.systemPanelDetail' },
  { id: 'section-usage', section: 'usage', titleKey: 'commandCenter.search.usagePanel', detailKey: 'commandCenter.search.usagePanelDetail' }
]
...
export function CommandCenterView({
  ...
  const searchProviders = useMemo<readonly CommandCenterSearchProvider[]>(
    () => [
      {
        id: 'navigation',
        label: t('commandCenter.search.navigate'),
        search: async searchQuery => {
          const routeHits: RouteSearchHit[] = NAVIGATION_SEARCH_ENTRIES.filter(entry =>
            ...
            detail: entry.detailKey ? t(entry.detailKey) : entry.detail,
            kind: 'route',
            route: entry.route,
          ...
          const sectionHits: SectionSearchHit[] = SECTION_SEARCH_ENTRIES.filter(entry =>
            matchesSearchQuery(
              ...
              entry.titleKey ? t(entry.titleKey) : entry.title,
              entry.detailKey ? t(entry.detailKey) : entry.detail,
              t(SECTION_LABELS[entry.section])
            )
          ).map(entry => ({
            detail: entry.detailKey ? t(entry.detailKey) : entry.detail,
            kind: 'section',
  ...
  return (
    <OverlayView
      closeLabel={t('shell.statusbar.closeCommandCenter')}
      headerContent={
        <OverlaySearchInput
      ...
      <OverlaySplitLayout>
        ...
        <OverlayMain>
          <header className="mb-4 flex items-center justify-between gap-2">
            <div>
              <h2 className="text-sm font-semibold text-foreground">{t(SECTION_LABELS[section])}</h2>
              <p className="text-xs text-muted-foreground">{t(SECTION_DESCRIPTIONS[section])}</p>
            </div>
            {section === 'system' && (
              <OverlayActionButton disabled={systemLoading} onClick={() => void refreshSystem()}>
                ...
                {systemLoading ? t('commandCenter.refreshing') : t('commandCenter.refresh')}
              </OverlayActionButton>
            )}
            {section === 'usage' && (
              <OverlayActionButton disabled={usageLoading} onClick={() => void refreshUsage(usagePeriod)}>
                ...
                {usageLoading ? t('commandCenter.refreshing') : t('commandCenter.refresh')}
              </OverlayActionButton>
            )}
          </header>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pr-1">
              {!hasGlobalSearchResults ? (
                <OverlayCard className="px-3 py-4 text-sm text-muted-foreground">
                  {t('commandCenter.search.noResults')}
                </OverlayCard>
              ) : (
            ...
            <div className="grid min-h-0 flex-1 grid-rows-[auto_minmax(0,1fr)] gap-3">
              <OverlayCard className="p-3 text-sm">
                {status ? (
                  ...
                  <div className="text-xs text-muted-foreground">{t('commandCenter.system.loadingStatus')}</div>
                )}
              </OverlayCard>

              <OverlayCard className="min-h-0 overflow-hidden p-2">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-medium text-muted-foreground">{t('commandCenter.system.recentLogs')}</span>
                  {systemError && (
    ...
    <div className="grid min-h-0 flex-1 grid-rows-[auto_auto_minmax(0,1fr)] gap-3">
      ...
      <OverlayCard className="p-3">
        {totals ? (
          ...
          <div className="text-xs text-muted-foreground">
            {t('commandCenter.usage.noUsage', { days: period })}{' '}
      ...
      <div className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)] gap-3">
        <OverlayCard className="p-3">
          <div className="mb-2 flex items-baseline justify-between">
            <span className="text-xs font-medium text-muted-foreground">{t('commandCenter.usage.dailyTokens')}</span>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <section className="min-w-0">
              <div className="mb-1.5 text-[0.7rem] font-medium uppercase tracking-wide text-muted-foreground">
                {t('commandCenter.usage.topModels')}
              </div>
              {byModel.length === 0 ? (
                <div className="text-xs text-muted-foreground">{t('commandCenter.usage.noModelUsage')}</div>
              ) : (
            ...
            <section className="min-w-0">
              <div className="mb-1.5 text-[0.7rem] font-medium uppercase tracking-wide text-muted-foreground">
                {t('commandCenter.usage.topSkills')}
              </div>
              {topSkills.length === 0 ? (
                <div className="text-xs text-muted-foreground">{t('commandCenter.usage.noSkillActivity')}</div>
              ) : (
                <ul className="space-y-1">
                  {topSkills.slice(0, 6).map(entry => (
                    ...
                    >
                      ...
                      <span className="shrink-0 text-[0.65rem] text-muted-foreground">
                        {t('commandCenter.usage.actionsCount', { count: entry.total_count.toLocaleString() })}
                      </span>
                    </li>
                  ))}
                </ul>
