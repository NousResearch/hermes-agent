    ...
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      async function runExec(ctx: SlashActionCtx): Promise<void> {
        ...
        try {
          ...
          // slash.exec resolves _ok even when the backend rejected the command
          // — the failure rides in `result.error` (see slash.exec in
          // tui_gateway/server.py). Surface it instead of rendering output as
          // a success line.
          if (result?.error) {
            renderSlashOutput(`error: ${result.error}`)
