...
describe('resumeSession failure recovery', () => {
  ...
  it('does not restore unchanged cache proof after the accepted runtime is optimistically edited', async () => {
    ...
    const cached = [
      {
        id: 'cached-before-edit',
        parts: [{ text: 'cached before edit', type: 'text' as const }],
  ...
  it('publishes an unproven remote legacy transcript before its deferred resume settles', async () => {
    ...
    vi.mocked(requestGatewayForAgent).mockImplementation(
      (_connectionId, _profile, method, params) => requestGateway(method, params) as never
    )
    ...
    expect(
      Array.from({ length: window.localStorage.length }, (_value, index) => window.localStorage.key(index)).filter(
        key => key?.startsWith('hermes.transcript-tail.v3')
      )
    ).toEqual([])
  ...
  it.each([
    ...
    'drops a deferred REST result when the current same-scope row has a newer display revision than %s',
    async (_case, responseRevision) => {
      const persisted = deferred<Awaited<ReturnType<typeof getLatestSessionMessages>>>()
      const deferredResume = deferred<SessionResumeResponse>()
      const oldProof = {
        connectionId: 'conn-1',
  ...
  it.each([
    ...
    'keeps pagination bookkeeping bound to accepted tip B for %s',
    async (_case, tipBPagination, expectedTruncated) => {
      const staleTipA = deferred<Awaited<ReturnType<typeof getLatestSessionMessages>>>()
      ...
      await waitFor(() => expect(transcriptTailState('stored-1')?.possiblyTruncated).toBe(expectedTruncated))
  ...
  it('keeps the complete transcript with the live tail after a full renderer restart', async () => {
    ...
    await waitFor(() =>
      expect(JSON.stringify(resumedState?.messages)).toContain('older question removed by compression')
    )
