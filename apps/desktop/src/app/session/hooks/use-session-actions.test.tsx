...
function ResumeHarness({
  ...
  const ref = <T,>(value: T): MutableRefObject<T> => ({ current: value })
  const resolvedRuntimeIdByStoredSessionIdRef = runtimeIdByStoredSessionIdRef ?? ref(new Map<string, string>())
  const resolvedSessionStateByRuntimeIdRef = sessionStateByRuntimeIdRef ?? ref(new Map<string, ClientSessionState>())
  ...
  const actions = useSessionActions({
    ...
    sessionStateByRuntimeIdRef: resolvedSessionStateByRuntimeIdRef,
    syncSessionStateToView: vi.fn(),
    updateSessionState: (sessionId, updater) => {
      const previous = resolvedSessionStateByRuntimeIdRef.current.get(sessionId) ?? createClientSessionState(null)
      const next = updater(previous)
      resolvedSessionStateByRuntimeIdRef.current.set(sessionId, next)
