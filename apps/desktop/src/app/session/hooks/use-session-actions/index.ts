    ...
    async (storedSessionId: string, replaceRoute = false, capturedOwner?: SessionProfileRoute) => {
      ...
      const isCurrentResume = () => {
        ...
        const transcriptAuthorityStillCurrent = enforceCapturedTranscriptAuthority
          ? Boolean(authorityState && (authorityState.transcriptAuthorityEpoch ?? 0) === transcriptAuthorityEpoch)
          : true
      ...
      const immediateConnectionId =
        ownerRouteForImmediateCache?.connectionId ||
        listedForImmediateCache?.connection_id ||
      ...
      try {
        ...
        // persisted display.
        const publishedDisplayProvenance = (): PersistedDisplayTranscriptProvenance | null => persistedDisplayProvenance
        ...
        const persistedDisplayMatchesResumedSession =
          !displayedTipId || !resumedStoredSessionId || displayedTipId === resumedStoredSessionId
            ...
            ? getLatestSessionMessages(resumedStoredSessionId, sessionRestScope, {
                ...
                .then(rebound => {
                  ...
                  const reboundMatchesAcceptedRuntime =
                    ...
                    (!reboundLineageRootId || !displayLineageRootId || reboundLineageRootId === displayLineageRootId)

                  if (!reboundMatchesAcceptedRuntime) {
                    return null
                  }

                  const provenance = provenanceFromMessagesResponse(rebound, storedSessionId, sessionRestScope)
        ...
        if (acceptedRuntimeTipHydration && resumedStoredSessionId) {
          void (async () => {
            ...
            const persistedWithClarifyProjection = pendingClarify
              ? restorePendingClarifyToolCall(persistedWithLiveProjection, pendingClarifyToolPayload(pendingClarify))
                  .messages
