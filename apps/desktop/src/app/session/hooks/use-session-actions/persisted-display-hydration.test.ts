...
describe('persisted display hydration', () => {
  ...
  it.each([
    ...
    const result = await startPersistedDisplayHydration(target, state.deps, { displayRevision: 7, hit: true })
  ...
  it('does not let an unproven empty legacy page erase a listed transcript', async () => {
    ...
    const result = await startPersistedDisplayHydration(listedTarget, state.deps, { hit: false, displayRevision: null })
