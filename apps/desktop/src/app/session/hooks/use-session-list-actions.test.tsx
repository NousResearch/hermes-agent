...
describe('LINE recents filtering', () => {
  ...
  it('does not preserve an active LINE session while paging one profile', async () => {
    const local = row('local', { profile: 'work' })
    const line = row('line', { profile: 'work', source: 'line', title: 'LINE chat' })

    setSessions([local, line])
    setSelectedStoredSessionId(line.id)
    listAllProfileSessions.mockResolvedValue({ sessions: [local] })
