...
function BuyCreditsOutcome({
  ...
  if (outcome.kind === 'success') {
    return (
      <div className="mt-2 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
        {copy.addedBalanceRefreshing(formatMoney(outcome.amountUsd ?? amount))}
      </div>
    )
  }
...
function UsageBar({ bar, fallbackLabel }: { bar?: BillingUsageRowView['bar']; fallbackLabel: string }) {
  ...
  const resolvedBar = bar ?? {
    label: t.settings.billing.usageFallback(fallbackLabel),
    state: 'neutral',
    tone: 'topup',
    value: 0
  }
...
function UsageRefreshRow({
  ...
  return (
    <div className="flex min-w-0 items-center justify-end gap-1.5 pt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
      <span>{copy.updatedAgo(formatUsageUpdatedAgo(updatedAt, now))}</span>
...
function BillingFixtureSelect({
  ...
  return (
    <Select onValueChange={value => onValueChange(value as BillingFixtureSelection)} value={value}>
      <SelectTrigger
        aria-label={copy.billingFixture}
  ...
  return (
    <SettingsContent>
      ...
      {view.accountRows.length > 0 && (
        <>
          <SectionHeading icon={BarChart3} title={copy.account} />
      ...
      {view.usageRows.length > 0 && (
        <>
          <SectionHeading icon={BarChart3} title={copy.usage} />
...
function validateAutoReloadInputs(
  ...
  const threshold = validateBillingAmount(translateNow('settings.billing.threshold'), thresholdRaw, bounds)

  if (threshold.error || threshold.amount == null) {
    return { error: threshold.error }
  }

  const reloadTo = validateBillingAmount(translateNow('settings.billing.reloadTo'), reloadToRaw, bounds)
  ...
  if (reloadTo.amount <= threshold.amount) {
    return { error: translateNow('settings.billing.reloadGreaterThanThreshold') }
  }
...
function validateBillingAmount(
  ...
  if (!cleaned || !/^\d+(\.\d{1,2})?$/.test(cleaned)) {
    return { error: translateNow('settings.billing.invalidAmount', label) }
  }

  const amount = Number(cleaned)

  if (!(amount > 0)) {
    return { error: translateNow('settings.billing.amountGreaterThanZero', label) }
  }

  const min = parseAmount(bounds.min_usd)

  if (min != null && amount < min) {
    return { error: translateNow('settings.billing.minimumAmount', label, formatMoney(min)) }
  }

  const max = parseAmount(bounds.max_usd)

  if (max != null && amount > max) {
    return { error: translateNow('settings.billing.maximumAmount', label, formatMoney(max)) }
  }
