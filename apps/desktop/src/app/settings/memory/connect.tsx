...
export function MemoryConnect({ provider }: { provider: string }) {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      setPhase('error')
      setDetail(copy.couldNotStart)
      notifyError(err, copy.connectFailed)
    ...
    timer.current = setInterval(() => {
      void (async () => {
        try {
          ...
          if (next.state === 'error') {
            setPhase('error')
            setDetail(locale === 'ar' ? copy.connectionFailed : next.detail || copy.connectionFailed)
          } else {
            setPhase('idle')
          }
        } catch {
