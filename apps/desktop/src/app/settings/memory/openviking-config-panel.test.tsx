...
const OPENVIKING_SERVICE_API_KEY_URL = 'https://console.volcengine.com/vikingdb/openviking/region:openviking+cn-beijing'

vi.mock('@/hermes', () => ({
  getOpenVikingHealth: () => getOpenVikingHealth(),
...
beforeEach(() => {
  ...
  getOpenVikingHealth.mockResolvedValue({
    label: 'Healthy',
...
describe('OpenVikingConfigPanel', () => {
  it('renders saved setup while the live health check is still pending', async () => {
    const health = deferred<{
      label: string
      message: string
      status: 'healthy'
    }>()
    getOpenVikingHealth.mockReturnValueOnce(health.promise)

    await renderPanel()

    expect(await screen.findByText('OpenViking settings')).toBeTruthy()
  ...
  it('shows a retry action when the initial setup load fails', async () => {
    getOpenVikingSetup.mockRejectedValueOnce(new Error('network unavailable'))

    await renderPanel()

    expect(await screen.findByText('OpenViking settings could not be refreshed. Try again.')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: 'Retry' }))

    expect(await screen.findByText('OpenViking settings')).toBeTruthy()
    expect(getOpenVikingSetup).toHaveBeenCalledTimes(2)
  })
  ...
  it('renders unhealthy and unreachable OpenViking status as the primary summary signal', async () => {
    ...
    expect(
      screen.getByText(
        'OpenViking rejected the credentials. Check the API key and account/user details, then try again.'
      )
    ).toBeTruthy()
  })

  it('keeps configuration available when the linked profile is invalid', async () => {
    getOpenVikingSetup.mockResolvedValue(
      setupPayload({
        health: {
          label: 'Profile invalid',
          message:
            'The linked OpenViking profile could not be loaded. Fix the profile file, refresh profiles, or choose another profile.',
          status: 'unreachable'
        }
      })
    )
    ...
    expect(await screen.findByText('Profile invalid')).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Configure' })).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: 'View OpenViking status details' }))
    expect(
      await screen.findByText(
        'The selected OpenViking profile could not be read. Fix the profile file, refresh profiles, or choose another profile.'
      )
    ).toBeTruthy()
  })

  it('refreshes OpenViking profiles when the setup wizard opens', async () => {
    getOpenVikingSetup.mockResolvedValueOnce(setupPayload()).mockResolvedValueOnce(setupPayload({ profiles: [] }))
  ...
  it('does not let a late wizard refresh overwrite user edits', async () => {
    ...
    getOpenVikingSetup.mockResolvedValueOnce(setupPayload()).mockReturnValueOnce(openingRefresh.promise)
    ...
    await waitFor(() => expect(getOpenVikingSetup).toHaveBeenCalledTimes(2))
    await waitFor(() => expect((screen.getByLabelText('Profile name') as HTMLInputElement).value).toBe('typed_profile'))
  ...
  it('creates an OpenViking Service profile after validation', async () => {
    ...
    await waitFor(() => expect(getOpenVikingHealth).toHaveBeenCalledTimes(2))
  })

  it('rejects Agent IDs that the OpenViking CLI cannot store', async () => {
    await renderPanel()
    await openWizard()

    fireEvent.click(screen.getByRole('button', { name: 'Next' }))
    fireEvent.change(screen.getByLabelText('Agent ID'), { target: { value: 'team/hermes' } })
    fireEvent.change(screen.getByLabelText('OpenViking API key'), { target: { value: 'service-secret' } })

    expect(screen.getByText("Agent ID cannot contain '/' or '\\'.")).toBeTruthy()
    expect((screen.getByRole('button', { name: 'Continue' }) as HTMLButtonElement).disabled).toBe(true)
  })

  it('keeps the last loaded setup visible when the post-save refresh fails', async () => {
    await renderPanel()
    await openWizard()
    ...
    expect(await screen.findByText(/The last loaded setup is still shown/)).toBeTruthy()
    expect(screen.getByText('OpenViking settings')).toBeTruthy()
    expect(screen.queryByText('Loading OpenViking settings...')).toBeNull()
    expect(getOpenVikingHealth).toHaveBeenCalledTimes(1)
  })
  ...
  it('shows validation failures as a visible alert and does not save', async () => {
    ...
    const alert = await screen.findByRole('alert')
    expect(alert.textContent).toContain(
      'OpenViking could not be reached. Check that the server URL is correct and the server is running.'
    )
    expect(saveOpenVikingSetup).not.toHaveBeenCalled()
  })

  it('explains when a root key requires trusted server mode', async () => {
    validateOpenVikingSetup.mockResolvedValueOnce({
      ok: false,
      message: 'OpenViking root API keys require a trusted-mode server for tenant memory access.',
      role: 'root'
    })
    ...
    const alert = await screen.findByRole('alert')
    expect(alert.textContent).toContain('This root API key requires an OpenViking server configured in trusted mode.')
    expect(saveOpenVikingSetup).not.toHaveBeenCalled()
  })
  ...
  it('blocks the OpenViking CLI reserved profile name', async () => {
    await renderPanel()
    await openWizard()

    fireEvent.click(screen.getByRole('button', { name: 'Custom Server' }))
    fireEvent.click(screen.getByRole('button', { name: 'Next' }))
    fireEvent.change(screen.getByLabelText('Profile name'), { target: { value: 'active' } })

    expect(await screen.findByText("'active' is reserved by the OpenViking CLI.")).toBeTruthy()
    expect((screen.getByRole('button', { name: 'Continue' }) as HTMLButtonElement).disabled).toBe(true)
  })
  ...
  it('explains legacy OpenViking environment variables', async () => {
    ...
    expect(
      screen.getByText('Save this setup to clear old variable names. They are still used until then.')
    ).toBeTruthy()
  })
})
