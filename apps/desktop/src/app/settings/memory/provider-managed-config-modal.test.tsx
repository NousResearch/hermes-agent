...
async function renderModal(config = schema()) {
...
describe('ProviderManagedConfigModal', () => {
  ...
  it('submits the complete form once and explains when the setup takes effect', async () => {
    ...
    expect(runMemoryProviderAction).toHaveBeenCalledWith('example', 'save', {
      overwrite: false,
  ...
  it('dispatches conditional provider actions and refreshes status after success', async () => {
    ...
    await waitFor(() =>
      expect(runMemoryProviderAction).toHaveBeenCalledWith('example', 'start-local', {
  ...
  it('refreshes dynamic options without resetting other drafts', async () => {
    ...
    await waitFor(() => expect(getMemoryProviderConfig).toHaveBeenCalledWith('example'))
