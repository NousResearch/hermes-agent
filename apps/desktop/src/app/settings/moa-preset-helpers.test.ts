...
describe('MoA every_n fanout compatibility', () => {
  it('accepts canonical every_n:N fanout and keeps Save complete after an unrelated edit', () => {
    const config = configFixture()
    config.presets.default.fanout = 'every_n:3'
    expect(moaConfigComplete(config)).toBe(true)

    // Unrelated Studio edit (rename display path via temperature tweak)
    config.presets.default.reference_temperature = 0.25
    expect(moaConfigComplete(config)).toBe(true)

    const prepared = prepareMoaConfigForSave(config)
    expect(prepared.presets.default.fanout).toBe('every_n:3')
    expect(moaConfigComplete(prepared)).toBe(true)
  })

  it('rejects malformed every_n values while still accepting user_turn and per_iteration', () => {
    const base = configFixture()
    expect(moaConfigComplete(base)).toBe(true)

    for (const fanout of ['user_turn', 'per_iteration', 'every_n:2', 'every_n:10'] as const) {
      const cfg = configFixture()
      cfg.presets.default.fanout = fanout
      expect(moaConfigComplete(cfg)).toBe(true)
    }

    for (const fanout of ['every_n:0', 'every_n:-1', 'every_n:abc', 'every_n:', 'weekly', 'EVERY_N:3'] as const) {
      const cfg = configFixture()
      // EVERY_N:3 lowercases in canonicalizer - actually we lower case so EVERY_N:3 becomes valid
      cfg.presets.default.fanout = fanout as string
      if (fanout.toLowerCase() === 'every_n:3') {
        expect(moaConfigComplete(cfg)).toBe(true)
      } else {
        expect(moaConfigComplete(cfg)).toBe(false)
      }
    }
  })
})
