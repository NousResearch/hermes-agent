...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const effortOptions = reasoningEffortOptions(mainCaps?.reasoning_efforts, LEGACY_REASONING_DEFAULT_OPTIONS)
  const effortValue = normalizeReasoningEffort(getNested(config ?? {}, 'agent.reasoning_effort'), effortOptions) || 'medium'
                  ...
                  <SelectContent>
                    {effortOptions.map(value => (
                      <SelectItem key={value} value={value}>
                        {value === 'none' ? m.reasoningOff : t.shell.modelOptions[EFFORT_LABEL_KEYS[value] ?? 'medium']}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
