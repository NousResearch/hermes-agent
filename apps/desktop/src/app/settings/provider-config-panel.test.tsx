...
describe('ProviderConfigPanel', () => {
  it('renders the declared provider fields generically', async () => {
    ...
    expect(panel.getByText('mid')).toBeTruthy()
  }, 15_000)
  ...
  it('collapses and expands the fields', async () => {
    ...
    expect(await panel.findByLabelText('API URL')).toBeTruthy()
    fireEvent.click(panel.getByRole('button', { name: /Hindsight settings/ }))
    expect(panel.queryByLabelText('API URL')).toBeNull()
    fireEvent.click(panel.getByRole('button', { name: /Hindsight settings/ }))
    expect(await panel.findByLabelText('API URL')).toBeTruthy()
  })
