...
function ConnectedProviderRow({
  ...
  // IX Agency can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider IX Agency can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // IX Agency never deletes creds another tool owns behind a silent API call.
