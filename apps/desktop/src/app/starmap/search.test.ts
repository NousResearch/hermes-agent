...
describe('filterNodes', () => {
  const g = graph(
    [
      ...
      node({ id: 'memory:memory:1', label: 'Build server notes', timestamp: 1_700_000_000 }),
      node({
        category: 'devops',
        createdBy: 'agent',
        id: 'deploy-skill',
  ...
  it('requires every term (AND)', () => {
    expect(filterNodes(g, 'server build', EMPTY_FILTERS).map(n => n.id)).toEqual(['memory:memory:1'])
