 ...
 *  - BOUNDED: at most MAX_PRELOAD sessions per launch (top of the visible list). The render cache's own LRU cap (200 files)
 *    bounds disk.
...
/** Order the preload set: visible (non-archived) rows, list order, capped. */
export function selectPreloadSessions(sessions: SessionInfo[], max: number = MAX_PRELOAD): SessionInfo[] {
  return sessions.filter(s => !s.archived).slice(0, Math.max(0, max))
}
