...
const outcomeCopy = DELEGATION_STATUS_COPY.en
...
describe('delegate card status glyph', () => {
  ...
  it('does not label a completed lifecycle with an unverified outcome as done', () => {
    ...
    expect(screen.getByLabelText(outcomeCopy.unverified)).toBeTruthy()
  ...
  it('marks a partial outcome as partial, not done', () => {
    ...
    expect(screen.getByLabelText(outcomeCopy.partial)).toBeTruthy()
    expect(screen.queryByLabelText(copy.statusDone)).toBeNull()
  })
  ...
  it('labels a background delegation as dispatched, not done', () => {
    ...
    expect(screen.getByLabelText(outcomeCopy.dispatched)).toBeTruthy()
    expect(screen.queryByLabelText(copy.statusDone)).toBeNull()
  })
})
