...
function Thumbnail({ image, className }: { image: MediaCarouselImage; className?: string }) {
  const { src } = useResolvedSrc(image.src)

  return <img alt={image.title ?? mediaName(image.src)} className={cn('block', className)} src={src} />
}
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto">
          {images.map((image, thumbIndex) => (
            <button
              ...
              type="button"
            >
              <Thumbnail image={image} className="h-9 w-14 rounded-md object-cover" />
            </button>
          ))}
        </div>
