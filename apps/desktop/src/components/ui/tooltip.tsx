    ...
    <TooltipPrimitive.Portal>
      ...
      >
        {/* bg-foreground/text-background auto-inverts per theme. leading-normal
            ...
            Vazirmatn-first stack: the @font-face unicode-range serves Vazir
            ONLY for Arabic-script chars, so Persian labels get a Persian face
            while Latin labels keep rendering in Arial exactly as before.
            dir comes from a majority vote for plain-string labels, auto
            fallback for node labels (plaintext CSS in styles.css owns
            alignment); LTR labels are unaffected. */}
        ...
        <span
          className="box-decoration-clone inline bg-foreground px-1.5 py-1 text-[11px] font-bold leading-normal text-background [font-family:Vazirmatn,Arial,sans-serif] [&>*]:!inline"
          dir={typeof children === 'string' ? voteDirection(children) : 'auto'}
        >
          {children}
        </span>
      </TooltipPrimitive.Content>
