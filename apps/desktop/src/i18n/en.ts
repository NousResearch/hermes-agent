...
export const en = {
  connectors: {
    ...
    connectTitle: (app: string) => `Connect ${app}?`,
    describe: (app: string) => `Hermes signs in to ${app} in your browser and asks before reading anything there.`,
  ...
  notifications: {
    ...
    errors: {
      ...
      openaiRejectedApiKeyWithStatus: (status: string) => `OpenAI rejected the API key (${status} invalid_api_key).`,
    ...
    voice: {
      ...
      liveUnavailable: (reason: string) => `GPT-Live voice chat is not available: ${reason}. Using speech-to-text instead.`
    },
    native: {
  ...
  settings: {
    ...
    plugins: {
      ...
      installModal: {
        ...
        missingEnv: (vars: string) => `Missing env vars: ${vars}. Add them in Settings → Keys.`
      }
    },
    vault: {
    ...
    gateway: {
      ...
      authNeedsOauth: (provider: string) => `This gateway uses OAuth. Sign in with ${provider} to authorize this desktop app.`,
      ...
      connectedTo: (baseUrl: string, version?: string) => `Connected to ${baseUrl}${version ? ` · Hermes ${version}` : ''}`,
    ...
    mcp: {
      ...
      deepLinkNameConflict: (name: string) => `A server named ${name} already exists — choose a different name or cancel.`,
    ...
    localModels: {
      ...
      upToDateDetail: (tag: string, backend: string) => `Running llama.cpp ${tag} (${backend}) — the latest build Hermes ships.`,
      updateToast: (next: string) => `A newer local engine build (${next}) is available. Update from Settings → Local Models.`,
    ...
    toolsets: {
      ...
      nousAuthNeededMessage: (provider: string) => `${provider} is saved but won't activate until you sign in to Nous Portal.`,
      ...
      postSetupErrorMessage: (step: string) => `Check the ${step} log.`,
      ...
      webCapabilitySelectedMessage: (provider: string, capability: string) => `${provider} now handles web ${capability}.`,
  ...
  skills: {
    ...
    hub: {
      ...
      resultCount: (count: number, ms: number | null) => `${count} result${count === 1 ? '' : 's'}${ms !== null ? ` in ${ms}ms` : ''}`,
  ...
  commandCenter: {
    ...
    generatePet: {
      ...
      hatched: 'It hatched!',
      hatchRow: (state: string, done: number, total: number) => `Sketching frame ${done} of ${total}…`,
  ...
  messaging: {
    ...
    } as Record<string, { label?: string; help?: string; placeholder?: string }>,
    platformIntro: {} as Record<string, string>
  },
  ...
  profiles: {
    ...
    soulPlaceholder: (mode: string) => `The system prompt / persona for this profile.\nLeave blank to keep the ${mode} default.`,
  ...
  composer: {
    ...
    } as Record<string, { label: string; description: string; text: string }>
  },
  ...
  onboarding: {
    ...
    } as Record<string, { short: string; description: string }>,
  ...
  preview: {
    ...
    web: {
      ...
      finishedRestarting: (message?: string) => `Hermes finished restarting the preview server${message ? `: ${message}` : ''}`,
      ...
      loadFailedConsole: (code: number | undefined, message: string) => `Load failed${code ? ` (${code})` : ''}: ${message}`,
  ...
  assistant: {
    ...
    mcpSetup: {
      installTitle: (server: string) => `Add the ${server} MCP server?`,
      enableTitle: (server: string) => `Enable the ${server} MCP server?`,
      authorizeTitle: (server: string) => `Authorize the ${server} MCP server?`,
  ...
  tips: {
    ...
    } satisfies Record<TipId, { title: string; text: string }> & {
