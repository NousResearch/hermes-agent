...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'この Desktop が操作するゲートウェイを選択します。ローカルは常に復旧先として利用でき、保存済みリモートと Hermes Cloud はシェルを再起動せずに切り替えられます。',
      appliesTo: '適用対象',
      allProfiles: 'すべてのプロファイル',
      defaultConnection: '独自のオーバーライドがないすべてのプロファイルのデフォルト接続。',
      profileConnection: profile =>
        `"${profile}" がアクティブプロファイルのときのみ使用される接続。ローカルに設定するとデフォルトを継承します。`,
      savedConnectionsTitle: '接続',
      savedConnectionsDesc: 'ローカル、Hermes Cloud、保存済みリモートゲートウェイをすぐに切り替えます。',
      addConnection: 'リモートを追加',
      connectedConnection: '使用中',
      connectConnection: '接続',
      connectionShortcut: (id, url) => `${url} · --connection ${id} で起動`,
      deleteConnection: name => `${name} を削除`,
      deleteConnectionConfirm: name => `保存済みゲートウェイ「${name}」を削除しますか？`,
      deletedConnectionTitle: 'ゲートウェイを削除しました',
      deletedConnection: name => `${name} を削除しました。`,
      deleteConnectionFailed: '保存済みゲートウェイを削除できませんでした',
      connectionNameTitle: '接続名',
      connectionNamePlaceholder: 'ホームラボ',
      connectionNameDesc: '表示名から CLI 起動用の固定ショートカット名が作成されます。',
      connectionIdDesc: id => `CLI: hermes desktop --connection ${id}`,
      connectionNameRequiredTitle: '接続名が必要です',
      connectionNameRequired: '保存またはサインインする前に、このリモートゲートウェイに名前を付けてください。',
      ...
      restartingTitle: 'ゲートウェイを切り替え中',
      savedTitle: 'ゲートウェイ設定を保存しました',
      restartingMessage: 'Hermes Desktop は接続を切り替えています。シェルは開いたままです。',
