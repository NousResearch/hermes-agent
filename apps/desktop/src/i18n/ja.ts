...
export const ja = defineLocale({
  ...
  rightSidebar: {
    ...
    terminal: 'ターミナル',
    hostVnc: 'ホスト VNC',
    hostVncShow: 'ホスト VNC を表示',
    hostVncHide: 'ホスト VNC を非表示',
    hostVncSameOrigin: 'ホスト VNC には Hermes Webapp と同じオリジンではなく、専用のオリジンまたはポートが必要です。',
  ...
  preview: {
    ...
    surface: {
      activate: label => `${label} をアクティブにする`,
      dock: label => `${label} をドッキング`,
      detach: label => `${label} を切り離す`,
      minimize: label => `${label} を最小化`,
      maximize: label => `${label} を最大化`,
      restore: label => `${label} を復元`,
      resize: label => `${label} のサイズを変更`,
      snapLayouts: label => `${label} のスナップレイアウト`,
      snapTo: (label, slot) => `${label} を${slot}にスナップ`,
      detachedTitle: label => `${label} は切り離されています`,
      detachedBody:
        'フローティングウィンドウのコントロールで、ドッキング、スナップ、移動、サイズ変更、最小化、または閉じることができます。',
      floating: 'フローティング',
      slots: {
        bottomHalf: '下半分',
        bottomLeftQuarter: '左下4分の1',
        bottomRightQuarter: '右下4分の1',
        centerThird: '中央3分の1',
        leftHalf: '左半分',
        leftThird: '左3分の1',
        leftTwoThirds: '左3分の2',
        rightHalf: '右半分',
        rightThird: '右3分の1',
        rightTwoThirds: '右3分の2',
        topHalf: '上半分',
        topLeftQuarter: '左上4分の1',
        topRightQuarter: '右上4分の1'
      }
    },
    loading: 'プレビューを読み込み中',
