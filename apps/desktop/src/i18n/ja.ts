...
export const ja = defineLocale({
  ...
  codingWorkspace: {
    ...
    agentWorktree: "エージェントのワークツリー",
    agentWorktreeNote: "このワークツリーはチャット中にエージェントが作成しました。",
    newChatHere: "このワークツリーで新しいチャット",
    projectFolder: "プロジェクトフォルダ",
    initializeGit: "Git リポジトリを初期化",
    initializeGitDescription: "空の最初のコミットでリポジトリを作成します。ファイルは未追跡のままです。",
    initializeGitFailed: "Git リポジトリを初期化できませんでした",
    searchProjects: "プロジェクトを検索",
    noProjects: "一致するプロジェクトなし",
    projectsFailed: "プロジェクトを読み込めませんでした",
    browse: "参照…",
    base: "元のブランチ",
    dirty: "未コミットの変更あり",
    clean: "変更なし",
    detached: "分離した HEAD",
    dirtyNotCopied: "未コミットの変更は新規ワークツリーにコピーされません。",
    createOnSend: "今ではなく、最初の送信時に作成します。",
    shared: "このチェックアウトは他のチャットと共有される場合があります。変更は分離されません。",
    preparing: "ワークスペースを準備中",
    workInProject: "プロジェクトで作業…",
    useAsProject: "プロジェクトとして使用",
    newChat: "別のワークスペースで新規チャット…",
    binding: "このチャットは元のワークスペースに固定されています。",
    selectCheckout: "チェックアウトを選択",
    folderFailed: "このフォルダをプロジェクトにできませんでした",
    unavailable: "チャットの接続先が確定するまでワークスペースを選択できません。",

    showControls: "コーディング操作を表示",
    showControlsDescription: "このプロファイルの新規チャットで入力欄の上にプロジェクトと作業場所を表示します。既存のチャットとサイドバー設定は変わりません。",
    saveFailed: "コーディング操作の設定を保存できませんでした",
  },
  common: {
    apply: '適用',
    back: '戻る',
