...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      restartCurrent: '現在のバックエンドを再起動',
      restartCurrentTitle: '現在のバックエンドを再起動',
      restartCurrentDesc:
        'Desktop が管理するローカルまたは SSH バックエンドを再起動します。実行中のターンは中断される場合があります。',
      restartingCurrent: 'バックエンドを再起動中…',
      restartCurrentSuccess: 'バックエンドを再起動し、Desktop を再接続しました。',
      restartCurrentCancelled: '再起動をキャンセルしました。バックエンドは実行中です。',
      restartCurrentRemote: 'リモートバックエンドは Desktop が管理していません。再接続のみ行えます。',
      restartCurrentOwnershipFailed: '再起動を停止しました。所有権を確認できなかったため、プロセスは終了していません。',
      restartCurrentFailed: 'バックエンドの再起動に失敗しました。接続は復旧可能です。',
      diagnostics: '診断',
