...
describe('desktop i18n languages', () => {
  it('normalizes supported locale aliases', () => {
    ...
    expect(normalizeLocale('Dansk')).toBe('da')
    expect(normalizeLocale('Danish')).toBe('da')
  })
  ...
  it('distinguishes exact locale ids from supported config aliases', () => {
    ...
    expect(isSupportedLocaleValue('da-DK')).toBe(true)
    expect(isSupportedLocaleValue('Dansk')).toBe(true)
  ...
  it('returns the persisted config value for supported locales', () => {
    ...
    expect(localeConfigValue('da')).toBe('da')
  })
})
