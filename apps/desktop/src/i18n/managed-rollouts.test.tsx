...
describe('managed rollout locale namespace', () => {
  it('resolves a non-English namespace through the catalog and useI18n path', () => {
    ...
    expect(getManagedRolloutMessages(TRANSLATIONS.zh)).toEqual(managedRolloutsZh)
  })
  ...
  it('translates observed-inventory safety copy and preserves its revision evidence in every locale', () => {
    for (const [locale, messages] of Object.entries(locales)) {
      const revision = messages.labels.inventoryRevision('revision-17', 427)

      expect(revision, locale).toContain('revision-17')
      expect(revision, locale).toContain('427')
      expect(messages.descriptions.inventoryObserved.length, locale).toBeGreaterThan(20)

      if (locale !== 'en') {
        expect(messages.descriptions.inventoryObserved, locale).not.toBe(managedRolloutsEn.descriptions.inventoryObserved)
        expect(revision, locale).not.toBe(managedRolloutsEn.labels.inventoryRevision('revision-17', 427))
      }
    }
  })
