...
export const ru = defineLocale({
  ...
  cron: {
    ...
    modelImpact: {
      ...
      message: count =>
        `${count} ${RU_NOUN(count, 'незакреплённая задача всё ещё использует', 'незакреплённые задачи всё ещё используют', 'незакреплённых задач всё ещё используют')} предыдущую модель. Используйте новую модель или закройте это сообщение, чтобы ничего не менять.`,
      detailMore: (names, remaining) => `${names} и ещё ${remaining}`,
      adopt: 'Использовать новые настройки по умолчанию',
      adopted: count => `${count} ${RU_NOUN(count, 'задача теперь использует', 'задачи теперь используют', 'задач теперь используют')} новые настройки по умолчанию.`,
      adoptFailed: 'Не удалось обновить запланированные задачи.',
