...
export const ru = defineLocale({
  ...
  settings: {
    ...
    appearance: {
      ...
      uiScaleDesc: percent =>
        `Масштабирует текст и элементы управления во всём приложении. Также работают Ctrl/Cmd с +, − и 0. Текущий масштаб: ${percent}%.`,
      ...
      embedsDesc:
        'Расширенные предпросмотры загружаются со сторонних сайтов (YouTube, X и других). «Спрашивать» показывает заглушку до разрешения, «Всегда» загружает автоматически, а «Выкл» оставляет обычные ссылки.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Добавьте анимированного питомца Petdex: он перемещается по окну и реагирует на работу Hermes — бежит во время выполнения инструментов, радуется успехам и грустит при ошибках.',
        restartHint:
        ...
        off: 'Выкл',
        scaleTitle: 'Размер',
        scaleDesc: 'Изменяет размер питомца и применяется сразу во всём приложении.',
        roamTitle: 'Перемещение',
        roamDesc: 'Разрешить питомцу самостоятельно перемещаться по окну во время ожидания.',
        chooseTitle: 'Выберите питомца',
        chooseDesc: 'Выбранный питомец будет установлен при необходимости и сразу активирован.',
        searchPlaceholder: 'Поиск питомцев…',
        unreachable: 'Не удалось открыть галерею Petdex. Проверьте подключение и снова откройте эту страницу.',
        ...
        deleteBody: 'Питомец будет удалён без возможности повторной установки.',
        ...
        noneAvailable: 'Нет доступных питомцев, которых можно включить.',
    ...
    about: {
      ...
      updateReady: count => `Доступно обновление (${countRu(count, 'изменение', 'изменения', 'изменений')}).`,
  ...
  agents: {
    ...
    emptyDesc: 'Когда ход делегирует работу, подагенты отображаются здесь.',
    running: 'Выполняется',
    failed: 'Ошибка',
    done: 'Готово',
    streaming: 'Стриминг',
  ...
  cron: {
    ...
    emptyDescNew:
      'Запланируйте запрос по расписанию cron. Hermes выполнит его и доставит результат в указанное место.',
    ...
    createDesc: 'Запланируйте автоматическое выполнение запроса. Используйте cron-выражение или фразу вроде «каждые 15 минут».',
    nameLabel: 'Название',
  ...
  composer: {
    ...
    snippetsDesc: 'Выберите стартовый запрос для вставки в редактор.',
  ...
  statusStack: {
    ...
    subagents: count => countRu(count, 'подагент', 'подагента', 'подагентов'),
  ...
  preview: {
    ...
    console: {
      ...
      sentMessage: count => `${countRu(count, 'запись', 'записи', 'записей')} добавлено в редактор`
    },
    web: {
  ...
  assistant: {
    thread: {
      ...
      restoreFromHere: 'Восстановить и перезапустить с этого запроса',
      restoreTitle: 'Восстановить эту точку?',
      restoreBody: 'Всё после этого запроса удаляется из разговора, и запрос запускается заново.',
