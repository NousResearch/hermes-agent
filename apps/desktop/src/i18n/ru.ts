...
export const ru = defineLocale({
  ...
  settings: {
    ...
    model: {
      applyAllProfiles: "Применить ко всем профилям",
      applyAllDesc: "Однократно применить провайдера и модель по умолчанию ко всем профилям этого подключения, используя настройки провайдера каждого профиля. Профили можно редактировать отдельно.",
      applyAllSuccess: "Применено",
      applyAllFailed: "Ошибка или не подтверждено",
      applyAllUnavailable: "Провайдер или модель не настроены для этого профиля.",
      applyAllConnectionRequired: "Выберите подключение или подключитесь заново перед применением ко всем профилям.",
      applyAllInterrupted: "Остановлено: подключение или профиль изменились.",
      applyAllNoProfiles: "Нет доступных профилей.",
      applyAllMoaUnavailable: "Пресеты Mixture of Agents нужно настроить в каждом профиле.",
