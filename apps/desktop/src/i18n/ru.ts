...
export const ru = defineLocale({
  ...
  settings: {
    ...
    plugins: {
      title: 'Плагины интерфейса приложения',
      blurb:
        'Функции интерфейса для всего приложения, а не инструменты агента. Изменения применяются сразу. Плагины с обеими частями также нужно включить для каждого профиля агента ниже.',
      ...
      agent: {
        ...
        blurb:
          'Инструменты, навыки и хуки только для выбранного профиля. Изменения сохраняются сразу; перезапустите бэкенд этого профиля, чтобы применить их.',
        ...
        scope: (profile, gateway) => `${profile} · ${gateway} — только этот профиль`,
        restartRequired: 'Требуется перезапуск',
        restartUnknown:
          'Этот бэкенд не сообщает об ожидающих применения изменениях плагинов. После изменения плагина перезапустите его процесс Hermes serve.',
        restartNotice:
          'Изменения сохранены. Перезапустите бэкенд этого профиля, чтобы применить их. Ничто не перезапускается автоматически.',
        restartBackend: 'Перезапустить бэкенд…',
        restartConfirm: 'Перезапустить бэкенд',
        restartTitle: profile => `Перезапустить бэкенд профиля ${profile}?`,
        restartDescription: (profile, gateway) =>
          `Бэкенд приложения для профиля ${profile} на ${gateway} будет перезапущен. Активные запуски в этом бэкенде будут прерваны. Приложение и другие профили останутся открытыми.`,
        restarting: 'Перезапуск бэкенда…',
        restarted: 'Изменения применены',
        restartUnverified:
          'Не удалось подтвердить применение изменений плагинов. Проверьте бэкенд и попробуйте ещё раз.',
        restartUnavailable: 'Перезапустите на хосте',
        restartInstructions: profile =>
          `Приложение не может безопасно перезапустить бэкенд этого подключения. Перезапустите процесс Hermes serve для профиля ${profile} на его хосте, затем проверьте ещё раз. Одно лишь переподключение не перезагружает плагины.`,
        checkAgain: 'Проверить ещё раз',
        activeRuns: count =>
          `Будут прерваны активные запуски: ${count}. Сначала завершите или остановите их, если хотите сохранить результаты работы.`,
        activityUnknown:
          'Не удалось проверить активные запуски. Они могут быть прерваны, если перезапустить бэкенд сейчас.',
        sources: {
          bundled: 'встроенный',
