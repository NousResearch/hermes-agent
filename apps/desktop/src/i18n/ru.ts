  ...
  settings: {
    ...
    appearance: {
      ...
      orb: {
        title: 'Орб мышления',
        intro:
          'Заменяет стандартный спиннер мышления на анимацию жидкого стекла (WebGPU). Создайте свой орб в конфигураторе и вставьте URL ниже, чтобы добавить его в приложение.',
        openConfigurator: 'Открыть конфигуратор орба',
        enableTitle: 'Использовать анимацию орба',
        enableDesc: 'Показывать орб вместо стандартного спиннера, пока Hermes думает.',
        urlTitle: 'Свой URL орба',
        urlDesc:
          'Вставьте URL из конфигуратора — все ползунки редактора закодированы в ссылке, поэтому орб отрисуется в точности как задумано.',
        urlPlaceholder: 'https://lersent001.github.io/orb/#effect=orb-glass-liquid&…',
        urlErrorNoParams: 'В этой ссылке нет параметров орба — скопируйте URL прямо из конфигуратора.',
        urlErrorNotOrb: 'Это не похоже на ссылку конфигуратора орба.',
        reset: 'Использовать орб по умолчанию'
      },
      pet: {
        title: 'Питомец',
        intro:
