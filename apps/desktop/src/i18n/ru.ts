...
export const ru = defineLocale({
  ...
  settings: {
    ...
    mobilePairing: {
      ...
      description: 'Подключите iPhone к этому шлюзу Hermes. Телефон использует тех же агентов, профили, сессии, память и навыки и не запускает отдельного агента.',
      show: 'Показать код подключения',
      retry: 'Повторить',
      hide: 'Скрыть код',
      refresh: 'Обновить код',
      scan: 'Откройте Hermes на iPhone и отсканируйте этот код.',
      expires: 'Для безопасности код скроется через две минуты. Не сообщайте его другим: он даёт доступ к вашему шлюзу.',
      qrAlt: 'QR-код подключения мобильного компаньона Hermes',
      publicUrlRequired: 'Задайте в dashboard.public_url приватный HTTPS-адрес, ведущий к этому шлюзу, затем повторите попытку. Localhost и обычный HTTP использовать нельзя.',
      tokenRequired: 'Это соединение не может выдать многоразовый локальный токен шлюза. Переключитесь на шлюз Hermes с аутентификацией по токену.',
      failed: 'Hermes не удалось создать код мобильного подключения. Проверьте соединение со шлюзом и повторите попытку.'
    },
    gateway: {
