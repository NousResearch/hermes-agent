...
export const ru = defineLocale({
  ...
  sidebar: {
    ...
    row: {
      ...
      stamp: 'Метка',
      stampClear: 'Снять все метки',
      stampCustom: 'Своя метка…',
      stampCustomPlaceholder: 'напр. Слито или Ждём CI',
      stampLimit: max => `Не более ${max} меток на сеанс`,
      stampSaved: label => `Метка: ${label}`,
      stampRemoved: label => `Метка ${label} снята`,
      stampCleared: 'Метка снята',
      stampRemove: label => `Удалить «${label}» из меню меток`,
      stampRestore: 'Вернуть удалённые метки',
      stampAdd: 'Добавить метку…',
      stampAddPlaceholder: 'напр. Блокировано',
      stampEmoji: 'Эмодзи…',
      stampEmojiEmpty: 'Эмодзи не найдены',
      stampEmojiPick: label => `Пометить ${label}`,
      stampEmojiSearch: 'Поиск эмодзи',
      stampColor: label => `Цвет метки «${label}»`,
      stampColorReset: 'Цвет по умолчанию',
