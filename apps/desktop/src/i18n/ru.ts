...
export const ru = defineLocale({
  ...
  settings: {
    ...
    config: {
      ...
      keepAwakeDesc: 'Не позволяйте этой машине спать так долго, иначе ночные пробеги продолжатся. Дисплей все равно может потемнеть.',
    },
    credentials: {
    ...
    gateway: {
      ...
      sshDesc: 'Hermes запускается удаленно через SSH и туннелируется к этому приложению — ничего не нужно запускать или подвергать риску. Требуется работающий SSH-доступ на основе ключей к хосту.',
      sshTrustHint: 'Первый представленный ключ хоста считается доверенным и закрепленным; более поздние изменения не закрываются.',
      sshHostTitle: 'Хозяин',
      ...
      sshErrAuth: 'Аутентификация SSH не удалась. Загрузите свой ключ в ssh-агент (ssh-add) или установите IdentityFile в ~/.ssh/config — Hermes запускает ssh в неинтерактивном режиме.',
      sshErrHostKey: 'Ключ хоста ИЗМЕНИЛСЯ с момента вашего последнего подключения. Убедитесь, что это ожидаемо, затем запустите ssh-keygen -R <host> и повторно подключитесь.',
      sshErrNotInstalled: 'Hermes не установлен на удаленном хосте. Установите его туда (curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh) или задайте путь Hermes.',
      sshErrPlatform: 'Неподдерживаемая удаленная платформа. Режим SSH Hermes Desktop поддерживает удаленные хосты Linux, macOS и Windows.',
    ...
    providers: {
      ...
      localEndpoint: {
        ...
        description: 'Наведите Hermes на любую конечную точку, совместимую с OpenAI (Zyphra, vLLM, llama.cpp, Ollama и т. д.).',
      },
    ...
    toolsets: {
      ...
      terminalBackend: {
        ...
        needsSetupHint: 'Вы можете выбрать этот бэкэнд сейчас — команды не будут выполняться, пока установка не будет завершена.',
      },
    },
  },
  skills: {
    ...
    visionModelHint: 'Vision использует конфигурацию вашей вспомогательной модели — модель с поддержкой изображений выбирается там, а не для каждого поставщика здесь.',
