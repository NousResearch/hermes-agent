...
export const ru = defineLocale({
  ...
  settings: {
    ...
    config: {
      ...
      keepAwakeTitle: 'Не давать компьютеру засыпать во время работы агента',
      keepAwakeDesc: 'Предотвращать системный сон только во время выполнения хода агента. Экран при этом может гаснуть.',
      closedDisplayTitle: 'Предотвращать сон при закрытом экране (экспериментально)',
      closedDisplayDesc:
        'Только macOS. Требуется однократная настройка администратора; поведение зависит от Mac, питания и дисплея.',
      closedDisplaySetupUnavailable: 'Настройка Power Protect доступна только в macOS Desktop.',
      closedDisplaySetupFailed:
        'Настройка Power Protect отменена или завершилась ошибкой. Экспериментальная функция отключена.',
