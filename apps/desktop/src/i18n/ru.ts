...
export const ru = defineLocale({
  ...
  settings: {
    ...
    config: {
      ...
      keepAwakeDesc:
        'Не позволяйте этой машине спать так долго, иначе ночные пробеги продолжатся. Дисплей все равно может потемнеть.'
    },
    credentials: {
    ...
    envActions: {
      ...
      manageInKeys: 'Управление ключами API',
      docs: 'Документация',
    ...
    gateway: {
      ...
      remoteAuthHint:
        'Хостинговые шлюзы используют OAuth или логин/пароль; самохостинг может использовать сессионный токен.',
      ...
      sshDesc:
        'Hermes запускается удаленно через SSH и туннелируется к этому приложению — ничего не нужно запускать или подвергать риску. Требуется работающий SSH-доступ на основе ключей к хосту.',
      sshTrustHint:
        'Первый представленный ключ хоста считается доверенным и закрепленным; более поздние изменения не закрываются.',
      sshHostTitle: 'Хозяин',
      sshHostDesc: 'user@host или псевдоним хоста из ~/.ssh/config.',
      sshHostPick: 'Выберите хост…',
      sshHostPickTitle: 'Хозяин',
      sshHostPickDesc: 'Псевдоним хоста из ~/.ssh/config или «Пользовательский», чтобы ввести его.',
      ...
      sshKeyDesc: 'Путь к секретному ключу. Пусто = ssh-агент или ~/.ssh/config.',
      sshHermesPathTitle: 'Тропа Гермеса (по желанию)',
      sshHermesPathDesc: 'Полный путь к удаленному двоичному файлу Hermes. Пусто = автоматическое обнаружение.',
      sshHermesPathPlaceholder: 'автоматическое обнаружение',
      sshTestConnection: 'Тестирование SSH',
      sshConnect: 'Connect',
      sshButtonsHint: 'Сохранение применяется при следующем запуске. Connect восстанавливает соединение.',
      sshReachable: (host, platform) => `Reachable: ${host} (${platform}) \u2014 Hermes found`,
      sshIncompleteHost: 'Перед подключением введите хост SSH.',
      sshErrUnreachable: 'Не удалось связаться с этим хостом по SSH. Проверьте хост, порт и вашу сеть.',
      sshErrAuth:
        'Аутентификация SSH не удалась. Загрузите свой ключ в ssh-агент (ssh-add) или установите IdentityFile в ~/.ssh/config — Hermes запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
        'Ключ хоста ИЗМЕНИЛСЯ с момента вашего последнего подключения. Убедитесь, что это ожидаемо, затем запустите ssh-keygen -R <host> и повторно подключитесь.',
      sshErrNotInstalled:
        'Hermes не установлен на удаленном хосте. Установите его туда (curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh) или задайте путь Hermes.',
      sshErrPlatform:
        'Неподдерживаемая удаленная платформа. Режим SSH Hermes Desktop поддерживает удаленные хосты Linux, macOS и Windows.',
      sshErrTimeout: 'Время ожидания SSH-соединения истекло. Возможно, хост недоступен или спит.',
      sshErrUpdateRequired: 'Обновите Hermes на удаленном хосте перед подключением к настольному SSH.',
    ...
    model: {
      ...
      appliesDesc:
        'Применяется к новым сессиям. Используйте выбор модели в компоновщике для горячей замены текущего чата.',
      provider: 'Провайдер',
      model: 'Модель',
      applying: 'Применение...',
    ...
    providers: {
      ...
      localEndpoint: {
        ...
        description:
          'Наведите Hermes на любую конечную точку, совместимую с OpenAI (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
    ...
    toolsets: {
      ...
      nousAuthNeededTitle: 'Войдите на портал Nous',
      nousAuthNeededMessage: provider => `${provider} is saved but won't activate until you sign in to Nous Portal.`,
      nousAuthSignIn: 'Войти',
      nousAuthDoneTitle: 'Портал Ноус подключен',
      nousAuthDoneMessage: 'Серверные части вашей подписки теперь активны.',
      ...
      postSetupInstalledHint: 'Установлен. Повторно запускайте настройку, только если что-то сломалось.',
      ...
      terminalBackend: {
        ...
        loading: 'Проверка серверной части выполнения…',
        failedLoad: 'Не удалось загрузить серверные части терминала',
        ready: 'Готовый',
        needsSetup: 'Требуется настройка',
        ...
        needsSetupHint:
          'Вы можете выбрать этот бэкэнд сейчас — команды не будут выполняться, пока установка не будет завершена.'
      }
    }
  },
  skills: {
    ...
    visionModelHint:
      'Vision использует конфигурацию вашей вспомогательной модели — модель с поддержкой изображений выбирается там, а не для каждого поставщика здесь.',
    visionModelLink: 'Выберите модель видения в Настройки → Модели.',
...
Leave blank to keep the ${mode} default.`,
  ...
  sidebar: {
    ...
    projects: {
      ...
      removeWorktreeConfirm:
        'Удалить из git (удаляет каталог worktree; ветвь остаётся) или просто скрыть ряд из боковой панели, оставив worktree на диске.',
      removeWorktreeDirty:
        'В этом worktree есть незафиксированные изменения. Принудительно удалить (отбрасывает их) или скрыть ряд и оставить на диске.',
  ...
  composer: {
    ...
    queueDelete: 'Удалить',
    queueResume: 'Резюме',
    queueResumeTip: 'Paused by Stop — возобновить отправку поворотов в очереди.',
  ...
  onboarding: {
    ...
    fireworksPitch: 'API прямой модели — пограничные модели, размещенные в Fireworks.',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'селф-хостинг',
        description:
          'Укажите Hermes на локальную или самохостинг OpenAI-совместимую конечную точку (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
  ...
  shell: {
    ...
    statusbar: {
      ...
      connectionCloudTooltip: host => `Connected to Hermes Cloud at ${host} \xB7 click to manage`,
      connectionSshTooltip: host => `Connected over SSH to ${host} \xB7 click to manage`,
      connectionRemoteTooltip: host => `Connected to remote backend ${host} \xB7 click to manage`,
      ...
      modelPinned: 'закреплено вами; новые чаты используют это вместо настроек по умолчанию.',
