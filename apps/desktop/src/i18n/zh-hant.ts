...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: '正在啟動 APEX 桌面版…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 APEX 桌面版。',
    ...
    native: {
      ...
      rejectAction: '拒絕',
      inputTitle: '需要輸入',
      inputBody: 'APEX 正在等待你的回應。',
      turnDoneTitle: 'APEX 已完成',
      turnDoneBody: '回覆已就緒。',
      turnErrorTitle: '本輪失敗',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 APEX 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒僅在 APEX 位於背景時觸發。',
      kinds: {
        approval: {
          label: '需要核准',
        ...
        input: {
          label: '需要輸入',
          description: 'APEX 提出了問題，或需要密碼或密鑰。'
        },
        turnDone: {
          label: '回覆就緒',
          description: 'APEX 在背景時完成了一輪對話。'
        },
        turnError: {
          label: '本輪失敗',
          description: '本輪以錯誤結束。'
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'APEX 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'APEX 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'APEX 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'APEX 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'APEX 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      heading: 'APEX 桌面版',
      ...
      automaticUpdatesDesc: 'APEX 會在背景自動檢查更新，並在有可用更新時通知你。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'APEX 桌面版 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 APEX 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 APEX 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc: '將此桌面殼層連線至遠端 APEX 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'APEX 桌面版 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · APEX ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。APEX 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
  ...
  skills: {
    tabSkills: '技能',
    tabSkillsSubtitle: '可為新工作階段啟用的能力。',
    tabToolsets: '工具集',
    all: '全部',
    searchSkills: '搜尋技能...',
  ...
  profiles: {
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 APEX 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
    cloneFrom: '複製來源',
    cloneFromNone: '無（空白）',
  ...
  cron: {
    ...
    newCron: '新排程工作',
    emptyDescNew: '按 cron 表達式排程一個提示詞。APEX 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 APEX 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    latestBodyBackend: '後端正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 APEX 已可安裝。',
    availableTitleBackend: '後端有可用更新',
    availableBodyBackend: '已連接的 APEX 後端有新版本可安裝。',
    availableBodyNoChangelog: '已有新版本可用。此安裝方式無法顯示更新日誌。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 APEX，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 APEX 時會使用新版本。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody: 'APEX 更新程式會在自己的視窗中接管，並在完成後重新開啟 APEX。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 APEX 會自動重新連線。',
    applyingClose: 'APEX 將關閉以套用更新。',
    errorTitle: '更新未完成',
  ...
  onboarding: {
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — APEX 會自動連線',
      loopback: '開啟瀏覽器登入 — APEX 會自動連線',
    ...
    authorizeThere: '請在那裡授權 APEX。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 APEX，連線會自動完成，無需複製或貼上。`,
  ...
  preview: {
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'APEX 正在重新啟動...',
      askRestart: '請 APEX 重新啟動伺服器',
      ...
      finishedRestarting: message => `APEX 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'APEX 無法重新啟動伺服器。',
      stillWorking: 'APEX 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.hermes/config.yaml）。APEX 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'APEX 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。',
    sudoPlaceholder: 'sudo 密碼',
    secretTitle: '需要密鑰',
    secretDesc: 'APEX 需要一個憑證才能繼續。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 APEX 桌面版 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 APEX 桌面版 以儲存圖片',
