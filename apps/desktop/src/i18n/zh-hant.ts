...
export const zhHant = defineLocale({
  ...
  titlebar: {
    ...
    openStarmap: '開啟記憶圖譜',
    openKeybinds: '鍵盤快捷鍵',
    layoutEditor: '版面配置編輯器',
    layoutEditorTitle: '版面配置編輯器 — ⌘+點擊可重設版面配置'
  },
  ...
  keybinds: {
    ...
    actions: {
      ...
      'nav.agents': '開啟代理',
      'session.new': '新增工作階段',
      'session.newTab': '新增工作階段分頁',
      'session.newWindow': '在新視窗開啟工作階段',
      'session.next': '下一個工作階段',
      'session.prev': '上一個工作階段',
      'session.slot.1': '切換到最近的工作階段 1',
      'session.slot.2': '切換到最近的工作階段 2',
      'session.slot.3': '切換到最近的工作階段 3',
      'session.slot.4': '切換到最近的工作階段 4',
      'session.slot.5': '切換到最近的工作階段 5',
      'session.slot.6': '切換到最近的工作階段 6',
      'session.slot.7': '切換到最近的工作階段 7',
      'session.slot.8': '切換到最近的工作階段 8',
      'session.slot.9': '切換到最近的工作階段 9',
      'session.focusSearch': '搜尋工作階段',
      'session.togglePin': '釘選／取消釘選目前的工作階段',
      ...
      'view.toggleReview': '切換審查窗格',
      'view.showFiles': '顯示檔案瀏覽器',
      'view.showTerminal': '切換終端機',
  ...
  settings: {
    ...
    plugins: {
      title: '桌面外掛',
      blurb:
        '載入到此應用程式的 UI 擴充功能——隨組建內建，或放入 desktop-plugins 資料夾（包括 Hermes 自己寫入的）。停用會即時卸載外掛，且重新啟動後仍維持停用。',
      count: n => `已安裝 ${n} 個`,
      openFolder: '開啟外掛資料夾',
      rescan: '重新掃描',
      reveal: '在檔案管理員中顯示',
      ...
      failed: '失敗',
      empty: '尚未安裝任何桌面外掛。',
      kinds: { bundled: '內建', disk: '磁碟上', runtime: '執行階段' }
    },
    notifications: {
      title: '通知',
    ...
    gateway: {
      ...
      remoteAuthHint: '託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
      cloudTitle: 'Hermes Cloud',
      cloudDesc: '登入一次 Hermes Cloud，即可從您帳號中的代理挑選——不需要貼任何 URL。',
      cloudSignInTitle: 'Hermes Cloud',
      cloudSignIn: '登入 Hermes Cloud',
      cloudSignedIn: '已登入 Hermes Cloud',
      cloudNeedsSignIn: '登入 Hermes Cloud 以探索您帳號中的代理。',
      cloudSignedInDesc: '您已登入。請在下方挑選代理；工作階段會自動重新整理。',
      cloudAgentsTitle: '您的代理',
      cloudOrgPickerTitle: '選擇組織',
      cloudOrgSelect: '選取',
      cloudOrgChange: '變更組織',
      cloudOrgRole: role => `角色：${role}`,
      cloudLoadingAgents: '正在載入您的代理…',
      cloudNoAgents: {
        before: '此帳號找不到任何代理。請在 ',
        linkText: 'Nous 入口網站',
        after: ' 建立一個，然後重新整理。'
      },
      cloudRefresh: '重新整理',
      cloudConnect: '連線',
      cloudConnecting: '連線中…',
      cloudDiscoverFailed: '無法載入您的 Hermes Cloud 代理',
      cloudConnectFailed: '無法連線至該代理',
      ...
      cloudConnectedTo: name => `已連線至 ${name}。`,
      cloudAgentProvisioning: '佈建中…',
    ...
    mcp: {
      ...
      serverEnabled: name => `已啟用 ${name} — 套用至新工作階段。`,
      serverDisabled: name => `已停用 ${name} — 套用至新工作階段。`,
      toggleFailed: name => `無法切換 ${name}`,
      tabServers: '伺服器',
      tabCatalog: '目錄',
      ...
      catalogInstallStarted: name => `正在安裝 ${name}... 完成後套用至新工作階段。`,
    ...
    model: {
      ...
      fallbackAdd: '新增備援',
      fallbackEmpty: '沒有備援模型——除非預設模型失敗，否則一律使用預設模型。',
      notInCatalog: '不在此提供方的模型清單中——呼叫可能會退回備援模型。',
      tasks: {
    ...
    providers: {
      ...
      disconnect: '解除連結',
      disconnectInTerminal: '解除連結（在終端機執行移除指令）',
      removeExternalGeneric: provider => `${provider} 由其自身的 CLI 管理——請在該處移除。`,
      removeTerminalConfirm: (provider, command) => `解除連結 ${provider}？將在終端機執行「${command}」以清除憑證。`,
      removeTerminalRunning: provider => `正在終端機執行 ${provider} 解除連結…`,
      removedTitle: '帳號已移除',
    ...
    toolsets: {
      ...
      modelInUse: '使用中',
      modelDefault: '預設',
      modelInactiveHint: '請先選擇此後端，再變更其模型。',
  ...
  skills: {
    ...
    hub: {
      ...
      connectedHubs: '已連線的中心：',
      featured: '精選技能',
      landingHint: '搜尋技能中心，瀏覽來自官方索引、GitHub 與社群來源的可安裝技能。',
      noResults: '技能中心裡找不到符合的技能。',
      ...
      trust: { builtin: '內建', trusted: '可信', community: '社群' },
      verdictSafe: '安全',
      verdictCaution: '注意',
      verdictDangerous: '危險',
      policyAllow: '允許安裝',
      policyAsk: '安裝前請審查',
      policyBlock: '安裝已被原則封鎖',
  ...
  starmap: {
    ...
    share: '分享圖譜',
    shareHint: '複製代碼以分享此圖譜，或貼上代碼以載入。僅包含版面配置，不含您的記憶或技能內容。',
    shareTitle: '匯入／匯出圖譜',
    ...
    importSuccess: nodes => `已載入含 ${nodes} 個節點的圖譜。`,
    importedBadge: '匯入的圖譜',
    resetToMine: '回到我的圖譜'
  },
  agents: {
  ...
  commandCenter: {
    ...
    maintenance: {
      runOps: '診斷',
      doctor: '執行 doctor',
      doctorDesc: '健康檢查安裝、設定與提供方',
      securityAudit: '安全性稽核',
      securityAuditDesc: '掃描設定與技能中的風險設定',
      backup: '建立備份',
      backupDesc: '將設定、記憶、技能與工作階段壓縮為 zip',
      debugShare: '除錯分享',
      debugShareDesc: '上傳去識別化的報告與記錄，取得可分享連結（6 小時後自動刪除）',
      debugShareRunning: '正在上傳除錯報告...',
      debugShareLinks: '分享連結',
      debugShareFailed: '除錯分享失敗',
      copyLink: '複製連結',
      linkCopied: '連結已複製',
      curator: '技能策展器',
      curatorDesc: '在背景審查並封存過時的代理自建技能',
      curatorPaused: '已暫停',
      curatorActive: '作用中',
      curatorDisabled: '已停用',
      ...
      memoryProvider: name => `作用中提供方：${name}`,
      builtinMemory: '內建',
      memoryFile: '代理記憶（MEMORY.md）',
      userFile: '使用者檔案（USER.md）',
      bytes: size => size,
      empty: '空',
      resetMemory: '重設記憶',
      resetUser: '重設使用者檔案',
      ...
      resetFailed: '記憶重設失敗',
      actionStarted: name => `${name} 已開始——正在追蹤記錄...`,
  ...
  sidebar: {
    ...
    projects: {
      sectionLabel: '專案',
      reorder: label => `重新排列 ${label}`,
      toggle: label => `切換 ${label} 的工作階段`,
      back: '全部專案',
      newButton: '新增專案',
      createTitle: '新增專案',
  ...
  onboarding: {
    ...
    localApiKeyPlaceholder: 'API 金鑰（選填——僅在您的端點需要時填寫）',
    couldNotSave: '無法儲存憑證。',
    connecting: '連線中',
    update: '更新',
    flowSubtitles: {
  ...
  zones: {
    ...
    minimize: '最小化',
    restore: '還原',
    closeRunningTitle: '關閉執行中的分頁？',
    closeRunningBody:
      '此聊天仍在工作（或正在等待您的輸入）。關閉分頁只會隱藏它——工作階段會保留進度，可從側邊欄重新開啟。',
