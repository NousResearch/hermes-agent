...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    localModels: {
      ...
      pillDiskBackedTip: size => `${size} 查找表會按需從磁碟讀取。這與一般系統記憶體溢出是分開的。`,
      pillTooBig: '超出本機記憶體',
      browseTitle: '發現更多模型',
      browseHint:
        '選擇發行者提供的 GGUF 量化版本。Hermes 會在使用前檢查完成的檔案，包括大型查找表是否保持磁碟支援；不會重寫模型。',
      browseFitEstimate: label => `估算：${label}`,
      browseEstimateTip:
        '這只根據下載大小。下載後，Hermes 會讀取 GGUF 標頭，並說明大型查找表是磁碟支援還是使用一般記憶體。',
      ...
      lookupResident: '查找表使用一般記憶體',
      lookupResidentTip: size => `${size} 查找表屬於模型的一般記憶體用量；它不是磁碟支援，也不會單獨卸載。`,
      lookupUpdateRequired: '磁碟支援查找表需要更新',
      lookupUpdateRequiredTip: engine =>
        `大型查找表只有在 llama.cpp ${engine} 或更新版本中才會保持磁碟支援。使用此模型前請更新本機引擎。`,
      lookupUnknown: '無法檢查查找表',
      lookupUnknownTip: 'Hermes 無法讀取這個 GGUF 標頭，因此無法確認查找表將如何放置。',
      browseDownloadConfirm: size => `這個發佈者提供的 GGUF 為 ${size}，可能不適合這台機器。仍要下載嗎？`,
