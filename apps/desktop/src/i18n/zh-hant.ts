...
export const zhHant = defineLocale({
  ...
  sidebar: {
    ...
    row: {
      ...
      stampClear: '清除全部標記',
      stampCustom: '自訂標記…',
      stampCustomPlaceholder: '例如：已合併 或 等待 CI',
      stampLimit: max => `每個工作階段最多 ${max} 個標記`,
      stampSaved: label => `已標記為 ${label}`,
      stampRemoved: label => `已移除 ${label}`,
      stampCleared: '已清除標記',
      stampRemove: label => `從標記選單中刪除「${label}」`,
      stampRestore: '恢復已刪除的標記',
      stampAdd: '新增標記…',
      stampAddPlaceholder: '例如：已封鎖',
      stampEmoji: '表情…',
      stampEmojiEmpty: '找不到表情',
      stampEmojiPick: label => `標記為 ${label}`,
      stampEmojiSearch: '搜尋表情',
      stampColor: label => `「${label}」的顏色`,
      stampColorReset: '預設顏色',
      deleteTitle: '刪除會話？',
