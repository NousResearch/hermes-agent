...
export const zhHant = defineLocale({
  ...
  rightSidebar: {
    ...
    terminal: '終端機',
    hostVnc: '主機 VNC',
    hostVncShow: '顯示主機 VNC',
    hostVncHide: '隱藏主機 VNC',
    hostVncSameOrigin: '主機 VNC 必須使用獨立的來源或連接埠，不能與 Hermes Webapp 同源。',
  ...
  preview: {
    ...
    closePane: '關閉預覽窗格',
    surface: {
      activate: label => `啟用 ${label}`,
      dock: label => `停駐 ${label}`,
      detach: label => `分離 ${label}`,
      minimize: label => `最小化 ${label}`,
      maximize: label => `最大化 ${label}`,
      restore: label => `還原 ${label}`,
      resize: label => `調整 ${label} 大小`,
      snapLayouts: label => `${label} 的貼齊版面`,
      snapTo: (label, slot) => `將 ${label} 貼齊到${slot}`,
      detachedTitle: label => `${label} 已分離`,
      detachedBody: '使用浮動視窗控制項進行停駐、貼齊、移動、調整大小、最小化或關閉。',
      floating: '浮動',
      slots: {
        bottomHalf: '下半部',
        bottomLeftQuarter: '左下四分之一',
        bottomRightQuarter: '右下四分之一',
        centerThird: '中央三分之一',
        leftHalf: '左半部',
        leftThird: '左側三分之一',
        ...
        topHalf: '上半部',
        topLeftQuarter: '左上四分之一',
        topRightQuarter: '右上四分之一'
      }
    },
    loading: '正在載入預覽',
    unavailable: '預覽不可用',
