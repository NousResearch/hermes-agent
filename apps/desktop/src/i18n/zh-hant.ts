...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        '選擇此 Desktop 要控制的閘道。本機閘道始終可作為復原入口；已儲存的遠端閘道與 Hermes Cloud 可在不重新啟動介面的情況下切換。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      defaultConnection: '預設連線適用於所有沒有自訂覆寫的設定檔。',
      profileConnection: profile => `僅當「${profile}」為作用中設定檔時使用此連線。設為本機可繼承預設連線。`,
      savedConnectionsTitle: '連線',
      savedConnectionsDesc: '在本機、Hermes Cloud 與已儲存的遠端閘道之間立即切換。',
      addConnection: '新增遠端閘道',
      connectedConnection: '目前使用',
      connectConnection: '連線',
      connectionShortcut: (id, url) => `${url} · 使用 --connection ${id} 啟動`,
      deleteConnection: name => `刪除 ${name}`,
      deleteConnectionConfirm: name => `刪除已儲存的閘道「${name}」？`,
      deletedConnectionTitle: '閘道已刪除',
      deletedConnection: name => `已刪除 ${name}。`,
      deleteConnectionFailed: '無法刪除已儲存的閘道',
      connectionNameTitle: '連線名稱',
      connectionNamePlaceholder: '家庭實驗室',
      connectionNameDesc: '友善名稱；Hermes 會建立供 CLI 啟動使用的固定捷徑名稱。',
      connectionIdDesc: id => `CLI 捷徑：hermes desktop --connection ${id}`,
      connectionNameRequiredTitle: '需要連線名稱',
      connectionNameRequired: '請先為此遠端閘道命名，再儲存或登入。',
      ...
      restartingTitle: '正在切換閘道',
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'Hermes Desktop 正在切換連線，介面會保持開啟。',
