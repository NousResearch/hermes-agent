...
export const zh: Translations = {
  ...
  rightSidebar: {
    ...
    terminal: '终端',
    hostVnc: '主机 VNC',
    hostVncShow: '显示主机 VNC',
    hostVncHide: '隐藏主机 VNC',
    hostVncSameOrigin: '主机 VNC 必须使用独立的来源或端口，不能与 Hermes Webapp 同源。',
  ...
  preview: {
    ...
    closePane: '关闭预览面板',
    surface: {
      activate: label => `激活 ${label}`,
      dock: label => `停靠 ${label}`,
      detach: label => `分离 ${label}`,
      minimize: label => `最小化 ${label}`,
      maximize: label => `最大化 ${label}`,
      restore: label => `还原 ${label}`,
      resize: label => `调整 ${label} 大小`,
      snapLayouts: label => `${label} 的贴靠布局`,
      snapTo: (label, slot) => `将 ${label} 贴靠到${slot}`,
      detachedTitle: label => `${label} 已分离`,
      detachedBody: '使用浮动窗口控件进行停靠、贴靠、移动、调整大小、最小化或关闭。',
      floating: '浮动',
      slots: {
        bottomHalf: '下半屏',
