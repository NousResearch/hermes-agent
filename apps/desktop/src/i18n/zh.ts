...
export const zh = defineLocale({
  ...
  sidebar: {
    ...
    row: {
      ...
      stampClear: '清除全部标记',
      stampCustom: '自定义标记…',
      stampCustomPlaceholder: '例如：已合并 或 等待 CI',
      stampLimit: max => `每个会话最多 ${max} 个标记`,
      stampSaved: label => `已标记为 ${label}`,
      stampRemoved: label => `已移除 ${label}`,
      stampCleared: '已清除标记',
      stampRemove: label => `从标记菜单中删除“${label}”`,
      stampRestore: '恢复已删除的标记',
      stampAdd: '添加标记…',
      stampAddPlaceholder: '例如：已阻止',
      stampEmoji: '表情…',
      stampEmojiEmpty: '未找到表情',
      stampEmojiPick: label => `标记为 ${label}`,
      stampEmojiSearch: '搜索表情',
      stampColor: label => `“${label}”的颜色`,
      stampColorReset: '默认颜色',
      deleteTitle: '删除会话？',
