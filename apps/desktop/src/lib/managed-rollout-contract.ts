...
// eslint-disable-next-line no-control-regex -- reject control bytes at the IPC boundary
const LINE_CONTROL_RE = /[\x00\r\n]/
// eslint-disable-next-line no-control-regex -- reject control bytes in source identity
...
function stringValue(value: unknown, path: string, allowEmpty = false): string {
  if (typeof value !== 'string' || (!allowEmpty && value.length === 0) || LINE_CONTROL_RE.test(value)) {
...
export function validateReviewedSourceBinding(value: unknown, path = 'reviewedSource'): ReviewedSourceBinding {
  ...
  if (!(/^(?:[A-Za-z]:[\\/]|\/)/.test(root)) || CONTROL_RE.test(root))
    {fail(`${path}.repositoryRoot`, 'expected absolute path')}

  if (/\s|[?#]/.test(origin) || CONTROL_RE.test(origin)) {fail(`${path}.originUrl`, 'credential-free remote required')}
  ...
  if (/\s/.test(ref) || CONTROL_RE.test(ref) || /\s/.test(profile) || CONTROL_RE.test(profile))
