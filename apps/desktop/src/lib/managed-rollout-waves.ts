...
// eslint-disable-next-line no-control-regex -- installation ids cannot contain NUL or newlines
const INVALID_INSTALL_ID_RE = /[\x00\r\n]/
...
function uniqueNonEmpty(values: readonly string[], label: string): string[] {
  ...
  if (copy.some(value => value.length === 0 || INVALID_INSTALL_ID_RE.test(value))) {invalid(`invalid-${label}`)}
...
export function makeWaves(selected: readonly string[], canaries: readonly string[], size: number): string[][] {
  ...
  if (normalizedCanaries.some(value => value.length === 0 || INVALID_INSTALL_ID_RE.test(value))) {invalid('invalid-canaries')}
