...
export function annotateLocalRosterRoutes(
  ...
  return rows.map(row => {
    ...
    const route = liveId ? live : candidates.length === 1 ? candidates[0] : local.length === 1 ? local[0] : undefined
...
/** Shared snapshot for the roster pane and cold-cache priming. */
async function fetchRosterSnapshot(activeConnectionId: null | string | undefined): Promise<RosterSnapshot> {
  ...
  if (typeof host.agents === 'function') {
    try {
      ...
      return {
        ...merged,
        profiles: routed.map(row => (row?.remoteSource ? { ...row, sourceReachable: false } : row)),
        fetchedAt: issuedAt
      }
    }
  }
