import { botMentionTag, mentionNameForms } from './data'
...
export function buildGroupChatTurnPrompt({ groupName, members, userName, viewer, deltaLines }: GroupChatTurnPromptInput) {
  ...
  // The mention tag bots are told to use for the human: the room-local name's
  // slug when one is set (and slug-able), else the @user sentinel. The slug
  // never collides with @user/@all — mentionNameForms rejects those forms.
  const userNameSlug = mentionNameForms(userName)[0]
  const userNameTag = userNameSlug ? `@${userNameSlug}` : '@user'

  return [
    `[Group chat: "${groupName}"] You are @${botMentionTag(viewer)}, one participant in a group chat with ${peerNames || 'no one else yet'} and the user${userName && userName.trim() ? ` (${userName.trim()})` : ''}.`,
    '',
    ...
    `- Mention a teammate as @name to pull them in; mention ${userNameTag} only for a judgment call or a result the user needs. Do not repeat points already made.`,
