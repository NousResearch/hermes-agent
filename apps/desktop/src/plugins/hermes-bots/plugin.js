...
import { $botChatFocused, $botsPaneVisible, $focusedBotOwner, $openBotChat, $selectedBot, $selectedRosterHydrated, $selectedRosterKey, focusedMentionProfile } from './bot-state';
...
import { $botMeta, $lastRoster, botHandle, botMentionTag, botSelectionKey, cachedUnionRoster, isActiveRosterBot, migrateBotMeta, primeRoster, resolveRosterMentions } from './data';
import { $groupChats, $groupChatWorkspace, assignLegacyThreads, handleSessionsGatewayTransition, hydrateGroupChatTombstones, pullGroupChatServerState, scheduleGroupChatServerSync, setGroupChatSyncDisposed, stopGroupChatServerSync, sweepGroupChatMembersForRemovedConnection, updateGroupChat } from './group-chat';
...
import { botChatOwnsWorkspace, BotsPane, releaseStaleOpenBotChat, selectedRosterBot, sessionOwnsWorkspace } from './roster-pane';
...
export default {
    ...
    register(ctx) {
        ...
        let tombstonesHydrated = Promise.resolve();
        try {
            // @ts-expect-error TODO(bot-mode-types): PluginStorage.get requires a fallback argument.
        ...
        const primeOnGatewayOpen = (state) => {
        ...
        ctx.register({
            ...
            data: {
                ...
                collapsible: true,
                hideOnly: true,
                tabTitle: () => _jsx(LocalizedTabTitle, { select: t => t.common.bots }),
        ...
        const registerRoutinesPane = (restoreDismissed) => {
            const dispose = ctx.register({
                ...
                data: {
                    tabTitle: () => _jsx(LocalizedTabTitle, { select: t => t.cron.title }),
        ...
        if (typeof host.paneVisibility === 'function') {
            ...
            const stopSidebarSync = $sidebarVisible.listen(visible => {
                ...
                if (visible) {
                    ...
                    // blocked target).
                    if (group) {
                        setBotsWorkspaceOwner(groupWorkspaceOwnerKey(group), null, 'New group conversations start in the group composer.');
                    }
                    else if (selected) {
            ...
            const stopFocusSync = typeof focusStore?.listen === 'function'
            ...
            // host.onEvent.
            const stopReclaimSync = typeof host.onEvent === 'function'
        ...
        ctx.register({
            ...
            data: {
                render: ({ sessionId }) => _jsx(BotChatEmpty, { sessionId: sessionId })
            }
        });
        ctx.register({
            id: 'new-agent',
            ...
            data: {
                handler: async (draft) => {
                    ...
                    const lines = mentionedBots.map(bot => {
                        ...
                        const target = bot.remoteSource && bot.connectionId ? `${bot.name}@${bot.connectionId}` : botHandle(bot.name);
