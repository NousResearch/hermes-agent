...
function Add({ label, onAdd }: { label: string; onAdd: (name: string) => Promise<unknown> }) {
  ...
  return <div className="flex gap-1"><Input aria-label={label} onChange={e => setValue(e.target.value)} value={value} /><Button disabled={!value.trim() || busy} onClick={() => void submit()} size="xs">{busy ? '…' : label}</Button></div>
}
