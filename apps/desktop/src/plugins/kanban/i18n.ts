...
const pl: KanbanMessages = {
  ...
  countTip: (running, ready) => `Kanban — w toku: ${running}, gotowe: ${ready}`,
  col: {
    triage: { label: 'Wstępne', help: 'Surowe pomysły — agent dopracowuje z nich specyfikację.' },
    todo: { label: 'Do zrobienia', help: 'Oczekuje na zależności albo nie ma przypisanego profilu.' },
    scheduled: { label: 'Zaplanowane', help: 'Oczekuje na nadejście zaplanowanego czasu.' },
    ready: { label: 'Gotowe', help: 'Zależności spełnione — przypisz profil, a dyspozytor uruchomi zadanie.' },
    running: {
      label: 'W toku',
      help: 'Zadanie podjęte przez wykonawcę — agent nad nim pracuje. Stan ustawia dyspozytor.'
    },
    blocked: { label: 'Zablokowane', help: 'Wykonawca poprosił o pomoc człowieka.' },
    review: { label: 'Weryfikacja', help: 'Agent weryfikujący sprawdza pracę. Stan ustawia dyspozytor.' },
    done: { label: 'Ukończone', help: 'Zadanie ukończone; zależne zadania podrzędne stają się gotowe.' },
    archived: { label: 'Zarchiwizowane', help: 'Ukryte w domyślnym widoku tablicy.' }
  },
  locked: {
    review: 'Dyspozytor ustawia stan Weryfikacja, gdy agent weryfikujący podejmie kartę.',
    running: 'Dyspozytor ustawia stan W toku, gdy wykonawca podejmie kartę.',
    scheduled: 'Stan Zaplanowane wymaga czasu wznowienia — ustawiają go agenci; nie można przeciągnąć tu karty.'
  },
  arcRunning: 'Agent pracuje teraz nad tym zadaniem.',
  arcStale: 'Zadanie podjęte, ale brak pulsu wykonawcy od ponad 2 minut — dyspozytor podejmie je ponownie.',
  title: 'Kanban',
  ...
  open: 'Otwórz',
  select: modifier => `Wybierz (${modifier}-kliknięcie)`,
  deselect: 'Odznacz',
  moveTo: label => `Przenieś do: ${label}`,
  delete: 'Usuń',
  reviewChecking: 'Agent weryfikujący sprawdza ukończoną pracę.',
  attachedTip: name => `Profil ${name} jest przypisany — dyspozytor przekaże mu zadanie przy następnym cyklu (≤1 min).`,
  orchestratorTip: name =>
    `Profil ${name} (orkiestrator) podejmie zadanie przy następnym cyklu i napisze specyfikację.`,
  autoAssignTip: name =>
    `Przy następnym cyklu dyspozytora automatycznie przypisze profil „${name}” (kanban.default_assignee).`,
  wontRun: 'nie uruchomi się',
  wontRunTip:
    'Gotowe karty są uruchamiane dopiero po przypisaniu profilu. Otwórz kartę i wybierz profil albo skonfiguruj profil domyślny w ustawieniach orkiestracji.',
  noHeartbeat: 'brak pulsu',
  expand: label => `Rozwiń: ${label}`,
  collapse: label => `Zwiń: ${label}`,
  newTaskIn: label => `Nowe zadanie w kolumnie „${label}”`,
  empty: 'Pusto',
  ...
  groupRunning: 'Grupuj zadania w toku według profilu',
  nSelected: n => `Wybrano: ${n}`,
  moveToShort: 'Przenieś do',
  assign: 'Przypisz',
  unassignAction: 'Usuń przypisanie',
  archive: 'Archiwizuj',
  clearSelection: 'Wyczyść wybór (Esc)',
  refused: 'odrzucono',
  bulkFailed: (failed, total, err) => `Nie udało się: ${failed} z ${total} — ${err}. Nieudane karty pozostają wybrane.`,
  titlePlaceholderTriage: 'Wstępny pomysł — agent dopracuje szczegóły',
  titlePlaceholder: 'Tytuł',
  descPlaceholder: 'Opis (opcjonalnie)',
  priority: 'Priorytet',
  workspace: 'Obszar roboczy',
  boardDefaultSuffix: ' · domyślny dla tablicy',
  workspaceOverride: 'Ścieżka obszaru roboczego (opcjonalne nadpisanie)',
  model: 'Model',
  modelInherit: 'Domyślny dla profilu',
  modelClear: 'Usuń nadpisanie modelu',
  modelHint:
    'Uruchamia to zadanie na określonym modelu i z wybraną głębokością rozumowania. Brak wartości używa ustawień przypisanego profilu.',
  workspaceInherit: 'Dziedziczy katalog projektu tablicy',
  workspaceInheritDir: dir => `Pozostaw puste, aby użyć katalogu ${dir}`,
  workspaceInheritGeneric: 'Pozostaw puste, aby odziedziczyć katalog projektu tablicy.',
  assignee: 'Przypisany profil',
  defaultOption: name => `${name} (domyślny)`,
  parkedOption: 'nieprzypisane (wstrzymane — nie uruchomi się)',
  skills: 'Umiejętności (rozdzielone przecinkami)',
  skillsPlaceholder: 'tłumaczenia, github',
  parent: 'Zadanie nadrzędne (blokuje do czasu ukończenia)',
  noParent: '— brak zadania nadrzędnego —',
  goalMode: 'Tryb celu (wykonawca działa, aż sędzia potwierdzi ukończenie)',
  creating: 'Tworzenie…',
  ...
  estimateTip: 'Przybliżone oszacowanie tokenów i złożoności przez model pomocniczy — wykonuje wywołanie modelu.',
  estimateTipLong:
    'Wykonuje szybkie wywołanie modelu pomocniczego, aby oszacować tokeny i złożoność. To wskazówka, nie rachunek.',
  roughEstimate: 'Przybliżone oszacowanie',
  tokUnit: 'tok.',
  couldNotEstimate: 'Nie udało się oszacować',
  complexity: { S: 'Małe', M: 'Średnie', L: 'Duże' },
  introBody:
    'To nie Ty uruchamiasz karty — robią to agenci. Umieść kartę w kolumnie Gotowe i przypisz profil, a agent podejmie ją w ciągu minuty. Bez przypisanego profilu zadanie się nie uruchomi. Wstępne: agent najpierw przepisuje pomysł jako kompletne zadanie. Do zrobienia: oczekuje na inne karty. Zaplanowane: oczekuje na czas uruchomienia. W toku i Weryfikacja: kolumny agentów — nie przenoś tam kart. Zablokowane: czeka na Ciebie. Wyniki pojawią się na karcie.',
  introGotIt: 'Rozumiem',
  evtCreated: (where, assignee) =>
    `utworzono${where ? ` · kolumna: ${where}` : ''}${assignee ? ` · przypisany profil: ${assignee}` : ''}`,
  evtMovedTo: col => `przeniesiono · nowa kolumna: ${col}`,
  evtParentReopened: parent => `ponownie otwarto zadanie nadrzędne: ${parent}`,
  evtAssignedTo: assignee => `przypisany profil: ${assignee}`,
  evtUnassigned: 'usunięto przypisanie',
  evtCommentBy: author => `komentarz — ${author}`,
  evtClaimedReview: 'podjęte przez agenta weryfikującego',
  evtClaimedWorker: 'podjęte przez wykonawcę',
  evtWorkerStarted: 'wykonawca rozpoczął pracę',
  evtCompleted: 'ukończono',
  evtBlocked: 'zablokowano — potrzebna pomoc człowieka',
  evtUnblocked: col => `odblokowano${col ? ` → ${col}` : ' → Gotowe'}`,
  evtReclaimed: 'podjęto ponownie — zwrócono do kolejki',
  evtSpecified: 'agent wstępny napisał specyfikację',
  evtPromoted: 'zależności ukończone — przeniesiono do Gotowe',
  evtScheduled: 'zaplanowano na później',
  evtArchived: 'zarchiwizowano',
  evtReprioritized: priority => `ustawiono priorytet: ${priority}`,
  someone: 'ktoś',
  metaPriority: 'Priorytet',
  metaTenant: 'Przestrzeń',
  metaCreatedBy: 'Utworzone przez',
  metaCreated: 'Utworzono',
  metaWorkerPid: 'PID wykonawcy',
  readyUnassignedTitle: 'Gotowe, ale nieprzypisane — ta karta się nie uruchomi.',
  readyUnassignedBody:
    'Dyspozytor podejmuje tylko gotowe karty z przypisanym profilem. Wybierz profil w polu Przypisany profil powyżej (albo ustaw profil domyślny w ustawieniach orkiestracji), a zadanie uruchomi się w ciągu minuty.',
  ...
  commentsHelpRunning:
    'To zadanie jest w toku. Twoja wiadomość zostanie włączona do bieżącej tury wykonawcy w ciągu kilku sekund — bez blokowania i odblokowywania. Opcja „Ponownie umieść w kolejce z notatką” zamiast tego uruchomi zadanie od początku z notatką w kontekście.',
  commentsHelp:
    'Komentarze są dodawane do wątku zadania. Gdy wykonawca podejmie zadanie, przeczyta je jako część kontekstu.',
  send: 'Wyślij',
  comment: 'Komentarz',
  messageWorker: 'Napisz do aktywnego wykonawcy…',
  addComment: 'Dodaj komentarz…',
  deliveredLive: 'Dostarczono aktywnemu wykonawcy w ciągu kilku sekund.',
  requeueWithNote: 'Ponownie umieść w kolejce z notatką',
  notePosted: 'Dodano notatkę — wykonawca wrócił do kolejki',
  ...
  taskActions: 'Działania zadania',
  copyTaskId: 'Skopiuj identyfikator zadania',
  ...
  projectHintPre:
    'Nowe zadania są uruchamiane w repozytorium projektu (osobne drzewo robocze dla każdego zadania); podczas tworzenia zadania nadal można nadpisać jego obszar roboczy. Zarządzaj projektami poleceniem ',
  projectHintCmd: 'hermes project',
  createBoard: 'Utwórz tablicę',
  orchestratorProfile: 'Profil orkiestratora',
  defaultAssignee: 'Domyślnie przypisany profil',
  defaultParen: '(domyślny)',
  autoDecompose: 'Automatycznie rozkładaj zadania wstępne',
...
export const KANBAN_LOCALES: PluginLocaleBundles = { en, pl, ja, zh, 'zh-hant': zhHant }
