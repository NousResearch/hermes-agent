...
const de: KanbanMessages = {
  ...
  countTip: (running, ready) => `Kanban — ${running} läuft, ${ready} bereit`,
  col: {
    triage: { label: 'Triage', help: 'Rohideen — ein Specifier arbeitet die Spezifikation aus.' },
    todo: { label: 'Zu erledigen', help: 'Wartet auf Abhängigkeiten oder ist nicht zugewiesen.' },
    scheduled: { label: 'Geplant', help: 'Wartet auf einen geplanten Zeitpunkt.' },
    ready: { label: 'Bereit', help: 'Abhängigkeiten erfüllt — Profil zuweisen, dann führt es der Dispatcher aus.' },
    running: { label: 'Läuft', help: 'Von einem Worker übernommen — ein Agent arbeitet daran. Wird vom Dispatcher gesetzt.' },
    blocked: { label: 'Blockiert', help: 'Der Worker fragt nach menschlicher Eingabe.' },
    review: { label: 'Review', help: 'Ein Review-Agent prüft die Arbeit. Wird vom Dispatcher gesetzt.' },
    done: { label: 'Erledigt', help: 'Abgeschlossen; abhängige Kind-Aufgaben werden bereit.' },
    archived: { label: 'Archiviert', help: 'In der Standard-Boardansicht ausgeblendet.' }
  },
  locked: {
    ...
    scheduled: 'Geplant braucht eine Weckzeit — Agents setzen sie; es kann nichts hineingezogen werden.'
  },
  arcRunning: 'Ein Agent arbeitet gerade daran.',
  arcStale: 'Übernommen, aber seit 2+ Minuten kein Worker-Heartbeat — der Dispatcher wird sie zurückholen.',
  title: 'Kanban',
  ...
  delete: 'Löschen',
  reviewChecking: 'Ein Review-Agent prüft die abgeschlossene Arbeit.',
  attachedTip: name => `${name} ist angehängt — der Dispatcher übergibt es beim nächsten Takt (≤1 Min.).`,
  orchestratorTip: name => `${name} (der Orchestrator) nimmt es beim nächsten Takt auf und schreibt die Spezifikation.`,
  autoAssignTip: name => `Wird beim nächsten Dispatch-Takt automatisch „${name}“ zugewiesen (kanban.default_assignee).`,
  wontRun: 'wird nicht ausgeführt',
  wontRunTip:
    'Bereite Karten laufen erst, wenn ein Profil zugewiesen ist. Öffne die Karte und setze einen Bearbeiter, oder konfiguriere einen Standard-Bearbeiter in den Orchestrierungseinstellungen.',
  ...
  titlePlaceholderTriage: 'Rohidee — ein Specifier arbeitet sie aus',
  ...
  model: 'Modell',
  modelInherit: 'Profilstandard',
  modelClear: 'Modell-Überschreibung löschen',
  modelHint: 'Führt diese Aufgabe auf einem bestimmten Modell und mit einer bestimmten Denktiefe aus. Nicht gesetzt verwendet das zugewiesene Profil.',
  ...
  parkedOption: 'nicht zugewiesen (geparkt — wird nicht ausgeführt)',
  skills: 'Skills (kommagetrennt)',
  skillsPlaceholder: 'translation, github',
  parent: 'Übergeordnet (blockiert, bis es fertig ist)',
  noParent: '— keine übergeordnete Aufgabe —',
  goalMode: 'Zielmodus (Worker läuft, bis ein Richter bestätigt, dass es fertig ist)',
  creating: 'Erstelle…',
  ...
  estimateTip: 'Grobe Token- + Komplexitätsschätzung vom Hilfsmodell — ruft ein Modell auf.',
  estimateTipLong: 'Führt einen schnellen Hilfsmodell-Aufruf aus, um Tokens + Komplexität zu schätzen. Ein grober Anhaltspunkt, keine Rechnung.',
  roughEstimate: 'Grobe Schätzung',
  tokUnit: 'tok',
  couldNotEstimate: 'Schätzung nicht möglich',
  complexity: { S: 'Klein', M: 'Mittel', L: 'Groß' },
  introBody:
    'Nicht du betreibst die Karten — Agents tun das. Lege eine Karte mit Bearbeiter in Bereit, und ein Agent nimmt sie innerhalb einer Minute auf. Ohne Bearbeiter kein Lauf. Triage: Ein Agent schreibt die Idee zuerst in eine richtige Aufgabe um. Zu erledigen: wartet auf andere Karten. Geplant: wartet auf einen Timer. Läuft und Review: die Spuren der Agents, nicht anfassen. Blockiert: wartet auf dich. Ergebnisse kommen auf die Karte zurück.',
  introGotIt: 'Verstanden',
  evtCreated: (where, assignee) =>
    `erstellt${where ? ` in ${where}` : ''}${assignee ? ` · ${assignee} zugewiesen` : ''}`,
  evtMovedTo: col => `verschoben nach ${col}`,
  evtParentReopened: parent => `übergeordnete Aufgabe ${parent} wieder geöffnet`,
  ...
  evtBlocked: 'blockiert — braucht menschliche Eingabe',
  evtUnblocked: col => `entsperrt${col ? ` → ${col}` : ' → Bereit'}`,
  evtReclaimed: 'zurückgeholt — zurück in der Warteschlange',
  evtSpecified: 'Spezifikation vom Triage-Agent geschrieben',
  evtPromoted: 'Abhängigkeiten erledigt — zu Bereit befördert',
  ...
  metaWorkerPid: 'Worker-PID',
  readyUnassignedTitle: 'Bereit, aber ohne Bearbeiter — diese Karte wird nie ausgeführt.',
  readyUnassignedBody:
    'Der Dispatcher übernimmt nur Bereit-Karten mit Bearbeiter. Wähle ein Profil im Bearbeiter-Feld oben (oder setze einen Standard-Bearbeiter in den Orchestrierungseinstellungen), dann läuft es innerhalb einer Minute.',
  diagnosticsN: n => `Diagnose · ${n}`,
  ...
  result: 'Ergebnis',
  latestSummary: 'Letzte Zusammenfassung',
  dependencies: 'Abhängigkeiten',
  blockedBy: 'Blockiert durch',
  blocks: 'Blockiert',
  comments: n => `Kommentare · ${n}`,
  commentsHelpRunning:
    'Diese Aufgabe läuft. Deine Notiz wird innerhalb weniger Sekunden in den aktuellen Durchlauf des Workers eingefaltet — kein Block/Unblock-Hin und Her. „Mit Notiz erneut einreihen“ startet die Aufgabe stattdessen mit deiner Notiz im Kontext neu.',
  commentsHelp:
    'Kommentare werden dem Aufgaben-Thread hinzugefügt. Wenn ein Worker die Aufgabe aufnimmt, liest er sie als Teil seines Kontexts.',
  send: 'Senden',
  comment: 'Kommentar',
  messageWorker: 'Laufendem Worker schreiben…',
  addComment: 'Kommentar hinzufügen…',
  deliveredLive: 'Wird dem laufenden Worker innerhalb weniger Sekunden zugestellt.',
  requeueWithNote: 'Mit Notiz erneut einreihen',
  notePosted: 'Notiz gepostet — Worker erneut eingereiht',
  ...
  projectHintPre:
    'Neue Aufgaben laufen im Repo des Projekts (ein Worktree pro Aufgabe); jede Aufgabe kann ihren Arbeitsbereich bei der Erstellung trotzdem überschreiben. Projekte verwalten mit ',
  projectHintCmd: 'hermes project',
  ...
  profileDescriptionsHint:
    'Beschreibungen leiten das Routing des Zerlegers. Automatisch mit dem Hilfsmodell generieren oder selbst schreiben.',
  profileGoodAt: 'Worin ist dieses Profil gut?',
  auto: 'Auto',
  boardArchived: path => `Board archiviert nach ${path}`,
  boardExported: path => `Board exportiert nach ${path}`,
  boardImported: name => `${name} importiert`,
  boardImportedAs: slug => `Name bereits vergeben — importiert als ${slug}`,
  deleteBoardConfirm:
    'Das Board wird archiviert, nicht gelöscht — Aufgaben und Anhänge bleiben auf der Festplatte und können wiederhergestellt werden.',
  ...
  notify: {
    ...
    blockedTitle: 'Aufgabe blockiert — braucht deine Eingabe',
    blockLoopTitle: 'Aufgabe an Triage weitergeleitet — braucht eine Entscheidung',
    gaveUpTitle: 'Aufgabe gestoppt',
    gaveUpBody: 'Hermes konnte diese Aufgabe nicht abschließen. Öffne Kanban, um den Grund zu sehen, und weise sie neu zu.',
    crashedTitle: 'Aufgabe ist auf ein Problem gestoßen — Hermes versucht es automatisch erneut',
    timedOutTitle: 'Aufgabe hat zu lange gedauert — Hermes versucht es automatisch erneut',
...
export const KANBAN_LOCALES: PluginLocaleBundles = { en, de, ja, zh, 'zh-hant': zhHant }
