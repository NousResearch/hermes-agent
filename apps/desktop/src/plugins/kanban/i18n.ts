...
const pt: KanbanMessages = {
  nav: 'Kanban',
  openBoard: 'Kanban: Abrir quadro',
  newTaskCommand: 'Kanban: Nova tarefa',
  countTip: (running, ready) => `Kanban — ${running} em andamento, ${ready} prontos`,
  col: {
    triage: { label: 'Triagem', help: 'Ideias brutas — um especificador detalha a especificação.' },
    todo: { label: 'A fazer', help: 'Aguardando dependências ou sem responsável.' },
    scheduled: { label: 'Agendado', help: 'Aguardando o horário agendado chegar.' },
    ready: { label: 'Pronto', help: 'Dependências atendidas — atribua um perfil e o despachante o executa.' },
    running: { label: 'Em andamento', help: 'Reivindicado por um trabalhador — um agente está trabalhando nele. Definido pelo despachante.' },
    blocked: { label: 'Bloqueado', help: 'O trabalhador solicitou intervenção humana.' },
    review: { label: 'Revisão', help: 'Um agente de revisão está conferindo o trabalho. Definido pelo despachante.' },
    done: { label: 'Concluído', help: 'Concluído; as tarefas filhas dependentes ficam prontas.' },
    archived: { label: 'Arquivado', help: 'Oculto da exibição padrão do quadro.' }
  },
  locked: {
    review: 'A revisão é definida pelo despachante quando um agente de revisão pega o cartão.',
    running: 'Em andamento é definido pelo despachante quando um trabalhador reivindica o cartão.',
    scheduled: 'Agendado precisa de um horário de despertar — os agentes o definem; um cartão não pode ser arrastado para ele.'
  },
  arcRunning: 'Um agente está trabalhando nisso agora.',
  arcStale: 'Reivindicado, mas sem heartbeat do trabalhador há 2+ minutos — o despachante vai retomá-lo.',
  title: 'Kanban',
  orchestrationSettings: 'Configurações de orquestração',
  newTask: 'Nova tarefa',
  filterCards: 'Filtrar cartões…',
  noMatch: 'Nenhuma tarefa corresponde aos filtros',
  noTasks: 'Nenhuma tarefa neste quadro',
  open: 'Abrir',
  select: modifier => `Selecionar (${modifier}+clique)`,
  deselect: 'Cancelar seleção',
  moveTo: label => `Mover para ${label}`,
  delete: 'Excluir',
  reviewChecking: 'Um agente de revisão está conferindo o trabalho concluído.',
  attachedTip: name => `${name} está anexado — o despachante o entrega no próximo ciclo (≤1 min).`,
  orchestratorTip: name => `${name} (o orquestrador) pega isto no próximo ciclo e escreve a especificação.`,
  autoAssignTip: name => `Atribui automaticamente a “${name}” (kanban.default_assignee) no próximo ciclo de despacho.`,
  wontRun: 'não vai rodar',
  wontRunTip:
    'Cartões prontos só rodam depois que um perfil é atribuído. Abra o cartão e defina um responsável, ou configure um responsável padrão nas configurações de orquestração.',
  noHeartbeat: 'sem heartbeat',
  ...
  groupRunning: 'Agrupar Em andamento por perfil',
  nSelected: n => `${n} selecionados`,
  moveToShort: 'Mover para',
  assign: 'Atribuir',
  unassignAction: 'Desatribuir',
  archive: 'Arquivar',
  clearSelection: 'Limpar seleção (Esc)',
  refused: 'recusado',
  bulkFailed: (failed, total, err) => `${failed} de ${total} falharam — ${err}. Os cartões que falharam continuam selecionados.`,
  titlePlaceholderTriage: 'Ideia aproximada — um especificador vai detalhá-la',
  titlePlaceholder: 'Título',
  descPlaceholder: 'Descrição (opcional)',
  priority: 'Prioridade',
  workspace: 'Workspace',
  boardDefaultSuffix: ' · padrão do quadro',
  workspaceOverride: 'Caminho do workspace (substituição opcional)',
  model: 'Modelo',
  modelInherit: 'Padrão do perfil',
  modelClear: 'Limpar substituição de modelo',
  modelHint: 'Executa esta tarefa em um modelo e uma profundidade de raciocínio específicos. Se não definido, usa o modelo e a profundidade do perfil atribuído.',
  workspaceInherit: 'Herda o diretório do projeto do quadro',
  workspaceInheritDir: dir => `Deixe vazio para herdar ${dir}`,
  workspaceInheritGeneric: 'Deixe vazio para herdar o diretório do projeto do quadro.',
  assignee: 'Responsável',
  defaultOption: name => `${name} (padrão)`,
  parkedOption: 'sem responsável (estacionado — não vai rodar)',
  skills: 'Habilidades (separadas por vírgula)',
  skillsPlaceholder: 'translation, github',
  parent: 'Tarefa pai (bloqueia até ser concluída)',
  noParent: '— sem tarefa pai —',
  goalMode: 'Modo objetivo (o trabalhador fica em loop até um juiz concordar que está concluído)',
  creating: 'Criando…',
  ...
  makesModelCall: 'faz uma chamada ao modelo',
  estimateTip: 'Estimativa aproximada de tokens e complexidade do modelo auxiliar — faz uma chamada ao modelo.',
  estimateTipLong: 'Faz uma chamada rápida ao modelo auxiliar para estimar tokens e complexidade. Um guia aproximado, não uma conta.',
  roughEstimate: 'Estimativa aproximada',
  tokUnit: 'tok',
  couldNotEstimate: 'Não foi possível estimar',
  complexity: { S: 'Pequena', M: 'Média', L: 'Grande' },
  introBody:
    'Você não roda os cartões — os agentes rodam. Coloque um cartão em Pronto com um responsável e um agente o pega em até um minuto. Sem responsável, não roda. Triagem: um agente primeiro reescreve a ideia como uma tarefa adequada. A fazer: aguardando outros cartões. Agendado: aguardando um temporizador. Em andamento e Revisão: as raias dos agentes — não mexa. Bloqueado: está aguardando você. Os resultados voltam no cartão.',
  introGotIt: 'Entendi',
  evtCreated: (where, assignee) =>
    `criado${where ? ` em ${where}` : ''}${assignee ? ` · atribuído a ${assignee}` : ''}`,
  evtMovedTo: col => `movido para ${col}`,
  evtParentReopened: parent => `tarefa pai ${parent} reaberta`,
  evtAssignedTo: assignee => `atribuído a ${assignee}`,
  evtUnassigned: 'sem responsável',
  evtCommentBy: author => `comentário de ${author}`,
  evtClaimedReview: 'reivindicado por um agente de revisão',
  evtClaimedWorker: 'reivindicado por um trabalhador',
  evtWorkerStarted: 'iniciado pelo trabalhador',
  evtCompleted: 'concluído',
  evtBlocked: 'bloqueado — precisa de intervenção humana',
  evtUnblocked: col => `desbloqueado${col ? ` → ${col}` : ' → Pronto'}`,
  evtReclaimed: 'retomado — devolvido à fila',
  evtSpecified: 'especificação escrita pelo agente de triagem',
  evtPromoted: 'dependências concluídas — promovido para Pronto',
  evtScheduled: 'agendado para depois',
  evtArchived: 'arquivado',
  evtReprioritized: priority => `prioridade definida como ${priority}`,
  someone: 'alguém',
  metaPriority: 'Prioridade',
  metaTenant: 'Tenant',
  metaCreatedBy: 'Criado por',
  metaCreated: 'Criado em',
  metaWorkerPid: 'PID do trabalhador',
  readyUnassignedTitle: 'Pronto, mas sem responsável — este cartão nunca vai rodar.',
  readyUnassignedBody:
    'O despachante só pega cartões Prontos que tenham um responsável. Escolha um perfil no campo Responsável acima (ou defina um responsável padrão nas configurações de orquestração) e ele roda em até um minuto.',
  ...
  blocks: 'Bloqueia',
  comments: n => `Comentários · ${n}`,
  commentsHelpRunning:
    'Esta tarefa está em andamento. Sua nota é incorporada ao turno atual do trabalhador em poucos segundos — sem dança de bloquear/desbloquear. “Reenfileirar com nota” reinicia a tarefa do zero com sua nota no contexto.',
  commentsHelp:
    'Comentários são adicionados à thread da tarefa. Quando um trabalhador pega a tarefa, ele os lê como parte do contexto.',
  send: 'Enviar',
  comment: 'Comentário',
  messageWorker: 'Enviar mensagem ao trabalhador em andamento…',
  addComment: 'Adicionar um comentário…',
  deliveredLive: 'Entregue ao trabalhador em andamento em poucos segundos.',
  requeueWithNote: 'Reenfileirar com nota',
  notePosted: 'Nota publicada — trabalhador reenfileirado',
  activity: n => `Atividade · ${n}`,
  runs: n => `Execuções · ${n}`,
  workerLog: 'Log do trabalhador',
  workerLogTail: 'Log do trabalhador · final',
  attachments: n => `Anexos · ${n}`,
  noAttachments: 'Ainda sem anexos.',
  uploadAttachment: 'Enviar anexo',
  taskActions: 'Ações da tarefa',
  copyTaskId: 'Copiar ID da tarefa',
  copyTitle: 'Copiar título',
  copiedId: id => `Copiado ${id}`,
  copiedTitle: 'Título copiado',
  close: 'Fechar',
  working: 'trabalhando',
  board: 'Quadro',
  newBoard: 'Novo quadro',
  switchBoard: 'Trocar de quadro',
  newBoardDots: 'Novo quadro…',
  exportDots: 'Exportar…',
  importDots: 'Importar…',
  renameDots: 'Renomear…',
  settingsDots: 'Configurações…',
  exportBoardTitle: 'Exportar quadro…',
  importBoardTitle: 'Importar quadro…',
  boardExported: path => `Quadro exportado para ${path}`,
  boardImported: name => `${name} importado`,
  boardImportedAs: slug => `Esse nome já estava em uso — importado como ${slug}`,
  renameBoardTitle: 'Renomear quadro',
  deleteBoardTitle: name => `Excluir "${name}"?`,
  deleteBoardConfirm: 'O quadro é arquivado, não apagado — as tarefas e os anexos permanecem no disco e podem ser restaurados.',
  boardArchived: path => `Quadro arquivado em ${path}`,
  boardSettingsFor: name => `Configurações do quadro — ${name}`,
  name: 'Nome',
  boardNamePlaceholder: 'Nome do quadro',
  slug: slug => `slug: ${slug}`,
  project: 'Projeto',
  noProject: 'Sem projeto (sandboxes temporários)',
  projectHintPre:
    'Novas tarefas rodam no repositório do projeto (um worktree por tarefa); cada tarefa ainda pode substituir o workspace na criação. Gerencie projetos com ',
  projectHintCmd: 'hermes project',
  createBoard: 'Criar quadro',
  orchestratorProfile: 'Perfil do orquestrador',
  defaultAssignee: 'Responsável padrão',
  defaultParen: '(padrão)',
  autoDecompose: 'Autodecompor tarefas de triagem',
  profileDescriptions: 'Descrições dos perfis',
  profileDescriptionsHint:
    'As descrições orientam o roteamento do decompositor. Gere automaticamente com o modelo auxiliar ou escreva as suas.',
  profileGoodAt: 'No que este perfil é bom?',
  auto: 'Automático',
  notify: {
    completedTitle: 'Tarefa concluída',
    blockedTitle: 'Tarefa bloqueada — precisa da sua contribuição',
    blockLoopTitle: 'Tarefa encaminhada para a triagem — precisa de uma decisão',
    gaveUpTitle: 'Tarefa interrompida',
    gaveUpBody: 'O Hermes não conseguiu concluir esta tarefa. Abra o Kanban para ver o motivo e reatribuí-la.',
    crashedTitle: 'A tarefa teve um problema — o Hermes tentará novamente de forma automática',
    timedOutTitle: 'A tarefa demorou demais — o Hermes tentará novamente de forma automática',
    openKanban: 'Abrir o Kanban',
    artifacts: (n: number) => `${n} artefatos`
  }
};

export const KANBAN_LOCALES: PluginLocaleBundles = { en, ja, zh, 'zh-hant': zhHant, pt }
