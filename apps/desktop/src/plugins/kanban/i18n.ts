...
export const ptBr: KanbanMessages = {
  nav: 'Kanban',
  openBoard: 'Abrir painel',
  newTaskCommand: 'Kanban: Criar nova tarefa',
  countTip: (running, ready) => `${running} executando · ${ready} salvas em Pronto`,
  col: {
    archived: { label: 'Arquivado', help: 'Tarefas concluídas ou arquivadas.' },
    blocked: { label: 'Bloqueado', help: 'Aguardando ação ou resposta humana.' },
    done: { label: 'Concluído', help: 'Tarefas finalizadas e verificadas.' },
    ready: { label: 'Pronto', help: 'Pronto para execução por um agente autônomo.' },
    review: { label: 'Em Revisão', help: 'Um agente de revisão está checando o resultado.' },
    running: { label: 'Executando', help: 'Um agente está trabalhando nesta tarefa agora.' },
    scheduled: { label: 'Agendado', help: 'Aguardando um temporizador para executar.' },
    todo: { label: 'A Fazer', help: 'Aguardando outras tarefas (dependências).' },
    triage: { label: 'Triagem', help: 'Um agente de triagem vai refinar a ideia e escrever a especificação.' }
  },
  locked: {
    review: 'Em revisão — o agente está verificando o trabalho.',
    running: 'Em execução — um agente está trabalhando nesta tarefa.',
    scheduled: 'Agendado — precisa de horário de despertar (definido por agentes).'
  },
  arcRunning: 'Um agente está trabalhando nisto agora.',
  arcStale: 'Reservado, mas sem sinal de vida há 2+ minutos — o despachante vai recuperar.',
  title: 'Kanban',
  ...
  delete: 'Excluir',
  reviewChecking: 'Um agente de revisão está verificando o trabalho concluído.',
  attachedTip: name => `${name} está anexado — o despachante entrega na próxima verificação (≤1m).`,
  orchestratorTip: name => `${name} (o orquestrador) pega isto na próxima verificação e escreve a especificação.`,
  autoAssignTip: name => `Atribui automaticamente a “${name}” (kanban.default_assignee) na próxima verificação.`,
  wontRun: 'não será executado',
  wontRunTip:
    'Cartões em Pronto só executam quando um perfil é atribuído. Abra o cartão e defina um encarregado, ou configure um encarregado padrão nas configurações de orquestração.',
  noHeartbeat: 'sem sinal de vida',
  ...
  allTenants: 'Todos os inquilinos (tenants)',
  showArchived: 'Mostrar arquivados',
  groupRunning: 'Agrupar Executando por perfil',
  ...
  bulkFailed: (failed, total, err) =>
    `${failed} de ${total} falharam — ${err}. Cartões com falha permanecem selecionados.`,
  titlePlaceholderTriage: 'Ideia bruta — um especificador vai detalhar',
  titlePlaceholder: 'Título',
  descPlaceholder: 'Descrição (opcional)',
  priority: 'Prioridade',
  workspace: 'Espaço de trabalho',
  boardDefaultSuffix: ' · padrão do quadro',
  workspaceOverride: 'Caminho do espaço de trabalho (substituição opcional)',
  model: 'Modelo',
  modelInherit: 'Padrão do perfil',
  modelClear: 'Limpar substituição de modelo',
  modelHint:
    'Executa esta tarefa em um modelo específico e profundidade de pensamento. Não definido usa o do perfil atribuído.',
  workspaceInherit: 'Herda o diretório de projeto do quadro',
  workspaceInheritDir: dir => `Deixe em branco para herdar ${dir}`,
  workspaceInheritGeneric: 'Deixe em branco para herdar o diretório de projeto do quadro.',
  assignee: 'Encarregado',
  defaultOption: name => `${name} (padrão)`,
  parkedOption: 'não atribuído (estacionado — não será executado)',
  skills: 'Habilidades (separadas por vírgula)',
  skillsPlaceholder: 'translation, github',
  parent: 'Pai (bloqueia até estar concluído)',
  noParent: '— sem pai —',
  goalMode: 'Modo objetivo (o trabalhador faz loop até um juiz concordar que está pronto)',
  creating: 'Criando…',
  ...
  makesModelCall: 'faz chamada ao modelo',
  estimateTip: 'Estimativa aproximada de tokens + complexidade pelo modelo auxiliar — faz chamada ao modelo.',
  estimateTipLong:
    'Executa uma chamada rápida de modelo auxiliar para estimar tokens + complexidade. Guia aproximado, não uma cobrança.',
  roughEstimate: 'Estimativa aproximada',
  tokUnit: 'tok',
  couldNotEstimate: 'Não foi possível estimar',
  complexity: { S: 'Pequena', M: 'Média', L: 'Grande' },
  introBody:
    'Você não executa os cartões — os agentes executam. Coloque um cartão em Pronto com um encarregado e um agente o pegará em até um minuto. Sem encarregado, sem execução. Triagem: um agente reescreve a ideia em uma tarefa adequada primeiro. A Fazer: aguardando outros cartões. Agendado: aguardando um temporizador. Executando e Em Revisão: as raias dos agentes, sem intervenção manual. Bloqueado: está aguardando você. Os resultados retornam no cartão.',
  introGotIt: 'Entendi',
  evtCreated: (where, assignee) =>
    `criado${where ? ` em ${where}` : ''}${assignee ? ` · atribuído a ${assignee}` : ''}`,
  evtMovedTo: col => `movido para ${col}`,
  evtParentReopened: parent => `pai ${parent} reaberto`,
  evtAssignedTo: assignee => `atribuído a ${assignee}`,
  evtUnassigned: 'não atribuído',
  evtCommentBy: author => `comentário de ${author}`,
  evtClaimedReview: 'reservado por um agente de revisão',
  evtClaimedWorker: 'reservado por um trabalhador',
  evtWorkerStarted: 'trabalhador iniciado',
  evtCompleted: 'concluído',
  evtBlocked: 'bloqueado — precisa de entrada humana',
  evtUnblocked: col => `desbloqueado${col ? ` → ${col}` : ' → Pronto'}`,
  evtReclaimed: 'recuperado — devolvido à fila',
  evtSpecified: 'especificação escrita pelo agente de triagem',
  evtPromoted: 'dependências concluídas — promovido a Pronto',
  evtScheduled: 'agendado para mais tarde',
  evtArchived: 'arquivado',
  evtReprioritized: priority => `prioridade definida para ${priority}`,
  someone: 'alguém',
  metaPriority: 'Prioridade',
  metaTenant: 'Inquilino (tenant)',
  metaCreatedBy: 'Criado por',
  metaCreated: 'Criado',
  metaWorkerPid: 'PID do trabalhador',
  readyUnassignedTitle: 'Pronto, mas não atribuído — este cartão nunca será executado.',
  readyUnassignedBody:
    'O despachante só pega cartões em Pronto que possuem um encarregado. Escolha um perfil no campo Encarregado acima (ou defina um encarregado padrão nas configurações de orquestração) e ele executará em até um minuto.',
  ...
  result: 'Resultado',
  latestSummary: 'Último resumo',
  dependencies: 'Dependências',
  blockedBy: 'Bloqueado por',
  blocks: 'Bloqueia',
  comments: n => `Comentários · ${n}`,
  commentsHelpRunning:
    'Esta tarefa está em execução. Sua nota é incorporada ao turno atual do trabalhador em poucos segundos — sem necessidade de bloquear/desbloquear. "Reenfileirar com nota" reinicia a tarefa do zero com sua nota no contexto.',
  commentsHelp:
    'Os comentários são adicionados ao tópico da tarefa. Quando um trabalhador pega a tarefa, ele os lê como parte do contexto.',
  send: 'Enviar',
  comment: 'Comentar',
  messageWorker: 'Enviar mensagem ao trabalhador em execução…',
  addComment: 'Adicionar um comentário…',
  deliveredLive: 'Entregue ao trabalhador em execução em poucos segundos.',
  ...
  boardImported: name => `Quadro ${name} importado`,
  boardImportedAs: slug => `Esse nome já está em uso — importado como ${slug}`,
  renameBoardTitle: 'Renomear quadro',
  deleteBoardTitle: name => `Excluir “${name}”?`,
  deleteBoardConfirm:
    'O quadro será arquivado, não apagado — as tarefas e os anexos permanecem no disco e podem ser restaurados.',
  ...
  projectHintPre:
    'Novas tarefas serão executadas no repositório do projeto (uma árvore de trabalho para cada tarefa); cada tarefa pode ter seu caminho de espaço de trabalho substituído ao ser criada. Para gerenciar projetos use ',
  projectHintCmd: 'hermes project',
  createBoard: 'Criar quadro',
  orchestratorProfile: 'Perfil do orquestrador',
  defaultAssignee: 'Encarregado padrão',
  defaultParen: '(padrão)',
  autoDecompose: 'Decompor tarefas de triagem automaticamente',
  profileDescriptions: 'Descrições de perfil',
  profileDescriptionsHint:
    'Descrições usadas para guiar o roteamento do decompositor. Use modelo auxiliar para gerar ou preencha manualmente.',
  profileGoodAt: 'Em que este perfil é bom?',
  auto: 'Automático',
  notify: {
    completedTitle: 'Tarefa concluída',
    blockedTitle: 'Tarefa bloqueada — precisa da sua entrada',
    blockLoopTitle: 'Tarefa enviada para triagem — precisa de decisão humana',
    gaveUpTitle: 'Tarefa abandonada',
    gaveUpBody: 'O Hermes não conseguiu concluir esta tarefa. Abra o Kanban para ver o motivo e reatribuí-la.',
    crashedTitle: 'Trabalhador caiu — vai tentar novamente',
    timedOutTitle: 'Tempo limite da tarefa esgotado — vai tentar novamente',
