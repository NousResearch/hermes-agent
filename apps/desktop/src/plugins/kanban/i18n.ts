...
const tr: KanbanMessages = {
  ...
  col: {
    ...
    running: {
      label: 'Çalışıyor',
      help: 'Bir işçi tarafından alındı — bir ajan üzerinde. Sevk edici tarafından ayarlanır.'
    },
  ...
  bulkFailed: (failed, total, err) =>
    `${total} işlemden ${failed} tanesi başarısız — ${err}. Başarısız kartlar seçili kalır.`,
  ...
  modelHint:
    'Bu görevi belirli bir modelde ve düşünme derinliğinde çalıştırır. Ayarlanmazsa atanan profilin kendi modeli kullanılır.',
  ...
  estimateTipLong:
    'Token + karmaşıklığı tahmin etmek için hızlı bir yardımcı-model çağrısı yapar. Kaba bir rehber, fatura değil.',
  ...
  introGotIt: 'Anladım',
  evtCreated: (where, assignee) => `oluşturuldu${where ? ` (${where})` : ''}${assignee ? ` · ${assignee} atandı` : ''}`,
  ...
  commentsHelp: 'Yorumlar görev başlığına eklenir. Bir işçi görevi aldığında onları bağlamının parçası olarak okur.',
  send: 'Gönder',
  comment: 'Yorum',
