...
export function buildRailTasks(
  ...
  const sessionTasks: RailTask[] = workingSessionIds.map((id, index) => {
    ...
    return {
      id: `session:${id}`,
      label: session ? sessionTitle(session) : translateNow('desktop.activity.sessionTask'),
      detail: translateNow('desktop.activity.agentTaskRunning'),
      status: 'running',
    ...
    ? [
        {
          ...
          label: translateNow('desktop.activity.previewRestart'),
...
function actionDetail(status: ActionStatusResponse): string {
  if (status.running) {
    return translateNow('desktop.activity.running')
  }

  return status.exit_code === 0
    ? translateNow('desktop.activity.completed')
    : translateNow('desktop.activity.failed', status.exit_code ?? 'unknown')
}
