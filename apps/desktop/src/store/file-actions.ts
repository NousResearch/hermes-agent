...
/** Whether "Open containing folder" can work for a session's workspace: only when that
 *  session's backend is this computer. A row tagged with a Connections gateway other than
 *  `local` runs there; an untagged row runs on the window's primary, remote or not. The
 *  sidebar menus already hide the item on a remote primary; the statusbar's workspace menu
 *  offered it for a remote bot's workspace and the click did nothing. */
