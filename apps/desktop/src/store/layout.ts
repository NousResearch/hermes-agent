...
const SIDEBAR_NAV_GROUP_OPEN_STORAGE_KEY = 'hermes.desktop.sidebarNavGroupOpen'
...
export const $sidebarNavGroupOpen = persistentAtom<Record<string, boolean>>(
  SIDEBAR_NAV_GROUP_OPEN_STORAGE_KEY,
  {},
  Codecs.json<Record<string, boolean>>(raw => {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
      return {}
...
export function setSidebarNavGroupOpen(id: string, open: boolean) {
  $sidebarNavGroupOpen.set({ ...$sidebarNavGroupOpen.get(), [id]: open })
}
