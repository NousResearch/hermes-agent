...
import { availableCapability, rolloutAttempt, rolloutSnapshot } from './managed-rollouts.test-fixtures'

const attempts = Array.from({ length: 500 }, (_, index) => rolloutAttempt(index))

function response(revision: number, snapshot = true) {
  return {
    revision,
    snapshot: snapshot
      ? rolloutSnapshot(revision, { attempts })
      : null
  }
}
...
describe('managed rollout store scale fixture', () => {
  ...
  it('measures a bounded 500-target snapshot instead of relying on row count alone', async () => {
    ...
    expect($managedRollouts.get().snapshot?.attempts).toHaveLength(500)
  ...
  it('bounds unchanged-revision traffic and never drops the terminal snapshot', async () => {
    ...
    expect($managedRollouts.get().snapshot?.phase).toBe('running')
  })
})
