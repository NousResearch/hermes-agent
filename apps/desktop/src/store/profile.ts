...
export function prewarmGatewayAgent(connectionId: null | string | undefined, profile: string): void {
  ...
  // SSH hover would spawn a backend on the remote host; only an explicit
  // open may dial it. Other remote sockets do not consume local pool capacity.
  if (registryConnectionKind(source) === 'ssh') {
    return
  }
