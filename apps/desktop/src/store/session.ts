...
// parent. The subagents store registers a predicate here (instead of session.ts
// importing the subagents store, which would create a circular import) so the
// watchdog and setSessionWorking can tell "no parent tokens, but children are
// still actively working" from a genuinely stuck/hung session.
