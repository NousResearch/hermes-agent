...
export interface ClarifyQuestion {
  choices?: null | string[]
  /** Legacy rich-clarify field; kept optional so older renderers that read it stay
   *  type-safe. Current backend emitters send only qid/question/choices/multi_select. */
  header?: string
  multi_select?: boolean
  /** Legacy rich-clarify options; see ``header`` above. */
  options?: { description?: null | string; label: string; recommended?: boolean }[]
  qid: string
  question: string
}
...
export interface ClarifyRequestPayload {
  ...
  choices?: null | string[]
  /** Legacy: when the backend carries an explicit deadline for the card. Not emitted
   *  by current emitters (timeout is enforced server-side); kept optional for renderers. */
  expires_at?: number
  multi_select?: boolean
  question?: string
