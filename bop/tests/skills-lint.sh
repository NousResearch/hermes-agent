...
SKILLS="ledger-writer capture-intake transcript-followup booking email-triage followup-drafter osr-intake dsm-reporter herk2-specforge herk2-watchdog claude-code-delegate brain-note memory-consolidate"
...
if [ "$installed_lines" -eq 13 ]; then
  pass "install.sh prints thirteen installed skill lines"
else
  fail "install.sh prints thirteen installed skill lines" "got $installed_lines"
fi
