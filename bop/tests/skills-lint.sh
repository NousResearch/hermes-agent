...
SKILLS="ledger-writer capture-intake transcript-followup booking email-triage followup-drafter osr-intake dsm-reporter herk2-specforge herk2-watchdog claude-code-delegate brain-note memory-consolidate crm-intake"
...
if [ "$installed_lines" -eq 14 ]; then
  pass "install.sh prints fourteen installed skill lines"
else
  fail "install.sh prints fourteen installed skill lines" "got $installed_lines"
fi
