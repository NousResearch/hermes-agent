...
class HermesCLI:
    ...
    def __init__(
        ...
        # Usage is refreshed asynchronously, so prompt_toolkit rendering never
        # performs network I/O. This cache exists only for this CLI session.
    ...
    @staticmethod
    def _weekly_account_limits_used_percent(snapshot: Any) -> Optional[float]:
        """Return the raw finite percentage for the duration-detected weekly quota."""
        for window in getattr(snapshot, "windows", ()) or ():
            try:
                ...
                continue
            if not (math.isfinite(duration) and math.isfinite(used)):
                continue
            if not 6 * 86_400 <= duration <= 8 * 86_400:
                continue
            return used
        return None

    @classmethod
    def _format_account_limits_badge(cls, snapshot: Any) -> str:
        """Format only the duration-detected Codex weekly quota for the footer."""
        used = cls._weekly_account_limits_used_percent(snapshot)
        if used is None:
            return ""
        displayed_used = max(0, min(100, round(used)))
        gauge = f"[{'█' * round(displayed_used / 10)}{'░' * (10 - round(displayed_used / 10))}]"
        return f"W{displayed_used}%{'+' if used > 100 else ''} {gauge}"

    @staticmethod
    def _account_limits_badge_style(used_percent: Any) -> str:
        """Return the weekly usage-pressure style from the raw percentage."""
        try:
    ...
    def _maybe_refresh_account_limits_badge(self, agent: Any) -> str:
        """Return cached Codex weekly usage and asynchronously refresh stale data."""
        ...
        def _worker() -> None:
            ...
            try:
                ...
                if snapshot:
                    used_percent = self._weekly_account_limits_used_percent(snapshot)
                ...
                with self._account_limits_lock:
                    if label:
                        ...
                        self._account_limits_used_percent = used_percent
                        self._account_limits_style = self._account_limits_badge_style(used_percent)
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        if account_limits:
            snapshot["account_limits_style"] = self._account_limits_badge_style(
                getattr(self, "_account_limits_used_percent", None)
            )
        ...
        try:
            ...
            if width < 52:
                ...
                if width < 76:
                    ...
                    if account_limits:
                        frags.append(("class:status-bar-dim", " · "))
                        frags.append((snapshot["account_limits_style"], account_limits))
                    if compressions:
                        frags.append(("class:status-bar-dim", " · "))
