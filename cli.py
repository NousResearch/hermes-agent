...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            from cron.daemon import daemon_alive as _raw_daemon_alive
            _daemon_cache = getattr(_raw_daemon_alive, "_cache", None)
            if _daemon_cache is None:
                _daemon_cache = {"result": False, "last": 0.0}
                _raw_daemon_alive._cache = _daemon_cache
            _now = time.monotonic()
            if _now - _daemon_cache["last"] > 5.0:
                _daemon_cache["result"] = _raw_daemon_alive()
                _daemon_cache["last"] = _now
            snapshot["cron_daemon_running"] = _daemon_cache["result"]
        except Exception:
            snapshot["cron_daemon_running"] = False
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            cron_running = snapshot.get("cron_daemon_running", False)
            if width < 52:
        ...
        try:
            ...
            if width < 52:
                ...
                if width < 76:
                    ...
                    if cron_running:
                        frags.append(("class:status-bar-dim", " · "))
                        frags.append(("class:status-bar-good", "⏱ cron"))
                    else:
                        frags.append(("class:status-bar-dim", " · "))
                        frags.append(("class:status-bar-warn", "⏱ cron"))
                    frags.append(("class:status-bar", " "))
                else:
                    if snapshot["context_length"]:
                        ctx_total = _format_context_length(snapshot["context_length"])
            ...
            if total_width > width:
                # Drop low-priority fragments from the right to preserve styles on what fits
                working = list(frags)
                _SEPS = (" · ", " │ ", " ")
                while total_width > width and len(working) > 7:
                    last = working.pop()
                    total_width -= self._status_bar_display_width(last[1])
                    if working and working[-1][1] in _SEPS:
                        sep = working.pop()
                        total_width -= self._status_bar_display_width(sep[1])
                if total_width <= width:
                    return working
                # Last resort: flatten to plain text and trim
                plain_text = "".join(text for _, text in working)
    ...
    def _on_tool_gen_start(self, tool_name: str) -> None:
        ...
        from agent.display import get_tool_emoji, get_skin_tool_prefix
        emoji = get_tool_emoji(tool_name, default="")
        prefix = get_skin_tool_prefix()
        _cprint(f"  {prefix} {emoji}preparing {tool_name}...")
