...
def _render_final_assistant_content(text: str, mode: str = "render"):
    ...
    if normalized_mode == "strip":
        ...
        from agent.terminal_bidi import normalize_terminal_text_block

        plain = realign_markdown_tables(_strip_markdown_syntax(text), panel_width)
        return _RichText(normalize_terminal_text_block(plain))
    ...
    plain = normalize_terminal_text_block(plain)
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _emit_reasoning_preview(self, reasoning_text: str) -> None:
        ...
        preview_text = normalize_terminal_text_block(preview_text)
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        def _emit_one(printed_line: str) -> None:
            from agent.terminal_bidi import normalize_terminal_text

            if not printed_line.lstrip().startswith("|"):
                printed_line = normalize_terminal_text(printed_line)
            _cprint(
    ...
    def show_banner(self):
        ...
        try:
            ...
            _agent_cfg = {}
        _min_ctx = resolve_minimum_context_length(_agent_cfg)
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            if _min_ctx < MINIMUM_CONTEXT_LENGTH and ctx_len >= _min_ctx:
                self._console_print(
                    f"[dim]   Allowed by agent.minimum_context_length: {_min_ctx:,} "
                    f"(recommended: {MINIMUM_CONTEXT_LENGTH:,}).[/]"
                )
            else:
                self._console_print(
                    f"[dim]   Hermes needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. "
                    f"Tool schemas + system prompt use a large fixed prefix.[/]"
                )
            ...
            elif _min_ctx < MINIMUM_CONTEXT_LENGTH and ctx_len >= _min_ctx:
                self._console_print(
                    "[dim]   RavenX/local: use `hermes local llamacpp-serve-ravenx` for "
                    "64K Hermes mode when RAM allows, or keep web mode with safe toolsets.[/]"
                )
            else:
                self._console_print(
    ...
    def _show_session_status(self):
        ...
        if agent is not None:
            try:
                from agent.cache_metrics import format_cache_status_line

                lines.append(format_cache_status_line(agent))
            except Exception:
                pass
            # Brain focus (if enabled)
            try:
                orch = getattr(agent, "_brain_orchestrator", None)
                if orch is not None:
                    ecn = orch.ecn.get_state()
                    task = ecn.get("current_task")
                    if task:
                        lines.append(
                            f"Focus: {task} (pinned={ecn.get('pinned')} "
                            f"level={ecn.get('focus_level')})"
                        )
            except Exception:
                pass
    ...
    @staticmethod
    def _sandbox_slash_args(text: str) -> str:
    ...
    def _is_sandbox_slash_command(self, text: str) -> bool:
        ...
        try:
            ...
            base = text.split(None, 1)[0].lower().lstrip("/")
            cmd = resolve_command(base)
            return bool(cmd and cmd.name in ("sandbox", "sbx"))
        except Exception:
            return False

    def _should_handle_sandbox_command_inline(
        self, text: str, has_images: bool = False
    ) -> bool:
        """Dispatch read-only /sandbox subcommands without interrupting the agent."""
        if not text or has_images or not getattr(self, "_agent_running", False):
            return False
        if not self._is_sandbox_slash_command(text):
            return False
        from hermes_cli.sandbox_slash import is_sandbox_read_only_while_agent_running

        return is_sandbox_read_only_while_agent_running(self._sandbox_slash_args(text))

    def _should_reject_sandbox_command_while_running(
        self, text: str, has_images: bool = False
    ) -> bool:
        """Block mutating /sandbox subcommands mid-run (avoid accidental interrupt)."""
        if not text or has_images or not getattr(self, "_agent_running", False):
            return False
        if not self._is_sandbox_slash_command(text):
            return False
        ...
        return not is_sandbox_read_only_while_agent_running(
            self._sandbox_slash_args(text)
        )
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical == "pwnbox" or canonical == "pb":
            self._handle_pwnbox_command(cmd_original)
        elif canonical == "pentest" or canonical == "pt":
            self._handle_pentest_command(cmd_original)
        elif canonical == "hunt" or canonical == "hb":
            self._handle_hunt_command(cmd_original)
        elif canonical == "ctx":
            self._handle_ctx_command(cmd_original)
        elif canonical == "mcpvex" or canonical == "mv":
            self._handle_mcpvex_command(cmd_original)
        elif canonical == "sandbox" or canonical == "sbx":
            self._handle_sandbox_command(cmd_original)
        ...
        elif canonical == "herens":
            self._handle_herens_slash_command(cmd_original)
        elif canonical == "rate":
    ...
    def _confirm_and_reload_mcp(self, cmd_original: str = "") -> None:
        ...
        if not confirm_required:
            with self._busy_command(self._slow_command_status(cmd_original)):
                self._reload_mcp(
                    invalidate_agent_cache=self._reload_mcp_invalidate_cache(
                        cmd_original
                    )
                )
            return
        ...
        with self._busy_command(self._slow_command_status(cmd_original)):
            self._reload_mcp(
                invalidate_agent_cache=self._reload_mcp_invalidate_cache(cmd_original)
            )

    @staticmethod
    def _reload_mcp_invalidate_cache(cmd_original: str) -> bool:
        return "--now" in (cmd_original or "").strip().lower().split()

    def _reload_mcp(self, *, invalidate_agent_cache: bool = False):
        ...
        try:
            ...
            if not invalidate_agent_cache:
                print(
                    "  ℹ️  Session tool list unchanged (prompt cache preserved). "
                    "Use `/reload-mcp --now` or `/new` to apply in this session."
                )
                return
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        try:
            ...
            def run_agent():
                ...
                if isinstance(agent_message, str) or isinstance(agent_message, list):
                    try:
                        from plugins.hermes_advanced.rag.chat_inject import (
                            maybe_inject_rag_user_message,
                        )

                        _injected = maybe_inject_rag_user_message(agent_message)
                        if _injected != agent_message:
                            agent_message = _injected
                    except Exception:
                        pass
    ...
    def run(self):
        ...
        try:
            from hermes_cli.bonsai_local import ensure_bonsai_server

            ensure_bonsai_server()
        except Exception:
            pass
        ...
        def handle_enter(event):
            ...
            if text or has_images:
                ...
                if self._should_handle_sandbox_command_inline(
                    text, has_images=has_images
                ):
                    self.process_command(text)
                    event.app.current_buffer.reset(append_to_history=True)
                ...
                if self._should_reject_sandbox_command_while_running(
                    text, has_images=has_images
                ):
                    _cprint(
                        "  Agent is running — use read-only `/sandbox` commands "
                        "(doctor, status, fleet, registry, …) mid-run, or `/stop` "
                        "before spawn/exec/teardown."
                    )
                    event.app.current_buffer.reset(append_to_history=True)
                    event.app.invalidate()
                    return
