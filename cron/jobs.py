...
for _attr in ["HERMES_DIR", "CRON_DIR", "JOBS_FILE", "TICKER_HEARTBEAT_FILE", "TICKER_SUCCESS_FILE", "OUTPUT_DIR"]:
    if _attr in globals():
        del globals()[_attr]
...
def _cron_dir() -> Path:
    if "CRON_DIR" in globals():  # test-only: allows pytest monkeypatch via cron.jobs.CRON_DIR = ...
...
def __getattr__(name: str) -> Any:
    ...
    elif name == "TICKER_HEARTBEAT_FILE":
        return get_hermes_home().resolve() / "cron" / "ticker_heartbeat"
    elif name == "TICKER_SUCCESS_FILE":
