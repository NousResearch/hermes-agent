...
def _compute_random_window_next_run(schedule: Dict[str, Any], last_run_at: Optional[str] = None) -> Optional[str]:
    ...
    for window in windows:
        try:
            start_t = _parse_window_time(window["start"])
            end_t = _parse_window_time(window["end"])
        except Exception:
            logger.warning("Invalid random window schedule entry: %r", window)
            continue
        parsed_windows.append((start_t, end_t))

    if not parsed_windows:
        return None

    # Look ahead a few days so invalid or already-passed daily windows recover.
    for day_offset in range(0, 7):
        day = (now + timedelta(days=day_offset)).date()
        for start_t, end_t in parsed_windows:
            start_dt = datetime.combine(day, start_t, tzinfo=timezone)
            end_dt = datetime.combine(day, end_t, tzinfo=timezone)
            if end_dt <= start_dt:
                end_dt += timedelta(days=1)

            # After a run, do not schedule again inside the same named window.
            if last_run_at and start_dt <= base < end_dt:
                continue
            if end_dt <= now:
                continue

            earliest = max(start_dt, now + timedelta(seconds=60))
            if earliest > end_dt:
                continue

            span = int((end_dt - earliest).total_seconds())
            return (earliest + timedelta(seconds=random.randint(0, max(span, 0)))).isoformat()
...
def compute_next_run(schedule: Dict[str, Any], last_run_at: Optional[str] = None) -> Optional[str]:
    ...
    if schedule["kind"] == "once":
        ...
        jitter_seconds = int(schedule.get("jitter_seconds") or 0)
        if jitter_seconds > 0:
            next_run = next_run + timedelta(seconds=random.randint(0, jitter_seconds))
        return next_run.isoformat()

    elif schedule["kind"] == "random_windows":
        return _compute_random_window_next_run(schedule, last_run_at=last_run_at)
        ...
        for i, job in enumerate(jobs):
            if job["id"] == job_id:
                ...
                if job["next_run_at"] is None:
                    ...
                    if kind in {"cron", "interval", "random_windows"}:
                        job["state"] = "error"
...
def advance_next_run(job_id: str) -> bool:
    ...
    with _jobs_file_lock:
        ...
        for job in jobs:
            if job["id"] == job_id:
                kind = job.get("schedule", {}).get("kind")
                if kind not in {"cron", "interval", "random_windows"}:
                    return False
                now = _hermes_now().isoformat()
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    for job in jobs:
        ...
        if not next_run:
            ...
            if not recovered_next and kind in {"cron", "interval", "random_windows"}:
        ...
        if next_run_dt <= now:
            ...
            if kind in {"cron", "interval", "random_windows"} and (now - next_run_dt).total_seconds() > grace:
