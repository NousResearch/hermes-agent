...
def _elapsed_seconds(later: datetime, earlier: datetime) -> float:
    ...
    Datetime comparisons using one ``ZoneInfo`` object ignore offset/fold
    differences. Scheduler timing and claim ages are absolute instants, so
    normalize both operands to UTC before subtracting.
    """
...
def _rearm_stale_error_recurring(d: _DueJob) -> datetime:
    ...
    if not (recovered_next and recovered_next_dt is not None
