...
# Dedicated cross-process lock file. Distinct from the scheduler's `.tick.lock`
# (which is held for the WHOLE tick, including multi-minute agent runs); this
# one is only ever held around a short load→modify→save of jobs.json, so a
# separate `hermes` CLI process (e.g. `hermes cron pause`) can serialize with
# the gateway's writes without blocking on a running agent.
...
def _jobs_lock():
    ...
    lock on ``_JOBS_LOCK_FILE`` (mutual exclusion between the gateway process
