...
def resume_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    A recurring job paused across one of its slots must not lose that slot
    silently: its stored ``next_run_at`` (already past) survives resume as
    the due instant, and the ordinary late / catch-up / fast-forward policy
    in the due scan decides what happens to it — the same restore-once
    contract ``_restore_unclaimed_slot`` holds for #107485, applied to the
    pause path (#113603: a disable/re-enable across a slot left no
    execution row and no incident).
    """
    job = resolve_job_ref(job_id)
    if not job:
        return None
    ...
    if (
        ...
        # Past slot from before the pause: keep it due instead of re-anchoring
        # past it (which consumes the occurrence with no ledger row).
