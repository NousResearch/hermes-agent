...
def _is_silent_response(content: str) -> bool:
    """Return True for standalone SILENT markers, including a missing bracket.

    Models occasionally emit ``[SILENT`` without the closing bracket. Treat that
    as the same suppression marker, but only when the entire response is the
    marker so normal messages mentioning SILENT are still delivered.
    """
    return bool(_SILENT_RESPONSE_RE.fullmatch(content or ""))
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        wants_mcp = (
            resolved_enabled_toolsets is None
            or "mcp" in resolved_enabled_toolsets
            or "native-mcp" in resolved_enabled_toolsets
        )
        if wants_mcp:
            try:
