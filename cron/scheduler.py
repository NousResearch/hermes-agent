...
def _run_job_script(script_path: str, timeout: int | None = None) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        ...
        timeout: Per-job timeout in seconds. When set, overrides the
            global script timeout (config / env / default). When None
            (default), the global timeout is used.
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    if context_from:
        from cron.jobs import output_dir as _cron_output_dir
        ...
        for source_job_id in context_from:
            ...
            if not source_job_id or not all(c in "0123456789abcdef" for c in source_job_id):
                ...
                continue
            try:
                job_output_dir = _cron_output_dir() / source_job_id
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        # Use the same effective fallback chain for auth fallback and AIAgent
        # construction. Per-job overrides (including explicit []) are honoured at
        # every site.
