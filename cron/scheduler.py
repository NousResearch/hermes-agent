...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    cron_hint = (
        ...
        "VERIFICATION: Execute the task fully — verify findings before "
        "reporting. When data is missing or degraded, report the gap "
        "rather than producing plausible output. "
        "SILENT: Only use [SILENT] when all checks ran and produced "
        "truly zero actionable output (no data, no events, everything "
        "nominal). Do NOT use [SILENT] to skip verification. "
