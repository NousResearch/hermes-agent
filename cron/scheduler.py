...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str]:
    ...
    installs run without ``moa``) > an empty list on any failure (no toolsets).
    ...
    globally without recreating every job. 3. An empty list on any lookup failure, so AIAgent does
    not load the full default set when the configured allowlist cannot be resolved.
    """
    ...
    try:
        ...
        logger.warning(
            "Cron toolset resolution failed; disabling toolsets for this run: %s",
            exc)
        return []
