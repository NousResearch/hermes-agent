...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    # Keep the explicit checkout pin in the sanitized worker env for #112729. The runtime
    # launcher uses isolated mode and bootstraps the same root itself, but removing the prior
    # worker-env contract is a separate change. See cron/scheduler_worker_env.py.
