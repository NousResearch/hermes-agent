...
def _load_credential_pool(runtime: dict, job_id: str):
    ...
    if not runtime_provider:
        return None
    try:
        # Not `load_pool(runtime_provider)`: a custom endpoint resolves with
        # provider "custom" but keeps its credentials under its own
        # `custom:<name>` key, so the bare name reads an empty pool and the agent
        # gets nothing to rotate through on a later 429. This helper prefers the
        # pool resolution already attached to the runtime -- the one selection
        # actually drew from, rather than a second read of the same store -- and
        # otherwise resolves the scoped key. Pre-existing for any custom-provider
        # job; a demotion to a named custom fallback makes it reachable far more
        # often.
...
def _resolve_cron_agent_setup(job: dict, job_id: str, job_name: str, jc) -> _CronAgentSetup:
    ...
    # Resolution can SUCCEED and still be unusable: every live credential in the
    # provider's pool may be serving a 429, in which case the runtime carries a
    # benched key and the request 429s before the agent's own fallback engages --
    # one doomed call on every tick, for as long as the quota stays drained.
    #
    # A cron run is single-shot, so there is nothing to restore: the next tick
    # resolves from the configured primary again and returns to it on its own
    # once the window lifts. The model moves with the provider -- swapping one
    # while keeping the other would send e.g. a Gemini model id to OpenRouter.
    #
    # `_check_model_drift` below deliberately keeps comparing the provider and
    # model captured BEFORE resolution: that guard exists to catch the operator's
    # global config drifting under an unpinned job, not to police a runtime
    # fallback the operator configured on purpose. The pre-existing AuthError
    # walk behaves the same way -- the difference is that a cooldown recurs every
    # tick while the quota stays drained.
    from hermes_cli.provider_cooldown import demote_if_rate_limited

    _demotion = demote_if_rate_limited(
        setup.runtime, lambda: get_fallback_chain(_cfg), subject=f"Job '{job_id}'"
    )
    setup.runtime = _demotion.runtime
    if _demotion.switched:
        setup.model = _demotion.model
