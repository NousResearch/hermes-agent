...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # process-global or profile-local runtime state inside run_job. Workdir
        # jobs temporarily set os.environ["TERMINAL_CWD"]; profile jobs use a
        # context-local Hermes home override, scheduler _hermes_home hook, and
        # temporary profile .env load into os.environ with snapshot/restore.
        # Workdir jobs also rely on run_job's _terminal_cwd_lock so concurrently
        # firing workdir-less jobs cannot observe the override. Jobs without
        # either field stay parallel-safe.
        sequential_jobs = [
            j for j in due_jobs
