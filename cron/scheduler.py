        ...
        if live_adapter_ready:
            ...
            try:
                ...
                if adapter_ok:
                    ...
                    # Guarded by _inchannel_seed_allowed: group-channel keys
                    # are user-isolated, so a seed without a user_id (origin-
                    # less managed cron into a shared channel) would create an
                    # orphan session no reply resolves to — fall back to the
                    # plain mirror instead.
                    if (
                        in_channel_surface
