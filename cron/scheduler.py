...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        # ``job["model"]`` is normalized to a string by create_job/update_job
        # (see jobs.py: normalized_model = str(model).strip() if isinstance(model, str) else None),
        # but legacy job records persisted before that guard was added (or hand-edited
        # jobs.json files) can still contain a dict like ``{"provider": ..., "model": ...}``.
        # ``or`` happily forwards a non-empty dict, so an unguarded read would later
        # crash AIAgent._anthropic_prompt_cache_policy with
        # ``AttributeError: 'dict' object has no attribute 'lower'``.  Defensively
        # coerce non-str values to "" here, but also pull out the inner model name
        # from the legacy dict shape so the job still runs on the right model.
        _raw_model = job.get("model")
        if isinstance(_raw_model, str):
            model = _raw_model
        elif isinstance(_raw_model, dict):
            # Legacy shape: {"provider": "...", "model": "..."}.  Use the inner model
            # name; provider/base_url are still sourced from job["provider"]/job["base_url"]
            # or the runtime override below, so we deliberately do not re-merge here.
            _inner = _raw_model.get("model")
            model = _inner if isinstance(_inner, str) else ""
            logger.warning(
                "Job '%s': legacy dict-shaped model field detected (%r); "
                "migrating to string. Re-save the job via cronjob update to "
                "clean up jobs.json.", job_id, _raw_model,
            )
        else:
            model = ""
        ...
        try:
            ...
            if os.path.exists(_cfg_path):
                ...
                # Use the same coercion as above — config.yaml's ``model`` field can
                # also be a dict like ``{default: "x", reasoning_effort: "high"}``,
                # in which case we want the ``default`` sub-key, not the whole dict.
                if not (isinstance(_raw_model, str) and _raw_model):
                    if isinstance(_model_cfg, str):
                        model = _model_cfg
                    elif isinstance(_model_cfg, dict):
                        model = _model_cfg.get("default", model) or model
        except Exception as e:
