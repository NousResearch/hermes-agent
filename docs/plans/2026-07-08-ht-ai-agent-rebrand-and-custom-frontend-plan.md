...
- **Phase 6 — Deep rebrand (recommend: don't, or only at a major version):** `HERMES_*` env vars, `~/.hermes` home (needs migration tooling), Python module renames, `X-Hermes-*` headers. ~12,300 occurrences; multi-week; breaks running installs.
