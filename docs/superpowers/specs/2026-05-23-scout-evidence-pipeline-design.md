...
Implement a two-profile research pipeline:

| Profile | Role | Default authority |
| --- | --- | --- |
| `scout` | Collector and analyst | Search, extract, read, evaluate, and produce a cited evidence packet. No persistent file changes. |
| `scout-publisher` | Evidence reviewer and vault publisher | Read a completed evidence packet, verify publication requirements, and write only approved research artifacts to authorized vault paths. |

`scout` remains the human-facing name for research requests so existing habits
and any Telegram routing remain stable. `scout-publisher` is not a general
assistant and is invoked only when durable retention is wanted after evidence
collection.

This split is preferred over a single mode-switched Scout because web pages,
documents, and other retrieved materials are untrusted inputs. The profile
that directly consumes broad external material should not also have broad
write authority.
...
| --- | --- | --- |
| Rapid fact check | Bounded factual query | Cited answer; no packet required unless publishing is requested. |
...
`scout-publisher` receives an evidence packet generated by `scout` or supplied
by Felipe. It does not perform broad open-ended research by default. It may
retrieve a cited source only to validate a challenged or publication-critical
claim.

The publisher may write a durable resource only when the packet satisfies the
publication gate:
...
If the gate fails, the publisher returns a rejection report listing missing
evidence. It does not repair gaps by inventing citations or silently publish
an incomplete note.

## Evidence Packet

The handoff artifact is a Markdown evidence packet written by the collector in
its response or provided directly to the publisher. It uses this minimum
schema:
...
The publisher appends an operations-log entry only after a note passes the
publication gate and the write succeeds.
...
relative reads, even though Collector is read-only:
...
Configure `scout-publisher` with:
...
The publisher does not need shell or general file modification access.

## Profile Identity Changes

### `scout` Identity

Rewrite Scout's identity around evidence collection rather than vault
maintenance:

- role: Chief Research Collector and Evidence Analyst
- primary output: cited answers and evidence packets
...
- hard statement that Scout cannot publish or modify durable state
- handoff instruction: request publication by sending a completed packet to
  `scout-publisher`

Remove claims that Scout maintains plan files or writes vault resources
directly.

### `scout-publisher` Identity

Create a compact publisher identity:

- role: evidence gatekeeper and vault publisher
- accepts completed evidence packets only
- checks claim-level provenance and vault schema
- refuses weak or incomplete evidence with a structured rejection report
- may use allowed vault write tools only after passing the gate
- must not broaden research scope or alter agent/system configuration
...
### Publisher Active Skills

Keep visible to `scout-publisher`:

- new `evidence-ledger`
- `vault-librarian`
- `scholar-evaluation`
- `literature-review` for validating systematic-review packet structure

No unrelated skill categories should be enabled for the publisher.
...
The publisher may retain that a note was published or rejected, with the
evidence status and durable note link; it must not create a second parallel
knowledge base.
...
3. Create `scout-publisher` from a minimal profile seed rather than cloning
   Scout's broad skill library.
4. Rewrite `scout/SOUL.md`, update `scout/config.yaml`, create its explicit
   workspace directory, and configure its disabled skill list.
5. Configure `scout-publisher/SOUL.md`, `config.yaml`, minimal skills, and
   isolated Hindsight identity.
6. Run profile/tool/prompt verification without restarting a running gateway.
7. Use one bounded fresh research request as a smoke evaluation:
   Collector produces an evidence packet; Publisher either publishes it with
   citations or explicitly rejects it for missing evidence.
...
- Collector tool definitions exclude mutation, terminal, delegation, and
  generation tools.
- Publisher tool definitions expose publication writes but not general writes.
- Skill prompt filtering hides disabled unrelated categories in each profile.

### Profile Checks

Verify against the live configured profiles:

- `scout` loads the collector identity and narrow toolsets.
- `scout-publisher` loads the publisher identity and narrow toolsets.
- Both profiles can load their intended skills.
...
| Packet lacks claim citations | Publisher rejects; no vault file written. |
| Valid packet | Publisher writes a cited resource and operations-log entry within allowlist. |
...
- both profiles expose only their intended tools and skills,
...
2. Disable or remove `scout-publisher`.
3. Remove `vault-publish` from any active toolset selection.
