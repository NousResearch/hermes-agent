...
def pick_candidates(conn, k: int) -> list[Candidate]:
    rows = conn.execute(
        """
        SELECT c.id, c.source, c.source_id, c.url, c.title, COALESCE(c.summary, '') AS summary,
               COALESCE(c.raw_json, '') AS raw_json, c.fetched_at,
               COALESCE(c.engagement_velocity, 0) AS engagement_velocity,
               COALESCE(c.points, 0) AS points,
               COALESCE(c.age_hours, 0) AS age_hours
          FROM candidates c
         WHERE COALESCE(c.status, 'new') != 'skipped'
           AND NOT EXISTS (
               SELECT 1 FROM drafts d
                WHERE d.candidate_id = c.id
                  AND d.status IN ('pending', 'approved')
           )
         ORDER BY COALESCE(c.engagement_velocity, 0) DESC, c.fetched_at DESC
         LIMIT ?
        """,
        (k,),
    ).fetchall()
    return [
        Candidate(
            ...
            engagement_velocity=float(r["engagement_velocity"] or 0.0),
...
def fetch_article_content(url: str) -> tuple[bool, str]:
    """
    Fetch article content using hermes CLI with web tool.
    Returns (success, content_or_error_reason).
    
    SOURCE-GROUNDING GATE: This must succeed before drafting.
    """
...
def derive_image_subject(post_text: str, candidate: Candidate) -> str:
    """Derive a short subject from post text for image prompt."""
    # Extract first sentence or use title as fallback
    lines = post_text.strip().split('\n')
    first_line = lines[0].strip() if lines else ""
    # Remove common prefixes
    first_line = re.sub(r'^(Breaking|Just in|New|Update):\s*', '', first_line, flags=re.IGNORECASE)
    # Truncate at first sentence
    match = re.search(r'^[^.!?]+', first_line)
    subject = match.group(0) if match else candidate.title
    # Limit to 80 chars
    if len(subject) > 80:
        subject = subject[:77] + "..."
...
def draft_id_for(candidate: Candidate, generated_at: int, text: str) -> str:
    digest = hashlib.sha1(f"{candidate.id}|{candidate.url}|{generated_at}|{text[:200]}".encode("utf-8")).hexdigest()[:10]
    return f"d_{generated_at}_{candidate.id}_{digest}"


def draft_markdown(draft_id: str, candidate: Candidate, post_text: str, version: str, generated_at: int) -> str:
    generated_iso = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(generated_at))
    return f"""Source: [{candidate.source}] {candidate.title}
URL: {candidate.url}
Velocity: {candidate.engagement_velocity:.2f}
Draft ID: {draft_id}
...
def write_draft(conn, candidate: Candidate, post_text: str, version: str) -> tuple[Path, str, Path | None]:
    """
    Write raw draft file and DB row with meta-preamble stripping and quality gates.
    Returns (draft_path, draft_id, image_path). image_path is intentionally
    None here: image concepting/generation is a post-draft writer-stage concern.

    NO DIRECT API POSTING: This function only writes local files and DB entries.
    """
    # Strip meta-preamble before any other processing
    ...
    if not gate_result.passed:
        # Log quality gate failures but still create draft for manual review
        failure_summary = "; ".join(gate_result.failures)
        print(f"warning: quality gate failures for candidate {candidate.id}: {failure_summary}", file=sys.stderr)
        # Could optionally add failures to draft metadata or skip writing
