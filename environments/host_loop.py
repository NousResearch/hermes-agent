...
Contract (per the sovereign local-loop pattern):
  ...
  auditor  : verifies each step / the final result (pytest, schema, nonzero-exit)
  manifest : a run record (JSON) proving what happened — the loop's audit trail

This is the local counterpart to environments/agent_loop.py (which targets
OpenAI-spec servers). The host loop never leaves the machine: brain is a local
model socket, hands are local processes. Fully air-gappable.

Subclass + register tools to build a sovereign agent for any surface.
"""
...
class LoopStep:
    turn: int
    role: str            # planner | executor | auditor
    action: str          # think | <tool> | audit | final
    detail: str
...
class LocalHostLoop:
    ...
    Override `planner()` and `tools` to specialize. The default planner speaks
    the ollama generate API; swap it for any local brain.
    """
    ...
    def _default_planner(self, prompt: str) -> str:
        # ollama default; override for a different local brain
        try:
            out = subprocess.run(
                ["curl", "-s", "-m", "180", "-d",
        ...
        except Exception as e:  # brain unreachable -> fail loud, not silent
    ...
    def audit(self, result: Any) -> bool:
        """A result is healthy if it's not an error dict and ok != False."""
