    ...
    try:
        with connect() as conn:
            for outcome in ('failure', 'cancelled', 'timed_out', 'success'):
                ...
                accepted = _owner_kanban_completion.complete_task(conn, tid, metadata={'published_pr': 'https://github.com/acme/repo/pull/7'})
            ...
            for outcome in (('success', 'failure') if 'completion_contract' in inspect.signature(kb.create_task).parameters else ()):
                ...
                owner = _owner_kanban_claims.claim_task(conn, tid)
                run_id = owner.current_run_id
                def reclaim():
                    with connect() as other:
                        _owner_kanban_db.block_task(other, tid, reason='Reassigned during acceptance')
                        _owner_kanban_transitions.unblock_task(other, tid)
                        state['replacement'] = _owner_kanban_claims.claim_task(other, tid).current_run_id
                state.update(conclusion=outcome, race=reclaim)
                accepted = _owner_kanban_completion.complete_task(conn, tid, expected_run_id=run_id, metadata={'published_pr': 'https://github.com/acme/repo/pull/7'})
