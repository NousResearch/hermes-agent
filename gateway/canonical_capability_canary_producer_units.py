...
NATIVE_PUBLICATION_SCHEMA = (
    "muncho-capability-canary-role-native-publication.v1"
)
PRODUCTION_DIFF_OBSERVATION_SCHEMA = (
...
GATEWAY_OBSERVER_SOURCE_PROJECTION_FILENAME = (
    "gateway-observer-source-projection.json"
)
GATEWAY_OBSERVER_PROPOSAL_EVENT_TYPE = (
    "capability.canary.gateway-evidence.proposed"
)
...
DEFAULT_KEY_BOOTSTRAP_RECEIPT = (
    DEFAULT_KEY_ROOT / "producer-key-bootstrap.json"
)
...
DEFAULT_TMPFILES_PATH = Path(
    "/etc/tmpfiles.d/muncho-capability-canary-producers.conf"
)
...
def _run_account_command(argv: Sequence[str]) -> None:
    if not argv or Path(argv[0]) not in {GROUPADD, USERADD, USERMOD}:
...
def _ensure_group(name: str) -> grp.struct_group:
    try:
        return grp.getgrnam(name)
    except KeyError:
        _run_account_command((str(GROUPADD), "--system", "--", name))
    try:
        return grp.getgrnam(name)
    except KeyError as exc:  # pragma: no cover - kernel/NSS boundary
        raise CapabilityProducerError("producer_group_unavailable") from exc


def _ensure_user(user: str, group: grp.struct_group) -> pwd.struct_passwd:
    try:
        ...
        try:
            ...
            raise CapabilityProducerError("producer_user_unavailable") from exc
    if (
        record.pw_uid <= 0
        or record.pw_gid != group.gr_gid
        or record.pw_dir != "/nonexistent"
        or record.pw_shell != "/usr/sbin/nologin"
    ):
        _fail("producer_user_identity_drifted")
    return record


def _ensure_supplementary_group(user: pwd.struct_passwd, group: grp.struct_group) -> None:
    if group.gr_gid not in os.getgrouplist(user.pw_name, user.pw_gid):
        _run_account_command(
            (
                str(USERMOD),
                "--append",
    ...
    if group.gr_gid not in os.getgrouplist(user.pw_name, user.pw_gid):
        _fail("producer_supplementary_group_unavailable")


def attest_producer_role_identities(
    expected: Mapping[str, Any],
) -> Mapping[str, Mapping[str, Any]]:
    """Read-only NSS proof of the exact sealed principals and memberships."""

    identities = _role_identities(expected)
    try:
        receipt_group = grp.getgrnam(PRODUCER_RECEIPT_WRITER_GROUP)
        bitrix_group = grp.getgrnam(BITRIX_OPERATIONAL_EDGE_SOCKET_GROUP)
        writer_group = grp.getgrnam(CANONICAL_WRITER_SERVICE_GROUP)
        writer_socket_group = grp.getgrnam(CANONICAL_WRITER_SOCKET_GROUP)
        routeback_group = grp.getgrnam(DISCORD_ROUTEBACK_GROUP)
        connector_group = grp.getgrnam(DISCORD_CONNECTOR_GROUP)
    except KeyError as exc:
        raise CapabilityProducerError("producer_group_unavailable") from exc
    expected_users = {item.user for item in identities.values()}
    if (
        receipt_group.gr_gid
        != identities["business_edge"].receipt_writer_gid
        or set(receipt_group.gr_mem) != expected_users
    ):
        _fail("producer_receipt_group_membership_drifted")
    observed: dict[str, Mapping[str, Any]] = {}
    for role, identity in identities.items():
        try:
            user = pwd.getpwnam(identity.user)
            primary = grp.getgrnam(identity.group)
        except KeyError as exc:
            raise CapabilityProducerError("producer_identity_unavailable") from exc
        expected_groups = {identity.gid, identity.receipt_writer_gid}
        if identity.bitrix_socket_gid is not None:
            expected_groups.add(identity.bitrix_socket_gid)
        if role == "canonical_writer":
            expected_groups.update(
                {writer_group.gr_gid, writer_socket_group.gr_gid}
            )
        if role == "discord_edge":
            expected_groups.update(
                {routeback_group.gr_gid, connector_group.gr_gid}
            )
        if (
            user.pw_uid != identity.uid
            or user.pw_gid != identity.gid
            or primary.gr_gid != identity.gid
            or user.pw_dir != "/nonexistent"
            or user.pw_shell != "/usr/sbin/nologin"
            or set(os.getgrouplist(identity.user, identity.gid))
            != expected_groups
            or (
                identity.bitrix_socket_gid is not None
                and bitrix_group.gr_gid != identity.bitrix_socket_gid
            )
        ):
            _fail("producer_role_membership_drifted")
        observed[role] = {
            "user": identity.user,
            "group": identity.group,
            "uid": identity.uid,
            "gid": identity.gid,
            "receipt_writer_gid": identity.receipt_writer_gid,
            "bitrix_socket_gid": identity.bitrix_socket_gid,
        }
...
def ensure_producer_role_identities() -> Mapping[str, Mapping[str, Any]]:
    ...
    try:
        ...
        raise CapabilityProducerError("producer_bitrix_group_unavailable") from exc
    rows: dict[str, Mapping[str, Any]] = {}
    for role in ENDPOINT_ROLES:
        user_name, group_name = PRODUCER_ROLE_ACCOUNTS[role]
        primary = _ensure_group(group_name)
        user = _ensure_user(user_name, primary)
        _ensure_supplementary_group(user, receipt_group)
        if role in {"business_edge", "canonical_writer"}:
            _ensure_supplementary_group(user, bitrix_group)
        if role == "canonical_writer":
            _ensure_supplementary_group(user, writer_group)
            _ensure_supplementary_group(user, writer_socket_group)
        if role == "discord_edge":
            _ensure_supplementary_group(user, routeback_group)
            _ensure_supplementary_group(user, connector_group)
        rows[role] = {
            "user": user_name,
            "group": group_name,
            "uid": user.pw_uid,
            "gid": primary.gr_gid,
            "receipt_writer_gid": receipt_group.gr_gid,
            "bitrix_socket_gid": (
                bitrix_group.gr_gid
                if role in {"business_edge", "canonical_writer"}
                else None
    ...
    return attest_producer_role_identities(rows)
    ...
    *,
    ...
    if role == "canonical_writer":
        supplementary_groups.extend(
            [CANONICAL_WRITER_SERVICE_GROUP, CANONICAL_WRITER_SOCKET_GROUP]
        )
    if role == "discord_edge":
        supplementary_groups.extend(
            [DISCORD_ROUTEBACK_GROUP, DISCORD_CONNECTOR_GROUP]
        )
    supplementary = [
        f"SupplementaryGroups={' '.join(supplementary_groups)}"
    ]
    ordering = (
        [
    ...
    *,
    ...
    for role in ENDPOINT_ROLES:
        ...
        contract = {
            ...
            "public_key_projection_path": str(
                producer_public_key_path(role)
            ),
...
def prepare_producer_foundation(
    ...
    *,
    ...
    identities_value = (
        ...
        if role_identities is None
        else copy.deepcopy(dict(role_identities))
    )
    ...
    unsigned_foundation = {
        ...
        "receipt_contract": {
            ...
            "slot_native_binding_kinds": {
                slot: list(SLOT_NATIVE_BINDING_KINDS[slot])
    ...
    preparation = dict(
        validate_foundation_preparation(
            {**unsigned, "preparation_sha256": _sha256_json(unsigned)}
        )
    )
    if preparation_path == DEFAULT_FOUNDATION_PREPARATION_PATH:
        _ensure_exact_directory(
            preparation_path.parent, uid=0, gid=0, mode=0o700
        )
        _publish_root_file(
...
def validate_foundation_preparation(value: Any) -> Mapping[str, Any]:
    raw = _strict(
        value,
        (
            "schema",
    ...
    if (
        ...
        != _sha256_bytes(
            producer_foundation_signature_payload(foundation)
        )
        or foundation.get("release_sha") != raw["revision"]
        or foundation.get("capability_plan_sha256")
        != raw["capability_plan_sha256"]
        or foundation.get("full_canary_plan_sha256")
        != raw["full_canary_plan_sha256"]
        or not isinstance(owner, Mapping)
        or owner.get("public_key_ed25519_hex")
    ...
    _digest(raw["key_bootstrap_receipt_sha256"], "producer_foundation_preparation_invalid")
    _digest(raw["owner_public_key_source_sha256"], "producer_foundation_preparation_invalid")
...
def _validate_key_bootstrap_receipt(value: Any) -> Mapping[str, Any]:
    ...
    for role, value in zip(ENDPOINT_ROLES, rows, strict=True):
        ...
        if (
            ...
            or row["public_key_projection_path"]
            != str(producer_public_key_path(role))
...
def validate_installed_producer_foundation(
    *,
    ...
    if (
        ...
        != preparation["key_bootstrap_receipt_sha256"]
    ...
    for role in ENDPOINT_ROLES:
        ...
        private_hex = private.public_key().public_bytes(
            serialization.Encoding.Raw,
            serialization.PublicFormat.Raw,
        ).hex()
    ...
    return {
        "schema": "muncho-capability-producer-installation-preflight.v1",
        "revision": foundation["release_sha"],
    ...
    *,
    ...
    for role in ENDPOINT_ROLES:
        ...
        result[role] = {
            ...
            "service_identity_sha256": identity_contract.service_identity_sha256s[
                role
            ],
            "uid": identity.uid,
            "gid": identity.gid,
...
def render_producer_units(
    *,
    ...
    manifest_unsigned = {
        ...
        "units": {
            path: _sha256_bytes(raw)
            for path, raw in sorted(identity.units.items())
        },
        "configs": {
            path: _sha256_bytes(raw)
            for path, raw in sorted(configs.items())
        },
        "auxiliary_files": {
            path: _sha256_bytes(raw)
            for path, raw in sorted(auxiliary_files.items())
        },
        "config_install_contract": {
        ...
        "service_identity_sha256s": dict(
            identity.service_identity_sha256s
        ),
        "bitrix_socket_access": {
            "group": BITRIX_OPERATIONAL_EDGE_SOCKET_GROUP,
            "group_gid": identity.role_identities[
                "business_edge"
            ].bitrix_socket_gid,
        ...
        "receipt_writer_group": {
            ...
            "gid": identity.role_identities[
                "business_edge"
            ].receipt_writer_gid,
            "members": list(ENDPOINT_ROLES),
            "run_directory_uid": trusted["receipt_contract"][
                "run_directory_uid"
            ],
            "run_directory_gid": trusted["receipt_contract"][
                "run_directory_gid"
            ],
            "run_directory_mode": trusted["receipt_contract"][
                "run_directory_mode"
            ],
    ...
    def collect(
        ...
        *,
        ...
        if (
            ...
            or re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{0,239}", run_id)
            is None
        ):
            _fail(code)
...
def publish_role_native_publication(
    *,
    ...
    if (
        ...
        or tuple(item.kind for item in collected)
        != publication_kinds
    ):
        _fail("native_publication_invalid")
    role_root = root / role
    ...
    def pump_slots(
        ...
        *,
        ...
        if (
            ...
            or any(
                slot not in PRODUCTION_PRE_CLEANUP_PUMP_SLOTS
                for slot in selected
            )
        ):
            _fail("native_publication_pump_slots_invalid")
        return {
            slot: self.pump.produce(
                slot=slot,
                payload=self._wait_payload(
                    slot, deadline=deadline, cancel=cancel
                ),
            )
            for slot in selected
        }
...
class CanonicalWriterProjectionNativeCollector:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or trusted["capability_plan_sha256"]
            != capability_plan_sha256
            or trusted["full_canary_plan_sha256"]
            != full_canary_plan_sha256
    ...
    def _required_values(slot: str, payload: Mapping[str, Any]) -> tuple[str, ...]:
        ...
        if slot == "workspace_writer":
            required.extend(
                (payload.get("owner_grant_id"), payload.get("owner_grant_sha256"))
            )
        ...
        for _page in range(self.maximum_pages):
            ...
            for row in rows:
                if (
                    ...
                    or not row["event_id"]
                    or any(
                        prior.get("event_id") == row["event_id"]
                        for prior in events
                    )
                ):
                    _fail("canonical_writer_native_evidence_invalid")
    ...
    def collect(
        ...
        *,
        ...
        if not events or any(
            not _nested_contains(events, expected) for expected in required
        ):
            _fail(code)
...
class DiscordEdgeNativeCollector:
    ...
    def _public_target(value: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if target_type == "public_channel":
            mapped = "public_guild_channel"
        elif target_type == "public_thread":
            mapped = "public_guild_thread"
        else:
        ...
        if target_type == "public_thread":
            parent = value.get("parent_channel_id")
            if parent is None:
                _fail("discord_native_evidence_invalid")
            target["parent_channel_id"] = parent
        return target
    ...
    def collect(
        ...
        *,
        ...
        target = DiscordPublicTarget.from_mapping(
            self._public_target(target_value)
        )
        ...
        if (
            ...
            or payload.get("private_denial_receipt_sha256")
            != private_denial_sha256
        ):
            _fail(code)
        ...
        edge_source = _sha256_json(
            {
                ...
                "receipt_public_key_id": self.contract[
        ...
        return (
            ...
            NativeEvidenceBinding(
                ...
                verification_receipt_sha256=_sha256_json(
                    {"replayed": first.replayed, "stable_after_dm_probe": True}
                ),
            ),
            NativeEvidenceBinding(
...
def _cleanup_observer_live(value: Mapping[str, Any]) -> bool:
    return (
        ...
        == f"/etc/systemd/system/{_CLEANUP_OBSERVER_UNIT}"
        and value.get("UnitFileState") in {"disabled", ""}
        and value.get("DropInPaths") in {"", "[]"}
...
def build_api_terminal_event_identity(conversation: Any) -> Mapping[str, Any]:
    ...
    if (
        ...
        or any(
            ...
            or re.fullmatch(r"[a-z][a-z0-9_.-]{0,63}", row["event"])
            is None
            for row in event_records
        )
        or type(unsigned["observed_at_unix_ms"]) is not int
        or type(unsigned["completed_at_unix_ms"]) is not int
        or not 0 < unsigned["observed_at_unix_ms"]
        <= unsigned["completed_at_unix_ms"]
    ):
        _fail(code)
...
def validate_gateway_observer_source_projection(
    ...
    *,
    ...
    if (
        ...
        is None
        or _digest(raw["source_fixture_sha256"], code)
        != raw["source_fixture_sha256"]
    ...
    for item in (*readiness.values(), runtime_source["gateway_process_identity_sha256"], runtime_source["discord_connector_readiness_sha256"]):
    ...
    for expected_sequence, item in enumerate(frames, start=1):
        ...
        source_digests.update(
            (record["frame_sha256"], record["payload_sha256"])
        )
    ...
    if (
        ...
        or len(
            [
                ...
                and item["reasoning_directive_effort_sha256"]
                == _sha256_bytes(b"max")
            ]
        )
        != 1
    ...
    proposal_tool_records = [
        ...
        and _sha256_bytes(
            item["tool_call_id"].encode("utf-8", errors="strict")
        )
        == proposal_identity["model_tool_call_id_sha256"]
        and item["result_sha256"]
        == proposal_identity["model_tool_result_sha256"]
    ...
    } if isinstance(restart, Mapping) else {}
    if (
        set(restart_unsigned)
        != {"schema", "service_unit", "command_sha256", "completed_at_unix_ms"}
        or restart.get("schema")
        != "muncho-production-capability-worker-restart.v1"
    ...
    if (
        ...
        != "muncho-capability-canary-api-terminal-identity.v1"
    ...
    for expected_ordinal, item in enumerate(terminal_records, start=1):
        ...
        if (
            ...
            or re.fullmatch(r"[a-z][a-z0-9_.-]{0,63}", record["event"])
            is None
        ):
            _fail(code)
    ...
    for name in (
        ...
        if (
            ...
            or re.fullmatch(
                r"[A-Za-z0-9][A-Za-z0-9._:-]{0,255}", terminal[name]
            )
            is None
        ):
            _fail(code)
...
def _extract_gateway_observer_model_proposal_contract(
    ...
    readbacks = [
        item
        for item in frames
        if getattr(item, "value", {}).get("event")
        == "canonical_case_readback"
    ]
    if len(readbacks) != 1:
        _fail(code)
    ...
    if any(
        not isinstance(item, str)
        or re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{0,255}", item)
        is None
    ...
    if (
        ...
        or re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{0,255}", tool_call_id)
        is None
    ):
        _fail(code)
    ...
    tool_call_id_sha256 = _sha256_bytes(
        tool_call_id.encode("utf-8", errors="strict")
    )
...
def _build_observer_activation_identity(
    *,
    ...
    try:
        ...
        endpoint = producer_readiness["endpoint_readiness"][
            "gateway_observer"
        ]
        endpoint_unsigned = {
            key: copy.deepcopy(item)
        ...
        if (
            ...
            or endpoint.get("readiness_sha256")
            != _sha256_json(endpoint_unsigned)
        ...
        unsigned = {
            ...
            "observer_endpoint_readiness_sha256": endpoint[
                "readiness_sha256"
            ],
            "observer_main_pid": endpoint["main_pid"],
            "observer_service_unit": endpoint["service_unit"],
            "observer_service_identity_sha256": endpoint[
                "service_identity_sha256"
            ],
...
def build_gateway_observer_source_projection(
    *,
    ...
    model_proposal_identities = (
        _extract_gateway_observer_model_proposal_identities(frames)
    )
    try:
        readiness_unsigned = {
            key: copy.deepcopy(item)
        ...
        readiness_identity = {
            ...
            "service_identity_sha256": collector_readiness[
                "service_identity_sha256"
            ],
    ...
    for expected_sequence, collected in enumerate(frames, start=1):
        try:
            ...
            if (
                ...
                or frame.get("canary_run_id")
                != frames[0].value.get("canary_run_id")
                or frame.get("fixture_sha256")
                != frames[0].value.get("fixture_sha256")
            ):
                _fail(code)
            record = {
                "sequence": expected_sequence,
        ...
        if event == "pre_api_request":
            ...
            if any(
                ...
                for item in route_values.values()
            ):
                _fail(code)
            efforts.append(
                {
            ...
            tool_calls.append(
                {
                    ...
                    "reasoning_directive_effort_sha256": (
                        _sha256_bytes(
                            directive_effort.encode("utf-8", errors="strict")
                        )
                        if isinstance(directive_effort, str)
                        and directive_effort
                        else None
                    ),
                }
            )
    ...
    membership = {
        "runtime": {
            ...
            "collector_readiness_receipt_sha256": readiness_identity[
                "receipt_sha256"
            ],
            "runtime_source_identity_sha256": _sha256_json(
                runtime_source_identity
            ),
        ...
        "workspace_gateway": {
            ...
            "worker_restart_receipt_sha256": worker_restart_receipt[
                "receipt_sha256"
            ],
            "model_proposal_core_identity_sha256": (
                model_proposal_identities["workspace_gateway"][
                    "identity_sha256"
                ]
            ),
        },
        "failure_gateway": {
            ...
            "model_proposal_core_identity_sha256": (
                model_proposal_identities["failure_gateway"][
                    "identity_sha256"
                ]
            ),
        },
    ...
    unsigned = {
        ...
        "api_terminal_event_identity": copy.deepcopy(
            dict(api_terminal_event_identity)
        ),
...
class GatewayObserverSourceNativeCollector:
    ...
    def _exact_payload(
        ...
        if (
            ...
            != _GATEWAY_OBSERVER_SOURCE_SLOT_SCHEMAS[slot]
    ...
    def _load(
        ...
        *,
        ...
        if (
            not isinstance(run_id, str)
            or re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{0,255}", run_id)
            is None
        ):
            _fail(code)
    ...
    def _runtime_bindings(
        ...
        if (
            ...
            != list(_GATEWAY_OBSERVER_REQUIRED_TOOLSETS)
        ...
        return (
            NativeEvidenceBinding(
                ...
                artifact_sha256=runtime_source[
                    "gateway_process_identity_sha256"
                ],
    ...
    def _workspace_bindings(
        ...
        if (
            ...
            or payload.get("reasoning_tool_call_id")
            != todo_records[0]["tool_call_id"]
        ...
        terminal_source = _sha256_json(
            {
                ...
                "session_create_request_id": terminal[
                    "session_create_request_id"
                ],
    ...
    def _failure_bindings(
        ...
        for expected, item in zip(
            ...
            if row["alternative_available"]:
                ...
                _fail(code)
            if expected in {"tool", "browser"} and not row[
                "alternative_available"
            ]:
                _fail(code)
        if (
...
class GatewayObserverCleanupNativeCollector:
    ...
    def _load_facts(self, run_id: str) -> tuple[Mapping[str, Any], str]:
        ...
        if (
            ...
            or value["capability_plan_sha256"]
    ...
    def _validate_facts(
        ...
        if not isinstance(observer, Mapping) or not isinstance(
            observer_state, Mapping
        ):
            _fail(code)
        observer_fields = {
            "role",
        ...
        if (
            ...
            or observer["service_state_sha256"]
            != _sha256_json(dict(observer_state))
        ...
        if (
            ...
            or proof["observer_state_sha256"]
            ...
            or proof["observed_at_unix"] * 1000
        ...
        for binding in _CLEANUP_CREDENTIAL_BINDINGS:
            ...
            if (
                ...
                or retirement.get("receipt_sha256")
                != _sha256_json(retirement_unsigned)
        ...
        if (
            ...
            != proof["stop_proof_sha256"]
            or key_retirement.get("receipt_sha256")
            != _sha256_json(key_unsigned)
    ...
    def collect(
        ...
        *,
        ...
        if (
            ...
            or diff.get("capability_plan_sha256")
            != self.config.capability_plan_sha256
            ...
            or diff.get("static_before_sha256")
            != diff.get("static_after_sha256")
        ...
        expected = {
            ...
            "non_observer_service_units": facts[
                "non_observer_stop_order"
            ],
...
def build_role_native_collector(
    ...
    if role == "discord_edge":
        ...
        if (
            ...
            or edge_file_sha256
            != contract["receipt_public_key_file_sha256"]
        ):
