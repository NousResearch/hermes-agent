...
def build_channel_directory(adapters: Dict[Any, Any]) -> Dict[str, Any]:
    ...
    # Platforms without native discovery rely on session history.
    for plat_name in ("telegram", "whatsapp", "signal", "qq_napcat", "email", "sms"):
...
def format_directory_for_display() -> str:
    ...
    for plat_name, channels in sorted(platforms.items()):
        ...
        if plat_name == "discord":
            ...
            lines.append(f"{display_names.get(plat_name, plat_name.title())}:")
            for ch in channels:
