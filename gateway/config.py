...
class SessionResetPolicy:
    """Automatic gateway-session reset policy."""
    mode: str = "none"
...
class GatewayConfig:
    ...
    def get_reset_policy(
        ...
        """Return the platform override, type override, or default policy, in that order."""
...
def _validate_gateway_config(config: "GatewayConfig") -> None:
    ...
    for policy in policies:
        if policy.mode not in {"none", "idle", "daily", "both"}:
            logger.warning("Invalid session reset mode=%s. Using none.", policy.mode)
            policy.mode = "none"
