    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            for plat in _shared_loop_targets:
                ...
                if plat == Platform.DISCORD and "workflow_router" in platform_cfg:
                    # Deterministic /wf-dev managed-dev-workflow control plane
                    # config: {enabled, channels: [...], ...}. Surfaced on the
                    # adapter's config.extra so _wf_dev_enabled / channel-scope
                    # gates can read it.
                    bridged["workflow_router"] = platform_cfg["workflow_router"]
