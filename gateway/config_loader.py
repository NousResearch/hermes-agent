...
def merge_platform_sections(yaml_cfg: dict, gateway_cfg: Any, gw_data: dict) -> dict:
    ...
    Finally api_server's port/key/host/cors_origins/model_name/outbound_files are bridged into ``extra`` so
    ...
    if isinstance(api_plat, dict):
        ...
        for key in ("port", "key", "host", "cors_origins", "model_name", "outbound_files"):
