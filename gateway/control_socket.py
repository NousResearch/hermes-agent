...
Verbs take an optional ``params`` object (``build_control_request``). A handler that declares one
parameter receives the whole request dict; a request-less handler stays argument-less. The payload
rides the same ACL boundary as the socket itself — see ``gateway/vault_unlock.py`` for the one verb
...
def _call_handler(handler: Callable[..., dict[str, Any]], request: dict[str, Any]) -> dict[str, Any]:
    """Invoke a verb handler with the request payload when it declares a parameter.

    ``identify``/``status`` take nothing; a verb whose answer depends on the caller's params
    (``vault-unlock``) declares one and receives the whole request dict.
    """
    try:
        takes_request = bool(inspect.signature(handler).parameters)
    except (TypeError, ValueError):  # C callables / builtins expose no signature
        takes_request = False
    return handler(request) if takes_request else handler()
