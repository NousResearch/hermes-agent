...
# Kept for compatibility with older diagnostics/tests that explicitly lock
# around raw inspection. Runtime writes use the canonical state.db coordinator
# returned by ``_db_write_lock`` below.
...
def _db_write_lock():
    ...
    return get_process_db_write_lock(_db_path())
...
def _connect() -> sqlite3.Connection:
    ...
    if schema_key not in _SCHEMA_READY_PATHS:
        # WAL mode and schema DDL are persistent. Repeating them on every
        # obligation update lengthens the shared writer critical section.
