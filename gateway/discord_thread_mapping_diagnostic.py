...
def inspect_discord_thread_mapping(
    *,
    ...
    state_report, active_session, candidates, state_errors = _inspect_state_db(
        state_db_path,
...
def _inspect_state_db(
    ...
    *,
    ...
    try:
        ...
        return report, active, candidates, []
    finally:
        conn.close()
...
def _fetch_session_metadata(
    ...
    if row is None:
        return None
    return _row_to_session_metadata(row, candidate=False)
...
def _fetch_candidate_orphan_sessions(
    ...
    *,
    ...
    if mapped_session_ids:
        placeholders = ",".join("?" for _ in mapped_session_ids)
        where.append(f"id NOT IN ({placeholders})")
        params.extend(sorted(mapped_session_ids))
