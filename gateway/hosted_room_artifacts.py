...
class RoomArtifactOutbox:
    ...
    def __init__(self, db_path: Path | str, *, root: Path | str | None = None, authorize_write=None) -> None:
        ...
        self.commit_acknowledgement = None
    ...
    def acknowledge(
        ...
        *,
        ...
        with self._lock, self._connect() as conn:
            ...
            if self.authorize_write is not None:
                self.authorize_write(conn, scope)
            if self.commit_acknowledgement is not None:
                self.commit_acknowledgement(conn, scope)
...
def terminal_artifact_manifest(db_path: Path | str, scope: RoomArtifactScope, *, outbox=None) -> dict[str, Any] | None:
    items = (outbox or RoomArtifactOutbox(db_path)).list(scope)
    if not items:
        return None
