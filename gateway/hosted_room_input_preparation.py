...
def reconstruct_accepted_payload(rpc, prompt, attachments, admission, *, retired=False, source_digests=None):
    """No staging or repair; source bytes or attested digests prove exact layout."""
    if source_digests is None:
        ...
        if (not isinstance(source_digests, list) or len(source_digests) != len(manifest)
            ...
            raise RuntimeStoreError('permission_denied')
        validate_media_batch_size(item['size'] for item in manifest)
        inputs = list(zip(manifest, source_digests))
    db = rpc.authority.db
