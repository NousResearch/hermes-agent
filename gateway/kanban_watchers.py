...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        # Event kinds the notifier claims (advances the cursor over) and may
        # surface to subscribers. NOT all "terminal": ``review_requested`` and
        # ``block_loop_detected`` leave the task live.
        ...
        #   - ``block_loop_detected``: a genuine unblock<->re-block loop
        #     (needs_input/failed) escalates the task to 'triage'. Surface it too,
        #     otherwise the task parks silently in triage and is never seen.
        #   - ``status`` covers dashboard drag-drop and ``_set_status_direct()``
        #     writes — surface those transitions to subscribers too.
        #   - ``archived`` / ``unblocked`` are claimed so the cursor advances past
        #     them (they can't wedge a later event behind an unclaimed row) but
        #     are delivered silently (see the else-branch below).
        NOTIFY_KINDS = (
                    ...
                    for ev in d["events"]:
                        ...
                        if kind == "completed":
                            ...
                            if ev.payload and ev.payload.get("reason"):
                                reason = f": {str(ev.payload['reason'])[:160]}"
                            msg = (
                                f"🛑 {board_tag}{tag}Kanban {sub['task_id']} escalated to "
