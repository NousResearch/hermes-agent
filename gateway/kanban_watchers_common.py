...
def _list_boards() -> list:
    ...
    try:
        return boards.list_boards(include_archived=False)
    except Exception:
        return [boards.read_board_metadata(boards.DEFAULT_BOARD)]


def _board_slugs() -> list:
    from hermes_cli.kanban_db_boards import DEFAULT_BOARD

    return [b.get("slug") or DEFAULT_BOARD for b in _list_boards()]
