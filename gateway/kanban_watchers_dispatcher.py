...
def _resolve_dispatcher_settings(kanban_cfg: dict, kb: Any) -> _DispatcherSettings:
    ...
    raw_failure_limit = kanban_cfg.get("failure_limit", _kbd().DEFAULT_FAILURE_LIMIT)
    try:
        failure_limit = int(raw_failure_limit)
    except (TypeError, ValueError):
        logger.warning("kanban dispatcher: invalid kanban.failure_limit=%r; using default %d",
                       raw_failure_limit, _kbd().DEFAULT_FAILURE_LIMIT)
...
def _log_spawn_results(results: Optional[list]) -> bool:
    ...
    for slug, res in (results or []):
        adopted = getattr(res, "adopted", None) if res is not None else None
        if res is not None and (getattr(res, "spawned", None) or adopted):
            any_spawned = any_spawned or bool(res.spawned)
            # Quiet by default: an idle gateway stays silent.
            logger.info(
                "kanban dispatcher [%s]: spawned=%d adopted=%d reclaimed=%d "
                "crashed=%d timed_out=%d promoted=%d auto_blocked=%d",
                slug, len(res.spawned), len(adopted or ()), res.reclaimed,
