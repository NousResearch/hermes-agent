...
class _Collector:
    ...
    def collect(self) -> list[dict]:
        ...
        for board_meta in _list_boards():
            slug = board_meta.get("slug") or boards.DEFAULT_BOARD
            db_path = board_meta.get("db_path")
            try:
                resolved_db_path = str(Path(db_path).expanduser().resolve()) if db_path else str(_owner_kanban_db.kanban_db_path(slug).resolve())
            except Exception:
