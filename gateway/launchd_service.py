...
def _gateway_run_command() -> list[str]:
    ...
    return [service_process.python_path(), "-m", "hermes_cli.main", *service_identity.profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    return [service_process.python_path(), "-m", "hermes_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
