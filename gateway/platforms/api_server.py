...
class APIServerAdapter(BasePlatformAdapter):
    ...
    # [jb] Authorization is performed UPSTREAM by the API-key boundary: every
    # handler runs ``_check_auth`` (Bearer API_SERVER_KEY, required even on
    # loopback — ``connect()`` refuses to start without it), and the only events
    # this adapter injects into the gateway loop are the synthetic /goal arming
    # + continuation turns of a managed mission driven by the co-located control
    # daemon. There is no platform user account for the env allowlist to name,
    # so the gateway's per-user allowlist must not default-deny these turns.
    # Declaring the NATIVE seam (see gateway/authz_mixin.py) replaces the
    # historical Platform.API_SERVER special-case that used to be patched into
    # ``_is_user_authorized`` in gateway/run.py.
    @property
