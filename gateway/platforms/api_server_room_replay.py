...
def _legacy_policy(adapter, record, scope, session_id):
    from gateway.platforms.api_server_authority_runs import run_admission
    owned = run_admission(adapter, record['run_id'])
    if owned is None:
        raise RuntimeStoreError('storage_unavailable')
    row = owned[1]
    turn = row['payload'].get('api_turn_v1', {})
    if row['target_session_id'] != session_id or turn.get('run_owner_scope') != scope:
        raise RuntimeStoreError('admission_conflict')
    policy = turn.get('settings', {}).get('room_execution_policy')
    if policy is None:
        raise RuntimeStoreError('storage_unavailable')
    return policy
...
def room_replay(adapter, request, dispatch, *, _openai_error):
    ...
    if policy is None:
        policy = _legacy_policy(adapter, record, scope, session_id)
