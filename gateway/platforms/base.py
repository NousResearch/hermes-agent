...
MEDIA_DELIVERY_EXTS: Tuple[str, ...] = (
    ...
    # Documents (uploaded as file attachments). Keep this broad enough to
    # cover SUPPORTED_DOCUMENT_TYPES entries that agents commonly produce as
    # artifacts (.py/.toml/.log/.sh), not only office-style documents.
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                local_files = []
                if not is_ephemeral_response and self.auto_attach_local_paths_enabled():
                    # Auto-detect bare local file paths for native media delivery
                    # (helps small models that don't use MEDIA: syntax). Skip
                    # system/command notices so config paths stay visible text
                    # instead of becoming native uploads. Gated so operators can
                    # disable opportunistic path scraping while keeping explicit
                    # MEDIA:<path> delivery available.
