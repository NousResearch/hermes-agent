    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if admission is not None:
                # Best-effort: closing a native lease must never skip the mandatory
                # turn-marker release and session cleanup below.
                try:
                    current = asyncio.current_task()
                    ...
                    logger.warning("[%s] Native turn admission abort failed", self.name, exc_info=True)
            await self._release_turn_marker(event)
