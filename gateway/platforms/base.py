...
def source_is_group_chat(source) -> bool:
    """Return True when a source is a public/multi-user chat."""
    chat_type = str(getattr(source, "chat_type", "") or "").lower()
    chat_id = str(getattr(source, "chat_id", "") or "")
    return chat_type in {"group", "supergroup", "channel"} or chat_id.startswith(("group:", "g:"))
...
class BasePlatformAdapter(ABC):
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if source_is_group_chat(event.source):
                logger.info(
                    "[%s] Suppressing public error notice for group chat %s",
                    self.name,
