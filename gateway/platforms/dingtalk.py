...
Configuration in config.yaml:
    platforms:
      dingtalk:
        enabled: true
        require_mention: true        # respond only when @mentioned in groups (default: true)
        extra:
...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Mention state is the structured `isInAtList` flag from the
        # DingTalk callback, not text parsing.
    ...
    async def _on_message(
        ...
        *,
        ...
        # Group-mention guard: skip group messages that don't @ the bot.
        # DMs are always processed.
        if is_group and self._require_mention and not is_bot_mentioned:
            logger.debug(
                "[%s] Group message without @mention, skipping",
                self.name,
            )
            return
...
class _IncomingHandler(
    ...
    async def process(self, message: "CallbackMessage"):
        ...
        try:
            ...
            # Structured mention signal — DingTalk sets isInAtList=true
            # when the bot is in the @-list of the incoming message.
            # This is the authoritative mention check; do NOT parse text.
            ...
            # _on_message returns as soon as the agent task is spawned —
            # not when the reply is ready.  DO NOT fire "Done" here; the
            # reply is still being generated.  The adapter emits Done from
            # send()/edit_message() success paths via _fire_done_reaction.
            await self._adapter._on_message(
                chatbot_msg, is_bot_mentioned=is_bot_mentioned,
            )
