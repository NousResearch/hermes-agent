...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _notification_kwargs(
        ...
        """Return ``{"silent": True}`` when non-final responses should suppress notifications.

        Matches the Telegram ``notifications: important`` convention --
        intermediate tool-call / progress messages get ``silent=True``
        while final replies (marked with ``"notify": True`` in metadata)
        go through normally.
        """
