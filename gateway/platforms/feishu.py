...
# Feishu Message Content 1.0 'post' type 'md' elements do not render tables,
# so we route these to interactive cards (Card JSON 2.0) which support them natively.
...
def _build_interactive_markdown_payload(content: str) -> str:
    """Build a Card JSON 2.0 interactive payload with a markdown element.

    Card JSON 2.0's markdown component supports tables, headings,
    code blocks, and ordered/unordered lists natively — features
    that are unavailable in Message Content 1.0 post format's 'md' tag.
    """
    return json.dumps(
        {
            "schema": "2.0",
...
class FeishuAdapter(BasePlatformAdapter):
        ...
        *,
        ...
        try:
            ...
            if not result.success:
                if msg_type == "post" and _POST_CONTENT_INVALID_RE.search(result.error or ""):
    ...
    def _build_outbound_payload(self, content: str) -> tuple[str, str]:
        # Feishu Message Content 1.0 post-type 'md' elements do not render
        # markdown tables; route these to interactive cards (Card JSON 2.0)
        # which support tables natively via the markdown component.
        if _MARKDOWN_TABLE_RE.search(content):
            return "interactive", _build_interactive_markdown_payload(content)
