...
# Structured markdown: tables, code blocks, h2/h3 headings, 3+ list items.
# These go to interactive cards with native table elements for full rendering.
...
def _extract_card_header(content: str) -> Optional[str]:
    """Extract the first # heading from content for use as the card title.

    Returns None when no heading is found so the card stays headerless.
    """
    if not content:
        return None
...
def _build_interactive_card_payload(content: str) -> str:
    ...
    Converts markdown tables to Feishu's native ``table`` element for
    pixel-perfect column alignment. All other content (headings, lists,
    code blocks, inline formatting) is rendered via ``markdown`` elements.
...
def _build_card_elements(content: str) -> List[Dict[str, Any]]:
    ...
    prose/text blocks or native ``table`` elements for detected markdown
    tables.
    """
    if not content:
...
def _split_content_segments(lines: List[str]) -> List[tuple[str, List[str]]]:
    ...
    while i < n:
        line = lines[i]
        # Try to detect a table: need at least 3 lines (header + sep + 1 data)
        if (
            i + 2 < n
...
def _table_col_name(header: str, all_headers: List[str]) -> str:
    """Generate a unique column name from a header string.

    Uses a short prefix of the header text, appending a number for uniqueness.
    """
    base = re.sub(r"[^a-zA-Z0-9_]", "_", header.lower())[:12] or "col"
    if all_headers.count(header) > 1 or any(
        h != header and re.sub(r"[^a-zA-Z0-9_]", "_", h.lower())[:12] == base
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _build_outbound_payload(self, content: str) -> tuple[str, str]:
        ...
        # multi-level headings, 3+ list items) — full native table rendering.
