...
class FeishuAdapter(BasePlatformAdapter):
    ...
    @staticmethod
    def _split_content_and_tables(content: str) -> list:
    ...
    def _build_outbound_payload(self, content: str) -> tuple[str, str]:
        """Build outbound Feishu message payload.

        Uses interactive card (JSON 2.0) for all messages so that markdown
        tables are rendered as native ``table`` components instead of being
        dumped as raw ``| |`` text that Feishu cannot render.

        Non-table text is placed in ``markdown`` elements which support bold,
        italic, strikethrough, links, code blocks, lists, and dividers.
        """
