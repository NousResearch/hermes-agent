...
adapter does not grow further and comment-related logic can evolve
independently.
...
_ADD_COMMENT_URI = "/open-apis/drive/v1/files/:file_token/comments"
_LEGACY_ADD_COMMENT_URI = "/open-apis/drive/v1/files/:file_token/new_comments"
...
_OPEN_ID_MENTION_RE = re.compile(r"@(ou_[A-Za-z0-9]+)")


def _build_comment_content_elements(text: str) -> List[Dict[str, Any]]:
    """Build Drive comment content elements, preserving @open_id mentions."""
    elements: List[Dict[str, Any]] = []
    cursor = 0
    for match in _OPEN_ID_MENTION_RE.finditer(text):
        if match.start() > cursor:
            segment = _sanitize_comment_text(text[cursor:match.start()])
            if segment:
                elements.append({"type": "text_run", "text_run": {"text": segment}})
        elements.append({"type": "person", "person": {"user_id": match.group(1)}})
        cursor = match.end()

    if cursor < len(text):
        segment = _sanitize_comment_text(text[cursor:])
        if segment:
            elements.append({"type": "text_run", "text_run": {"text": segment}})

    if not elements:
        elements.append({"type": "text_run", "text_run": {"text": ""}})
    return elements


def _build_legacy_reply_elements(text: str) -> List[Dict[str, str]]:
    """Build legacy new_comments reply_elements payload."""
    return [{"type": "text", "text": _sanitize_comment_text(text)}]
...
async def reply_to_comment(
    ...
    body = {
        "content": {
            "elements": _build_comment_content_elements(text),
        }
    }
...
async def add_whole_comment(
    ...
    if code != 0:
        legacy_body = {
            "file_type": file_type,
            "reply_elements": _build_legacy_reply_elements(text),
        }
        legacy_code, legacy_msg, _ = await _exec_request(
            client, "POST", _LEGACY_ADD_COMMENT_URI,
            paths={"file_token": file_token},
            body=legacy_body,
        )
        if legacy_code == 0:
            logger.info("[Feishu-Comment] add_whole_comment OK via legacy endpoint")
            return True
        logger.warning(
            "[Feishu-Comment] add_whole_comment legacy fallback FAILED: code=%s msg=%s",
            legacy_code, legacy_msg,
        )
    if code != 0:
...
_ALLOWED_NOTICE_TYPES = {"add_comment", "add_reply", "comment_add"}
