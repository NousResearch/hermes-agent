...
class LinearAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self.comment_wake_states = _split_csv(
            ...
            list(dict.fromkeys(self.start_states + self.waiting_states + [self.review_state, self.working_state])),
        )
        try:
    ...
    async def send(
        ...
        try:
            ...
            if comment_id:
                ...
                self._save_state()
            if target_state and not metadata.get("linear_skip_state_transition"):
    ...
    async def _poll_once(self) -> None:
        ...
        for issue in issues:
            ...
            if issue_id and issue_id not in self._seen_issue_ids and state_name in self.start_states:
                self._seen_issue_ids.add(issue_id)
                if is_status_issue or self.execution_mode != "kanban":
