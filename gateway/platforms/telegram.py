...
class TelegramAdapter(BasePlatformAdapter):
    ...
    @staticmethod
    def _telegram_chat_id_arg(chat_id: str) -> int | str:
        """Return a Telegram API chat_id argument without rejecting usernames.

        Telegram accepts integer IDs and string usernames.  Numeric IDs are
        still coerced so existing DM/group behavior is unchanged, while
        non-numeric configured targets (for example a channel username) are
        passed through to python-telegram-bot instead of failing locally.
        """
        try:
            return int(chat_id)
        except (TypeError, ValueError):
                    ...
                    try:
                        ...
                        try:
                            msg = await self._bot.send_message(
                                chat_id=self._telegram_chat_id_arg(chat_id),
                                text=chunk,
