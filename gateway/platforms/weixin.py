...
def _extract_reply_context(item_list: List[Dict[str, Any]]) -> tuple[str | None, str | None]:
    for item in item_list:
        ref = item.get("ref_msg") or {}
        if not isinstance(ref, dict) or not ref:
            continue
        ref_item = ref.get("message_item") or {}
        if not isinstance(ref_item, dict) or not ref_item:
            continue
        reply_to_message_id = str(
            ref.get("message_id")
            or ref.get("msg_id")
            or ref_item.get("message_id")
            or ref_item.get("msg_id")
            or ""
        ).strip() or None
        parts: List[str] = []
        ...
        if title:
            parts.append(title)
        reply_text = _extract_text([ref_item])
        if reply_text:
            parts.append(reply_text)
        reply_to_text = " | ".join(parts).strip() or None
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._group_archive_store = WeixinGroupArchiveStore()
    ...
    async def _process_message(self, message: Dict[str, Any]) -> None:
        ...
        if chat_type == "group":
            if not self._group_message_allowed(effective_chat_id):
                return
        ...
        reply_to_message_id, reply_to_text = _extract_reply_context(item_list)
        metadata = None
        policy: Dict[str, Any] | None = None
        if chat_type == "group":
            policy = self._effective_group_policy(effective_chat_id)
            mode = str(policy.get("mode") or "default").strip().lower() or "default"
            allow_model_dispatch = mode not in {"disabled", "collect_only"}
            metadata = build_group_message_metadata(
                trigger_reason="require_mention_disabled" if allow_model_dispatch else None,
                group_policy_mode=mode,
        ...
        if chat_type == "group":
            ...
            if bool(policy.get("archive_enabled")):
                await self._archive_group_message(
                    chat_id=effective_chat_id,
                    message_id=message_id or f"weixin:{sender_id}:{int(observed_at.timestamp())}",
            ...
            if mode in {"disabled", "collect_only"}:
                return
        ...
        *,
        ...
        try:
            await asyncio.to_thread(
                self._group_archive_store.archive_inbound_message,
                chat_id=chat_id,
            ...
            logger.exception("[%s] Failed to archive Weixin group message %s", self.name, message_id)
