...
def _can_reuse_live_send_session(send_session: Any) -> bool:
    """Return True when a live Weixin send session is safe to reuse.

    ``send_message`` may bridge into a fresh event loop from a sync tool call.
    Reusing an aiohttp session created on the gateway's loop from that new loop
    triggers aiohttp timeout-manager crashes. Only reuse the live adapter when
    its send session belongs to the current running loop.
    """
    if send_session is None or getattr(send_session, "closed", True):
        return False
    try:
...
async def send_weixin_direct(
    *,
    ...
    if live_adapter is not None and _can_reuse_live_send_session(send_session):
