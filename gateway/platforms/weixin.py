...
class WeixinAdapter(BasePlatformAdapter):
    ...
    async def _send_text_chunk_locked(
        ...
        *,
        ...
        if last_error is None:
            raise RuntimeError("_send_chunk_with_retry exited without error — this should not happen")
        raise last_error
    ...
    async def _download_remote_media(self, url: str) -> str:
        ...
        if self._send_session is None:
            raise RuntimeError("_download_remote_media called before _send_session is initialized")
        # Use asyncio.wait_for() instead of aiohttp ClientTimeout to avoid
        # "Timeout context manager should be used inside a task" errors.
        async def _do_fetch():
    ...
    async def _send_file(
        ...
        if self._send_session is None or self._token is None:
            raise RuntimeError("_send_file called before _send_session or _token is initialized")
