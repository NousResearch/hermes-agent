...
class GatewayRunner:
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        # Channel-to-repo binding awareness
        if source.platform == Platform.DISCORD:
            try:
                from tools.channel_bindings import get_binding as _get_binding

                _binding = _get_binding(source.chat_id)
                if _binding:
                    _wp = _binding.get("workspace_path", "")
                    _rn = _binding.get("repo_name", "")
                    _ru = _binding.get("repo_url", "")
                    _br = _binding.get("branch", "main")
                    context_prompt += (
                        f"\n\n[Bound project]\n"
                        f"This channel is bound to `{_rn}` ({_ru}).\n"
                        f"Repo cloned at `{_wp}` (branch: `{_br}`).\n"
                        f"Use `terminal(workdir='{_wp}', ...)` or `cd {_wp} && ...` "
                        f"when working with this repo.\n]"
                    )
            except Exception as e:
                logger.debug("Channel binding context injection failed: %s", e)
    ...
    async def _handle_bind_command(self, event: MessageEvent) -> str:
        """Handle /bind <url|new|info|unbind|list> [repo-name] command."""
        args = event.get_command_args().strip()
        chat_id = event.source.chat_id

        from tools.channel_bindings import (
            get_binding,
            set_binding,
            remove_binding,
            list_all_bindings,
        ...
        if not args or args == "info":
            ...
            if binding:
                return (
                    f"**Channel binding:**\n"
                    f"Repo: `{binding['repo_name']}`\n"
                    f"URL: {binding['repo_url']}\n"
                    f"Branch: `{binding.get('branch', 'main')}`\n"
                    f"Workspace: `{binding['workspace_path']}`"
                )
            return (
                "No binding for this channel.\n"
                "Use `/bind <github-url>` to clone an existing repo, "
                "or `/bind new <repo-name>` to create one."
            )
        ...
        if args == "list":
            ...
            lines = ["**All channel bindings:**"]
            for cid, b in bindings.items():
                lines.append(f"• <#{cid}> → `{b['repo_name']}` ({b['repo_url']})")
            return "\n".join(lines)
        ...
        if args == "unbind":
            binding = get_binding(chat_id)
            if not binding:
                return "No binding for this channel. Nothing to remove."
            if remove_binding(chat_id):
                return (
                    f"Binding removed.\n"
                    f"Was bound to `{binding['repo_name']}` — this channel "
                    f"will no longer have project context."
                )
            return "Failed to remove binding."

        # /bind new <owner/repo>
        if args.startswith("new"):
            parts = args.split(None, 1)
            repo_name = parts[1].strip() if len(parts) > 1 else None
            if not repo_name:
                return (
                    "Usage: `/bind new <owner/repo>`\n"
                    "Example: `/bind new vicente/my-project`"
                )
            ...
            if "/" not in repo_name:
                try:
                    proc = await asyncio.create_subprocess_shell(
                        "gh api user --jq '.login' 2>/dev/null || echo '__FAILED__'",
                        stdout=asyncio.subprocess.PIPE,
                        stderr=asyncio.subprocess.PIPE,
                    )
                    stdout, _ = await asyncio.wait_for(proc.communicate(), timeout=10)
                    current_user = stdout.decode().strip()
                    if current_user and current_user != "__FAILED__":
                        full_name = f"{current_user}/{repo_name}"
                    else:
                        return (
                            "Could not determine your GitHub username.\n"
                            "Run `gh auth login` first, then use full `owner/repo` format."
                        )
                except asyncio.TimeoutError:
                    return "Timed out determining GitHub user."
            else:
                full_name = repo_name
            ...
            try:
                proc = await asyncio.create_subprocess_shell(
                    f"gh repo view {full_name} --json name 2>/dev/null",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                )
                ...
                if proc.returncode == 0:
                    return (
                        f"Repo `{full_name}` already exists.\n"
                        f"Use `/bind https://github.com/{full_name}` to bind directly."
                    )
            except asyncio.TimeoutError:
                return "Timed out checking if repo exists."

            try:
                proc = await asyncio.create_subprocess_shell(
                    f"gh repo create {full_name} --public --clone",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                    cwd="/home/hermes-agent/projects",
                )
                stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=30)
                combined = f"{stdout.decode()} {stderr.decode()}"
                if proc.returncode != 0:
                    return f"Failed to create repo: {combined.strip()[:500]}"
            except asyncio.TimeoutError:
                return "Timed out creating repo."

            workspace_path = os.path.join("/home/hermes-agent/projects", repo)
            ...
            return (
                f"**Repo created and bound!** 🎉\n"
                f"Repo: `{full_name}`\n"
                f"URL: {repo_url}\n"
                f"Workspace: `{workspace_path}`\n\n"
                f"I'll now work with this repo when tasks come in this channel. "
                f"Just describe what you need!"
            )

        # Default: treat arg as GitHub URL or owner/repo
        github_url = args
        if not github_url.startswith("http"):
            if "/" not in github_url:
                return (
                    "Usage: `/bind <github-url>` or `/bind new <owner/repo>`\n"
                    "Examples:\n"
                    "• `/bind https://github.com/vicente/my-project`\n"
                    "• `/bind vicente/my-project`\n"
                    "• `/bind new my-project` (creates a new repo)"
                )
        ...
        parts = clean_url.replace("https://github.com/", "", 1).split("/", 2)
        if len(parts) < 2:
            return "Could not parse repo from URL. Use `https://github.com/owner/repo`."
        owner, repo = parts[0], parts[1]
        repo_name = f"{owner}/{repo}"

        projects_dir = "/home/hermes-agent/projects"
        workspace_path = os.path.join(projects_dir, repo)
        ...
        if os.path.exists(os.path.join(workspace_path, ".git")):
            ...
            return (
                f"**Repo already cloned and bound!**\n"
                f"Repo: `{repo_name}`\n"
                f"URL: {github_url}\n"
                f"Workspace: `{workspace_path}`"
            )
        ...
        try:
            ...
            try:
                ...
                if (
                    proc.returncode == 0
                    and "not logged into any GitHub host" not in output
                ):
            ...
            combined = f"{stdout.decode()} {stderr.decode()}"
            if proc.returncode != 0:
                return f"Failed to clone repo: {combined.strip()[:500]}"
        except asyncio.TimeoutError:
            return (
                f"Clone timed out. Clone manually:\n"
                f"`git clone {github_url} {workspace_path}`\n"
                f"Then re-run `/bind`."
            )
