...
def _auto_archive_one_home() -> None:
    """Run the config-gated auto-archive sweep for the scoped profile's store.

    Takes no path: ``acquire()`` resolves ``_default_db_path()`` fresh from ``get_hermes_home()``
    at call time, which the caller's ``_profile_runtime_scope`` has already pointed at the right
    profile — the same mechanism that makes ``load_config()`` read that profile's config."""
    from hermes_cli.config import load_config as _load_full_config
...
def _housekeeping_auto_archive() -> None:
    """Stale-session auto-archive for the CURRENTLY SCOPED profile; maybe_auto_archive() is gated by
    sessions.min_interval_hours. Opens its own SessionDB — SQLite connections are thread-bound.

    Registered through ``profile_scoped_chore()``, so ``_for_each_served_profile`` runs this once per
    served profile inside that profile's runtime scope. That matters twice over: ``load_config()`` then
    reads THAT profile's ``sessions`` block, and ``${VAR}`` in it expands through the profile's own
    secret scope instead of the launch process environment.

    Sweeping satellites is not optional. A served profile writes no ``gateway.pid`` of its own, so the
    dashboard stands down for it (``_auto_archive_owned_by_gateway`` in
    ``hermes_cli/web_server_sessions.py``) rather than tearing the WAL generation this process holds —
    which leaves the multiplexer as the only remaining sweeper for those stores (#109727).

    Never raises. ``_for_each_served_profile`` does not isolate its profiles, so an unreadable store
    would otherwise abandon every profile after it in the same tick — and the dashboard has already
    stood down for those (#110405 review). ``GatewayRunner._init_session_db()`` tolerates a failed
    primary-store init and keeps running, so an unavailable launch store alongside healthy satellites
    is a state the gateway genuinely reaches.
    """
    try:
        _auto_archive_one_home()
    except Exception as exc:
        logger.debug("Auto-archive tick skipped for the scoped profile: %s", exc)
...
def _start_gateway_housekeeping(
    ...
    chores += [
        ...
        # Per served profile like the chores above: a served satellite has no gateway.pid, so the
        # dashboard stands down for it and this is its only sweeper (#109727).
