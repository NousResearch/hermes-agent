...
class GatewayRunner:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # Intercept reserved words (e.g. "/model list") that are NOT model names
        from hermes_cli.model_switch import RESERVED_MODEL_WORDS
        if model_input.lower() in RESERVED_MODEL_WORDS and not explicit_provider:
            return (
                f"'{model_input}' is not a model name.\n"
                "Usage:\n"
