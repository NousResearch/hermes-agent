...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    # Every pooled credential for the primary is benched by a 429. The key we
    # just resolved would 429 too, so hand over to the fallback chain before
    ...
    if _fb_model:
        # Mirrors _try_resolve_fallback_provider: the caller needs the
        # fallback's model, not the configured primary's.
