...
class GatewayInboundMixin:
    ...
    async def _hm_release_approval_intercept(self, event, source) -> tuple[bool, Optional[str]]:
        ...
        if not event.allow_gateway_control or (event.text or "") != "freigegeben":
            return False, None
        ...
        if not isinstance(argv, list) or not argv or not all(isinstance(v, str) and v for v in argv):
            return True, (
                "Fehler: Freigabe konnte nicht verarbeitet werden. Release-ID: unbekannt. "
                "Dev: unbekannt. Test: unbekannt. Prod: unbekannt. Aktiv: unbekannt. "
                "Vorgänger: unbekannt. Rollback verfügbar: unbekannt."
            )
        from hermes_cli.kanban_release_approval import ApprovalContext, process_current_board_approval
