...
class GatewayInboundMixin:
    ...
    async def _hm_pre_gateway_dispatch_hook(
        ...
        try:
            ...
            _hook_results = await invoke_hook_async(
    ...
    async def _hm_admit_event(
        ...
        hook_result = self._hm_pre_gateway_dispatch_hook(event, source)
        event = await hook_result if inspect.isawaitable(hook_result) else hook_result
        if event is None:
            return None
        source = event.source
