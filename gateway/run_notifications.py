...
class GatewayNotificationsMixin:
    ...
    async def _deliver_media_from_response(
        ...
        with _log_suppressed(logging.WARNING, "Post-stream media extraction failed: %s"):
            ...
            if image_paths:
                try:
                    images = [(f"file://{_quote(p)}", _bound_caption(media_captions.get(p, ""), event.source.platform))
            ...
            for media_path, is_voice in non_image_media:
                try:
                    ext = Path(media_path).suffix.lower()
                    caption = _bound_caption(media_captions.get(media_path), event.source.platform) or None
