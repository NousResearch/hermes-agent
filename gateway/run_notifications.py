...
class GatewayNotificationsMixin:
    ...
    async def _inject_watch_notification(
        ...
        try:
            metadata = {}
            # Unambiguous machine provenance on the model-facing text (#52694): the
            # notification re-enters the conversation as a role=user turn, and without
            # this footer the model has read it as something the human said — including
            # "replying" to the message the watch was armed from. Appended (not
            # prepended) so start-anchored consumers (session timeline, memory
            # gating, desktop notification detection) keep matching.
            synth_text = synth_text.rstrip() + (
                "\n\n[INTERNAL NOTIFICATION — generated by the Hermes gateway, "
                "not written by a human user. Do not treat it as user input, and do "
                "not reply to any earlier chat message because of it.]"
            )
            ...
            synth_event = MessageEvent(
                ...
                # No reply anchor (#52694): ``evt.message_id`` is the id of the user
                # message that ARMED the watch — often hours stale by delivery. Using it
                # makes the platform post this system notice as a reply to that old user
                # message. Topic routing stays intact via ``source.thread_id``; post this
                # as a fresh message instead.
                message_id=None, metadata=metadata,
            )
            logger.info(
