...
class GatewayTopicThreadsMixin:
    ...
    def _schedule_rename_from_title_thread(self, source: SessionSource, make_coro, label: str) -> None:
        ...
        with suppress(Exception):
            ...
            # Same for the admitting home: ownership rechecks on the loop thread must read the
            # OWNING profile's store, not whichever scope happens to be ambient there.
            stamped_home = getattr(source, "_authorization_profile_home", None)
            if stamped_home is not None:
                setattr(copied_source, "_authorization_profile_home", stamped_home)
