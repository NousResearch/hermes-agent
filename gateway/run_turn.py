...
class GatewayTurnMixin:
    ...
    async def _handle_oauth_reauth_reply(self, event, source, ref_check: "ReplyRefCheck") -> None:
        ...
        """
        from gateway.run import (
            _OAUTH_REAUTH_SUBSYSTEM, extract_oauth_code_from_reply, _reset_oauth_reauth_notify_state,
        )
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str, run_generation: int):
        ...
        # oauth_reauth no-agent short-circuit — as early as feasible, before ANY session/agent-dispatch
        # machinery runs. This is a NARROW, ADDITIVE special case: check_reply_for_pending_ref() is reused
        # unchanged (not forked), and every other subsystem falls through untouched to the normal agent-turn
        # path below, exactly as before. Only a quote-reply whose quoted text carries a LIVE
        # "[ref:oauth_reauth:<id>]" tag is handled here, deterministically, with no LLM/agent turn at all.
        if getattr(event, "reply_to_text", None) and event.reply_to_message_id:
            from gateway.run import _OAUTH_REAUTH_SUBSYSTEM, check_reply_for_pending_ref
