...
class GatewayTurnMixin:
    ...
    def _hmwa_prepend_reasoning(self, agent_result, response, source, _intentional_silence):
        ...
        Return display text and an explicit speech override so auto-TTS never has to infer
        gateway provenance from visible text.
        """
    ...
    def _hmwa_carry_spoken_response_suffix(spoken_response, response_before, response_after):
        ...
        # A future replacing post-processor must not silently drop its warning.
        return after
    ...
    async def _hmwa_deliver_turn_response(
        ...
        spoken_response = response if _spoken_response_override is None else _spoken_response_override
        if _spoken_response_override is not None:
            # The base adapter owns non-streaming voice-input fallback delivery.
        ...
        if not _streaming_tts_done and self._should_send_voice_reply(
            event, spoken_response, agent_messages, already_sent=bool(agent_result.get("already_sent")),
        ):
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str, run_generation: int):
        ...
        try:
            ...
            if _footer_line and response and not agent_result.get("already_sent") and not _intentional_silence:
                _response_before_footer = response
                response = f"{response}\n\n{_footer_line}"
                _spoken_response_override = self._hmwa_carry_spoken_response_suffix(
                    _spoken_response_override, _response_before_footer, response
                )
