...
class TurnRunner:
    ...
    def _progress_build_message(self, tool_name, preview, args) -> Optional[str]:
        ...
        prepared = prepare_tool_preview(
            tool_name, args, fallback=preview, max_len=self._preview_cap()
        )
        preview = (
        ...
        return (
            ...
            else f"{emoji} {verb}{tool_verb_connector(tool_name)}{preview}"
        )
    ...
    async def _task_card_publish(self, st) -> None:
        ...
        destination_supported = getattr(
            st.adapter, "native_task_card_destination_supported", None
        )
        if (
            callable(destination_supported)
            and destination_supported(
                ctx.source.chat_id,
        ...
        if not st.native_failed:
            ...
            if getattr(result, "error", None) in _CARD_DESTINATION_REFUSALS:
                self._task_card_uncardable_destination(
                    st, getattr(result, "error", "unknown error")
                )
                if st.publication_suppressed:
                    return
            else:
    ...
    def _task_card_uncardable_destination(
        self, st, reason: str = "no thread anchor"
    ) -> None:
        ...
        if self._ctx.tool_progress_enabled:
            ...
            logger.info(
                ...
                "tool progress continues as an editable text update",
                reason,
            )
            return
        st.publication_suppressed = True
        logger.info(
            "Slack native task cards are unsupported for this destination (%s); "
            "tool progress stays off for this turn",
            reason,
        )
    ...
    async def send_progress_messages(self):
        ctx = self._ctx
        adapter = (
            self._runner._delivery_adapter_for(ctx.source)
    ...
    def _setup_stream_consumer(self, platform_key):
        ...
        want_stream_deltas = not ctx.scheduled_heartbeat and scfg.enabled_for(
            plat_streaming
        )
        ...
        if want_stream_deltas or want_interim_messages:
            try:
                ...
                adapter = self._runner._delivery_adapter_for(ctx.source)
                if adapter:
                    supports_incremental_stream = getattr(
                        adapter, "SUPPORTS_MESSAGE_EDITING", True
                    ) or bool(getattr(adapter, "SUPPORTS_NATIVE_STREAMING", False))
                    consumer_stream_deltas = (
                        want_stream_deltas and supports_incremental_stream
                    )
        ...
        delta_sinks = [
            ...
            for sc in (
                stream_consumer
                if stream_consumer and stream_consumer.stream_deltas_enabled
                else None,
                stts,
            )
            if sc is not None
        ...
        def interim_assistant_cb(text: str, *, already_streamed: bool = False) -> None:
            if not ctx._run_still_current():
                return
            # Retain the raw payload when it carries a MEDIA directive: display cleaning
            # strips the directive before send, so this is the only carrier that can
            # reach the post-turn delivery rails (#99409).
            if text and "MEDIA:" in text:
                ctx.interim_media_payloads.append(str(text))
            if stts is not None:
    ...
    def _notice_callback_sync(self, notice) -> None:
        ...
        render_notification(
            present,
            platform=self._ctx.source.platform,
            user_config=self._ctx.user_config,
    ...
    def _clarify_callback_sync(
        self, question: str, choices, multi_select: bool = False, questions=None
    ) -> str:
    ...
    def _ask_clarify_question(
        self, question, choices, multi_select, rearm: bool = True
    ) -> tuple[str, bool]:
        ...
        def _text_fallback():
            ...
            return (
                None
                if coro is None
                else self._schedule(coro, "Clarify text fallback failed to schedule")
            )
    ...
    def _approval_notify_sync(self, approval_data: dict) -> None:
            ...
            try:
                ...
                if outcome == "sent":
                    ...
                    register_timeout_notice(
                        ...
                        approval_data,
                        command=cmd,
                        card_message_id=getattr(
                            fut.result(timeout=0), "message_id", None
                        ),
                    )
                    return
        ...
        try:
            ...
            if fut is not None:
                ...
                register_timeout_notice(
                    self, approval_data, command=cmd, card_message_id=None
                )
        except Exception as e:
    ...
    def _load_turn_history(self, agent, reused_cached_agent):
        ...
        if reused_cached_agent and getattr(agent, "session_id", None) == ctx.session_id:
            ...
            if selected is not agent_history:
                ...
                log = (
                    logger.error
                    if streak >= _TRANSCRIPT_LAG_ESCALATION_TURNS
                    else logger.warning
                )
                log(
    ...
    def _resume_note_interactive(self) -> bool:
        ...
        return bool(
            getattr(
                self._runner._delivery_adapter_for(self._ctx.source),
                "interactive_resume",
                True,
            )
        )
    ...
    def _run_conversation_with_approval(
        ...
        try:
            ...
            with notification_turn(
                agent,
                muted=ctx.mute_notification_reply,
    ...
    def _append_interim_media_tags(self, final_response: str, result, payloads) -> str:
        """Append MEDIA:<path> directives retained from raw interim commentary (#99409).
    ...
    def run_sync(self):
        ...
        try:
            ...
            pending_fallback_notice = getattr(
            ...
            logger.warning(
                "Model resolution failed for session %s: %s", ctx.session_key or "", exc
            )
            ...
            if is_rate_limited_auth_error(exc.__cause__):
                ...
                return {
                    "final_response": _gateway_provider_error_reply(str(exc)),
                    "messages": [],
                    "api_calls": 0,
                    "tools": [],
                }
        ...
        final_response = self._append_interim_media_tags(
            final_response, result, ctx.interim_media_payloads
        )
