    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                _ended_in_db = self._is_session_ended_in_db(entry.session_id)
                ...
                # reached yet (the legacy-import path above persists on
                # import specifically so this check stays accurate).
                _purged_by_prune = not _ended_in_db and self._is_routing_entry_purged(session_key)
                if _ended_in_db or _purged_by_prune:
                    logger.warning(
                        "gateway.session: routing key %r -> %s is %s; "
                        "dropping stale entry and recovering/recreating the "
                        "session (#54878, #59512)",
                        session_key, entry.session_id,
                        "ended in state.db" if _ended_in_db
