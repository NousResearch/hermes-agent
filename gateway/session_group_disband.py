    ...
    with lock:
        ...
        if saved is None:
            ...
            def seal(conn):
                ...
                links = [dict(r) for r in conn.execute('SELECT * FROM hosted_room_links WHERE room_id=? ORDER BY member_id', (room_id,))]
                if service is None:
                ...
                record = dict(version=1, identity=identity, output=output,
                    revision=conn.execute('SELECT next_seq FROM hosted_rooms WHERE room_id=?', (room_id,)).fetchone()[0],
        ...
        for member, link in links.items():
            ...
            def complete_member(conn):
                validate(conn)
                updated = dict(saved, revoked=[*saved['revoked'], member])
        ...
        def finish(conn):
            ...
            if saved and set(saved['revoked']) != set(saved['routes']):
                raise RuntimeStoreError('runtime_coordination_required')
            conn.execute('UPDATE hosted_room_disband_fences SET revocation_complete_at=COALESCE(revocation_complete_at,?) '
