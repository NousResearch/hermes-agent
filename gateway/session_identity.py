...
def resolve_identity(
    ...
    *adapter* is the receiving adapter when the caller holds it; otherwise the source's own
    ...
    # unless the caller supplied the frozen primary home. Resolve from that authority without
    # re-reading a later process-global HERMES_HOME; a named primary lives at
    # <root>/profiles/<name>, so sibling profiles are children of <root>, not of the primary.
    if runtime_name == transport_name:
        ...
        try:
            ...
            validate_profile_name(canonical_runtime)
        except ValueError as exc:
            source.profile_route_rejected = True
            raise IdentityUnresolved(f"{where}: invalid profile {runtime_name!r}") from exc
        primary_path = Path(primary_home)
        root = (
            primary_path.parent.parent
            if primary_path.parent.name == "profiles"
            else primary_path
        )
        runtime_home = (
            root
            if canonical_runtime == "default"
            else root / "profiles" / canonical_runtime
        )
        if canonical_runtime != "default" and not named_profile_is_live(runtime_home):
            source.profile_route_rejected = True
            raise IdentityUnresolved(
                f"{where}: routed profile {canonical_runtime!r} is not served"
            )
    else:
