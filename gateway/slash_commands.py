...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_ooo_command(
        ...
        *,
        ...
        allow_agent_fallthrough: bool = True,
    ) -> str:
        """Handle /ooo <command> [args] through the native Ouroboros router.

        The legacy CLI fast-path and agent skill fallthrough are intentionally
        bypassed: the native router always returns a direct user-visible string.
        ``allow_agent_fallthrough`` is accepted for caller compatibility only.
        """
        del allow_agent_fallthrough  # Native /ooo never falls through to the agent.

        raw = event.get_command_args().strip()
        source = getattr(event, "source", None)

        platform_obj = getattr(source, "platform", None)
        platform_value = getattr(platform_obj, "value", None)
        platform = str(platform_value or platform_obj or "unknown")

        chat_type = str(getattr(source, "chat_type", "") or "")
        chat_id = getattr(source, "chat_id", None)
        channel_id = getattr(source, "parent_chat_id", None) or chat_id
        thread_id = getattr(source, "thread_id", None)
        if not thread_id and chat_type.lower() == "thread":
            thread_id = chat_id
        ...
        raw_idempotency_key = (
            ...
            or getattr(getattr(event, "raw_message", None), "id", None)
        )
        if raw_idempotency_key is not None and str(raw_idempotency_key):
            ...
            idempotency_key = "ooo:" + hashlib.sha256(fingerprint.encode("utf-8")).hexdigest()[:24]

        try:
            from gateway import ouroboros_native as _ooo_native
            from gateway.ouroboros_state import OooStateContext, OooStateStore

            native_ctx = _ooo_native.OooNativeContext(
                cwd=os.getcwd(),
                state_store=OooStateStore(),
                state_context=OooStateContext(
                    platform=platform,
                ...
                allow_mutating_side_effects=False,
            )
            response = await _ooo_native.handle_ooo_native(raw, native_ctx)
            return str(response.text)
        except Exception as exc:  # pragma: no cover - exercised by wrapper tests.
            logger.warning("Native /ooo router failed: %s", type(exc).__name__, exc_info=True)
            return "Ouroboros /ooo 처리 중 오류가 발생했습니다. `/ooo help`로 사용법을 확인한 뒤 다시 시도하세요."
