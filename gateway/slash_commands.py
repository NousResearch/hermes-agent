            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        # Build override WITH context_length so the new model's
                        # context window takes effect immediately on resume.
                        _sw_override = {
                        ...
                        if ctx:
                            _sw_override["context_length"] = str(ctx)
                        
                        _self._session_model_overrides[_session_key] = _sw_override
                        ...
                        # Display context (already computed and stored in override above)
                        _sw_ctx_display = _sw_override.get("context_length")
                        if _sw_ctx_display:
                            lines.append(t("gateway.model.context_label", tokens=f"{int(_sw_ctx_display):,}"))
                        if mi:
                            if mi.max_output:
