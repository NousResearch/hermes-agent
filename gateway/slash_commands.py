...
class GatewaySlashCommandsMixin:
    ...
    async def _carry_gateway_session_workspace(
        ...
        try:
            await self._session_db.update_session_cwd(
                new_session_id,
                cwd,
    ...
    async def _handle_workspace_command(self, event: MessageEvent) -> str:
        ...
        if arg.lower() in {"clear", "reset", "default"}:
            workspace_db = getattr(self, "_session_db", None)
            if workspace_db is None:
                return t("gateway.workspace.save_failed", error="session database unavailable")
            try:
                await workspace_db.clear_session_workspace(session_entry.session_id)
            except Exception as exc:
        ...
        if workspace_db is None:
            return t("gateway.workspace.save_failed", error="session database unavailable")
        try:
            await workspace_db.set_session_workspace(session_entry.session_id, cwd)
        except Exception as exc:
