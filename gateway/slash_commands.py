        ...
        if caller_uid:
            ...
            if caller_is_dm:
                ...
                if caller_keys_on_alt and not (bool(row_chat) and bool(caller_chat)):
                    return False
                # A DM WITH a chat_id is keyed purely on chat_id
                # (build_session_key appends chat_id, never the participant, for
                # DMs that carry one) and thread equality is already proven by
                # origin_ok above. A p2p DM is 1:1, so an equal non-blank chat_id
                # proves the same owner even when the persisted user_id is a
                # stale snapshot from a DIFFERENT id namespace — e.g. Feishu
                # flips source.user_id from the app-scoped open_id to the
                # tenant-scoped user_id once the contacts scope is granted, and
                # reset_session persists the pre-grant open_id, so row_uid never
                # equals the runtime caller_uid (#89123). This mirrors the
                # live-origin branch (_same_origin_chat), which already treats an
                # equal non-empty DM chat_id as sufficient; the persisted
                # fallback must stay in lock-step with it and with the key.
                if bool(row_chat) and bool(caller_chat) and row_chat == caller_chat:
                    return True
                # No chat_id on one/both sides: the DM is keyed on the
                # participant (user_id_alt or user_id); require the same owner
                # AND the same (possibly-blank) chat_id, exactly as before.
                return (
