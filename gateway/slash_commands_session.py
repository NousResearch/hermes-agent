...
class GatewaySessionCommandsMixin:
    ...
    async def _list_titled_sessions(self, source, session_key: str, allow_all: bool) -> list[dict]:
        ...
        ``titled_only`` filters in SQL. Dropping untitled rows AFTER a ``LIMIT 10`` fetch made a
        lane whose ten most recent sessions happen to be unnamed report "No named sessions found"
        while ``/sessions`` listed those same names and ``/resume <title>`` still resolved them."""
