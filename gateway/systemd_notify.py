"""Minimal systemd ``sd_notify`` support for the gateway watchdog.

The notifier is intentionally optional and stdlib-only.  A missing or malformed
systemd environment disables it without changing normal gateway behavior.
"""
...
def notify(message: str) -> bool:
    """Send one ``sd_notify`` datagram, returning whether it was sent.

    ``NOTIFY_SOCKET`` may name either a filesystem Unix datagram socket or an
    abstract Linux socket (written by systemd as ``@name``).  Errors are
    deliberately swallowed: the watchdog must never turn an optional service
    integration into a gateway startup failure.
    """
...
def watchdog_interval_seconds() -> Optional[float]:
    ...
    ``WATCHDOG_USEC`` is supplied by systemd only when ``WatchdogSec`` is
    enabled for the unit.  Invalid, zero, negative, non-finite, or missing
    values disable the application watchdog.
    """
...
class SystemdWatchdog:
    ...
    def _lag_tolerance(self) -> float:
        ...
        try:
            ...
            return max(0.1, interval * 0.25)
        return max(0.0, value) if math.isfinite(value) else max(0.1, interval * 0.25)

    def start(self) -> bool:
        """Start the loop-progress sampler if systemd watchdog is enabled."""
        if not self.enabled:
            return False
    ...
    def record_tick(self, *, scheduled_at: float, now: float) -> bool:
        """Record one event-loop opportunity and feed systemd when on time.

        The method is separate from the task loop so deterministic tests can
        prove the lag latch without blocking a real event loop.
        """
    ...
    async def _run(self) -> None:
        ...
        if interval is None:
            return
        # Feed at half the systemd deadline.  A late wake-up is compared with
        # the expected wake-up time rather than wall-clock time, so clock
        # adjustments cannot make a healthy loop appear stalled.
    ...
    async def stop(self) -> None:
        """Stop feeding systemd and optionally send ``STOPPING=1``."""
        self._stopping = True
        task = self._task
