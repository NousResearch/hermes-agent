"""Telegram DM topic-mode helpers for ``GatewayRunner``.

Extracted from ``gateway/run.py`` as part of the god-file decomposition
campaign (Phase 4, Cluster B-1). This mixin holds the core topic-mode
detection, lobby/lane classification, binding persistence, recovery, and
session-key normalization logic.

The methods are lifted verbatim (modulo ``Platform`` import moved to
top-level so they reach ``Platform.TELEGRAM`` from this module).  Behavior
is unchanged.
"""
...
class GatewayTelegramTopicMixin:
    """Core Telegram DM topic-mode helpers lifted from ``GatewayRunner``."""
