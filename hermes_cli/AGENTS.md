...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  ...
  version/active features, Windows pause token including stopped manual serves) and re-executes
  ...
  restart, verification, receipt); the parent relays the exit code. Fork upstream sync (which
  can swap the tree again) finishes before this handoff. Recovery ownership transfers only
  after child spawn succeeds; manual serves resume after dependency/build/config work, with
  an explicit failure cleanup before the child's hard exit. Every "purge `sys.modules`" /
