...
def _save_codex_tokens(
    ...
    *,
    ...
    """Save Codex OAuth tokens to the selected Hermes auth store."""
...
def _recover_codex_tokens_from_cli(
    ...
    *,
    ...
    """Adopt a valid Codex CLI token pair into the selected Hermes auth store."""
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if should_refresh:
        # Lock the active profile store and the actual provider-state source.
        # Named profiles commonly resolve Codex from the global root; using
        # only the profile lock lets two profiles spend the same single-use
        ...
        ) as (_auth_store, _state, state_source_path):
