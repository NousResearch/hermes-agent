...
CODEX_OAUTH_USER_AGENT = f"ht-cli/{_HERMES_CLI_VERSION}"
...
def _auth_file_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        # Guard both the new HT-branded and legacy default data dirs (Phase 6).
        real_home_auths = {
            (Path.home() / ".ht-ai-agent" / "auth.json").resolve(strict=False),
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize HT AI Agent:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"HT AI Agent is waiting for the OAuth callback on {redirect_uri}")
