...
DEFAULT_OMP_ACP_BASE_URL = "acp://omp"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "omp-acp": ProviderConfig(
        id="omp-acp",
        name="Oh My Pi ACP",
        auth_type="external_process",
        inference_base_url=DEFAULT_OMP_ACP_BASE_URL,
        base_url_env_var="OMP_ACP_BASE_URL",
    ),
...
def resolve_provider(
    ...
    *,
    ...
    _PROVIDER_ALIASES = {
        ...
        "omp": "omp-acp", "oh-my-pi": "omp-acp", "omp-acp-agent": "omp-acp", "pi": "omp-acp",
...
# Per-provider defaults for external-process (subprocess-backed ACP) providers.
# Keyed by provider_id; each entry supplies the env vars and fallback command
# used to resolve and launch the backing CLI.
_EXTERNAL_PROCESS_PROVIDER_DEFAULTS: Dict[str, Dict[str, Any]] = {
    "copilot-acp": {
        "command_env": "HERMES_COPILOT_ACP_COMMAND",
        "legacy_command_env": "COPILOT_CLI_PATH",
        "default_command": "copilot",
        "args_env": "HERMES_COPILOT_ACP_ARGS",
        "default_args": ["--acp", "--stdio"],
        "cli_display_name": "Copilot CLI",
        "missing_cli_hint": "Install GitHub Copilot CLI or set HERMES_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
    },
    "omp-acp": {
        "command_env": "HERMES_OMP_ACP_COMMAND",
        "legacy_command_env": "OMP_CLI_PATH",
        "default_command": "omp",
        "args_env": "HERMES_OMP_ACP_ARGS",
        "default_args": ["acp"],
        "cli_display_name": "Oh My Pi (omp)",
        "missing_cli_hint": "Install Oh My Pi (curl -fsSL https://omp.sh/install | sh) or set HERMES_OMP_ACP_COMMAND/OMP_CLI_PATH.",
    },
}
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    defaults = _EXTERNAL_PROCESS_PROVIDER_DEFAULTS.get(provider_id, {})
    command = (
        os.getenv(defaults.get("command_env", ""), "").strip()
        or os.getenv(defaults.get("legacy_command_env", ""), "").strip()
        or defaults.get("default_command", "")
    )
    raw_args = os.getenv(defaults.get("args_env", ""), "").strip()
    args = shlex.split(raw_args) if raw_args else list(defaults.get("default_args", []))
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            f"Could not find the {defaults.get('cli_display_name', provider_id)} command '{command}'. "
            f"{defaults.get('missing_cli_hint', '')}",
            provider=provider_id,
            code="missing_cli",
        )
