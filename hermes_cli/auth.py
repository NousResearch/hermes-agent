"""
Multi-provider authentication system for IX Agency.
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in IX Agency.
    ...
    # the user initiated inside IX Agency (manual add / device-code / PKCE), plus
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    pointing IX Agency at a non-production Nous Portal (e.g. a hosted agent
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize IX Agency:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"IX Agency is waiting for the OAuth callback on {redirect_uri}")
...
#
# IX Agency maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from IX Agency auth store (~/.hermes/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to IX Agency auth store (~/.hermes/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into IX Agency auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating IX Agency auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to IX Agency auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. IX Agency keeps its OWN
        ...
        # the Codex CLI (or another IX Agency process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from IX Agency's own Codex token store.
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other IX Agency processes
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    # written by an older IX Agency (or hand-edited) may carry a non-xAI
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's IX Agency root, refuse rather than silently corrupt cross-profile
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — IX Agency
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with IX Agency's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, IX Agency's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another IX Agency install sharing "
            "~/.hermes/auth.json) called POST /api/oauth/token with IX Agency's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only IX Agency may call the "
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip IX Agency models — they're not reliable for agentic tool-calling
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "IX Agency is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("IX Agency is not logged into Nous Portal.",
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on IX Agency' "
...
def _login_openai_codex(
    ...
    *,
    ...
    # Check for existing IX Agency-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in IX Agency auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("IX Agency will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("IX Agency will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — IX Agency gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(IX Agency creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to IX Agency auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in IX Agency auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(IX Agency creates its own local OAuth session)")
    print()
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to IX Agency auth store (~/.hermes/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting IX Agency login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting IX Agency login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older IX Agency builds still see
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("IX Agency will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `hermes model` or configure an API key to use IX Agency.")
        else:
