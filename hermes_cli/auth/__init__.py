...
# hermes_cli/auth/codex.py (F-C2 step 3). Re-exported via the bottom-of-file
# ``from .codex import *`` so legacy imports such as
# ``from hermes_cli.auth import _login_openai_codex`` keep working. Tests
# that patch the login helper must now target
# ``hermes_cli.auth.codex._codex_device_code_login``.
