...
def _write_through_codex_tokens_to_global_root(
    ...
    Codex refresh tokens are single-use with rotation-family reuse detection: a profile that
    resolved the grant from the root fallback and refreshed it must write the rotated chain
    back to root — including the ``credential_pool.openai-codex`` entries the runtime actually
    ...
    # HERMES_HOME points at a profile path (mirrors the guard in _write_through_xai_oauth_to_
    # global_root). Uses the unmodified HOME env, not Path.home() which fixtures may
    # monkeypatch — and falls back to USERPROFILE because HOME is often unset on Windows,
    # which would otherwise silently disable the guard.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_env = os.environ.get("HOME", "") or os.environ.get("USERPROFILE", "")
        if real_home_env:
    ...
    try:
        with _auth_store_lock(target_path=global_path):
            ...
            # Root's own previous singleton tokens drive the pool-sync alias check (#39236):
            # legacy singleton-aliases in the ROOT store must be refreshed alongside the chain.
            ...
            _store_provider_state(root_store, "openai-codex", dict(state), set_active=False)
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    from hermes_cli.auth import (
        ...
        _load_provider_state_with_source, _same_path, _save_auth_store,
        _save_provider_state, _utc_now_z, _write_through_codex_tokens_to_global_root)
