...
def _normalize_provider(provider: str) -> str:
    ...
    if custom_key:
        return custom_key
    if normalized and normalized not in {"auto", "openrouter"}:
        try:
            resolved = auth_mod.resolve_provider(normalized)
        except Exception:
            pass
        else:
            if resolved:
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "nous", "openai-codex", "qwen-oauth", "minimax-oauth"} else AUTH_TYPE_API_KEY
