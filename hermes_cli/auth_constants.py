...
# ``source`` prefix stamped on a secret resolved from the credential pool rather than an env var.
# Callers key routing decisions off it, so it must not drift from the f-strings that build it.
