...
# days / 426543 files" symptom users hit. The dependency/test-env names mirror
# ``agent.skill_utils.EXCLUDED_SKILL_DIRS`` (the project's canonical
# "regeneratable dir" set). We deliberately do NOT exclude ``.archive`` here
# because the curator's ``skills/.archive/`` holds restorable user skills that
# must survive a backup.
_EXCLUDED_DIRS = {
