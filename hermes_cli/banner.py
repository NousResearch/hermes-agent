...
def check_for_updates() -> Optional[int]:
    ...
    # update-check mode (incl. stable-tags pattern/remote) changed since the
    # last check.
    now = time.time()
    try:
...
def _format_update_notice(behind: int) -> str:
    ...
    When the last update check ran in stable-tags mode, render a stable
    release notice (using the tag metadata captured in the update context)
    instead of the branch commit-behind wording; the stable path returns
    ``UPDATE_AVAILABLE_NO_COUNT`` so it lands in the same non-zero branch.
    """
