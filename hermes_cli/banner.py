...
def check_for_updates() -> Optional[int]:
    ...
    # Resolve repo_dir + local_head up front so the cache key can invalidate
    # when the local checkout's HEAD moves (e.g. after a successful
    # `git pull`/rebase). Without this guard the cache survives for up to 6h
    # and prints a stale "behind" count even though the local checkout is
    # now caught up.
    ...
    # local HEAD has changed since the last check. The version guard matters
    ...
    elif repo_dir is not None and (repo_dir / ".git").exists():
