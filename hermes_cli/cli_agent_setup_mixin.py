...
class CLIAgentSetupMixin:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        try:
            ...
            # A CLI-created ID is supplied to AIAgent, not generated by it.
            # Only its captured creation receipt can identify that case as fresh.
            identity = getattr(self, "_plugin_session_identity", None)
            if isinstance(identity, dict) and identity.get("session_id") == self.session_id:
                self.agent._plugin_session_identity = dict(identity)
                if self._resumed or self.conversation_history:
                    self.agent._plugin_session_identity["session_origin"] = "resume"
