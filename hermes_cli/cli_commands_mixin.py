...
class CLICommandsMixin:
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for IX Agency config/state.
            ...
            try:
                ...
                if response:
                    try:
                        ...
                        label = _skin.get_branding("response_label", "⚕ IX Agency")
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a IX Agency-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"IX Agency keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while IX Agency is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while IX Agency is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while IX Agency is busy."
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update IX Agency to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update IX Agency')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update IX Agency"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update IX Agency",
