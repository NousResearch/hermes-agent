...
Extracted from cli.py to keep the main module focused on the interactive
REPL while config I/O lives in its own module.
"""
...
def load_cli_config() -> Dict[str, Any]:
    ...
    """
    # Resolve hermes_home lazily so tests that monkeypatch cli._hermes_home
    # work correctly (the original load_cli_config lived in cli.py and used
    # cli._hermes_home directly).  Use sys.modules to avoid circular import
    # during cli.py module load (when CLI_CONFIG = load_cli_config() fires).
    import sys as _sys
    _cli_mod = _sys.modules.get("cli")
    if _cli_mod is not None and hasattr(_cli_mod, "_hermes_home"):
        _hermes_home = _cli_mod._hermes_home
    else:
        _hermes_home = get_hermes_home()
    ...
    if config_path.exists():
        try:
            ...
            # First: expand ${ENV_VAR} references in the file config.
