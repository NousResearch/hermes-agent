...
class CLIInitMixin:
    ...
    def _init_toolsets(self, toolsets):
        ...
        if toolsets and "all" not in toolsets and "*" not in toolsets:
            # MCP server names only resolve after discover_mcp_tools runs; ask the
            # same resolver that assembled the configured toolset list so portable
            # plugin MCP servers are not misreported as typos (#119457).
            try:
                from hermes_cli.tools_config import enabled_mcp_server_names
                mcp_names = set(enabled_mcp_server_names(CLI_CONFIG))
            except Exception:
