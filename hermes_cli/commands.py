...
# Telegram bot shortcuts for McpVex slash recipes (key → (input, description)).
MCPVEX_TELEGRAM_SHORTCUTS: dict[str, tuple[str, str]] = {
    "mcpvex_rollout": ("rollout status", "McpVex WA rollout status"),
    "mcpvex_rollout_sync": ("rollout sync", "McpVex rollout state sync"),
}
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef(
        "thinking",
        "Toggle live thinking bubbles and style",
        "Configuration",
        args_hint="[on|off|status|blockquote|code|plain|collapse|persist]",
        subcommands=(
            "on",
            "off",
            "status",
    ...
    CommandDef(
        "dream",
        "Run an offline memory/dream consolidation pass",
        "Tools & Skills",
        args_hint="[now|status]",
        subcommands=("now", "status"),
    ),
    CommandDef(
        "focus",
        "Pin, show, or clear persistent ECN task focus for this session",
        "Session",
        args_hint="[text | show | clear | status]",
        subcommands=("show", "clear", "status"),
    ),
    CommandDef(
        "suggest-skills",
        "Suggest skills from high-confidence experience lessons",
        "Tools & Skills",
        aliases=("skill_suggestions",),
        args_hint="[min-confidence]",
    ),
    CommandDef(
        "plan",
        "Herens Plan-Execute: set, view, or advance session plan",
        "Tools & Skills",
        args_hint="[set|get|phase <phase>|validate]",
        subcommands=("set", "get", "phase", "validate"),
    ),
    CommandDef(
        "herens",
        "Herens autonomous OS: status, strategy, nudge, caduceus, handoffs",
        "Tools & Skills",
        aliases=("hn",),
        args_hint="[status|strategy|nudge|caduceus|handoff|evolve|metrics|tools|errors|cot|tot|uncertainty|transfer|inbox|optimize|peers|enable] ...",
        subcommands=(
            "status",
            "strategy",
            "nudge",
    ...
    CommandDef(
        "rate",
        "Rate the last response 1-10 to train Herens feedback loop",
        "Tools & Skills",
        args_hint="<1-10>",
    ),
    CommandDef(
    ...
    CommandDef(
        "swarm",
        "Coordinated N-agent swarm (start, status, read, kill, repair, doctor, list)",
        "Tools & Skills",
        args_hint="[<N> goal | start|execute|resume|status|read|kill|repair|doctor|list ...]",
        subcommands=(
            "start",
            "execute",
            "resume",
    ...
    CommandDef(
        "pwnbox",
        "PWNBOX CTF hunt on instance URL (accept or agent)",
        "Tools & Skills",
        aliases=("pb",),
        args_hint="[accept|agent|status|metrics] <url>",
        subcommands=(
            "accept",
            "agent",
            "orchestrate",
    ...
    CommandDef(
        "pentest",
        "Strix-style pentest scans (run, resume, watch, list, status)",
        "Tools & Skills",
        aliases=("pt",),
        args_hint="[run|resume|watch|list|status|export|doctor|stop|validate-scope] ...",
        subcommands=(
            "run",
            "resume",
            "watch",
    ...
    CommandDef(
        "hunt",
        "Hunt Brain — engagement, recon, pentest stack, workflow",
        "Tools & Skills",
        aliases=("hb",),
        args_hint="[init|stack|workflow|intake|status|doctor|...] [url]",
        subcommands=(
            "init",
            "scope",
            "start",
    ...
    CommandDef(
        "ctx",
        "Universal local GGUF context stack (status, doctor, bench, wire)",
        "Tools & Skills",
        args_hint="[status|doctor|bench|wire] [model_id]",
        subcommands=("status", "doctor", "bench", "wire", "help"),
    ),
    CommandDef(
        "up",
        "One-command hunt box (compose + bootstrap)",
        "Tools & Skills",
        args_hint="[--apply]",
        cli_only=False,
    ),
    CommandDef(
        "mcpvex",
        "McpVex security framework (doctor, status, scope, wiring, lab, ctf, bb)",
        "Tools & Skills",
        aliases=("mv",),
        args_hint="[doctor|status|scope|wiring|lab|assess|ctf|bb|ops|ready|failures|scorecard ...]",
        subcommands=(
            "doctor",
            "status",
            "scope",
    ...
    CommandDef(
        "sandbox",
        "Isolated Docker/Kata/gVisor Kali sandbox (doctor, spawn, exec, ctf)",
        "Tools & Skills",
        aliases=("sbx",),
        args_hint="[doctor|spawn|exec|teardown|put|get|download|ctf|poc ...]",
        subcommands=(
            "doctor",
            "spawn",
            "exec",
...
# Canonical commands that must survive Slack's 50-slash cap. New gateway commands
# (e.g. hunt) append to COMMAND_REGISTRY and can otherwise clamp these off the
# manifest even though Telegram still surfaces them — breaking parity tests.
_SLACK_PRIORITY_CANONICALS = (
    "help",
    "restart",
    "new",
    "stop",
...
#   - hunt: operator-heavy hunt stack; reached via /hermes hunt on Slack.
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _cmd_by_name = {cmd.name: cmd for cmd in COMMAND_REGISTRY}
...
class SlashCommandAutoSuggest(AutoSuggest):
    ...
    def get_suggestion(self, buffer, document):
        ...
        if len(parts) == 1 and not text.endswith(" "):
            ...
            if candidates:
                # Prefer the shortest match so /he completes to /help before
                # longer siblings like /herens; alphabetical order breaks ties.
                candidates.sort(key=lambda x: (len(x), x))
                best = candidates[0]
                return Suggestion(best[len(word):])
            return None
