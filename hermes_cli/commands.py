...
COMMAND_REGISTRY: list[CommandDef] = [
    # 会话
    CommandDef("new", "开始新会话（全新会话ID + 历史）", "会话",
               aliases=("reset",)),
    ...
    CommandDef("save", "保存当前对话", "会话",
               cli_only=True),
    CommandDef("retry", "重试上一条消息（重新发送给智能体）", "会话"),
    CommandDef("undo", "撤销上一轮用户/助手对话", "会话"),
    ...
    CommandDef("btw", "临时侧问（使用会话上下文，不持久化）", "会话",
               args_hint="<question>"),
    CommandDef("agents", "显示活跃的智能体和运行中的任务", "会话",
               aliases=("tasks",)),
    CommandDef("queue", "将提示词加入队列（不打断当前任务）", "会话",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "在下次工具调用后注入消息（不打断流程）", "会话",
               args_hint="<prompt>"),
    CommandDef("status", "显示会话信息", "会话"),
    CommandDef("profile", "显示当前配置文件名和主目录", "信息"),
    CommandDef("sethome", "设置此聊天为主频道", "会话",
    ...
    CommandDef("verbose", "循环切换工具进度显示：关闭 → 新 → 全部 → 详细",
               "配置", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("yolo", "切换 YOLO 模式（跳过所有危险命令确认）",
               "配置"),
    ...
    CommandDef("reload-mcp", "从配置重新加载 MCP 服务器", "工具与技能",
               aliases=("reload_mcp",)),
    CommandDef("browser", "通过 CDP 将浏览器工具连接到实时 Chrome", "工具与技能",
    ...
    CommandDef("restart", "排空活跃任务后优雅重启网关", "会话",
               gateway_only=True),
    CommandDef("usage", "显示当前会话的 token 使用量和速率限制", "信息"),
    CommandDef("insights", "显示使用洞察和分析", "信息",
               args_hint="[days]"),
    CommandDef("platforms", "显示网关/消息平台状态", "信息",
               cli_only=True, aliases=("gateway",)),
    CommandDef("copy", "将上一条助手回复复制到剪贴板", "信息",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "从剪贴板附加图片", "信息",
               cli_only=True),
    CommandDef("image", "附加本地图片文件用于下次提示词", "信息",
