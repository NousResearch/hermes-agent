"""Slash command definitions and autocomplete for the IX Agency CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of IX Agency config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal IX Agency works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while IX Agency is working", "Configuration",
    ...
    CommandDef("update", "Update IX Agency to the latest version", "Info"),
    CommandDef("version", "Show IX Agency version", "Info", aliases=("v",)),
...
# Telegram allows up to 100 BotCommands. IX Agency ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full IX Agency
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to IX Agency or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
