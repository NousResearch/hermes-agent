...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_HOMESERVER_URL": {
        "description": "Matrix homeserver URL (e.g., https://matrix.org or https://matrix.example.org:8448)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID for the bot account (e.g., @hermes:matrix.org)",
    ...
    "MATRIX_HOME_CHANNEL": {
        "description": "Default Matrix room ID for cron job delivery (e.g., !roomid:matrix.org)",
        "prompt": "Matrix Home Channel room ID",
        "url": None,
        "password": False,
        "category": "messaging",
    },
    ...
    "MATRIX_ALLOW_ALL_USERS": {
        "description": "Allow all Matrix users to interact with the bot (true/false). Default: false.",
        "prompt": "Allow all Matrix users (true/false)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Device ID returned by the Matrix login API. Strongly recommended — prevents Synapse from creating a new device session on every restart and replaying old messages.",
        "prompt": "Matrix Device ID (from login response)",
        "url": None,
        "password": False,
        "category": "messaging",
    },
    "MATRIX_E2EE": {
        "description": "Enable Matrix end-to-end encryption (true/false). Default: false. Requires the libolm system package and 'pip install mautrix[e2be] base58'.",
        "prompt": "Enable Matrix E2EE (true/false)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "MATRIX_VERIFY_SSL": {
        "description": "Verify SSL certificates for Matrix homeserver (true/false). Default: true. Set to false for self-signed certs.",
        "prompt": "Verify SSL (true/false)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "MATRIX_PASSWORD": {
        "description": "Bot account password. Used once to bootstrap cross-signing keys so clients don't show 'device not verified' warnings. Optional on homeservers that don't require password auth for cross-signing upload (e.g. some Synapse configurations).",
        "prompt": "Matrix bot account password (optional, for cross-signing bootstrap)",
        "url": None,
        "password": True,
        "category": "messaging",
        "advanced": True,
    ...
    "MATRIX_RECOVERY_KEY": {
        "description": "Matrix E2EE recovery key (e.g. 'EsAB cdef ...'). Printed by the gateway on first E2EE bootstrap. Used on every restart to load cross-signing private keys from SSSS and self-sign the bot's device, removing the 'not verified by owner' warning.",
        "prompt": "Matrix E2EE recovery key (printed on first gateway start with E2EE)",
        "url": None,
        "password": True,
        "category": "messaging",
        "advanced": True,
