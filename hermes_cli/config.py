...
DEFAULT_CONFIG = {
    ...
    "kanban": {
        ...
        "failure_limit": 2,
        # How many times a worker that EXHAUSTS its per-turn iteration budget
        # gets a clean fresh-budget requeue before the exhaustion starts
        # counting toward ``failure_limit``. A ship-loop (implement → open PR →
        # resolve review threads) routinely needs one more budget to finish; a
        # bounded resume gives that headroom without an infinite loop (after the
        # cap, exhaustion is a counted failure and the breaker still trips).
        # Set 0 to restore the pre-resume behavior (exhaustion always counts).
        "budget_resume_limit": 1,
