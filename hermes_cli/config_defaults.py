...
DEFAULT_CONFIG = {
    ...
    # read/write/search/patch. A profile listed here is refused access to
    # anything outside its roots (symlink escapes resolved via realpath on
    # local backends). Profiles NOT listed here — including the default
    # profile — are unrestricted, so an empty map (the default) changes no
    # behavior. Useful for confining a client-facing profile away from the
