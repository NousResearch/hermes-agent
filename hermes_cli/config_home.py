...
def initialize_home(
    home: Path, subdirs: tuple[str, ...], ensured: dict[str, tuple[int, int, int]],
) -> None:
    ...
    try:
        _ensure_directory(home, create=not managed and not named_profile, secure=not managed)
        ...
        for subdir in required:
            ...
            _ensure_directory(
                home / subdir, create=not managed, secure=not managed, parents=not named_profile,
            )
        if managed:
            if named_profile and named_profile_home_is_unavailable(home):
                raise FileNotFoundError(f"Named profile home is missing or being deleted: {home}")
            _ensure_directory(
                home / "logs" / "curator", create=True, secure=False, parents=not named_profile,
            )
        try:
    ...
    for alias in (home, home.resolve()):
        identity = _hermes_home_identity(
            alias, include_ctime=profile_deletion_marker_path(alias) is not None,
        )
