...
def _cmd_promote(args) -> int:
    """List, approve, or reject pending CodeAct skill promotion candidates."""
    from agent import codeact_promotion as promo

    action = getattr(args, "promote_action", "list")

    if action == "list":
        pending = promo.load_pending()
        if not pending:
            print("curator: no pending promotion candidates")
            print(
                "(candidates are flagged when a CodeAct session defines the same "
                "helper function ≥3 times with auto_promote disabled)"
            )
            return 0
        print(f"pending promotion candidates ({len(pending)}):\n")
        for i, entry in enumerate(pending, 1):
            name = entry.get("fn_name", "?")
            desc = entry.get("description", "")
            count = entry.get("occurrence_count", "?")
            sessions = entry.get("seen_in_sessions", [])
            tags = entry.get("tags", [])
            print(f"  {i}. {name}")
            if desc:
            ...
            if sessions:
                print(f"     sessions:   {', '.join(sessions[:3])}")
            if tags:
    ...
    if action == "approve":
        fn_name = getattr(args, "fn_name", None)
        if not fn_name:
            print("curator: --name is required for 'approve'")
            return 1
        pending = promo.load_pending()
        entry = next((p for p in pending if p.get("fn_name") == fn_name), None)
        if entry is None:
            print(f"curator: no pending candidate named '{fn_name}'")
            return 1
        candidate = promo.PromotionCandidate.from_dict(entry)
        try:
            skill_path = promo.write_promoted_skill(candidate)
            promo.remove_pending(fn_name)
            print(f"curator: promoted '{fn_name}' → {skill_path}")
            return 0
        except Exception as exc:
            print(f"curator: failed to promote '{fn_name}': {exc}")
            return 1

    if action == "reject":
        fn_name = getattr(args, "fn_name", None)
        if not fn_name:
            print("curator: --name is required for 'reject'")
            return 1
        found = promo.remove_pending(fn_name)
        if found:
            print(f"curator: rejected and removed '{fn_name}' from pending list")
            return 0
        print(f"curator: no pending candidate named '{fn_name}'")
        return 1

    if action == "sessions":
        from agent import curator as _curator_mod

        candidates = _curator_mod.find_codeact_promotion_candidates_from_sessions()
        if not candidates:
            print("curator: no promotion candidates found across recent sessions")
            print("(run CodeAct sessions that define helper functions, then re-check)")
            return 0
        print(
            f"found {len(candidates)} candidate function(s) across recent sessions:\n"
        )
        for c in candidates:
            fn_name = c.get("fn_name", "?")
            desc = c.get("description", "")
            count = c.get("occurrence_count", "?")
            sessions = c.get("seen_in_sessions", [])
            print(f"  - {fn_name} (x{count}) — {desc[:80]}")
            if sessions:
                print(f"    sessions: {', '.join(s[:16] for s in sessions[:3])}")
        return 0

    print(f"curator: unknown promote action '{action}'")
    return 1
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_promote = subs.add_parser(
        "promote",
        help="List, approve, or reject CodeAct skill promotion candidates",
    )
    p_promote.add_argument(
        "promote_action",
        nargs="?",
        default="list",
        choices=("list", "approve", "reject", "sessions"),
        help="Action: list (default), approve, reject, sessions (scan recent sessions)",
    )
    p_promote.add_argument(
        "--name",
        dest="fn_name",
        default=None,
        help="Function name (required for approve/reject)",
    )
