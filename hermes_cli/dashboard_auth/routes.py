...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    if getattr(p, "supports_password", False):
        # Password providers do not have an OAuth/PKCE start_login() redirect.
        # Send direct hits/bookmarks for /auth/login?provider=basic back to
        # the server-rendered login form instead of raising a 500.
