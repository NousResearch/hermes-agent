...
Diagnoses issues with IX Agency setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 IX Agency Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses IX Agency' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give IX Agency a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# IX Agency Persona\n\n"
                "<!-- Edit this file to customize how IX Agency communicates. -->\n\n"
                "You are IX Agency, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a IX Agency one.
                        check_info(
