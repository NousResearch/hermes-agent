...
def _maybe_skip_auth_provider(label: str, *aliases: str) -> bool:
    """Skip an auth-provider row the operator declared optional.

    Honors ``doctor.ignore_auth_providers`` for *every* auth row (Nous,
    Codex, MiniMax, xAI, ...) rather than special-casing individual
    providers. When the row is ignored we print a single info line and
    return True so the caller can skip its own warning/summary handling.
    """
    if _is_doctor_auth_ignored(label, *aliases):
        check_info(f"{label} skipped (disabled in doctor.ignore_auth_providers)")
        return True
    return False
