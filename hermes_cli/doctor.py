...
def run_doctor(args):
    ...
    isolate_requested = bool(getattr(args, 'isolate', False))
    json_requested = bool(getattr(args, 'json', False))
    if isolate_requested:
        from hermes_cli.doctor_isolate import render_isolation_report, run_isolation_diagnostic
        if json_requested:
            ...
            report = run_isolation_diagnostic()
            render_isolation_report(report)
        return
    if json_requested:
        if not runtime_requested:
            print(json.dumps({
                "schema_version": 1,
                "status": "error",
                "error_class": "DiagnosticFlagRequired",
            }, sort_keys=True))
            return
