...
def _profile_suffix(hermes_home: str | None = None) -> str:
...
def get_service_name(hermes_home: str | None = None) -> str:
...
def get_systemd_unit_path(system: bool = False, hermes_home: str | None = None) -> Path:
    name = get_service_name(hermes_home)
    if system:
...
def systemd_unit_is_current(system: bool = False, hermes_home: str | None = None) -> bool:
    unit_path = get_systemd_unit_path(system=system, hermes_home=hermes_home)
    if not unit_path.exists():
        return False
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False, hermes_home: str | None = None):
    system = _select_systemd_scope(system)
    svc = get_service_name(hermes_home)
    unit_path = get_systemd_unit_path(system=system, hermes_home=hermes_home)
    scope_flag = " --system" if system else ""
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            status_home: str | None = None
            user_unit = get_systemd_unit_path(system=False)
            system_unit = get_systemd_unit_path(system=True)
            if not user_unit.exists() and not system_unit.exists():
                # When running inside a profile session, HERMES_HOME points at
                # ~/.hermes/profiles/<name>, but the default profile installs
                # the base unit name (hermes-gateway.service) with no suffix.
                default_home = str(get_default_hermes_root())
                default_user_unit = get_systemd_unit_path(system=False, hermes_home=default_home)
                default_system_unit = get_systemd_unit_path(system=True, hermes_home=default_home)
                if default_user_unit.exists() or default_system_unit.exists():
                    status_home = default_home

            if user_unit.exists() or system_unit.exists() or status_home is not None:
                systemd_status(deep, system=system, full=full, hermes_home=status_home)
