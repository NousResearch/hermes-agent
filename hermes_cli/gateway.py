...
def _gateway_issue(code: str, message: str, severity: str = "warning") -> dict[str, str]:
    return {
        "code": code,
        "message": message,
        "severity": severity,
...
def build_gateway_troubleshoot_report(platform: str | None = None) -> dict[str, object]:
    """Build a sanitized gateway troubleshooting report.

    The report intentionally excludes token, API key, channel, and user values.
    It only surfaces setup state, persisted runtime health, and suggested next
    commands.
    """
    ...
    runtime_platforms_raw = runtime.get("platforms", {}) or {}
    runtime_platforms = {
        str(name).strip().lower(): pdata or {}
        for name, pdata in runtime_platforms_raw.items()
        if str(name).strip()
    }

    config_names = {plat.value for plat in config.platforms}
    runtime_names = set(runtime_platforms)
    platform_names = sorted(config_names | runtime_names)
    if requested_platform:
        platform_names = [name for name in platform_names if name == requested_platform]

    report_platforms: list[dict[str, object]] = []
    report_issues: list[dict[str, str]] = []
    report_next_steps: list[str] = []

    if requested_platform and not platform_names:
        issue = _gateway_issue(
            "unknown_platform",
            f"No gateway config or runtime record was found for '{requested_platform}'.",
        )
        report_issues.append(issue)
        report_next_steps.append("hermes gateway setup")

    for name in platform_names:
        platform_obj = next((plat for plat in config.platforms if plat.value == name), None)
        platform_config = config.platforms.get(platform_obj) if platform_obj else None
        runtime_config = runtime_platforms.get(name, {}) or {}

        enabled = bool(platform_config.enabled) if platform_config else False
        configured = (
            bool(config._is_platform_connected(platform_obj, platform_config))
            if platform_obj and platform_config
            else False
        )
        runtime_state = runtime_config.get("state") or "unknown"
        error_code = runtime_config.get("error_code")
        error_message = runtime_config.get("error_message")
        ...
        if platform_config is None:
            issues.append(
                _gateway_issue(
                    "runtime_without_config",
                    "Runtime health exists for this platform, but no platform config was found.",
                )
            )
            next_steps.append("hermes gateway setup")
        elif not enabled:
            issues.append(
                _gateway_issue(
                    "platform_disabled",
                    "Platform is present but disabled in gateway config.",
                    severity="info",
                )
            )
            next_steps.append("hermes gateway setup")
            ...
            issues.append(
                _gateway_issue(
                    "missing_config",
                    "Platform is enabled but required credentials or connection settings are missing.",
                )
            )
            next_steps.append("hermes gateway setup")

        if runtime_state == "fatal":
            message = str(error_message or error_code or "Platform reported a fatal runtime error.")
            issues.append(_gateway_issue("runtime_fatal", message, severity="error"))
            next_steps.append("hermes logs gateway --level warning")
            next_steps.append("hermes gateway restart")
        elif runtime_state in {"error", "failed"}:
            message = str(error_message or error_code or "Platform reported a runtime error.")
            issues.append(_gateway_issue("runtime_error", message))
            next_steps.append("hermes logs gateway --level warning")

        if enabled and configured and runtime_state in {"unknown", "starting"}:
            next_steps.append("hermes gateway status --deep")
        ...
        report_next_steps.extend(next_steps)

    if not requested_platform and not any(p.get("enabled") for p in report_platforms):
        issue = _gateway_issue(
            "no_enabled_platforms",
            "No enabled gateway platforms were found.",
            severity="info",
        )
        report_issues.append(issue)
        report_next_steps.append("hermes gateway setup")
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        report_issues.append(issue)
        report_next_steps.append("hermes logs gateway --level warning")
        report_next_steps.append("hermes gateway status --deep")
    ...
    return {
        ...
        "next_steps": _unique_strings(report_next_steps),
    }


def print_gateway_troubleshoot_report(
    platform: str | None = None,
    *,
    json_output: bool = False,
) -> None:
    report = build_gateway_troubleshoot_report(platform)
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
        return

    print("Gateway troubleshooting")
    gateway = report.get("gateway", {})
    if isinstance(gateway, dict):
        state = gateway.get("state") or "unknown"
        print(f"  Gateway state: {state}")
        if gateway.get("exit_reason"):
            print(f"  Last exit reason: {gateway['exit_reason']}")

    platforms = report.get("platforms", [])
    if not platforms:
        print("  No matching gateway platforms found.")
    for pdata in platforms:
        if not isinstance(pdata, dict):
            continue
        print()
        print(f"  {pdata.get('name')}")
        print(
            "    enabled: {enabled}  configured: {configured}  runtime: {runtime}".format(
                enabled="yes" if pdata.get("enabled") else "no",
                configured="yes" if pdata.get("configured") else "no",
                runtime=pdata.get("runtime_state") or "unknown",
            )
        )
        issues = pdata.get("issues") or []
        if issues:
            print("    issues:")
            for issue in issues:
                if isinstance(issue, dict):
                    print(f"      - [{issue.get('code')}] {issue.get('message')}")
        next_steps = pdata.get("next_steps") or []
        if next_steps:
            print("    next:")
...
def _gateway_command_inner(args):
    ...
    elif subcmd == "troubleshoot":
        print_gateway_troubleshoot_report(
