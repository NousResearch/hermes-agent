...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    fleet excluded from its sweep (#41403).  Default-scope callers
    (``gateway status``, cron checks, ``stop_profile_gateway``'s orphan
    reaper — which SIGTERM/SIGKILLs what it is fed) keep seeing only the
    current profile's service.  The systemd branch has always been
    fleet-wide (``hermes-gateway*``) and is unaffected: the orphan reaper
    never runs on systemd hosts.
    """
    pids: set = set()
