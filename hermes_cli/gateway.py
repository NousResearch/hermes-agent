...
LAUNCHD_LAUNCHER_NAME = "hermes-gateway"
_LAUNCHD_LAUNCHER_MARKER = "# hermes launchd launcher"


def _launchd_launcher_script(python: Path) -> str:
    return (
        "#!/bin/sh\n"
        f"{_LAUNCHD_LAUNCHER_MARKER} (generated by `hermes gateway install`, safe to delete).\n"
        "# macOS System Settings > Login Items & Extensions labels a LaunchAgent by the basename of\n"
        "# ProgramArguments[0]; without this file the gateway is listed as an anonymous \"python\".\n"
        "# A symlink can't do it (CPython derives base_executable from the basename), so exec keeps\n"
        "# sys.executable, venv detection and the PID identical to a direct launch.\n"
        f'exec "$(dirname "$0")/{python.name}" "$@"\n'
    )


def _launchd_launcher_name() -> str:
    """``hermes-gateway``, scoped per profile like the launchd label so several gateways stay distinguishable."""
    suffix = _profile_suffix()
    return f"{LAUNCHD_LAUNCHER_NAME}-{suffix}" if suffix else LAUNCHD_LAUNCHER_NAME


def _launchd_launcher_path() -> str:
    """ProgramArguments[0] for the launchd plist: a ``hermes-gateway`` exec wrapper beside the venv interpreter.

    macOS System Settings → Login Items & Extensions lists a LaunchAgent under the basename of
    ProgramArguments[0], so the gateway showed up as an anonymous ``python`` next to the user's other
    background items. Written on demand and refreshed when its text changes; a file we did not write
    (no marker) keeps the name and the plain interpreter is used instead.
    """
    python = Path(get_python_path())
    launcher = python.with_name(_launchd_launcher_name())
    script = _launchd_launcher_script(python)
    try:
        existing = launcher.read_text(encoding="utf-8") if launcher.is_file() else None
        if existing == script:
            return str(launcher)
        if (launcher.exists() or launcher.is_symlink()) and (existing is None or _LAUNCHD_LAUNCHER_MARKER not in existing):
            return str(python)
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    program = _launchd_launcher_path() if external_supervisor and is_macos() else get_python_path()
    return [program, "-m", "hermes_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
