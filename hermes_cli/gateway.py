...
_PLATFORMS = [
    ...
    },
    {
        ...
        "label": "QQ (NapCat)",
        "emoji": "🐧",
        "token_var": "QQ_NAPCAT_WS_URL",
        "setup_instructions": [
            "1. Start NapCat separately and enable its OneBot 11 WebSocket service",
            "2. Copy the WebSocket URL exposed by NapCat (for example ws://127.0.0.1:3001)",
            "3. If you configured an access token in NapCat, copy it as well",
            "4. Add your QQ numeric ID to the allowlist, and optionally set a home target like group:123456",
        ],
        "vars": [
            {"name": "QQ_NAPCAT_WS_URL", "prompt": "NapCat WebSocket URL", "password": False,
             "help": "The OneBot/NapCat websocket endpoint Hermes should connect to."},
            {"name": "QQ_NAPCAT_ACCESS_TOKEN", "prompt": "Access token (optional)", "password": True,
             "help": "Leave empty if NapCat websocket auth is disabled."},
            {"name": "QQ_NAPCAT_ALLOWED_USERS", "prompt": "Allowed QQ user IDs (comma-separated)", "password": False,
             "is_allowlist": True,
             "help": "Numeric QQ IDs allowed to message Hermes in DMs."},
            {"name": "QQ_NAPCAT_ALLOWED_GROUPS", "prompt": "Allowed QQ group IDs (comma-separated, optional)", "password": False,
             "help": "If set, Hermes only processes messages from these groups unless allow-all-groups is enabled."},
            {"name": "QQ_NAPCAT_HOME_CHANNEL", "prompt": "Home target (e.g. dm:123456 or group:987654)", "password": False,
             "help": "Used for cron/notification delivery when only the platform name is specified."},
        ],
    },
    {
        "key": "email",
