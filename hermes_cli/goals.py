...
_TOOL_LIMIT_RESPONSE_RE = re.compile(
    r"maximum number of tool-calling iterations"
    r"|tool[- ]call(?:ing)? iteration limit"
    r"|tool[- ]call(?:ing)? limit"
    r"|resource limits? prevent(?:ed)? full completion"
    r"|resource limit(?:s)? (?:hit|reached)"
    r"|hit .*?(?:tool|turn|time|context|execution) limit",
    re.IGNORECASE | re.DOTALL,
)


def _response_hit_tool_iteration_limit(last_response: str) -> bool:
    """Return True for the max-iteration summary generated by run_agent.

    Hitting the per-turn tool-call iteration cap is a continuation signal for
    standing goals, not evidence that the user objective is complete.  The
    agent's final summary often says PARTIAL/BLOCKED, and the judge prompt used
    to treat generic "blocked" language as DONE, collapsing a 20-turn goal into
    a single capped turn.
    """

    text = str(last_response or "")
    return bool(_TOOL_LIMIT_RESPONSE_RE.search(text))
...
JUDGE_SYSTEM_PROMPT = (
    ...
    "- The response explains the goal is permanently unachievable or needs "
    "new user input before any further autonomous work can proceed.\n\n"
    "A goal is NOT done when the response says work is partial, still in "
    "progress, or stopped because a tool-call, turn, time, context, execution, "
    "or other resource limit was hit. Resource-limit stops mean CONTINUE unless "
    "the goal's own turn budget is exhausted.\n\n"
...
class GoalManager:
    ...
    def evaluate_after_turn(
        ...
        *,
        ...
        if _response_hit_tool_iteration_limit(last_response):
            state.last_verdict = "continue"
            state.last_reason = "previous turn hit the tool-call iteration limit"
            ...
            return {
                ...
                "message": (
                    ...
                    "previous turn hit the tool-call iteration limit"
                ),
            }
