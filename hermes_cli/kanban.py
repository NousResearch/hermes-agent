...
def _cmd_edit(args: argparse.Namespace) -> int:
    ...
    if all(value is None for value in (title, body, priority, result, completion_contract)):
    ...
    with kbc.connect_closing() as conn:
        try:
            ...
            return _err(f"cannot edit {args.task_id}: a live worker is running it. Wait for the "
                        f"worker or `hermes kanban reclaim {args.task_id}` to release it, then retry.")
        except ValueError as exc:
            return _err(f"kanban edit: {exc}", 2)
    return _ok_or_err(
        ok,
        f"cannot edit {args.task_id} (unknown id, --result used on a task that is not done, "
        f"or --completion-contract used on a done/archived task)",
