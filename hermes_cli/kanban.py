...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    if board_override:
        try:
            normed = _owner_kanban_boards._normalize_board_slug(board_override)
        ...
        if normed != _owner_kanban_boards.DEFAULT_BOARD and not _owner_kanban_boards.board_exists(normed):
            return _err(f"kanban: board {normed!r} does not exist. "
                        f"Create it with `hermes kanban boards create {normed}`.")
        board_scope = _owner_kanban_boards.scoped_current_board(normed)
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        data = _owner_kanban_stats.known_assignees(conn)
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    try:
        all_boards = _owner_kanban_boards.list_boards(include_archived=False)
    except Exception:
        all_boards = []
    if len(all_boards) > 1:
        other_count = len(all_boards) - 1
        print(f"Board: {_owner_kanban_boards.get_current_board()} ({other_count} other board{'s' if other_count != 1 else ''} — "
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        comments = kanban_db.list_comments(conn, args.task_id)
        events = kanban_db.list_events(conn, args.task_id)
        parents = kanban_db.parent_ids(conn, args.task_id)
        children = kanban_db.child_ids(conn, args.task_id)
        runs = _owner_kanban_stats.list_runs(conn, args.task_id, **rsk)
        # Workers hand off via task_runs.summary; tasks.result stays NULL unless set.
        latest_summary = _owner_kanban_stats.latest_summary(conn, args.task_id)
        if not want_json:
            graph = kanban_db.task_graph_context(conn, task.id)
    ...
    if task.max_retries is not None:
        ...
        if cfg_val is not None and int(cfg_val) != kanban_db_dispatch.DEFAULT_FAILURE_LIMIT:
            print(f"  max-retries: {int(cfg_val)} (config kanban.failure_limit)")
        else:
            print(f"  max-retries: {kanban_db_dispatch.DEFAULT_FAILURE_LIMIT} (default)")
...
def _cmd_assign(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ok = kanban_db.assign_task(conn, args.task_id, profile)
...
def _cmd_set_model(args: argparse.Namespace) -> int:
    ...
    try:
        with kbc.connect_closing() as conn:
            ok = kanban_db.set_model_override(conn, args.task_id, model, provider=provider)
...
def _cmd_reclaim(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        ok = _owner_kanban_claims.reclaim_task(conn, args.task_id, reason=getattr(args, "reason", None))
...
def _cmd_reassign(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ok = _owner_kanban_claims.reassign_task(conn, args.task_id, profile, reclaim_first=reclaim, reason=getattr(args, "reason", None))
    return _ok_or_err(
        ok,
...
def _cmd_diagnostics(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        if getattr(args, "task", None):
            task = kanban_db.get_task(conn, args.task)
            if task is None:
                return _err(f"no such task: {args.task}")
            diags_by_task = {args.task: kd.compute_task_diagnostics(
                task, kanban_db.list_events(conn, args.task), _owner_kanban_stats.list_runs(conn, args.task),
                graph=kanban_db.task_graph_context(conn, args.task), config=diag_config)}
        else:
            ...
            if ids:
                ...
                graph_by = kanban_db.task_graph_contexts(conn, ids)
                for r in rows:
                    tid = r["id"]
...
def _cmd_link(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        kanban_db.link_tasks(conn, args.parent_id, args.child_id)
...
def _cmd_unlink(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        ok = kanban_db.unlink_tasks(conn, args.parent_id, args.child_id)
...
def _cmd_claim(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        task = _owner_kanban_claims.claim_task(conn, args.task_id, ttl_seconds=args.ttl)
        if task is None:
            existing = kanban_db.get_task(conn, args.task_id)
            if existing is None:
                return _err(f"no such task: {args.task_id}")
...
def _cmd_comment(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        kanban_db.add_comment(conn, args.task_id, author, body)
...
def _cmd_attach(args: argparse.Namespace) -> int:
    ...
    try:
        with kbc.connect_closing() as conn:
            att_id = kanban_db.store_attachment_bytes(conn, args.task_id, name, data, content_type=content_type,
...
def _cmd_attachments(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        if kanban_db.get_task(conn, args.task_id) is None:
            return _err(f"no such task: {args.task_id}")
        atts = kanban_db.list_attachments(conn, args.task_id)
...
def _cmd_attach_rm(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        removed = kanban_db.delete_attachment(conn, args.attachment_id)
    if removed is None:
...
def _goal_mode_handoff_rejection(task: Optional[_owner_kanban_db_models.Task], evidence: str):
...
def _goal_gate_error(conn, tid: str, evidence: str, handoff: str, blocked_hint: str,
    ...
    None to allow."""
    verdict, rejection = _goal_mode_handoff_rejection(kanban_db.get_task(conn, tid), evidence)
    if verdict == "blocked":
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        def op(tid):
            ...
            return _owner_kanban_completion.complete_task(conn, tid, result=args.result, summary=summary, metadata=metadata,
...
def _cmd_edit(args: argparse.Namespace) -> int:
    ...
    if rc:
        return rc
    with kbc.connect_closing() as conn:
        ok = _owner_kanban_completion.edit_completed_task_result(conn, args.task_id, result=args.result,
...
def _commented(conn, reason: Optional[str], author, prefix: str, op):
    ...
    def run(tid):
        if reason:
            kanban_db.add_comment(conn, tid, author, f"{prefix}: {reason}")
        return op(tid)
    return run
...
def _cmd_block(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        op = _commented(conn, reason, author, "BLOCKED", lambda tid: _owner_kanban_db.block_task(
...
def _cmd_schedule(args: argparse.Namespace) -> int:
    ...
    suffix = f": {reason}" if reason else ""
    with kbc.connect_closing() as conn:
        op = _commented(conn, reason, author, "SCHEDULED", lambda tid: _owner_kanban_transitions.schedule_task(
...
def _cmd_unblock(args: argparse.Namespace) -> int:
    ...
    suffix = f": {reason}" if reason else ""
    with kbc.connect_closing() as conn:
        op = _commented(conn, reason, author, "UNBLOCK", lambda tid: _owner_kanban_transitions.unblock_task(conn, tid))
...
def _cmd_request_review(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        persisted_run = _owner_kanban_stats.latest_run(conn, tid)
...
def _cmd_request_changes(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ok, detail = _owner_kanban_transitions.request_changes(conn, tid, reason=reason, expected_run_id=_worker_run_id_for(tid))
        if not ok:
...
def _cmd_reopen_review(args: argparse.Namespace) -> int:
    ...
    if reason is not None:
        reason = str(_owner_kanban_transitions.redact_review_value(reason.strip())).strip() or None
    author = _profile_author() if reason else None
    suffix = f": {reason}" if reason else ""
    with kbc.connect_closing() as conn:
        def op(tid):
            if not _owner_kanban_transitions.reopen_review_task(conn, tid):
                return False
            if reason:
                kanban_db.add_comment(conn, tid, author or "operator", f"CHANGES REQUESTED: {reason}")
            return True
...
def _cmd_promote(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        for tid in ids:
            ok, err = _owner_kanban_transitions.promote_task(conn, tid, actor=author, reason=reason, dry_run=dry_run)
...
def _cmd_archive(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        if purge_ids:
            return _bulk_apply(purge_ids, lambda tid: _owner_kanban_transitions.delete_archived_task(conn, tid), lambda tid: f"Deleted {tid}",
                               lambda tid: f"cannot delete {tid} (must already be archived)")
        return _bulk_apply(ids, lambda tid: _owner_kanban_transitions.archive_task(conn, tid),
                           lambda tid: f"Archived {tid}", lambda tid: f"cannot archive {tid}")


def _cmd_stats(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        stats = _owner_kanban_stats.board_stats(conn)
...
def _cmd_log(args: argparse.Namespace) -> int:
    content = _owner_kanban_stats.read_worker_log(args.task_id, tail_bytes=args.tail)
    if content is None:
...
def _cmd_context(args: argparse.Namespace) -> int:
    with kbc.connect_closing() as conn:
        text = _owner_kanban_context.build_worker_context(conn, args.task_id)
    print(text)
    return 0
