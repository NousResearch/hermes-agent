...
def _board_slug_arg(args: argparse.Namespace, cmd: str, *, must_exist: bool) -> tuple[Optional[str], int]:
    ...
    try:
        normed = _owner_kanban_db_boards._normalize_board_slug(args.slug)
    except ValueError as exc:
        return None, _err(f"kanban boards {cmd}: {exc}", 2)
    if must_exist:
        if not normed or not _owner_kanban_db_boards.board_exists(normed):
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    boards = _owner_kanban_db_boards.list_boards(include_archived=bool(getattr(args, "all", False)))
    current = _owner_kanban_db_boards.get_current_board()
    for b in boards:
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if rc:
        return rc
    already = _owner_kanban_db_boards.board_exists(normed) and normed != _owner_kanban_db_boards.DEFAULT_BOARD
    meta = _owner_kanban_db_boards.create_board(
    ...
    if getattr(args, "switch", False):
        _owner_kanban_db_boards.set_current_board(meta["slug"])
...
def _cmd_boards_rm(args: argparse.Namespace) -> int:
    ...
    try:
        res = _owner_kanban_db_boards.remove_board(args.slug, archive=not force_delete)
    except ValueError as exc:
...
def _cmd_boards_show(args: argparse.Namespace) -> int:
    current = _owner_kanban_db_boards.get_current_board()
    meta = _owner_kanban_db_boards.read_board_metadata(current)
...
def _cmd_boards_rename(args: argparse.Namespace) -> int:
    ...
    if rc:
        return rc
    meta = _owner_kanban_db_boards.write_board_metadata(normed, name=args.name)
...
def _cmd_boards_set_default_workdir(args: argparse.Namespace) -> int:
    ...
    if rc:
        return rc
    new_val = _owner_kanban_db_boards.write_board_metadata(normed, default_workdir=args.path).get("default_workdir")
    if new_val:
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    ...
    slug = args.slug or _owner_kanban_db_boards.get_current_board()
