...
def _board_task_counts(slug: str) -> dict[str, int]:
    ...
    try:
        # Cross-board surface: resolve this board's own DB even when the shell
        # carries the dispatcher's HERMES_KANBAN_DB pin, so `boards list` shows
        # per-board numbers instead of the pinned board's everywhere (#123733).
        db_path = kb.kanban_db_path(board=slug, allow_env_override=False)
