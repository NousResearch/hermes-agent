...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    def add_column_if_missing(table: str, column: str, ddl: str) -> None:
        """Add a column idempotently, tolerating concurrent/partial migrations."""
        table_cols = {
        ...
        try:
            ...
            # Multiple gateway/dispatcher processes can touch the board during
            # post-update startup. If another process won the migration race,
            # SQLite reports a duplicate column; that is already the desired
            # end state, so continue instead of breaking the dispatcher.
        ...
        for row in rows:
            ...
            if cur.rowcount == 1:
                ...
                crash_details.append(
                    (row["id"], pid, row["claim_lock"], protocol_violation, error_text)
                )
...
def task_age(task: Task) -> dict:
    ...
    age_since_started = now - int(task.started_at) if task.started_at else None
    time_to_complete = (
