...
_REBUILD_SPECS = {
    ...
    "kanban_notify_subs": (
        ...
        " thread_id TEXT NOT NULL DEFAULT '', chat_type TEXT, user_id TEXT,"
...
def create_task(
    ...
    *,
    ...
    ``model_override`` is an optional model slug to pass to the worker
    as ``-m <model>``. ``None`` means use the assignee profile's default
    model. This lets orchestrators route simple cards to cheaper/faster
    models while preserving high-tier defaults for hard cards.
    """
    ...
    with write_txn(conn):
        conn.execute(
            ...
            INSERT OR IGNORE INTO kanban_notify_subs
                (task_id, platform, chat_id, thread_id, chat_type, user_id, notifier_profile, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (task_id, platform, chat_id, thread_id or "", chat_type, user_id, notifier_profile, now),
        )
        if chat_type:
            # Repair legacy rows only when an authoritative source supplies
            # the missing session-key component. Unknown stays NULL so the
            # notifier can fail closed instead of waking a wrong session.
            conn.execute(
                """
