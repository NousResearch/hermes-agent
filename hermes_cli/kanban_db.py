...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "review_phase" not in cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN review_phase TEXT")
    if "routing_verdict" not in cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN routing_verdict TEXT")
    if "admission_snapshot" not in cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN admission_snapshot TEXT")
    if "closeout_evidence" not in cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN closeout_evidence TEXT")
    if "continuation_of" not in cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN continuation_of TEXT")
            ...
            with write_txn(conn):
                ...
                conn.execute(
                    ...
                    INSERT INTO tasks (
                        ...
                        review_phase, routing_verdict, admission_snapshot, closeout_evidence, continuation_of
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        task_id,
