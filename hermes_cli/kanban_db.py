...
def unblock_task(
    ...
    with write_txn(conn):
        ...
        # is a fresh start for the retry budget. The due columns are cleared
        # because the card is no longer time-gated: leaving a stale ``due_at``
        # behind would re-wake (or falsely alarm about) a card already running.
        cur = conn.execute(
