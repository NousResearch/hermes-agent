...
class KanbanActorContext:
    """Trusted identity for native workflow mutations.

    Worker-issued outcomes are additionally bound to the task's active run and
    claim; callers cannot manufacture that binding from model input.
    """
...
class WorkflowError(RuntimeError):
    """Base class for durable native-workflow failures."""
...
class WorkflowAuthorizationError(PermissionError, WorkflowError):
    """The caller lacks the scoped workflow capability."""


class WorkflowIntegrityError(WorkflowError):
    """The materialized aggregate diverges from its durable ledger."""
            ...
            with write_txn(conn, allow_nested=True):
                ...
                for parent_id in parents:
                    _validate_enrolled_link(
                        conn, parent_id, None, child_tenant=tenant,
                    )
                conn.execute(
                    """
                    INSERT INTO tasks (
...
def _validate_enrolled_link(
    ...
    """Keep prerequisite links touching workflow members tenant- and board-safe."""
    parent = conn.execute("SELECT tenant FROM tasks WHERE id=?", (parent_id,)).fetchone()
    child = (conn.execute("SELECT tenant FROM tasks WHERE id=?", (child_id,)).fetchone()
    ...
    task_ids = [parent_id] + ([child_id] if child_id is not None else [])
    rows = conn.execute(
        "SELECT m.task_id,m.tenant_snapshot,m.board_identity,w.tenant AS workflow_tenant,"
        "w.board_identity AS workflow_board_identity FROM kanban_workflow_members m "
        "JOIN kanban_workflows w ON w.id=m.workflow_id WHERE m.task_id IN ("
        + ",".join("?" for _ in task_ids) + ") AND m.removed_event_id IS NULL",
        task_ids,
    ).fetchall()
    if not rows:
        return
    ...
    for row in rows:
        ...
        if (not row["tenant_snapshot"] or not row["board_identity"]
            ...
            raise WorkflowIntegrityError("enrolled task tenant/board identity is inconsistent")
