...
def set_task_workspace(
    ...
    * Validation reuses the create-time contracts: ``workspace_kind`` must be
      in :data:`VALID_WORKSPACE_KINDS`; ``dir``/``worktree`` require an
...
def _fetch_pr_status(owner: str, repo: str, number: str) -> Optional[dict]:
    ...
    try:
        ...
        if proc.returncode != 0:
            _log.debug(
                "gh pr status fetch failed (%s/%s#%s): %s",
        ...
        _log.debug(
            "gh pr status fetch errored (%s/%s#%s): %s", owner, repo, number, exc
        )
        return None
...
def _active_pr_guard_applies(pr_url: str) -> tuple:
...
def _compute_active_pr_guard(pr_url: str) -> tuple:
    ...
    if status is None:
        _log.debug(
            "active_pr guard: indeterminate PR status for %s; failing closed", pr_url
        )
        return (True, None)
    ...
    *,
    ...
    for row in ready_rows:
        ...
        guard_reason = check_respawn_guard(
            conn, row["id"], exempt_out=guard_exemptions
        )
        if guard_reason is not None:
            ...
            continue
        # Observability for the exemption path: without this, a card that
        # respawns despite an open PR is indistinguishable in task_events
        # from one that never had a PR at all.
        if guard_exemptions and not dry_run:
            with write_txn(conn):
                for _ex in guard_exemptions:
                    _append_event(conn, row["id"], "respawn_allowed", _ex)
        if dry_run:
...
def auto_subscribe_session(conn: sqlite3.Connection, task_id: str) -> bool:
    """Subscribe the calling session to a new task's completion/block events.

    True iff a row exists for that session afterwards (surfaced as ``subscribed``
    by both the ``kanban_create`` tool and ``hermes kanban create`` so a caller can
    fall back to an explicit ``kanban notify-subscribe``). Gated by
    ``kanban.auto_subscribe_on_create`` (default True). Failures are logged and
    swallowed: bookkeeping must never fail the create that triggered it.
    """
