...
-- Immutable presentation binding for the exact message a release approver confirms. Identity
-- columns never change; only ``gate_status`` advances through the persisted release saga.
...
-- Mutable release truth used for the approval-time recheck. Release publishers update this row
-- whenever Dev or workflow state changes; the gate above remains the immutable visible snapshot.
...
-- Persisted idempotent saga. The adapter receives ``operation_key`` and must treat retries with
-- that key as one operation. An ambiguous adapter outcome remains resumable with the same key.
