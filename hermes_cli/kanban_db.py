...
-- Per-assignee crash-loop quarantine. Populated by
-- ``evaluate_crash_breaker`` when an assignee's crashed-event rate exceeds
-- the configured threshold (default: 3 crashes in 120s). The dispatcher's
-- ready/review loops consult ``is_assignee_quarantined`` before claiming
-- so a single broken venv / missing module / wedged credential storm
-- doesn't burn the whole board's spawn budget.
--
-- ``probe_in_flight`` is the single-probe gate: when cooldown elapses,
-- exactly one spawn is permitted. Probe success → row deleted. Probe
-- crash → cooldown doubled (capped at max_cooldown), probe_in_flight
-- cleared so the next post-cooldown tick gets one more probe.
            ...
            with write_txn(conn):
                ...
                conn.execute(
                    ...
                    INSERT INTO tasks (
                        ...
                        skills, max_retries, goal_mode, goal_max_turns, session_id, max_iterations
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        task_id,
...
def complete_task(
    ...
    *,
    ...
    _clear_failure_counter(conn, task_id)
    # Crash-loop breaker: if this task was the probe spawn for a
    # quarantined assignee, a successful completion proves the
    # assignee is healthy again — clear the quarantine.
    try:
...
def _cleanup_workspace(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    try:
        ...
        if _active_children:
            ...
            return
        # NOTE: the scratch dir is NOT removed here. ``complete_task`` (this
        # function's caller) runs inside the worker process with cwd == the
        # scratch dir; ``shutil.rmtree``-ing it inline deletes the cwd out from
        # under the live worker and crashes every subsequent ``os.getcwd()``
        # (fork PR #6). The dir is reaped out-of-process by
        # ``gc_scratch_workspaces`` on the dispatcher tick instead — which still
        # honors the #28818 containment guard against user source trees. We only
        # kill the now-dead tmux session here, and let a parent's deferred
        # cleanup proceed (#33774).
...
def _resolve_skill_under_home(skill_name: str, hermes_home: Optional[str]) -> bool:
    ...
    if cfg.is_file():
        try:
            ...
            ext = (
                (parsed.get("skills") or {}) if isinstance(parsed, dict) else {}
            ).get("external_dirs") or []
...
def _validate_task_skills(
    ...
    *,
    include_kanban_worker: bool = True,
) -> list[str]:
...
def _is_review_claimed_run(conn: sqlite3.Connection, task_id: str) -> bool:
...
def ensure_worktree(task: Task, path: Path) -> None:
    ...
    base_ref = (
        os.environ.get("HERMES_KANBAN_WORKTREE_BASE_REF", "").strip() or "myfork/main"
    )
    branch_exists = (
        subprocess.run(
    ...
    # Pin the worker git identity into the new worktree's local config and
    # install the pre-commit hook so every subsequent commit — including
    # auto-format follow-ups and rebase resolution commits — is attributed
    # correctly without relying on env-var inheritance.
    pin_workspace_git_identity(path)


def pin_workspace_git_identity(path: Path) -> None:
    """Pin the kanban worker git identity in a workspace directory.

    Two layers of defence:

    1. Sets ``user.name`` / ``user.email`` in the workspace's local
       ``.git/config`` (or in the git config of any git repo rooted at
       ``path``).  This survives subprocess shells, rebase-resolution
       commits, and auto-format follow-ups that don't carry ``-c`` flags.

    2. Installs a ``pre-commit`` hook that hard-rejects commits whose
       author email does not match the configured worker identity.  The
       hook is written to a ``hooks/`` directory under the workspace root
       (not ``.git/hooks/`` directly, which is reset by every
       ``git worktree add``) and wired via ``core.hooksPath``.

    Opt-out: set ``HERMES_KANBAN_ENFORCE_GIT_IDENTITY=false`` in the
    environment, or ``kanban.enforce_worker_git_identity: false`` in
    ``config.yaml``.  The git-config write (layer 1) always runs; only
    the hook installation (layer 2) is affected by the opt-out.

    Best-effort for non-git workspaces (scratch dirs that aren't git
    repos yet): the config write silently skips when ``.git`` is absent;
    the hook directory is created at ``path/.hermes-hooks/`` so it will
    be picked up once ``git init`` runs.
    """
    import subprocess

    git_name, git_email = _resolve_kanban_git_identity()
    ...
    # Layer 1: per-workspace git config (only when inside a git repo).
    if in_git_repo:
        for key, value in [("user.name", git_name), ("user.email", git_email)]:
            try:
                ...
                _log.warning(
                    "pin_workspace_git_identity: git config %s failed: %s", key, exc
                )

    # Layer 2: pre-commit hook (opt-out via env or config).
    _enforce = _enforce_worker_git_identity()
    if not _enforce:
        return

    # Write the hook to a stable directory under the workspace root
    # (not .git/hooks/ — that directory is recreated by git worktree add).
    hooks_dir = path / ".hermes-hooks"
    hooks_dir.mkdir(parents=True, exist_ok=True)
    hook_path = hooks_dir / "pre-commit"
    hook_content = (
        "#!/bin/sh\n"
        "# kanban worker identity guard — auto-generated by pin_workspace_git_identity\n"
        "# Rejects commits whose GIT_AUTHOR_EMAIL does not match the configured\n"
        "# worker identity.  Opt-out: HERMES_KANBAN_ENFORCE_GIT_IDENTITY=false\n"
        'if [ "${HERMES_KANBAN_ENFORCE_GIT_IDENTITY:-true}" = "false" ]; then\n'
        "    exit 0\n"
        "fi\n"
        f"_expected_email={git_email!r}\n"
        "_author_email=$(git var GIT_AUTHOR_IDENT 2>/dev/null"
        " | sed 's/.*<\\([^>]*\\)>.*/\\1/')\n"
        'if [ "$_author_email" != "$_expected_email" ]; then\n'
        '    echo "ERROR: worker commits must be authored as $_expected_email,'
        ' got $_author_email" >&2\n'
        '    echo "Run: git -c user.email=$_expected_email'
        ' -c \\"user.name=Sahil (AI)\\"'
        ' commit --amend --reset-author --no-edit" >&2\n'
        "    exit 1\n"
        "fi\n"
    )
    try:
        ...
        _log.warning("pin_workspace_git_identity: failed to write hook: %s", exc)
        return

    # Wire core.hooksPath to our stable directory (only inside a git repo).
    if in_git_repo:
        try:
            subprocess.run(
            ...
            _log.warning(
                "pin_workspace_git_identity: git config core.hooksPath failed: %s",
                exc,
            )


def _enforce_worker_git_identity() -> bool:
    """Return True when the pre-commit identity guard should be installed.

    Checks (in priority order):
    1. ``HERMES_KANBAN_ENFORCE_GIT_IDENTITY`` env var (``false`` → off).
    2. ``kanban.enforce_worker_git_identity`` in config.yaml (``false`` → off).
    3. Defaults to ``True``.
    """
    env_val = os.environ.get("HERMES_KANBAN_ENFORCE_GIT_IDENTITY", "").strip().lower()
    if env_val == "false":
        return False
    try:
        ...
        cfg_val = kanban_cfg.get("enforce_worker_git_identity")
        if cfg_val is not None and str(cfg_val).strip().lower() == "false":
            return False
    except Exception:
        pass
    return True
...
# ---------------------------------------------------------------------------
#
# Tracks `crashed` events per assignee. When the rate exceeds
# ``max_crashes`` within ``window_seconds``, the assignee is quarantined for
# ``cooldown_seconds``. After the cooldown elapses, exactly one probe spawn
# is permitted; success clears the quarantine, crash doubles the cooldown
# (capped at ``max_cooldown_seconds``).
#
# Tuned for the failure mode that motivated this (a worker corrupting the
# shared venv → every spawn crashes within seconds): 3-in-120s catches
# bursts quickly without tripping on the occasional one-off SIGKILL.
...
# Truncation for `last_reason` and the `quarantined` event payload so the
# notifier message stays well under platform per-message size caps.
...
class DispatchResult:
    ...
    as ``(task_id, assignee)`` pairs. Excludes tasks that get a probe slot
    — those are recorded in ``quarantine_probes`` instead."""
...
def _extract_crash_reason(payload_text: Optional[str]) -> Optional[str]:
    ...
    if pid is not None:
        return (f"pid {pid}" + (f" exit {code}" if code is not None else ""))[
...
def is_assignee_quarantined(
    ...
    * ``(True, False)``  — quarantined; dispatch must skip.
    ...
    with write_txn(conn):
        ...
        conn.execute(
            "UPDATE kanban_quarantine SET probe_in_flight = 1 " "WHERE assignee = ?",
            (assignee,),
        )
        return (False, True)
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    if crash_details:
        ...
        for tid, pid, claimer, protocol_violation, error_text in crash_details:
            ...
            # Crash-loop breaker: if this task was the probe for a
            # quarantined assignee, extend the cooldown with backoff so
            # we don't immediately storm another probe. Cleared on
            # probe success in ``complete_task``.
            try:
                probe_assignee = _probe_assignee_for_task(conn, tid)
                if probe_assignee:
                    extend_assignee_quarantine(
                        conn,
                        probe_assignee,
                        reason=error_text,
                    )
...
def check_respawn_guard(conn: sqlite3.Connection, task_id: str) -> Optional[str]:
    ...
    ``"active_pr"``
        ...
        The guard is suppressed (returns ``None``) when any of the
        following resume signals post-date the PR comment:
        ``rejected`` event, run with ``outcome='rejected'``,
        ``unblocked`` event, ``merge_requested`` event (prior merge
        worker died before finishing — must respawn to complete the
        merge), ``reclaimed`` event, or run with
        ``outcome='reclaimed'`` (prior run did not complete its
        intent).
    ...
    if latest_pr_ts is not None:
        ...
        # merge_requested: the prior worker was asked to merge but died
        # (gateway restart, reclaim, crash) before completing. The
        # dispatcher must be allowed to respawn to finish the merge.
        merge_event = conn.execute(
            "SELECT MAX(created_at) AS ts FROM task_events "
            "WHERE task_id = ? AND kind = 'merge_requested'",
            (task_id,),
        ).fetchone()
        # reclaimed event / run: explicit signal that the prior run did
        # not complete its intent — respawn is correct.
...
def _dispatch_once_locked(
    ...
    *,
    ...
    # assignee. Runs after crash detection so this tick's crashes count.
    if crash_breaker_enabled:
        try:
            result.quarantined = evaluate_crash_breaker(
                conn,
                max_crashes=crash_breaker_max_crashes,
                window_seconds=crash_breaker_window_seconds,
                cooldown_seconds=crash_breaker_cooldown_seconds,
                max_cooldown_seconds=crash_breaker_max_cooldown_seconds,
            )
        except Exception:
            pass  # never break dispatch on a breaker fault
    ...
    for row in ready_rows:
        ...
        if guard_reason is not None:
            ...
            continue
        # Crash-loop quarantine: skip spawn when the assignee is in
        # cooldown after a burst of crashes. ``is_assignee_quarantined``
        # atomically consumes the single probe slot when cooldown has
        # elapsed, so a quarantined assignee with a fresh probe slot
        # gets exactly one spawn this tick and subsequent same-tick
        ...
        try:
            ...
            if claimed.workspace_kind == "worktree":
                ...
                # For scratch and dir workspaces: pin identity now so the
                # workspace is ready before the worker's first git commit.
                # (worktree kind is handled inside _resolve_worktree_workspace.)
    ...
    for row in review_rows:
        ...
        if profile_exists is not None and not profile_exists(row["assignee"]):
            result.skipped_nonspawnable.append(row["id"])
            continue
        # Crash-loop quarantine also blocks reviewer spawns: a broken
        # venv breaks reviewers too, and the breaker exists to stop
        # storming a deterministically-broken assignee.
        is_probe = False
        if crash_breaker_enabled and not dry_run:
            blocked_q, is_probe = is_assignee_quarantined(conn, row["assignee"])
            if blocked_q:
...
def _resolve_kanban_git_identity() -> tuple[str, str]:
    ...
    try:
        ...
        kanban_cfg = {}
    name = kanban_cfg.get("git_identity_name") or DEFAULT_KANBAN_GIT_IDENTITY_NAME
    email = kanban_cfg.get("git_identity_email") or DEFAULT_KANBAN_GIT_IDENTITY_EMAIL
