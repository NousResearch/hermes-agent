...
def decompose_triage_task(
    ...
    try:
        with write_txn(conn):
            ...
            if cross_board and _has_decompose_event(conn, task_id):
                # A cross-board root stays in ``triage`` (see docstring), so
                # ``--all`` sweeps re-find it forever; a repeat decompose must
                # be an idempotent no-op, not a duplicate child batch.
                return None
            if cross_board:
