...
def _backfill_owner_events(conn: sqlite3.Connection) -> None:
    """Seed pre-contract creation receipts and scrub non-contract payloads.

    Historical tasks retain nullable provenance. Partial migration-test schemas
    have nothing to backfill. Existing owner receipts are never replaced.
    """
    from hermes_cli.kanban_db import _owner_contract_payload_from_json

    if not all(_table_exists(conn, table) for table in ("tasks", "task_events", "owner_events")):
        return
    if not {"id", "title", "created_by", "created_at", "status"}.issubset(_column_names(conn, "tasks")):
        return
    rows = conn.execute(
        """
        ...
        FROM tasks t WHERE NOT EXISTS (
            SELECT 1 FROM owner_events o WHERE o.task_id = t.id AND o.kind = 'created'
        )
        """
    ).fetchall()
    for row in rows:
        ...
        conn.execute(
            ...
            """,
            (row["id"], row["title"], row["created_by"], row["created_at"],
             json.dumps(safe, ensure_ascii=False) if safe is not None else None,
             1 if row["status"] == "archived" else 0, row["created_at"]),
        )
...
def _scrub_owner_event_payloads(conn: sqlite3.Connection) -> None:
    ...
    for row in conn.execute("SELECT id, kind, payload FROM owner_events WHERE payload IS NOT NULL").fetchall():
        ...
        if serialized != row["payload"]:
            conn.execute("UPDATE owner_events SET payload = ? WHERE id = ?", (serialized, row["id"]))
