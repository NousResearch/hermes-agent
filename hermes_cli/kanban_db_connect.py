...
def repair_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> RepairResult:
    ...
    quarantined."""
    path = db_path if db_path is not None else _owner_kanban_db.kanban_db_path(board=board)
    try:
        resolved = path.resolve()
    except OSError:
        resolved = path
...
# column existed.
_LATER_TASK_COLUMNS = (
    ("worker_scope", "worker_scope TEXT"),
    ("worker_pid_started_at", "worker_pid_started_at INTEGER"),
    ("worker_registered_at", "worker_registered_at INTEGER"),
    ("reclaim_reserved_at", "reclaim_reserved_at INTEGER"),
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    for name, ddl in (("worker_scope", "worker_scope TEXT"), ("stop_pending", "stop_pending INTEGER")):
        if _table_exists(conn, "task_runs") and name not in run_cols:
...
_REBUILD_SPECS = {
    ...
    "task_runs": (
        ...
        " worker_pid INTEGER, worker_scope TEXT, stop_pending INTEGER, max_runtime_seconds INTEGER,"
    ...
    with _scope_stop_intents_lock:
        ...
        if not levels:
            if conn.in_transaction:
                raise RuntimeError(
                    "Scope stops inside a transaction require an outer write_txn boundary"
                )
            return False
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        # A raw caller-owned BEGIN has no commit callback. Pure savepoint
        # composition is supported, but its scope stops must be refused before
        # effects instead of being queued early or silently discarded.
        with _scope_stop_intents_lock:
            if _scope_stop_intents_by_conn.get(id(conn)):
                _push_scope_stop = True
            else:
                _push_scope_stop = False
        if _push_scope_stop:
...
from hermes_cli import kanban_db_models as _models
