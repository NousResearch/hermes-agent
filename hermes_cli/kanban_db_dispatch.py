...
def enforce_max_runtime(
    ...
    ``failure_limit`` carries the dispatcher budget; direct callers retain the default.
    """
    timed_out: list[str] = []
    now = int(time.time())
    host_prefix = _kb._host_prefix()
...
def detect_crashed_workers(
    ...
    ``failure_limit`` governs ordinary crashes; protocol-violation and systemic
    thresholds remain separate, and saved per-task overrides still take precedence.
    """
    sweep = _reclaim_dead_workers(conn, board=board)
    # Outside the main txn: account each crash and maybe trip the breaker.
    auto_blocked = _account_crashes(conn, sweep.crash_details, failure_limit=failure_limit) if sweep.crash_details else []
