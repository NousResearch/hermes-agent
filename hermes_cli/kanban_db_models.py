"""Kanban row models and additive schema."""
...
class Task:
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        g = lambda col, default=None: _kb._row_get(row, col, default)  # noqa: E731
        parsed = _kb._json_or(g("skills"))
...
_TASK_OPTIONAL_COLUMNS = (
    "worker_scope", "branch_name", "project_id", "tenant", "result", "idempotency_key", "worker_pid",
    ...
    def from_row(cls, row: sqlite3.Row) -> "Run":
        return cls(
            **{
            ...
            id=int(row["id"]),
            worker_scope=_kb._row_get(row, "worker_scope"),
            started_at=int(row["started_at"]),
            ended_at=_kb._opt_int(row["ended_at"]),
            metadata=_kb._json_or(row["metadata"]),
        )
...
class Event:
    ...
    def from_row(cls, row: sqlite3.Row) -> "Event":
        run_id = _kb._row_get(row, "run_id")
        return cls(
            id=row["id"], task_id=row["task_id"], kind=row["kind"],
            payload=_kb._json_or(row["payload"]), created_at=row["created_at"], run_id=_kb._opt_int(run_id),
        )
