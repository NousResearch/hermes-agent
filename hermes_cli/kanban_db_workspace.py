...
def _managed_scratch_path_info(p: Path) -> tuple[bool, Optional[str]]:
    ...
    try:
        home = _owner_kanban_boards.kanban_home()
    except OSError:
        home = None
    if home is not None:
        with contextlib.suppress(OSError):
            roots.append(((home / "kanban" / "workspaces").resolve(strict=False), _owner_kanban_boards.DEFAULT_BOARD))
        entries: list[Path] = []
        with contextlib.suppress(OSError):
...
def _scratch_tip_sentinel_path() -> Path:
    ...
    return _owner_kanban_boards.kanban_home() / _SCRATCH_TIP_SENTINEL_NAME
...
def _resolve_worktree_workspace(task: Task, *, board: Optional[str] = None) -> tuple[Path, str]:
    ...
    if not task.workspace_path:
        board_slug = board if board else _owner_kanban_boards.get_current_board()
        board_default = (_owner_kanban_boards.read_board_metadata(board_slug).get("default_workdir") or "").strip()
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    if kind == "scratch" and not task.workspace_path:
        p = _owner_kanban_boards.workspaces_root(board=board) / task.id
    elif kind == "scratch":
