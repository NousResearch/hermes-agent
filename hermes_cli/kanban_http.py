...
@dataclass
class OrchestrationSettingsRequest:
...
def resolve_board(board: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        raise KanbanAPIError(status_code=400, detail=str(exc))
...
def http_get_task(
    ...
    try:
        ...
        if task is None:
            raise KanbanAPIError(status_code=404, detail=f"task {task_id} not found")
...
def http_create_task(payload: CreateTaskRequest, board: Optional[str]):
    board = resolve_board(board)
    conn = _conn(board=board)
    try:
        ...
        raise KanbanAPIError(status_code=400, detail=str(e))
    finally:
        conn.close()

def http_update_task(task_id: str, payload: UpdateTaskRequest, board: Optional[str]):
    board = resolve_board(board)
    conn = _conn(board=board)
    try:
        task = kanban_db.get_task(conn, task_id)
        ...
        if payload.assignee is not None:
            try:
                ...
                raise KanbanAPIError(status_code=409, detail=str(e))
            if not ok:
                raise KanbanAPIError(status_code=404, detail="task not found")
        ...
        if payload.status is not None:
            ...
            if s == "done":
                ...
                raise KanbanAPIError(status_code=400, detail=f"unknown status: {s}")
            if not ok:
            ...
            with kanban_db.write_txn(conn):
                ...
                if payload.title is not None:
                    if not payload.title.strip():
                        raise KanbanAPIError(status_code=400, detail="title cannot be empty")
                    sets.append("title = ?")
...
def http_delete_task(task_id: str, board: Optional[str]):
    board = resolve_board(board)
    conn = _conn(board=board)
    try:
...
def http_add_comment(task_id: str, payload: CommentRequest, board: Optional[str]):
    if not payload.body.strip():
        raise KanbanAPIError(status_code=400, detail="body is required")
    board = resolve_board(board)
    conn = _conn(board=board)
    try:
...
def http_add_link(payload: LinkRequest, board: Optional[str]):
    board = resolve_board(board)
    conn = _conn(board=board)
    try:
...
def http_bulk_update(payload: BulkTaskRequest, board: Optional[str]):
    ...
    if not ids:
        raise KanbanAPIError(status_code=400, detail="ids is required")
    results: list[dict] = []
    board = resolve_board(board)
    conn = _conn(board=board)
    try:
...
def http_get_run_endpoint(
    ...
    try:
        ...
        if r is None:
            raise KanbanAPIError(status_code=404, detail=f"run {run_id} not found")
        return {"run": _run_dict(r)}
    finally:
        conn.close()

def http_inspect_run_endpoint(
    run_id: int,
    board: Optional[str] = None,
):
...
def http_reclaim_task_endpoint(
    task_id: str,
...
def http_specify_task_endpoint(
    task_id: str,
...
def http_reassign_task_endpoint(
    task_id: str,
...
def http_subscribe_home(task_id: str, platform: str, board: Optional[str]):
...
def http_unsubscribe_home(task_id: str, platform: str, board: Optional[str]):
...
def http_get_stats(board: Optional[str]):
...
def http_get_assignees(board: Optional[str]):
...
def http_list_boards(include_archived: bool = False):
...
def http_create_board_endpoint(payload: CreateBoardRequest):
...
def http_rename_board(slug: str, payload: RenameBoardRequest):
    ...
    if not normed or not kanban_db.board_exists(normed):
        raise KanbanAPIError(status_code=404, detail=f"board {slug!r} does not exist")
...
def http_delete_board(slug: str, delete: bool = False):
...
def http_switch_board(slug: str):
...
def http_list_profile_roster():
    ...
    try:
        ...
        raise KanbanAPIError(status_code=500, detail=f"failed to list profiles: {exc}")
    return {
        "profiles": [
            {
                "name": p.name,
...
def http_update_profile_description(profile_name: str, payload: DescribeRequest):
    ...
    try:
        ...
        if not profile_dir.is_dir():
            raise KanbanAPIError(status_code=404, detail=f"profile '{profile_name}' not found")
        ...
        raise KanbanAPIError(status_code=500, detail=f"failed to update profile: {exc}")
    return {"ok": True, "profile": canon, "description": text}

def http_auto_describe_profile(profile_name: str, payload: DescribeAutoRequest):
    ...
    try:
        ...
        raise KanbanAPIError(status_code=500, detail=f"describer crashed: {exc}")
    return {
        "ok": bool(outcome.ok),
...
def http_decompose_task_endpoint(
    task_id: str,
...
def http_get_orchestration_settings():
...
def http_set_orchestration_settings(payload: OrchestrationSettingsRequest):
    ...
    try:
        ...
        raise KanbanAPIError(status_code=500, detail=f"failed to load config: {exc}")
    ...
    try:
        ...
        raise KanbanAPIError(status_code=500, detail=f"failed to save config: {exc}")

    # Echo back the resolved state (callers usually re-render from it).
    return get_orchestration_settings()

def fetch_events(cursor: int, board: Optional[str] = None):
    conn = kanban_db.connect(board=board)
    try:
        rows = conn.execute(
            "SELECT id, task_id, run_id, kind, payload, created_at FROM task_events WHERE id > ? ORDER BY id ASC LIMIT 200",
            (cursor,),
        ).fetchall()
        out = []
        new_cursor = cursor
