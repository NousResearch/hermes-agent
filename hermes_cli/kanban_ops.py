...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        max_in_progress_per_profile = _owner_kanban_worker_spawn._positive_int(
        ...
        max_in_progress = kbd.resolve_max_in_progress(
            _owner_kanban_worker_spawn._positive_int(_kanban_cfg.get("max_in_progress"), None)
        )
        # CLI --max is the more explicit signal, so it wins over kanban.max_spawn.
        cli_max = getattr(args, "max", None)
        max_spawn = (
            cli_max if cli_max is not None else _owner_kanban_worker_spawn._positive_int(_kanban_cfg.get("max_spawn"), None)
        )
    except Exception:
...
def _cmd_watch(args: argparse.Namespace) -> int:
    ...
    print(f"Watching kanban events (initial board '{_owner_kanban_boards.get_current_board()}'). Ctrl-C to stop.", flush=True)
...
def _cmd_gc(args: argparse.Namespace) -> int:
    ...
    import shutil
    scratch_root = _owner_kanban_boards.workspaces_root()
    removed_ws = 0
    with kbc.connect_closing() as conn:
        rows = conn.execute(
    ...
    with kbc.connect_closing() as conn:
        removed_events = _owner_kanban_stats.gc_events(conn, older_than_seconds=event_days * 24 * 3600)
    removed_logs = _owner_kanban_stats.gc_worker_logs(older_than_seconds=log_days * 24 * 3600)
