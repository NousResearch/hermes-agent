...
_TASK_DICT_FIELDS = (
    ...
    "session_id", "workflow_template_id", "current_step_key", "completion_contract", "last_failure_error", "worker_scope",
)
_SHOW_RUN_FIELDS = (
    "id", "profile", "step_key", "status", "outcome", "summary", "error",
    "metadata", "worker_pid", "worker_scope", "started_at", "ended_at",
)
_RUNS_RUN_FIELDS = (
    "id", "profile", "status", "outcome", "started_at", "ended_at",
    "summary", "error", "metadata", "worker_pid", "worker_scope", "step_key",
)
...
def _fmt_task_line(t: _owner_kanban_db_models.Task) -> str:
...
def _task_to_dict(t: _owner_kanban_db_models.Task) -> dict[str, Any]:
