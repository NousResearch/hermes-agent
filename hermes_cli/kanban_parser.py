...
_SPECS = [
    ...
    _cmd("log", [
             ...
             # Separate dest: a subparser default would clobber the parent-level
             # --board already parsed into args.board.
             _arg("--board", dest="log_board", default=None, metavar="<slug>",
                  help="Board whose logs dir to read (same as passing --board before "
                       "the subcommand; default: the current board)."),
         ],
         help="Print the worker log for a task (board-anchored logs dir: "
              "boards/<slug>/logs/, or kanban/logs/ on the default board)"),
