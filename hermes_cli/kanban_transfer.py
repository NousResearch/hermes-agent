...
def _scrub_local_state(conn: sqlite3.Connection) -> None:
    ...
    conn.execute("UPDATE task_runs SET claim_lock = NULL, claim_expires = NULL, worker_pid = NULL")
    # Export may snapshot a board that has not yet been migrated by this
    # install. Clear optional ownership columns when present; import runs
    # the same scrub after migration and must distrust archive contents.
    for table, fields in (
        ("tasks", ("worker_scope", "worker_pid_started_at", "worker_registered_at", "reclaim_reserved_at")),
        ("task_runs", ("worker_scope", "stop_pending")),
    ):
        present = {row[1] for row in conn.execute(f"PRAGMA table_info({table})")}
        resets = [f"{field} = NULL" for field in fields if field in present]
        if resets:
            conn.execute(f"UPDATE {table} SET {', '.join(resets)}")
...
def export_board(
    ...
    *,
    ...
    slug = _owner_kanban_boards._normalize_board_slug(board) or _owner_kanban_boards.get_current_board()
    if not _owner_kanban_boards.board_exists(slug):
        raise ValueError(f"board {slug!r} does not exist")

    db_path = _owner_kanban_db.kanban_db_path(slug)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        meta = _owner_kanban_boards.read_board_metadata(slug)
        ...
        attachments = copy_regular_files(_owner_kanban_boards.attachments_root(slug), staged / "attachments") if include_attachments else 0
        logs = copy_regular_files(_owner_kanban_boards.worker_logs_dir(slug), staged / "logs") if include_logs else 0
...
def _available_slug(preferred: str) -> str:
    ...
    if not _owner_kanban_boards.board_exists(preferred):
        return preferred
    # Leave headroom for the suffix inside the 64-char slug limit.
    stem = preferred[:58].rstrip("-_") or "board"
    n = 2
    while _owner_kanban_boards.board_exists(f"{stem}-{n}"):
        n += 1
    return f"{stem}-{n}"
...
def _relocate_imported_rows(conn: sqlite3.Connection, slug: str) -> tuple[dict[str, int], list[str]]:
    ...
    * Runtime state is scrubbed again (untrusted input, one UPDATE).
    ...
    warnings: list[str] = []
    now = int(time.time())
    attachments_dir = _owner_kanban_boards.attachments_root(slug)
...
def import_board(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        requested = _owner_kanban_boards._normalize_board_slug(slug or manifest.get("board") or archive_root)
        if not requested:
            raise ValueError(
        ...
        board_root = _owner_kanban_boards.board_dir(target)
    ...
    _owner_kanban_boards.write_board_metadata(
        target,
        name=name,
    ...
    _owner_kanban_db_connect.init_db(board=target)
    ...
    if activate:
        _owner_kanban_boards.set_current_board(target)
    ...
    return {
        ...
        "name": name,
        "path": str(_owner_kanban_boards.board_dir(target)),
        "db_path": str(_owner_kanban_db.kanban_db_path(target)),
        "source": {
