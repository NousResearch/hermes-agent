...
_REASON_TAG = re.compile(r"^[a-z][a-z0-9_]{0,63}$")


def _reason_tag_or_none(value: Any) -> Optional[str]:
    """The closed reason tag only, never the free text behind it (#111303).

    ``turn_loop_errors`` builds reasons like ``local_processing_error(Error during API call #3:
    <body>)``, whose parenthesized half can echo a provider / HTTP request or response body.
    Anything that is not a bare ``snake_case`` tag is dropped rather than serialized.
    """
    text = _str_or_none(value)
    if text is None:
        return None
    tag = text.split("(", 1)[0].strip()
    return tag if _REASON_TAG.match(tag) else None
...
def _owns_kanban_task() -> bool:
    """The dispatcher-owned gate in front of every ``HERMES_KANBAN_*`` identity read.

    ``HERMES_KANBAN_TASK`` is inherited by delegate children and in-process cron runs, so the
    env var alone would attribute a child's checkpoints to the parent worker's run.
    """
    try:
...
def telemetry_enabled() -> bool:
    ...
    try:
        ...
        return bool((load_config_readonly() or {}).get(CONFIG_SECTION, {}).get(CONFIG_KEY, False))
    except Exception:
        return False
...
def snapshot(
    ...
    *,
    ...
    reason = _reason_tag_or_none(exit_reason) if phase != PHASE_TURN else None
    warning_injected = getattr(agent, "_iteration_budget_warning_injected", None) is True
    # Stamped by the injection site; the sticky flag alone would make every later snapshot
    # claim the warning happened at its own ``recorded_at``.
    warning_at = _int_or_none(getattr(agent, "_iteration_budget_warning_at", None))
    ...
    return {
        ...
        "termination": {
            ...
            "warning_at": warning_at if warning_injected else None,
        },
...
def emit(
    ...
    *,
    ...
    # Throttle first: the gates below read config, and this runs on every activity touch.
    ...
    if not _owns_kanban_task():
        return None  # a delegate child / in-process cron run inherits HERMES_KANBAN_TASK
...
def report_for(
    ...
    if run is not None and run.task_id != task_id:
        # A run id from another task would report that run's time / handoff / termination
        # under this task's identity — the cross-attribution the unknown-id guard below
        # exists to prevent, one notch narrower.
        return None
    ...
    termination_reason = _reason_tag_or_none(snap_termination.get("reason")) or _str_or_none(
