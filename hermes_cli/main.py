...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    preflight_merge_base: Optional[str] = None
    ...
    try:
        ...
        # A managed install may be a depth-1 checkout. Repair enough history
        # to prove that the current checkout and the pinned update target are
        # related before any stash, checkout, reset, or rebase mutates user
        # state. This guard applies even when local-commit preservation is off.
        pinned_target_sha = _git_update_commit_sha(
            git_cmd, PROJECT_ROOT, f"origin/{branch}"
        )
        if pinned_target_sha is None:
            print(f"✗ Could not pin origin/{branch} before updating.")
            sys.exit(1)
        ...
        preflight_merge_base = preflight["merge_base"]
        if not preflight_merge_base:
            print(f"✗ {preflight['error']}")
            print("  No stash, checkout, reset, or rebase was attempted.")
            sys.exit(1)
        ...
        pinned_merge_base: Optional[str] = preflight_merge_base
        ...
        if pinned_pre_update_sha != original_checkout_sha:
        ...
        # Count against the same immutable target that passed the preflight.
        update_range = f"{pinned_pre_update_sha}..{pinned_target_sha}"
        result = subprocess.run(
        ...
        pre_pull_sha = pinned_pre_update_sha
        try:
            if preserve_local_commits:
                print(
            ...
            pinned_update = _apply_pinned_git_update(
                ...
                preserve_local_commits=preserve_local_commits,
                original_branch=current_branch,
                original_head=original_checkout_sha,
            )
