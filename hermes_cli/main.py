#!/usr/bin/env python3
"""
IX Agency CLI - Main entry point.
...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → IX Agency + Honcho
    hermes version             Show version
    hermes update              Update to latest version
    hermes uninstall           Uninstall IX Agency
...
def _print_fast_version_info() -> None:
    ...
    print(f"IX Agency v{__version__} ({__release_date__})")
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to IX Agency' own profile selector.
        """
        try:
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether IX Agency itself has been explicitly configured (model
    ...
    # Only count these if IX Agency has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants IX Agency to use their tokens.
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this IX Agency checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `hermes update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the IX Agency checkout, run `git restore -- ui-tui`\n"
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with IX Agency — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. IX Agency still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like IX Agency isn't configured yet -- no API keys or providers found."
        )
        print()
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with IX Agency?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ IX Agency'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_postinstall(args):
    ...
    print("⚕ IX Agency post-install bootstrap")
    print()
...
#
# IX Agency uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  IX Agency only falls back to a lightweight backend (OpenRouter,")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use IX Agency URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    IX Agency will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate IX Agency CLI with a provider."""
...
def cmd_backup(args):
    """Back up IX Agency home directory to a zip file."""
...
def cmd_uninstall(args):
    """Uninstall IX Agency (or just the Chat GUI with --gui)."""
...
# Critical files that IX Agency must be able to import immediately after an
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    an unrelated "IX Agency" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "IX Agency is damaged and can't be opened." The
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ IX Agency Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ IX Agency Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    ...
                    print("  running IX Agency desktop window and retry.")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "IX Agency is
    ...
    if source_mode:
        print("→ Launching IX Agency Desktop from source build...")
    ...
    print(f"→ Launching packaged IX Agency Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the IX Agency Desktop Electron app to protect its own
...
def _kill_stale_dashboard_processes(
    ...
    """
    # When the IX Agency Desktop Electron app spawns this dashboard as a
...
def _update_via_zip(args):
    """Update IX Agency by downloading a ZIP archive.
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if IX Agency behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but IX Agency couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but IX Agency couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if IX Agency behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but IX Agency couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but IX Agency couldn't drop "
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official IX Agency repository.")
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ IX Agency is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart IX Agency from a different terminal, "
...
def _detect_concurrent_hermes_instances(
    ...
    The IX Agency Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    ...
    #      hermes.exe sitting *under* a non-Hermes parent (e.g. a IX Agency
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close IX Agency Desktop, exit any open `hermes` REPLs, and")
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the IX Agency Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       IX Agency Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close IX Agency Desktop, exit other `hermes` REPLs, stop the "
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# IX Agency — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other IX Agency processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← IX Agency Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the IX Agency desktop app / other IX Agency terminals, then re-run:"
    )
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating IX Agency...")
    try:
...
def cmd_update(args):
    """Update IX Agency to the latest version.
    ...
    if is_managed():
        managed_error("update IX Agency")
        return
...
def _cmd_update_pip(args):
    """Update IX Agency via pip (for PyPI installs)."""
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all IX Agency windows/gateways and re-run: hermes update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # Warn if legacy IX Agency gateway unit files are still installed.
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy IX Agency gateway unit(s) detected:")
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     IX Agency {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: IX Agency {mf.hermes_requires}")
...
def cmd_dashboard(args):
    ...
    # the same logs directory as every other IX Agency surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
...
def cmd_logs(args):
    """View and filter IX Agency log files."""
...
def cmd_console(args):
    """Open the safe IX Agency command console."""
...
def cmd_acp(args):
    """Launch IX Agency as an ACP server."""
    try:
...
def main():
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and IX Agency shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the IX Agency app. `status` reports the driver's grant\n"
