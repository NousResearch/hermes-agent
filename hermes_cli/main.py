...
def _android_excluded_extras() -> set[str]:
    """Return extras that should be skipped on Android/Termux.

    These extras have transitive deps without wheels for android_arm64 + cp313
    (e.g. ctranslate2 via faster-whisper, davey via discord.py[voice]).
    The [voice] extra already uses ``sys_platform != 'android'`` markers in
    pyproject.toml, but [messaging] pulls in discord.py[voice] which requires
    the Rust-based davey package that cannot build on Termux.
    """
    if sys.platform != "android":
        return set()
    # voice: faster-whisper -> ctranslate2 (no android wheels)
    # messaging: discord.py[voice] -> davey (Rust/maturin, no android support)
    # matrix: mautrix[encryption] -> python-olm (libolm archived, build fails)
    return {"voice", "messaging", "matrix"}
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    excluded = _android_excluded_extras()

    if excluded:
        # On Android/Termux: build a custom extras string that skips
        # extras known to have incompatible transitive dependencies.
        all_extras = _load_installable_optional_extras()
        installable = [e for e in all_extras if e not in excluded]
        extras_str = ",".join(installable)
        print(f"  ℹ Android/Termux detected — skipping extras: {', '.join(sorted(excluded))}")
        try:
            subprocess.run(
                install_cmd_prefix + ["install", "-e", f".[{extras_str}]", "--quiet"],
                cwd=PROJECT_ROOT,
                check=True,
                env=env,
            )
            ...
            print(
                "  ⚠ Custom extras install failed, falling back to base + individual..."
            )
    ...
    for extra in _load_installable_optional_extras():
        if extra in excluded:
            print(f"  ⊘ Skipping extra [{extra}] — incompatible with Android/Termux")
            continue
        try:
            subprocess.run(
