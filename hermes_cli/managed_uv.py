...
def pip_install_hint(package: str) -> str:
    ...
    Names Hermes' own uv when it exists: the installer keeps the managed binary in
    the private ``$HERMES_HOME/uv`` dir without putting it on PATH, so a bare ``uv``
    would fail for installer-only users.
    """
