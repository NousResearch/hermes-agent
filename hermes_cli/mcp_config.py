...
def _print_tools(tools: List[Tuple[str, str, str]], width: int, desc_max: int) -> None:
...
def _probe_single_server(
    ...
    Returns ``(tool_name, title, description)`` tuples; raises on connection failure. ``title`` is the
    MCP spec's optional human-readable display name (e.g. "abc xyz 123" for ``abc_xyz_123``) and is ""
    when the server doesn't set one — callers fall back to ``tool_name``. ``details`` is an out-param
    filled with ``schema_chars``/``prompts``/``resources`` so the return shape stays stable.

    ``truncate_descriptions``: True (default) keeps the historical 80-char CLI cut; pass False from
    API/dashboard callers that need the full description (e.g. the dashboard test route).
    """
    ...
    async def _probe():
        ...
        try:
            for t in server._tools:
                ...
                # Truncate for CLI display only; dashboard/API callers ask for the full text.
