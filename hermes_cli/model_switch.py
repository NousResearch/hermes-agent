...
def _apply_picker_preferences(
    ...
    """Apply the user's ``model.picker`` config (hide + order) to the picker rows.

    Two optional, purely-cosmetic config knobs let a user tailor the interactive
    ``/model`` dropdown without touching which providers are *usable* (typed
    ``/model <slug>/...`` and every provider stay reachable regardless):

    ```yaml
    model:
      picker:
        hide:  [openai-api, anthropic]      # slugs to drop from the dropdown
        order: [claude-apr, claude-bpr, ...] # front-anchored display order
    ```

    - ``hide``: drop matching provider rows (by slug, case-insensitive). The
      **currently-active** provider is NEVER hidden — you must always be able to
      see/switch off what you're on. Unknown slugs are ignored.
    - ``order``: rows whose slug appears here are moved to the front in the given
      order; every other row keeps its original relative order after them. Unknown
      slugs in ``order`` are ignored (no empty rows). Rows not listed are NOT
      hidden — ordering and hiding are independent.

    Missing/malformed config is a no-op (returns ``rows`` unchanged); this never
    raises into the picker path.
    """
    try:
        from hermes_cli.config import load_config
