    ...
    if provider_changed or explicit_provider:
        ...
        if _user_pdef is not None and _user_pdef.base_url:
            ...
            try:
                ...
                api_mode = runtime.get("api_mode", "")
                # Normalize target_provider to the canonical registry id.
                # PATH A sets target_provider from resolve_provider_full().id,
                # which for models.dev-backed providers is the models.dev id
                # (e.g. "kimi-for-coding"), NOT the PROVIDER_REGISTRY key
                # ("kimi-coding"). resolve_runtime_provider() already maps
                # back to the canonical id via resolve_provider(); without
                # adopting it here, downstream consumers keyed on the
                # registry id — notably the credential pool — load nothing
                # and mid-session 403/429 rotation silently falls through to
                # fallback instead of rotating to the second account.
                _runtime_provider = str(runtime.get("provider", "") or "").strip()
                if _runtime_provider and _runtime_provider != target_provider:
...
def list_authenticated_providers(
    ...
    *,
    ...
    # PATCH 2026-07-20: Intentional-subset shadow suppression. Pre-compute
    # the set of endpoints that a custom_providers entry deliberately
    # narrows (explicit models + discover_models:false). When a built-in
    # canonical provider shares that endpoint, hide the built-in row so the
    # picker shows ONLY the user's subset. Used by Sections 1 and 2b.
    _subset_endpoints: set = set()
    if isinstance(custom_providers, list):
        for _e in custom_providers:
            if not isinstance(_e, dict):
                continue
            _e_url = (_e.get("base_url") or _e.get("url") or _e.get("api") or "").strip().rstrip("/").lower()
            _e_models = _e.get("models", {})
            _e_has_models = bool(_e_models) if isinstance(_e_models, (dict, list)) else bool(_e_models)
            _e_discover = _e.get("discover_models", True)
            if isinstance(_e_discover, str):
                _e_discover = _e_discover.lower() not in {"false", "no", "0"}
            if _e_url and _e_has_models and not _e_discover:
                _subset_endpoints.add(_e_url)
    ...
    from hermes_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hermes_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    from hermes_cli.providers import _LABEL_OVERRIDES as _LABELS
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Skip pure alias keys (e.g. "kimi", "moonshot") that are NOT
        # registered providers.  The canonical slug ("kimi-coding") has
        # its own PROVIDER_TO_MODELS_DEV entry and will emit the row.
        # Without this skip the alias steals the mdev_id slot and the
        # canonical slug gets deduped, re-appearing later under a
        # different label.
        if hermes_id not in PROVIDER_REGISTRY and hermes_id not in _LABELS:
            continue
        # Skip aliases that map to the same models.dev provider (e.g.
        # kimi-coding and kimi-coding-cn both → kimi-for-coding).
        # The first one with valid credentials wins (#10526).
        # Exception: providers with their own _LABEL_OVERRIDES entry are
        # independent provider slugs (e.g. kimi-coding-b / kimi-coding-c
        # are isolated Kimi account pools, not aliases of kimi-coding).
        # Deduping them by mdev_id would hide them from the picker.
        if mdev_id in seen_mdev_ids and hermes_id not in _LABELS:
            continue
        ...
        # Providers with their own label override (e.g. kimi-coding-b/c
        # for isolated Kimi account pools) use the override instead of the
        # models.dev display name so the picker distinguishes accounts.
        display_name = _LABELS.get(hermes_id, pinfo.name if pinfo else mdev_id)

        # PATCH 2026-07-20: hide built-in row when an intentional-subset
        # custom provider supersedes this endpoint.
        if pconfig and _norm_url(getattr(pconfig, "inference_base_url", "")) in _subset_endpoints:
    ...
    # PATCH 2026-07-20: _subset_endpoints is computed above (before Section 1)
    # so both Section 1 and Section 2b can check it.
    ...
    for _cp in _canon_provs:
        ...
        # PATCH 2026-07-20: If a custom_providers intentional-subset covers
        # this built-in provider's endpoint, hide the built-in row so the
        # picker shows only the user's curated subset.
        _cp_endpoint_norm = ""
        if _cp_config and getattr(_cp_config, "inference_base_url", ""):
            _cp_endpoint_norm = _norm_url(_cp_config.inference_base_url)
        if _cp_endpoint_norm and _cp_endpoint_norm in _subset_endpoints:
            continue
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            if not api_key:
                ...
                api_key = (get_env_value(key_env) or "").strip() if key_env else ""
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #
            # PATCH 2026-07-20: Intentional-subset exemption. #16970's intent
            # is to hide *redundant shadows* — entries with no explicit model
            # list that merely duplicate a built-in endpoint. But a custom
            # provider that sets ``discover_models: false`` AND declares an
            # explicit ``models:`` subset is deliberately narrowing the
            # built-in catalog (e.g. OpenCode Zen 64-model catalog narrowed to
            # a 12-model working subset). These intentional subsets must stay
            # visible in the picker; swallowing them defeats the narrowing.
            _grp_url_norm = _pair_key[1]
            _is_intentional_subset = (
                bool(grp.get("models"))
                and not grp.get("discover_models", True)
            )
            if (
                _grp_url_norm
                and _grp_url_norm in _builtin_endpoints
                and not _is_intentional_subset
            ):
                continue
