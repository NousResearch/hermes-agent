...
def _find_model_in_custom_providers(
    ...
    """Search custom_providers for a matching model.
    
    Args:
        model_input: The model name to search for.
        custom_providers: List of custom provider dicts from config.
    
    Returns:
        (provider_slug, model_id, model_input) if found, else None.
    """
    if not custom_providers:
        return None
    
    model_lower = model_input.strip().lower()
    # Normalize dots to hyphens for matching (claude-sonnet-4.6 -> claude-sonnet-4-6)
    ...
    for cp in custom_providers:
        ...
        # models can be a dict {model_id: model_info} or a list [model_id, ...]
        if isinstance(cp_models, dict):
            model_ids = list(cp_models.keys())
        ...
        for mid in model_ids:
            ...
            if mid_lower == model_lower:
                return (cp_slug, mid, model_input)
            # Normalized match (dots <-> hyphens)
            if mid_lower.replace(".", "-") == model_normalized:
                return (cp_slug, mid, model_input)
            # Bare name match (for vendor/model format)
            if "/" in mid_lower:
                _, bare = mid_lower.split("/", 1)
                if bare == model_lower or bare.replace(".", "-") == model_normalized:
                    return (cp_slug, mid, model_input)
    ...
    # =================================================================
        ...
        if alias_result is not None:
            ...
            if key in MODEL_ALIASES:
                ...
                # --- Step c2: Search custom_providers catalog ---
                if custom_providers:
                    custom_match = _find_model_in_custom_providers(raw_input, custom_providers)
                    if custom_match is not None:
                        target_provider, new_model, _ = custom_match
                        logger.debug(
                            "Model '%s' found in custom provider '%s' as '%s'",
                            raw_input, target_provider, new_model,
                        )
                        # Skip aggregator conversion and catalog search
                        # since we already found the model
                        pass
