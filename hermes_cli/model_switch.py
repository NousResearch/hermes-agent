...
_HERMES_MODEL_WARNING = (
    ...
    "for use with IX Agency. They lack the tool-calling capabilities "
...
def list_authenticated_providers(
    ...
    *,
    ...
    # requiring a IX Agency release. Falls back to the in-repo
    ...
    # --- 1. Check IX Agency-mapped providers ---
    from hermes_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hermes_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # --- 2. Check IX Agency-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hermes_cli.providers import HERMES_OVERLAYS
    from hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → IX Agency provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use IX Agency IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve IX Agency slug — e.g. "github-copilot" → "copilot"
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is IX Agency's canonical write key (matches
            ...
            # IX Agency writes ``models:`` as a dict keyed by model id, but older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # IX Agency's own writer uses; a hyphen variant is accepted
            ...
            # active model. IX Agency's own writer (main.py::_save_custom_provider)
