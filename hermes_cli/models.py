...
def get_default_model_for_provider(provider: str) -> str:
    ...
    if provider in _SILENT_DEFAULT_PROVIDERS:
        ...
        # snapshot carries the default). Checked before the plugin-profile
        # fallback below so populating ``models`` never defeats the
        # ``not models`` shortcut for these metered aggregators.
