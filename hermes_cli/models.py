...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry(
        "xiaomi", "Xiaomi MiMo", "Xiaomi MiMo (MiMo-V2 models — pro, omni, flash)"
    ),
    ProviderEntry(
        "nvidia",
        "NVIDIA NIM",
    ...
    ProviderEntry(
        "nvidia-nim",
        "NVIDIA NIM",
        "NVIDIA NIM (hosted inference — integrate.api.nvidia.com)",
    ),
]
    ...
    if direct_match:
        ...
        if not has_creds:
            try:
                ...
                if direct_match in store.get(
                    "providers", {}
                ) or direct_match in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
    ...
    if normalized == "openai-codex":
        ...
        if codex_models:
            ...
            auto = get_close_matches(
                requested_for_lookup, codex_models, n=1, cutoff=0.9
            )
            if auto:
                return {
                    "accepted": True,
