...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    "litellm": [
        "opus4.6[1M]",
        "sonnet4.6[1M]",
        "claude-opus-4-6@default[1M]",
        "claude-sonnet-4-6@default[1M]",
        "opus",
        "sonnet",
        "haiku",
        "gemini-3-flash",
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry(
        "litellm",
        "LiteLLM",
        "LiteLLM (self-hosted proxy — Anthropic, OpenAI, Gemini via unified gateway)",
    ),
]
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        if catalog_models:
            ...
            provider_label = (
                "OpenAI Codex"
                if normalized == "openai-codex"
                else "xAI Grok OAuth (SuperGrok Subscription)"
            )
            return {
                "accepted": True,
                "persist": True,
