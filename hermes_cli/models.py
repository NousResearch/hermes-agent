...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append IX Agency-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. IX Agency refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older IX Agency that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older IX Agency that doesn't ship
...
#
# Some vendors expose several IX Agency provider slugs (one per endpoint /
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to IX Agency' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether IX Agency should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a IX Agency release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a IX Agency release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a IX Agency release.
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: IX Agency' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"IX Agency will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so IX Agency cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models IX Agency actually ships.  (#46850)
