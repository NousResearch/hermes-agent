...
def _is_version_only_difference(requested: str, candidate: str) -> bool:
    """True when two model ids differ purely by digit values in place.
...
def _match_in_catalog(
    ...
    *,
    ...
    # A version-only difference from a catalog entry (e.g. a newly released
    # model the listing hasn't caught up with) must not auto-correct to a
    # different version — that silently swaps pricing/context (#101975).
    # Fall through to suggestions + the branch's soft-accept instead.
    if auto and not _is_version_only_difference(query, auto[0]):
