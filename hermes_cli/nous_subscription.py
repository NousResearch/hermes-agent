...
_FEATURES: Dict[str, _FeatureSpec] = {
    "web": _FeatureSpec(
        ...
        "Web search & extract (Firecrawl)", "Firecrawl/Exa/Parallel/Tavily/Perplexity/Keenable/Linkup key or SearXNG",
        ("PARALLEL_API_KEY", "TAVILY_API_KEY", "PERPLEXITY_API_KEY", "FIRECRAWL_API_KEY", "FIRECRAWL_API_URL", "LINKUP_API_KEY"),
    ),
...
def _web_feature(web_cfg: Dict[str, object], tool_enabled: bool, managed: bool, web_gw: bool, direct_firecrawl: bool) -> NousFeatureState:
    ...
    direct = {
        ...
        "linkup": _any_env("LINKUP_API_KEY") and not web_gw,
        "searxng": _any_env("SEARXNG_URL"),
    }
    web_managed = backend == "firecrawl" and managed and not direct_firecrawl
    active = web_managed or direct.get(backend) or direct.get(search_backend) or (extract_backend in ("tavily", "perplexity", "linkup") and direct[extract_backend])
    return _state(
...
def _get_gateway_direct_credentials() -> Dict[str, bool]:
    ...
    return {
        "web": _any_env("FIRECRAWL_API_KEY", "FIRECRAWL_API_URL", "PARALLEL_API_KEY", "TAVILY_API_KEY", "PERPLEXITY_API_KEY", "EXA_API_KEY", "LINKUP_API_KEY", "SEARXNG_URL"),
