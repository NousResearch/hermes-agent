...
class PluginManager:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        ``platform`` kwargs) two extra kwargs: ``agent`` (the live
        ``AIAgent`` for this turn) and ``session_key`` (a stable
        per-session identifier). A callback may return one of three
