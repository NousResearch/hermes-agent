...
def _get_pre_tool_call_directive_details(
    ...
    wins independent of registration order: any plugin's valid veto wins over an earlier
    plugin's request for human confirmation (#87420). Ties among equal rank break by
    registration order (first wins). ``modify`` is orthogonal and always accumulates. A
    ``serve`` is valid only when its ``"result"`` KEY is present (``result: null`` is a serve
    of ``None``); a block without a message is ignored (fail-open); irrelevant returns are
    ignored."""
