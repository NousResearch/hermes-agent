...
def _as_plugin_path(path: Any, *, what: str = "plugin path") -> Path:
    """Coerce a plugin filesystem location to ``Path``.

    Manifests store directory locations as ``str`` (entrypoint ``path``
    values are module specs, not files). Call sites that need ``.exists()``
    or ``.is_dir()`` must go through this helper so a string never raises
    ...
    if isinstance(path, str):
        ...
        return Path(path)
    raise TypeError(
        f"{what} must be a filesystem path (str or Path), "
...
class PluginContext:
    ...
    def register_skill(
        ...
        *path* may be a ``Path`` or a filesystem string. String paths are
        coerced so plugin ``register()`` helpers that pass ``str`` do not
        crash load with ``'str' object has no attribute 'exists'``.

        Raises:
            ValueError: if *name* contains ``':'`` or invalid characters,
                or *path* is not a usable filesystem location.
        ...
        try:
            path = _as_plugin_path(path, what="skill path")
        except (TypeError, ValueError) as exc:
...
class PluginManager:
    ...
    def _scan_directory_level(
        ...
        *,
        ...
        manifests: List[PluginManifest] = []
        try:
            path = _as_plugin_path(path, what="plugin directory")
        except (TypeError, ValueError):
            return manifests
        if not path.is_dir():
            return manifests
