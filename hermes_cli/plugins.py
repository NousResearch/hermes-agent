...
class PluginContext:
    ...
    def register_transcription_provider(self, provider) -> None:
        ...
        **but only when** ``provider.name`` is NOT a built-in STT
        provider name (``local``, ``local_command``, ``groq``,
        ``openai``, ``mistral``, ``xai``). Built-ins always win — the
        registry rejects shadowing names with a warning.

        Coexists with the existing in-tree dispatcher rather than
        replacing it. The 6 built-in STT backends keep their native
        implementations in ``tools/transcription_tools.py``; this hook
        is for *new* engines (OpenRouter, SenseAudio, Gemini-STT,
