...
def _generate_minimal_manifest(plugin_dir: Path) -> str:
    """Generate a minimal plugin.yaml for a skill-only bundle.

    Uses the directory name as plugin name (validated against the namespace
    pattern). Tries to extract a description from README.md's first
    paragraph; falls back to a generic description.
    """
    ...
    if readme.exists():
        try:
            ...
            pass  # README read failure is non-fatal
...
def _autogen_plugin_shim(plugin_dir: Path, console) -> None:
    """If this plugin is a skill-only bundle lacking a hermes wrapper,
    generate __init__.py and (optionally) plugin.yaml from static templates.

    Tracks generated files in .hermes-autogen/shim.lock so updates can
    safely regenerate without clobbering user-authored customizations.

    No-op if:
    - skills/ subdirectory does not exist
    - __init__.py already exists (don't overwrite upstream)
    """
    skills_dir = plugin_dir / "skills"
    if not skills_dir.is_dir():
        return  # not a skill bundle

    # Reject plugin names containing ':' to avoid qualified-name ambiguity
    ...
    if init_existed:
        return  # upstream provides its own __init__.py, leave alone

    # Count skills for user output
    skill_count = sum(
    ...
    try:
        ...
        # Rollback any partially-written files
        for p in written:
            try:
                p.unlink()
            except Exception:
