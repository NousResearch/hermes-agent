...
def _reset_plugin_locks_after_fork():
    ...
    # Close the child's duplicate descriptions, never LOCK_UN: flock ownership is
    # shared with the parent. Vanished threads cannot release inherited RLocks.
        ...
        with open(lock_path, "a+b", buffering=0) as handle:
            ...
            try:
                ...
                try:
                    ...
                    # An inherited context may unwind in the fork child, whose
                    # at-fork hook already closed its duplicate handle.
