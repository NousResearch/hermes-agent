...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    # A copy per caller: the cached value is shared, and these four scalars are read-only to every
    # caller today — a future one that mutates its result must not poison the next reader.
    return dict(_cached_profile_read(profile_dir / "profile.yaml", "profile-meta", _read))
