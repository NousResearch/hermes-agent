...
def _cmd_bind_board(args, conn, proj) -> str:
    ...
    if proj.primary_path:  # best-effort: point the bound board's default_workdir at the primary repo
        try:
            ...
            slug = _owner_kanban_boards._normalize_board_slug(args.board)
            if slug and (slug == _owner_kanban_boards.DEFAULT_BOARD or _owner_kanban_boards.board_exists(slug)):
                _owner_kanban_boards.write_board_metadata(slug, default_workdir=proj.primary_path)
        except Exception:
            pass
