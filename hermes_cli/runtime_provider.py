...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    # Explicit credentials bypass the pool by contract: a pinned api_key/base_url
    # is not a pooled credential, so "is this provider's pool cooling" is not a
    # question about it, and probing would read a store the caller deliberately
    # opted out of. Callers that own a fallback chain probe such a runtime
    # directly (see ``runtime_credentials_cooling_down_until``) rather than
    # trusting an annotation that was never stamped.
    if str(runtime.get("source") or "") == "explicit":
        return runtime
    # Whichever rung answered, the credentials behind it can be serving a 429:
    # a pool whose every live entry is benched still resolves, because the
    # ladder falls through to the config/env key -- routinely the SAME
    # credential the pool is cooling. Annotating here rather than inside each
    # rung keeps the ladder's rungs ignorant of the policy and means a rung
    # added later cannot forget it. The runtime stays fully usable; only
    # callers that own a fallback chain read the annotation.
    return _annotate_pooled_cooldown(
        runtime,
        pool_for_runtime(runtime),
        # OpenRouter is the one provider for which "resolved without a key"
        # means the pool is drained rather than "this endpoint needs no auth".
        keyless_means_drained=(
