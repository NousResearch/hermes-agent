...
def _openrouter_should_use_pool(requested_provider, model_cfg, explicit_api_key, explicit_base_url) -> bool:
    """Allow the OpenRouter pool for native OpenRouter requests and OpenRouter-backed custom configs."""
    ...
    # An explicit OpenRouter endpoint is safe for the OpenRouter pool even when
    # config.yaml retains provider: custom.
    effective_base = cfg_base_url or env_base_urls
    if effective_base and base_url_host_matches(effective_base, "openrouter.ai"):
        return True

    if requested_provider in {"openrouter", "auto"}:
        return not bool(env_base_urls or (cfg_base_url and _cfg_provider(model_cfg) in {"auto", "custom"}))
