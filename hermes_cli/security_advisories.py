...
class Advisory:
    ...
    ``vulnerable_below``: ``(version_source, floor_version, fixed_version)`` triples for
    range-based advisories (e.g. Node CVEs, "vulnerable from floor_version up to but not including
    fixed_version") — as opposed to ``compromised``, which is for an exact set of known-bad
    releases (e.g. a compromised PyPI package snapshot). ``version_source`` is either a PyPI
    package name (resolved the same way as ``compromised``, via ``importlib.metadata``) or the
    reserved name ``"node"`` (resolved via ``hermes_constants.find_node_executable`` + ``--version``
    — whichever Node Hermes actually uses). ``floor_version`` is required (not optional/implicit
    ``0.0.0``) so a multi-release-line advisory (e.g. fixed at 20.18.1 on the 20.x line and 22.10.0
    on the 22.x line) can be expressed as two triples without a fix on one line false-positiving
    against an unrelated, older, unaffected line.
    """
    ...
    vulnerable_below: tuple[tuple[str, str, str], ...] = ()
...
def detect_compromised(advisories: Iterable[Advisory] = ADVISORIES) -> list[AdvisoryHit]:
    ...
    for advisory in advisories:
        for source, floor_version, fixed_version in advisory.vulnerable_below:
            ...
            if _semver_tuple(floor_version) <= installed_t < _semver_tuple(fixed_version):
