...
def run_audit(
    *,
    ...
    accepted_risks: Optional[set[str]] = None,
) -> list[Finding]:
    ...
    ``accepted_risks`` is a set of advisory IDs (GHSA-xxx, CVE-yyy) to
    suppress from output and from the --fail-on gate.
    """
    ...
    for ids in raw.values():
        all_ids.extend(ids)
    # Remove accepted-risk IDs before fetching details so we don't pay for
    # advisories the user has already marked as accepted.
    if accepted_risks:
        all_ids = [vid for vid in all_ids if vid not in accepted_risks]
    details = _osv_fetch_details(all_ids)

    findings: list[Finding] = []
    for comp, ids in raw.items():
        for vid in ids:
            if accepted_risks and vid in accepted_risks:
                continue
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    # Accepted risks: CLI --accept flags win over config.
    cli_accept = set(getattr(args, "accept", []) or [])
    config_accept: set[str] = set()
    try:
        from hermes_cli.config import load_config_readonly

        raw = load_config_readonly().get("security_audit") or {}
        if isinstance(raw, dict):
            cfg_list = raw.get("accepted_risks")
            if isinstance(cfg_list, list):
                config_accept = {str(x) for x in cfg_list if x}
    except Exception:
        pass
    accepted_risks = config_accept | cli_accept
