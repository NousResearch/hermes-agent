...
class ComputerUseLaunchContext:
    ...
    # Trusted transport attestation, not guest-supplied metadata or a host socket.
    desktop_attestor: Callable[[], tuple] | None = field(default=None, repr=False)
    # CUA-only admission; never part of ordinary terminal/file lease validation.
    access_epoch: Callable[[], int] | None = field(default=None, repr=False)
    ...
    def __post_init__(self):
        ...
        if self.access_epoch is not None and not callable(self.access_epoch):
            raise TypeError("access_epoch must be callable")
        if self.desktop_attestor is not None:
            if not callable(self.desktop_attestor):
                raise TypeError("desktop_attestor must be callable")
            if not self.private_daemon or not self.desktop_only:
                raise ValueError("desktop attestation requires private desktop-only transport")
            if self.driver_command is None:
                raise ValueError("desktop attestation requires an explicit private driver")
...
def _attested_desktop_identity(attestor) -> tuple:
    try:
        identity = attestor()
    except Exception as exc:
        raise SessionExecutionError("remote desktop attestation failed") from exc

    def immutable(value):
        return type(value) in (str, bytes, int, bool) or (
            type(value) is tuple and all(immutable(part) for part in value))

    if type(identity) is not tuple or not identity or not immutable(identity):
        raise SessionExecutionError("remote desktop identity must be a nonempty immutable tuple")
    return identity
...
class SessionExecutionContext:
    ...
    # Trusted provider initialization on the routed side, AFTER host routing
    # cleanup. Guest display handles must never enter the host launch env.
    backend_shell_init: str | None = field(default=None, repr=False)
    # Explicit terminal operations fence approval waits without invalidating
    # the shared resource lease or ordinary parent execution.
    terminal_access_epoch: Callable[[], int] | None = field(default=None, repr=False)
    _desktop: tuple | None = field(default=None, init=False, repr=False)

    def __post_init__(self):
        object.__setattr__(self, "command_prefix", _command_prefix(self.command_prefix))
        if self.backend_shell_init is not None:
            if not isinstance(self.backend_shell_init, str) or "\0" in self.backend_shell_init:
                raise ValueError("backend_shell_init must be NUL-free shell text")
            if not self.command_prefix:
                raise ValueError("backend_shell_init requires a routing command_prefix")
        if self.terminal_access_epoch is not None and not callable(self.terminal_access_epoch):
            raise TypeError("terminal_access_epoch must be callable")
        ...
        if self.computer_use is not None and self.computer_use.desktop_only:
            attestor = self.computer_use.desktop_attestor
            object.__setattr__(self, "_desktop", _attested_desktop_identity(attestor) if attestor else _desktop_identity(values))
    ...
    def check(self):
        ...
        if self.computer_use is not None and self.computer_use.desktop_attestor is not None:
            if _attested_desktop_identity(self.computer_use.desktop_attestor) != self._desktop:
                raise SessionExecutionError("remote desktop endpoint was replaced")
...
class TargetSelection:
    """One non-provisioning target/authority snapshot for a pending tool call.

    Provider-backed execution requires this explicit non-provisioning selector.
    ``check`` captures intended owner/profile/config/incarnation/control and must
    be side-effect-free. ``realize(before_start=check)`` carries the caller's
    registration fence through prerequisites to allocation/restart admission.
    Callers retain ``check`` through dispatch, not on a reusable resource lease.
    """
    realize: Callable[..., "SessionExecutionLease"]
    check: Callable[[], None]
