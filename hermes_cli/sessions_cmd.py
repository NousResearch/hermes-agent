...
def _cmd_list(db, args):
    ...
    layouts = {  # (has_ws, has_titles): header, row formatter
        (True, True): (f"{'Title':<28} {'Workspace':<18} {'Last Active':<13} {'Src':<6} {'ID'}",
                       lambda s: f"{_title(s, 26):<28} {_ws(s):<18} {_ago(s):<13} {s.get('source', ''):<6} {s['id']}"),
        (True, False): (f"{'Preview':<38} {'Workspace':<18} {'Last Active':<13} {'Src':<6} {'ID'}",
                        lambda s: f"{_preview(s, 36):<38} {_ws(s):<18} {_ago(s):<13} {s['source']:<6} {s['id']}"),
        (False, True): (f"{'Title':<32} {'Preview':<40} {'Last Active':<13} {'Src':<6} {'ID'}",
                        lambda s: f"{_title(s, 30):<32} {_preview(s, 38):<40} {_ago(s):<13} {s.get('source', ''):<6} {s['id']}"),
        (False, False): (f"{'Preview':<50} {'Last Active':<13} {'Src':<6} {'ID'}",
