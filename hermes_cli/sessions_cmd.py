...
def _cmd_stamp(db, args):
    """One short per-session label stored server-side, so scripts and bots stamp a session on the
    ...
    try:
        ...
        print(f"Error: {e}")
        return 1
    stored = db.get_session_stamp(resolved_session_id)
    print(f"Session '{resolved_session_id}' stamped: {stored}" if stored
