...
def run_setup_wizard(args):
    ...
    if provider_idx == 0:  # Nous Portal
        ...
        try:
            ...
            from hermes_cli.auth import ProviderConfig, _login_nous
    ...
    if selected_backend == "local":
        ...
        default_singularity = config.get("terminal", {}).get(
        ...
        try:
            ...
            if uv_bin:
                result = subprocess.run(
                    [uv_bin, "pip", "install", "swe-rex[modal]>=1.4.0"], capture_output=True, text=True
                )
            else:
                result = subprocess.run(
                    [sys.executable, "-m", "pip", "install", "swe-rex[modal]>=1.4.0"], capture_output=True, text=True
                )
    ...
    if "browserbase" in selected_keys:
        ...
        if existing:
            ...
            if not node_modules.exists() and shutil.which("npm"):
                ...
                print_warning(
                    "    Node.js not found — browser tools require: npm install (in the hermes-agent directory)"
                )
            ...
            if tinker_existing and wandb_existing:
                ...
                try:
                    ...
                    if tinker_dir.exists() and (tinker_dir / "pyproject.toml").exists():
                        ...
                        if uv_bin:
                            result = subprocess.run(
                                [uv_bin, "pip", "install", "-e", str(tinker_dir)], capture_output=True, text=True
                            )
                        else:
                            result = subprocess.run(
