...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        for k, tdef in TOOLSETS.items():
            ...
            all_keys.add(k)
        # Disabled toolsets are subtracted at TOOL granularity, so a toolset that shares tools with
        # a kept one (e.g. ``reference`` ⊂ ``file``) must not be disabled or it would strip
        # read_file/search_files from the minimal surface.
        kept_tools = {tool for toolset in keep for tool in resolve_toolset(toolset)}
        disabled = sorted(
            toolset for toolset in all_keys - keep
