...
def _run_interactive_curator_audit(c) -> None:
    import json
    from pathlib import Path
    from rich.prompt import Confirm

    from agent.curator import run_curator_review, load_state

    c.print("\n[bold cyan]Evaluating skills for overlaps (dry run)...[/]")

    # Running the curator LLM in dry run model, to just have a look
    run_curator_review(synchronous=True, dry_run=True)

    # get the output state generated by the curator
    state = load_state()
    report_path = Path(state.get("last_report_path", "")) / "run.json"

    if not report_path.exists():
        c.print("[bold red]Failed to generate curation report![/]")
        return
    ...
    # if the results (consolidated and pruned) gave by curator llm are empty
    if not consolidated and not pruned:
        c.print("[dim]No overlapping or stale skills detected..\nEverything looks fine![/]")
        return
    
    # If there are any consolidation , give interactive terminal render to user
    if consolidated:
        table = Table(title="Proposed Consolidations (Overlapping Skills)")

        table.add_column("Skill Name", style="cyan")
        table.add_column("Merge into", style="green")
        table.add_column("Rationale", style="dim")

        for item in consolidated:
            table.add_row(item.get("name", ""), item.get("into", ""), item.get("reason", ""))

        c.print(table)

    if pruned:
        pruned_names = [p.get("name", "") if isinstance(p, dict) else str(p) for p in pruned]
        c.print(f"\n[bold yellow]Stale Skills to Prune:[/] {', '.join(pruned_names)}")

    c.print()
    if Confirm.ask("Apply these structural merges and prunes now?"):
        c.print("[bold green]Applying changes...[/]")

        # dry_run=False cuz now we want to change the files after review
        run_curator_review(synchronous=True, dry_run=False)
        c.print("[bold green]Success![/] Skill curation complete.")

    else:
        c.print("[dim]Curation cancelled. No changes made![/]")


def do_audit(name: Optional[str] = None, console: Optional[Console] = None,
             deep: bool = False, interactive: bool = False) -> None:
    ...
    # [NEW] Native security audit runs here
    if interactive:
        c.print("[bold]Running interactive curator..")
        return _run_interactive_curator_audit(c) # Custom defined helper function (see just above this function)
...
def handle_skills_slash(cmd: str, console: Optional[Console] = None) -> None:
    ...
    if action == "browse":
        ...
        do_audit(name=name, console=c, deep=deep, interactive=interactive)
