...
def get_prompt_toolkit_style_overrides() -> Dict[str, str]:
    ...
    return {
        ...
        # Clarify / approval panels prefer dedicated skin keys when present,
        # falling back to the derived values above (fully backward-compatible
        # — skins that don't set these keys behave exactly as before).
        "clarify-border": skin.get_color("clarify_border", input_rule),
        "clarify-title": f"{skin.get_color('clarify_title', title)} bold",
        "clarify-question": f"{skin.get_color('clarify_question', text)} bold",
        "clarify-choice": skin.get_color("clarify_choice", dim),
        "clarify-selected": f"{skin.get_color('clarify_selected', title)} bold",
        "clarify-active-other": f"{skin.get_color('clarify_active_other', title)} italic",
        "clarify-countdown": skin.get_color("clarify_countdown", input_rule),
        "sudo-prompt": f"{error} bold",
        "sudo-border": input_rule,
        "sudo-title": f"{error} bold",
        "sudo-text": text,
        "approval-border": skin.get_color("approval_border", input_rule),
        "approval-title": f"{skin.get_color('approval_title', warn)} bold",
        "approval-desc": f"{skin.get_color('approval_desc', text)} bold",
        "approval-cmd": f"{skin.get_color('approval_cmd', dim)} italic",
        "approval-choice": skin.get_color("approval_choice", dim),
        "approval-selected": f"{skin.get_color('approval_selected', title)} bold",
