...
def _build_full_manifest(
    ...
    it stays in sync with the rest of IX Agency. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a IX Agency deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with IX Agency in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your IX Agency agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "IX Agency")
    ...
    """
    name = getattr(args, "name", None) or "IX Agency"
    description = getattr(args, "description", None) or "Your IX Agency agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your IX Agency app\n"
