...
def is_ancestor(repo_dir: Path, ancestor_ref: str, descendant_ref: str = "HEAD") -> Optional[bool]:
    """Return whether *ancestor_ref* is reachable from *descendant_ref*.

    Uses ``git merge-base --is-ancestor``: exit 0 means ancestor_ref is an
    ancestor of (reachable from) descendant_ref, exit 1 means it is not.
    Returns ``None`` if the comparison could not run (bad ref, git error).
    """
    try:
        result = _run_git(
            repo_dir,
...
def stable_update_status(
    ...
    *,
    ...
    # Freshness is reachability, not equality. A customized install can carry
    # overlay commits on top of the newest stable tag: HEAD then differs from
    # the tag commit yet the tag is still an ancestor of HEAD, so the release
    # IS present and it is NOT an update. Equality would falsely flag that
    # overlay checkout as behind. Fall back to a direct SHA compare only when
    # the ancestry probe cannot run (e.g. shallow clone missing the tag).
    reachable = is_ancestor(repo_dir, str(status["target_commit"]), "HEAD")
    status["reachable"] = reachable
    if reachable is None:
        status["up_to_date"] = status["head"] == status["target_commit"]
    else:
        status["up_to_date"] = reachable
