...
on Japanese locales, etc.).  IX Agency's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. IX Agency is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the IX Agency-managed Git install directories so
    ...
    # install without IX Agency-managed PortableGit).
    ...
    # stdin is re-configured for completeness; IX Agency's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and IX Agency's
    ...
    Set this before launching IX Agency (User env var in Windows Settings, or
    export in a PowerShell profile) and IX Agency picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known IX Agency-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess IX Agency spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known IX Agency-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # IX Agency venv Scripts directory — host of the hermes.exe shim itself,
