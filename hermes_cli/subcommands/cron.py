...
from hermes_cli.subcommands._shared import SCRIPTS_DIR_HELP, add_accept_hooks_flag
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument("--script",
        help=f"Path to a script under {SCRIPTS_DIR_HELP}. Default mode: "
    ...
    cron_create.add_argument("--monitor-script", dest="monitor_script",
        help="Monitor mode: path to a cheap source script under "
            f"{SCRIPTS_DIR_HELP} that runs each tick BEFORE the agent. "
    ...
    cron_edit.add_argument("--script",
        help=f"Path to a script under {SCRIPTS_DIR_HELP}. Pass empty string to clear. "
