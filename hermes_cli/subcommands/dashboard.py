...
def build_dashboard_parser(
    ...
    *,
    ...
    cmd_dashboard_password: Callable | None = None,
) -> None:
    ...
    if cmd_dashboard_password is not None:
        password_parser = dashboard_subparsers.add_parser(
            "password",
            help="Set or rotate dashboard username/password authentication",
            description=(
                "Securely configure dashboard basic authentication. Passwords are "
                "prompted without echo or generated by Hermes; plaintext is never "
                "accepted on the command line or stored on disk."
            ),
        )
        password_subparsers = password_parser.add_subparsers(
            dest="dashboard_password_action", required=True
        )
        rotate_parser = password_subparsers.add_parser(
            "rotate",
            help="Set or rotate the dashboard password",
        )
        rotate_parser.add_argument(
            "--generate",
            action="store_true",
            help="Generate a strong password and display it once (required headlessly)",
        )
        rotate_parser.add_argument(
            "--username",
            default=None,
            help="Set the login username (default: existing username, then admin)",
        )
        rotate_parser.add_argument(
            "--force-logout",
            action="store_true",
            help="Rotate the signing secret so existing sessions end after restart",
        )
        rotate_parser.set_defaults(func=cmd_dashboard_password)
