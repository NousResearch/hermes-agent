...
TIPS = [
    ...
    "delegate_task can't spawn Claude Code — ACP delegation requires a CLI that advertises `--acp`, which the `claude` CLI doesn't; shell out with `claude -p` via the terminal tool instead.",
