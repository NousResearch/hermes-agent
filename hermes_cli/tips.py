...
TIPS = [
    ...
    "Ctrl+Z suspends IX Agency to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs IX Agency itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire IX Agency home directory.",
    ...
    "hermes gateway install sets up IX Agency as a system service (systemd/launchd).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call IX Agency tools programmatically — results stay out of context.",
    ...
    "When exiting, IX Agency prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "IX Agency runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to IX Agency via a local macOS server.",
    ...
    "IX Agency loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs IX Agency as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make IX Agency your own.",
    ...
    "Before a gateway session reset, IX Agency auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope IX Agency state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — IX Agency auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of IX Agency config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while IX Agency is working.',
    ...
    'hermes dashboard embeds the full IX Agency TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes IX Agency block commands when the tirith scanner itself errors out.',
