...
def _plugin_stt_providers() -> list[dict]:
    ...
    Mirrors ``_plugin_tts_providers()`` for the transcription side.
    Built-in STT rows stay hardcoded in ``TOOL_CATEGORIES["stt"]``;
    this function only injects PLUGIN-registered providers (e.g. Bifrost).

    Defensive: plugins whose name collides with a built-in STT provider
    are filtered out — the registry already rejects them at registration
    time, but filtering here keeps the picker invariant.
    """
    try:
...
def _visible_providers(
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    # Inject plugin-registered STT backends (mirror of TTS). Plugin rows
    # render BELOW the hardcoded built-in rows. Built-in shadowing
    # is filtered out by ``_plugin_stt_providers`` defensively.
