...
def _update_check_fetch_popen_kwargs() -> dict:
    """Isolate a git fetch so timeout cleanup can terminate its transport children."""
...
def _run_update_check_fetch(
    ...
    *,
    ...
    """Run one bounded update fetch and remove debris left by that failed transfer."""
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if fetch_result is not None and fetch_result.returncode == 0:
        ...
        fetch_result = _run_update_check_fetch(
            git_cmd, depth_args, "origin", branch, _m().PROJECT_ROOT)
