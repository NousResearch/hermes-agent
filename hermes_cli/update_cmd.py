...
from hermes_cli.update_cmd_windows import (  # noqa: F401
    ...
    _resume_windows_gateways_and_merge_outcome, _resume_windows_update_runtimes,
...
def _apply_pulled_update(
    ...
    if is_fork and branch == "main":
        # Upstream sync can replace the tree a second time. Finish ALL code swaps
        # before the handoff, so the dependency/restart tail never mixes imports.
...
def _hand_off_post_swap(args, **payload_kwargs) -> None:
    ...
    if code is None:
        ...
        if payload_kwargs.get("gateway_mode"):
            _write_gateway_update_exit_code(False)
        # Also covers a parent already carrying the hard-exit marker: atexit is
        # only a fallback, not the owner of a refused handoff's recovery.
        _resume_windows_update_runtimes(payload_kwargs.get("_windows_gateway_resume"))
        code = 1
    sys.exit(code)
...
def _execute_post_swap(payload: dict, args, gateway_mode: bool) -> None:
    ...
    try:
        # Resolve config only after adopting the pause token: config failures also
        # have to resume the gateways stopped by the pre-swap process.
        opts = _replace(
        ...
        # cmd_update hard-exits handoff children after SystemExit, bypassing
        # atexit. Settle the idempotent pause token before its receipt is finalized.
        _resume_windows_update_runtimes(_windows_gateway_resume)
...
def _finish_pulled_update(
    ...
    # Manual backends can hold .pyd files too: resume only after installs and
    # migrations, but before fleet verification and receipt finalization.
    if _windows_gateway_resume:
    ...
    if opts.no_gateway_restart:
        ...
        # kept for the next normal update. This updater already runs on the pulled
        # tree; the gateways keep their loaded code until they are restarted.
        # Windows pause/resume still runs for gateways stopped before the swap;
        # only the fleet restart + verification are deferred.
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        if _windows_gateway_resume is None:
            # A serve-only install still needs a serializable recovery envelope.
