...
def _reload_config_modules() -> None:
    ...
    pulled migration. ``_subprocess_compat`` / ``main_dashboard`` / ``dashboard_procs`` reload too so the
    later dashboard cleanup sees symbols the update added (the kill path resolves ``main_dashboard``
    lazily at call time). ``tools_config`` reloads as well: migrations import its
    ...
    for mod_name in (
        ...
        "hermes_cli.config_migrations", "hermes_cli._subprocess_compat", "hermes_cli.main_dashboard",
