...
def _resume_windows_update_runtimes(token: dict | None) -> None:
    """Settle both recovery obligations before a handoff process can hard-exit."""
    from hermes_cli.update_cmd import _m
    try:
        ...
        if token:
            _m()._relaunch_stopped_serves(token.get("stopped_serves") or {})
...
def _clear_windows_venv_holders_or_exit(args, gateway_mode: bool, _windows_gateway_resume):
    ...
    if holders and (serve_entries := _m()._ledger_manual_serve_holders(holders)):
        def _stop_and_park(pids):
            ...
            serve_resume = {"pending": True, "entries": serve_entries}
            if _windows_gateway_resume is not None:
                # The same object is held by atexit and serialized for the child.
                # Successful handoff disarms only the parent's copy.
                _windows_gateway_resume["stopped_serves"] = serve_resume
            import atexit as _serve_atexit
            _serve_atexit.register(_m()._relaunch_stopped_serves, serve_resume)
