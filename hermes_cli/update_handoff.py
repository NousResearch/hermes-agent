...
def _release_parent_recovery(payload: dict[str, Any]) -> None:
    """The persisted child copy owns recovery once Popen succeeds, before any wait/output."""
    token = payload.get("windows_gateway_resume")
    if token:
        token["resume_needed"] = False
        serve_resume = token.get("stopped_serves")
        if serve_resume:
            serve_resume["pending"] = False
...
def continue_update_in_fresh_interpreter(payload: dict[str, Any], *, argv_tail: list[str]) -> int | None:
    ...
    try:
        ...
        # No child exists: keep every parent recovery hook armed and let the
        # caller reattach the receipt and write the interrupted-install marker.
        logger.warning("Could not prepare post-swap hand-off: %s", exc)
        print(f"  ⚠ Could not prepare the post-update interpreter: {exc}")
        return None
    if _running_from_windows_shim():
        try:
            ...
            return None
        _release_parent_recovery(payload)
