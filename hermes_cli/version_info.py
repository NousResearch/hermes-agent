...
def _run_git(repo_dir: Path, *args: str) -> str | None:
    try:
        result = subprocess.run(
            ...
            # Git emits UTF-8; without this the reader thread decodes with the
            # Windows ANSI code page and a non-UTF-8 locale (e.g. cp936) crashes
            # with UnicodeDecodeError before update even starts (#122239).
...
def _git_version_info(repo_dir: Path, *, include_untracked: bool = False) -> VersionInfo:
    ...
    try:
        ...
        dirty_result = subprocess.run(
            ...
            timeout=3,
            cwd=str(repo_dir),
            encoding="utf-8", errors="replace",  # same cp936 crash class as _run_git (#122239)
        )
