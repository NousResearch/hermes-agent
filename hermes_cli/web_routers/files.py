...
async def fs_read_data_url(
    ...
    relative_to_file: Optional[str] = None, max_bytes: Optional[int] = None
):
    from hermes_cli.web_server import _FS_DATA_URL_MAX_BYTES
    target, target_stat = _fs_regular_file(await _fs_download_path(path, profile, session_id))
    root = None
    source = None
    source_stat = None
    if relative_to_file:
        source, source_stat = _fs_regular_file(await _fs_download_path(relative_to_file, profile, session_id))
        root = source.parent
