...
async def test_mcp_server(
    ...
    # tại. Nếu chỉ key theo config server, lần test đầu (chưa có token) bị
    # cache là lỗi "cần xác thực" — sau khi user hoàn tất OAuth login, request
    # tiếp theo trong TTL vẫn nhận lại y nguyên lỗi cũ dù token đã tồn tại.
    # Thêm hậu tố oauth={True/False} khiến 2 trạng thái dùng 2 cache key khác nhau.
        ...
        "tools": [
            {
                "name": t,
                "description": d,
                # Additive-optional, same convention as schema_chars: only present when
                # the server actually set one, so a probe without titles is byte-identical
                # to the pre-title output — old renderers see nothing new.
