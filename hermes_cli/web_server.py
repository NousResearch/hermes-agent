...
app = FastAPI(title="IX Agency", version=__version__, lifespan=_lifespan)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates IX Agency (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real IX Agency provider slug — fine.
    ...
    1. Vendor-name → IX Agency-provider mapping: when the provider string is
       not a known IX Agency provider/alias (e.g. ``moonshotai``, ``x-ai`` is
...
def _count_status_active_sessions() -> int:
    ...
    while another IX Agency process is writing to it.
    """
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical IX Agency credential-store basenames (see
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the hermes-webui image sharing the IX Agency source tree),
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's IX Agency root is actually /opt/data
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — IX Agency Cloud
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "IX Agency updates are managed outside this dashboard in "
...
async def check_hermes_update(force: bool = False):
    """Report whether a IX Agency update is available, without applying it.
    ...
    Returns:
        ...
        current_version: installed IX Agency version string
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another IX Agency surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    IX Agency supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run IX Agency from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect IX Agency to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use IX Agency from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect IX Agency to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use IX Agency in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use IX Agency through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from IX Agency via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to IX Agency through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect IX Agency to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use IX Agency inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect IX Agency to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use IX Agency through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect IX Agency to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect IX Agency to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose IX Agency as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "IX Agency").strip() or "IX Agency"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, IX Agency appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — IX Agency-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"IX Agency PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into IX Agency even
    when they also have a separate IX Agency-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    IX Agency never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside IX Agency, so the disconnect
    ...
    user sees exactly what executes, and IX Agency then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside IX Agency — run the disconnect command to remove it."
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the IX Agency-managed PKCE file and auth-store entry.
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both IX Agency file AND credential pool.
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not IX Agency,
...
# ---------------------------------------------------------------------------
# /api/console — safe IX Agency Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full IX Agency CLI
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. IX Agency Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall IX Agency inside WSL2 to use the dashboard's /chat "
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the IX Agency backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "IX Agency Teal",         "description": "Classic dark teal — the canonical IX Agency look"},
    {"name": "default-large", "label": "IX Agency Teal (Large)", "description": "IX Agency Teal with bigger fonts and roomier spacing"},
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with IX Agency exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  IX Agency backend listening on {host}:{actual_port}")
            else:
                print(f"  IX Agency Web UI → http://{host}:{actual_port}")
