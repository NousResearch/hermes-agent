...
def _classify_command_center_process(command: str) -> Optional[Dict[str, str]]:
    """Map local processes to Command Center rows.

    This endpoint is intentionally read-only and returns a short, sanitized
    command preview. It is for the local/gated dashboard's operating overview,
    not for remote process management.
    """
    lower = command.lower()
    if "crashpad_handler" in lower or "helper.app" in lower:
        return None
    specs = [
        ("hermes_gateway", "Gateway", "Slack / API / Gateway の待受", "read_only", ["gateway run"]),
        ("hermes_dashboard", "Dashboard", "Command Center 表示基盤", "read_only", ["hermes dashboard", "web_server"]),
        ("hermes_cli", "Hermes CLI", "Rinoとの主会話 / 作業セッション", "read_only", ["/bin/hermes", " venv/bin/hermes", "hermes chat"]),
        ("cron_worker", "Cron worker", "スケジュール実行中の自動化", "automation", ["cron tick", "cron/scheduler", "ops_dashboard_snapshot"]),
        ("codex", "Codex worker", "外部 coding agent / MCP worker", "read_only", ["codex", "codex.app"]),
        ("claude", "Claude worker", "外部 coding agent", "read_only", ["claude --", "claude-code", "claude code"]),
        ("screenpipe", "Screenpipe", "画面ログ / 録画パイプライン", "local_capture", ["screenpipe"]),
        ("lp_studio", "LP Studio", "LP編集プレビューサーバー", "local_server", ["studio-server"]),
    ]
    for kind, label, mission, risk, needles in specs:
        if any(needle in lower for needle in needles):
            return {"kind": kind, "label": label, "mission": mission, "risk": risk}
    return None


@app.get("/api/command-center/processes")
async def get_command_center_processes():
    """Return agent-like local processes for the LP-style Command Center.

    The response deliberately avoids env vars and only includes a truncated
    command line. If ps is unavailable, degrade to an empty list rather than
    ...
    for line in output.splitlines():
        ...
        if len(parts) < 4:
            continue
        pid_s, stat_s, etime_s, command = parts
        meta = _classify_command_center_process(command)
        if not meta:
            continue
        try:
            pid = int(pid_s)
        ...
        if key in seen:
            continue
        seen.add(key)
        preview = re.sub(r"\s+", " ", command).strip()
        if len(preview) > 180:
            preview = preview[:177] + "..."
        rows.append({
            "pid": pid,
    ...
    rows.sort(key=lambda row: (priority.get(str(row.get("kind")), 99), row.get("pid", 0)))
    return {"processes": rows[:80]}
...
async def get_command_center_snapshot():
    """Return the local Command Center snapshot generated by scripts/command_center_snapshot.py.

    Read-only endpoint. It does not run the generator, spawn agents, or inspect
    secrets; it only serves the last local JSON artifact so the dashboard can
    show mission/current_step/artifact data without inventing state.
    """
    path = get_hermes_home() / "state" / "command_center_snapshot.json"
    if not path.exists():
        return {
            "exists": False,
            "path": str(path),
            "message": "Snapshot not generated yet. Run ~/.hermes/scripts/command_center_snapshot.sh or command_center_snapshot.py.",
        }
    try:
    ...
    if isinstance(data, dict):
        ...
        return data
    return {"exists": True, "path": str(path), "error": "snapshot root is not an object"}
