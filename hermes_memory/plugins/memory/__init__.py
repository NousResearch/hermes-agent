...
1. Bundled providers: ``hermes_memory/plugins/memory/<name>/`` (shipped with hermes-agent)
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 1. Bundled providers (hermes_memory/plugins/memory/<name>/)
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    Checks both bundled (``hermes_memory/plugins/memory/<name>/``) and user-installed
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = (
        f"hermes_memory.plugins.memory.{name}" if _is_bundled else f"_hermes_user_memory.{name}"
    )
    ...
    if module_name in sys.modules:
        ...
        # Handle relative imports within the plugin — register parent packages
        _mem_pkg = Path(__file__).resolve().parent
        _plugins_pkg = _mem_pkg.parent
        _hm_pkg = _plugins_pkg.parent
        for parent, parent_path in (
            ("hermes_memory", _hm_pkg),
            ("hermes_memory.plugins", _plugins_pkg),
            ("hermes_memory.plugins.memory", _mem_pkg),
        ):
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = (
        f"hermes_memory.plugins.memory.{active_provider}.cli"
        if _is_bundled
