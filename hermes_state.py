...
class SessionDB:
    ...
    def prune_sessions(
        ...
        def _do(conn):
            ...
            sids_tuples = [(sid,) for sid in session_ids]
            conn.executemany("DELETE FROM messages WHERE session_id = ?", sids_tuples)
            conn.executemany("DELETE FROM sessions WHERE id = ?", sids_tuples)
