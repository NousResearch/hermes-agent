    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 21:
                # v21: switch from inline FTS5 to external-content FTS5.
                ...
                if fts5_available:
                    ...
                    fts_schema = {
                        ...
                        (row["sql"] if isinstance(row, sqlite3.Row) else row[1])
                        for row in schema_rows
                    }

                    def _uses_v21_external_schema(sql: str | None) -> bool:
                    ...
                    base_is_current = _uses_v21_external_schema(
                        fts_schema.get("messages_fts")
                    )
                    trigram_sql = fts_schema.get("messages_fts_trigram")
                    trigram_is_current_or_absent = (
                        trigram_sql is None
                        or _uses_v21_external_schema(trigram_sql)
                    )

                    if not (base_is_current and trigram_is_current_or_absent):
                        self._drop_fts_triggers(cursor)
                        for _tbl, _drop_sql in (
                            ("messages_fts", "DROP TABLE IF EXISTS messages_fts"),
                            (
                            ...
                            try:
                                ...
                                if self._is_trigram_unavailable_error(exc):
                                    self._warn_trigram_unavailable(exc)
                                    # An old inline trigram table remains. Keep
                                    # the previous schema version so a runtime
                                    # with trigram support retries the migration.
                                    fts_migrations_complete = False
                                else:
                                    self._warn_fts5_unavailable(exc)
                                    fts5_available = False
