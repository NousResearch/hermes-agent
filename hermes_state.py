...
class SessionDB:
    ...
    def clear_session_workspace(self, session_id: str) -> None:
        """Atomically clear all fields that identify a session workspace."""
        if not session_id:
            return
    ...
    def set_session_workspace(self, session_id: str, cwd: str) -> None:
        """Atomically replace a session workspace and discard stale git metadata."""
        if not session_id or not cwd:
            return
