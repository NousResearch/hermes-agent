...
class SessionCompressionMixin:
    ...
    def find_live_compression_child(self, parent_session_id: str) -> Optional[Dict[str, Any]]:
        """The unique live leaf of a compression continuation chain, else None. A stale agent
        whose parent was rotated elsewhere (possibly more than once) may recover only when
        every hop names exactly one canonical continuation and the chain ends in a live leaf.

        Every hop is resolved from one SQLite read snapshot. Malformed child metadata,
        invalid lifecycle states, canonical forks (live or closed siblings), live
        intermediates with descendants, and cycles fail closed rather than selecting a
        transcript heuristically."""
        ...
        with self._read_ctx() as conn:
            ...
            def _canonical_children(session_id: str):
                ...
                for row in rows:
                    ...
                    # Markers only disqualify a child when bound to the queried parent
                    # (continuations inherit model_config verbatim); any other marker
                    # shape is malformed lineage metadata and fails closed. A reset fork
                    # is its own conversation, never the continuation (#114271).
                    markers = [
                        marker
            ...
            # Explicit deferred transaction: both the pooled read-only connection and the
            # writer run with isolation_level=None, so this pins one WAL snapshot for the
            # whole walk (a concurrent rotation cannot be half-observed).
            conn.execute("BEGIN")
            try:
                ...
                while current_session_id not in seen:
                    ...
                    if not _ended_by_compression(
                        conn.execute(_ENDED_ROW_SQL, (current_session_id,)).fetchone()
                    ):
                        return None
