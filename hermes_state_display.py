...
class SessionDisplayMixin:
        ...
        for _ in range(100):
            ...
            row = conn.execute(
                ...
                SELECT child.id, child.parent_session_id, parent.end_reason,
                       {_sql_json_extract("child.model_config", "$._branched_from")} AS branched_from,
                       {_sql_json_extract("child.model_config", "$._delegate_from")} AS delegated_from,
                       child.source
                FROM sessions child
    ...
    def _remove_proven_message_clone_sources(self, conn, rows, selected_session_ids) -> list:
        ...
        source_by_clone_id: dict[int, int] = {}
        ...
        for start in range(0, len(ids), chunk_size):
            ...
            mappings = conn.execute(
                "SELECT lineage.source_message_id, lineage.clone_message_id "
            ...
            for mapping in mappings:
                source_id = int(mapping["source_message_id"])
                clone_id = int(mapping["clone_message_id"])
                hidden_source_ids.add(source_id)
                if clone_id in visible_ids:
                    source_by_clone_id[clone_id] = source_id

        input_position = {int(row["id"]): index for index, row in enumerate(rows)}
        logical_position: dict[int, int] = {}

        def _logical_position(message_id: int) -> int:
            path: list[int] = []
            seen: set[int] = set()
            current_id = message_id
            while True:
                if current_id in logical_position:
                    position = logical_position[current_id]
                    break
                if current_id in seen:
                    position = min(input_position[path_id] for path_id in seen)
                    break
                seen.add(current_id)
                path.append(current_id)
                source_id = source_by_clone_id.get(current_id)
                if source_id is None or source_id not in input_position:
                    position = input_position[current_id]
                    break
                current_id = source_id
            for path_id in path:
                logical_position[path_id] = position
            return position

        retained_rows = [row for row in rows if int(row["id"]) not in hidden_source_ids]
        return sorted(
            retained_rows,
            key=lambda row: (
                _logical_position(int(row["id"])),
                input_position[int(row["id"])],
            ),
        )
    ...
    def _get_display_lineage_message_rows_from_conn(
        ...
        if include_compacted and limit is not None and self._indexed_display_page_ready(conn, session_ids):
            if self._selected_lineage_has_clone_provenance(conn, session_ids):
                return self._get_indexed_display_lineage_rows_from_conn(
                    conn,
                    session_ids,
                    limit=max(0, limit),
                    offset=max(0, offset),
            ...
            ordered_session_ids = list(reversed(session_ids)) if latest else session_ids
            for session_id in ordered_session_ids:
                if remaining_limit <= 0:
                    break
                segment_offset = 0
                if remaining_offset:
                    segment_count = self._count_indexed_display_segment_rows_from_conn(
                        conn, session_id, selected_session_ids=session_ids
                    )
                    if remaining_offset >= segment_count:
                        remaining_offset -= segment_count
                        continue
                    segment_offset = remaining_offset
                    remaining_offset = 0
                segment = self._get_indexed_display_segment_rows_from_conn(
                    conn,
                    session_id,
    ...
    def _indexed_display_page_ready(self, conn, session_ids: list[str]) -> bool:
        """Whether exact provenance plus durable order can page without a legacy full scan."""
        if not session_ids:
            return False
        boundary_row = conn.execute(
            "SELECT value FROM state_meta WHERE key = ? LIMIT 1",
        ...
        if legacy_ceiling > 0:
            legacy_row = conn.execute(
                ...
                "AND (active = 1 OR compacted = 1) AND id <= ? LIMIT 1",
                [*session_ids, legacy_ceiling],
            ).fetchone()
            if legacy_row is not None:
                return False
        missing = conn.execute(
            f"SELECT 1 FROM messages WHERE session_id IN ({placeholders}) "
            "AND (active = 1 OR compacted = 1) AND display_order IS NULL LIMIT 1",
            session_ids,
        ).fetchone()
        return missing is None

    @staticmethod
    def _selected_lineage_has_clone_provenance(conn, session_ids: list[str]) -> bool:
        placeholders = ",".join("?" for _ in session_ids)
        return conn.execute(
            "SELECT 1 FROM message_clone_lineage AS lineage "
            "CROSS JOIN messages AS clone ON clone.id = lineage.clone_message_id "
            f"WHERE clone.session_id IN ({placeholders}) LIMIT 1",
            session_ids,
        ).fetchone() is not None

    @staticmethod
    def _get_indexed_display_lineage_rows_from_conn(
        conn,
        ...
        *,
        ...
        """Page a provenance-bearing lineage by the clones' source-derived global order."""
        ...
        for session_id in session_ids:
            segment_queries.append(
                "SELECT * FROM ("
                "  SELECT candidate.* FROM messages AS candidate "
                "  WHERE candidate.session_id = ? "
                "    AND (candidate.active = 1 OR candidate.compacted = 1) "
                "    AND candidate.display_order IS NOT NULL "
                "    AND NOT EXISTS ("
                "      SELECT 1 FROM message_clone_lineage AS lineage "
                "      JOIN messages AS clone ON clone.id = lineage.clone_message_id "
                "      WHERE lineage.source_message_id = candidate.id "
                "        AND clone.session_id IN (SELECT session_id FROM selected)"
                "    ) "
                f"  ORDER BY candidate.display_order {direction}, candidate.id {direction} LIMIT ?"
                ")"
            )
            segment_params.extend((session_id, window))
        rows = conn.execute(
            f"WITH selected(session_id) AS (VALUES {selected_values}), "
            f"candidates AS ({' UNION ALL '.join(segment_queries)}) "
            "SELECT * FROM candidates "
            f"ORDER BY display_order {direction}, id {direction} LIMIT ? OFFSET ?",
            [*session_ids, *segment_params, limit, offset],
        ).fetchall()
        if latest:
            rows.reverse()
        return rows
    ...
    @staticmethod
    def _count_indexed_display_segment_rows_from_conn(
        conn, session_id: str, *, selected_session_ids: list[str]
    ) -> int:
        """Count one exact-provenance segment so offset can be consumed without materializing rows."""
        placeholders = ",".join("?" for _ in selected_session_ids)
        row = conn.execute(
            "SELECT COUNT(*) FROM messages AS candidate "
            "WHERE candidate.session_id = ? AND (candidate.active = 1 OR candidate.compacted = 1) "
            "AND candidate.display_order IS NOT NULL "
            "AND NOT EXISTS ("
            "  SELECT 1 FROM message_clone_lineage AS lineage "
            "  JOIN messages AS clone ON clone.id = lineage.clone_message_id "
            "  WHERE lineage.source_message_id = candidate.id "
            f"    AND clone.session_id IN ({placeholders})"
            ")",
    ...
    @staticmethod
    def _get_indexed_display_segment_rows_from_conn(
        conn,
        session_id: str,
        *,
        ...
        """Read one bounded logical segment, excluding sources cloned onto the selected lineage."""
        direction = "DESC" if latest else "ASC"
        placeholders = ",".join("?" for _ in selected_session_ids)
        rows = conn.execute(
            "SELECT candidate.* FROM messages AS candidate "
            ...
            f"    AND clone.session_id IN ({placeholders})"
            ") "
            f"ORDER BY candidate.display_order {direction}, candidate.id {direction} LIMIT ? OFFSET ?",
            [session_id, *selected_session_ids, limit, offset],
        ).fetchall()
        if latest:
            rows.reverse()
        return rows
    ...
    def _decode_message_rows(self, rows) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            msg = dict(row)
            # These are internal paging/index columns, not part of the public
            # message shape returned by SessionDB.get_messages().
        ...
        with self._read_transaction_ctx() as conn:
            ...
            if not unchanged:
                rows = self._get_display_lineage_message_rows_from_conn(
                    conn, lineage_ids if include_compacted else [tip_id], include_compacted=include_compacted,
                    limit=limit, offset=offset, latest=latest,
                )
