...
class SessionDBProvider(ABC):
    ...
    @abstractmethod
    def append_messages_batch(self, session_id: str, messages: List[Dict[str, Any]],
                              **kwargs: Any) -> int:
        """Append messages atomically; returns the inserted count."""

    @abstractmethod
    def replace_messages(self, session_id: str, messages: List[Dict[str, Any]],
                         active_only: bool = False, **kwargs: Any) -> None:
    ...
    @abstractmethod
    def list_sessions_rich(self, source: str = None, limit: int = 20, offset: int = 0,
                           **kwargs: Any) -> List[Dict[str, Any]]:
        """Rich session listing for the dashboard/TUI."""

    @abstractmethod
    def session_count(self, source: str = None, **kwargs: Any) -> int:
    ...
    @abstractmethod
    def set_meta(self, key: str, value: str, **kwargs: Any) -> None:
        """Upsert a state_meta value."""
    ...
    def add_pre_commit_hook(self, hook: Callable[[Any], None]) -> None:
        """Register ``hook(conn)`` to run INSIDE the write transaction after the mutation
