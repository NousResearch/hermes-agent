...
# parent, or an explicit branch / delegate / reset / tool sub-session that
# happened to be born under the same parent_id?" The predicate intentionally
# excludes ``child.started_at >= parent.ended_at`` — the timestamp ordering
# upstream explicitly abandoned in ``get_compression_tip()``'s docstring because
# gateway + compression races can insert the real continuation row *before* the
...
# branch/delegate/reset/tool markers on the child), not by timing.
#
# Markers are BOUND to ``child.parent_session_id``, never matched by presence: a
# continuation inherits its parent's ``model_config`` verbatim, so a leftover
# marker pointing at some EARLIER parent would presence-match it as a fork and
# silently drop the real continuation from the kill set. Same binding and the
# same marker set as ``_NON_CONTINUATION_CHILD_FILTER_SQL`` in this module and
# ``_is_compression_child_row``'s ``include_reset=True`` walk. Reset forks count
# here because a reset starts a separate user-visible conversation even though
# its row keeps ``parent_session_id`` for lineage (#114271) — deleting a
# compression chain must not take one with it.
#
...
_COMPRESSION_CONTINUATION_PREDICATE = (
    ...
    f"AND COALESCE({_sql_json_extract('child.model_config', '$._branched_from')}, '') != child.parent_session_id "
    f"AND COALESCE({_sql_json_extract('child.model_config', '$._delegate_from')}, '') != child.parent_session_id "
    f"AND COALESCE({_sql_json_extract('child.model_config', '$._reset_from')}, '') != child.parent_session_id "
...
""" + _UPSERT_KEEP_EXISTING_SQL,
    ...
    def delete_session(
        ...
        tip. Explicit branch / reset / tool-source children are orphaned (``parent_session_id → NULL``) so
        they remain accessible independently — a reset fork is its own conversation even when its
        parent ended in compression (#114271).
        """
