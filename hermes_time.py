...
# ``safe_strftime``/``_repair_surrogates`` live in their own stdlib-only leaf module. A process
# that crosses an on-disk upgrade keeps THIS module cached from the pre-upgrade revision, so a
# consumer imported for the first time afterwards must not need a name from here that the cached
# copy lacks. Re-exported for compatibility with existing ``from hermes_time import
# safe_strftime`` callers.
from hermes_time_format import _repair_surrogates, safe_strftime  # noqa: F401
