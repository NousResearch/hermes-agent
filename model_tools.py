...
_VAULT_NO_PASSWORD_NOTE = (" Vault note: on a login/checkout form call browser_vault_list first, then browser_vault_fill, or "
                           "browser_vault_save_login when nothing is saved for the site. For a non-login exported-file password "
                           "call browser_vault_fill_export_password (the user is asked in their UI). For a one-time / 2FA code "
                           "call browser_vault_enter_code. Never type a password, card number, CVC or "
