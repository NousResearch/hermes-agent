...
def handle_function_call(
    ...
    try:
        ...
        # Keep a single large result from consuming the next model request: preserve the full
        # payload as a local artifact and return only a structured preview/reference.
        try:
            from tools.tool_result_artifacts import externalize_large_tool_result
            result = externalize_large_tool_result(
                tool_name=function_name, result=result, session_id=ids.session_id or "",
                task_id=ids.task_id or "", tool_call_id=ids.tool_call_id or "")
