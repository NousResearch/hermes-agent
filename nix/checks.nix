...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        # Verify HERMES_NODE is set in wrapper and points to Node 24+
        # (Hermes pins its production toolchain to Node 24)
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          ...
          test "$NODE_MAJOR" -ge 24 || \
            (echo "FAIL: Node v$NODE_MAJOR < 24, Hermes requires Node 24"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 24"
