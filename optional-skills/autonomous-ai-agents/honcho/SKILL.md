---
name: honcho
description: Configure and use Honcho memory with Jade -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Hermes Agent
license: MIT
...
# Honcho Memory for Jade

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Jade profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this Jade profile's AI peer
...
Honcho models conversations as interactions between **peers**. Jade creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Jade instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Jade run |
...
Each Jade profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Jade when Honcho memory is active.
