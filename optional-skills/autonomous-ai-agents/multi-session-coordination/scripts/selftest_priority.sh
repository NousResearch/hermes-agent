...
OUT=$(co register --task "task B" 2>&1); log "$OUT"; B=$(first_line "$OUT"); B8=$(printf '%.8s' "$B")
OUT=$(co register --task "task C" 2>&1); log "$OUT"; C=$(first_line "$OUT"); C8=$(printf '%.8s' "$C")
...
OUT=$(co register --task "task E" 2>&1); log "$OUT"; E=$(first_line "$OUT"); E8=$(printf '%.8s' "$E")
...
OUT=$(co register --task "task F" 2>&1); log "$OUT"; F=$(first_line "$OUT"); F8=$(printf '%.8s' "$F")
