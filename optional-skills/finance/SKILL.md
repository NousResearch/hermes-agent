...
metadata:
  hermes:
    tags: [stocks, finance, market, trading, crypto, yahoo-finance, portfolio, investing]
    category: finance
...
No API key needed. Optional Alpha Vantage key (ALPHA_VANTAGE_KEY env var) for enriched data.
...
- User wants 52-week highs/lows or volume data
...
```bash
python3 $SCRIPT quote AAPL
python3 $SCRIPT quote AAPL TSLA NVDA MSFT
```

### search QUERY
Find stocks by company name or partial symbol.
```bash
python3 $SCRIPT search "Tesla"
python3 $SCRIPT search "semiconductor"
```
Returns top 5 matches: symbol, name, exchange, type.

### history SYMBOL [--range RANGE]
Price history. Ranges: 1mo, 3mo, 6mo, 1y, 5y (default: 1mo).
```bash
python3 $SCRIPT history AAPL --range 1y
```
Output: list of {date, open, close, high, low, volume} + stats (min, max, avg, total_return_pct).

### compare SYMBOL1 SYMBOL2 [...]
Side-by-side comparison: price, change%, 52w performance.
```bash
python3 $SCRIPT compare AAPL MSFT GOOGL AMZN
```

### crypto SYMBOL [SYMBOL2...]
Crypto prices via Yahoo Finance (BTC-USD format handled automatically).
```bash
python3 $SCRIPT crypto BTC
python3 $SCRIPT crypto ETH SOL XRP BNB
```
...
- market_cap and pe_ratio require valid session crumb — may return null.
- Crypto symbols: pass BTC not BTC-USD (tool appends -USD automatically).
...
# Should return current Apple stock price
```
