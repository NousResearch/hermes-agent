...
This is an optional third-party workflow, not a Jade core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Jade native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Jade native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Jade native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Jade usage patterns
...
- Do not silently choose Parallel when Jade native tools are already sufficient.
