...
- `tmux` available for stable authenticated sessions during Jade terminal calls (desktop app flow only)
...
### Service Account (recommended for Jade)
...
## Jade Execution Pattern (desktop app flow)

Jade terminal commands are non-interactive by default and can lose auth context between calls.
