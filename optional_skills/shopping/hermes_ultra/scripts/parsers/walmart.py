"""Walmart parser — selectors loaded from ``selectors.yaml``."""
...
class WalmartParser(BaseSiteParser):
    ...
    def parse(self, html: str, url: str = "") -> ProductData:
        ...
        # --- HTML fallbacks ---
        if not data.name:
            data.name = extract_text(html, get_selectors("walmart", "name"))

        if not data.price:
            price_text = extract_text(html, get_selectors("walmart", "price"))
            if price_text:
                data.price = parse_price(price_text, "USD")

        if not data.original_price:
            orig_text = extract_text(html, get_selectors("walmart", "original_price"))
            if orig_text:
