...
function capabilityStatus(condition, warningCondition = false) {
    if (condition)
        return "healthy";
    if (warningCondition)
        return "watch";
...
export function assessDashboardArchitecture(snapshot) {
    ...
    const status = capabilities.reduce((worst, capability) => (severityRank(capability.status) > severityRank(worst) ? capability.status : worst), summary.status);
    return {
