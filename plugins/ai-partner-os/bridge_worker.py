"""Hermes External Linkage WebSocket bridge (AITuberKit v2 compatible)."""
...
def _hermes_python() -> str:
    return os.environ.get("AI_PARTNER_OS_PYTHON") or sys.executable
...
def _handle_chat_message(
    ...
    *,
    ...
    prompt = (
        f"{system_prompt}\n\n"
        "Return only the spoken reply for the AI Partner character. Do not call tools.\n\n"
        f"User input:\n{user_text}"
    )
    ...
    websocket.send(
        json.dumps(
            _v2_envelope(
                ...
                payload={
                    "protocol": "v2",
                    "server": "hermes-ai-partner-os-bridge",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Hermes AI Partner OS External Linkage bridge")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8010)
    parser.add_argument("--session-id", default="hermes-ai-partner-os")
    parser.add_argument(
        "--system-prompt",
        default="You are an AI partner character in AI Partner OS. Reply in natural Japanese.",
    )
