...
Target ~{summary_budget} tokens. Be CONCRETE — include file paths, command outputs, error messages."""
    ...
    def probe_activation(self) -> dict[str, Any]:
        ...
        self.set_activation_status(
            ...
            state="ready",
            detail=(
                "binary capabilities and certified V2 compact/finalize/"
                "prepare/discard lifecycle probe passed"
            ),
        )
        return {**capabilities, "protocol_probe": protocol}

    def _probe_protocol_contract(self) -> dict[str, Any]:
        """Exercise the exact adapter/binary wire contract in an isolated store.
        ...
        with TemporaryDirectory(prefix="ares-context-governor-probe-") as store_dir:
            ...
            receipt = candidate.get("receipt") if isinstance(candidate, dict) else None
            if not isinstance(receipt, dict) or receipt.get("schema") != (
                "ContextCompactionReceiptV2"
            ):
                raise ValueError("compact-v2 returned an invalid certified candidate")
            receipt_id = receipt.get("receipt_id")
            messages = candidate.get("compacted_messages")
            ...
            finalized_receipt = (
                finalized.get("receipt") if isinstance(finalized, dict) else None
            )
            if (
                not isinstance(finalized_receipt, dict)
                or finalized_receipt.get("receipt_id") != receipt_id
            ):
            ...
            if (
                ...
                or prepared.get("schema") != "PendingReceiptInfoV2"
                or prepared.get("receipt_id") != receipt_id
                or prepared.get("verified") is not True
            ):
            ...
            if (
                ...
                or discarded.get("schema") != "ReceiptDiscardResultV2"
                or discarded.get("receipt_id") != receipt_id
                or discarded.get("discarded") is not True
            ):
    ...
    def _recover_pending_before_compaction(
        self,
        ...
        try:
            ...
            return (
                f"pending context-governor receipt {receipt_id} could not be "
                ...
                if candidate != deterministic_projection:
                    ...
                    try:
                        ...
                        if (
                            ...
                            metrics["llm_call_reason"] = f"{checkpoint_reason}:prepared"
                        else:
                            observed = (
    ...
    def _prepare_response(self, response: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not all(checks):
                raise ValueError(
                    "prepare-v2 result did not match the finalized receipt"
                )
        except Exception:
    ...
    def validate_pending_compression(
        ...
        **kwargs,
        ...
        expected = info.get("expected_compacted_messages") or []
        return (
            self._committed_pending_projection(committed_messages, expected) is not None
        )
            ...
            for durable in durable_candidates:
                ...
                try:
                    ...
                    self._record_summary_warning(
                        ...
                        "it remains staged and bound for a later same-process "
                        "recovery attempt",
                    )
                    logger.warning(
                    ...
                    # Keep the matching authenticated admission in memory. The
                    # next ordinary compress() call retries activation through
                    # _recover_pending_before_compaction, so a transient outage
                    # during startup recovery cannot require another restart.
                    return
                return
