...
CRYPTO_BOT_PM_STATUS_SCHEMA = _schema(
    ...
    (
        "Read-only crypto_bot PM status for the Hermes autonomous branch-local "
        "target loop, with live Gitea read context when available. "
...
CRYPTO_BOT_PM_DEVELOPMENT_WORKSTREAM_SCHEMA = _schema(
    ...
    (
        ...
        "selects the first safe development slice for policy classification and "
        "autonomous branch-local execution when allowed, and "
...
CRYPTO_BOT_PM_DEVELOPMENT_SLICE_SCHEMA = _schema(
    ...
    (
        ...
        "packet for the recommended crypto_bot completion candidate. Safe "
        "branch-local execution is governed by infrastructure policy rather "
        "than per-task Operator approval. This tool itself does not call Gitea "
        "write APIs or write files."
    ),
)
