...
OPERATOR_AUTHORITY_METADATA = {
    "tool": "hermes_pm_generate_kanban_proposal_packet",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a proposal-only Hermes PM Kanban packet from PM status "
            "and an optional read-only Gitea snapshot. The tool never mutates "
            "Gitea and has no apply mode."
        )
    )
    ...
    parser.add_argument(
        "--live-gitea-read",
        action="store_true",
        help="Capture a live read-only Gitea snapshot with GET requests only.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.live_gitea_read:
            snapshot = capture_gitea_snapshot(
                base_url=DEFAULT_GITEA_BASE_URL,
                owner=DEFAULT_OWNER,
                repo=DEFAULT_REPO,
            )
        ...
        packet = build_kanban_proposal_packet(
            pm_status=pm_status,
            gitea_snapshot=snapshot,
            work_state=work_state,
    ...
    if args.format == "text":
        print(redact_text(format_kanban_packet_text(packet)))
    else:
