(function () {
  ...
  function achievementField(t, achievement, field) {
    if (achievement && achievement.state === "secret") {
      return tx(t, "secretDefinition." + field, achievement[field] || "");
    }
    return tx(
      t,
      "definitions." + (achievement && achievement.id || "") + "." + field,
      achievement && achievement[field] || "",
    );
  }

  function achievementCategory(t, category) {
    return tx(t, "categories." + catalogId(category), category || "");
  }

  function achievementTier(t, tier) {
    return tier ? tx(t, "tierNames." + catalogId(tier), tier) : "";
  }

  function achievementMetric(t, metric) {
    const fallback = String(metric || "").replace(/_/g, " ");
    return tx(t, "metrics." + metric, fallback);
  }

  function achievementCriteria(t, achievement) {
    if (!achievement) return "";
    if (achievement.state === "secret") {
      return tx(t, "criteria.secret", achievement.criteria || "");
    }
    if (achievement.threshold_metric && Array.isArray(achievement.tiers) && achievement.tiers.length) {
      const ladder = achievement.tiers
        .slice()
        .sort(function (a, b) { return Number(a.threshold || 0) - Number(b.threshold || 0); })
        .map(function (tier) {
          return achievementTier(t, tier.name) + " " + Number(tier.threshold || 0).toLocaleString();
        })
        .join(tx(t, "criteria.separator", "; "));
      return tx(t, "criteria.threshold", achievement.criteria || "", {
        metric: achievementMetric(t, achievement.threshold_metric),
        ladder: ladder,
      });
    }
    if (Array.isArray(achievement.requirements) && achievement.requirements.length) {
      const requirements = achievement.requirements.map(function (requirement) {
        return achievementMetric(t, requirement.metric) + " ≥ " + Number(requirement.gte || 1).toLocaleString();
      }).join(tx(t, "criteria.separator", "; "));
      return tx(t, "criteria.requirements", achievement.criteria || "", { requirements: requirements });
    }
    return tx(t, "criteria.default", achievement.criteria || "");
  }

  function localizeAchievement(t, achievement) {
    return Object.assign({}, achievement, {
      name: achievementField(t, achievement, "name"),
      description: achievementField(t, achievement, "description"),
      category: achievementCategory(t, achievement.category),
      criteria: achievementCriteria(t, achievement),
      localized_tier: achievementTier(t, achievement.tier || achievement.next_tier),
      share_tier_suffix: tx(t, "share.tier_suffix", "TIER"),
      share_unlocked_stamp: tx(t, "share.unlocked_stamp", "UNLOCKED"),
    });
  }
  ...
  async function buildShareImage(achievement) {
    ...
    // Tier badge pill
    const badgeLabel = (achievement.localized_tier || tier).toUpperCase() + " " + (achievement.share_tier_suffix || "TIER");
    ...
    const stamp = "◆ " + (achievement.share_unlocked_stamp || "UNLOCKED");
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
      const tierPart = achievement.tier
        ? tx(t, "share.tier_part", "{tier} tier ", { tier: achievement.localized_tier || achievement.tier })
        : "";
  ...
  function AchievementCard({ achievement }) {
    const { t } = useI18n();
    achievement = localizeAchievement(t, achievement);
    ...
    let tierLabel;
    if (achievement.tier) {
      tierLabel = achievementTier(t, achievement.tier);
    } else if (targetTier) {
      tierLabel = tx(t, "tier.target", "Target {tier}", { tier: achievementTier(t, targetTier) });
  ...
  function AchievementsPage() {
    ...
    const highest = highestRaw ? achievementTier(t, highestRaw) : tx(t, "stats.none_yet", "None yet");
    ...
    return React.createElement("div", { className: "ha-page" },
      ...
      React.createElement("div", { className: "ha-stats" },
        ...
        React.createElement(StatCard, {
          label: tx(t, "stats.latest", "Latest"),
          value: latest[0] ? achievementField(t, latest[0], "name") : tx(t, "stats.none_yet", "None yet"),
          hint: latest[0] ? achievementCategory(t, latest[0].category) : tx(t, "stats.latest_hint_empty", "run Hermes more"),
        })
      ),
      ...
      React.createElement("div", { className: "ha-toolbar" },
        React.createElement("div", { className: "ha-pills" }, categories.map(function (cat) {
          ...
          const pillLabel = cat === "All" ? allCategoryLabel : achievementCategory(t, cat);
