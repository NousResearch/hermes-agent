(function () {
  ...
  // Achievement definitions keep English copy as an API fallback. Locales use
  // the stable ACHIEVEMENTS id directly, so translating a badge never changes
  // its unlock state or requires a locale-specific scan snapshot.
  function localizedCategory(t, category) {
    const categories = t && t.achievements && t.achievements.categories;
    return (categories && categories[category]) || category;
  }

  function localizedAchievement(t, achievement) {
  ...
  function localizedMetric(t, achievement, metric) {
    const metrics = t && t.achievements && t.achievements.metrics;
    const spec = achievement.criteria_spec || {};
    return (metrics && metrics[metric]) || (spec.metric_labels && spec.metric_labels[metric]) || null;
  }

  function localizedCriteria(t, achievement) {
    const spec = achievement.criteria_spec;
    if (!spec || !spec.kind) return achievement.criteria;
    if (spec.kind === "secret_hidden") {
      return tx(t, "criteria.secret_hidden", "Secret: exact requirement hidden until Hermes sees the first matching signal. Keep using Hermes across debugging, tools, memory, skills, plugins, and model workflows to reveal it.");
    }
    if (spec.kind === "tiered") {
      const metric = localizedMetric(t, achievement, spec.metric);
      const ladder = (spec.tiers || []).map(function (tier) { return tier.name + " " + tier.threshold; }).join(", ");
      return metric && ladder ? tx(t, "criteria.tiered", "Requirement: {metric}. Tier ladder: {ladder}.", { metric: metric, ladder: ladder }) : achievement.criteria;
    }
    if (spec.kind === "requirements") {
      const requirements = (spec.requirements || []).map(function (requirement) {
        const metric = localizedMetric(t, achievement, requirement.metric);
        return metric && (metric + " ≥" + (requirement.gte || 1));
      });
      return requirements.length && requirements.every(Boolean) ? tx(t, "criteria.requirements", "Requirement: {requirements}.", { requirements: requirements.join("; ") }) : achievement.criteria;
    }
  ...
  function ShareDialog({ achievement, onClose }) {
    const { t } = useI18n();
    const displayAchievement = localizedAchievement(t, achievement);
    ...
    hooks.useEffect(function () {
      let cancelled = false;
      let createdUrl = null;
      buildShareImage(displayAchievement).then(function (blob) {
        if (cancelled) return;
    ...
    }, [achievement.id, displayAchievement.name, displayAchievement.description]);
    ...
    return React.createElement("div", {
      ...
      React.createElement("div", { className: "ha-share-dialog", role: "dialog", "aria-label": tx(t, "share.dialog_label", "Share achievement") },
        React.createElement("div", { className: "ha-share-head" },
          React.createElement("strong", null, tx(t, "share.header", "Share: {name}", { name: displayAchievement.name })),
        ...
        React.createElement("div", { className: "ha-share-preview" },
          ...
          previewUrl && React.createElement("img", { src: previewUrl, alt: tx(t, "share.card_alt", "{name} share card", { name: displayAchievement.name }) })
        ),
      ...
      React.createElement(C.CardContent, { className: "ha-card-content" },
        React.createElement("div", { className: "ha-card-head" },
          ...
          React.createElement("div", { className: "ha-card-title-wrap" },
            React.createElement("div", { className: "ha-card-title" }, displayAchievement.name),
            React.createElement("div", { className: "ha-card-category" }, displayAchievement.category)
          ),
          React.createElement("div", { className: "ha-badges" },
            ...
            state === "unlocked" && React.createElement("button", {
              ...
              "aria-label": tx(t, "card.share_label", "Share {name}", { name: displayAchievement.name }),
            }, tx(t, "card.share_text", "Share"))
          )
        ),
        React.createElement("p", { className: "ha-description" }, displayAchievement.description),
        achievement.criteria && React.createElement("details", { className: "ha-criteria" },
          ...
          React.createElement("p", null, localizedCriteria(t, achievement))
        ),
  ...
  function AchievementsPage() {
    ...
    return React.createElement("div", { className: "ha-page" },
      ...
      React.createElement("div", { className: "ha-stats" },
        ...
        React.createElement(StatCard, { label: tx(t, "stats.latest", "Latest"), value: latest[0] ? localizedAchievement(t, latest[0]).name : tx(t, "stats.none_yet", "None yet"), hint: latest[0] ? localizedAchievement(t, latest[0]).category : tx(t, "stats.latest_hint_empty", "run Hermes more") })
      ),
      ...
      React.createElement("div", { className: "ha-toolbar" },
        React.createElement("div", { className: "ha-pills" }, categories.map(function (cat) {
          ...
          const pillLabel = cat === "All" ? allCategoryLabel : localizedCategory(t, cat);
