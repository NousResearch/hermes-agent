(function () {
  ...
  function _t(k,f){return window.__HERMES_UI__[k]||f;}
const tierClass = function (tier) {
  ...
  async function api(path, options) {
    ...
    var locale = localStorage.getItem("hermes-locale");
    if (locale === "zh") {
      opts.headers["Accept-Language"] = "zh-CN";
    }
    ...
    return React.createElement("div", {
      ...
      React.createElement("div", { className: "ha-share-dialog", role: "dialog", "aria-label": "Share achievement" },
        ...
        React.createElement("div", { className: "ha-share-preview" },
          status === "rendering" && React.createElement("div", { className: "ha-share-placeholder" }, _t("share_rendering","Rendering…")),
        ...
        React.createElement("div", { className: "ha-share-actions" },
          ...
          }, _t("share_on_x","Share on X")),
          React.createElement("button", {
            className: "ha-share-btn",
            onClick: copyToClipboard,
            disabled: status !== "ready" && status !== "copied",
            title: "Copy the image to paste into your post",
          }, status === "copied" ? _t("share_copied","Copied ✓") : _t("share_copy","Copy image")),
          React.createElement("button", {
            className: "ha-share-btn",
            onClick: download,
            disabled: status !== "ready" && status !== "copied",
          }, _t("share_download","Download PNG"))
        ),
        React.createElement("p", { className: "ha-share-hint" },
          _t("share_hint","Share on X opens a pre-filled post in a new tab. Click Copy image first if you want the 1200×630 badge attached — X lets you paste it right into the tweet composer. Download PNG saves the file for use anywhere.")
        )
      )
    );
  }
      ...
      ["Copper", "Silver", "Gold", "Diamond", "Olympian"].map(function (tier, index, arr) {
        return React.createElement(React.Fragment, { key: tier },
          React.createElement("span", { className: "ha-tier-step ha-tier-" + tier.toLowerCase() },
            ...
            _t("tier_" + tier.toLowerCase(), tier)
          ),
    ...
    return React.createElement("div", { className: "ha-page ha-page-loading" },
      React.createElement("section", { className: "ha-hero ha-loading-hero" },
        React.createElement("div", null,
          React.createElement("div", { className: "ha-kicker" }, _t("hero_kicker","Agentic Gamerscore")),
          React.createElement("h1", null, _t("hero_title","Hermes Achievements")),
          React.createElement("p", null, _t("hero_desc","Scanning Hermes session history. First scan can take 5–10 seconds on large histories."))
        ),
      ...
      React.createElement("div", { className: "ha-stats" },
        ["Unlocked", "Discovered", "Secrets", "Highest tier", "Latest"].map(function (label) {
          const lbl = _t('loading_stat_' + label.toLowerCase().replace(/ /g,'_'), label);
          return React.createElement(C.Card, { key: label, className: "ha-stat ha-skeleton-stat" },
            React.createElement(C.CardContent, { className: "ha-stat-content" },
              React.createElement("div", { className: "ha-stat-label" }, lbl),
      ...
      React.createElement("section", { className: "ha-guide ha-loading-guide" },
        React.createElement("div", null,
          React.createElement("strong", null, _t("loading_guide_title","Scan status")),
        ...
        React.createElement("div", null,
          React.createElement("strong", null, _t("loading_guide_what","What is scanned")),
  ...
  function AchievementCard({ achievement }) {
    ...
    const stateLabel = state === "unlocked" ? _t("state_unlocked","Unlocked") : (state === "secret" ? _t("state_secret","Secret") : _t("state_discovered","Discovered"));
    const targetTier = achievement.next_tier || achievement.tier;
    const tierLabel = achievement.tier ? _t("tier_"+achievement.tier.toLowerCase(), achievement.tier) : (targetTier ? (_t("tier_target","Target ") + _t("tier_"+targetTier.toLowerCase(), targetTier)) : (state === "secret" ? _t("tier_hidden","Hidden") : (unlocked ? _t("tier_complete","Complete") : _t("tier_objective","Objective"))));
      ...
      React.createElement(C.CardContent, { className: "ha-card-content" },
        React.createElement("div", { className: "ha-card-head" },
          ...
          React.createElement("div", { className: "ha-card-title-wrap" },
            ...
            React.createElement("div", { className: "ha-card-category" }, _t(achievement.category, achievement.category))
          ),
          React.createElement("div", { className: "ha-badges" },
            ...
            state === "unlocked" && React.createElement("button", {
              ...
              title: _t("share_title","Share this achievement"),
              "aria-label": _t("share_aria","Share ") + achievement.name,
            }, _t("share_btn","Share"))
          )
        ),
        ...
        achievement.criteria && React.createElement("details", { className: "ha-criteria" },
          React.createElement("summary", null, state === "secret" ? _t("criteria_how_to_reveal","How to reveal") : _t("criteria_what_counts","What counts")),
        ...
        React.createElement("div", { className: "ha-evidence-slot" },
          achievement.evidence ? React.createElement("div", { className: "ha-evidence" },
            React.createElement("span", { className: "ha-evidence-label" }, _t("evidence_label","Evidence")),
            React.createElement("span", { className: "ha-evidence-title" }, achievement.evidence.title || achievement.evidence.session_id || "session")
          ) : React.createElement("div", { className: "ha-evidence ha-evidence-empty", "aria-hidden": "true" }, _t("evidence_empty","No evidence yet"))
        ),
        React.createElement("div", { className: "ha-progress-row" },
  ...
  function AchievementsPage() {
    ...
    function load() {
      setLoading(true);
      api("/achievements")
        .then(function (payload) { setData(payload); setError((payload && payload.error) || null); window.__HERMES_UI__ = (payload && payload._ui) || {}; })
    ...
    if (scanInFlight) {
      ...
      const headline = scanMode === "pending"
        ? _t("scan_pending","Starting achievement scan…") : _t("scan_in_progress","Building achievement profile…");
    ...
    return React.createElement("div", { className: "ha-page" },
      React.createElement("section", { className: "ha-hero" },
        React.createElement("div", null,
          React.createElement("div", { className: "ha-kicker" }, _t("hero_kicker","Agentic Gamerscore")),
          React.createElement("h1", null, _t("hero_title","Hermes Achievements")),
          React.createElement("p", null, _t("hero_desc","Collectible Hermes badges earned from real session history. Known unfinished achievements are shown as Discovered; Secret achievements stay hidden until the first matching behavior appears."))
        ),
        React.createElement(C.Button, { onClick: load, className: "ha-refresh" }, _t("rescan","Rescan"))
      ),
      scanBanner,
      error && React.createElement(C.Card, { className: "ha-error" }, React.createElement(C.CardContent, null, String(error))),
      React.createElement("div", { className: "ha-stats" },
        React.createElement(StatCard, { label: _t("stat_unlocked","Unlocked"), value: (data ? data.unlocked_count : 0) + " / " + (data ? data.total_count : 0), hint: _t("hint_earned","earned badges") }),
        React.createElement(StatCard, { label: _t("stat_discovered","Discovered"), value: discovered.length, hint: _t("hint_known","known, not earned yet") }),
        React.createElement(StatCard, { label: _t("stat_secrets","Secrets"), value: secret.length, hint: _t("hint_hidden","hidden until first signal") }),
        React.createElement(StatCard, { label: _t("stat_highest","Highest tier"), value: highestTier ? _t("tier_"+highestTier.toLowerCase(), highestTier) : _t("hint_none","None yet"), hint: _t("hint_tier_ladder","Copper → Silver → Gold → Diamond → Olympian") }),
        React.createElement(StatCard, { label: _t("stat_latest","Latest"), value: latest[0] ? latest[0].name : _t("hint_none","None yet"), hint: latest[0] ? _t(latest[0].category, latest[0].category) : _t("hint_run_more","run Hermes more") })
      ),
      React.createElement("section", { className: "ha-guide" },
        React.createElement("div", null,
          React.createElement("strong", null, _t("guide_tiers","Tiers")),
          React.createElement(TierLegend, null)
        ),
        React.createElement("div", null,
          React.createElement("strong", null, _t("guide_secrets","Secret achievements")),
          React.createElement("p", null, _t("guide_secrets_desc","Secrets hide their exact trigger. Once Hermes sees a related signal, the card becomes Discovered and shows its requirement."))
        )
      ),
      React.createElement("div", { className: "ha-toolbar" },
        React.createElement("div", { className: "ha-pills" }, categories.map(function (cat) {
          return React.createElement("button", { key: cat, onClick: function () { setCategory(cat); }, className: cat === category ? "active" : "" }, cat === "All" ? _t("filter_all","All") : _t(cat, cat));
        })),
        React.createElement("div", { className: "ha-pills" }, ["all", "unlocked", "discovered", "secret"].map(function (v) {
          var label = {all: _t("state_filter_all","all"), unlocked: _t("state_filter_unlocked","unlocked"), discovered: _t("state_filter_discovered","discovered"), secret: _t("state_filter_secret","secret")}[v];
      ...
      latest.length > 0 && React.createElement("section", { className: "ha-latest" },
        React.createElement("h2", null, _t("recent_unlocks","Recent unlocks")),
      ...
      visibility === "secret" && visible.length === 0 && React.createElement(C.Card, { className: "ha-secret-empty" },
        React.createElement(C.CardContent, { className: "ha-secret-empty-content" },
          React.createElement("strong", null, _t("secret_empty_title","No hidden secrets left in this scan.")),
          React.createElement("p", null, _t("secret_empty_clue","Clue: secrets usually start from unusual failure or power-user patterns — port conflicts, permission walls, missing env vars, YAML mistakes, Docker collisions, rollback/checkpoint use, cache hits, or tiny fixes after lots of red text."))
        )
      ),
