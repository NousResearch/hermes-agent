(function () {
  ...
  function localizeTier(t, tier) {
    const names = t && t.achievements && t.achievements.tierNames;
    return (names && names[tier]) || tier;
  }

  function localizeCategory(t, category) {
    const categories = t && t.achievements && t.achievements.categories;
    return (categories && categories[category]) || category;
  }

  function localizeCriteria(t, achievement) {
    const catalog = t && t.achievements;
    const copy = catalog && catalog.criteriaText;
    if (!copy) return achievement.criteria;

    if (achievement.state === "secret") return copy.secret;

    if (achievement.threshold_metric) {
      const metric =
        (catalog.metrics && catalog.metrics[achievement.threshold_metric]) ||
        achievement.threshold_metric.replace(/_/g, " ");
  ...
  function localizeAchievement(t, achievement) {
    ...
    return Object.assign({}, achievement, {
      ...
      description: isHidden
        ...
        : ((item && item.description) || achievement.description),
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
      const tierPart = achievement.tier
        ? ((achievement.display_tier || achievement.tier) + " " +
          (achievement.tier_suffix || "tier") + " ")
        : "";
  ...
  function StatCard(props) {
    const numericValue = /^[\d\s/.,%+-]+$/.test(String(props.value || ""));
  ...
  function AchievementCard({ achievement }) {
    ...
    let tierLabel;
    if (achievement.tier) {
      tierLabel = localizeTier(t, achievement.tier);
    } else if (targetTier) {
      tierLabel = tx(t, "tier.target", "Target {tier}", {
        tier: localizeTier(t, targetTier),
      });
      ...
      React.createElement(C.CardContent, { className: "ha-card-content" },
        ...
        React.createElement("div", { className: "ha-progress-row" },
          ...
          React.createElement("span", {
            className: "ha-progress-text",
            dir: state === "secret" ? undefined : "ltr",
          }, progressText)
        )
      ),
  ...
  function AchievementsPage() {
    ...
    const visible = achievements.filter(function (a) {
      ...
      return localizeAchievement(t, achievement);
    });
    ...
    const latest = unlocked.slice().sort(function (a, b) {
      return (b.unlocked_at || 0) - (a.unlocked_at || 0);
    }).slice(0, 5).map(function (achievement) {
      return localizeAchievement(t, achievement);
    });
    const highestTier = ["Olympian", "Diamond", "Gold", "Silver", "Copper"].find(function (tier) {
      return unlocked.some(function (a) { return a.tier === tier; });
    });
    const highest = highestTier
      ? localizeTier(t, highestTier)
      : tx(t, "stats.none_yet", "None yet");
    ...
    return React.createElement("div", { className: "ha-page" },
      ...
      React.createElement("div", { className: "ha-toolbar" },
        React.createElement("div", { className: "ha-pills" }, categories.map(function (cat) {
          ...
          const pillLabel =
            cat === "All" ? allCategoryLabel : localizeCategory(t, cat);
