...
def _to_input_image_part(value: str) -> Dict[str, str]:
    ...
    The shared resolver applies the read denylist and magic-byte validation,
    bounded here by the 25MB cap and :data:`_ACCEPTED_INPUT_MIME`. Public URLs
    pass through for the API to fetch; local files and ``data:`` URIs are
    inlined and re-encoded under their *sniffed* MIME, so a mislabelled header
    ...
    if source.kind is SourceKind.REMOTE:
        return {"type": "input_image", "image_url": source.value}

    encoded = base64.b64encode(source.read_bytes()).decode("ascii")
    return {
        "type": "input_image",
        "image_url": f"data:{source.mime};base64,{encoded}",
    }
