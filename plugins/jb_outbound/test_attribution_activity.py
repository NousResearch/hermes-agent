"""Tests de l'attribution des drafts (stamp department/skill_id/job_id).
...
le front-matter des skills (``casquette:`` gold, ``department:`` custom), et le pont
scheduler → plugin (cron/scheduler.py::run_job).
"""
...
def posts(monkeypatch):
    """Active la boucle de proposition et capture tous les POST loopback."""
...
# ---------------------------------------------------------------------------
# Stamp d'attribution sur les drafts
# ---------------------------------------------------------------------------
...
def test_passif_sans_box():
    # JB_DECISION_PUSH_URL absent → job_started est un no-op total (plugin passif).
...
def test_scheduler_run_job_pose_et_nettoie_le_contexte(posts, tmp_path, monkeypatch):
    """run_job (scheduler réel) pose le contexte pendant le job et le nettoie après."""
    _write_skill(tmp_path, "relance-devis", _GOLD)
