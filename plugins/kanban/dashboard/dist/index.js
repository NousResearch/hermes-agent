...
(function () {
  ...
  const FALLBACK_COLUMN_HELP = {
    ...
    review: "Waiting for review or a human decision",
  ...
  function withCompletionSummary(patch, count, t) {
    ...
    const label = count && count > 1
      ? tx(t, "taskText.selectedTasks", "{count} selected task(s)", { count: count })
      : tx(t, "taskText.thisTask", "this task");
    ...
    return h("div", {
      ...
      h("div", { className: "hermes-kanban-attention-bar" },
        ...
        h("button", {
          ...
          title: tx(t, "tooltips.hideUntilReload", "Hide until next page reload"),
          type: "button",
        }, "\u2715"),
      ),
      expanded
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "cli_hint") {
        ...
        const fallback = function () {
          window.prompt(tx(t, "taskText.copyPrompt", "Copy this command:"), cmd);
        };
        try {
      ...
      reassignAction
        ? h("div", { className: "hermes-kanban-diag-reassign-row" },
            ...
            h("select", {
              ...
              h("option", { value: "" },
                tx(t, "taskText.unassigned", "(unassigned)")),
  ...
  function DocsLink() {
    ...
    return h("a", {
      ...
      title: tx(t, "tooltips.openDocs", "Open Hermes Kanban docs in a new tab"),
      "aria-label": tx(t, "tooltips.docsLabel", "Hermes Kanban documentation"),
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    const loadAll = useCallback(function () {
      Promise.all([
        ...
        setMsg({
          ok: false,
          text: tx(t, "orchestration.loadFailed", "Failed to load: {error}", {
    ...
    const saveSettings = function (patch) {
      ...
      return SDK.fetchJSON(`${API}/orchestration`, {
        ...
        setMsg({
          ok: true,
          text: tx(t, "orchestration.settingsSaved", "Settings saved."),
        });
        return res;
      }).catch(function (err) {
        setMsg({
          ok: false,
          text: tx(t, "orchestration.saveFailed", "Save failed: {error}", {
            error: err.message || String(err),
          }),
        });
      });
    ...
    const saveProfileDescription = function (name, description) {
      ...
      return SDK.fetchJSON(`${API}/profiles/${encodeURIComponent(name)}`, {
        ...
        setMsg({
          ok: true,
          text: tx(t, "orchestration.descriptionSaved", "Description saved for {name}.", {
            name: name,
          }),
        });
      }).catch(function (err) {
    ...
    const autoGenerateDescription = function (name, overwrite) {
      ...
      return SDK.fetchJSON(`${API}/profiles/${encodeURIComponent(name)}/describe-auto`, {
        ...
        if (res && res.ok) {
          ...
          setMsg({
            ok: true,
            text: tx(t, "orchestration.descriptionGenerated", "Auto-generated description for {name}.", {
              name: name,
            }),
          });
        } else {
          setMsg({
            ok: false,
            text: tx(t, "orchestration.generateFailed", "Auto-generate failed: {error}", {
              error: (res && res.reason) ||
                tx(t, "orchestration.unknownError", "unknown error"),
            }),
          });
        }
      }).catch(function (err) {
    ...
    const headerLabel = expanded
      ? tx(t, "orchestration.expanded", "▾ Orchestration settings")
      : tx(t, "orchestration.collapsed", "▸ Orchestration settings");
    ...
    const modePill = h("button", {
      ...
      h("span", { className: "ml-1 font-semibold" },
        settings === null
          ? "…"
          : (autoOn
              ? tx(t, "orchestration.auto", "Auto")
              : tx(t, "orchestration.manual", "Manual")))
    );
    ...
    if (!expanded) {
      return h("div", { className: "flex items-center gap-3 text-xs" },
        ...
        h("button", {
          ...
          title: tx(t, "tooltips.configureOrchestrator", "Configure the kanban orchestrator (profile picker, default assignee, auto-decompose, profile descriptions)"),
        }, headerLabel),
      );
    }
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        h("div", { className: "flex items-center justify-between" },
          ...
          modePill,
          h(Button, { onClick: loadAll, size: "sm" },
            tx(t, "orchestration.reload", "Reload")),
        ),
        msg ? h("div", {
        ...
        settings ? h("div", { className: "grid gap-3 sm:grid-cols-3" },
          h("div", { className: "flex flex-col gap-1" },
            ...
            h(Select, Object.assign({
              ...
              h(SelectOption, { value: "" },
                tx(t, "orchestration.defaultValue", "(default: {value})", {
            ...
            h("div", { className: "text-[10px] text-muted-foreground" },
              tx(t, "orchestration.resolved", "Resolved: {value}", {
                value: settings.resolved_orchestrator_profile ||
                  tx(t, "orchestration.defaultTag", "default"),
              })),
            h("div", { className: "text-[10px] text-muted-foreground" },
          ...
          h("div", { className: "flex flex-col gap-1" },
            ...
            h("div", { className: "text-[10px] text-muted-foreground" },
              tx(t, "orchestration.resolved", "Resolved: {value}", {
                value: settings.resolved_default_assignee ||
                  tx(t, "orchestration.defaultTag", "default"),
              })),
          ),
          h("div", { className: "flex flex-col gap-1" },
        ...
        h("div", { className: "border-t pt-3" },
          ...
          h("div", { className: "text-[10px] text-muted-foreground pb-2" },
            tx(t, "orchestration.profileDescriptionsHint", "Descriptions guide the decomposer's routing. Click ⚗ to auto-generate, or edit and save.")),
          profiles.length === 0
            ? h("div", { className: "text-xs text-muted-foreground" },
                tx(t, "orchestration.noProfiles", "No profiles installed."))
    ...
    return h("div", { className: "flex flex-col gap-1 border-l-2 pl-2",
      ...
      h("div", { className: "flex items-center gap-2 text-xs" },
        ...
        p.is_default ? h("span", { className: "text-[10px] text-muted-foreground" },
          "(" + tx(t, "orchestration.defaultTag", "default") + ")") : null,
        p.description_auto && p.description
          ? h("span", { className: "text-[10px] text-yellow-600" },
              tx(t, "orchestration.autoReview", "auto — review"))
          : null,
        !p.description
          ? h("span", { className: "text-[10px] text-yellow-600" },
              "⚠ " + tx(t, "orchestration.noDescription", "no description"))
          : null,
      ),
      h("div", { className: "flex items-center gap-2" },
        ...
        h(Button, {
          ...
          title: tx(t, "tooltips.saveProfileDescription", "Save the description above as user-authored"),
        }, busy === "save"
          ? tx(t, "orchestration.saving", "Saving…")
          : tx(t, "orchestration.save", "Save")),
        h(Button, {
          onClick: function () { props.onAuto(p.name, true); },
          size: "sm",
          disabled: !!busy,
          title: tx(t, "tooltips.autoProfileDescription", "Auto-generate a description from this profile's skills and model"),
        }, busy === "auto"
          ? tx(t, "orchestration.generating", "Generating…")
          : "⚗ " + tx(t, "orchestration.generate", "Auto")),
      ),
    );
  }
    ...
    return h("div", { className: "hermes-kanban-boardswitcher" },
      h("div", { className: "hermes-kanban-boardswitcher-inner" },
        h("div", { className: "flex flex-col gap-0.5" },
          ...
          h("div", { className: "flex items-center gap-2" },
            h(Select, Object.assign({
              ...
              "aria-label": tx(t, "tooltips.switchBoard", "Switch kanban board"),
              title: tx(t, "tooltips.boardsIndependent", "Boards are independent work streams. Each board has its own tasks, tenants, and assignees."),
            ...
            h("span", { className: "text-xs text-muted-foreground" },
              tx(t, currentTotal === 1 ? "extra.tasksCountOne" : "extra.tasksCount",
                currentTotal === 1 ? "{count} task" : "{count} tasks", {
                  count: currentTotal || 0,
                })),
          ),
        ),
        ...
        h(Button, {
          ...
          size: "sm",
          className: "h-8",
          title: tx(t, "tooltips.createBoard", "Create a new board. Useful when you want an unrelated work stream (different project, different team, isolated scratch area)."),
        }, tx(t, "newBoard", "+ New board")),
        props.board !== "default"
          ? h(Button, {
            onClick: function () {
  ...
  function NewBoardDialog(props) {
    ...
    function onSubmit(ev) {
      ...
      if (!slug.trim()) {
        setErr(tx(t, "extra.slugRequired", "slug is required"));
        return;
      }
      setSubmitting(true);
      setErr(null);
        ...
        h("div", { className: "flex flex-col gap-3" },
          ...
          h("div", { className: "flex flex-col gap-1" },
            ...
            h(Input, {
              ...
              placeholder: tx(t, "extra.descriptionPlaceholder", "What goes on this board?"),
              className: "h-8",
            }),
          ),
          h("div", { className: "flex flex-col gap-1" },
            h(Label, { className: "text-xs" },
              tx(t, "extra.projectDirectory", "Project directory"), " ",
              h("span", { className: "text-muted-foreground" },
                "(" + tx(t, "extra.projectDirectoryHint", "recommended") + ")")),
            h(Input, {
              value: projectDirectory,
              onChange: function (e) { setProjectDirectory(e.target.value); },
              placeholder: tx(t, "extra.projectDirectoryPlaceholder",
                "Absolute path to the project folder"),
              title: tx(t, "extra.projectDirectoryHelp",
            ...
            h("div", { className: "text-xs text-muted-foreground" },
              tx(t, "extra.projectDirectoryExplanation",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      h("div", { className: "flex flex-col gap-1",
                 title: tx(t, "tooltips.fuzzySearch", "Fuzzy-match tasks by id, title, or description. Matches across all columns.") },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: tx(t, "tooltips.assigneeFilter", "Filter by assigned Hermes profile. Profiles are the named agent identities that claim and work on tasks.") },
      ...
      h("label", { className: "flex items-center gap-2 text-xs",
                   title: tx(t, "tooltips.includeArchived", "Include archived tasks in the board view. Archived tasks are hidden by default.") },
        h(Checkbox, {
          checked: props.includeArchived,
          "aria-label": tx(t, "showArchived", "Show archived"),
          onCheckedChange: function (checked) { props.setIncludeArchived(checked === true); },
        }),
        tx(t, "showArchived", "Show archived"),
      ),
      h("label", { className: "flex items-center gap-2 text-xs",
                   title: tx(t, "tooltips.laneByProfile", "Group the Running column by assigned profile") },
        h(Checkbox, {
          checked: props.laneByProfile,
          "aria-label": tx(t, "lanesByProfile", "Lanes by profile"),
      ...
      h(Button, {
        ...
        size: "sm",
        title: tx(t, "tooltips.refreshBoard", "Reload the board from the database. The board auto-refreshes on task events; this is for forcing a re-read."),
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "hermes-kanban-bulk" },
      ...
      h(Button, {
        ...
        size: "sm",
        title: tx(t, "bulk.moveTodoTitle", "Move selected tasks to Todo."),
      }, tx(t, "bulk.moveTodo", "→ todo")),
      h(Button, {
        onClick: function () { props.onApply({ status: "ready" }); },
        size: "sm",
        title: tx(t, "bulk.moveReadyTitle", "Move selected tasks to Ready. Ready tasks are picked up by the dispatcher on the next tick."),
      }, tx(t, "bulk.moveReady", "→ ready")),
      h(Button, {
        onClick: function () { props.onApply({ status: "blocked" },
          tx(t, "bulk.blockConfirm", "Block {count} task(s)?", {
            count: props.count,
          })); },
        size: "sm",
        title: tx(t, "bulk.blockTitle", "Block selected tasks. Releases any active claims."),
      }, tx(t, "bulk.block", "Block")),
      h(Button, {
        onClick: function () { props.onApply({ status: "ready" },
          tx(t, "bulk.unblockConfirm", "Unblock {count} task(s)?", {
            count: props.count,
          })); },
        size: "sm",
      ...
      h(Button, {
        ...
        size: "sm",
        title: tx(t, "bulk.completeTitle", "Mark selected tasks as done. Releases any claims and unblocks dependent children. You'll be asked for a completion summary."),
      ...
      h(Button, {
        ...
        size: "sm",
        title: tx(t, "bulk.archiveTitle", "Archive selected tasks. They disappear from the default board view but remain in the database."),
      }, tx(t, "archive", "Archive")),
      h(Button, {
        onClick: function () {
        ...
        size: "sm",
        variant: "destructive",
        title: tx(t, "bulk.deleteTitle", "Permanently delete selected tasks. This cannot be undone."),
      }, tx(t, "extra.delete", "Delete")),
      h("div", { className: "hermes-kanban-bulk-priority",
                 title: tx(t, "bulk.priorityTitle", "Set priority on selected tasks. Higher = claimed first.") },
        h(Input, {
          type: "number",
          value: priority,
      ...
      h("div", { className: "hermes-kanban-bulk-reassign",
        ...
        h(Select, Object.assign({
          ...
          h(SelectOption, { value: "" }, tx(t, "bulk.reassign", "— reassign —")),
          h(SelectOption, { value: "__none__" }, tx(t, "bulk.unassign", "(unassign)")),
        ...
        h(Button, {
          ...
          disabled: !assignee,
          size: "sm",
          title: tx(t, "bulk.applyAssignee", "Apply the selected assignee to all selected tasks."),
        }, tx(t, "apply", "Apply")),
      ),
      h("label", { className: "hermes-kanban-bulk-reclaim-first",
        ...
        tx(t, "bulk.reclaimFirst", "Reclaim first"),
      ),
      h("div", { className: "flex-1" }),
      h(Button, {
      ...
      h(Button, {
        ...
        size: "sm",
        title: tx(t, "bulk.clearTitle", "Deselect all tasks and hide this bar."),
      }, tx(t, "clear", "Clear")),
    );
  }
  ...
  function Column(props) {
    ...
    const lanes = useMemo(function () {
      ...
      for (const tk of props.column.tasks) {
        const key = tk.assignee || tx(t, "taskText.unassigned", "(unassigned)");
    ...
    return h("div", {
      ...
      h("div", { className: "hermes-kanban-column-header",
        ...
        h(Checkbox, {
          ...
          title: tx(t, "tooltips.selectAllColumn", "Select all tasks in this column"),
          "aria-label": tx(t, "tooltips.selectAllColumnLabel", "Select all tasks in {column}", {
            column: colLabel || props.column.name,
          }),
        ...
        h("span", { className: "hermes-kanban-column-count",
                    title: tx(t,
                      props.column.tasks.length === 1
                        ? "tooltips.columnTaskCountOne"
                        : "tooltips.columnTaskCount",
                      props.column.tasks.length === 1
                        ? "{count} task in this column"
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      if (selectedCards.length > 1 && props.selected) {
        ...
        ghost.textContent = selectedCards.length + " " +
          tx(i18n, "taskText.cards", "cards");
          ...
          h("div", { className: "hermes-kanban-card-row" },
            h("label", {
              ...
              h(Checkbox, {
                ...
                onClick: function (e) { e.stopPropagation(); },
                "aria-label": tx(i18n, "tooltips.selectTask", "Select task {id}", {
                  id: t.id,
                }),
              }),
            ),
            h("span", { className: "hermes-kanban-card-id",
                        title: tx(i18n, "tooltips.taskId", "Task id: {id}. Use this id with kanban_show, /kanban show, or hermes kanban show.", {
                          id: t.id,
                        }) }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  ...
                  title: tx(i18n,
                    t.warnings.count === 1
                      ? "tooltips.activeDiagnosticsOne"
                      : "tooltips.activeDiagnostics",
                    t.warnings.count === 1
                    ...
                    {
                      count: t.warnings.count,
                      severity: t.warnings.highest_severity ||
                        tx(i18n, "warning", "warning"),
                    }),
            ...
            t.priority > 0
              ? h(Badge, { className: "hermes-kanban-priority",
                           title: tx(i18n, "tooltips.priority", "Priority {value}. Higher-priority tasks are claimed first by the dispatcher.", {
                             value: t.priority,
                           }) }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "hermes-kanban-tag",
                           title: tx(i18n, "tooltips.tenant", "Tenant: {value}. Free-form tag for grouping tasks (customer, project, team).", {
                             value: t.tenant,
                           }) }, t.tenant)
              : null,
            progress
              ? h("span", {
                  ...
                  title: tx(i18n, "tooltips.childProgress", "{done} of {total} child tasks done", {
                    done: progress.done,
          ...
          h("div", { className: "hermes-kanban-card-row hermes-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "hermes-kanban-assignee",
                            title: tx(i18n, "tooltips.assignedProfile", "Assigned to Hermes profile {profile}", {
                              profile: "@" + t.assignee,
                            }) }, "@", t.assignee)
              : h("span", { className: "hermes-kanban-unassigned",
                            title: needsAssignee
                              ...
                              : tx(i18n, "tooltips.noProfile", "No profile assigned.") },
                  tx(i18n, "unassigned", "unassigned")),
            t.comment_count > 0
              ? h("span", { className: "hermes-kanban-count",
                            title: tx(i18n,
                              t.comment_count === 1
                                ? "tooltips.taskCommentsOne"
                                : "tooltips.taskComments",
                              t.comment_count === 1
                                ? "{count} comment on this task"
                                : "{count} comments on this task",
                              { count: t.comment_count }) }, "💬 ", t.comment_count)
              : null,
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ? h("span", { className: "hermes-kanban-count",
                            title: tx(i18n,
                              "tooltips.taskDependencies" +
                                (t.link_counts.parents === 1 ? "OneParent" : "ManyParents") +
                                (t.link_counts.children === 1 ? "OneChild" : "ManyChildren"),
                              t.link_counts.parents === 1
                                ? (t.link_counts.children === 1
                                    ? "{parents} parent, {children} child. Children stay blocked until their parent is done."
                                    : "{parents} parent, {children} children. Children stay blocked until their parent is done.")
                                : (t.link_counts.children === 1
                                    ? "{parents} parents, {children} child. Children stay blocked until their parent is done."
                                    : "{parents} parents, {children} children. Children stay blocked until their parent is done."),
                              {
                                parents: t.link_counts.parents,
                                children: t.link_counts.children,
                              }) },
                  "↔ ", t.link_counts.parents + t.link_counts.children)
              : null,
            h("span", { className: "hermes-kanban-ago",
                        title: t.created_at
                          ? tx(i18n, "tooltips.taskCreated", "Created {date}", {
                              date: t.created_at,
                            })
                          : "" },
              timeAgo ? timeAgo(t.created_at, i18n.common && i18n.common.timeAgo) : ""),
          ),
        ),
      ),
    );
        ...
        h("div", { className: "flex flex-col gap-3" },
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ...
                  : tx(t, "tooltips.workerAssigneeInput", "Hermes profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready."),
          ...
          h("div", { className: "flex flex-col gap-1" },
            ...
            h(Select, Object.assign({
              value: parent,
              className: "h-8 text-sm",
              title: tx(t, "tooltips.parentTask", "Optional parent task. A child stays blocked in its current column until the parent is marked done."),
          ...
          h("div", { className: "flex gap-2 items-center" },
            ...
            goalMode ? h(Input, {
              ...
              title: tx(t, "tooltips.goalTurns", "Turn budget for the goal loop. Blank = backend default (20)."),
              min: 1,
            }) : null,
          ),
        ),
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "hermes-kanban-section" },
      h("div", { className: "hermes-kanban-section-head" },
        `${tx(i18n, "extra.attachments", "Attachments")} (${atts.length})`),
      h("input", {
        ref: fileRef,
        type: "file",
        multiple: true,
      ...
      h("div", { className: "flex items-center gap-2 mb-2" },
        h(Button, {
          ...
          onClick: function () { if (fileRef.current) fileRef.current.click(); },
            ...
            ? tx(i18n, "extra.uploading", "Uploading…")
            : tx(i18n, "extra.uploadFile", "Upload file")),
      ),
      ...
      atts.length === 0
        ? h("div", { className: "text-xs text-muted-foreground" },
            tx(i18n, "extra.noAttachments", "— no attachments —"))
        : atts.map(function (a) {
            return h("div", {
              key: a.id,
              ...
              h("button", {
                type: "button",
                className: "hermes-kanban-drawer-close",
                title: tx(i18n, "extra.removeAttachment", "Remove attachment"),
                onClick: function () {
                  if (window.confirm(tx(i18n, "extra.confirmRemoveAttachment",
    ...
    return h("div", { className: "hermes-kanban-drawer-body" },
      ...
      h("div", { className: "hermes-kanban-drawer-meta" },
        h(MetaRow, {
          ...
          value: getColumnLabel(i18n, t.status),
        }),
        ...
        t.goal_mode ? h(MetaRow, {
          label: tx(i18n, "extra.goalMode", "Goal mode"),
          value: t.goal_max_turns
            ? tx(i18n, "taskText.goalOnMax", "on (max {count} turns)", {
                count: t.goal_max_turns,
              })
            : tx(i18n, "taskText.goalOn", "on"),
        }) : null,
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "hermes-kanban-comment" },
                h("div", { className: "hermes-kanban-comment-head" },
                  h("span", { className: "hermes-kanban-comment-author" },
                    c.author || tx(i18n, "taskText.anonymous", "anon")),
                  h("span", { className: "hermes-kanban-comment-ago" },
                    timeAgo ? timeAgo(c.created_at, i18n.common && i18n.common.timeAgo) : ""),
                ),
            ...
            isDiag
              ? h("div", { className: "hermes-kanban-event-header" },
                  ...
                  h("span", { className: "hermes-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at, i18n.common && i18n.common.timeAgo) : ""),
                )
  ...
  function RunHistorySection(props) {
    ...
    const fmtElapsed = function (run) {
      ...
      if (secs < 60) {
        return tx(t, "taskText.elapsedSeconds", "{count}s", { count: secs });
      }
      if (secs < 3600) {
        return tx(t, "taskText.elapsedMinutes", "{count}m", {
          count: Math.round(secs / 60),
        });
      }
      return tx(t, "taskText.elapsedHours", "{count}h", {
        count: (secs / 3600).toFixed(1),
      });
    };
      ...
      h("div", { className: "hermes-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
            ...
            }, "+" + tx(t, "taskText.earlierAttempts", "{count} earlier", {
              count: runs.length - 3,
            }))
          : null,
      ),
        ...
        return h("div", { key: r.id, className: cn("hermes-kanban-run", outcomeClass) },
          h("div", { className: "hermes-kanban-run-head" },
            ...
            h("span", { className: "hermes-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at, t.common && t.common.timeAgo) : ""),
          ),
          r.summary
            ...
            ? (function () {
                ...
                return h("details", {
                  ...
                  h("summary", { className: "hermes-kanban-run-meta-label" },
                    tx(t, "taskText.metadata", "Metadata")),
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "hermes-kanban-pre hermes-kanban-log" },
        data.content || tx(t, "taskText.empty", "(empty)"));
    }
    ...
    return h("div", { className: "hermes-kanban-section" },
      h("div", { className: "hermes-kanban-section-head-row" },
        ...
        h("button", {
          ...
          onClick: load,
          className: "hermes-kanban-edit-link",
          title: tx(t, "tooltips.refreshLog", "Refresh log"),
        }, tx(t, "taskText.refresh", "refresh")),
      ),
      body,
      data && data.truncated
        ? h("div", { className: "text-xs text-muted-foreground" },
      ...
      h("div", { className: "hermes-kanban-section-head-row" },
        ...
        editing
          ...
          : h("button", {
              ...
              onClick: function () { setEditing(true); },
              className: "hermes-kanban-edit-link",
              title: tx(t, "tooltips.editDescription", "Edit description"),
            }, tx(t, "extra.edit", "edit")),
      ),
      editing
        ? h("textarea", {
  ...
  function StatusActions(props) {
    ...
    const specifyButton = (task.status === "triage" && props.onSpecify)
      ? h(Button, {
            ...
            props.onSpecify().then(function (res) {
              if (res && res.ok) {
                const suffix = res.new_title
                  ? tx(t, "taskText.retitled", " — retitled: {title}", {
                      title: res.new_title,
                    })
                  : "";
                setSpecifyMsg({
                  ok: true,
                  text: tx(t, "taskText.specified", "Specified{suffix}", {
                    suffix: suffix,
                  }),
                });
              } else {
                setSpecifyMsg({
                  ok: false,
                  text: tx(t, "taskText.specifyFailed", "Specify failed: {error}", {
                    error: (res && res.reason) ||
                      tx(t, "orchestration.unknownError", "unknown error"),
                  }),
                });
          ...
          ? tx(t, "taskText.specifying", "Specifying…")
          : "✨ " + tx(t, "taskText.specify", "Specify"))
      : null;
    ...
    const decomposeButton = (task.status === "triage" && props.onDecompose)
      ? h(Button, {
            ...
            props.onDecompose().then(function (res) {
              if (res && res.ok) {
                if (res.fanout && res.child_ids && res.child_ids.length) {
                  setDecomposeMsg({
                    ok: true,
                    text: tx(t, "taskText.decomposed", "Decomposed into {count} children: {ids}", {
                  ...
                  setDecomposeMsg({
                    ok: true,
                    text: tx(t, "taskText.singleTask", "Single task (no fanout){suffix}", {
                      suffix: suffix,
                    }),
                  });
                }
                ...
                setDecomposeMsg({
                  ok: false,
                  text: tx(t, "taskText.decomposeFailed", "Decompose failed: {error}", {
                    error: (res && res.reason) ||
                      tx(t, "orchestration.unknownError", "unknown error"),
                  }),
                });
          ...
          ? tx(t, "taskText.decomposing", "Decomposing…")
          : "⚗ " + tx(t, "taskText.decompose", "Decompose"))
      : null;
    ...
    return h("div", null,
      h("div", { className: "hermes-kanban-actions" },
        specifyButton,
        decomposeButton,
        b(tx(t, "taskText.moveTriage", "→ triage"),
          { status: "triage" }, task.status !== "triage"),
        b(tx(t, "taskText.moveReady", "→ ready"),
          { status: "ready" }, task.status !== "ready"),
      ...
      h("div", { className: "hermes-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          const title = hc.subscribed
            ? `${tx(t, "sendingUpdates", "Sending updates to")} ${target}. ` +
              tx(t, "taskText.stopNotifications", "Click to stop.")
