...
def _conn(board: Optional[str] = None):
    ...
    return kanban_db.connect(board=board, source="dashboard")
...
def update_task(task_id: str, payload: UpdateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if payload.title is not None or payload.body is not None:
            ...
            kanban_db.edit_task_fields(conn, task_id, changes=changes)
...
def _set_status_direct(
    ...
    """Compatibility shim for dashboard callers; mutation lives in kanban_db."""
    return kanban_db.set_task_status_direct(conn, task_id, new_status)
