...
def register_owner_token_routes() -> None:
    for path in ("owner-snapshot", "owner-events"):
...
def _require_owner_read(request: Request) -> None:
    ...
    if principal is not None and _OWNER_READ_SCOPE in set(getattr(principal, "scopes", ())):
        return
...
def _owner_identity(board: Optional[str]) -> tuple[str, str]:
    ...
    return get_active_profile_name(), board or kanban_db.get_current_board()


def _owner_event_cursor(conn: sqlite3.Connection) -> int:
    """Return the monotonic durable owner-event sequence, including deleted gaps."""
    sequence = conn.execute("SELECT seq FROM sqlite_sequence WHERE name = 'owner_events'").fetchone()
    maximum = conn.execute("SELECT COALESCE(MAX(id), 0) FROM owner_events").fetchone()
    return max(int(sequence[0]) if sequence is not None else 0, int(maximum[0]))


def _owner_task(row: sqlite3.Row) -> dict[str, Any]:
    task_id, title, created_by, created_at = row["id"], row["title"], row["created_by"], row["created_at"]
    if (
...
def owner_snapshot(
    ...
    """Bounded complete snapshot and cursor from one SQLite read transaction."""
    _require_owner_read(request)
    if not 1 <= limit <= _OWNER_SNAPSHOT_MAX_LIMIT:
        raise HTTPException(status_code=400, detail=f"limit must be 1..{_OWNER_SNAPSHOT_MAX_LIMIT}")
    ...
    try:
        ...
        if len(task_rows) > limit:
            raise HTTPException(status_code=413, detail=f"Kanban board exceeds the requested {limit}-task snapshot bound")
        task_ids = [row["id"] for row in task_rows]
        created_by_task: dict[str, list[sqlite3.Row]] = {task_id: [] for task_id in task_ids}
        if task_ids:
            event_rows = conn.execute(
                "SELECT id AS event_id, task_id, kind, payload, created_at AS event_created_at FROM owner_events "
                f"WHERE kind = 'created' AND task_id IN ({_placeholders(task_ids)}) ORDER BY id ASC",
                tuple(task_ids),
            ).fetchall()
...
def owner_events(
    ...
    if not 1 <= limit <= _OWNER_EVENTS_MAX_LIMIT:
        raise HTTPException(status_code=400, detail=f"limit must be 1..{_OWNER_EVENTS_MAX_LIMIT}")
    profile_id, board_id = _owner_identity(_resolve_board(board))
    conn = _conn(board=board_id)
    try:
        conn.execute("BEGIN")
        ...
        if after > latest_cursor:
            raise HTTPException(status_code=409, detail="owner event cursor is ahead of the authoritative stream")
        rows = conn.execute(
            "SELECT id AS event_id, task_id AS id, kind, payload, "
            "created_at AS event_created_at, title, created_by, task_created_at AS created_at, archived "
            "FROM owner_events WHERE owner_events.id > ? ORDER BY owner_events.id ASC LIMIT ?",
            (after, limit + 1),
        ).fetchall()
        has_more = len(rows) > limit
        scanned = rows[:limit]
        events = [{**_owner_event(row), "task": _owner_task(row), "archived": bool(row["archived"])} for row in scanned]
        next_cursor = int(scanned[-1]["event_id"]) if has_more else latest_cursor
        return {
            "contract_version": _OWNER_CONTRACT_VERSION, "profile_id": profile_id, "board_id": board_id,
            "after": after, "next_cursor": next_cursor, "has_more": has_more, "events": events,
        }
    finally:
        if conn.in_transaction:
            conn.rollback()
...
def _patch_title_body(conn, task_id: str, payload: UpdateTaskBody, board: Optional[str]) -> None:
    ...
    with kanban_db.write_txn(conn):
        ...
        title_changed = payload.title is not None and payload.title.strip() != current.title
        sets, vals = [], []
