...
from hermes_cli import kanban_worker_handoff
...
def _normalize_slug_or_400(slug: str) -> Optional[str]:
    with _value_error_400():
        return _owner_kanban_boards._normalize_board_slug(slug)
...
def _resolve_board(board: Optional[str]) -> Optional[str]:
    ...
    if normed and normed != _owner_kanban_boards.DEFAULT_BOARD and not _owner_kanban_boards.board_exists(normed):
...
def _existing_board_slug(slug: str) -> str:
    ...
    if not normed or not _owner_kanban_boards.board_exists(normed):
...
def _with_board_pinned(board: Optional[str], fn: Callable[[], Any]) -> Any:
    ...
    with _owner_kanban_boards.scoped_current_board(_resolve_board(board) or _owner_kanban_boards.DEFAULT_BOARD):
        return fn()
...
def _require_task(conn: sqlite3.Connection, task_id: str) -> _owner_kanban_db_models.Task:
    return _require(kanban_db.get_task, conn, task_id, "task")


def _require_run(conn: sqlite3.Connection, run_id: int) -> _owner_kanban_db_models.Run:
    return _require(_owner_kanban_stats.get_run, conn, run_id, "run")
...
def _task_dict(task: _owner_kanban_db_models.Task, *, latest_summary: Optional[str] = None) -> dict[str, Any]:
    d = asdict(task)
    # Derived age metrics so the UI can colour stale cards without client deltas.
    try:
        d["age"] = _owner_kanban_stats.task_age(task)
    except Exception:
...
def _attachment_dict(a: _owner_kanban_db_models.Attachment) -> dict[str, Any]:
...
def get_board(
    ...
    with _board_conn(board) as (board, conn):
        ...
        summary_map = _owner_kanban_stats.latest_summaries(conn, [t.id for t in tasks])
        for t in tasks:
...
async def get_board_endpoint(
    ...
    return await _read_board(
        ...
        board=board or _owner_kanban_boards.get_current_board(),
...
def get_task(
    ...
    with _board_conn(board) as (board, conn):
        ...
        task_d = _task_dict(task, latest_summary=_owner_kanban_stats.latest_summary(conn, task_id))
        links = _links_for(conn, task_id)
        child_summaries = _owner_kanban_stats.latest_summaries(conn, links["children"])
        ...
        return {
            ...
            "child_results": [
                ...
                for c in children],
            "runs": [asdict(r) for r in _owner_kanban_stats.list_runs(conn, task_id, state_type=run_state_type, state_name=run_state_name)]}
...
async def upload_task_attachment(
    ...
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        dest_dir = _owner_kanban_boards.task_attachments_dir(task_id, board=board)
...
def download_attachment(attachment_id: int, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn):
        ...
        root = _owner_kanban_boards.attachments_root(board=board).resolve()
        try:
...
class _StatusTransitionRefused(Exception):
    """A valid transition deferred because worker shutdown is unconfirmed."""
...
def _drag_to(conn, task_id: str, s: str) -> bool:
    ...
    if s == "ready" and current and current.status in ("blocked", "scheduled"):
        return _owner_kanban_transitions.unblock_task(conn, task_id)
    if current is not None and current.status == "review":
        return _owner_kanban_transitions.reopen_review_task(conn, task_id)
...
_STATUS_HANDLERS: dict[str, Any] = {
    "done": lambda conn, tid, p: _owner_kanban_completion.complete_task(conn, tid, result=p.result, summary=p.summary, metadata=p.metadata),
    "blocked": lambda conn, tid, p: _owner_kanban_db.block_task(conn, tid, reason=getattr(p, "block_reason", None)),
    "scheduled": lambda conn, tid, p: _owner_kanban_transitions.schedule_task(conn, tid, reason=getattr(p, "block_reason", None)),
    "review": lambda conn, tid, p: _owner_kanban_transitions.request_review(
...
def _patch_status(conn, task_id: str, payload: UpdateTaskBody, review_assignee_deferred: bool) -> None:
    ...
    s = payload.status
    if s == "archived":
        ok = _owner_kanban_transitions.archive_task(conn, task_id)
    else:
        with _map_errors(409, _StatusTransitionRefused), _map_errors(400, _StatusRejected):
...
def delete_task(task_id: str, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn):
        if not _owner_kanban_transitions.delete_task(conn, task_id):
...
def _set_status_direct(
    ...
    if prev["status"] == "running" and new_status == "ready":
        resume_status = _owner_kanban_claims._retry_status_for_run(
            conn, task_id, prev["current_run_id"]
        )
        if resume_status == "review":
            effective_status = (
                "review"
                if _owner_kanban_transitions._parents_satisfied(conn, task_id)
                else "todo"
            )
    ...
    if leaving_running:
        reserved = kanban_claims.reserve_reclaim(conn, task_id, prev)
        if reserved is None or not kanban_claims._recheck_reclaim_reservation(
            conn, task_id, reserved["claim_lock"], reserved["claim_expires"], reserved,
        ):
            raise _StatusTransitionRefused(
                f"task {task_id} changed before shutdown; refresh and retry")
        prev = reserved
        pid, claim_lock, pid_started, scope = (
            kanban_worker_identity._worker_termination_tuple(prev)
        )
        termination = kanban_worker_identity._terminate_reclaimed_worker(
            pid, claim_lock,
            scope_unit=scope or None,
            pid_started_at=pid_started, task_id=task_id, run_id=prev["current_run_id"],
        )
        if kanban_worker_identity._worker_survived_termination(termination):
            kanban_worker_identity._defer_reclaim_for_live_worker(
                conn, task_id, prev["claim_lock"], int(time.time()),
                termination, expected_run_id=prev["current_run_id"],
                reason=(
                    f"dashboard_status_{new_status}_stop_unconfirmed"
                ),
            )
            if scope:
                kanban_worker_handoff._mark_run_scope_stopping(
                    conn, task_id, scope,
                    reason="dashboard_status_stop_unconfirmed",
                    expected_run_id=prev["current_run_id"],
                )
            raise _StatusTransitionRefused(
    ...
    with kanban_db_connect.write_txn(conn):
        cur = conn.execute(
            ...
            "  reclaim_reserved_at = CASE WHEN ? = 'running' "
            "      THEN reclaim_reserved_at ELSE NULL END, "
            "  last_heartbeat_at = CASE WHEN ? = 'running' "
            "      THEN last_heartbeat_at ELSE NULL END "
            "WHERE id = ? AND status IS ? AND claim_lock IS ? AND current_run_id IS ? "
    ...
    for pid, claim_lock, pid_started, scope in terminations:
        kanban_worker_identity._terminate_reclaimed_worker(
            pid, claim_lock,
            scope_unit=scope or None,
...
def _bulk_apply_one(conn, tid: str, payload: BulkTaskBody, board: Optional[str], entry: dict) -> None:
    ...
    if payload.archive and not _owner_kanban_transitions.archive_task(conn, tid):
    ...
    if payload.assignee is not None:
        try:
            ok = (_owner_kanban_claims.reassign_task(conn, tid, payload.assignee or None, reclaim_first=True) if payload.reclaim_first
...
def terminate_run_endpoint(run_id: int, payload: TerminateRunBody, board: Optional[str] = _BOARD_Q):
    ...
    with _board_conn(board) as (board, conn):
        ...
        if not _owner_kanban_claims.reclaim_task(conn, r.task_id, reason=payload.reason, expected_run_id=run_id):
...
def reclaim_task_endpoint(task_id: str, payload: ReclaimBody, board: Optional[str] = Query(None)):
    ...
    with _board_conn(board) as (board, conn):
        if not _owner_kanban_claims.reclaim_task(conn, task_id, reason=payload.reason):
...
def reassign_task_endpoint(task_id: str, payload: ReassignBody, board: Optional[str] = Query(None)):
    ...
    with _board_conn(board) as (board, conn):
        ok = _owner_kanban_claims.reassign_task(
...
def get_stats(board: Optional[str] = Query(None)):
    ...
    with _board_conn(board) as (board, conn):
        return _owner_kanban_stats.board_stats(conn)
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    with _board_conn(board) as (board, conn):
        return {"assignees": _owner_kanban_stats.known_assignees(conn)}
...
def get_task_log(task_id: str, tail: Optional[int] = Query(None, ge=1, le=2_000_000), board: Optional[str] = Query(None)):
    ...
    with _board_conn(board) as (board, conn):
        _require_task(conn, task_id)
    content = _owner_kanban_stats.read_worker_log(task_id, tail_bytes=tail, board=board)
    log_path = _owner_kanban_stats.worker_log_path(task_id, board=board)
...
def _board_counts(slug: str) -> dict[str, int]:
    ...
    try:
        if not _owner_kanban_db.kanban_db_path(board=slug).exists():
            return {}
...
def list_boards(include_archived: bool = Query(False)):
    ...
    boards = _owner_kanban_boards.list_boards(include_archived=include_archived)
    current = _owner_kanban_boards.get_current_board()
...
def create_board_endpoint(payload: CreateBoardBody):
    ...
    with _value_error_400():
        meta = _owner_kanban_boards.create_board(
            payload.slug, default_workdir=default_workdir, project_id=project_id, **_board_display_kwargs(payload))
    if payload.switch:
        with _value_error_400():
            _owner_kanban_boards.set_current_board(meta["slug"])
    return {"board": _annotate_board_meta(meta), "current": _owner_kanban_boards.get_current_board()}
...
def rename_board(slug: str, payload: RenameBoardBody):
    ...
    meta = _owner_kanban_boards.write_board_metadata(
...
def delete_board(slug: str, delete: bool = Query(False, description="Hard-delete instead of archive")):
    ...
    with _value_error_400():
        res = _owner_kanban_boards.remove_board(slug, archive=not delete)
    return {"result": res, "current": _owner_kanban_boards.get_current_board()}
...
async def export_board_endpoint(slug: str, body: ExportBoardBody):
    ...
    if not output:
        staging = _owner_kanban_boards.kanban_home() / "kanban" / "board-exports"
        try:
...
async def import_board_endpoint(body: ImportBoardBody):
    ...
    result = await _run_transfer(
        ...
        "POST /boards/import")
    return {**result, "current": _owner_kanban_boards.get_current_board()}
...
def switch_board(slug: str):
    ...
    normed = _existing_board_slug(slug)
    _owner_kanban_boards.set_current_board(normed)
...
def _ws_board(raw: Optional[str]) -> Optional[str]:
    try:
        return _owner_kanban_boards._normalize_board_slug(raw) if raw else None
    except ValueError:
        return None
