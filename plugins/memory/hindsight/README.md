...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{chat}`, `{session}`. Example: `hermes-{profile}-{platform}-{chat}` isolates gateway chats while retaining cross-session recall inside each chat. Empty placeholders collapse cleanly (e.g. `hermes-{user}` with no user becomes `hermes`). |
