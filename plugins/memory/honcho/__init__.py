...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def _format_first_turn_context(self, ctx: dict) -> str:
        ...
        Peer-card / representation sections come from Honcho as free-form text
        with no schema validation, so they are sanitized on render (see
        ``_sanitize_card_lines``). ``summary`` is session-summary text produced
        by Honcho itself and is rendered verbatim.
        """
        parts: List[str] = []
        for key, header in _CONTEXT_SECTIONS:
            value = ctx.get(key, "")
            if not value:
                continue
