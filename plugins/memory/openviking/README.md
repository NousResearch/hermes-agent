...
| `OPENVIKING_API_KEY` | (none) | User/admin key, or a trusted-mode root key |
...
In API-key mode, OpenViking derives account/user identity from user and admin
keys. Trusted-mode root-key setups require `OPENVIKING_ACCOUNT` and
`OPENVIKING_USER`; Hermes sends those identity headers when the server requests
trusted identity. Local deployments without a key also use those values.
