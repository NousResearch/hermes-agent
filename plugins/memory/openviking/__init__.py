...
class _VikingClient:
    ...
    def health(self) -> bool:
        try:
            identity, _health = _probe_openviking_identity(
                self,
                authenticated=_is_openviking_service_endpoint(self._endpoint)
    ...
    def authenticated_health_payload(self) -> dict:
        """Probe health with configured credentials for the managed service gateway."""
...
def _probe_openviking_identity(
    ...
    *,
    ...
    health = (
        client.authenticated_health_payload()
        if authenticated
        else client.health_payload()
    )
...
def _validate_openviking_setup_values(
    ...
    *,
    ...
    try:
        ...
        identity, health = _probe_openviking_identity(
            client,
            authenticated=_is_openviking_service_endpoint(endpoint) and bool(api_key),
        )
        if identity == _OPENVIKING_IDENTITY_UNHEALTHY:
...
def _classify_runtime_openviking_health(client: _VikingClient, endpoint: str) -> tuple[str, str]:
    ...
    try:
        if hasattr(client, "health_payload"):
            identity, _health = _probe_openviking_identity(
                client,
                authenticated=_is_openviking_service_endpoint(endpoint),
            )
            if identity == _OPENVIKING_IDENTITY_UNHEALTHY:
                return (
                    "responded",
