...
class DiscordAdapter(BasePlatformAdapter):
    ...
    VOICE_TIMEOUT = 300
    # Minimum seconds to wait for one voice playback. The effective timeout
    # scales with clip duration so long readbacks are not cut off at two minutes.
...
# fail bots that never enabled Members Intent in the Developer Portal.
    ...
    async def play_in_voice_channel(self, guild_id: int, audio_path: str) -> bool:
        ...
        # bot while duration probing, floor acquisition, decoding, or speaking;
        # re-arm it when this attempt finishes, including on failure.
        ...
        try:
            ...
            if mixer is not None:
                ...
                if pcm:
                    ...
                    # replies while ambient audio continues underneath.
                    wait_start = time.monotonic()
    ...
    async def _voice_timeout_handler(
        ...
        # the channel. Skip auto-disconnect while voice replies are disabled.
            ...
            for batch_idx in range(0, len(valid_media_paths), _MEDIA_CHUNK):
                ...
                def _build_form(_batch=batch):
                    ...
                    for _idx, _path in enumerate(_batch):
                        _fh = open(_path, "rb"); handles.append(_fh)
                        form.add_field(f"files[{_idx}]", _fh, filename=os.path.basename(_path))
                    return form, handles

                try:
                    batch_data, batch_err = await _standalone_post_multipart_with_429_retry(session, url, headers=auth_headers, form_factory=_build_form, req_kw=_req_kw, error_label=f"Discord media batch ({batch_idx // _MEDIA_CHUNK + 1})")
                    if batch_err is not None:
                        warning = _standalone_sanitize_error(batch_err); logger.error(warning); warnings.append(warning); continue
                    last_data = batch_data
                    if caption_pending: caption_pending = False
                except Exception as e:
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _voice_keys_to_bridge = ("auto_join_user_id", "auto_join_text_channel_id", "auto_join_channel_id", "auto_join_delay_seconds", "auto_join_greeting_text", "auto_join_send_text_greeting", "auto_leave_on_user_exit", "voice_timeout_seconds", "multi_agent_voice_channel_id", "voice_floor_ttl_seconds", "voice_log_only")
