...
def _probe_audio_duration_seconds(audio_path: str) -> Optional[float]:
    ...
    because:
      - Hermes voice flows sometimes hand ``send_voice`` an MP3 / WAV /
        other container (Edge TTS, OpenAI TTS, ElevenLabs non-Opus), and
        ``OggOpus(non_ogg_file)`` raises — we fall through to the
...
def _voice_message_waveform_enabled() -> bool:
    ...
    to skip the ffmpeg decode work entirely and always ship a flat
    128-byte waveform. Useful for tight-budget deployments or when
    ffmpeg isn't worth the cycles per voice message.  Default
...
def _compute_voice_message_waveform(audio_path: str, duration_secs: float) -> bytes:
    ...
    decode error, empty audio) falls back to a flat
    128-waveform — Discord accepts it and the voice message still sends.

    Operators can also opt out entirely via
    ``DISCORD_VOICE_MESSAGE_WAVEFORM=false`` to skip the ffmpeg decode
    ...
    # Operator opt-out: skip the whole ffmpeg decode pipeline.
    ...
    try:
        raw = proc.stdout
        # Malformed PCM (odd byte count) → fall back rather than silently
        # drop a trailing byte.
        if len(raw) % 2 != 0:
            return fallback
        ...
        # Handle clips shorter than target_samples (e.g. a 5-sample PCM
        # blob with target_samples=30): shrink target_samples to match
        # rather than fail. Discord accepts fewer samples.
        if num_samples < target_samples:
            target_samples = max(1, num_samples)

        # Window into target_samples buckets, compute per-bucket RMS,
        # map RMS → dBFS → uint8. Pure-Python (struct + math) — no
        # optional numpy dependency needed for the waveform computation.
        window_size = max(1, num_samples // target_samples)
        waveform = bytearray(target_samples)
        for i in range(target_samples):
            start = i * window_size
            end = start + window_size
            chunk = raw[start * 2 : end * 2]
            samples = struct.unpack("<{}h".format(window_size), chunk)
            sum_sq = sum(s * s for s in samples)
            rms = math.sqrt(sum_sq / window_size)
            # RMS → dBFS → uint8. int16 full-scale peak is 32767.
            rms_clamped = max(rms, 1.0)  # avoid log10(0)
            dbfs = 20.0 * math.log10(rms_clamped / 32767.0)
            # Map [FLOOR, 0] → [0, 255] linearly, clamp.
            normalized = ((dbfs - _VOICE_WAVEFORM_FLOOR_DBFS) / (-_VOICE_WAVEFORM_FLOOR_DBFS)) * 255.0
            waveform[i] = max(0, min(255, int(normalized)))
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                # Offload ffmpeg decode + RMS computation to a thread so a slow
                ...
                # in this adapter for voice-side work (pcm_to_wav, STT).
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    # RMS work and always ship a flat 128-byte waveform.
