...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _auto_join_voice_for_member(self, member, before, after) -> None:
        ...
        if was_in_configured_channel and not is_in_configured_channel:
            departing_id = str(getattr(member, "id", ""))
            another_allowed_member_remains = any(
                str(getattr(candidate, "id", "")) in user_ids
                and str(getattr(candidate, "id", "")) != departing_id
                for candidate in getattr(before_channel, "members", [])
            )
            if another_allowed_member_remains:
                return
