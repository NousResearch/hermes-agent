...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_with_files(self, to_addr: str, body: str, files: List[Tuple[Path, str]], *, lenient: bool, subject_override: Optional[str] = None) -> str:
        """Send a reply with attachments; *lenient* logs-and-skips unattachable files instead of raising."""
        msg, msg_id, _ = self._new_reply(to_addr, body, subject_override=subject_override)
    ...
    async def send_document(self, chat_id: str, file_path: str, caption: Optional[str] = None,
        ...
        return await self._run_send(self._send_email_with_attachment, (chat_id, caption or "", file_path, file_name, subject_override), "[Email] Send document failed: %s")

    def _send_email_with_attachment(self, to_addr: str, body: str, file_path: str, file_name: Optional[str] = None, subject_override: Optional[str] = None) -> str:
        """Send an email with a single file attachment via SMTP (raises if unattachable)."""
        return self._send_with_files(to_addr, body, [(Path(file_path), file_name or Path(file_path).name)], lenient=False, subject_override=subject_override)
