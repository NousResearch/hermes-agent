...
# NetEase/163 uses ``smtp.mail``; some MTAs use ``mailfrom`` or
...
# ``client-ip`` is deliberately not matched: it carries an IP address, which is
# never a comparable sender domain.
...
def _first_aligned_domain(values: List[str], from_domain: str) -> str:
    ...
    OR/first-match semantics: any candidate value whose domain aligns with the
    visible From domain authenticates the message, and the first such value in
    the provider's property order wins. This intentionally accepts a later
    alias when an earlier one is misaligned (e.g. a duplicated property), which
    is slightly more permissive than a last-wins parser but keeps provider
    alias compatibility; alignment against the visible From domain is still
    always required.
    """
    for value in values:
...
# Match a single "method=result" token anywhere in a clause. Used only as a
# fallback for malformed headers that mix the authserv-id and the first method
# clause without a separating ``;`` (e.g. ``mx.example.com spf=pass ...``).
# Restricted to the methods Hermes evaluates so property names like
# ``mailfrom=`` are never misread as methods.
_AUTH_LEGACY_METHOD_RE = re.compile(
    r"\b(dmarc|dkim|spf)\s*=\s*([a-z]+)\b", re.IGNORECASE
)
...
def _parse_auth_method_results(
    ...
    # RFC 8601 places the authserv-id first (e.g. ``mx.example.net``), but some
    # servers (Exchange Online, self-hosted MTAs) omit it and start directly
    # with ``method=result`` clauses. Only skip the first token when it cannot
    # be a method clause (contains no ``=``); a header with no ``;`` at all is
    # parsed as a single clause. A new method token, including one Hermes does
    # not evaluate such as ARC, always starts a fresh property scope.
    parts = trusted.split(";")
    ...
    for clause in parts:
        ...
        if method_match is None:
            # Malformed but common shape: the authserv-id and the first method
            # clause share one token without a ``;`` separator. Fall back to a
            # whitelisted non-anchored search so real SPF/DKIM/DMARC verdicts
            # still authenticate; the anchored path remains the security
            # boundary for property isolation.
            method_match = _AUTH_LEGACY_METHOD_RE.search(clause)
        if method_match:
            current = (
