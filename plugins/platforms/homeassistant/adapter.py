...
def _connect_error_detail(exc: BaseException) -> str:
    ...
    if errno_val == 65 or "No route to host" in text or "EHOSTUNREACH" in text:
        return (
            f"{text} — macOS is blocking this launchd gateway from the local network. "
            "Allow Hermes Gateway in System Settings → Privacy & Security → Local Network, "
            "then run `hermes doctor --fix` and `hermes gateway restart`. "
