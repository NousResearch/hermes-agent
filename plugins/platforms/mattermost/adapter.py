...
def _coerce_max_post_length(value: Any, default: int = DEFAULT_MAX_POST_LENGTH) -> int:
    """Return a safe Mattermost post length from config input."""
    try:
        limit = int(value)
...
def _resolve_max_post_length(extra: Optional[Dict[str, Any]] = None) -> int:
    """Resolve the effective Mattermost post length from ``config.yaml``."""
    raw = extra.get("max_post_length") if isinstance(extra, dict) else None
...
class MattermostAdapter(BasePlatformAdapter):
    ...
    splits_long_messages = True  # send() chunks via self.max_post_length
    ...
    async def _post_preserving_thread(
        ...
        warning_payload = dict(flat_payload)
        ...
        # Keep the warning separate from the answer chunk.  Prefixing and then
        # re-chunking can expose part of the answer before a later flat chunk
        # fails, losing the last visible id and causing a full retry to
        # duplicate that partial content.  A warning-only partial success is
        # harmless: the gateway may retry the complete answer without
        # duplicating any answer text.
        try:
            await self._api_post("posts", warning_payload)
        except Exception as e:
            logger.warning(
                "Mattermost: flat-fallback warning post failed in %s: %s",
                chat_id,
                e,
            )
        flat_payload["message"] = original
    ...
    async def send(
        ...
        if (
            ...
            and self.message_len_fn(formatted) > self.max_post_length
        ):
            # Streaming must have exactly one owner for overflow splitting.
            # Returning before any post lets StreamConsumer/the gateway split
            # or fall back without Mattermost silently creating continuation
            # posts whose ids the streaming editor cannot safely adopt.
            return SendResult(
                success=False,
                error="Mattermost streaming preview exceeds configured post limit",
            )
...
def _apply_yaml_config(yaml_cfg: dict, mattermost_cfg: dict) -> dict | None:
    ...
    Existing env vars take precedence over YAML for credential/connection
    settings. ``max_post_length`` is behavioral config, so it is validated and
    returned as ``PlatformConfig.extra`` without creating or mutating an env
    variable.
    """
    seeded: Dict[str, Any] = {}

    if "max_post_length" in mattermost_cfg:
        coerced_limit = _coerce_max_post_length(mattermost_cfg["max_post_length"])
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # direct sends can override this via ``mattermost.max_post_length`` in
        # config.yaml, clamped to Mattermost's 16,383-character cap.
