...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def send_with_effect(
        ...
        *,
        ...
        """Send an iMessage bubble/screen effect (slam, confetti, fireworks, …)."""
        body = {
            "spaceId": chat_id,
            "text": self.format_message(content),
            "effect": effect,
            "format": "markdown" if markdown and _markdown_enabled() else "text",
        }
        try:
    ...
    async def reply_to_message(
        ...
        *,
        ...
        """Send an iMessage native threaded reply."""
        body = {
            "spaceId": chat_id,
    ...
    async def edit_native_message(
        ...
        *,
        ...
        """Edit one of Hermes' previous iMessage messages."""
        body = {
            "spaceId": chat_id,
            "messageId": message_id,
            "text": self.format_message(content),
    ...
    async def unsend_message(self, chat_id: str, message_id: str) -> SendResult:
        """Unsend one of Hermes' previous iMessage messages."""
        try:
    ...
    async def send_poll(
        ...
        """Send a native iMessage poll."""
        try:
            data = await self._sidecar_call(
                "/send-poll",
                {"spaceId": chat_id, "question": question, "options": options},
            )
        except Exception as e:
            return SendResult(success=False, error=str(e))
        self._record_sent_message(data.get("messageId"))
    ...
    async def send_mini_app(
        ...
        *,
        ...
        """Send a customized iMessage mini-app card."""
        body: Dict[str, Any] = {
            "spaceId": chat_id,
            "appName": app_name,
    ...
    async def send_contact(
        ...
        **contact_fields: Any,
        ...
        """Share a native iMessage contact card."""
        body: Dict[str, Any] = {"spaceId": chat_id, **contact_fields}
        try:
            data = await self._sidecar_call("/send-contact", body)
        except Exception as e:
            return SendResult(success=False, error=str(e))
        self._record_sent_message(data.get("messageId"))
        return SendResult(success=True, message_id=data.get("messageId"))
    ...
    async def set_background(
        ...
        *,
        ...
        """Set or clear an iMessage conversation background."""
        body: Dict[str, Any] = {"spaceId": chat_id, "clear": clear}
        if background:
            body["background"] = background
        try:
            data = await self._sidecar_call("/set-background", body)
        except Exception as e:
            return SendResult(success=False, error=str(e))
        self._record_sent_message(data.get("messageId"))
        return SendResult(success=True, message_id=data.get("messageId"))
