"""Opt-in local Microsoft Teams/ Bot Framework playground seam.

The repository does not contain the Microsoft 365 Agents Playground wire protocol.
This module therefore exposes a generic Bot Framework-compatible activity contract:
``GET /health`` for a handshake and ``POST /api/messages`` for activity envelopes.
A future adapter can replace those paths without changing production Teams config.
"""
...
class PlaygroundConfig:
    ...
    def from_extra(cls, extra: dict | None) -> "PlaygroundConfig":
        extra = extra or {}
        raw = extra.get("playground_url") or extra.get("base_url")
        ...
        return cls(enabled=bool(url), base_url=url, allow_private=allow)
...
def validate_playground_url(raw: str, *, allow_private: bool = False) -> str:
    ...
    if parsed.query or parsed.fragment:
        raise PlaygroundValidationError("Playground URL cannot contain a query or fragment.")
...
def generated_test_url(config: PlaygroundConfig) -> str | None:
    if not config.enabled or not config.base_url:
        return None
    return f"{config.base_url}/hermes/teams-playground"


def generated_callback_url(config: PlaygroundConfig) -> str | None:
    if not config.enabled or not config.base_url:
        return None
    return f"{config.base_url}/api/messages"
