...
def check_telegram_requirements() -> bool:
    ...
    try:
        ...
        InlineQueryResultArticle, InputTextMessageContent = (
            getattr(_tg, n) for n in ("InlineQueryResultArticle", "InputTextMessageContent"))
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def edit_message(
        ...
        # Guest mode: stream consumer drives progressive edits via inline_message_id. Intercept
        # before any path that calls int(chat_id)/int(message_id) — the inline_message_id is a
        # string like "AAMCAgAD..." that cannot be cast to int — and before the shared outbound-slot
        # budget below, which has no meaning for a guest chat (no chat-scoped send/edit budget: the
        # bot never joined the chat, so there is no chat to hold a slot for).
