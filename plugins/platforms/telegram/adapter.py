...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _on_platform_update(self, update, context) -> None:
        ...
        try:
            ...
            # Skip per-update normalization work when nothing can consume the
            # event: no hook subscriber AND no configured reaction action for
            # this platform (the gateway boundary holds the authoritative
            # action mapping; this is just the inertness gate).
