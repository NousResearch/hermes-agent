    ...
    async def _polling_heartbeat_loop(self) -> None:
        ...
        *general* request path (not the getUpdates pool), so a healthy long-poll
        ...
        while True:
            ...
            # Issue #62047: PTB 22.6 raises telegram.error.NetworkError (and
            # telegram.error.TimedOut, which inherits from NetworkError) on
            # connectivity failures. These are NOT OSError, so they fall
            # through the except clause above and would otherwise be
            # silently swallowed by the generic except Exception below.
