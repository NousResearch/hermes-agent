...
class TelegramFallbackTransport(httpx.AsyncBaseTransport):
    ...
    def __init__(self, fallback_ips: Iterable[str], *, transport_factory=None, **transport_kwargs):
        ...
        # Production supplies its fenced backend; retain the ordinary helper's
        # default and recovery policy for callers without live-todo capability.
        ...
        for ip in self._attempt_order():
            ...
            try:
                ...
                if admission is not None and admission.dispatched:
                    # Never reinterpret a possibly written operation as a safe
                    # connection retry (including proxy/TLS continuation failures).
                    raise
    ...
    @staticmethod
    def _note_uninstrumented_dispatch(transport):
        admission = operation_admission.get()
        if admission is not None and not isinstance(transport, AdmissionHTTPTransport):
            # Only the adapter's backend can prove no bytes were written. Foreign
            # transports (including SDK test doubles) are conservative, never unsent.
