...
HTTPX's request hook and httpcore's header trace precede pool/stream awaits.
Use httpcore's public network-backend interface instead: drain first, then
check and synchronously enqueue bytes with StreamWriter.write under the fence.
No suspension is allowed between those last two operations. Bytes enqueued
before stop are possibly dispatched, even if drain/receipt subsequently fails.
"""
...
class OperationAdmission:
    ...
    def check(self):
        if not self.source.admitted():
            raise AdmissionRevoked("live todo operation revoked")

    def write(self, writer, data):
        source = self.source
        with source.registration.lock, source.lock:
            self.check()
            # Set BEFORE enqueue: even a synchronous write error is conservative.
            self.dispatched = True
            writer.write(data)


operation_admission: ContextVar[OperationAdmission | None] = ContextVar("telegram_operation_admission", default=None)
...
def map_core_errors():
    ...
    except (httpcore.TimeoutException, httpcore.ProtocolError,
...
class AdmissionStream(httpcore.AsyncNetworkStream):
    ...
    async def write(self, buffer, timeout=None):
        ...
        try:
            async with asyncio.timeout(timeout):
                # This await includes flow control and connection-loss checkpoints.
    ...
    async def aclose(self):
        self.writer.close()
        # Do not let TLS shutdown block a failed pool's recovery indefinitely.
        try:
    ...
    def get_extra_info(self, info):
        if info == "is_readable":
            # Error-driven connection_lost sets an exception, not EOF. A closed
            # idle stream must expire rather than poison httpcore's pool scan.
            if self.reader.at_eof() or self.reader.exception() is not None or self.writer.is_closing():
                return True
...
class AdmissionHTTPTransport(httpx.AsyncBaseTransport):
    """Adapter-owned HTTP/1 pool, using public HTTPX/httpcore transport APIs.

    Telegram's SDK defaults to HTTP/1.1. Keeping that invariant avoids sharing
    a multiplexed writer across operation contexts. Ordinary requests have no
    admission context and retain connection retries and fallback recovery.
    """
    def __init__(self, *, limits=None, socket_options=None, proxy=None, retries=0,
        ...
        if proxy is not None:
            core_proxy = httpcore.Proxy(str(proxy.url), auth=proxy.raw_auth,
                                       headers=proxy.headers.raw, ssl_context=proxy.ssl_context)
        self.pool = httpcore.AsyncConnectionPool(
            ssl_context=httpx.create_ssl_context(verify=verify, cert=cert, trust_env=trust_env),
            proxy=core_proxy, max_connections=limits.max_connections,
            max_keepalive_connections=limits.max_keepalive_connections,
            keepalive_expiry=limits.keepalive_expiry, http1=True, http2=False,
            retries=retries, socket_options=socket_options, network_backend=AdmissionBackend())
