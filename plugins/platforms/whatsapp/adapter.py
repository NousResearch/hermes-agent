...
# Automatic inbound ack: a fun-emoji reaction fired by the adapter (not the
# agent) on every accepted inbound DM. The message text is matched against
# thematic context rules first (✈️ travel, ✉️ email, 🎫 tickets, ...); when
# nothing matches, a small rotation keeps consecutive acks varied. Kept in
# code as a data table so one edit changes the mapping; no config gate.
_AUTO_REACT_EMOJIS = ("🔥", "⚡️", "🎯", "🙂", "👍")

# Ordered (keywords, emoji) pairs for the thematic ack. First match wins, so
# specific domains precede generic ones — "search hotels" acks 🏨, not 🔍.
# Keywords are matched whole-word and case-insensitive, so "mail" never fires
# inside "email" and "fly" never fires inside "butterfly".
_AUTO_REACT_CONTEXT_RULES = (
    (("flight", "flights", "fly", "flies", "flying", "airport", "airline", "airlines", "boarding"), "✈️"),
    (("hotel", "hotels", "airbnb", "motel", "checkin", "check-in", "check in", "lodging", "resort"), "🏨"),
    (("trip", "trips", "travel", "vacation", "itinerary", "sightseeing"), "🧳"),
    (("mau5trap", "mau5", "deadmau5"), "🐭"),
    (("music", "track", "tracks", "release", "releases", "remix", "remixes", "song", "songs", "album", "playlist", "demo", "demos", "mastering", "mixdown"), "🎵"),
    (("money", "pay", "paid", "payment", "payments", "invoice", "invoices", "royalty", "royalties", "fee", "fees", "budget", "price", "pricing", "quote"), "💰"),
    (("ticket", "tickets", "queue", "kanban", "dispatch", "triage"), "🎫"),
    (("meeting", "meet", "call", "calls", "calendar", "schedule", "scheduling", "agenda", "appointment", "standup", "sync"), "📅"),
    (("email", "emails", "mail", "gmail", "inbox", "outlook", "draft"), "✉️"),
    (("code", "codes", "git", "repo", "repos", "deploy", "deployment", "bug", "bugs", "debug", "debugging", "server", "script", "scripts", "build", "api", "endpoint", "commit", "push", "branch"), "💻"),
    (("report", "reports", "metric", "metrics", "analytics", "dashboard", "data", "spreadsheet", "sheet", "sheets"), "📊"),
    (("search", "searches", "searching", "find", "finding", "look up", "lookup", "google", "research", "dig up"), "🔍"),
    (("voice", "voicenote", "voice note", "audio", "transcribe", "transcription", "listen"), "🎙️"),
    (("whatsapp", "telegram", "discord", "dm", "dms", "message", "messages", "text", "texts"), "💬"),
    (("news", "headlines", "digest", "brief", "briefing", "update", "updates"), "📰"),
    (("weather", "rain", "forecast", "temperature", "sunny"), "🌦️"),
    (("food", "dinner", "lunch", "breakfast", "restaurant", "coffee", "recipe", "recipes"), "🍽️"),
    (("phone", "dial", "ring"), "📞"),
    # Generic aside/detail cues always ack 👍 so Jimi knows the addition is
    # factored in. Appended LAST so specific domains keep priority ("search
    # hotels" still 🏨). "task"/"tasks" were dropped from the 🎫 rule above so
    # the "cue task" phrase reaches this rule instead of being shadowed by a
    # bare "task" match.
    (("by the way", "btw", "cue task", "for context", "for reference", "also", "one more thing", "just so you know", "extra detail", "note that"), "👍"),
)

# Precompiled once at import; one regex per rule with whole-word boundaries
# applied to every alternative.
_AUTO_REACT_CONTEXT_PATTERNS = tuple(
    (
        re.compile(
            r"\b(?:{})\b".format("|".join(re.escape(kw) for kw in keywords)),
            re.IGNORECASE,
        ),
        emoji,
    )
    for keywords, emoji in _AUTO_REACT_CONTEXT_RULES
)


def _pick_context_react_emoji(text: str) -> Optional[str]:
    """Return the thematic ack emoji for an inbound message, or None.

    Whole-word, case-insensitive keyword match; the first rule that hits
    wins. None means "no theme" — the caller falls back to the rotation.
    """
    if not text:
        return None
    for pattern, emoji in _AUTO_REACT_CONTEXT_PATTERNS:
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._auto_react_index = 0  # rotation cursor for the themed inbound ack
    ...
    # ── Processing lifecycle reactions (native emoji ack) ───────────────
    #
    # Opt-in emoji set for the shared reaction-ack flow in base.py's
    # ``on_processing_complete``: 👍 fires when background processing begins
    # (``on_processing_start`` below), then the base hook swaps it for
    # ✅/❌ on completion. Gate the whole flow with WHATSAPP_REACTIONS
    # (default true — restores the pre-restart behaviour where Toby
    # natively reacted to every message it processed).

    _ACK_EMOJI = "\U0001f44d"  # 👍 receipt ack
    _OK_EMOJI = "\u2705"       # ✅ processing succeeded
    _FAIL_EMOJI = "\u274c"     # ❌ processing failed

    def _reactions_enabled(self) -> bool:
        """Env gate for the automatic per-message reaction acks."""
        return _wenv("WHATSAPP_REACTIONS", "true").strip().lower() not in {
            "false", "0", "no", "off",
        }

    async def _add_reaction(self, chat_id: str, message_id: str, emoji: Optional[str]) -> bool:
        """Native emoji reaction via the bridge. Soft-fails (False), never raises."""
        if not self._running or not self._http_session:
            return False
        ...
        try:
            ...
            # Must wrap in `async with` — a bare await leaves the response
            # object alive until GC, holding its TCP socket in CLOSE_WAIT
            # (see #18451).
            async with self._http_session.post(
                ...
                if resp.status == 200:
                    return True
                logger.debug(
                    "[%s] add_reaction (%s) failed with HTTP %s",
                    self.name,
                    emoji,
                    resp.status,
                )
    ...
    async def _remove_reaction(self, chat_id: str, message_id: str) -> bool:
        """Retract our reaction from a message (empty-emoji react). Soft-fails."""
        return await self._add_reaction(chat_id, message_id, "")

    async def on_processing_start(self, event: MessageEvent) -> None:
        """Ack the triggering message while background processing runs.

        Themed from the message text (✈️ travel, ✉️ email, 🎫 tickets, ...),
        falling back to a small rotation when nothing matches. The ✅/❌
        completion swap in ``base.py`` is untouched.
        """
        ...
        if not (chat_id and message_id):
            return
        # Loop safety: DMs only (never react in group chats), never react to a
        # reaction-type message (reacting to a reaction would echo another
        # reaction). The bot's own outbound echo is already dropped at the
        # bridge, so it never reaches this hook.
        if getattr(event.source, "chat_type", None) == "group":
            return
        metadata = getattr(event, "metadata", None) or {}
        if metadata.get("whatsapp_native_type") == "reactionMessage":
            return
        emoji = _pick_context_react_emoji(getattr(event, "text", "") or "")
        if emoji is None:
            emoji = _AUTO_REACT_EMOJIS[self._auto_react_index % len(_AUTO_REACT_EMOJIS)]
            self._auto_react_index += 1
        await self._add_reaction(chat_id, message_id, emoji)
