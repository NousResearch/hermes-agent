...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def _resolve_channel_skills(self, chat_id: str) -> list[str] | None:
        """Resolve auto-loaded skills for a WhatsApp chat from config."""
        return resolve_channel_skills(self.config.extra, chat_id)

    def _resolve_channel_prompt(self, chat_id: str) -> str | None:
        """Resolve per-channel prompt for a WhatsApp chat from config."""
    ...
    async def _build_message_event(self, data: Dict[str, Any]) -> Optional[MessageEvent]:
        ...
        try:
            ...
            chat_id = data.get("chatId", "")
            _channel_prompt = self._resolve_channel_prompt(chat_id)
            _auto_skill = self._resolve_channel_skills(chat_id)
