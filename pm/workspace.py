...
def _copy_core_inputs(source: Path, destination: Path) -> None:
    ...
    # The managed venv reads its version identity from the snapshot, not the
    # checkout: without these the venv CLI reports vunknown (#122425).
    for stamp in ("install-stamp.json", ".install_method"):
        if (source / stamp).is_file():
            files.add(stamp)

    # Only directory exclusions apply below the root. The root lock is
    # regenerated by the workspace (it is never in ``files``), while a nested
    # project's lock (e.g. ``pm/uv.lock``, which PM hashes for its own runtime
    # identity) is a build input and must survive (#122425). Root dotfiles and
    # checkout state never enter -- only package dirs and ``files`` are copied --
    # but a nested dotfile ships with its package.
...
def sync_sources(project_root: Path, *, source: Path | None = None,
                 plugin_dirs: list[Path] | Mapping[Path, Path] | None = None) -> Path:
    """Refresh the committed generation's code snapshot from *source*; keep its lock.

    ``hermes update`` moves the checkout, but a code-only change leaves the venv
    stamp current, so the sync is a no-op and the committed workspace snapshot
    keeps running the old code (#122425). This re-snapshots the core inputs
    (and the current member set) over the committed generation, then re-syncs
    its environment frozen: the dependency graph never re-resolves.

    Returns the refreshed workspace root. Anything without a committed
    generation is an InstallError naming ``hermes pm install``; a refreshed
    snapshot the preserved lock no longer fits restores the previous snapshot
    and names the same remedy.
    """
    import uuid
    ...
    recorded = Path(fact["resolved_lock"]).resolve()
    environment_path = Path(fact["environment"]).resolve()
    generations = install_state_dir(project) / "environments"
    workspace = recorded.parent
    if (workspace.name != "workspace" or workspace.parent.parent.resolve() != generations.resolve()
            or recorded != workspace / "uv.lock" or not recorded.is_file()):
        raise InstallError("venv", "recorded dependency lock is missing", "run `hermes pm install`")
    if not (workspace / "pyproject.toml").is_file():
        raise InstallError("venv", f"recorded workspace is missing: {workspace}",
                           "run `hermes pm install`")
    if environment_path.parent != workspace.parent or not (environment_path / "pyvenv.cfg").is_file():
        raise InstallError("venv", "committed dependency environment is missing",
                           "run `hermes pm install`")
    if plugin_dirs is None:
        plugin_dirs = enabled_member_dirs()

    generation = workspace.parent
    saved_lock = recorded.read_bytes()
    # A killed refresh must not pile up staging dirs; only this run's names are live.
    for leftover in (*generation.glob("workspace.refresh-*"), *generation.glob("workspace.superseded-*")):
        shutil.rmtree(leftover, ignore_errors=True)
    staged = generation / f"workspace.refresh-{uuid.uuid4().hex}"
    backup = generation / f"workspace.superseded-{uuid.uuid4().hex}"
    _generate_pyproject(plugin_dirs, staged, source=source)
    (staged / "uv.lock").write_bytes(saved_lock)
    # Sibling generations keep external relative paths at the same depth, so
    # stage beside the workspace and swap: the live path never changes depth.
    ...
    try:
        ...
        environment = managed_environment(environment_path, env=source_build_environment(source),
                                          explicit=True)
        extras = fact.get("extras")
        environment.sync(workspace, extras=list(extras) if isinstance(extras, list) else [], frozen=True)
        environment.check()
    except BaseException as exc:
        rejected = generation / f"workspace.superseded-{uuid.uuid4().hex}"
        os.replace(workspace, rejected)
        os.replace(backup, workspace)
        shutil.rmtree(rejected, ignore_errors=True)
        if isinstance(exc, InstallError):
            raise
        raise InstallError("venv", f"workspace refresh failed: {exc}",
                           "run `hermes pm install` to re-resolve") from exc
