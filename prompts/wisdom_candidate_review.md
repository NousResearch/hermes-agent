...
Be concise and compelling: the goal is that the user wants to share it. Never include secrets, tokens, hostnames of private infrastructure, customer names, or file contents. If a skill appears to embed credentials, do not recommend it and mention `credential_shaped_content` in its portability notes only if it is otherwise recommended.
