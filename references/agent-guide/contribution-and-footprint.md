# Contribution and Footprint Reference

> Normative companion to the root `AGENTS.md`. Read this for every contribution, bug premise check, design review, or new capability decision.
...
- **Behavior contracts over snapshots.** Tests should assert how two pieces of
  ...
  config version literals, enumeration counts). See [Testing — Don’t write change-detector tests]
  (testing.md#dont-write-change-detector-tests).
