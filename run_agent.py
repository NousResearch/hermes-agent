...
class AIAgent:
    ...
    def __init__(
        ...
        init_agent(
            ...
            ask_user_questions_callback=ask_user_questions_callback,  # KENSEI CUSTOM
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``sys.modules`` lookup, so retries do not repay import cost.
        try:
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        ...
        mark_reconciled = getattr(self._todo_store, "mark_history_reconciled", None)
        if callable(mark_reconciled):
            # KENSEI CUSTOM: one-shot reconciliation gate (consumed by
            # agent/turn_context.py). Mark even when nothing was restored —
            # the revision guard inside the store protects correctness;
            # the flag only prevents re-scanning history every turn.
            mark_reconciled()
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
                     ...
                     tool_request_middleware_trace: Optional[list[dict[str, Any]]] = None, skip_tool_execution_middleware: bool = False) -> str:
