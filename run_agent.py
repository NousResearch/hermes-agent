...
class AIAgent:
    ...
    def _persist_session(
        ...
        # ADD: persist accumulated token counts
        if self._session_db and self.session_id:
            try:
                # Use a direct SET (not increment) since we track cumulative totals
                ...
                logger.debug("Token count persist failed: %s", e)
        ...
        def _call():
            try:
                ...
                if deltas_were_sent["yes"]:
                    ...
                    logger.warning(
                        "Streaming failed after partial delivery, not falling back: %s",
                        e,
                    )
                    result["error"] = e
                else:
