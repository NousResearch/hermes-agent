...
class AIAgent:
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        now = datetime.now()
        prompt_parts.append(f"Conversation started: {now.strftime('%A, %B %d, %Y %I:%M %p')}")
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        _supports_reasoning = "openrouter" in self.base_url.lower() or "nousresearch" in self.base_url.lower()
    ...
    def flush_memories(self, messages: list = None, min_turns: int = None):
        ...
        flush_content = (
            "[System: The session is being compressed. Please save anything worth remembering to your memories.]"
        )
        ...
        try:
            ...
            if _supports_reasoning:
                if self.reasoning_config is not None:
                    ...
                    summary_extra_body["reasoning"] = {"enabled": True, "effort": "xhigh"}
        ...
        while api_call_count < self.max_iterations:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    is_client_error = is_client_status_error or any(
                        phrase in error_msg
                        for phrase in [
                            "error code: 400",
                            "error code: 401",
                    ...
                    print(
                        f"⚠️  OpenAI-compatible API call failed (attempt {retry_count}/{max_retries}): {str(api_error)[:100]}"
                    )
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if invalid_tool_calls:
                        ...
                        invalid_preview = (
                            invalid_tool_calls[0][:80] + "..."
                            if len(invalid_tool_calls[0]) > 80
                            else invalid_tool_calls[0]
                        )
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if reasoning_text:
                            ...
                            content_preview = (
                                final_response[:80] + "..." if len(final_response) > 80 else final_response
                            )
