...
from agent.prompt_builder import build_skills_system_prompt, build_context_files_prompt, build_environment_hints, load_soul_md, TOOL_USE_ENFORCEMENT_GUIDANCE, TOOL_USE_ENFORCEMENT_MODELS, DEVELOPER_ROLE_MODELS, GOOGLE_MODEL_OPERATIONAL_GUIDANCE, OPENAI_MODEL_EXECUTION_GUIDANCE, get_grounding_contract
...
class AIAgent:
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Hallucination-zero grounding contract — injected unconditionally so the
        # Python-side agent is covered by the same contract as the TS ModelRouter path.
        # Re-evaluated each call so env-var changes (multi-tenant, dynamic config) are honored.
        _grounding_contract = get_grounding_contract()
        if _grounding_contract:
            prompt_parts.append(_grounding_contract)
