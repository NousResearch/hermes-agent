        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    # Minimax (even through Fireworks) sometimes emits
                    # malformed JSON for tool-call arguments — OpenRouter
                    # rejects it mid-stream with:
                    #   "minimax midstream error: invalid params,
                    #    invalid function arguments json string"
                    # Strategy:
                    #   retry 1 → normal retry (may succeed on second attempt)
                    #   retry 2 → strip last assistant message with tool_calls,
                    #             inject corrective tool-use reminder
                    #   retry 3 → trigger fallback to next model
                    if classified.reason == FailoverReason.invalid_tool_args:
                        if retry_count == 1:
                            self._vprint(
                                f"{self.log_prefix}⚠️  Model generated invalid tool-call "
                                f"JSON — retrying (attempt {retry_count}/{max_retries})...",
                                force=True,
                            )
                            # Normal retry; let the model self-correct
                        elif retry_count == 2:
                            self._vprint(
                                f"{self.log_prefix}⚠️  Model repeated invalid tool-call "
                                f"JSON — injecting corrective and retrying...",
                                force=True,
                            )
                            # Strip the last assistant message that had tool_calls
                            _stripped = 0
                            for _i in range(len(messages) - 1, -1, -1):
                                ...
                                if (
                                    ...
                                    messages.pop(_i)
                                    _stripped += 1
                                    break  # Only strip the most recent one
                            if _stripped > 0:
                                self._vprint(
                                    f"{self.log_prefix}🔧 Stripped {_stripped} bad assistant message(s)",
                                    force=True,
                                )
                            # Inject a corrective reminder into the system
                            _corrective = (
                                "IMPORTANT: When calling a tool, you MUST format "
                                "the arguments as valid JSON. Every argument value "
                                "must be proper JSON (strings in double quotes, "
                                "numbers without quotes, booleans as true/false, "
                                "no trailing commas, no unmatched brackets). "
                            ...
                            # Also drop the last user message if it was a tool
                            # result, to keep role alternation clean
                            if (
                                messages
                                and messages[-1].get("role") == "tool"
                            ):
                                messages.pop()
                        # else: retry_count >= 3 → falls through to fallback
