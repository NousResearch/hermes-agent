...
def status(env: dict[str, str]) -> None:
    ...
    for database in sorted({env["AGENT_DB_NAME"], env["AGENT_CALENDAR_DB_NAME"], env.get("AGENT_CRM_DB_NAME", env["AGENT_DB_NAME"]), env.get("AGENT_FITNESS_DB_NAME", env["AGENT_DB_NAME"])}):
        ensure_database(env, database)
        ensure_migration_ledger(env, database)
...
def main() -> None:
    ...
    if args.command == "up":
        ...
        print(f"Agent Core DB ready: {env['AGENT_DB_NAME']} + {env['AGENT_CALENDAR_DB_NAME']} + crm:{env.get('AGENT_CRM_DB_NAME', env['AGENT_DB_NAME'])} + fitness:{env.get('AGENT_FITNESS_DB_NAME', env['AGENT_DB_NAME'])}")
        ...
        for module in ["agent_core", "factory", "calendar", "crm", "fitness"]:
