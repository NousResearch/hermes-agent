...
#   <out>/repo-runtime-logs/ any <repo>/logs/*.log|*.jsonl  (only if present)
#   <out>/*.md               LEFT UNTOUCHED - the plain-language session reports humans drop here
...
# Needs: bash, coreutils. Uses git if on PATH (else snapshot skipped, noted). Zips via zip|python3|python.
set -euo pipefail
...
[ "$rt_count" -gt 0 ] && add_check OK "copied $rt_count repo runtime log(s) into repo-runtime-logs/"
...
IDX="$OUT/HANDOFF-INDEX.md"
{
  ...
  echo "## Contents"
  echo
  ( cd "$OUT" && find . -type f ! -name 'HANDOFF-INDEX.md' | sed 's#^\./##' | sort | while IFS= read -r r; do
      sz="$(wc -c <"$OUT/$r" | tr -d ' ')"
      echo "- \`$r\`  ($sz bytes)"
    done )
  echo
...
if command -v zip >/dev/null 2>&1; then
  ( cd "$parent" && zip -rq "$ZIP" "$base" )
elif command -v python3 >/dev/null 2>&1; then
  ( cd "$parent" && python3 -m zipfile -c "$ZIP" "$base" )
elif command -v python >/dev/null 2>&1; then
  ( cd "$parent" && python -m zipfile -c "$ZIP" "$base" )
else
...
PY
  ...
  stage_sorted="$(cd "$OUT" && find . -type f | sed 's#^\./##' | tr -d '\r' | sed '/^$/d' | sort)"
