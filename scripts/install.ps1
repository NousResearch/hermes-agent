...
"@ 2>$null
            ...
            # Smoke-test the entry point. uv pip install on Windows can create
            # uv-trampoline executables that hardcode the path to uv-managed
            # Python (e.g. %LOCALAPPDATA%\uv\python\cpython-3.11.X-windows-x86_64-none\).
            # If that Python directory is moved or cleaned (uv cache purge, system
            # temp cleanup), every `hermes` command fails with "uv trampoline failed
            # to spawn Python child process / entity not found (os error 2)". A
            # setuptools C launcher (generated by pip, not uv pip) references the
            # venv's own python.exe, which is self-contained and survives cache
            # wipes.
            $hermesExe = Join-Path $scriptsDir "hermes.exe"
            if (Test-Path $hermesExe) {
                Write-Info "Verifying entry point with a smoke test..."
                $prevEAP = $ErrorActionPreference
                $ErrorActionPreference = "Continue"
                $smokeOutput = & $hermesExe --version 2>&1
                $smokeExitCode = $LASTEXITCODE
                $ErrorActionPreference = $prevEAP
                if ($smokeExitCode -ne 0) {
                    Write-Warn "Entry point smoke test failed (exit code $smokeExitCode)"
                    Write-Warn "This usually means the entry point's Python reference is broken."
                    Write-Info "Regenerating entry points with self-contained launchers..."
                    Push-Location $InstallDir
                    try {
                        $env:UV_PROJECT_ENVIRONMENT = "$InstallDir\venv"
                        & $pythonExe -m pip install --no-build-isolation --no-deps --force-reinstall -e . 2>&1 | Out-Null
                        if ($LASTEXITCODE -eq 0) {
                            # Re-test after regeneration
                            $prevEAP2 = $ErrorActionPreference
                            $ErrorActionPreference = "Continue"
                            & $hermesExe --version 2>&1 | Out-Null
                            $retestExit = $LASTEXITCODE
                            $ErrorActionPreference = $prevEAP2
                            if ($retestExit -eq 0) {
                                Write-Success "Entry points regenerated with self-contained launchers"
                                Write-Info "Test: hermes --version passes"
                            } else {
                                Write-Warn "Entry point still fails after pip regeneration"
                                Write-Info "Workaround: `"$pythonExe`" -m hermes_cli.main <command>"
                            }
                        } else {
                            Write-Warn "pip regeneration failed; entry points may still be uv trampolines"
                        }
                    } finally {
                        Pop-Location
                    }
                } else {
                    Write-Info "Entry point smoke test passed"
                }
            }
        }
    }
