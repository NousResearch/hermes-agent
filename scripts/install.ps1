# ============================================================================
# Nautilus Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\nautilus-agent" } else { "$env:LOCALAPPDATA\hermes\nautilus-agent" }),
    ...
    #     `nautilus desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "|             * Nautilus Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI assistant for science.                |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Install-Uv {
    # Nautilus owns its own uv at $HermesHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `nautilus update` stay in sync.
...
function Install-Git {
    ...
    which Nautilus uses to run shell commands.
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Nautilus needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Nautilus copy of Git ourselves is predictable and
    ...
    ``HERMES_GIT_BASH_PATH`` (User scope) so Nautilus can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Nautilus features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Nautilus needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Nautilus can find it even before
    ...
    Write-Warn "Could not locate bash.exe -- Nautilus may not find Git Bash."
...
function Test-Node {
    ...
    # Prefer a Nautilus-managed Node from a previous run over a too-old system one.
    $managedNode = "$HermesHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (Nautilus-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Nautilus-managed Node.js $NodeVersion LTS..."
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\nautilus-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `nautilus
                    # update` path (#4735).
                    ...
                    $stashName = "nautilus-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Nautilus behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running nautilus processes) and try again."
                throw
            }
        }
    }
    ...
    # next `nautilus update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `nautilus.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'nautilus-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install nautilus-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and nautilus.exe broken with
    ...
    # here, not 30 seconds later when the user runs `nautilus`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `nautilus dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `nautilus dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `nautilus dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up nautilus command..."
    
    if ($NoVenv) {
        $nautilusBin = "$InstallDir"
    } else {
        $nautilusBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so nautilus is globally available
    # On Windows, the nautilus.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$nautilusBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$nautilusBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $nautilusBin"
    } else {
    ...
    $env:Path = "$nautilusBin;$env:Path"
    
    Write-Success "nautilus command ready"
}
...
function Write-BootstrapMarker {
    ...
    # update via `nautilus update` which moves HEAD legitimately).
...
function Copy-ConfigTemplates {
    ...
    # (the default PS5 behaviour), and Nautilus's prompt-injection scanner
    ...
    if (-not (Test-Path $soulPath)) {
        $soulContent = @"
# Nautilus Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Nautilus communicates with you.
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'nautilus setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\nautilus-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\nautilus-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\nautilus-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Nautilus` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    ...
    #
    # The Tauri bootstrap installer's launch_nautilus_desktop command
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Nautilus-managed
    ...
    # are NOT trying to work around). The Nautilus icon + product name are
    ...
    $buildLog = "$env:TEMP\nautilus-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Nautilus` rename: app-builder's
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        # with the Nautilus icon, but without this the shortcut can keep drawing
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'nautilus setup'."
        return
    }
    ...
    # Run nautilus setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    $nautilusCmd = "$InstallDir\venv\Scripts\nautilus.exe"
    if (-not (Test-Path $nautilusCmd)) {
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'nautilus whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: nautilus gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Start-Process -FilePath $nautilusCmd -ArgumentList "gateway" `
            ...
            Write-Warn "Failed to start gateway. Run manually: nautilus gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nautilus gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   nautilus            " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nautilus setup      " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nautilus config     " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nautilus config edit" -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nautilus gateway    " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nautilus update     " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Nautilus repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Nautilus to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
