...
#
# Or with options:
#   bash awesome-hermes-agent/scripts/install.sh --no-venv --skip-setup
#
# ============================================================================
...
# Configuration
REPO_URL_SSH="git@github.com:uaixo/awesome-hermes-agent.git"
REPO_URL_HTTPS="https://github.com/uaixo/awesome-hermes-agent.git"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   'ht update' runs never inject bundled skills either"
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, HT AI Agent installs the code under"
            ...
            echo "                   Installs optional deps + runs ht setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│               HT AI Agent Installer                     │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent (fork of Hermes Agent).        │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download HT AI Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  (from a clone of https://github.com/uaixo/awesome-hermes-agent)"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. HT AI Agent install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "HT AI Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if HT AI Agent behaves unexpectedly."
                    else
...
EOF
    ...
    # `ht` is the primary command; `hermes` is kept as a legacy alias.
    rm -f "$command_link_dir/ht"
    cp "$command_link_dir/hermes" "$command_link_dir/ht"
    chmod +x "$command_link_dir/ht"
    log_success "Installed ht launcher → $command_link_display_dir/ht (legacy alias: hermes)"
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# HT AI Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# HT AI Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# HT AI Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        log_info "  Future 'ht update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Hermes Agent browser tools|^# HT AI Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "HT AI Agent will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# HT AI Agent browser tools — explicit browser override."
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'ht setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for HT AI Agent to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'ht whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'ht whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'ht gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: ht gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: ht gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}ht${NC}              Start chatting"
    echo -e "   ${GREEN}ht setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}ht config${NC}       View/edit configuration"
    echo -e "   ${GREEN}ht config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}ht gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}ht update${NC}       Update to latest version"
    echo ""
    echo -e "   ('hermes' still works as a legacy alias for 'ht')"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'ht' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'ht' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'ht' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up HT AI Agent for pip install"
