...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m hermes_cli.main setup --install-profile "$INSTALL_PROFILE" < /dev/tty
    else
        python -m hermes_cli.main setup --install-profile "$INSTALL_PROFILE" < /dev/tty
    fi
}
