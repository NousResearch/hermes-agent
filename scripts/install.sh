#!/bin/bash
# ============================================================================
# Nautilus Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/TJ-coding/nautilus-agent/main/scripts/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Nautilus terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:TJ-coding/nautilus-agent.git"
REPO_URL_HTTPS="https://github.com/TJ-coding/nautilus-agent.git"
...
#   code at /usr/local/lib/nautilus-agent, command at /usr/local/bin/nautilus,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   'nautilus update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Nautilus.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.hermes/nautilus-agent"
            echo "                   default (root, Linux): /usr/local/lib/nautilus-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.hermes, or \$HERMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Nautilus installs the code under"
            echo "  /usr/local/lib/nautilus-agent and links the command into"
            echo "  /usr/local/bin/nautilus (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERMES_HOME/nautilus-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs nautilus setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Nautilus Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI assistant for science.               │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `nautilus update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Nautilus-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Nautilus Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install nautilus command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `nautilus` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/nautilus-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/nautilus-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/nautilus-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HERMES_HOME/nautilus-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/nautilus-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/nautilus-agent/.git" ]; then
            INSTALL_DIR="$HERMES_HOME/nautilus-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/nautilus-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/nautilus-agent"
        ...
        # /usr/local/bin/nautilus wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/nautilus"
...
get_nautilus_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/nautilus" ]; then
        echo "$link_dir/nautilus"
    else
        echo "nautilus"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/TJ-coding/nautilus-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Nautilus owns its own uv at $HERMES_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `nautilus update` stay in sync.
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/nautilus-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/nautilus-uv-installer.$$.sh")"
...
# string clears that floor; anything below it is replaced with the Nautilus-
...
check_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ] && node_satisfies_build "$("$HERMES_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$HERMES_HOME/node/bin/node" --version) found (Nautilus-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Nautilus-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    # the nautilus command uses (get_command_link_dir): /usr/local/bin for root
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Nautilus install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Nautilus Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `nautilus update` path
                # (#4735).
                ...
                local stash_name
                stash_name="nautilus-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Nautilus behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up nautilus command..."

    if [ "$USE_VENV" = true ]; then
        NAUTILUS_BIN="$INSTALL_DIR/venv/bin/nautilus"
    else
        NAUTILUS_BIN="$(which nautilus 2>/dev/null || echo "")"
        if [ -z "$NAUTILUS_BIN" ]; then
            log_warn "nautilus not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$NAUTILUS_BIN" ]; then
        log_warn "nautilus entry point not found at $NAUTILUS_BIN"
    ...
    # Create a user-facing shim for the nautilus command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $NAUTILUS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$NAUTILUS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/nautilus"
    cat > "$command_link_dir/nautilus" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NAUTILUS_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/nautilus"
    log_success "Installed nautilus launcher → $command_link_display_dir/nautilus"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "nautilus command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v nautilus' \
                >/dev/null 2>&1; then
        ...
        log_info "nautilus not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Nautilus Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Nautilus Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Nautilus Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so nautilus works immediately
    export PATH="$command_link_dir:$PATH"
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `nautilus update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'nautilus update'." \
        ...
        log_info "  Future 'nautilus update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Nautilus Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Nautilus will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (nautilus --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `nautilus update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'nautilus setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Nautilus to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'nautilus whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                NAUTILUS_CMD="$(get_nautilus_command_path)"
                $NAUTILUS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'nautilus whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'nautilus gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $NAUTILUS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $NAUTILUS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: nautilus gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: nautilus gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $NAUTILUS_CMD gateway > "$HERMES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.hermes/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: nautilus gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: nautilus gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}nautilus${NC}            Start chatting"
    echo -e "   ${GREEN}nautilus setup${NC}      Configure API keys & settings"
    echo -e "   ${GREEN}nautilus config${NC}     View/edit configuration"
    echo -e "   ${GREEN}nautilus config edit${NC} Open config in editor"
    echo -e "   ${GREEN}nautilus gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}nautilus update${NC}     Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'nautilus' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'nautilus' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'nautilus' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Nautilus for pip install"
    ...
    NAUTILUS_CMD="$(command -v nautilus 2>/dev/null || echo "")"
    if [ -n "$NAUTILUS_CMD" ]; then
        log_info "Running nautilus setup..."
        "$NAUTILUS_CMD" setup
    else
        log_warn "nautilus command not found on PATH"
...
# extract a tree MISSING the electron binary, so the `electron`->`Nautilus` rename
...
# _purge_electron_build_cache() used by `nautilus desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Nautilus-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `nautilus desktop`),
    ...
    # trips Gatekeeper's tamper check ("Nautilus is damaged and can't be opened").
    ...
    # checkout stays clean for the next `nautilus update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Nautilus-managed Node
