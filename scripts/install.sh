...
install_desktop() {
    ...
    # checkout. Only one installer binary is published and it is always the
    # newest, so it is routinely pointed at a ref that predates ae9f42accf and
    # therefore lacks this helper. The binding repair is an optimisation, not
    # a precondition, so MODULE_NOT_FOUND must not fail the desktop stage; a
    # helper that IS present and fails still propagates its exit code.
