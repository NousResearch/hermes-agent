...
def _markdown_link(label: str, url: object) -> str:
    ...
    if parsed.scheme == "https" and parsed.netloc:
        return f"[{label}](<{text}>)"
    return f"{label}: evidência indisponível"
...
def publish(channel_id: str, projection: Projection, state_path: Path, token: str, opener: Callable[..., Any] = urlopen) -> tuple[str, str]:
    ...
    if len(projection.content) > 2000:
        raise ProjectionError("projection exceeds Discord's 2,000-character message limit")
    state = load_state(state_path)
    message_id = state.get(projection.key)
    payload = {"content": projection.content, "allowed_mentions": {"parse": []}}
    if message_id:
        message = discord_request("PATCH", f"/channels/{channel_id}/messages/{message_id}", token, payload, opener)
        action = "edited"
    else:
        message = discord_request("POST", f"/channels/{channel_id}/messages", token, payload, opener)
        action = "created"
