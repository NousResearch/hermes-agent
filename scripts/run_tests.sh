...
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.hermes/hermes-agent/venv"; do
  if [ -f "$candidate/bin/activate" ] && [ -x "$candidate/bin/python" ] && "$candidate/bin/python" -V >/dev/null 2>&1; then
    VENV="$candidate"
    break
  fi
done
...
if [ -z "$VENV" ]; then
  echo "error: no runnable virtualenv found in $REPO_ROOT/.venv, $REPO_ROOT/venv, or $HOME/.hermes/hermes-agent/venv" >&2
  exit 1
fi
