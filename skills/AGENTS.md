...
archives go to `${HERMES_HOME:-~/.hermes}/skills/.archive/` and are restorable. Core `agent/curator.py` (review
...
prune|backup|rollback`; telemetry `tools/skill_usage.py` owns `${HERMES_HOME:-~/.hermes}/skills/.usage.json`
