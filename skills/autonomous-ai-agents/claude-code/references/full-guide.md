...
|---|---|---|
| `claude` CLI | Delegate an implementation or review task | Prompt and approved working directory |
...
- A working Claude Code CLI installation and an approved repository/worktree.
- Authentication must be configured through the CLI's supported login flow or an already configured provider credential; do not place secrets in prompts.

## Purpose

Claude Code provides structured guidance for the tasks it covers. Use the patterns and workflows below to complete your work efficiently.
...
## Codex Usage Limit Fallback Pattern

When Codex hits ChatGPT usage limits, Claude Code is the immediate
fallback. Verified pattern (2026-06-24):
...
- `--model sonnet` is sufficient for well-specified Rust coding tasks
  (both batches produced compiling code on first try)
- `--add-dir` is needed when the task spec references files outside
  the current working directory (e.g., monorepo sibling crates)
- Claude Code print mode does NOT hit usage limits when Codex does
  (different auth: claude.ai OAuth vs ChatGPT account)
- Two parallel `claude -p` agents touching different files completed
  in ~5 minutes each with zero conflicts
- Both agents could run `cargo check` in the main workspace (not worktrees)
- Task specs should say "Do NOT run cargo test in worktrees" and
  "Do NOT modify Cargo.toml" -- same rules as Codex
...
11. **`--bare` skips OAuth** — requires `ANTHROPIC_API_KEY` env var or an `apiKeyHelper` in settings. VERIFIED 2026-06-24: `claude --bare -p "test"` returns "Not logged in" on a system with only claude.ai OAuth (no API key). Use `claude -p` without `--bare` instead -- print mode still skips interactive dialogs.
11. **Session limits reset at 5am local time** -- Claude Code (claude.ai OAuth) has daily session limits. When hit, the process exits immediately with "You've hit your session limit" and no code is written. Codex CLI has separate monthly limits (reset ~23rd of each month). Use them as fallbacks for each other. When both are limited, implement directly via Hermes terminal/patch tools.
12. **Codex CLI usage limits** -- `codex exec` hits monthly ChatGPT usage limits. Error: "You've hit your usage limit. Upgrade to Plus." Resets mid-month. Separate from Claude Code limits.
...
### Claude Code + Ollama Models (2026-07-11)

Claude Code can use ollama models via `ANTHROPIC_BASE_URL=http://127.0.0.1:11434` + `ANTHROPIC_AUTH_TOKEN=ollama`.

**Critical:** `sonnet` (minimax-m3:cloud) FAILS for tool-use through ollama — burns all max-turns with zero output. `haiku` (kimi-k2.7-code:cloud) WORKS — completed multi-file Rust task in one pass.

For Claude Code + ollama: always use `--model haiku`. Avoid `--model sonnet`. See `references/claude-code-ollama-models.md` for details.

### Crate integration field-guessing pitfall (2026-06-24)

When Claude Code or subagents add MCP tools that wrap sibling crate APIs (claim-ledger, boundary-compiler, etc.), they GUESS struct field names and get them wrong. Examples from this session:
- EvidenceLink: agent wrote `evidence_link_id/claim_id/source_ref/notes` -- actual fields are `source_id/span_id/quote/digest/support_role`
- SupportState: agent wrote `Contested` -- actual variant is `Contradicted`
- SupportJudgment: agent missed `contradiction_refs: Vec<String>` and `proof_debt: Vec<ProofDebt>` fields
- SearchResult: agent wrote `r.id()` and `r.updated_at` -- no such fields, use `r.source.result_id()`
- MemoryStats: agent wrote `s.get("facts")` -- it's a struct, use `s.total_facts`

**Fix:** Always include "grep the actual struct definition before writing construction code" in task specs. Better yet, paste the struct definition INTO the task spec so the agent doesn't have to search for it.
