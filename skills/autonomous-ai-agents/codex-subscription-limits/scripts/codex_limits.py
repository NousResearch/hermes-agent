    ...
    for path in paths:
        ...
        for item in iter_dict_values(payload):
            ...
            if token and account_id:
                break
        if token:
            return AuthMaterial(access_token=token, account_id=account_id, source=str(path.expanduser()))
    return AuthMaterial()


def fetch_wham(timeout: int, usage_url: str, auth_paths: list[Path]) -> dict[str, Any]:
    auth = discover_auth(auth_paths)
    if not auth.access_token:
        searched = ", ".join(str(p.expanduser()) for p in auth_paths)
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        elif args.provider in ("auto", "app-server"):
            try:
                state = normalize_app_server(fetch_app_server(args.timeout, args.codex_command))
            except Exception:
                if args.provider == "app-server":
                    raise
                auth_paths = [Path(p) for p in args.auth_file] or [Path("~/.codex/auth.json"), Path("~/.hermes/auth.json")]
                state = normalize_wham(fetch_wham(args.timeout, args.usage_url, auth_paths))
        else:
            auth_paths = [Path(p) for p in args.auth_file] or [Path("~/.codex/auth.json"), Path("~/.hermes/auth.json")]
            state = normalize_wham(fetch_wham(args.timeout, args.usage_url, auth_paths))
