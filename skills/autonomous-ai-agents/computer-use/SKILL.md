...
| `blocked pattern in type text` | You tried to `type` a shell command matching the dangerous-pattern block list (`curl ... \| bash`, `sudo rm -rf`, etc.). Stop and inspect the refusal reason. Never split, disguise, or reroute a blocked command to evade the guard. Use only a supported, authorized alternative that respects the refusal; if none is available, ask the user how to proceed. The GUI retry ladder is for delivery failures, not safety refusals |
