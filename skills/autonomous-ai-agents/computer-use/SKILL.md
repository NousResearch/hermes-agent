...
| `blocked pattern in type text` | You tried to `type` a shell command matching the dangerous-pattern block list (`curl … piped into a shell`, `sudo rm -rf`, etc.). Break the command up or reconsider |
