...
system (`${HERMES_HOME:-~/.hermes}/plugins/`) is otherwise documented separately.
...
- Write access to `$HERMES_HOME/desktop-plugins/` (usually
  `${HERMES_HOME:-~/.hermes}/desktop-plugins/`).
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `${HERMES_HOME:-~/.hermes}/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
