...
opening a browser, user signs in, token lands in `${HERMES_HOME:-~/.hermes}/auth.json` — is a
...
1. Read Hermes's existing Portal credential out of `${HERMES_HOME:-~/.hermes}/auth.json`.
