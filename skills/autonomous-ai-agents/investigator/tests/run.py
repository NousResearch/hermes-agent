#!/usr/bin/env python3
"""run.py — grouped test suites for the investigator skill.

  basic  (DEFAULT)  every mocked class in test_iterate.py — offline, no model calls, ~seconds.
  live              only the model-calling class (TestLive) — needs a reachable model.
  all               both.
...
Live classes are gated on INVESTIGATOR_TEST_LIVE (set here for live/all, so direct
...
def main(argv=None):
    ...
    if args.suite in ("live", "all"):
        ...
        os.environ["INVESTIGATOR_TEST_LIVE"] = "1"
