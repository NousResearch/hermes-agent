...
1. Read this `SKILL.md` before rewriting or explaining the workflow.
2. If the user supplied text, perform the rewrite in the same turn; do not only describe what the skill would do or ask the user to provide text they already supplied.
3. Preserve the request's meaning and intended audience. Return the rewritten text first, followed by a short list of material changes.
4. If no text was supplied, ask for the text and any voice sample instead of pretending an edit occurred.
