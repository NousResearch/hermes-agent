...
```
Jade -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. Add `twozero_td` MCP server to Jade config (if missing)
...
3. **Restart Jade session** to pick up the new MCP server
