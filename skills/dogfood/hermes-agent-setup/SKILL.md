...
```bash
cd ~/.hermes/hermes-agent
source venv/bin/activate 2>/dev/null || source .venv/bin/activate  # or: source venv/bin/activate
...
```bash
cd ~/.hermes/hermes-agent && source venv/bin/activate 2>/dev/null || source .venv/bin/activate
