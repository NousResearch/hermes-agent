...
Default native list/get resources can contain bodies; `$select` reduces payload. Missing selected fields are not evidence of an empty body or no protection. Verify the command-specific serialization, especially protection fields, before using defaults. Shared and native schemas differ.
