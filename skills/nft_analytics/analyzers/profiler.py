...
class WalletProfiler:
    ...
    def __init__(
        ...
        tensor_txs: Optional[List[Dict]] = None,
    ) -> None:
        self.wallet       = wallet
        self.transactions = transactions
        self.owned_assets = owned_assets or []
        self.tensor_txs   = tensor_txs or []
    ...
    def _get_tensor_prices(self) -> tuple:
        """Tensor'dan gerçek alış/satış fiyatlarını çıkar."""
        buy_price:  Dict[str, float] = {}
        sell_price: Dict[str, float] = {}
        for tx in self.tensor_txs:
            mint = (tx.get("mint") or {}).get("onchainId", "")
            if not mint:
                continue
            amount = (tx.get("grossAmount") or 0)
            price  = float(amount) / 1e9 if amount > 1000 else float(amount)
            tx_type = (tx.get("txType") or "").upper()
            buyer  = (tx.get("buyer")  or {}).get("address", "")
            seller = (tx.get("seller") or {}).get("address", "")
            if tx_type in ("BUY", "ACCEPT_BID") and buyer == self.wallet and price > 0:
                buy_price[mint] = price
            elif tx_type in ("SELL", "LIST_AND_SELL") and seller == self.wallet and price > 0:
                sell_price[mint] = price
    ...
    def _compute_net_roi(self) -> float:
        ...
        """
        # Önce Tensor fiyatlarını dene (daha doğru)
        tensor_buy, tensor_sell = self._get_tensor_prices()
        _, _, helius_buy, helius_sell = self._acquire_sell_maps()

        # Tensor varsa onu kullan, yoksa Helius'a düş
        buy_price  = {**helius_buy,  **tensor_buy}
        sell_price = {**helius_sell, **tensor_sell}
