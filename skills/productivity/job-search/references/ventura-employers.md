# Ventura Area Employer Reference

## Salary Ranges (verified from Glassdoor/Indeed/ZipRecruiter, 2025-2026)

| Company | Role | Location | Salary Range |
|---|---|---|---|
| Northrop Grumman | Principal Embedded Software Engineer | Camarillo, CA | $114K–$213K/yr |
| Northrop Grumman | Sr. Principal Project Management | Camarillo, CA | $94K–$176K/yr |
| Teledyne | Sr. Software Engineer | Thousand Oaks, CA | $163K–$259K |
| Amgen | Director-level software roles | Thousand Oaks, CA | Not posted (WD1 ATS blocks scraping) |

## ATS / Job Board Quirks

### Teledyne FLIR Careers (Workday)
- URL: `https://flir.wd1.myworkdayjobs.com/en-US/flircareers`
- Workday ATS blocks most scraping tools
- Search for "Thousand Oaks, CA" or "Camarillo, CA" manually
- Glassdoor/LinkedIn better sources for salary + role discovery

### Northrop Grumman
- Direct careers site: `jobs.northropgrumman.com`
- LinkedIn often has cleaner data than NG's own site
- Most Camarillo software roles are embedded/firmware — filter accordingly

### Amgen
- Direct: `amgen.wd1.myworkdayjobs.com/Careers`
- Workday blocks scraping; use Indeed/LinkedIn/Glassdoor for discovery
- Salary not typically posted on Amgen WD1 postings

## Key Search Patterns

```
# Northrop Grumman Camarillo
site:jobs.northropgrumman.com OR site:linkedin.com "software engineer" OR "systems engineer" OR "program manager" Camarillo

# Amgen Thousand Oaks
site:careers.amgen.com OR site:linkedin.com "senior engineer" OR "staff engineer" OR "manager" OR "associate director" Thousand Oaks engineering

# Teledyne Thousand Oaks/Camarillo (FLIR/Workday)
site:linkedin.com OR site:indeed.com "software development engineer" OR "systems engineer" OR "program manager" Teledyne Thousand Oaks OR Camarillo

# Salary ranges
site:ziprecruiter.com OR site:glassdoor.com [company] [role] Thousand Oaks OR Camarillo
```

## Clearance Notes

- Northrop Grumman roles often require US citizenship + ability to obtain DoD clearance
- Most roles do NOT require active clearance at application stage — ability to obtain is what's checked
- Flag this in job reports if citizenship or clearance status might be a blocker for Gordon

## Last Verified: 2026-05-08