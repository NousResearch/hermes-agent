...
## 13. Slack comparison invocation

```text
!cyber-vc-analyst compare Red Access Security vs Noma Security.
Use my vault notes first, then use Return on Security MCP if available. Keep
the output comparison-ready and identify the stronger near-term investment case.
```

## 14. Slack triage invocation

```text
!cyber-vc-analyst triage Red Access Security.
Give me a fast IC-style read with the likely category, why now, core risks, and
whether this deserves a full memo.
```

## 15. Direct company skill with depth

```text
!cyber-vc-company deep Red Access Security
```

## 16. Direct theme skill with depth

```text
!cyber-vc-theme deep SOC Automation / AI SOC
```

## 17. Direct compare skill

```text
!cyber-vc-compare Red Access Security vs Noma Security
```

## 18. Direct competitors skill
...
- Use `quick`, `standard`, or `deep` as an optional leading depth token when
  you want to control how much research the skill performs.
