...
For long-lived processes: HT AI Agent gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside HT AI Agent you have two practical options:
...
## Debugging HT AI Agent-specific Processes
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For HT AI Agent subagents, debug one process at a time.
