...
```
Here are your captions — tell me what to fix, or open the editor for visual edits:
...
**Always include the dashboard link** from the response (if present):
```
✏️ Edit visually (text, phonetics, style, re-burn): http://localhost:9119/captions/{job_id}
```

The dashboard editor lets the user make all changes without spending tokens.
Only use the chat correction loop (Steps 4-5) if the user explicitly prefers it or doesn't have the dashboard open.
