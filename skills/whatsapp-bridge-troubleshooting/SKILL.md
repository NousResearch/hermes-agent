...
- **Bridge HTTP port**: `3000` (health check: `curl http://127.0.0.1:3000/health`)
...
**Check**: `tail -20 ~/.hermes/logs/gateway.log | grep -E "bridge process exited|reconnected"` — if you see alternating exit/reconnect lines within seconds of each other, the bridge is looping.

**Workaround**: Kill the bridge process once (`kill $(pgrep -f bridge.js)`) and wait for the gateway to respawn it. If the loop persists after a clean respawn, the bridge is hitting a fatal error on startup — check the bridge's stderr via `cat /proc/$(pgrep -f bridge.js)/fd/2` for Baileys/auth errors.

## Restarting the WhatsApp bridge

### Gateway self-restart is blocked from inside the gateway process

The gateway process (`hermes gateway restart`, `systemctl restart hermes-gateway.service`, or `kill` + respawn) **cannot be invoked from within the running gateway** — the SIGTERM propagates to child processes and kills the restart command before it completes. This is true for both the user systemd service and the system-level service.

**Workaround**: Use a separate shell outside the gateway. Options:
- `sudo systemctl restart hermes-gateway.service` from a non-gateway shell
- The `~/.hermes/dashboard.sh` management script (start/stop/restart/status) with PID file at `~/.hermes/dashboard.pid`
...
Then restart the gateway (`kill -USR1 $(cat ~/.hermes/gateway.pid)`) so the bridge respawns with the new env.
...
After patching, restart the gateway: `kill -USR1 $(cat ~/.hermes/gateway.pid)`.
