...
- Timed record wrapper (macOS): `starter-kits/agent-launch-closeout-kit/scripts/demo-capture-timed-record-wrapper.sh`
...
6. On macOS, prefer `bash starter-kits/agent-launch-closeout-kit/scripts/demo-capture-timed-record-wrapper.sh` when one-command timed capture is the fastest path; otherwise capture manually and then run the headless finalize helper.
