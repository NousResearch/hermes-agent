...
# Use Close Chat (v2.2 · 2026-06-28)
...
Project OS Sync (capability-based — ทำเฉพาะไฟล์ที่ `.project/` มีจริง · อัปเฉพาะเมื่อเปลี่ยนจริง):
- `OverviewProgress.md` (ถ้ามี) — ย้ายงานที่ verified รอบนี้ไป "เสร็จแล้ว" + อัปงานค้าง/ถัดไป + commit SHA ล่าสุด (อิง git จริง)
- `FeatureSpec.md` (ถ้ามี) — มีฟีเจอร์ใหม่/เลื่อน mock→real ที่พิสูจน์แล้ว → อัปบัญชี + path:line
- `BusinessPlan.md`/`-Full.md` (ถ้ามี) — มี feature ใหม่/ราคา/insight ลูกค้า/คู่แข่ง ที่ขยับแผนไหม → อัป [fact] + ต่อ log (ห้ามทับ log เก่า)
- `DesignSystem.md` (ถ้ามี) — ดีไซน์/ธีมเปลี่ยน หรือเจอจุดโค้ดไม่ตรงดีไซน์ → อัป
กฎ: ไฟล์ไหนรอบนี้ไม่มีอะไรเปลี่ยน → เขียนว่า "ไม่เปลี่ยน" (ห้ามบังคับอัปจนไฟล์รก/ข้อมูลลม) · ทุกค่าต้องมีหลักฐาน · รายละเอียดเทมเพลตอยู่ในแต่ละ shortcut (Use OverviewProgress/FeatureSpec/BusinessPlan/DesignSystem)
...
Project OS: <อัปอะไรในไฟล์ .project/* (OverviewProgress/FeatureSpec/BusinessPlan/DesignSystem) หรือ "ไม่เปลี่ยน" รายไฟล์ หรือ N/A ถ้าไม่มี>
...
- v2.2 (2026-06-28): ขยาย Business Plan Sync → **Project OS Sync** ครบชุด 4 ไฟล์ (OverviewProgress/FeatureSpec/BusinessPlan/DesignSystem) · อัปเฉพาะไฟล์ที่มี + เฉพาะที่เปลี่ยนจริง · เปลี่ยนบรรทัด Output เป็น "Project OS" · pair กับ New Chat v1.8 (ที่อ่านไฟล์ชุดนี้ตอนเปิด)
