...
>
> มาตรฐานกลางตัวจริงที่คำสั่งนี้ยึด (v2.3): ชุด `Tech Tools/Hermes Agent/design-system-standard-v2/` (spec + emotion-matrix + checklist 77 + tokens + tools + ENTRY-FOR-OTHER-PROJECTS.md) · ไฟล์เก่า `60-Design/design-systems/design-system-standard.md` เป็นฉบับอ้างอิงประวัติ ไม่ใช่ตัวรัน
...
ต้นทาง seed กลาง: `Tech Tools/Hermes Agent/design-system-standard-v2/` (สมองของเครื่องมือ — เจ้าของไม่ต้องเปิด)
...
2. **contrast ผ่าน `tools/contrast-audit.js` บนหน้าเรนเดอร์จริง ทั้ง Front และ Admin = 0 fail** · `contrast-check.mjs` (static) เป็นแค่ pre-check เสริม ไม่พอ (เคยพลาดข้อความบนการ์ดพื้นสว่าง)
...
ตัวตั้งต้น (template ที่แก้ครบแล้ว ใช้ลอกโครง): `design-system-standard-v2/preview/onemanfleet-ds.html` + `tokens/{front,admin}.tokens.json`
