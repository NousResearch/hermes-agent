...
// Scripted todo_list calls must go through the resident `tool_call` bridge:
// todo_list is deferred by default (`tools/tool_search.py::_DEFAULT_DEFERRED_TOOLS`),
// and a direct `todo` / `todo_list` call is rejected at validation.
    ...
    toolCalls: [
      {
        name: 'tool_call',
        args: {
          calls: [{ name: 'todo_list', arguments: { todos: [{ id: '1', content: 'Verify', status: 'completed' }] } }],
        },
      },
    ],
  },
