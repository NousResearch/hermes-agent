...
class TestSlashCommands:
    ...
    def test_compact_survives_process_restart(self, tmp_path, monkeypatch):
        ...
        assert len(active) == 1
        assert summary in active[0]["content"]
        assert active[0]["_compressed_summary"] is True

    def test_compact_reports_persistence_failure_and_preserves_active_history(self, tmp_path, monkeypatch):
        from run_agent import AIAgent
        ...
        persisted = restarted_db.get_messages(state.session_id, include_inactive=True)
