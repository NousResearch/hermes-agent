...
class TestRefreshOauthToken:
    def test_returns_none_without_refresh_token(self, tmp_path, monkeypatch):
        # 2026-06-29 upstream merge: _refresh_oauth_token now re-reads LIVE
        # credential sources (adopt Claude Code's already-rotated token rather
        # than racing it). Isolate Path.home to an empty dir so the adopt path
        # finds nothing and the empty-refreshToken contract holds.
        monkeypatch.setattr("agent.anthropic_adapter.Path.home", lambda: tmp_path)
    ...
    def test_failed_refresh_returns_none(self, tmp_path, monkeypatch):
        # See test_returns_none_without_refresh_token — isolate Path.home so the
        # upstream live-cred-adopt path doesn't return a real host token.
        monkeypatch.setattr("agent.anthropic_adapter.Path.home", lambda: tmp_path)
        creds = {
            "accessToken": "old",
            "refreshToken": "refresh-123",
...
class TestBuildAnthropicKwargs:
    ...
    def test_fast_mode_omitted_for_unsupported_model(self):
        """fast_mode=True on Opus 4.6 must not claim unavailable Fast."""
        kwargs = build_anthropic_kwargs(
            model="claude-opus-4-6",
            messages=[{"role": "user", "content": "hi"}],
            tools=None,
