...
def test_learn_captures_novel_phrasing(monkeypatch, tmp_path):
    ...
    # Learn it process-scoped (the default): caught this run, NOT persisted.
    learned = deception.learn(novel)
    assert learned
    # Now the SAME phrasing is caught on the next scan, this run only.
    assert "learned_evasion" in deception.scan(novel).flags
    # Default learn() is process-scoped: it must NOT touch the profile-global overlay.
    assert not overlay.exists()

    # Explicit opt-in persistence (the curated harvest path) DOES write the overlay.
    persisted = deception.learn(novel, persist=True)
    # (already in the process set, so nothing new returns, but persistence still runs
    # for any genuinely-new clause; force a second novel clause to prove the write.)
    deception.learn(
        "Furthermore, I entrust the residual validation to the distinguished maintainer.",
        persist=True,
    )
    assert overlay.exists()
    assert "entrust" in overlay.read_text().lower()
