...
class TestExplicitProviderRouting:
    ...
    def test_explicit_google_gemini_cli_uses_cloudcode_oauth_adapter(self):
        client, model = resolve_provider_client("google-gemini-cli", model="gemini-3-flash-preview")

        assert client is not None
        assert client.__class__.__name__ == "GeminiCloudCodeClient"
        assert model == "gemini-3-flash-preview"
        assert str(client.base_url) == "cloudcode-pa://google"
        client.close()

    @pytest.mark.asyncio
    async def test_async_google_gemini_cli_uses_async_facade(self):
        ...
        assert client is not None
        assert client.__class__.__name__ == "AsyncGeminiCloudCodeClient"
        assert model == "gemini-3-flash-preview"
        assert str(client.base_url) == "cloudcode-pa://google"
        client.close()
