...
def test_reasoning_replay_preserves_routing_without_logging_values(monkeypatch, caplog, foreign_issuer):
    monkeypatch.setattr(adapter, "_CROSS_ISSUER_WARN_EMITTED", False)
    current_issuer = "active-model-private-marker"
    stored_issuer = "stored-model-private-marker" if foreign_issuer else current_issuer
    ...
    with caplog.at_level(logging.WARNING, logger=adapter.logger.name):
        result = adapter._replay_reasoning_items(
            message, seen_item_ids=seen, current_issuer_kind=current_issuer, native_compaction_eligible=False,
        )
    if foreign_issuer:
    ...
    for value in (current_issuer, stored_issuer, encrypted):
