...
@pytest.mark.parametrize(
    "code_heading", ["## Goal.", "## Goal", HISTORICAL_TASK_HEADING, *_CONTINUATION_HEADINGS]
)
def test_nonheadings_and_code_examples_do_not_compete_with_schema(code_heading):
    summary = (
        _summary_from_sections()
        + "\n\n## Ordinary Notes\n"
...
def test_assistant_tool_only_fallback_keeps_previous_route_reference_only(newline):
    compressor = _compressor()
    compressor._summary_has_user_turn = True
    compressor._previous_summary = _summary_from_sections().replace("\n", newline)
    summary = compressor._build_static_fallback_summary(
        [
...
@pytest.mark.parametrize("display_kind", [None, "steer"])
def test_post_handoff_correction_controls_context_dependent_continue(display_kind):
