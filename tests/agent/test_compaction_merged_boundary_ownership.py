...
def test_merged_handoff_preserves_replayed_live_request_after_summary(multimodal):
    prior = "Prior user context retained before compaction."
    latest = "Finish the pending roadmap, keeping the corrected scope."
    content = (
        _header_merged_list([_text(prior), _image()])
        if multimodal
        else _header_merged_string(prior)
    )
    carrier = _message(content, metadata=True)
    compressor = ContextCompressor.__new__(ContextCompressor)
    compressor.quiet_mode = True
    result = compressor._reappend_inflight_user_task(
        [carrier], {"role": "user", "content": latest}
    )

    assert result[0]["_inflight_replay_merged"] is True
    projected = ContextCompressor._strip_context_summary_handoff_message(result[0])
    assert projected is not None
    visible = str(projected["content"])
    assert prior in visible
    assert latest in visible
    assert "REAL_SUMMARY_BODY" not in visible
    assert "LIST_SUMMARY_BODY" not in visible
    assert ContextCompressor._is_actionable_user_turn(projected) is True
...
def test_header_merged_string_selects_last_prefix_qualified_delimiter():
    ...
    assert handoff is not None and live is not None
    assert "OWNED-SUMMARY" in handoff["content"]
    assert "QUOTED-SUMMARY-PREFIX-IN-PRIOR" not in handoff["content"]
    assert live["content"] == prior
