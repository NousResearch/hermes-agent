...
def test_openai_codex_stale_provider_reset_is_rechecked_after_one_hour(tmp_path, monkeypatch):
    """A multi-day reset hint must not suppress a successful Codex recheck for days."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    ...
    assert entry is not None
    assert entry.id == "cred-1"
    assert entry.last_status == "probing"


def test_openai_codex_stale_provider_reset_reenters_via_lease(tmp_path, monkeypatch):
    """Delegated children must share the same one-hour Codex cooldown expiry."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "weekly-reset", "auth_type": "oauth", "priority": 0,
        "source": "manual:device_code", "access_token": "tok-1",
        "last_status": "exhausted", "last_status_at": time.time() - 7200,
        "last_error_code": 429, "last_error_reason": "usage_limit_reached",
        "last_error_reset_at": time.time() + 604800,
    }]}})
    ...
    assert pool.current().last_status == "probing"


def test_openai_codex_stale_recheck_is_claimed_by_only_one_pool(tmp_path, monkeypatch):
    """Two preloaded processes must not both recheck the same stale credential."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "weekly-reset", "auth_type": "oauth", "priority": 0,
    ...
    claimed = first_pool.select()
    assert claimed is not None
    assert claimed.last_status == "probing"
    assert second_pool.has_available() is False
    assert second_pool.select() is None

    persisted = json.loads((tmp_path / "hermes" / "auth.json").read_text())
    assert persisted["credential_pool"]["openai-codex"][0]["last_status"] == "probing"


def test_normal_availability_refreshes_status_from_disk_across_pools(tmp_path, monkeypatch):
    """A preloaded pool must not reuse an entry another process exhausted."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "one", "auth_type": "oauth", "priority": 0,
        "source": "manual:device_code", "access_token": "tok-1",
    }]}})
    ...
    first_pool = load_pool("openai-codex")
    second_pool = load_pool("openai-codex")
    assert first_pool.mark_exhausted_and_rotate(
        status_code=429,
    ...
    persisted = json.loads((tmp_path / "hermes" / "auth.json").read_text())
    assert persisted["credential_pool"]["openai-codex"][0]["last_status"] == "exhausted"
    assert second_pool.has_available() is False
    assert second_pool.select() is None
    assert second_pool.acquire_lease() is None


def test_codex_probe_becomes_healthy_only_after_matching_success(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "weekly-reset", "auth_type": "oauth", "priority": 0,
    ...
    from agent.credential_pool import load_pool
    pool = load_pool("openai-codex")
    assert pool.select().last_status == "probing"

    assert pool.mark_success(api_key_hint="tok-1") is True
...
def test_late_probe_success_cannot_overwrite_newer_429(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "weekly-reset", "auth_type": "oauth", "priority": 0,
    ...
    from agent.credential_pool import load_pool
    probe_pool = load_pool("openai-codex")
    failure_pool = load_pool("openai-codex")
    claimed = probe_pool.select()
    assert claimed is not None and claimed.last_status == "probing"

    assert failure_pool.mark_exhausted_and_rotate(
        status_code=429,
        error_context={"reason": "usage_limit_reached"},
        api_key_hint="tok-1",
    ) is None
    assert probe_pool.mark_success(api_key_hint="tok-1") is False
...
def test_late_probe_failure_cannot_overwrite_newer_success(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "weekly-reset", "auth_type": "oauth", "priority": 0,
    ...
    from agent.credential_pool import load_pool
    stale_probe_pool = load_pool("openai-codex")
    reclaim_pool = load_pool("openai-codex")
    first_claim = stale_probe_pool.select()
    assert first_claim is not None and first_claim.last_status == "probing"

    auth_path = tmp_path / "hermes" / "auth.json"
    auth_doc = json.loads(auth_path.read_text())
    auth_doc["credential_pool"]["openai-codex"][0]["last_status_at"] = time.time() - 301
    auth_path.write_text(json.dumps(auth_doc))

    reclaimed = reclaim_pool.select()
    assert reclaimed is not None and reclaimed.last_status == "probing"
    assert reclaim_pool.mark_success(api_key_hint="tok-1") is True

    stale_probe_pool.mark_exhausted_and_rotate(
        status_code=429,
        error_context={"reason": "usage_limit_reached"},
        api_key_hint="tok-1",
    )
...
def test_abandoned_codex_probe_can_be_reclaimed_after_five_minutes(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    old_claim_at = time.time() - 301
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "abandoned", "auth_type": "oauth", "priority": 0,
        "source": "manual:device_code", "access_token": "tok-1",
        "last_status": "probing", "last_status_at": old_claim_at,
        "last_error_code": 429, "last_error_reason": "usage_limit_reached",
        "last_error_reset_at": time.time() + 604800,
    }]}})
    ...
    pool = load_pool("openai-codex")
    assert pool.has_available() is True
    reclaimed = pool.select()
    assert reclaimed is not None
    assert reclaimed.last_status == "probing"
    assert reclaimed.last_status_at > old_claim_at


def test_fresh_codex_probe_is_not_reclaimed(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "active", "auth_type": "oauth", "priority": 0,
        "source": "manual:device_code", "access_token": "tok-1",
        "last_status": "probing", "last_status_at": time.time() - 60,
        "last_error_code": 429, "last_error_reason": "usage_limit_reached",
    }]}})
...
def test_openai_codex_stale_provider_reset_is_not_rechecked_before_one_hour(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "cred-1", "label": "limited", "auth_type": "oauth", "priority": 0,
        "source": "manual:device_code", "access_token": "tok-1", "last_status": "exhausted",
        "last_status_at": time.time() - 3599, "last_error_code": 429,
        "last_error_reason": "usage_limit_reached", "last_error_reset_at": time.time() + 604800,
    }]}})
...
def test_openai_codex_healthy_sibling_is_preferred_over_stale_recheck(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [
        {
            "id": "stale", "label": "stale", "auth_type": "oauth", "priority": 0,
            "source": "manual:device_code", "access_token": "tok-stale", "last_status": "exhausted",
            "last_status_at": time.time() - 7200, "last_error_code": 429,
            "last_error_reason": "usage_limit_reached", "last_error_reset_at": time.time() + 604800,
        },
        {
            "id": "healthy", "label": "healthy", "auth_type": "oauth", "priority": 1,
            "source": "manual:device_code", "access_token": "tok-healthy", "last_status": "ok",
        },
    ]}})
    ...
    entry = pool.select()
    assert entry is not None
    assert entry.id == "healthy"


def test_mark_exhausted_unknown_api_key_hint_does_not_mutate_current(tmp_path, monkeypatch):
    """An unmatched request token must fail closed instead of poisoning pool.current."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [
        {
        ...
        {
            "id": "cred-2", "label": "two", "auth_type": "oauth", "priority": 1,
            "source": "manual:device_code", "access_token": "tok-2",
        },
    ]}})
    ...
    pool.mark_exhausted_and_rotate(status_code=429, api_key_hint="unknown-request-token")

    persisted = json.loads((tmp_path / "hermes" / "auth.json").read_text())
    assert all(row.get("last_status") is None for row in persisted["credential_pool"]["openai-codex"])


def test_mark_exhausted_unknown_hint_does_not_clear_stale_codex_status(tmp_path, monkeypatch):
    """An unknown request token must not activate a stale cooldown candidate."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    monkeypatch.setattr("hermes_cli.auth._import_codex_cli_tokens", lambda: None)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {"openai-codex": [{
        "id": "stale", "label": "stale", "auth_type": "oauth", "priority": 0,
        "source": "manual:device_code", "access_token": "tok-stale",
        "last_status": "exhausted", "last_status_at": time.time() - 7200,
        "last_error_code": 429, "last_error_reason": "usage_limit_reached",
        "last_error_reset_at": time.time() + 604800,
    }]}})
    ...
    result = pool.mark_exhausted_and_rotate(
        status_code=429,
        api_key_hint="unknown-request-token",
    )
