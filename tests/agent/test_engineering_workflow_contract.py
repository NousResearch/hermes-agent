...
def test_host_receipts_must_match_the_exact_attempt_and_snapshot():
    ...
    assert (
        verify_receipts(CTX, [receipt("unit", exit_code=1), receipt("lint")]) is False
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "providers": [
                {**CATALOGUE["providers"][0], "models": ["work-model"]},
                CATALOGUE["providers"][1],
            ]
        },
        {
            "providers": [
                {**CATALOGUE["providers"][0], "authenticated": False},
                CATALOGUE["providers"][1],
            ]
        },
        {
            "providers": [
                {**CATALOGUE["providers"][0], "unavailable_models": ["plan-model"]},
                CATALOGUE["providers"][1],
            ]
        },
    ],
