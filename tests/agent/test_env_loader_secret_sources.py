...
def test_single_profile_scoped_load_keeps_override_behavior(tmp_path, monkeypatch):
    """A standalone profile launch still loads its own dotenv with override semantics.

    Its process home and scoped home agree; a foreign-home override is a routed
    profile even when the multiplex flag is off, and must not publish globally.
    """
