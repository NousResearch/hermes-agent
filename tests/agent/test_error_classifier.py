...
class TestClassifyApiError:
    ...
    def test_403_kimi_usage_limit_classified_as_billing(self):
        """Kimi Code Plan 403 'reached your usage limit for this billing cycle'
        is billing exhaustion, not auth. Must rotate credential pool so the
        second account is reached (regression guard for the two-account
        quota-failover setup)."""
        e = MockAPIError(
    ...
    def test_403_usage_limit_with_transient_signal_is_rate_limit(self):
        """403 'usage limit, try again in 5 minutes' is a transient rate
        limit, not billing exhaustion - must not burn a credential."""
        e = MockAPIError(
            "usage limit exceeded. try again in 5 minutes",
            status_code=403,
        )
