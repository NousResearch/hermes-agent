...
def test_disabled_express_surface_moves_the_client_to_the_studio_host():
    """An ``AQ.`` key routed to the express surface whose project never enabled the Vertex API is not
    a bad key: ``AQ.`` keys are issued for both surfaces, so the same request is retried on the AI
    Studio host — once — and that host is kept for the client's remaining calls."""
    ...
    client = GeminiNativeClient(api_key="AQ.express-key", http_client=DummyHTTP())
    response = client.chat.completions.create(
...
def test_disabled_express_surface_fallback_also_covers_streaming():
    ...
    client = GeminiNativeClient(api_key="AQ.express-key", http_client=SimpleNamespace(close=lambda: None))
...
def test_403_that_is_not_service_disabled_is_raised_without_a_second_request():
    ...
    permission 403 is about the key itself and must surface as-is."""
    from agent.gemini_native_adapter import GeminiAPIError, GeminiNativeClient
