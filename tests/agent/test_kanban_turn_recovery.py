...
``cli.py`` / ``cli_chat_turn_mixin.py`` driven through ``cli.main``.
"""
...
def test_single_query_exit_code_policy(monkeypatch):
    ...
    composed contract: transient provider walls release a worker as the neutral
    75, other failures and unfinished turns are honest 1s, interrupts are 130,
    success 0."""
    from cli import _single_query_exit_code
    ...
    for reason in ("rate_limit", "billing", "upstream_rate_limit", "timeout", "overloaded"):
        assert _single_query_exit_code(_failed(reason=reason)) == 75
    # a non-transient worker failure is an honest 1
    assert _single_query_exit_code(_failed(reason="auth")) == 1
