...
class TestMemoryInjectionRejectsMalformedSchema:
    ...
    def test_discover_with_reexport_fallback_no_literal(self, tmp_path, monkeypatch):
        """Plugin whose __init__.py uses re-export without either literal
        string is still detected via the fallback import path.

        Regression: _is_memory_provider_dir() fast path scans for
        "MemoryProvider" or "register_memory_provider" in the first 8K
        of __init__.py.  A provider that keeps its implementation in a
        separate file and only re-exports at the root may not contain
        either string; the fallback import path must catch it.
        """
        from plugins.memory import _is_memory_provider_dir

        plugin_dir = tmp_path / "plugins" / "reexportfallback"
        plugin_dir.mkdir(parents=True)

        (plugin_dir / "impl.py").write_text(
            "from agent.memory_provider import MemoryProvider\n"
            "class ReExportedProvider(MemoryProvider):\n"
            "    @property\n"
            "    def name(self): return 'reexportfallback'\n"
            "    def is_available(self): return True\n"
            "    def initialize(self, **kw): pass\n"
            "    def sync_turn(self, *a, **kw): pass\n"
            "    def get_tool_schemas(self): return []\n"
        ...
        (plugin_dir / "__init__.py").write_text(
            "from .impl import ReExportedProvider\n"
        )
