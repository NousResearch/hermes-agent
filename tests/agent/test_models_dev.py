...
class TestProbeVerifiedOverrides:
    ...
    github-copilot Claude 4.x is listed in models.dev at 200K/64K but the
    /v1/messages long-context path serves 1,000,000/128,000 (opus) and
    1,000,000/64,000 (sonnet). The single shared ``_resolve_probe_override``
    helper must correct all three metadata read paths consistently.
    """
    ...
    def test_helper_resolves_opus_via_hermes_id(self):
        ...
        assert result == {"context_window": 1_000_000, "max_output_tokens": 128_000}
    ...
    def test_helper_resolves_sonnet_with_sonnet_output(self):
        """Sonnet keeps its 64K max output, not opus's 128K."""
        result = _resolve_probe_override("copilot", "claude-sonnet-4.6")
        assert result == {"context_window": 1_000_000, "max_output_tokens": 64_000}
    ...
    def test_capabilities_correct_context_and_output(self):
        """Both context_window and max_output_tokens are corrected."""
        with self._setup_no_config_overrides(), \
             patch("agent.models_dev.fetch_models_dev",
                   return_value=COPILOT_UNDERREPORT_REGISTRY):
    ...
    def test_info_corrects_context_and_output(self):
        """get_model_info reports the corrected 1M/128K for opus."""
        with self._setup_no_config_overrides(), \
             patch("agent.models_dev.fetch_models_dev",
                   return_value=COPILOT_UNDERREPORT_REGISTRY):
