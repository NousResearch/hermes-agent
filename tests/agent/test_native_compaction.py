...
def _assistant_item(text):
    """Wire shape the converter emits for a locally synthesised assistant turn.

    Assistant input items must carry an explicit ``type: "message"`` and an
    ``output_text`` parts list; a bare ``{"role", "content"}`` dict is rejected
    by stricter Responses relays with HTTP 400 "Cannot determine type of
    'item'". Asserting the pre-tag shape here made the membership checks below
    vacuously true, so they are pinned to this helper instead.
    """
    return {
        "type": "message",
        "role": "assistant",
...
class TestPrunePreCheckpointItems:
    ...
    def test_adapter_applies_prune_end_to_end(self):
        ...
        assert _assistant_item("ok") in items
        assert items.index(_assistant_item("ok")) > 0
...
class TestCheckpointGatedOnCurrentEligibility:
    ...
    def test_ineligible_request_keeps_pre_feature_wire(self):
        ...
        assert _assistant_item("on it") in items
    ...
    def test_eligible_request_still_restructures(self):
        ...
        assert _assistant_item("on it") not in items
    ...
    def test_build_kwargs_without_field_does_not_prune(self):
        ...
        assert _assistant_item("on it") in kwargs["input"]
    ...
    def test_build_kwargs_with_field_prunes(self):
        ...
        assert _assistant_item("on it") not in kwargs["input"]
    ...
    def test_auxiliary_responses_adapter_never_prunes(self, monkeypatch):
        ...
        assert _assistant_item("on it") in seen["input"]
