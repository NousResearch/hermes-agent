...
def candidate(*, candidate_id="c-1", scope="project-a", status=CandidateStatus.PENDING):
...
def test_pending_save_and_read_back():
    ...
    assert read_pending(conn, "c-1") == result
...
def test_decision_transitions_pending_and_read_back():
    ...
    assert decide_pending(
        conn, "c-1", decision="approved", decided_by="owner-1", reason="verified source"
    ) is True
    assert read_pending(conn, "c-1") is None
    assert read_decision(conn, "c-1") == {
...
def test_decision_requires_actor_and_reason_and_is_one_shot():
    ...
    with pytest.raises(ValueError):
        decide_pending(conn, "c-1", decision="approved", decided_by="", reason="x")
    with pytest.raises(ValueError):
        decide_pending(conn, "c-1", decision="invalid", decided_by="owner-1", reason="x")
    assert decide_pending(conn, "c-1", decision="rejected", decided_by="owner-1", reason="not accepted") is True
    assert decide_pending(conn, "c-1", decision="approved", decided_by="owner-2", reason="retry") is False
...
def test_approved_candidate_applies_atomically_and_reads_back():
    ...
    current_table(conn)
    save_pending(conn, candidate())
    decide_pending(conn, "c-1", decision="approved", decided_by="owner-1", reason="verified")

    assert apply_approved_to_current(conn, "c-1", applied_by="owner-1", reason="apply approved patch") is True
    assert conn.execute("SELECT value FROM current_state WHERE state_key = 'budget' AND scope = 'project-a'").fetchone() == ("200",)
    assert read_decision(conn, "c-1")["status"] == "applied"
...
def test_pending_candidate_cannot_apply():
    ...
    with pytest.raises(ValueError, match="only approved"):
        apply_approved_to_current(conn, "c-1", applied_by="owner-1", reason="premature")
...
def test_current_value_conflict_rolls_back_without_apply():
    ...
    with pytest.raises(RuntimeError, match="current state conflict"):
        apply_approved_to_current(conn, "c-1", applied_by="owner-1", reason="apply approved patch")
    assert conn.execute("SELECT value FROM current_state WHERE state_key = 'budget' AND scope = 'project-a'").fetchone() == ("150",)
    assert read_decision(conn, "c-1")["status"] == "approved"
