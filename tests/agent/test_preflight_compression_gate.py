...
def test_content_above_threshold_triggers():
    ...
    # Fork's estimate_messages_tokens_rough runs ~3.5 chars/token (composition
    # chars-per-token), not 4. Use 5 chars/token so the content is comfortably
    # ABOVE threshold under the real estimator regardless of the exact ratio.
    messages = [_msg("x" * ((THRESHOLD_TOKENS + 1000) * 5))]
    assert _should_run_preflight_estimate(
        messages, PROTECT_FIRST_N, PROTECT_LAST_N, THRESHOLD_TOKENS
    ) is True
...
def test_content_below_threshold_does_not_trigger():
    ...
    tripped."""
    # Fork's estimator runs ~3.5 chars/token; use 3 chars/token so the content
    # is comfortably BELOW threshold under the real estimator (a *4 multiplier
    # over-counts at 3.5 chars/tok and would wrongly trip the gate).
    messages = [_msg("x" * ((THRESHOLD_TOKENS - 1000) * 3))]
    assert _should_run_preflight_estimate(
        messages, PROTECT_FIRST_N, PROTECT_LAST_N, THRESHOLD_TOKENS
    ) is False
