...
def test_dispatcher_owned_worker_persists_routing_decision(monkeypatch):
    monkeypatch.setenv("HERMES_KANBAN_TASK", "HSK-005")
    agent = _Agent()
...
def test_ordinary_session_does_not_persist_routing_decision(monkeypatch):
    ...
    assert agent.routing_decision == decision
    assert "routing_decision" not in agent._session_init_model_config
    assert agent._session_db.patches == []


def test_agent_initial_selection_persists_in_real_session_store(tmp_path, monkeypatch):
    ...
    try:
        decision = initialize_agent_routing_decision(
            agent,
            routing_context={"task_id": "HSK-005", "selected_profile": "rozmilo-codex"},
            selected_at="2026-09-09T12:00:00Z",
        )
...
def test_resumed_session_keeps_original_decision_and_fallback_history(tmp_path, monkeypatch):
    from hermes_state import SessionDB
...
def test_agent_fallback_updates_session_decision_with_classifier_reason(monkeypatch):
    ...
    initialize_agent_routing_decision(
        agent,
        routing_context={"task_id": "HSK-005"},
        selected_at="2026-09-09T12:00:00Z",
    )
