...
class TestMemoryContextSanitization:
    ...
    def test_emitted_memory_context_note_is_strippable(self):
        """Contract between builder and stripper: the exact note that
        ``build_memory_context_block`` emits must be removed by
        ``sanitize_context``.  If the wording changes without a matching
        fragment in ``_MEMORY_NOTE_BODY_PATTERNS``, this fails at the source
        of the drift instead of leaking the note to the model/UI."""
        from agent.memory_manager import _MEMORY_CONTEXT_NOTE, sanitize_context
        out = sanitize_context("user text\n\n" + _MEMORY_CONTEXT_NOTE)
        assert out.strip() == "user text", f"note not stripped: {_MEMORY_CONTEXT_NOTE!r}"
