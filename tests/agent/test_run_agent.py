...
class TestRunConversation:
    ...
    def test_fork_execution_provenance_reaches_finalize_hooks(self, agent):
        """A caller that seeds `_execution_kind`/`_execution_id` before
        `run_conversation` has them preserved by `build_turn_context` (any
        non-"live" kind), so the finalize-path hooks report that provenance
        verbatim. (A detached background-review fork is `_persist_disabled`
        and skips these two hooks entirely; the tag contract still holds for
        every execution that does fire them.)"""
        self._setup_agent(agent)
