...
def test_loop_cap_zero_disables_and_junk_falls_back():
    ...
    assert LoopCapConfig.from_mapping({"max_tool_searches": "x"}).max_tool_searches == LoopCapConfig().max_tool_searches
...
def test_tool_search_cap_blocks_after_limit_regardless_of_hard_stop():
    # The tool_search discovery bridge is capped like web_search: distinct
    # queries dodge the repeated-failure detector, so a block here proves the
    # per-turn loop cap fired independently of hard_stop_enabled.
    controller = ToolCallGuardrailController(
        ToolCallGuardrailConfig(
            hard_stop_enabled=False,
