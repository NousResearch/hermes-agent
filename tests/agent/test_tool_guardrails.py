...
def test_per_tool_mapping_parses_and_keeps_legacy_names():
...
def test_per_tool_cap_blocks_any_named_tool_regardless_of_hard_stop():
    ...
    for i in range(2):
        assert controller.before_call("todo", {"action": f"write{i}"}).action == "allow"
...
def test_legacy_web_search_cap_still_enforced_without_per_tool():
    """Regression guard: a _LOOP_CAPS tool with no per_tool entry keeps its named cap."""
    controller = ToolCallGuardrailController(
        ToolCallGuardrailConfig(hard_stop_enabled=False, loop_caps=LoopCapConfig(max_web_searches=1))
    )
    assert controller.before_call("web_search", {"query": "a"}).action == "allow"
    decision = controller.before_call("web_search", {"query": "b"})
    assert decision.action == "block"
    assert decision.code == "loop_web_search_cap"
