...
def test_session_end_lifecycle_hook_includes_gateway_route(mock_invoke_hook):
    """Turn hooks expose the stable gateway key and current source metadata."""
    agent = _StubAgent(raise_in=())
    ...
    assert end_calls[0].kwargs["session_key"] == agent._gateway_session_key
    assert end_calls[0].kwargs["source"] == agent._gateway_session_source
    assert end_calls[0].kwargs["source"] is not agent._gateway_session_source
