...
class TestCodexAppServerModule:
    ...
    def test_close_escalates_to_kill_when_terminate_raises(self) -> None:
    ...
    def test_close_raises_fixed_error_when_process_quiescence_is_unconfirmed(
        self,
    ) -> None:
        from agent.transports.codex_app_server import CodexAppServerClient
        ...
        client._pending = {}
        client._pending_lock = threading.Lock()
        client.__dict__["_proc"] = FaultyProcess()

        with pytest.raises(RuntimeError) as exc_info:
            client.close()

        assert str(exc_info.value) == "codex app-server process teardown unconfirmed"
        assert "private" not in str(exc_info.value)
        assert client._closed is True

    def test_close_confirms_quiescence_after_kill_raises(self) -> None:
        from agent.transports.codex_app_server import CodexAppServerClient
        ...
        class RetryableProcess:
            ...
            def kill(self):
                ...
                if self.kill_calls == 1:
                    raise OSError("private first kill detail")

            def wait(self, timeout=None):
                self.wait_calls += 1
                return 0

        proc = RetryableProcess()
        client = object.__new__(CodexAppServerClient)
        client._closed = False
        client._pending = {}
        client._pending_lock = threading.Lock()
    ...
    def test_close_unblocks_in_flight_request(self, monkeypatch) -> None:
        import agent.transports.codex_app_server as cas
        ...
        monkeypatch.setattr(cas.subprocess, "Popen", lambda *args, **kwargs: FakeProcess())
        ...
        thread.start()
        assert request_written.wait(timeout=1)
        try:
            client.close()

            assert finished.wait(timeout=0.5)
            assert isinstance(result.get("error"), cas.CodexAppServerError)
            assert "closed" in str(result["error"]).lower()
        finally:
            thread.join(timeout=3)

    def test_request_after_close_does_not_retain_pending_rpc(self, monkeypatch) -> None:
        import agent.transports.codex_app_server as cas
