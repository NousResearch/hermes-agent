...
def test_interrupt_failure_log_is_metadata_only(caplog: pytest.LogCaptureFixture):
    client = FakeClient()
    session = make_session(client)
    setattr(session, "_client", client)
    session._thread_id = "thread-fake-001"
    setattr(
        client,
        "_request_handler",
        lambda *_args: (_ for _ in ()).throw(
            RuntimeError("private /tmp/codex.sock?token=secret")
        ),
    )
    ...
    assert "turn/interrupt non-fatal" in caplog.text
    assert "private" not in caplog.text
    assert "token=secret" not in caplog.text


def test_compact_uses_local_client_snapshot_while_close_clears_session_fields():
    entered_alive_check = threading.Event()
    release_alive_check = threading.Event()

    class ClosingClient(FakeClient):
        def is_alive(self) -> bool:
            entered_alive_check.set()
            assert release_alive_check.wait(timeout=2)
            return False
    ...
    def compact() -> None:
        try:
            holder["result"] = session.compact_thread(
                turn_timeout=2, notification_poll_timeout=0.01
            )
    ...
    assert entered_alive_check.wait(timeout=2)
    ...
    assert holder["result"].should_retire is True


@pytest.mark.parametrize("operation_name", ["run_turn", "compact_thread"])
def test_close_after_start_before_client_snapshot_returns_interrupted_retirement(
    operation_name: str,
):
    client = FakeClient()
    session = make_session(client)
    ensure_started = session.ensure_started

    def start_then_close() -> str:
        thread_id = ensure_started()
        session.close()
        return thread_id

    session.ensure_started = start_then_close

    if operation_name == "run_turn":
        result = session.run_turn("must not reach closed transport")
    else:
...
def test_startup_initialization_runs_under_single_owner_lock():
    class Ownership:
        held = False
    ...
    class OwnershipClient(FakeClient):
        def initialize(self, **kwargs):
            assert ownership.held is True
            return super().initialize(**kwargs)

    client = OwnershipClient()
    session = make_session(client)
    setattr(session, "_startup_lock", OwnershipLock())

    assert session.ensure_started() == "thread-fake-001"
    assert ownership.held is False


def test_close_waits_for_startup_unwind_after_client_teardown_failure():
    client = FakeClient()
    client.close = MagicMock(side_effect=RuntimeError("private teardown detail"))
    session = make_session(client)
    setattr(session, "_client", client)
    session._startup_active = True
    startup_done = MagicMock()
    startup_done.wait.return_value = True
    session._startup_done = startup_done

    with pytest.raises(RuntimeError, match="session teardown unconfirmed"):
        session.close()

    startup_done.wait.assert_called_once()
...
class TestLifecycle:
    ...
    def test_close_waits_for_blocked_factory_and_closes_published_client(self):
        ...
        class BlockingStartupClient(FakeClient):
            ...
            def initialize(self, **kwargs):
                self.close_seen.wait(timeout=2)
                self.initialize_release.wait(timeout=2)
                if self._closed:
        ...
        session = CodexAppServerSession(
            cwd="/tmp",
            client_factory=blocking_factory,
        )
        publication_lock = ObservableLock()
        startup_done = ObservableEvent(session._startup_done)
        setattr(session, "_client_publication_lock", publication_lock)
        setattr(session, "_startup_done", startup_done)
        ...
        close_thread = threading.Thread(
            target=lambda: (session.close(), close_finished.set())
        )
        start_thread.start()
        assert factory_entered.wait(timeout=1)
        close_thread.start()
        assert publication_lock.second_entry.wait(timeout=1)
        assert not close_finished.is_set()

        factory_release.set()
        assert client.close_seen.wait(timeout=1)
        assert startup_done.wait_entered.wait(timeout=1)
        assert not close_finished.is_set()
        client.initialize_release.set()
    ...
    def test_closed_session_rejects_all_future_startup(self):
        client_factory = MagicMock(return_value=FakeClient())
        session = CodexAppServerSession(cwd="/tmp", client_factory=client_factory)

        session.close()

        with pytest.raises(InterruptedError, match="startup interrupted"):
            session.ensure_started()
        turn = session.run_turn("must not start")
        compact = session.compact_thread()

        assert turn.interrupted is True
        assert compact.interrupted is True
        client_factory.assert_not_called()

    def test_factory_failure_releases_startup_ownership_before_close(self):
        ...
        assert session._startup_done.is_set()
        session.close()
...
class TestRunTurn:
    def test_prestart_interrupt_does_not_spawn_codex_client(self):
        client_factory = MagicMock(return_value=FakeClient())
        session = CodexAppServerSession(cwd="/tmp", client_factory=client_factory)
        session.request_interrupt()

        result = session.run_turn("must not start")

        assert result.interrupted is True
        client_factory.assert_not_called()

    def test_interrupt_of_started_idle_session_requires_retirement(self):
        ...
        session.request_interrupt()
        result = session.run_turn("must not reuse closed transport")
        repeated = session.run_turn("terminal interrupt remains latched")

        assert result.interrupted is True
        assert result.should_retire is True
        assert repeated.interrupted is True
        assert repeated.error is None
        assert client._closed is True
        assert not any(method == "turn/start" for method, _params in client.requests)

    def test_interrupt_close_failure_is_fixed_and_retryable(self):
        class FailingCloseClient(FakeClient):
            ...
            def close(self):
                ...
                if self.close_calls == 1:
                    raise RuntimeError("/private/path?token=secret-value")
                super().close()
    ...
    def test_interrupt_during_startup_closes_client_before_returning(self):
        class BlockingStartupClient(FakeClient):
            def __init__(self):
                super().__init__()
        ...
        def run():
            result_holder["result"] = session.run_turn("must stop during startup")
            finished.set()

        thread = threading.Thread(target=run)
        thread.start()
        assert client.initialize_entered.wait(timeout=1)
        try:
            session.request_interrupt()
    ...
    def test_interrupt_after_startup_check_prevents_turn_start(self):
        class CloseAwareClient(FakeClient):
            def request(self, method, params=None, timeout=30.0):
                if self._closed:
                    raise RuntimeError("client closed")
        ...
        client = CloseAwareClient()
        session = make_session(client)
        coercion_entered = threading.Event()
        coercion_release = threading.Event()
        result_holder = {}

        def blocking_coerce(user_input):
            coercion_entered.set()
            coercion_release.wait(timeout=2)
            return str(user_input)

        def run():
            result_holder["result"] = session.run_turn("must not start")

        with patch.object(session_mod, "_coerce_turn_input_text", blocking_coerce):
            thread = threading.Thread(target=run)
            thread.start()
            assert coercion_entered.wait(timeout=1)
            try:
                session.request_interrupt()
            finally:
                coercion_release.set()
                thread.join(timeout=2)
    ...
    def test_interrupt_closes_active_turn_without_leaking_transport_error(self):
        class CloseAwareActiveClient(FakeClient):
            def __init__(self):
                super().__init__()
        ...
        client = CloseAwareActiveClient()
        session = make_session(client)
        result_holder = {}

        def run():
            result_holder["result"] = session.run_turn(
                "stop active turn", notification_poll_timeout=0.01
            )
        ...
        assert not thread.is_alive()
        assert result_holder["result"].interrupted is True
        assert result_holder["result"].should_retire is True
    ...
    @pytest.mark.parametrize("with_server_request", [False, True])
    def test_close_race_keeps_local_thread_filter_for_queued_notifications(
        self, with_server_request
    ):
        class BlockingNotificationClient(FakeClient):
            def __init__(self):
                super().__init__()
                self.notification_entered = threading.Event()
                self.notification_release = threading.Event()

            def take_notification(self, timeout: float = 0.0):
                self.notification_entered.set()
                self.notification_release.wait(timeout=2)
                return super().take_notification(timeout)

            def close(self):
                # Keep the captured local client readable long enough to
                # expose filtering after session.close() clears publication.
                return None

        client = BlockingNotificationClient()
        if with_server_request:
            client.queue_server_request("unknown/request", request_id="request-1")
        client.queue_notification(
            "item/completed",
            threadId="thread-child-001",
            # Deliberately collide on turn identity: the foreign thread is
            # the only dimension that may reject this notification.
            turnId="turn-fake-001",
            item={
                "type": "agentMessage",
                "id": "foreign-message",
                "text": "must stay foreign",
            },
        )
        session = make_session(client)
        result_holder = {}
        ...
        run_thread.start()
        assert client.notification_entered.wait(timeout=1)
        session.close()
        assert session._thread_id is None
        client.notification_release.set()
        run_thread.join(timeout=2)

        assert not run_thread.is_alive()
        assert result_holder["result"].projected_messages == []
    ...
    def test_steer_close_race_returns_false_without_leaking_runtime_error(
        self, caplog
    ):
        sensitive = "private /tmp/codex.sock?token=steer-secret"

        class CloseRaceClient(FakeClient):
            def __init__(self):
                super().__init__()
                self.request_entered = threading.Event()
                self.close_released_request = threading.Event()

            def request(self, method, params=None, timeout=30.0):
                if method != "turn/steer":
                    return super().request(method, params, timeout)
                self.request_entered.set()
                self.close_released_request.wait(timeout=5)
                if self._closed:
            ...
            def close(self):
                super().close()
                self.close_released_request.set()

        client = CloseRaceClient()
        session = make_session(client)
        session.ensure_started()
        ...
        def steer():
            try:
                result["accepted"] = session.request_steer("Use Postgres instead")
            except BaseException as exc:  # pragma: no cover - assertion reports escape
                errors.append(exc)

        thread = threading.Thread(target=steer)
        with caplog.at_level("DEBUG"):
            thread.start()
            assert client.request_entered.wait(timeout=1)
            session.close()
        ...
        assert "steer-secret" not in caplog.text
...
def test_run_turn_serializes_compact_thread_queue_consumption():
    phase: Queue[str] = Queue()
    run_poll_entered = threading.Event()
    run_poll_release = threading.Event()
    compact_request_release = threading.Event()
    ...
    class BlockingClient(FakeClient):
        ...
        def request(self, method, params=None, timeout=30.0):
            if method == "thread/compact/start":
                phase.put("overlap")
                compact_request_release.wait(timeout=2)
                self.queue_notification(
                    "turn/started",
                    threadId="thread-fake-001",
                    turn={"id": "compact-turn-001"},
                )
                self.queue_notification(
                    "turn/completed",
                    threadId="thread-fake-001",
                    turn={"id": "compact-turn-001", "status": "completed"},
                )
            return super().request(method, params, timeout)

        def take_notification(self, timeout: float = 0.0):
            if self._first_notification:
                self._first_notification = False
                run_poll_entered.set()
                run_poll_release.wait(timeout=2)
            return super().take_notification(timeout)
    ...
    session = make_session(client)
    setattr(session, "_operation_lock", ObservableLock())
    results = {}
    run_thread = threading.Thread(
        target=lambda: results.setdefault(
            "turn",
            session.run_turn(
                "serialize queue consumers",
                turn_timeout=2.0,
                notification_poll_timeout=0.01,
            ),
        )
    ...
    run_thread.start()
    assert run_poll_entered.wait(timeout=1)
    compact_thread.start()
    try:
        assert phase.get(timeout=1) == "serialized"
    finally:
        run_poll_release.set()
        compact_request_release.set()
        run_thread.join(timeout=2)
        compact_thread.join(timeout=2)

    assert not run_thread.is_alive()
    assert not compact_thread.is_alive()
    assert results["turn"].interrupted is False
    assert results["compact"].interrupted is False


class TestCompactThread:
    def test_interrupt_before_compact_request_prevents_start(self):
        class CloseAwareClient(FakeClient):
            def request(self, method, params=None, timeout=30.0):
                if self._closed:
                    raise RuntimeError("client closed")
        ...
        projector_release = threading.Event()
        result_holder = {}
        real_projector = session_mod.CodexEventProjector

        def blocking_projector():
            projector_entered.set()
            projector_release.wait(timeout=2)
            return real_projector()

        def run():
            result_holder["result"] = session.compact_thread(turn_timeout=1)

        with patch.object(session_mod, "CodexEventProjector", blocking_projector):
            thread = threading.Thread(target=run)
            thread.start()
            assert projector_entered.wait(timeout=1)
            try:
                session.request_interrupt()
            finally:
        ...
        assert not thread.is_alive()
        assert not any(
            method == "thread/compact/start" for method, _params in client.requests
        )
        assert result_holder["result"].interrupted is True
        assert result_holder["result"].should_retire is True
    ...
    def test_compact_completion_reconciles_interrupt_from_terminal_callback(self):
        client = FakeClient()
        client.queue_notification(
            "turn/started",
            threadId="thread-fake-001",
        ...
        def interrupt_on_completion(note):
            if note.get("method") == "turn/completed":
                session_holder["session"].request_interrupt()

        session = make_session(client, on_event=interrupt_on_completion)
