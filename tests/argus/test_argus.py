...
class TestArgus(unittest.TestCase):
    ...
    def test_repeat_tool_calls_detection(self):
        ...
        # Detect entropy
        detections = _entropy.detect_repeat_tool_calls(self.argus.cursor, session_id)
    ...
    def test_repeat_commands_detection(self):
        ...
        # Detect entropy
        detections = _entropy.detect_repeat_commands(self.argus.cursor, session_id)
    ...
    def test_stuck_loop_detection(self):
        ...
        # Detect entropy
        detections = _entropy.detect_stuck_loops(self.argus.cursor, session_id)
    ...
    def test_no_file_changes_detection(self):
        ...
        # Detect entropy
        detections = _entropy.detect_no_file_changes(self.argus.cursor, session_id)
    ...
    def test_error_cascade_detection(self):
        ...
        detections = _entropy.detect_error_cascade(self.argus.cursor, session_id)
    ...
    def test_budget_pressure_high_ratio_with_entropy(self):
        ...
        mock_messages = [
            {"role": "assistant", "timestamp": str(time.time() - 300)}
        ] * 75
        ...
        with patch("entropy.SessionDB", return_value=mock_db):
            detections = _entropy.detect_budget_pressure(
                self.argus.cursor, session_id, 90, "/fake/state.db"
            )
    ...
    def test_budget_pressure_critical_at_85_percent(self):
        ...
        mock_messages = [
            {"role": "assistant", "timestamp": str(time.time() - 600)}
        ] * 80
    ...
    def test_budget_pressure_no_flag_low_ratio(self):
        ...
        mock_messages = [
            {"role": "assistant", "timestamp": str(time.time() - 120)}
        ] * 20
    ...
    def test_decision_restart_on_critical_entropy(self):
        ...
        directive_checks = [{"check_type": "pipeline_compliance", "passed": True}]

        # Make decision
        decision = self.argus.make_decision(
            session_id, entropy_detections, directive_checks
        )
    ...
    def test_decision_restart_on_directive_violation(self):
        ...
        entropy_detections = []
        directive_checks = [{"check_type": "pipeline_compliance", "passed": False}]
    ...
    def test_action_recording(self):
        ...
        self.argus.cursor.execute(
            "SELECT * FROM watcher_actions WHERE session_id = ?", (session_id,)
        )
        result = self.argus.cursor.fetchone()
...
class TestArgusRestart(unittest.TestCase):
    ...
    @patch("actions.pause_job")
    @patch("actions.resume_job")
    def test_restart_cron_session_pauses_and_resumes(self, mock_resume, mock_pause):
        ...
        self.argus.cursor.execute(
            "SELECT restart_count, status FROM sessions WHERE session_id = ?",
            (session_id,),
        )
    ...
    def test_restart_delegate_session_kills_and_respawns(self, mock_run):
        ...
        kill_called = any(
            "kill" in c.lower() or "SIGTERM" in c or "SIGKILL" in c for c in calls
        )
        self.assertTrue(kill_called, "Should kill delegate process on restart")

    def test_restart_manual_session_records_action(self):
        """Manual session restart via execute_action records action but cannot force restart."""
        ...
        # Action recording is handled by execute_action, not _restart_session
        self.argus.execute_action(
            session_id, {"action": "restart", "reason": "Test restart"}
        )

        # Verify action was recorded by execute_action
        self.argus.cursor.execute(
            "SELECT * FROM watcher_actions WHERE session_id = ?", (session_id,)
        )
...
class TestArgusPromptInjection(unittest.TestCase):
    ...
    def test_inject_prompt_delegate_respawns_with_prompt(self, mock_run):
        ...
        corrective_prompt = (
            "ENTROPY CORRECTION: Stop looping. Complete one task at a time."
        )
        self.argus._inject_prompt(session_id, corrective_prompt)
        ...
        self.assertTrue(
            kill_called, "Should kill delegate to respawn with corrective prompt"
        )
...
class TestArgusTelegramNotification(unittest.TestCase):
    ...
    def test_send_notification_formats_message(self, mock_run):
        ...
        self.argus._send_notification(
            session_id, "kill", "High entropy: repeat tool calls"
        )

        self.argus.cursor.execute(
            "SELECT message FROM notifications WHERE session_id = ?", (session_id,)
        )
...
class TestArgusCollectMetrics(unittest.TestCase):
    ...
    def test_collect_metrics_populates_tool_calls(self):
        ...
        self.argus.cursor.execute(
            "SELECT tool_call_count FROM sessions WHERE session_id = ?", (session_id,)
        )
        row = self.argus.cursor.fetchone()
    ...
    def test_collect_metrics_updates_quality_score(self):
        ...
        self.argus.cursor.execute(
            "SELECT quality_gate_score FROM sessions WHERE session_id = ?",
            (session_id,),
        )
        row = self.argus.cursor.fetchone()
...
class TestArgusSandboxedCron(unittest.TestCase):
    ...
    @patch(
        "actions.pause_job", side_effect=Exception("cron.jobs unavailable in sandbox")
    )
    @patch(
        "actions.resume_job", side_effect=Exception("cron.jobs unavailable in sandbox")
    )
    def test_restart_cron_handles_cron_jobs_failure(self, mock_resume, mock_pause):
        ...
        # Should still record the restart action via execute_action
        self.argus.cursor.execute(
            "SELECT * FROM watcher_actions WHERE session_id = ?", (session_id,)
        )
        action = self.argus.cursor.fetchone()
    ...
    @patch("actions.pause_job", side_effect=Exception("cron.jobs unavailable"))
    @patch("actions.trigger_job", side_effect=Exception("cron.jobs unavailable"))
    @patch("actions.update_job", side_effect=Exception("cron.jobs unavailable"))
    def test_inject_prompt_cron_handles_cron_jobs_failure(
        self, mock_update, mock_trigger, mock_pause
    ):
    ...
    @patch(
        "argus.subprocess.run",
        side_effect=FileNotFoundError("[Errno 2] No such file or directory: kill"),
    )
    def test_restart_delegate_handles_missing_kill_command(self, mock_run):
        ...
        self.argus.cursor.execute(
            "SELECT restart_count FROM sessions WHERE session_id = ?", (session_id,)
        )
