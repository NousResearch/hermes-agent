...
def _json_instance_equal(left: Any, right: Any) -> bool:
    """Compare values using JSON Schema's JSON-instance equality rules."""
    ...
    if isinstance(left, list) or isinstance(right, list):
        return (
            ...
            and all(
                _json_instance_equal(left_item, right_item)
    ...
    if isinstance(left, Mapping) or isinstance(right, Mapping):
        return (
            ...
            and all(_json_instance_equal(left[key], right[key]) for key in left)
        )
    return False
...
def _schema_errors(
    ...
    if "$ref" in schema:
        errors = _schema_errors(
            instance, _resolve_ref(root, schema["$ref"]), root, path
        )
        siblings = {key: value for key, value in schema.items() if key != "$ref"}
        if siblings:
            errors.extend(_schema_errors(instance, siblings, root, path))
        return errors

    errors: list[str] = []
    if "const" in schema:
        expected = schema["const"]
        if not _json_instance_equal(instance, expected):
            errors.append(f"{path}: expected const {expected!r}")
    if "enum" in schema and not any(
        _json_instance_equal(instance, expected) for expected in schema["enum"]
    ):
    ...
    if isinstance(instance, list):
        ...
        if schema.get("uniqueItems") and any(
            _json_instance_equal(instance[left], instance[right])
    ...
    if isinstance(negated, Mapping) and not _schema_errors(
        instance, negated, root, path
    ):
...
def test_shared_conformance_vectors_match_python_evaluator() -> None:
    ...
    for vector in vectors["vectors"]:
        ...
        decision = evaluate_authority_operation(artifact, request)
