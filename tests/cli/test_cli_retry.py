...
def test_retry_fails_closed_when_same_target_has_divergent_prefix(tmp_path):
    cli = _make_cli()
    cli._session_db.close()
    db = SessionDB(db_path=tmp_path / "state.db")
    cli._session_db = db
    cli.session_id = "cli-prefix-mismatch"
    db.create_session(cli.session_id, source="cli")
    db.append_message(cli.session_id, "user", "DURABLE PREFIX ASK")
    db.append_message(cli.session_id, "assistant", "durable prefix answer")
    db.append_message(cli.session_id, "user", "SAME TARGET")
    db.append_message(cli.session_id, "assistant", "failed answer")

    history = db.get_messages_as_conversation(cli.session_id)
    history[0]["content"] = "WARM PREFIX ASK"
    history[1]["content"] = "warm prefix answer"
    cli.conversation_history = history
    cli._pending_input = MagicMock()
    before_rows = _message_rows(db, cli.session_id)
...
def test_rewind_matches_warm_history_to_its_persistence_projection(
    ...
    if prefix_kind == "buried_ephemeral":
        ...
        expected_prefix = ["OLDER ASK", "candidate answer\nverified answer"]
        ...
        expected_prefix = ["OLDER ASK\n[screenshot]", "older answer"]
...
def test_retry_last_rejects_media_before_db_or_memory_mutation():
    ...
    assert cli.retry_last() is None
    assert cli.conversation_history is history
    db.get_active_conversation_snapshot.assert_not_called()
...
def test_retry_last_db_failure_leaves_warm_history_unchanged():
    cli = _make_cli()
    db = MagicMock()
    db.get_active_conversation_snapshot.side_effect = OSError("db unavailable")
    cli._session_db = db
    history = [
        {"role": "user", "content": "retry me"},
        {"role": "assistant", "content": "old answer"},
