...
def test_feature_probe_fails_closed_when_env_sanitizer_fails(monkeypatch):
    from tools.computer_use import linux_wayland

    def fail_sanitizer(_env):
        raise RuntimeError("sanitizer unavailable")

    monkeypatch.setattr("tools.environments.local._sanitize_subprocess_env", fail_sanitizer)
    run = Mock()
    monkeypatch.setattr(linux_wayland.subprocess, "run", run)

    features = linux_wayland.probe_driver_features("/driver", _wayland_env())

    assert features == linux_wayland.CuaDriverFeatures()
    run.assert_not_called()


def test_native_wayland_requires_opt_in_and_driver_feature(monkeypatch):
    ...
    assert not native_wayland_enabled("/driver", {}, env, features=native)
...
def test_stale_wayland_socket_refuses_native_wayland_opt_in(monkeypatch):
    from tools.computer_use.linux_wayland import CuaDriverFeatures, native_wayland_enabled
...
def test_manual_env_opt_in_remains_subject_to_feature_gate(monkeypatch):
    from tools.computer_use.linux_wayland import CuaDriverFeatures, WAYLAND_ENABLE_ENV, native_wayland_enabled
    ...
    assert native_wayland_enabled("/driver", {}, env, features=native)
    assert not native_wayland_enabled("/driver", {}, env, features=no_features)


def test_native_wayland_child_env_forwards_safe_opt_in(monkeypatch):
    from tools.computer_use import linux_wayland

    _socket_ready(monkeypatch)
    monkeypatch.setattr(
        linux_wayland,
        "probe_driver_features",
        lambda *_args, **_kwargs: linux_wayland.CuaDriverFeatures(
            wayland_native=True,
            manifest_supported=True,
        ),
    )
    got = linux_wayland.native_wayland_child_env(
        "/driver",
        {"computer_use": {"native_wayland": True}},
        _wayland_env(),
    )
    assert got[linux_wayland.WAYLAND_ENABLE_ENV] == "1"


def test_native_wayland_child_env_fails_closed_without_feature(monkeypatch):
    from tools.computer_use import linux_wayland
    ...
    assert got[linux_wayland.WAYLAND_ENABLE_ENV] == "0"
