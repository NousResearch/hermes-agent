...
def _real_hermes_roots_for_gate() -> list[Path]:
    ...
    Inlined rather than imported from ``hermes_state_guard``: this runs at
    conftest import time, and importing ``hermes_state`` here would freeze its
    ``DEFAULT_DB_PATH`` against the unsandboxed ``HERMES_HOME``, which is the
    escape this gate closes. Mirrors ``_real_platform_state_roots`` there.

    ``Path.home()`` alone is wrong because a dispatched worker has ``HOME``
    remapped to ``<root>/profiles/<name>/home``.
    """
    roots: list[Path] = []
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        # _REAL_KANBAN_ROOT below comes from the pre-sandbox env, which is
        # wrong under a remapped HOME. The module guard resolves the root
        # from HERMES_REAL_HOME instead, so consult both.
