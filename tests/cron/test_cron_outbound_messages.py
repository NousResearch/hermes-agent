...
def isolate_cron_authority():
    from cron import outbound
    token = outbound._current_run.set(None)
    yield
...
class TestSendGate:
    def _bind_cron(self, monkeypatch, *, allow=True):
        from cron import outbound
        run = outbound.CronMessagingRun(
            "job-1", "run-1",
            {"platform": "telegram", "chat_id": "2027045491", "thread_id": "104992"},
            allow, outbound.OUTBOUND_FILE,
        )
        outbound._current_run.set(run)
