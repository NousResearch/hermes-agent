...
def test_cron_paths_unmangle_windows_hermes_home(monkeypatch):
    import cron.jobs as jobs_mod
    import hermes_constants

    original_platform = hermes_constants.sys.platform
    monkeypatch.setattr(hermes_constants.sys, "platform", "win32")
    monkeypatch.setenv("HERMES_HOME", r"C:\c\Users\kevin\AppData\Local\hermes")

    reloaded = importlib.reload(jobs_mod)
    try:
        assert str(reloaded.HERMES_DIR) == r"C:\Users\kevin\AppData\Local\hermes"
        assert r"C:\c\Users" not in str(reloaded.JOBS_FILE)
    finally:
