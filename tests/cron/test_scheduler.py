...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        """When config.yaml is malformed, cron fails closed: the run is blocked with a
        loud, plain-language config.yaml message instead of silently running on the
        last-known-good copy the interactive loader would have served."""
        ...
        with patch("cron.scheduler._hermes_home", tmp_path), \
            ...
            with caplog.at_level(logging.WARNING):
                success, output, _response, error = run_job(job)

        assert success is False
        assert error is not None and "[blocked_config]" in error
        assert "config.yaml could not be loaded" in f"{output} {error}"
