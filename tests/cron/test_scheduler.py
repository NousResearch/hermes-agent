...
class TestRunJobModelFieldTypeGuard:
    """Regression: ``job["model"]`` is normalized to ``str`` by
    ``create_job`` / ``update_job`` (see cron/jobs.py: ``normalized_model =
    str(model).strip() if isinstance(model, str) else None``), but legacy job
    records persisted before that guard was added — or jobs.json files
    hand-edited or migrated from another Hermes install — can still contain
    a dict like ``{"provider": "...", "model": "..."}``. The bare
    ``job.get("model") or os.getenv("HERMES_MODEL") or ""`` read in
    ``run_job`` used to forward that dict straight to ``AIAgent(...)``,
    which crashed in ``_anthropic_prompt_cache_policy`` with
    ``AttributeError: 'dict' object has no attribute 'lower'`` and
    poisoned the job's last_status forever — every subsequent tick
    re-loaded the same dict and re-crashed identically.
    """

    @staticmethod
    def _patches(tmp_path, mock_agent_cls):
        """Common mocks shared by every model-field test in this class."""
        fake_db = MagicMock()
        return [
            patch("cron.scheduler._hermes_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            ...
            patch("run_agent.AIAgent", mock_agent_cls),
        ]

    def test_dict_shaped_model_does_not_crash_and_inner_model_is_used(self, tmp_path):
        """Legacy ``{"provider": "...", "model": "..."}`` payload must be
        flattened to the inner model name so the AIAgent receives a str,
        not a dict."""
        job = {
            "id": "legacy-dict-model",
            "name": "legacy",
            "prompt": "hello",
            "model": {"provider": "custom", "model": "MiniMax-M3"},
        }
        mock_agent = MagicMock()
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        mock_agent_cls = MagicMock(return_value=mock_agent)

        ctx_managers = self._patches(tmp_path, mock_agent_cls)
        # Enter all patches
        ...
        # Invariant: the agent was constructed AND received a str model.
        assert success is True
        assert error is None
        assert mock_agent_cls.called, "AIAgent must be constructed even with legacy model shape"
        model_kwarg = mock_agent_cls.call_args.kwargs["model"]
        assert isinstance(model_kwarg, str), (
            f"AIAgent model kwarg must be str, got {type(model_kwarg).__name__}: {model_kwarg!r}"
        )
        assert model_kwarg == "MiniMax-M3"

    def test_str_model_passes_through_unchanged(self, tmp_path):
        """The guard must not regress the happy path: a plain string model
        still reaches AIAgent exactly as stored."""
        job = {
            "id": "str-model",
            "name": "str",
            "prompt": "hello",
    ...
    def test_dict_with_non_string_inner_model_falls_back_to_empty_string(self, tmp_path):
        """If the legacy dict shape has a non-string ``"model"`` value
        (corruption, weird migration, hand-edited), the guard must coerce
        to ``""`` instead of forwarding the bad inner value."""
        job = {
            "id": "bad-inner",
            "name": "bad",
            "prompt": "hello",
            "model": {"provider": "custom", "model": 12345},
        }
        mock_agent = MagicMock()
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        mock_agent_cls = MagicMock(return_value=mock_agent)
        ...
        assert success is True
        assert error is None
        assert mock_agent_cls.call_args.kwargs["model"] == ""

    def test_missing_model_field_still_runs(self, tmp_path):
        """The guard must not invent a model when ``job["model"]`` is
        absent — it should fall through to env / config / empty."""
        job = {
            "id": "no-model",
            "name": "no-model",
            "prompt": "hello",
        ...
        assert success is True
        assert error is None
        # Whatever it resolves to, it MUST be a str — never a dict.
        model_kwarg = mock_agent_cls.call_args.kwargs["model"]
        assert isinstance(model_kwarg, str)
