...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # would fail closed and the slot would never come up. Register the
         # bundled basic password provider so the gate has a provider and
         # the dashboard binds. Keeps this test focused on the s6
...
def test_dashboard_opt_in_starts(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # bundled basic password provider so the gate has a provider and
         # the dashboard binds (vs fail-closed). Keeps the test focused on
         # s6 supervision, not auth.
         "-e", "HERMES_DASHBOARD_BASIC_AUTH_USERNAME=admin",
         "-e", "HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
