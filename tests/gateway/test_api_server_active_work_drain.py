...
class TestAPIServerAdapterWorkCount:
    ...
    async def test_concurrency_limit_excludes_current_pending_admission(self):
        ...
        # 2026-07-15 parity merge: the downstream chat handler may return a
        # route/service status depending on surrounding test state. This test's
        # contract is only that the request's own pending reservation is
        # excluded from the concurrency cap, so it must not be rejected with 429.
        assert response.status != 429
