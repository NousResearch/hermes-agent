...
class TestRunBackgroundTask:
    ...
    @pytest.mark.asyncio
    async def test_successful_task_skips_missing_media_attachment(self, tmp_path):
        """Missing MEDIA local files must not be sent as attachments."""
        runner = _make_runner()
        mock_adapter = AsyncMock()
        mock_adapter.send = AsyncMock()
        mock_adapter.send_document = AsyncMock()
        missing = tmp_path / "missing.pdf"
        response = f"Here is the file\nMEDIA:{missing}"
        mock_adapter.extract_media = MagicMock(return_value=([(str(missing), False)], "Here is the file"))
        mock_adapter.extract_images = MagicMock(return_value=([], "Here is the file"))
        runner.adapters[Platform.TELEGRAM] = mock_adapter
