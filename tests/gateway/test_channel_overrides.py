...
class TestResolveSessionAgentRuntimePriority:
    ...
    def test_channel_override_beats_global(self):
        ...
        def resolve_channel_provider(provider, model=None):
        ...
        with patch("gateway.run._resolve_gateway_model", return_value="global/model"), \
             ...
             patch(
                 ...
                 side_effect=resolve_channel_provider,
             ):
        ...
        assert resolved == {"provider": "openrouter", "model": "channel/model"}

    @pytest.mark.parametrize(
        ("global_model", "global_mode", "channel_model", "resolved_mode"),
        [
            ("gpt-5.6-sol", "codex_responses", "grok-4.6", "chat_completions"),
            ("grok-4.6", "chat_completions", "gpt-5.6-sol", "codex_responses"),
        ],
    )
    def test_model_only_channel_override_re_resolves_current_provider_transport(
        self, global_model, global_mode, channel_model, resolved_mode
    ):
        resolved = {}
        ...
        runner.config = GatewayConfig(
            platforms={
                Platform.DISCORD: PlatformConfig(
                    enabled=True,
                    channel_overrides={
                        "chan_1": ChannelOverride(model=channel_model),
                    },
                ),
            },
        )
        ...
        with patch("gateway.run._resolve_gateway_model", return_value=global_model), \
            ...
            model, runtime = runner._resolve_session_agent_runtime(
                source=source,
                user_config={"model": {"default": global_model}},
            )

        assert model == channel_model
        assert runtime["api_mode"] == resolved_mode
        assert resolved == {"provider": "cliproxyapi", "model": channel_model}
