...
class TestCommandBypassActiveSession:
    ...
    async def test_clear_bypasses_guard(self):
        """Gateway /clear aliases /new and must dispatch directly (#40123)."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()
    ...
    async def test_clear_takes_same_cancel_handoff_as_new(self):
        """Gateway /clear must route through the cancel+serialize handoff
        (_dispatch_active_session_command) exactly like /new and /reset —
        the weaker direct-dispatch branch would skip the in-flight task
        cancellation. /status is the non-reset control (#40123)."""
        ...
        # A non-reset bypass command must NOT take the cancel handoff.
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()
        adapter._dispatch_active_session_command = AsyncMock()
