...
class TestCommandBypassActiveSession:
    ...
    @pytest.mark.asyncio
    async def test_task_bypasses_guard(self):
        """/task must bypass so task-tree browsing never interrupts runs."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/task"))

        assert sk not in adapter._pending_messages
        assert any("handled:task" in r for r in adapter.sent_responses)
