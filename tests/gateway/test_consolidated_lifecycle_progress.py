...
async def test_consolidated_status_retains_native_warning_visibility(monkeypatch, diagnostic, suppress_warnings):
    from gateway.warning_notifications import DiagnosticText

    _, tr, ctx, adapter = rail(mode="all")
    ctx.user_config = {"display": {"suppress_warning_notifications": suppress_warnings}}
    monkeypatch.setattr(gateway_run, "_load_gateway_config", lambda: {"compression": {"progress_notices": True}})
    message = DiagnosticText(COMPACTION_DONE_STATUS) if diagnostic else COMPACTION_DONE_STATUS
    tr._status_callback_sync("compacted", message)
    task = asyncio.create_task(tr.send_progress_messages())
    try:
        await asyncio.wait_for(tr._progress_started.wait(), 1)
    finally:
        await stop(task)
    assert bool(adapter.visible) is not (diagnostic and suppress_warnings)
    if adapter.visible:
        assert list(adapter.visible.values()) == [COMPACTION_DONE_STATUS]


@pytest.mark.asyncio
@pytest.mark.parametrize("quiet_reason", ["scheduled_heartbeat", "muted_diagnostic"])
async def test_quiet_native_turn_does_not_handoff_terminal_progress(monkeypatch, tmp_path, quiet_reason):
    captured = {}
    ...
    def make(adapter):
        ...
        async def run(**kwargs):
            ...
            if quiet_reason == "scheduled_heartbeat":
                kwargs["scheduled_heartbeat"] = True
            else:
                kwargs["persist_user_display_metadata"] = {"notification_category": "diagnostic"}
    ...
    adapter, result = await progress._run_with_agent(
        monkeypatch, tmp_path, Agent, session_id="quiet-lifecycle", adapter_cls=Capture,
        config_data={"compression": {"progress_notices": True}, "display": {
            "tool_progress": "full", "suppress_warning_notifications": True,
        }},
    )
    assert result["final_response"] == "context exhausted"
    assert "_complete_lifecycle_progress" not in result
    assert adapter.sent == []
    assert adapter.edits == []
    if quiet_reason == "scheduled_heartbeat":
        assert captured["ctx"].progress_queue is None
    else:
        assert captured["ctx"].mute_notification_reply is True
