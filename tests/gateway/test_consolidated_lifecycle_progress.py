...
@pytest.mark.asyncio
@pytest.mark.parametrize("outcome", ["cancel", "merge_error", "handoff"])
async def test_queued_exhaustion_retains_descendant_through_ancestor_settlement(monkeypatch, tmp_path, outcome):
    ...
    turn = asyncio.create_task(progress._run_with_agent(
        monkeypatch, tmp_path, Agent, session_id="queued-lifecycle", pending_text="queued exhaustion", adapter_cls=Capture,
