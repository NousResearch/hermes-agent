...
def test_ignore_other_user_mentions_admission_contract(adapter, monkeypatch):
    ...
    assert adapter._discord_message_admission(message, claim=False)[0] is True

    adapter.config.extra["ignore_other_user_mentions"] = True
    assert adapter._discord_message_admission(message, claim=False)[0] is False

    hermes = adapter._client.user
    both_mentioned = make_message(
        channel=FakeTextChannel(channel_id=789),
        content=f"<@{hermes.id}> can you help <@111>?",
        mentions=[hermes, other_human],
    )
    assert adapter._discord_message_admission(both_mentioned, claim=False)[0] is True
    ...
    assert adapter._discord_message_admission(dm_message, claim=False)[0] is True
