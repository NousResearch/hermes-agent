...
Retargeted from ``gateway.platforms.discord`` to
``plugins.platforms.discord.adapter`` after the Discord platform
migration (the adapter now lives under ``plugins/platforms/discord/``).
"""
...
def _mock_ffmpeg(pcm_bytes: bytes):
    ...
    return patch(
        "plugins.platforms.discord.adapter.subprocess.run",
...
def test_very_long_duration_uses_flat_fallback(tmp_path):
    ...
    with patch("plugins.platforms.discord.adapter.subprocess.run") as mock_run:
...
def test_ffmpeg_argv_uses_file_prefix_against_leading_dash(tmp_path):
    ...
    with patch("plugins.platforms.discord.adapter.subprocess.run", side_effect=_capture):
...
class TestProbeAudioDuration:
    ...
    def test_uses_file_prefix_argv(self, tmp_path):
        ...
        with patch("plugins.platforms.discord.adapter.subprocess.run", side_effect=_cap):
...
async def test_send_voice_prefers_ffprobe_over_mutagen_for_duration(tmp_path, monkeypatch):
    ...
    with patch("plugins.platforms.discord.adapter.subprocess.run", side_effect=_fake_subprocess_run):
...
async def test_send_voice_offloads_waveform_computation_to_thread(tmp_path):
    """Regression guard: the ffmpeg + RMS work must run in a worker
    ...
    with patch(
        "plugins.platforms.discord.adapter._compute_voice_message_waveform",
