...
class TestEmailFilters(unittest.TestCase):
    """Test automated email filtering logic."""
    ...
    def test_noreply_sender_filtered(self):
        """Emails from noreply@ addresses should be filtered."""
        adapter = self._make_adapter()
        raw_email = MIMEText("Hello", "plain", "utf-8")
        raw_email["From"] = "noreply@example.com"
        raw_email["Subject"] = "Test"
        raw_email["Message-ID"] = "<msg1@test.com>"

        mock_imap = MagicMock()
        mock_imap.uid.side_effect = lambda cmd, *args: (
            ("OK", [b"1"])
            if cmd == "search"
            else ("OK", [(b"1", raw_email.as_bytes())])
            if cmd == "fetch"
            else ("NO", [])
        )
    ...
    def test_auto_submitted_header_filtered(self):
        """Emails with Auto-Submitted header should be filtered."""
        adapter = self._make_adapter()
        raw_email = MIMEText("Hello", "plain", "utf-8")
        raw_email["From"] = "user@example.com"
        raw_email["Auto-Submitted"] = "auto-generated"

        mock_imap = MagicMock()
        mock_imap.uid.side_effect = lambda cmd, *args: (
            ("OK", [b"2"])
            if cmd == "search"
            else ("OK", [(b"2", raw_email.as_bytes())])
            if cmd == "fetch"
            else ("NO", [])
        )
    ...
    def test_x_auto_response_suppress_header_filtered(self):
        """Emails with X-Auto-Response-Suppress header should be filtered."""
        adapter = self._make_adapter()
        raw_email = MIMEText("Hello", "plain", "utf-8")
        raw_email["From"] = "user@example.com"
        raw_email["X-Auto-Response-Suppress"] = "All"

        mock_imap = MagicMock()
        mock_imap.uid.side_effect = lambda cmd, *args: (
            ("OK", [b"3"])
            if cmd == "search"
            else ("OK", [(b"3", raw_email.as_bytes())])
            if cmd == "fetch"
            else ("NO", [])
        )
    ...
    def test_precedence_bulk_header_filtered(self):
        """Emails with Precedence: bulk header should be filtered."""
        adapter = self._make_adapter()
        raw_email = MIMEText("Hello", "plain", "utf-8")
        raw_email["From"] = "user@example.com"
        raw_email["Precedence"] = "bulk"

        mock_imap = MagicMock()
        mock_imap.uid.side_effect = lambda cmd, *args: (
            ("OK", [b"4"])
            if cmd == "search"
            else ("OK", [(b"4", raw_email.as_bytes())])
            if cmd == "fetch"
            else ("NO", [])
        )
