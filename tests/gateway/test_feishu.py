...
class TestFeishuAudioDurationProbe(unittest.TestCase):
    """Regression coverage for #16524 / #8300.

    Audio messages must include a positive ``duration`` (ms) so Feishu
    renders them as voice bubbles.  The probe must be best-effort and
    never raise — falling back to a size-based heuristic and finally a
    fixed minimum so the field is always populated.
    """

    @patch.dict(os.environ, {}, clear=True)
    def test_returns_positive_int_for_real_file(self):
    ...
    def test_size_heuristic_scales_with_file_bytes(self):
        ...
        # Force the size-heuristic branch by making mutagen + ffprobe
        # both unavailable.  ``builtins.__import__`` blocks mutagen;
        # PATH-stripping blocks ffprobe.
        original_import = (
