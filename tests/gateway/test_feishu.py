...
def _reaction_adapter(*, delete_success=True):
    adapter = _plain_feishu_adapter()
    tracker = SimpleNamespace(created=[], deleted=[], created_on=[], deleted_on=[])
    ...
    # A message we already sent in this chat: the badge rides on it, never on the inbound message.
    adapter._outbound_message_by_chat["oc_chat"] = "om_ours"
    return adapter, tracker


def _run_processing(adapter, outcome):
    event = SimpleNamespace(message_id="om_msg", source=SimpleNamespace(chat_id="oc_chat"))
    asyncio.run(adapter.on_processing_start(event))
    asyncio.run(adapter.on_processing_complete(event, outcome))


def test_processing_badge_rides_on_our_own_message_not_the_inbound_one(fake_lark_requests, monkeypatch):
    """Feishu pushes a notification to the AUTHOR of a reacted message. The inbound message is the
    user's, so badging it pings them on every turn; badging one of ours only reaches the app."""
    ...
    assert tracker.created_on == ["om_ours"]
    assert tracker.deleted_on == ["om_ours"]


def test_processing_badge_is_skipped_when_we_have_sent_nothing_yet(fake_lark_requests, monkeypatch):
    """Nothing of ours to ride on (first turn after a restart) → no badge at all. Falling back to the
    inbound message would be exactly the notification this placement change removes."""
    monkeypatch.delenv("FEISHU_REACTIONS", raising=False)
    adapter, tracker = _reaction_adapter()
    adapter._outbound_message_by_chat.clear()
    _run_processing(adapter, ProcessingOutcome.SUCCESS)
...
def test_tracking_failure_cannot_break_a_send(fake_lark_requests, monkeypatch):
    """The tracker runs inside the send path, after the message is on the wire: a bookkeeping bug
    there must not surface as a failed send, which callers retry (duplicate delivery)."""
    monkeypatch.delenv("FEISHU_REACTIONS", raising=False)
    adapter, _tracker = _reaction_adapter()
    sent = SimpleNamespace(success=lambda: True, data=SimpleNamespace(message_id="om_sent"))

    def _boom(_response, _field_name):
        raise RuntimeError("bookkeeping bug")

    monkeypatch.setattr(
        "plugins.platforms.feishu.adapter.FeishuAdapter._extract_response_field", staticmethod(_boom)
    )
    adapter._client.im.v1.message = SimpleNamespace(create=lambda request: sent, reply=lambda request: sent)
    response = asyncio.run(
        adapter._send_raw_message(
            chat_id="oc_chat", msg_type="text", payload='{"text": "hi"}', reply_to=None, metadata=None
        )
    )
...
def test_processing_failure_skips_cross_mark_when_typing_removal_fails(fake_lark_requests, monkeypatch):
    ...
    assert tracker.created == ["Typing"]
    assert tracker.deleted == ["r_typing"]
    assert "om_msg" in adapter._pending_processing_reactions  # handle kept for LRU eviction
