...
async def native_turn():
    ...
    async with Bot("123456:synthetic-full-progress", request=request, get_updates_request=updates) as bot:
        ...
        turn = TurnRunner(SimpleNamespace(_delivery_adapter_for=lambda _s: adapter), ctx)
...
async def test_full_native_telegram_multipart_receipts_close_edit_ownership(native_turn, operation):
    ...
    if wire.flood:
        # Removing the HTTP refusal does not expire the native per-chat penalty.
        assert adapter._send_flood_cooldown_remaining(turn._ctx.source.chat_id) > 0
        adapter._telegram_send_cooldown_until[turn._ctx.source.chat_id] = 0.0
