...
def _enrolled_receiver(runner, source, profile):
    """Provide real registered-policy provenance for downstream control tests."""
    adapter = SimpleNamespace(
        typed_command_prefix="/",
        _owner_profile=profile,
        config=PlatformConfig(extra={"allow_admin_from": ["user-1"], "allow_from": ["user-1"]}),
    )
    if profile:
        maps = getattr(runner, "_profile_adapters", None) or {}
        maps.setdefault(profile, {})[source.platform] = adapter
        runner._profile_adapters = maps
    else:
...
async def test_dm_label_without_one_to_one_proof_cannot_control_group_chats(
    ...
    assert "Home chat you already selected" in result
...
async def test_unknown_chat_type_fails_closed_even_with_private_transport_proof(
    ...
    assert result.startswith("Your account can’t control Group Chats")
...
async def test_rate_limits_are_isolated_by_receiving_adapter_profile(
    ...
    _enrolled_receiver(runner, secondary.source, "ops")
    primary = _event("/group 1 send from primary", message_id="rate-primary")
    _enrolled_receiver(runner, primary.source, None)
...
async def test_secondary_profile_cannot_view_or_apply_group_chat_approvals(
    ...
    runner = _runner()
    _enrolled_receiver(runner, event.source, "ops")

    result = await runner._handle_rooms_command(event)

    assert "owner’s main, authorized Hermes chat" in result
...
async def test_stop_requires_admin_when_operator_enabled_slash_gating(
    ...
    result = await _runner(extra=extra, talk_users=["admin", "member"])._handle_room_command(
        _event("/group 1 stop", user_id="member")
    )
    from gateway.group_home_consent import text
    assert result == text("control_unavailable")
...
async def test_room_history_and_mutation_require_an_explicit_admin(
    ...
    denial = "Your account can’t control Group Chats"
...
async def test_explicit_home_group_allows_only_the_selecting_operator(
    ...
    monkeypatch.setenv("SIGNAL_ALLOWED_USERS", "user-1")
    runner = _runner(extra={"allow_from": ["user-1"], "group_allow_admin_from": ["user-1"]})
    runner._is_user_authorized_for_source = lambda _source: True
    runner.config.platforms[Platform.SIGNAL].home_channel = HomeChannel(
        platform=Platform.SIGNAL,
        chat_id="chat-signal",
    ...
    home = runner.config.platforms[Platform.SIGNAL].home_channel
