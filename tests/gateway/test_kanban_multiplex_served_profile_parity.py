...
def test_worker_for_served_profile_gets_its_own_env_and_toolset_pin(served, monkeypatch):
    ...
    monkeypatch.setattr(_owner_kanban_worker_spawn, "_open_worker_log", lambda task, board: open("/dev/null", "w"))
    monkeypatch.setattr(kbd, "_hermes_argv", lambda: ["hermes"], raising=False)
    _owner_kanban_worker_spawn._default_spawn(task, str(served.alpha), board=None)
