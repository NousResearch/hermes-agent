...
class FailingAdapter:
    ...
    async def _send_with_retry(self, chat_id=None, content=None, metadata=None, **kwargs):
        # Mirror the base class: an exception inside send() propagates —
        # the notifier's except path is what this adapter exercises.
...
class ReportedFailureAdapter:
    ...
    async def _send_with_retry(self, chat_id=None, content=None, metadata=None, **kwargs):
        # Single attempt, reported failure — the notifier must count it as
        # a failed delivery (no retries: max_retries=0 stand-in).
        return await self.send(chat_id, content, metadata=metadata)
