...
class TestReconcileOrphanedRunning:
    def test_null_claim_lock_orphan_requeued(self, conn):
        ...
        reconciled = _owner_kanban_worker_recovery.reconcile_orphaned_running(conn)
    ...
    def test_reconciled_event_and_note_logged(self, conn):
        ...
        _owner_kanban_worker_recovery.reconcile_orphaned_running(conn)
    ...
    def test_healthy_running_task_untouched(self, conn):
        ...
        assert _owner_kanban_worker_recovery.reconcile_orphaned_running(conn) == []
        assert conn.execute(
            "SELECT status FROM tasks WHERE id=?", (tid,)
    ...
    def test_dead_worker_pid_orphan_requeued(self, conn):
        ...
        assert _owner_kanban_worker_recovery.reconcile_orphaned_running(conn) == [tid]
