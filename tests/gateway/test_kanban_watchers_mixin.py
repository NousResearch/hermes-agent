...
def test_stall_respawn_cooldown_is_benign_not_bad():
    # A respawn guard is cooling a task down -> healthy, self-clearing.
    res = _result(respawn_guarded=[("t1", "rate_limit_cooldown")])
    assert _stall_streak_is_bad(True, False, [("b", res)]) is False


def test_stall_auth_denial_is_hard_fault_even_with_benign_decline():
    for benign in ({}, {"rate_limited": ["t2"]}):
        res = _result(respawn_guarded=[("t1", "blocker_auth")], **benign)
        assert _stall_streak_is_bad(True, False, [("b", res)]) is True
    assert _stall_streak_is_bad(True, False, [
        ("auth", _result(respawn_guarded=[("t1", "blocker_auth")])),
        ("busy", _result(skipped_per_profile_capped=[("t2", "p", 1)])),
    ]) is True


def test_stall_non_cooldown_guards_do_not_suppress_warning():
    for reason in ("recent_success", "active_pr", "unknown_future_reason"):
        res = _result(respawn_guarded=[("t1", reason)])
        assert _stall_streak_is_bad(True, False, [("b", res)]) is True
