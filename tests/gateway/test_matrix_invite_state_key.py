...
def _make_invite_event(
    room_id="!bridged:example.org", sender="@owner:example.org", state_key=None
):
...
class TestOnInviteStateKey:
    ...
    @pytest.mark.asyncio
    async def test_invite_to_self_with_different_localpart_case_still_joins(self):
        """The target compare uses _is_self_sender, so homeserver case drift is not "someone else"."""
        adapter = _make_adapter()
        adapter._join_room_by_id = AsyncMock(return_value=True)
