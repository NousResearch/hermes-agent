...
@pytest.mark.parametrize("work_records", [False, True])
async def test_passive_invitation_can_copy_but_cannot_execute(setup, surface, work_records):
    ...
    body = dict(room_id="room", home_install_id=HOME, authority_gateway_id=HOME,
                authority_epoch=1, member_id="reviewer", replication=True, passive_only=True, work_records=work_records)
    async with TestClient(TestServer(app)) as http:
        if surface == "http":
            owner = PeerRunsHTTPClient(base_url=str(http.make_url("/")), api_key=KEY, timeout_seconds=2)
            result = await asyncio.to_thread(owner.issue_invitation, **body, grant_id="passive")
        else:
        ...
        assert probe["work_records_version"] == records.VERSION
        ...
        if work_records:
            ...
            ack = await asyncio.to_thread(sender.replicate_work_records, grant=result["grant"],
                target_profile="default", record=record)
            assert ack["revision"] == record["revision"] and ack["passive"] is True
