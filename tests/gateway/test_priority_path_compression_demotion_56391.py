...
@pytest.mark.asyncio
async def test_priority_interrupt_queues_context_instead_of_redirecting_it():
    runner, agent_mock, sk = _make_runner(compression_in_flight=False)
    agent_mock._supports_active_turn_redirect = True
    agent_mock.redirect.return_value = True
    event = _make_event("use my current position")
    event.ephemeral_user_context = "Location: 1.0, 2.0"

    await runner._handle_message(event)

    agent_mock.redirect.assert_not_called()
    agent_mock.interrupt.assert_called_once_with("use my current position")
    queued = runner.adapters[Platform.TELEGRAM]._pending_messages.get(sk)
    assert queued is event
    assert queued.ephemeral_user_context == "Location: 1.0, 2.0"
