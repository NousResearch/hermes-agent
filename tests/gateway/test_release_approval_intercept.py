...
def test_non_control_event_cannot_trigger_release_approval(monkeypatch):
    monkeypatch.setattr(
        "hermes_cli.config_effective.load_user_config_effective",
        lambda: {"kanban": {"release_approval": {
            "enabled": True,
    ...
    source = _source()
    event = MessageEvent(
        text="freigegeben", source=source, allow_gateway_control=False,
        reply_to_message_id="visible-message-9",
    )
...
def test_exact_reply_bypasses_busy_base_guard_and_runner_consumes_after_auth(monkeypatch):
    ...
    def authorize(_self, source):
        auth_calls.append(source.user_id)
        return True

    runner._is_user_authorized_for_source = MethodType(authorize, runner)
    runner._draining = False
    adapter = _BusyAdapter()
    adapter.gateway_runner = runner  # type: ignore[assignment]
    adapter._message_handler = runner._handle_message
    adapter.set_busy_session_handler(runner._handle_active_session_busy_message)
    runner._delivery_adapter_for = MethodType(lambda _self, _source: adapter, runner)
    source = _source()
    event = MessageEvent(
        text="freigegeben",
