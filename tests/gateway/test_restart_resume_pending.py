...
async def test_restart_notifies_home_channel_even_without_active_sessions():
    ...
    assert adapter.sent == [
        ...
        "Send a message in this chat after restart and I'll try to resume where you left off."
    ]
...
async def test_restart_home_channel_notification_not_deduped_across_threads():
    ...
    assert "Send a message in this topic after restart" in adapter.sent_calls[0][1]
    assert "Send a message in this chat after restart" in adapter.sent_calls[1][1]
