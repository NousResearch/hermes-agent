    ...
    *,
    ...
    if config_data and "streaming" in config_data:
        runner.config = SimpleNamespace(
            streaming=StreamingConfig.from_dict(config_data["streaming"])
        )
    monkeypatch.setattr(gateway_run, "_hermes_home", tmp_path)
    monkeypatch.setattr(gateway_run, "_resolve_runtime_agent_kwargs", lambda: {"api_key": "***"})
    source = SessionSource(
...
async def test_run_agent_suppresses_interim_commentary_by_default(monkeypatch, tmp_path):
    adapter, result = await _run_with_agent(
        ...
        tmp_path,
        CommentaryAgent,
        session_id="sess-commentary-default-off",
    )
...
@pytest.mark.asyncio
async def test_run_agent_tool_progress_does_not_enable_interim_commentary(monkeypatch, tmp_path):
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        CommentaryAgent,
