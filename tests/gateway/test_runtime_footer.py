...
def test_format_session_token_usage_matches_session_stats_shape():
    from gateway.runtime_footer import format_session_token_usage
...
# ---------------------------------------------------------------------------
# session_tokens cost — appended when the session row carries a cost
# ---------------------------------------------------------------------------

def test_format_session_token_usage_appends_cost_when_present():
    from gateway.runtime_footer import format_session_token_usage
    out = format_session_token_usage({
        "input_tokens": 509_200,
        "output_tokens": 5_800,
        "cache_read_tokens": 349_400,
        "api_call_count": 13,
        "total_tokens": 515_000,
        "estimated_cost_usd": 0.0184,
    })
    assert out == "📊 Sesión · 13 req · In: 858.6K (cache: 349.4K) · Out: 5.8K · Total: 515.0K $0.0184"


def test_format_session_token_usage_prefers_actual_over_estimated_cost():
    from gateway.runtime_footer import format_session_token_usage
    out = format_session_token_usage({
        "input_tokens": 1_000,
        "output_tokens": 25,
    ...
    assert out.endswith("$0.0184")


def test_format_session_token_usage_omits_cost_when_zero_or_unknown():
    from gateway.runtime_footer import format_session_token_usage
    out = format_session_token_usage({
        "input_tokens": 1_000,
        "output_tokens": 25,
    ...
    assert "$" not in out
    assert out.endswith("Total: 3.0K")
