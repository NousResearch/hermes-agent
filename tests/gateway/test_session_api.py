...
@pytest.mark.parametrize(
    ("query", "expected"),
    [
        pytest.param("", ["tip history"], id="omitted"),
        pytest.param("?include_compacted=false", ["tip history"], id="false"),
        pytest.param(
            "?include_compacted=true",
            ["parent history", "tip history"],
            id="true",
        ),
    ],
)
async def test_session_messages_compacted_flag_preserves_tip_only_default(
    adapter, session_db, query, expected
):
    session_db.create_session("root", "api_server")
    session_db.append_message("root", "user", "parent history")
    session_db.end_session("root", "compression")
    session_db.create_session("tip", "api_server", parent_session_id="root")
    session_db.append_message("tip", "assistant", "tip history")

    app = _create_session_app(adapter)
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.get(f"/api/sessions/root/messages{query}")
        assert resp.status == 200
        payload = await resp.json()

    assert payload["resolved_tip_id"] == "tip"
    assert [message["content"] for message in payload["data"]] == expected
