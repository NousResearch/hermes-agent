...
async def test_startup_warms_profile_metadata_without_changing_context(tmp_path, monkeypatch, pinned):
    ...
    import model_tools
    monkeypatch.setattr(model_tools, "get_tool_definitions", lambda quiet_mode=False: [])
    monkeypatch.setattr("hermes_cli.config.load_config_readonly",
                        lambda: {"agent": {"environment_probe": False}})
    ...
    for index in (0, 1, 0):
        ...
        (profile / ".env").write_text(f"STARTUP_MODEL_API_KEY={credential}\n", encoding="utf-8")
        ...
        try:
            ...
            before = len(calls)
            expected_probes = 0 if pinned or index in warmed else 1
            await runner._warm_turn_prerequisites()
            assert len(calls) == before + expected_probes
