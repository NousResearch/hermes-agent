...
@pytest.mark.asyncio
async def test_status_command_does_not_mix_partial_config_with_legacy_provider(
    monkeypatch,
):
    """A higher-tier model must not borrow a provider from legacy billing."""
    session_entry = SessionEntry(
        session_key=build_session_key(_make_source()),
        session_id="sess-1",
        created_at=datetime.now(),
    ...
    runner._session_db._db.get_dominant_session_model_route.return_value = {
        ...
        "billing_provider": "dominant-provider",
    }
    monkeypatch.setattr(
        "gateway.run._load_gateway_config",
        lambda: {"model": {"default": "configured-model"}},
    )
    monkeypatch.setattr(
    ...
    assert "**Model:** `configured-model`" in result
    assert "configured-model` (legacy-provider)" not in result
    assert "configured-model` (dominant-provider)" not in result
