...
async def test_status_command_reports_running_agent_without_interrupt(monkeypatch):
    ...
    running_agent.base_url = None
    running_agent.session_total_tokens = 321
    running_agent.session_input_tokens = 210
    running_agent.session_output_tokens = 111
    running_agent.session_cache_read_tokens = 12
    running_agent.session_cache_write_tokens = 7
    running_agent.session_api_calls = 4
    ...
    assert "**Input tokens:** 210" in result
    assert "**Output tokens:** 111" in result
    assert "**Cache read:** 12" in result
    assert "**Cache write:** 7" in result
    assert "**Context:** 30,000 / 200,000 (15%)" in result
    assert "**Compressions:** 2" in result
