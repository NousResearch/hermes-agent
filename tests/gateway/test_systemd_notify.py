...
def test_notify_sends_real_unix_datagram(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert notify("READY=1\nSTATUS=Gateway running") is True
        assert receiver.recv(4096) == b"READY=1\nSTATUS=Gateway running"
    finally:
        receiver.close()


@pytest.mark.skipif(not hasattr(socket, "AF_UNIX"), reason="Unix datagram sockets are unavailable")
...
def test_notify_disables_cleanly_without_unix_socket_support(monkeypatch):
    monkeypatch.setenv("NOTIFY_SOCKET", "/tmp/hermes-test-notify")

    import gateway.systemd_notify as notify_mod

    monkeypatch.delattr(notify_mod.socket, "AF_UNIX")
...
def test_watchdog_interval_disables_without_unix_socket_support(monkeypatch):
    monkeypatch.setenv("NOTIFY_SOCKET", "/tmp/hermes-test-notify")
...
def test_notify_rejects_invalid_messages(monkeypatch, message):
    monkeypatch.setenv("NOTIFY_SOCKET", "/tmp/hermes-test-notify")

    from gateway.systemd_notify import notify

    assert notify(message) is False


@pytest.mark.asyncio
async def test_watchdog_sends_ready_and_heartbeat_when_enabled(monkeypatch):
    calls: list[str] = []
    monkeypatch.setenv("NOTIFY_SOCKET", "/tmp/hermes-test-notify")
    ...
    watchdog = notify_mod.SystemdWatchdog(lag_tolerance_seconds=0.05)
    ...
    assert any(message == "WATCHDOG=1" for message in calls)
...
@pytest.mark.asyncio
async def test_watchdog_stop_is_idempotent_when_disabled(monkeypatch):
    monkeypatch.delenv("NOTIFY_SOCKET", raising=False)
    monkeypatch.delenv("WATCHDOG_USEC", raising=False)
...
@pytest.mark.asyncio
async def test_watchdog_stop_notifies_systemd_only_once(monkeypatch):
    calls: list[str] = []
    monkeypatch.setenv("NOTIFY_SOCKET", "/tmp/hermes-test-notify")
    monkeypatch.setenv("WATCHDOG_USEC", "1000000")
