...
def test_local_endpoint_requires_explicit_opt_in():
    with pytest.raises(PlaygroundValidationError):
        validate_playground_url("http://127.0.0.1:3979", allow_private=False)
    assert validate_playground_url("http://127.0.0.1:3979", allow_private=True) == "http://127.0.0.1:3979"
    assert validate_playground_url("http://playground.local:3979", allow_private=True) == "http://playground.local:3979"
...
def test_url_generation_only_happens_for_valid_configured_endpoint():
    config = PlaygroundConfig.from_extra({"playground_url": "http://localhost:3979", "playground_allow_private": True})
    assert generated_test_url(config) == "http://localhost:3979/hermes/teams-playground"
...
async def test_health_handshake_uses_short_timeout_and_sanitizes_errors(monkeypatch):
    ...
    assert seen["url"] == "http://localhost:3979/health"
