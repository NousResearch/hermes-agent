...
@pytest.mark.asyncio
async def test_business_final_does_not_enter_route_erasing_delivery_ledger():
    adapter = _adapter()
    event = MessageEvent(
    ...
    with patch("gateway.delivery_ledger.record_obligation") as record_obligation:
        obligation_id = await adapter._record_delivery_obligation(
            event,
            "agent:main:telegram:dm:telegram-business:bc-123:customer-chat",
            "final answer",
            adapter,
            False,
        )
