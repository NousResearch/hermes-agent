...
from hermes_state import SessionDB, _normalize_telegram_topic_profile_name
...
def test_normalize_profile_name():
    assert _normalize_telegram_topic_profile_name(None) == "default"
    assert _normalize_telegram_topic_profile_name("") == "default"
    assert _normalize_telegram_topic_profile_name(" coder ") == "coder"


def test_fresh_schema_is_v3(tmp_path: Path):
    db = SessionDB(db_path=tmp_path / "fresh.db")
    db.apply_telegram_topic_migration()
    assert db.get_meta("telegram_dm_topic_schema_version") == "3"
    cols = {r[1] for r in db._conn.execute("PRAGMA table_info('telegram_dm_topic_mode')")}
...
def test_default_kwarg_keeps_single_profile_behavior(tmp_path: Path):
    db = SessionDB(db_path=tmp_path / "state.db")
    _session(db, "sess-d")
    db.enable_telegram_topic_mode(chat_id=CHAT, user_id=CHAT)
    db.bind_telegram_topic(
        chat_id=CHAT, thread_id="1", user_id=CHAT,
        session_key="k", session_id="sess-d",
    )
    assert db.is_telegram_topic_mode_enabled(chat_id=CHAT, user_id=CHAT)
    assert db.get_telegram_topic_binding(chat_id=CHAT, thread_id="1")["session_id"] == "sess-d"
    db.close()
