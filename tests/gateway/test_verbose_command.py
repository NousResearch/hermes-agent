...
class TestVerboseCommand:
    ...
    async def test_defaults_to_all_when_no_tool_progress_set(self, tmp_path, monkeypatch):
        """When tool_progress is not in config, uses platform default then cycles."""
        hermes_home = tmp_path / "hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
        config_path.write_text(
        ...
        # Telegram default is "new" → cycles to all.
    ...
    async def test_per_platform_isolation(self, tmp_path, monkeypatch):
        ...
        default: Telegram = 'new', Slack = 'off' (quiet Slack default).
        """
        hermes_home = tmp_path / "hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
