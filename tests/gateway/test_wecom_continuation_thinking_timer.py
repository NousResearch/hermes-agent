...
class TestContinuationCallThinkingArmsImmediately:
    ...
    async def test_continuation_thinking_after_tool_pushes_first_frame_now(self):
        ...
        try:
            ...
            immediate_ticks = _drain_timer_ticks(sc)
            assert immediate_ticks >= 1, (
                "continuation-call thinking must push its first frame "
                "immediately (like the first call); got no _TIMER_TICK within "
                "the sub-second window — the frame is stuck until the next "
                "call_later tick / the ~19s model return"
            )
            assert sc._tool_timer_tick_count > tick_count_before, (
                "no tick rendered the 💭 Thinking frame after the continuation "
                "signal — arm path hit the need_arm=False no-op"
            )
        finally:
            sc.finish()
            await task
