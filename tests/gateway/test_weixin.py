...
class TestWeixinDirectSend:
    def test_send_weixin_direct_skips_live_adapter_from_foreign_loop(self, monkeypatch):
        class _FakeTokenStore:
            def __init__(self, *_args, **_kwargs):
                pass

            def restore(self, _account_id):
                return None

            def get(self, _account_id, _chat_id):
                return None
        ...
        live_adapter.send = AsyncMock(side_effect=AssertionError("foreign-loop live adapter must not be reused"))

        monkeypatch.setattr(weixin, "ContextTokenStore", _FakeTokenStore)
        monkeypatch.setattr(weixin.aiohttp, "ClientSession", _EphemeralSession)
        monkeypatch.setitem(weixin._LIVE_ADAPTERS, "test-token", live_adapter)

        with patch.object(WeixinAdapter, "send", new_callable=AsyncMock) as temp_send_mock:
            temp_send_mock.return_value = SendResult(success=True, message_id="temp-msg")
        ...
        temp_send_mock.assert_awaited_once()
