...
class TestWeixinGroupPolicyRuntime:
    @patch("gateway.platforms.weixin.has_group_policy", return_value=True)
    @patch(
        "gateway.platforms.weixin.get_group_policy",
        return_value={
    ...
    def test_explicit_group_policy_overrides_disabled_env_gate(self, _policy_mock, _has_policy_mock):
        adapter = _make_adapter()
        adapter._session = object()
        ...
        message = {
            ...
            "to_user_id": "project@chatroom",
            "message_id": "m1",
            "msg_type": 1,
            "item_list": [{"type": weixin.ITEM_TEXT, "text_item": {"text": "在吗"}}],
        }
        ...
        assert event.metadata["group_policy_mode"] == "default"
        assert event.metadata["allow_model_dispatch"] is True
        assert event.metadata["group_trigger_reason"] == "require_mention_disabled"
        assert event.metadata["explicit_addressed"] is False
    ...
    def test_group_message_captures_reply_context(self, _policy_mock, _has_policy_mock):
        adapter = _make_adapter()
        adapter._session = object()
        adapter._group_policy = "disabled"
        adapter.handle_message = AsyncMock()
        ...
        assert event.reply_to_text == "马嘎 | 上一条内容"
    ...
    def test_collect_only_group_archives_without_dispatch(self, _policy_mock, _has_policy_mock):
        adapter = _make_adapter()
        adapter._session = object()
        adapter._group_policy = "disabled"
        adapter.handle_message = AsyncMock()
        ...
        with patch.object(adapter._group_archive_store, "archive_inbound_message", return_value={}) as archive_mock:
            message = {
                "from_user_id": "wxid_user",
                "to_user_id": "project@chatroom",
                "message_id": "m2",
                "msg_type": 1,
                "item_list": [{"type": weixin.ITEM_TEXT, "text_item": {"text": "只采集别回复"}}],
            }
