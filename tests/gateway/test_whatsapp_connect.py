...
def _mock_create_task(coro):
    """Test-safe replacement for asyncio.create_task.

    connect() passes a coroutine object to create_task(). In production this is
    scheduled immediately; in tests a bare MagicMock would drop it and trigger
...
def _connect_patches(mock_proc, mock_fh, mock_client_cls=None):
    """Return common patches needed to reach connect() health-check logic."""
    base = [
