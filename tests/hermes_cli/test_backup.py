...
class TestQuickSnapshot:
    ...
    def test_recovery_hint_names_only_commands_that_can_run(self):
        """Both warning sites end by appending ``_snapshot_recovery_hint()``, and the hint must
        name a surface that exists. It used to read "hermes snapshot list" — not a CLI
        subcommand, so the advice died with ``invalid choice: 'snapshot'`` exactly when the user
        had lost data."""
        from hermes_cli.backup import _snapshot_recovery_hint

        hint = _snapshot_recovery_hint()
        assert _unrunnable_commands_in(hint) == set()
        assert "hermes snapshot" not in hint
    ...
    def test_failed_db_copy_hint_points_at_a_runnable_command(self, hermes_home, monkeypatch, capsys):
        ...
        helper, so a stale inline string fails here too."""
        import hermes_cli.backup as backup_mod
        ...
        warning = next(ln for ln in out.splitlines() if "If sessions disappear" in ln)
        # Drop the snapshot directory path that precedes the recovery guidance.
        guidance = warning.split(str(backup_mod._quick_snapshot_root(hermes_home)), 1)[1]
        assert _unrunnable_commands_in(guidance.lstrip(". ")) == set()
