...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            # This fork's config_defaults.py sets apply_approval=True by
            # default (G1 self-update approval gate, deliberately kept --
            # see Standing Exclusion 2 in Update & Merge Policy.md).
            # Every other cmd_update() call in this file already bypasses
            # the resulting staging step with approved=True; this new
            # upstream test needs the same to reach the migration-prompt
            # logic it actually exercises, matching that convention.
            cmd_update(mock_args, approved=True)
