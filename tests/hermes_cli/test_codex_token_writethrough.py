...
def profile_and_root(tmp_path, monkeypatch):
    ...
    # The source-aware fallback inside _load_provider_state_with_source resolves the
    # root through get_default_hermes_root(), not through the patched helper.
    monkeypatch.setattr(hermes_constants, "get_default_hermes_root", lambda: tmp_path / "root")
