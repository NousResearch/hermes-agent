...
def _usable_bash_for_syntax_check():
    ...
    Called at test time, not module import: a subprocess probe inside a
    decorator argument would run during collection on every platform.
    """
    ...
    if git_exe:
        candidates.append(
            str(os.path.dirname(os.path.dirname(git_exe)) / "bin" / "bash.exe")
        )
    seen = set()
