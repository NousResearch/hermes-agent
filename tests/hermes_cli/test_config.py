...
class TestOptionalEnvVarsRegistry:
    ...
    def test_nimble_api_key_registered(self):
        """NIMBLE_API_KEY is listed in OPTIONAL_ENV_VARS."""
        from hermes_cli.config import OPTIONAL_ENV_VARS
        assert "NIMBLE_API_KEY" in OPTIONAL_ENV_VARS

    def test_nimble_api_key_is_tool_category(self):
        """NIMBLE_API_KEY is in the 'tool' category."""
        from hermes_cli.config import OPTIONAL_ENV_VARS
        assert OPTIONAL_ENV_VARS["NIMBLE_API_KEY"]["category"] == "tool"

    def test_nimble_api_key_is_password(self):
        """NIMBLE_API_KEY is marked as password."""
        from hermes_cli.config import OPTIONAL_ENV_VARS
        assert OPTIONAL_ENV_VARS["NIMBLE_API_KEY"]["password"] is True

    def test_nimble_api_key_has_url(self):
        """NIMBLE_API_KEY signup URL points at online.nimbleway.com."""
        from hermes_cli.config import OPTIONAL_ENV_VARS
        assert OPTIONAL_ENV_VARS["NIMBLE_API_KEY"]["url"] == "https://online.nimbleway.com"

    def test_nimble_in_env_vars_by_version(self):
        """NIMBLE_API_KEY is listed in ENV_VARS_BY_VERSION."""
        from hermes_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
        for vars_list in ENV_VARS_BY_VERSION.values():
            all_vars.extend(vars_list)
        assert "NIMBLE_API_KEY" in all_vars
