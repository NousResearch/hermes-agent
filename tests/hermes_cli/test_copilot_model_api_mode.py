...
def test_copilot_failed_catalog_fetch_is_not_repeated():
    """A catalog miss must not double the timeout before routing a request."""
    from unittest.mock import patch

    from hermes_cli.models import copilot_model_api_mode

    with patch("hermes_cli.models.fetch_github_model_catalog", return_value=None) as fetch:
        assert copilot_model_api_mode("grok-4.6", api_key="token") == "chat_completions"

    fetch.assert_called_once_with(api_key="token")
