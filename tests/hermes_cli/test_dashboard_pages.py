...
class TestMountOrder:
    def test_pages_mount_before_the_spa_catch_all(self):
        """Guard the ordering in web_server.py itself: a future edit that moves the page mount
        below ``mount_spa(app)`` makes every plugin page unreachable, and a runtime test on a
        freshly built app would not notice because the module is imported once."""
        from pathlib import Path

        import hermes_cli.web_server as ws

        source = Path(ws.__file__).read_text(encoding="utf-8")
        page_mount = source.index("_mount_plugin_pages(app)")
        spa_mount = source.index("mount_spa(app)")
        assert page_mount < spa_mount
