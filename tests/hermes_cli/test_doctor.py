...
@pytest.fixture(autouse=True)
def _reset_doctor_report_json_state():
    """run_doctor(json=True) leaves doctor_report._json_mode set; reset it so later
    tests calling check functions directly still print."""
    from hermes_cli import doctor_report

    yield
    doctor_report._json_mode = False
    doctor_report._json_results = []
    doctor_report._json_current_section = ""
...
class TestDoctorOutputModes:
    ...
    def _run_doctor(self, monkeypatch, tmp_path, *, json_mode=False,
        ...
        monkeypatch.setattr(doctor_connectivity, "_APIKEY_PROVIDERS_CACHE", [])
        monkeypatch.setitem(
            sys.modules,
            "model_tools",
