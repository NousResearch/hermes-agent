...
class TestAddCommand:
    ...
    def test_denied_selected_fallback_is_not_persisted(self, isolated_home):
        """The final fallback-chain write is guarded after picker-route restoration."""
        from hermes_cli.routing_policy import RoutingPolicyError

        primary = {"provider": "anthropic", "default": "claude-sonnet-4-6"}
        _write_config(isolated_home, {
            "model": primary,
            "routing_policy": {"enabled": True, "deny": {"models": ["denied-*"]}},
        })

        def fake_picker(args=None):
            from hermes_cli.config import load_config, save_config
            cfg = load_config()
            cfg["model"] = {"provider": "openrouter", "default": "denied-model"}
            save_config(cfg)
