...
class TestMatrixHiddenOnWindows:
    ...
    def test_matrix_hidden_on_macos(self, monkeypatch):  # (#64065)
        """#64065: matrix must also be hidden on macOS — python-olm has
        no macOS wheel and fails to build on modern toolchains (cmake 4.x,
        Clang 21+)."""
        import hermes_cli.gateway as gateway_mod
        ...
        assert "matrix" not in keys, (
            "matrix must be hidden on macOS (#64065) — python-olm has "
            "no macOS wheel and no working build path"
        )
