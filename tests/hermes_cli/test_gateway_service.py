...
class TestProfileArg:
    ...
    def test_launchd_plist_includes_configured_service_wrapper(self, tmp_path, monkeypatch):
        ...
        # The configured preflight remains the outer executable while upstream's timestamping
        # and external-supervisor contract stays intact inside it.
        assert program_args[0] == str(wrapper)
        assert program_args[1:4] == [
            gateway_cli.get_python_path(),
            "-m",
            "hermes_cli.stderr_timestamp",
        ]
        assert "gateway" in program_args
        assert "run" in program_args
        assert "--external-supervisor" in program_args
