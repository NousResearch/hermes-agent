...
def test_opted_goal_allows_done_only_when_registered_gate_allows(monkeypatch):
    ...
    _register_gate("verify-gate", gate)
    manager = goals.GoalManager("governed")
    manager.set("governed goal", completion_gate="verify-gate")
...
def test_gate_continue_prevents_done_and_requests_more_work(monkeypatch):
    ...
    manager.set("needs verification", completion_gate="verify-gate")
...
def test_gate_blocked_transitions_goal_to_blocked_pause(monkeypatch):
    ...
    manager.set("unsafe goal", completion_gate="verify-gate")
...
def test_missing_opted_gate_fails_closed(monkeypatch):
    ...
    goal.set("governed goal", completion_gate="verify-gate")
...
def test_invalid_or_crashed_gate_fails_closed(callback, fragment):
    _register_gate("verify-gate", callback)

    result = plugins.evaluate_goal_completion_gate(
        "verify-gate", session_id="s", generation=1.0, candidate_state="done",
    )
...
def test_timed_out_gate_fails_closed(monkeypatch):
    ...
    _register_gate("verify-gate", slow_gate)
    result = plugins.evaluate_goal_completion_gate(
        "verify-gate", session_id="s", generation=1.0, candidate_state="done",
    )
...
def test_duplicate_gate_registration_is_rejected():
    ...
    first.register_goal_completion_gate("verify-gate", lambda **_: {"action": "allow"})

    with pytest.raises(ValueError, match="already registered"):
        second.register_goal_completion_gate("verify-gate", lambda **_: {"action": "allow"})


def test_unload_removes_completion_gate_registration():
    manager = _register_gate("verify-gate", lambda **_: {"action": "allow"})
    assert manager.has_goal_completion_gate("verify-gate") is True

    manager.unload()

    assert manager.has_goal_completion_gate("verify-gate") is False
