...
# See #48956 (originally #23530, reverted in #23813, re-added here).
# ──────────────────────────────────────────────────────────────────────
...
class TestGoalSurvivesCompressionRotation:
    def test_load_goal_after_session_id_rotates(self, hermes_home):
        ...
        readable from the new session_id (forwarded by the
        _compress_context block in conversation_compression.py).
        ...
        # Simulate the _compress_context forwarding block:
        ...
        # Counters/subgoals preserved verbatim.
        assert mgr2.state.turns_used == mgr.state.turns_used
        assert mgr2.state.subgoals == mgr.state.subgoals
