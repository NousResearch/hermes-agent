...
class TestWindowsAutostartRepair:
    ...
    def test_windows_installer_uses_no_autostart_and_skips_repair(self):
        ...
        NOT re-register the logon task post-install (#95372, #97389). The
        PowerShell install argv and the (now absent) autostart repair hook
        are both inside the ``is_windows`` branch, so on Linux the fake
        selected a branch whose `powershell` doesn't exist on PATH."""
        from unittest.mock import MagicMock
        from hermes_cli import tools_config
