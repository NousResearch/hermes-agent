...
def test_request_review_shares_the_live_worker_fence(conn):
    ...
    assert kb.request_review(conn, tid, summary="handoff", reviewer="reviewer") is True
    assert kb.get_task(conn, tid).status == "review"

    tid2, run2 = _claimed_running_task(conn)
    ok, reason = kb.request_review(conn, tid2, summary="steal", with_reason=True, reviewer="reviewer")
    assert ok is False and "live claim" in reason
    assert kb.request_review(conn, tid2, summary="own", expected_run_id=run2, reviewer="reviewer") is True
