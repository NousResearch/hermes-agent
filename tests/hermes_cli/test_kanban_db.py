...
def test_dispatch_memory_gate_throttles_when_low(kanban_home, monkeypatch):
    ...
    # Mock memory check to return low value
    monkeypatch.setattr(
    ...
    with kb.connect() as conn:
        result = _dispatch_once_locked(conn, dry_run=True)
...
def test_resolve_dispatch_memory_min_mb_reads_config(monkeypatch):
    """_resolve_dispatch_memory_min_mb reads kanban.dispatch_memory_min_mb from config."""
