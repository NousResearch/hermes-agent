...
import hermes_cli.kanban_db_workspace as _owner_kanban_db_workspace
...
def test_schedule_task_parks_time_delay_without_dispatching(kanban_home):
    with kbc.connect() as conn:
        ...
        assert _owner_kanban_transitions.schedule_task(conn, t, reason="run next week") is True
        task = kb.get_task(conn, t)
        assert task.status == "scheduled"
        assert _owner_kanban_claims.claim_task(conn, t) is None
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    with kbc.connect() as conn:
        ...
        _owner_kanban_claims.claim_task(conn, t, claimer=f"{host}:worker")
        ...
        _owner_kanban_claims.release_stale_claims(conn, signal_fn=lambda _p, _s: None)
        row = conn.execute(
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    with kbc.connect() as conn:
        ...
        for i in range(6):
            ...
            _owner_kanban_claims.claim_task(conn, tid, claimer=f"{host}:w{i}")
            conn.execute(
            ...
            crashed = _owner_kanban_worker_recovery.detect_crashed_workers(conn)
            # Rate-limited requeues are NOT crashes.
            assert tid not in crashed
            rl = getattr(_owner_kanban_worker_recovery.detect_crashed_workers, "_last_rate_limited", [])
            assert tid in rl
...
def test_recompute_ready_honours_dispatcher_failure_limit(kanban_home):
    ...
    with kbc.connect() as conn:
        ...
        conn.execute(
            ...
            "WHERE id=?",
            (_owner_kanban_db_dispatch.DEFAULT_FAILURE_LIMIT, t),
        )
        conn.commit()
        ...
        promoted = _owner_kanban_transitions.recompute_ready(
            conn, failure_limit=_owner_kanban_db_dispatch.DEFAULT_FAILURE_LIMIT + 2
        )
        assert promoted == 1
        task = kb.get_task(conn, t)
        assert task.status == "ready"
        assert task.consecutive_failures == _owner_kanban_db_dispatch.DEFAULT_FAILURE_LIMIT
        ...
        assert _owner_kanban_transitions.recompute_ready(conn, failure_limit=1) == 0
...
def test_delete_archived_task_removes_related_rows(kanban_home):
    with kbc.connect() as conn:
        ...
        _owner_kanban_claims.claim_task(conn, tid)
        _owner_kanban_completion.complete_task(conn, tid, result="done")
        ...
        assert _owner_kanban_transitions.delete_archived_task(conn, tid) is True
...
def test_complete_task_persists_scratch_artifacts_before_cleanup(kanban_home):
    ...
    with kbc.connect() as conn:
        ...
        run = _owner_kanban_stats.latest_run(conn, t)
    ...
    assert persisted.parent == _owner_kanban_boards.task_attachments_dir(t)
...
def test_dir_child_completion_unblocks_deferred_scratch_parent(kanban_home, tmp_path):
    ...
    with kbc.connect() as conn:
        ...
        _owner_kanban_completion.complete_task(conn, parent, result="handoff")
        assert parent_ws.exists(), "deferred while dir child active"

        _owner_kanban_completion.complete_task(conn, child, result="built")
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    ...
    assert not _owner_kanban_db_workspace._is_managed_scratch_path(kanban_root)

    logs_dir = kanban_root / "logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    assert not _owner_kanban_db_workspace._is_managed_scratch_path(logs_dir)
    ...
    assert not _owner_kanban_db_workspace._is_managed_scratch_path(board_root)
    ...
    assert not _owner_kanban_db_workspace._is_managed_scratch_path(board_logs)
    ...
    assert not _owner_kanban_db_workspace._is_managed_scratch_path(board_workspaces)
    task_dir = board_workspaces / "task-42"
    task_dir.mkdir(parents=True, exist_ok=True)
    assert _owner_kanban_db_workspace._is_managed_scratch_path(task_dir)
...
class TestSharedBoardPaths:
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        assert _owner_kanban_boards.kanban_home() == default_home
        assert _owner_kanban_db.kanban_db_path() == default_home / "kanban.db"
        assert _owner_kanban_boards.workspaces_root() == default_home / "kanban" / "workspaces"
        assert (
            _owner_kanban_stats.worker_log_path("t_0d214f19")
        ...
        assert _owner_kanban_db.kanban_db_path() != profile_home / "kanban.db"
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        _owner_kanban_worker_spawn._default_spawn(task, str(tmp_path / "ws"))
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    with kbc.connect() as conn:
        ...
        _owner_kanban_completion.complete_task(conn, a)

        # C is running (not done) — blocks child B.
        c = kb.create_task(conn, title="parent-running")
        _owner_kanban_claims.claim_task(conn, c, claimer="worker:1")
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    argv = _owner_kanban_worker_spawn._resolve_hermes_argv()
...
def _make_task(**overrides) -> "kb.Task":
    ...
    return _owner_kanban_db_models.Task(**defaults)
...
def test_dispatch_max_in_progress_blocks_review_when_at_limit(
    ...
    with kbc.connect() as conn:
        ...
        _owner_kanban_claims.claim_task(conn, running)
...
def test_write_txn_preserves_original_exception_when_rollback_fails(kanban_home):
    ...
    with kbc.connect() as conn:
        ...
        with pytest.raises(sqlite3.OperationalError) as excinfo:
            with _owner_kanban_db_connect.write_txn(wrapper):
