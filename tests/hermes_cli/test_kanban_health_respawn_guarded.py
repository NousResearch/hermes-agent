...
* ``hermes_cli/kanban.py`` — ``health_state``, the ``--force`` daemon.
...
def _run_gateway_ticks(kb, monkeypatch, caplog, tick_result_fn, ticks):
    ...
    monkeypatch.setattr(kb, "dispatch_once", tick_result_fn)
...
def _run_cli_ticks(kb, monkeypatch, capsys, results, ticks):
    ...
    monkeypatch.setattr(kanban_cli, "kb", kb, raising=False)
