...
async def test_notifier_notify_plus_wake_sends_and_wakes(kanban_home):
    ...
    try:
        ...
        _owner_kanban_db.block_task(conn, passive_tid, reason="passive block")
        _owner_kanban_db.block_task(conn, active_tid, reason="active block")
    finally:
        conn.close()
...
async def test_notifier_plain_notify_never_wakes_even_with_session_id(kanban_home):
    ...
    try:
        ...
        kbn.add_notify_sub(conn, task_id=tid, platform="telegram", chat_id="chat1")
        _owner_kanban_db.block_task(conn, tid, reason="plain notify block")
    finally:
        conn.close()
...
async def test_notifier_wake_forwards_persisted_chat_type_and_user_id(kanban_home):
    ...
    try:
        ...
        _owner_kanban_db.block_task(conn, tid, reason="group block")
    finally:
        conn.close()
...
async def test_notifier_wake_only_skips_send_and_advances_cursor(kanban_home):
    ...
    try:
        ...
        _owner_kanban_db.block_task(conn, tid, reason="wake only block")
    finally:
        conn.close()
...
async def test_gateway_create_autosubscribes_on_explicit_board(kanban_home):
    ...
    _owner_kanban_boards.create_board("projx")
...
def _make_done_task_with_sub(kb, conn, *, title, chat_id):
    ...
    assert _owner_kanban_completion.complete_task(conn, tid, summary="done")
    return tid
