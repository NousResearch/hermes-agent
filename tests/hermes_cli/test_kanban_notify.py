...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, 'create "explicit wins" --notify discord:channel --json',
         {("discord", "channel", "", None, "gateway-prof")}),
        (True, 'create "same-platform different-chat" --notify telegram:target-chat --json',
         {("telegram", "target-chat", "", None, "gateway-prof")}),
        (True, 'create "same-chat different-thread" --notify telegram:ambient-chat:other-thread --json',
         {("telegram", "ambient-chat", "other-thread", None, "gateway-prof")}),
        (True, 'create "matching explicit target" --notify telegram:ambient-chat:ambient-thread --json',
         {("telegram", "ambient-chat", "ambient-thread", "u1", "gateway-prof")}),
        (True, 'create "no subscriptions" --notify discord:channel --no-subscribe --json', set()),
    ],
    ids=(
        "disabled-suppresses-ambient",
        "disabled-keeps-explicit",
        "explicit-overrides-ambient",
        "same-platform-different-chat-does-not-inherit-user",
        "same-chat-different-thread-does-not-inherit-user",
        "matching-explicit-target-keeps-user",
        "no-subscribe-suppresses-all",
    ),
)
