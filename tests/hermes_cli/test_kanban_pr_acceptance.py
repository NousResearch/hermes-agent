...
def test_pr_completion_requires_current_required_evidence(github):
    with connect() as conn:
        for conclusion in ("failure", "pending", "cancelled", "timed_out", "action_required", "neutral", "skipped", None, "success"):
            ...
            ok = _owner_kanban_completion.complete_task(conn, tid, metadata={"published_pr": "https://github.com/acme/repo/pull/7"})
        ...
        for fault in ("missing", "stale", "head_change"):
            ...
            assert not _owner_kanban_completion.complete_task(conn, tid, metadata={"published_pr": "https://github.com/acme/repo/pull/7"})
            assert kb.get_task(conn, tid).status != "done"
            github.pop(fault)
        # Omission and a sibling repository cannot downgrade the stored declaration.
        tid = kb.create_task(conn, title="publish", completion_contract="acme/repo")
        assert not _owner_kanban_completion.complete_task(conn, tid, summary="local green")
        assert not _owner_kanban_completion.complete_task(conn, tid, metadata={"published_pr": "https://github.com/other/repo/pull/7"})
        before = len(github["requests"])
        local = kb.create_task(conn, title="local", completion_contract="local-only")
        assert _owner_kanban_completion.complete_task(conn, local, summary="https://github.com/acme/repo/pull/7 is background context")
        ...
        for conclusion in ("success", "failure"):
            ...
            def reclaim():
                with connect() as rival:
                    assert _owner_kanban_db.block_task(rival, tid, reason="Reassigned during acceptance")
                    assert _owner_kanban_transitions.unblock_task(rival, tid)
                    github["replacement"] = _owner_kanban_claims.claim_task(rival, tid).current_run_id
            github.update(conclusion=conclusion, race=reclaim)
            assert not _owner_kanban_completion.complete_task(conn, tid, expected_run_id=run_id,
