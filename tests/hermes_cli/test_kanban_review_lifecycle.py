...
def test_review_dispatch_gate_prevents_phantom_reviewer(
    ...
    with kbc.connect() as conn:
        ...
        kb.claim_task(conn, tid)
        # A reviewer distinct from the implementer: the review lane refuses to
        # dispatch a card back to the profile that wrote it, so without this the
        # gate under test would not be the only thing suppressing the claim.
        kb.request_review(
...
def test_active_pr_guard_skipped_for_review_lane_but_defers_ready_lane(
    ...
    with kbc.connect() as conn:
        ...
        review_id = kb.create_task(conn, title="review me", assignee="impl")
        claimed = kb.claim_task(conn, review_id)
        assert claimed is not None
        kb.add_comment(conn, review_id, author="worker", body=pr_comment)
        # Implementer != reviewer, or the review lane refuses the handoff for a
        # reason that has nothing to do with the active_pr guard under test.
        assert kb.request_review(
...
def test_review_dispatch_preserves_task_skills_and_adds_reviewer_skill(
    ...
    with kbc.connect() as conn:
        ...
        # The reviewer must differ from the implementer for the lane to dispatch
        # at all; the subject here is the skills it carries into that spawn.
        assert kb.request_review(
            conn,
            task_id,
            summary="ready",
...
def test_review_dispatch_honors_global_and_per_profile_caps(
    ...
    with kbc.connect() as conn:
        ...
        for title in ("review one", "review two"):
            task_id = kb.create_task(conn, title=title, assignee="impl")
            implementation = kb.claim_task(conn, task_id)
            assert implementation is not None
            # Reviewer distinct from implementer; the subject is the cap, not
