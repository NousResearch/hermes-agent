...
def _signal_recorder():
    ...
    return sent, signal_fn


def test_reaper_signals_live_pid_holding_a_terminal_run(kb, monkeypatch):
    """The t_6a6ac2d3 process: alive, run terminal, task-row pid cleared."""
    monkeypatch.setattr(kb, "_pid_alive", lambda pid: True)
    sent, signal_fn = _signal_recorder()
    ...
    with kb.connect_closing() as conn:
        ...
        # block_task wiped the task-row pid — that is the hole.
        assert kb.get_task(conn, tid).worker_pid is None

        reaped = kb.reap_superseded_workers(conn, signal_fn=signal_fn)
        assert reaped == [tid]
        assert 424242 in {pid for pid, _sig in sent}
        ...
        assert "superseded_worker_reaped" in kinds, (
            "the kill must be auditable in task_events"
        )

        # One-shot: the next tick does not re-signal the same run.
        sent.clear()
        assert kb.reap_superseded_workers(conn, signal_fn=signal_fn) == []
        assert sent == []


def test_reaper_never_signals_the_current_run(kb, monkeypatch):
    """A live worker on the task's CURRENT run must be left alone."""
    monkeypatch.setattr(kb, "_pid_alive", lambda pid: True)
    sent, signal_fn = _signal_recorder()
    ...
    with kb.connect_closing() as conn:
        ...
        assert kb.reap_superseded_workers(conn, signal_fn=signal_fn) == []
        assert sent == [], "the live current run must never be signalled"


def test_reaper_never_signals_another_hosts_pid(kb, monkeypatch):
    """Host scoping: another host's claim_lock owns a different process."""
    monkeypatch.setattr(kb, "_pid_alive", lambda pid: True)
    sent, signal_fn = _signal_recorder()
    ...
    with kb.connect_closing() as conn:
        ...
        run_id = kb.latest_run(conn, tid).id
        kb._set_worker_pid(conn, tid, 424242)
        # Re-stamp the run's spawn lock as belonging to a different host.
        conn.execute(
            "UPDATE task_runs SET spawn_claim_lock = ? WHERE id = ?",
            ("some-other-host:1234", run_id),
        )
        conn.commit()
        assert kb.block_task(conn, tid, reason="decide please",
                             expected_run_id=run_id)
        ...
        assert kb.reap_superseded_workers(conn, signal_fn=signal_fn) == []
        assert sent == [], "never signal a pid owned by another host's lock"


def test_reaper_runs_in_the_dispatch_tick(kb, monkeypatch,
                                          all_assignees_spawnable):
    """The reaper is wired into dispatch_once, not just importable."""
    monkeypatch.setattr(kb, "_pid_alive", lambda pid: True)
    ...
    with kb.connect_closing() as conn:
        ...
        monkeypatch.setattr(
            ...
            lambda pid, sig, **kw: signalled.append((pid, sig)),
        )
        res = kb.dispatch_once(conn, spawn_fn=lambda *a, **k: 99002)
        assert res.reaped_superseded == [tid]
        assert 424242 in {pid for pid, _ in signalled}


def test_spawn_pid_migration_is_idempotent_on_an_existing_db(kb):
    """The additive task_runs migration must not break an existing board."""
    with kb.connect_closing() as conn:
        tid = kb.create_task(conn, title="ship the thing", assignee="a")
        conn.commit()
    # Re-running init_db over a live DB is the upgrade path.
    kb.init_db()
    kb.init_db()
    with kb.connect_closing() as conn:
        cols = {
        ...
        assert {"spawn_pid", "spawn_claim_lock"} <= cols
...
@pytest.mark.parametrize("kind", [
    "status", "promoted", "unblocked", "reclaimed", "review_reopened",
])
