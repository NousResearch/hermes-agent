...
def worker_env(kanban_home, monkeypatch):
    ...
    # The producer gate is dispatcher ownership; a delegate-child marker in the ambient
    # environment would (correctly) suppress every emit below.
...
def _patch_config(monkeypatch, config):
    """Patch both readers: the hot path uses ``load_config_readonly``."""
...
def test_telemetry_is_off_by_default_and_opt_in(kanban_home, monkeypatch):
    ...
    _patch_config(monkeypatch, {"kanban": {wb.CONFIG_KEY: True}})
...
def test_config_key_is_registered_in_default_config():
    """Every config reader has a registry entry (hermes_cli/AGENTS.md)."""
    from hermes_cli.config_defaults import DEFAULT_CONFIG

    assert wb.CONFIG_KEY in DEFAULT_CONFIG[wb.CONFIG_SECTION]
...
def test_checkpoint_throttle_runs_before_the_config_gate(worker_env, monkeypatch):
    """A rate-limited checkpoint never pays a config load on the activity hot path."""
    monkeypatch.delenv(wb.ENV_OPT_IN, raising=False)

    def _boom():
        raise AssertionError("config read on a throttled checkpoint")

    monkeypatch.setattr("hermes_cli.config.load_config_readonly", _boom)
    monkeypatch.setattr(wb, "_last_checkpoint_emit", time.monotonic())
    assert wb.emit(_agent(), phase=wb.PHASE_TURN) is None
...
def test_emit_is_a_noop_in_a_delegate_child_or_cron_context(worker_env, monkeypatch):
    """A child/cron run inherits HERMES_KANBAN_TASK — dispatcher ownership is the gate."""
    from agent.delegation_context import non_dispatcher_owned_context

    assert wb.emit(_agent(), phase=wb.PHASE_TERMINAL, force=True) is not None
    with non_dispatcher_owned_context():
        assert wb.emit(_agent(), phase=wb.PHASE_TERMINAL, force=True) is None
    # Only the owning worker's snapshot landed.
    assert len(_events(worker_env.task_id)) == 1
...
def test_completed_worker_keeps_the_same_normalized_fields(worker_env):
    wb.emit(
        _agent(total=60, used=60, remaining=0, api_calls=61, _iteration_budget_warning_injected=True,
               _iteration_budget_warning_at=1_700_000_000),
    ...
    assert report["termination"]["reason"] == "max_iterations_reached"  # the tag only
    assert report["termination"]["at"] is not None
    # The injection time is reported, not this snapshot's own recorded_at.
    assert report["termination"]["warning_at"] == 1_700_000_000
...
def test_parent_subagent_and_retry_accounting_are_distinguishable(worker_env, monkeypatch):
    ...
    try:
        ...
        # The owning worker may link its own running child by naming the run it claimed.
        kb.link_tasks(conn, parent_id, worker_env.task_id, expected_child_run_id=second.id)
    finally:
        conn.close()
...
def test_termination_reason_carries_the_tag_but_no_error_body(worker_env):
    """A loop-error exit keeps its closed tag; the provider body behind it never lands."""
    body = f"Error during API call #3: HTTP 500 from {SECRET} — upstream said no"
    snap = wb.emit(_agent(), phase=wb.PHASE_TERMINAL, force=True,
                   exit_reason=f"local_processing_error({body})")
    assert snap["termination"]["reason"] == "local_processing_error"
    assert SECRET not in json.dumps(wb.collect(worker_env.task_id))

    # A reason that is not a bare snake_case tag is dropped, not serialized.
    assert wb.snapshot(
        _agent(), phase=wb.PHASE_TERMINAL, exit_reason=body
    )["termination"]["reason"] is None
...
def test_run_id_from_another_task_is_rejected(worker_env):
    """``--run N`` must belong to the task, or another attempt's record is misattributed."""
    conn = kbc.connect()
    try:
        other = kb.create_task(conn, title="other", assignee=PROFILE)
        kb.claim_task(conn, other)
        other_run = kb.latest_run(conn, other)
    finally:
        conn.close()

    assert other_run.id != worker_env.run_id
    assert wb.collect(worker_env.task_id, run_id=other_run.id) is None
    assert wb.collect(other, run_id=other_run.id) is not None
...
def test_turn_finalizer_records_terminal_snapshot_with_the_right_phase(kanban_home, monkeypatch):
    ...
    # Inside a delegate child / in-process cron run the inherited env var is not ownership.
    from agent.delegation_context import non_dispatcher_owned_context
