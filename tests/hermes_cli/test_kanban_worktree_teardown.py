...
def test_complete_task_reaps_clean_worktree(kanban_home: Path, repo: Path) -> None:
    with kbc.connect_closing() as conn:
        ...
        assert kb.complete_task(conn, tid, summary="done", claimer="worker")
    assert not wt.exists()
...
def test_parent_worktree_deferred_until_children_done(
    ...
    with kbc.connect_closing() as conn:
        ...
        assert kb.complete_task(conn, parent, summary="parent done", claimer="worker")
        ...
        assert kb.complete_task(conn, child, summary="child done", claimer="worker")
