...
class TestMcpLogin:
    ...
    def test_login_false_success_no_token(self, tmp_path, capsys, monkeypatch):
        ...
        manager._entries[manager._key("googledrive")] = previous_entry
        ...
        assert manager._entries[manager._key("googledrive")] is previous_entry
    ...
    def test_login_genuine_success_with_token(self, tmp_path, capsys, monkeypatch):
        ...
        reset_manager_for_tests()
        manager = get_manager()
        manager._entries[manager._key("realserver")] = object()
        ...
        def mock_probe(name, cfg, connect_timeout=30):
            ...
            staged_token.write_text('{"access_token": "x"}', encoding="utf-8")
            return [("a", "d"), ("b", "d"), ("c", "d")]
        ...
        assert manager._key("realserver") not in manager._entries
    ...
    def test_login_clears_tokens_but_keeps_discovered_server_metadata(self, tmp_path, capsys, monkeypatch):
        ...
        def mock_probe(name, cfg, connect_timeout=30):
            ...
            staging_home = get_oauth_reauth_staging_home(name)
            assert staging_home is not None
            staging_dir = staging_home / "mcp-tokens"
            state_at_probe.update({p.name: p.exists() for p in staging_dir.glob("tv*")})
            state_at_probe["meta"] = (staging_dir / "tv.meta.json").exists()
            (staging_dir / "tv.json").write_text('{"access_token": "fresh"}', encoding="utf-8")
            return [("a", "d")]
