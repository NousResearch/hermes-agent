"""Memory OAuth routes: the declared surface reports support and carries only state/connected/auth."""
...
def test_status_keeps_only_state_connected_and_auth(client, monkeypatch):
    ...
    assert client.get("/api/memory/providers/silent/oauth/status").json() == raw  # legacy dashboard: as-is
    ...
    assert client.get("/api/memory/providers/silent/oauth/status?surface=declared").json() == {
        "state": "error", "detail": "Authorization did not complete", "auth": None, "supported": True}
