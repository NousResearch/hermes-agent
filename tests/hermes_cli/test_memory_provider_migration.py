...
def test_recover_at_startup_one_shot_is_scoped_per_profile_home(tmp_path, monkeypatch):
    """A multiplex gateway serves several profile homes; two of them configuring the same
    provider name must each get their own one-shot recovery. Recovering it for one home must
    not skip the other. The one-shot must still hold when the SAME home retries the SAME name.
    """
...
def test_recover_at_startup_lazy_installs_off_still_one_shots_per_home(tmp_path, monkeypatch):
    """A failed lazy-install gate on profile A must not consume profile B's one-shot for the
    same provider name. Each home still warns once and never retries inside the process.
    """
    from hermes_constants import reset_hermes_home_override, set_hermes_home_override
    ...
    monkeypatch.setattr(
        mig,
        "migrate_home",
        lambda *args, **kwargs: pytest.fail("must not migrate when lazy installs are off"),
    )
    warnings: list[str] = []
    monkeypatch.setattr(mig.logger, "warning", lambda msg, *args, **kwargs: warnings.append(msg % args if args else msg))
