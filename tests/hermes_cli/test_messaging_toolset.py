...
def test_example_config_keeps_messaging_out_of_default_presets():
    example = (Path(__file__).parents[2] / "cli-config.yaml.example").read_text()

    assert "telegram: [hermes-telegram, messaging]" in example
    assert "telegram: [hermes-telegram]\n" in example
    assert "cronjob, messaging)" not in example
