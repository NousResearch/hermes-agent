...
def test_unknown_reasoning_attests_the_configured_level_actually_sent(monkeypatch, tmp_path):
    # An unrecognized --reasoning keeps the configured level; the report must name that level,
    # not echo the rejected override.
    observed = _stub_runtime(monkeypatch, tmp_path, configured_effort="medium")
    _response, result = oneshot._run_agent(
        "identity", model="test-model", provider="openai-api", reasoning="turbo"
    )
    assert observed["reasoning_config"] == {"enabled": True, "effort": "medium"}
    assert result["reasoning_effort"] == "medium"


def test_unset_reasoning_is_attested_as_null(monkeypatch, tmp_path):
