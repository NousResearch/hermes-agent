...
class TestIsOpencodeKeyless:
    ...
    def test_free_aliases_are_keyless_regardless_of_key(self):
        """ALIASES maps free / opencode_free → opencode-free. The predicate
        must treat them like the dedicated provider, not only the placeholder."""
        for alias in ("free", "opencode_free"):
            assert is_opencode_keyless(alias, "sk-anything")
            assert is_opencode_keyless(alias, "")
            assert is_opencode_keyless(alias, None)
