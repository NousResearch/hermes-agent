...
@pytest.fixture(params=("tree:0", "blob:none"))
def partial_clone(tmp_path: Path, request) -> tuple[Path, Path, str, bool]:
    seed = tmp_path / "seed"
    seed.mkdir()
    ...
    partial_filter = request.param
    _git(tmp_path, "clone", "-q", "--no-local", f"--filter={partial_filter}", remote.as_uri(), str(checkout))
    assert _git(checkout, "config", "--get", "remote.origin.promisor").stdout.strip() == "true"
    assert _git(checkout, "config", "--get", "remote.origin.partialclonefilter").stdout.strip() == partial_filter
    legacy_extension = partial_filter == "tree:0"
    if legacy_extension:
        _git(checkout, "config", "extensions.partialclone", "origin")
    return checkout, remote, partial_filter, legacy_extension
...
def test_known_promisor_assertion_recovers_with_complete_refetch(partial_clone):
    checkout, _remote, _partial_filter, _legacy_extension = partial_clone
    ...
    assert _git(checkout, "config", "--get", "extensions.partialclone", check=False).returncode != 0
...
def test_failed_recovery_restores_partial_clone_contract(partial_clone, monkeypatch):
    checkout, _remote, _partial_filter, legacy_extension = partial_clone
    ...
    extension = _git(checkout, "config", "--get", "extensions.partialclone", check=False)
    if legacy_extension:
        assert extension.returncode == 0 and extension.stdout.strip() == "origin"
    else:
        assert extension.returncode != 0
...
def test_unrelated_fetch_failure_does_not_mutate_partial_clone(partial_clone):
    checkout, _remote, partial_filter, legacy_extension = partial_clone
    failure = subprocess.CompletedProcess(
