...
def test_no_deps_reinstall_after_manual_removal_of_enabled_plugin_is_not_vetoed(
    monkeypatch, tmp_path
):
...
def test_reinstall_drops_install_metadata_rows_for_missing_dirs(
    monkeypatch, tmp_path, caplog
):
    """Ghost rows (directory gone) must not be re-merged into the sidecar on the
    next install; the surviving plugin's own record is still written (#122135)."""
    import logging
    ...
    metadata["ghost"] = {"pinned": False, "revision": "0" * 40, "source": "ghost-source"}
    metadata_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")

    with caplog.at_level(logging.WARNING, logger="hermes_cli.plugins_cmd_install"):
        _install_plugin_core(repo.as_uri(), force=True)

    fresh = _metadata(home)
    assert "ghost" not in fresh
    assert fresh["demo"]["source"] == repo.as_uri()
