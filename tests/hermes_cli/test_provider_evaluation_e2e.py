...
def _manifest(tmp_path: Path) -> dict:
    value = {
        ...
        "runtime": {"provider_id": "fake", "model": "fake", "endpoint_class": "local", "runtime_name": "fake", "server_version": "1", "protocol": "chat"},
        "template_and_parser": {"chat_template_sha256": "a", "tool_call_template_sha256": "b", "parser_name": "fake", "parser_version": "1", "parser_mode": "json"},
        "decoding": {"temperature": 0, "top_p": 1, "max_output_tokens": 10, "seed_policy": "fixed"},
        "context": {"model_context_length": 1024, "hermes_context_setting": "default", "compression_enabled": True, "system_prompt_sha256": "a", "tool_schema_sha256": "b"},
        "hermes": {"revision": "r", "dirty_tree": False, "package_lock_sha256": "a", "profile": "test", "config_sha256": "a", "source_tag": "test", "rules": [], "skills": [], "memory": {}, "toolsets": ["hermes-cli"], "disabled_toolsets": [], "mcp_catalog_digest": "a"},
        ...
        "rollback": {"current_route_id": "route", "recipe": "restore", "owner": "test", "artifact_sha256": "a"},
    }
    return pe.load_manifest(_write_json(tmp_path / "manifest.json", value), capture_tools=False)
...
def test_full_catalog_executes_through_fake_subprocess_and_real_sessiondb(tmp_path: Path):
    home = tmp_path / "home"
    ...
    (home / "skills" / "fixture-skill" / "SKILL.md").write_text("SKILL_MARKER=loaded\n", encoding="utf-8")
    ...
    for index, case in enumerate(pe.get_full_suite_cases(), start=1):
        pair = {"pair_id": f"pair-{index:03d}", "case_id": case.case_id, "repetition": 1, "seed": 20260715, "arm_order": ["candidate", "incumbent"], "run_id": root.name}
        schedule.append(pair)
        for arm in pair["arm_order"]:
            receipt = pe._run_attempt(case=case, pair=pair, arm=arm, manifest=manifest, attempt_root=root / "raw" / arm / pair["pair_id"] / "1", run_root=root, base_home=home, fixture_root=fixture, hermes_executable=str(fake), timeout=10)
            receipts.append(receipt)
    pe._write_receipts(root / "receipts.jsonl", receipts)
    cases = {case.case_id: case for case in pe.get_full_suite_cases()}
    observations, failures = pe._observations_from_receipts(root, receipts, schedule, cases)
    assert not failures
    assert len(observations) == 27
    summary = pe.scoring.score_evaluation(observations, repetitions=1, expected_case_ids=cases, replicates=16)
    ...
    compression = [receipt for receipt in receipts if receipt["case_id"].startswith("compression.")]
    assert compression and all(receipt["session"]["compression_events"] > 0 for receipt in compression)
    ...
    first_stdout.write_text(first_stdout.read_text(encoding="utf-8") + "tampered\n", encoding="utf-8")
