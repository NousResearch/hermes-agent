...
class TestNousFlowClearsApiKey:
    ...
    def test_api_key_removed_after_switch_to_nous(self, home_with_custom_provider):
        """Switching from a custom provider to Nous Portal must not leave
        ...
        mock_state = {"access_token": "nous-token"}

        with (
            patch(
                "hermes_cli.model_setup_flows.get_provider_auth_state",
                return_value=mock_state,
            ),
            patch(
                "hermes_cli.model_setup_flows.resolve_nous_runtime_credentials",
                return_value=nous_creds,
            ),
            patch(
                "hermes_cli.model_setup_flows.get_curated_nous_model_ids",
                return_value=["hermes-3-70b", "hermes-3-405b"],
            ),
            patch(
                "hermes_cli.model_setup_flows.get_pricing_for_provider",
                return_value={},
            ),
            patch(
                "hermes_cli.model_setup_flows.check_nous_free_tier",
                return_value=False,
            ),
            patch(
                "hermes_cli.model_setup_flows.union_with_portal_paid_recommendations",
                side_effect=lambda models, pricing, url: (models, pricing),
            ),
            patch(
                "hermes_cli.model_setup_flows._prompt_model_selection",
                return_value="hermes-3-70b",
            ),
            patch("hermes_cli.model_setup_flows._save_model_choice"),
            patch(
                "hermes_cli.model_setup_flows._update_config_for_provider"
            ) as mock_update,
            patch(
                "hermes_cli.model_setup_flows.get_env_value",
                return_value=None,
            ),
            patch("hermes_cli.model_setup_flows.save_config") as mock_save,
            patch("hermes_cli.model_setup_flows.save_env_value"),
            patch("hermes_cli.model_setup_flows.prompt_enable_tool_gateway"),
            patch("builtins.print"),
        ):
            _model_flow_nous(stale_config, current_model="qwen3.6-35b-fast")

        mock_update.assert_called_once_with("nous", "https://inference.nous.research/v1")

        # The config saved to disk must NOT contain api_key or api_mode
        assert mock_save.called, "save_config should have been called"
        saved_cfg = mock_save.call_args[0][0]
        model_section = saved_cfg.get("model", {})
        assert "api_key" not in model_section, (
            f"api_key must be cleared when switching to Nous Portal, "
            f"got: {model_section}"
        )
...
class TestOpenRouterFlowClearsApiKey:
    ...
    def test_api_key_removed_after_switch_to_openrouter(self, home_with_custom_provider):
        """Switching from a custom provider to OpenRouter must not leave
        api_key in the model section of config.yaml."""
        from hermes_cli.model_setup_flows import _model_flow_openrouter

        # config.yaml already has model.api_key from the custom provider
        saved = yaml.safe_load(
            (home_with_custom_provider / "config.yaml").read_text()
        )
        assert "api_key" in saved["model"], "precondition: custom api_key is present"

        with (
            patch("hermes_cli.model_setup_flows._prompt_api_key", return_value=("or-key", False)),
            patch(
                "hermes_cli.model_setup_flows._prompt_model_selection",
                return_value="anthropic/claude-opus-4",
            ),
            patch("hermes_cli.model_setup_flows._save_model_choice"),
            patch("hermes_cli.model_setup_flows.model_ids", return_value=["anthropic/claude-opus-4"]),
            patch("hermes_cli.model_setup_flows.get_pricing_for_provider", return_value={}),
            patch("hermes_cli.model_setup_flows.deactivate_provider"),
            patch("builtins.print"),
        ):
            _model_flow_openrouter({})
...
class TestApiKeyProviderFlowClearsApiKey:
    ...
    def test_api_key_removed_after_switch_to_deepseek(self, home_with_custom_provider):
        """Switching from a custom provider to DeepSeek (or any api-key
        provider) must not leave api_key in the model section of config.yaml."""
        from hermes_cli.model_setup_flows import _model_flow_api_key_provider
        ...
        with (
            patch("hermes_cli.model_setup_flows._prompt_api_key", return_value=("ds-key", False)),
            patch(
                "hermes_cli.model_setup_flows._prompt_model_selection",
                return_value="deepseek-chat",
            ),
            patch("hermes_cli.model_setup_flows._save_model_choice"),
            patch("hermes_cli.model_setup_flows.deactivate_provider"),
            patch("hermes_cli.model_setup_flows.fetch_api_models", return_value=[]),
            patch(
                "hermes_cli.model_setup_flows.list_agentic_models",
                return_value=["deepseek-chat"],
                create=True,
            ),
            patch("builtins.print"),
