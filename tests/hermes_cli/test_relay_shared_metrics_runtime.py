...
def test_completed_logical_calls_close_in_stack_order(direct_runtime):
    coordinator = relay_runtime.SESSION_COORDINATOR
    profile_key = relay_runtime.current_profile_key()
    lease = coordinator.acquire_conversation(
        profile_key=profile_key,
        session_id="logical-completion-order",
        platform="cli",
    )
    ...
    coordinator.complete_logical_call(
        ...
        handle=handles[0],
        output={"outcome": "success", "model": "auxiliary"},
    )
    assert list(turn.logical_llm_calls) == ["request-1", "request-2"]
    assert not [event for event in direct_runtime.events if event[0] == "scope.pop.rejected"]

    coordinator.complete_logical_call(
        turn,
        request_id="request-2",
        handle=handles[1],
        output={"outcome": "failed", "model": "primary"},
    )
    assert turn.logical_llm_calls == {}
    assert turn.logical_llm_outputs == {}
    logical_pops = [
        event
        for event in direct_runtime.events
        if event[0] == "scope.pop" and event[1] in handles
    ]
    assert [event[1] for event in logical_pops] == list(reversed(handles))
    assert [event[2]["output"] for event in logical_pops] == [
        {"outcome": "failed", "model": "primary"},
        {"outcome": "success", "model": "auxiliary"},
    ]
