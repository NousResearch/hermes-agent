...
def _drive_reload(cli):
    ...
    with patch("tools.mcp_tool_lifecycle.shutdown_mcp_servers"), \
         ...
         patch("tools.mcp_tool_discovery.discover_mcp_tools", return_value=["tool_a", "tool_b"]), \
         patch("tools.mcp_tool_agent.refresh_agent_mcp_tools"), \
         patch("tools.mcp_tool._servers", {"srv": object()}), \
         patch("tools.mcp_tool._lock", threading.RLock()):
        cli._reload_mcp()


def _contents(db, session_id):
    return [m.get("content") or "" for m in db.get_messages_as_conversation(session_id)]


def test_reload_note_persisted_after_a_turn(tmp_path):
    ...
    _drive_reload(_make_cli(agent, history))

    contents = _contents(db, session_id)
...
def test_reload_note_persisted_on_resumed_session_without_duplicating(tmp_path):
