...
async def test_serve_timer_runs_due_curator_once_and_honors_pause(tmp_path, monkeypatch):
    ...
    try:
        ...
        # Register the active session BEFORE unpausing: with a 20 ms tick, the
        # ticker can fire in the gap between set_paused(False) and the session
        # registration and run once (observed under a loaded 56-worker run).
        import tui_gateway.server as gateway
        with gateway._sessions_lock:
