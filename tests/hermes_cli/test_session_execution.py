...
def test_trusted_remote_desktop_identity_needs_private_driver_and_is_revalidated():
    import sys
    from hermes_cli import session_execution as api
    identity = [("connection", "owner", "guest-generation", "endpoint")]
    attestor = lambda: identity[0]
    with pytest.raises(ValueError, match="private"):
        api.ComputerUseLaunchContext(desktop_attestor=attestor)
    with pytest.raises(ValueError, match="driver"):
        api.ComputerUseLaunchContext(private_daemon=True, desktop_only=True, desktop_attestor=attestor)
    launch = api.ComputerUseLaunchContext(private_daemon=True, desktop_only=True,
                                          driver_command=sys.executable, desktop_attestor=attestor)
    # No host Wayland socket is fabricated for a remote endpoint.
    context = api.SessionExecutionContext(computer_use=launch)
    context.check()
    identity[0] = ("connection", "owner", "replacement-generation", "endpoint")
    with pytest.raises(api.SessionExecutionError, match="replaced"):
        context.check()
    for bad in ((), [], ([],), (object(),)):
        identity[0] = bad
        with pytest.raises(api.SessionExecutionError, match="identity"):
            api.SessionExecutionContext(computer_use=launch)
    def broken():
        raise RuntimeError("remote transport missing")
    with pytest.raises(api.SessionExecutionError, match="attestation"):
        api.SessionExecutionContext(computer_use=api.ComputerUseLaunchContext(
            private_daemon=True, desktop_only=True, driver_command=sys.executable, desktop_attestor=broken))


def test_backend_shell_init_is_trusted_routed_text_not_host_environment():
    import sys
    from dataclasses import FrozenInstanceError
    from hermes_cli import session_execution as api
    init = "export WAYLAND_DISPLAY=guest-display"
    assert api.SessionExecutionContext().backend_shell_init is None
    with pytest.raises(ValueError, match="routing command_prefix"):
        api.SessionExecutionContext(backend_shell_init=init)
    for invalid in (42, b"text", "text\0suffix"):
        with pytest.raises(ValueError, match="NUL-free shell text"):
            api.SessionExecutionContext(command_prefix=(sys.executable,), backend_shell_init=invalid)
    context = api.SessionExecutionContext(command_prefix=(sys.executable,), backend_shell_init=init)
    assert context.backend_shell_init == init
    assert context.apply_env({"CANARY": "host"}) == {"CANARY": "host"}
    assert init not in repr(context)
    with pytest.raises(FrozenInstanceError):
        context.backend_shell_init = "changed"
