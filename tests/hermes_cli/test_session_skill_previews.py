...
class TestCompactionPreview:
    ...
    def test_hidden_mixed_carrier_does_not_replace_visible_preview(self, db):
        carrier = (
            f"{_MERGED_PRIOR_CONTEXT_HEADER}\n"
            "preserved model-facing text\n\n"
            f"{_MERGED_SUMMARY_DELIMITER}\n\n"
            f"{MICRO_SUMMARY_PREFIX}\n\n"
            f"{HISTORICAL_TASK_HEADING}\nold tool work\n\n"
            f"{_SUMMARY_END_MARKER}"
        ...
        db.create_session(session_id="s1", source="cli", model="m")
        db.append_message("s1", role="user", content=carrier, display_kind="hidden")
        db.append_message("s1", role="user", content="later real user preview")
