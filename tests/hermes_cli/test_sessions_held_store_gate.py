...
def test_store_rewrites_refuse_and_name_the_holder_until_forced(action, state_db, foreign_holder, capsys):
    ...
    # HELD-STORE subcommand can point the command at another database, so the default resolver
    # IS the operated-on path, and the refusal names it. (Offline commands such as
    # `set-journal-mode --db` never open the live store and are not gated here.)
    assert str(state_db) in out
    subparsers = _sessions_subparsers()
    assert not [opt for name in sessions_cmd._HELD_STORE_ACTIONS for a in subparsers[name]._actions
