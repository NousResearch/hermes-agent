...
def _args(mode, db=None, force=True):
    # --force is accepted for compatibility and no longer waives any check (the holder
    # scan covers Windows now); passing it mirrors the documented flag surface.
...
def test_set_journal_mode_refuses_while_another_process_holds_the_store(tmp_path, monkeypatch, capsys):
    # Runs on windows-latest too now: the psutil holder scan covers Windows, so this is
    # the end-to-end proof the retired win32 stopgap used to stand in for.
    db = tmp_path / "state.db"
    _wal_store(db)
    monkeypatch.setattr("hermes_state.DEFAULT_DB_PATH", db)
...
@pytest.mark.parametrize("target,current,cross_vm,expected", [
    # The win32 lane no longer carries its own refusal: the psutil holder scan covers Windows,
    # so admission is lane-uniform and the enumeration in cmd_set_journal_mode is the gate.
