...
def _run(monkeypatch, capsys, argv_tail, db_path):
    ...
    # Every store the CLI opens resolves to the seeded file — kwargs preserved, so an
    # action the dispatcher opens READ-ONLY stays read-only and a write through it fails
    # loudly instead of passing on a store the real command could never write to.
    monkeypatch.setattr(hermes_state, "SessionDB", lambda **kwargs: SessionDB(db_path, **kwargs))
