...
def test_positive_watchdog_config_generates_notify_unit(monkeypatch):
    monkeypatch.setattr(
        gateway_cli,
        "load_gateway_config",
        lambda: GatewayConfig(systemd_watchdog_seconds=120),
    )
...
def test_managed_watchdog_override_keeps_generated_unit_disabled(tmp_path, monkeypatch):
...
def test_integral_decimal_watchdog_config_keeps_runtime_and_unit_disabled(tmp_path, monkeypatch):
    hermes_home = tmp_path / "home"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        'gateway:\n  systemd_watchdog_seconds: "120.0"\n',
        encoding="utf-8",
    )
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
    ...
    assert load_gateway_config().systemd_watchdog_seconds == 0
    unit = gateway_cli.generate_systemd_unit(system=False)
    assert "Type=simple" in unit
    assert "WatchdogSec=" not in unit
