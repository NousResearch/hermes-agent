...
def test_structured_pypi_check_exposes_the_fetched_latest_version(
    ...
    with patch("hermes_cli.config.detect_install_method", return_value="pip"), \
         patch("hermes_cli.banner.check_via_pypi", return_value=1), \
         patch("hermes_cli.banner._fetch_pypi_latest", return_value="2.0.0"):
        result = hm._get_update_check_result()
...
def test_structured_pypi_result_feeds_auto_runtime_verifier(
    tmp_path, monkeypatch
):
    ...
    ok, _detail = update_auto._verify_health(
        before, expected_version=result["latest_version"]
    )
