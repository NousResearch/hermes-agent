...
non-rate-limit failure must still fail fast. The helper takes ``fetch_args`` so the same
retry policy covers both apply-path fetches (``fetch origin <branch>`` and, for a release
pin, ``fetch --no-tags origin <target_ref>``).
"""
...
class TestFetchWithRateLimitRetry:
    ...
    def test_passes_fetch_args_through_verbatim(self, monkeypatch):
        # A release pin fetches a specific ref (``--no-tags origin <sha>``) via arg passthrough.
        out, stub, _ = _run(
            monkeypatch, [_result(0)], fetch_args=["--no-tags", "origin", "abc1234"])
        assert out.returncode == 0
