...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid_from_print_fallback(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.hermes.gateway")
    ...
    def test_returns_empty_when_launchctl_is_blocked(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.hermes.gateway")

        def fake_run(cmd, **kwargs):
            raise PermissionError("launchctl blocked")
...
class TestParentPidProbe:
    """Unit tests for process-parent probing used by gateway restarts."""

    def test_get_parent_pid_returns_none_when_ps_is_blocked(self, monkeypatch):
        def fake_run(cmd, **kwargs):
            raise PermissionError("ps blocked")

        monkeypatch.setattr(gateway_cli.subprocess, "run", fake_run)

        assert gateway_cli._get_parent_pid(12345) is None
