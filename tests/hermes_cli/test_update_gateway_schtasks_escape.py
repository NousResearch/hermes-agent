...
gateway cannot satisfy the check.
"""
...
def test_task_route_first_direct_spawn_only_when_no_task(monkeypatch):
    spawn_via, spawn_detached = _cold_start(monkeypatch, task_result=True)
    ...
    spawn_via, spawn_detached = _cold_start(monkeypatch, task_result=False)
    spawn_via.assert_called_once()
...
def test_preexisting_pid_never_counts_as_newly_spawned(monkeypatch):
    ...
    assert gateway_windows._spawn_via_scheduled_task(hermes_home="C:\\h") is False

    monkeypatch.setattr(gateway_windows, "_wait_for_gateway_ready", lambda **kw: [9999, 10001])
    assert gateway_windows._spawn_via_scheduled_task(hermes_home="C:\\h") is True
