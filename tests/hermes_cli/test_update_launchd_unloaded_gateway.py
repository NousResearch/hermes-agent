...
def launchd(monkeypatch):
    ...
    monkeypatch.setattr(launchd_service, "get_launchd_label", lambda: "ai.hermes.gateway", raising=False)
    monkeypatch.setattr(launchd_service, "get_launchd_plist_path", lambda: state["plist"], raising=False)
    ...
    monkeypatch.setattr(launchd_service, "launchd_restart", fake_restart, raising=False)
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from gateway.launchd_service import _parse_launchd_pid_from_print_output
