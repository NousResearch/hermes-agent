...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        monkeypatch.setattr(ws, "_dashboard_plugin_is_enabled", lambda *a, **k: True)
        ws._get_dashboard_plugins(force_rescan=True)

        # The catch-all SPA route intercepts API routes mounted after it.
        # Unmount it temporarily, mount plugin APIs, then remount it.
        spa_routes = []
