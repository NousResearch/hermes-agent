...
def test_ws_client_is_allowed_bypasses_loopback_when_remote_ws_enabled(monkeypatch):
    from hermes_cli.web_server import _ws_client_is_allowed
    from types import SimpleNamespace

    # Mock WebSocket client
    ws_mock = SimpleNamespace(client=SimpleNamespace(host="192.168.1.100"))

    # When env var is not set, it should block remote IP
    monkeypatch.delenv("HERMES_DASHBOARD_ALLOW_REMOTE_WS", raising=False)
    assert not _ws_client_is_allowed(ws_mock)

    # When env var is enabled, it should allow remote IP
    monkeypatch.setenv("HERMES_DASHBOARD_ALLOW_REMOTE_WS", "1")
    assert _ws_client_is_allowed(ws_mock)

    # When env var is disabled explicitly, it should block remote IP
    monkeypatch.setenv("HERMES_DASHBOARD_ALLOW_REMOTE_WS", "0")
    assert not _ws_client_is_allowed(ws_mock)
