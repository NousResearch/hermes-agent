...
class TestBuildSchemaFromConfig:
    ...
    def test_web_config_normalizes_legacy_smart_approval_to_manual(self):
        from hermes_cli.web_server import (
            _denormalize_config_from_web,
            _normalize_config_for_web,
        )

        legacy = {"approvals": {"mode": "smart", "timeout": 60}}
        assert _normalize_config_for_web(legacy)["approvals"] == {
            "mode": "manual",
            "timeout": 60,
        }
        assert _denormalize_config_from_web(legacy)["approvals"] == {
            "mode": "manual",
            "timeout": 60,
        }
