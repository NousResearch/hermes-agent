...
async def test_cron_job_mutations_resolve_the_owner_when_the_hint_is_another_profile(isolated_profiles):
    ...
    # pause_cron_job/delete_cron_job are admin-gated (_require_dashboard_admin) since the Mini App
    # tiered access control landed; a bare Request with no request.state.token_principal set
    # resolves to the cookie/session caller's unrestricted scope, same as this direct call always
    # got before that gate existed.
    request = Request(scope={"type": "http", "headers": []})
    paused = await _rt_cron.pause_cron_job(request, job_id, profile="default")
    ...
    await _rt_cron.delete_cron_job(request, job_id, profile="default")
