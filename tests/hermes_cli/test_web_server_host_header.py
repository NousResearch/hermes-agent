...
class TestConfiguredPublicWebSocketOrigin:
    def test_public_origin_requires_exact_scheme_host_and_port(self, monkeypatch):
        ...
        monkeypatch.setattr(ws.app.state, "bound_host", "127.0.0.1", raising=False)
        monkeypatch.setattr("hermes_cli.dashboard_auth.prefix.resolve_public_url", lambda: "https://public.example:8443")
        def fake(headers):
            return SimpleNamespace(headers=headers, client=SimpleNamespace(host="127.0.0.1"))
        assert wsc._ws_host_origin_reason(fake({"host": "127.0.0.1:9119", "origin": "https://public.example:8443"})) is None
        assert wsc._ws_host_origin_reason(fake({"host": "127.0.0.1:9119", "origin": "http://public.example:8443"})) is not None
        assert wsc._ws_host_origin_reason(fake({"host": "127.0.0.1:9119", "origin": "https://public.example:9443"})) is not None
