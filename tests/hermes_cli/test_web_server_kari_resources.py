...
# --------------------------- 按账号微调直授端点(grant_user,slice B)---------------------------
def _ugrant(user_id="sub-1", node_uid="child-1", kind="workflow", resource_id="flow_a"):
    return {"user_id": user_id, "node_uid": node_uid, "kind": kind, "resource_id": resource_id}


def test_user_grants_empty(client):
    assert client.get("/api/kari/grants/user").json() == {"grants": []}


def test_user_grant_add_list_delete_roundtrip(client):
    assert client.post("/api/kari/grants/user", json=_ugrant()).json() == {"ok": True}
    assert client.post("/api/kari/grants/user", json=_ugrant(resource_id="flow_b")).json() == {"ok": True}

    grants = client.get("/api/kari/grants/user", params={"user_id": "sub-1"}).json()["grants"]
    assert {g["resource_id"] for g in grants} == {"flow_a", "flow_b"}
    assert all(g["user_id"] == "sub-1" for g in grants)

    deleted = client.post("/api/kari/grants/user/delete", json=_ugrant()).json()
    assert deleted == {"ok": True, "removed": True}
    remaining = client.get("/api/kari/grants/user", params={"user_id": "sub-1"}).json()["grants"]
    assert {g["resource_id"] for g in remaining} == {"flow_b"}
    # 撤销不存在的直授不报错,removed=False。
    assert client.post("/api/kari/grants/user/delete", json=_ugrant()).json() == {"ok": True, "removed": False}


def test_user_grant_add_invalid_rejected(client):
    resp = client.post("/api/kari/grants/user", json=_ugrant(kind="bogus"))
    assert resp.status_code == 400
    assert client.get("/api/kari/grants/user").json() == {"grants": []}


def test_user_grant_independent_of_role_grant(client):
    # 按账号直授 与 角色授权 两条端点 / 两张表互不影响。
    client.post("/api/kari/grants/user", json=_ugrant(user_id="sub-1", resource_id="flow_a"))
    client.post("/api/kari/grants", json=_grant(role="财务", node_uid="child-1", kind="workflow", resource_id="flow_a"))
    assert len(client.get("/api/kari/grants/user").json()["grants"]) == 1
    assert len(client.get("/api/kari/grants").json()["grants"]) == 1
