...
def test_print_whats_new_notice_silent_when_no_brief(monkeypatch, capsys):
    from hermes_cli import whats_new as wn
    from hermes_cli.update_cmd import _print_whats_new_notice

    # Pin a version with NO brief on disk and an empty unseen list, so the
    # silence invariant does not depend on which briefs ship in the repo
    # (this PR ships v0.21.0 while main is 0.21.0 — the old "current
    # version has no brief" premise broke on rebase).
    monkeypatch.setattr(wn, "get_current_version", lambda _root: "0.20.0")
    monkeypatch.setattr(wn, "unseen_versions", lambda _home, _root, _cur: [])
