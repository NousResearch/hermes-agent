"""Kanban worker session-end emission on hard-exit paths (HM2 / #85022).

A kanban worker's memory-provider ``on_session_end`` lifecycle event fires
from ``_run_cleanup`` → ``_shutdown_agent_memory_provider``. On a normal exit
that runs via the single-query ``finally:`` → ``_finalize_single_query`` — base
behaviour, not something these tests re-assert. The genuinely-missed exits are
the hard ``os._exit(0)`` paths that bypass ``finally:`` / ``atexit`` entirely:

  - ``_signal_handler_q`` (kanban SIGTERM/SIGHUP handler) — ``os._exit(0)``
  - the exit watchdog thread — ``os._exit(0)``

Both now call the shared ``_emit_session_end_before_hard_exit()`` helper right
before ``os._exit(0)``. These tests lock in that each hard-exit path emits
``on_session_end`` exactly once, that a raising provider never prevents the
process exit, and that no double-emission is possible when the normal
``finally:`` also runs.

The helper is deliberately NOT a parallel implementation: it routes through
the same ``_shutdown_agent_memory_provider`` the normal ``_run_cleanup`` path
uses, so it inherits the same ``flush_pending(timeout=10)`` drain and the same
``_session_messages``-vs-no-arg call shape (review points 1 & 4).

Interactive behaviour is unchanged: the interactive path already delivered the
memory session-end via the same ``shutdown_memory_provider`` helper and is
untouched here.
"""
...
def _recording_agent(log, *, raise_on_end=False):
    ...
    emission actually exercises the provider's on_session_end (and the
    ``flush_pending`` drain that precedes it). Mirrors the surface
    cli._active_agent_ref / _run_cleanup expect.
    """
    mm = MemoryManager()
...
def test_helper_runs_flush_pending_before_shutdown():
    """Review #85022 point 1: the hard-exit flush must be a superset of the
    normal one, i.e. it must drain ``flush_pending`` before shutting down. The
    helper routes through ``_shutdown_agent_memory_provider``, so a recording
    MemoryManager's flush_pending is exercised before shutdown_memory_provider."""
    flush_log = []

    class _FlushRecordingManager(MemoryManager):
        def flush_pending(self, timeout=None):
            flush_log.append("flush_pending")
            return True

    mm = _FlushRecordingManager()
    provider = _RecordingProvider([])
    mm._providers = [provider]
    ...
    cli_mod._emit_session_end_before_hard_exit()
    assert "flush_pending" in flush_log  # flush drained before the shutdown
...
# handler, which spawns the REAL cli._emit_session_end_before_hard_exit() on a
# background thread and joins it, right before os._exit(0) — recording the
# emission to a marker file — so we can assert the signal path delivered
# on_session_end exactly once.
_SIGNAL_PATH_SRC = """
...
# Mirror the production handler's kanban branch shape: SIGALRM deadman, then the
# session-store flush, then the memory session-end emit on a background thread
# with a bounded join, then os._exit(0).
def handler(signum, frame):
    if os.environ.get("HERMES_KANBAN_TASK"):
        ...
        emit_thread = threading.Thread(
            target=cli._emit_session_end_before_hard_exit,
            name="kanban-session-end-emit",
            daemon=True,
        )
        emit_thread.start()
        emit_thread.join(timeout=4.0)
        sys.stdout.flush()
        sys.stderr.flush()
        os._exit(0)
