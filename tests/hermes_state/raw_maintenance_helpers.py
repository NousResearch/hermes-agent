...
def ledger(conn, sid, kind):
    if kind.startswith('admission'):
        status = 'unknown' if kind.endswith('unknown') else ('terminal' if kind.endswith('terminal') else 'queued')
        ...
        status = 'unknown' if kind.endswith('unknown') else ('terminal' if kind.endswith('terminal') else 'registered')
