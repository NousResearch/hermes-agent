...
from hermes_state_common import MESSAGE_CLONE_LINEAGE_LEGACY_CEILING_KEY
...
def test_delegation_delivery_bumps_revision_once_and_invalidates_conditional_page(db):
    db.create_session("root", "desktop")
    db.append_message("root", "user", "before delivery")
    known_revision = db.get_display_revision("root")
    metadata = {"delegation_id": "delegate-1", "delivery_notice": "done"}

    first_id = db.append_delegation_delivery("root", "delegated result", metadata)

    assert db.get_display_revision("root") == known_revision + 1
    page = db.get_display_message_page(
        "root", limit=120, include_compacted=True, known_display_revision=known_revision
    )
    assert page["unchanged"] is False
    ...
    replay_id = db.append_delegation_delivery("root", "delegated result", metadata)
...
def test_display_message_page_non_compacted_reads_only_resolved_tip(db):
    db.create_session("root", "desktop")
    db.append_message("root", "user", "parent history")
    db.end_session("root", "compression")
    db.create_session("continued", "desktop", parent_session_id="root")
    db.append_message("continued", "assistant", "tip history")

    active_page = db.get_display_message_page(
        "root", limit=120, latest=False, include_compacted=False
    )
    compacted_page = db.get_display_message_page(
        "root", limit=120, latest=False, include_compacted=True
    )

    assert active_page["resolved_tip_id"] == "continued"
    assert [message["content"] for message in active_page["messages"]] == ["tip history"]
    assert [message["content"] for message in compacted_page["messages"]] == [
        "parent history",
        "tip history",
    ]


@pytest.mark.parametrize("requested_id", ["child", "root"])
def test_display_message_page_tolerates_malformed_legacy_model_config(db, requested_id):
    db.create_session("root", "desktop")
    db.append_message("root", "user", "root history")
    db.create_session("child", "desktop", parent_session_id="root")
    db.append_message("child", "assistant", "child history")
    db._execute_write(
        lambda conn: conn.execute(
            "UPDATE sessions SET model_config = ? WHERE id = 'child'", ("{malformed",)
        )
    )

    page = db.get_display_message_page(
        requested_id, limit=120, latest=False, include_compacted=True
    )
...
def test_clone_provenance_round_trips_with_exported_lineage(db, tmp_path):
    ...
    source_contents = [
        message["content"]
        for message in db.get_display_message_page(
            "parent", limit=120, latest=False, include_compacted=True
        )["messages"]
    ]
    ...
    try:
        ...
        assert [message["content"] for message in page["messages"]] == source_contents
        assert [
            message["content"]
            for message in imported.get_display_message_page(
                "parent", limit=1, offset=1, latest=False, include_compacted=True
            )["messages"]
        ] == source_contents[1:2]
        assert [
            message["content"]
            for message in imported.get_display_message_page(
                "parent", limit=2, latest=True, include_compacted=True
            )["messages"]
        ] == source_contents[-2:]
        tail_rows = [
            message
...
def test_multihop_clone_provenance_indexed_pages_match_exact_fallback(
    db, monkeypatch, tmp_path
):
    db.create_session("root", "desktop")
    source_id = db.append_message("root", "user", "tail")
    db.end_session("root", "compression")
    db.create_session("child", "desktop", parent_session_id="root")
    db.append_message("child", "assistant", "child summary")
    db._execute_write(
        lambda conn: db._clone_message_rows_with_lineage(conn, [source_id], "child")
    )
    child_clone_id = next(
        message["id"]
        for message in db.get_messages("child")
    ...
    db.create_session("grandchild", "desktop", parent_session_id="child")
    db.append_message("grandchild", "assistant", "grandchild summary")
    db._execute_write(
        lambda conn: db._clone_message_rows_with_lineage(
            conn, [child_clone_id], "grandchild"
        )
    )

    indexed_pages = [
        db.get_display_message_page(
            "root", limit=limit, offset=offset, latest=latest, include_compacted=True
        )["messages"]
        for limit, offset, latest in ((120, 0, False), (1, 1, False), (2, 0, True), (1, 1, True))
    ]
    monkeypatch.setattr(db, "_indexed_display_page_ready", lambda *_args: False)
    fallback_pages = [
        db.get_display_message_page(
            "root", limit=limit, offset=offset, latest=latest, include_compacted=True
        )["messages"]
    ...
    assert indexed_pages == fallback_pages
    assert [message["content"] for message in indexed_pages[0]] == [
        "tail",
        "child summary",
        "grandchild summary",
    ]

    exported = json.loads(json.dumps(db.export_session_lineage("root")))
    imported = SessionDB(tmp_path / "multihop-imported.db")
    try:
        assert imported.import_sessions(exported["segments"])["ok"] is True
        imported_pages = [
            imported.get_display_message_page(
                "root",
                limit=limit,
                offset=offset,
                latest=latest,
        ...
        assert [
            [message["content"] for message in page] for page in imported_pages
        ] == [[message["content"] for message in page] for page in fallback_pages]
    finally:
        imported.close()
...
@pytest.mark.parametrize(
    "edges",
    [
        pytest.param([(1, 2), (2, 1)], id="cycle"),
        pytest.param([(1, 2), (3, 2)], id="conflicting-sources"),
    ],
)
def test_import_rejects_invalid_clone_provenance_graph_atomically(db, edges):
    payload = [
        {
            "id": "root",
            "source": "desktop",
        ...
        },
        {
            ...
            "source": "desktop",
            "parent_session_id": "root",
            "messages": [{"id": 2, "role": "assistant", "content": "two"}],
            "message_clone_lineage_version": 1,
            "message_clone_lineage": [
                {"source_message_id": source_id, "clone_message_id": clone_id}
                for source_id, clone_id in edges
            ],
        },
    ]
...
@pytest.mark.parametrize(
    "topology", ["multiple-roots", "siblings", "branch", "delegate", "reset", "tool"]
)
def test_import_rejects_clone_provenance_across_display_lineages(db, topology):
    if topology == "multiple-roots":
        sessions = [
            {"id": "source", "source": "desktop", "messages": [{"id": 1, "role": "user"}]},
            {"id": "clone", "source": "desktop", "messages": [{"id": 2, "role": "assistant"}]},
        ]
    elif topology == "siblings":
        sessions = [
            {"id": "root", "source": "desktop", "end_reason": "compression"},
            {
                "id": "source",
                "source": "desktop",
                "parent_session_id": "root",
            ...
            },
            {
                ...
                "source": "desktop",
                "parent_session_id": "root",
                "messages": [{"id": 2, "role": "assistant"}],
            },
        ]
    else:
        markers = {
            "branch": {"_branched_from": "source"},
            "delegate": {"_delegate_from": "source"},
            "reset": {"_reset_from": "source"},
            "tool": {},
        }
        sessions = [
            {
            ...
            {
                "id": "clone",
                "source": "tool" if topology == "tool" else "desktop",
                "parent_session_id": "source",
                "model_config": markers[topology],
                "messages": [{"id": 2, "role": "assistant"}],
            },
        ]
    sessions[-1]["message_clone_lineage_version"] = 1
    sessions[-1]["message_clone_lineage"] = [
        {"source_message_id": 1, "clone_message_id": 2}
    ]
    ...
    assert result["ok"] is False
    assert result["imported"] == 0
    assert all(db.get_session(session["id"]) is None for session in sessions)
...
def test_clone_provenance_round_trips_with_export_all(db, tmp_path):
    ...
    try:
        ...
        exported_again = json.loads(json.dumps(imported.export_all()))
        imported_again = SessionDB(tmp_path / "imported-all-again.db")
        try:
            assert imported_again.import_sessions(exported_again)["ok"] is True
            second_page = imported_again.get_display_message_page(
                "parent", limit=120, latest=False, include_compacted=True
            )
            assert [message["content"] for message in second_page["messages"]] == source_contents
        finally:
            imported_again.close()
    finally:
        imported.close()


def test_import_mixed_message_ids_preserves_payload_order(db):
    result = db.import_sessions(
        [
            {
                "id": "mixed-ids",
                "source": "desktop",
                "messages": [
                    {"id": 100, "role": "user", "content": "integer id"},
                    {"role": "assistant", "content": "missing id"},
                    {"id": "opaque", "role": "assistant", "content": "opaque id"},
                ],
            }
        ]
    )
    ...
    assert result["ok"] is True
    page = db.get_display_message_page(
        "mixed-ids", limit=120, latest=False, include_compacted=True
    )
    assert [message["content"] for message in page["messages"]] == [
        "integer id",
        "missing id",
...
def _seed_indexed_display_lineage(db, prefix, rows_per_segment):
    root_id = f"{prefix}-root"
    child_id = f"{prefix}-child"
    db.create_session(root_id, "desktop")
    db.end_session(root_id, "compression")
    db.create_session(child_id, "desktop", parent_session_id=root_id)

    def insert_rows(conn):
        next_order = 1
        for session_id in (root_id, child_id):
            rows = []
            for index in range(rows_per_segment):
                identity = f"{prefix}-{session_id}-{index}".encode()
                rows.append(
                    (
                        session_id,
                        "assistant",
            ...
            conn.executemany(
                ...
                "(session_id, role, content, timestamp, display_order, display_identity) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                rows,
            )

    db._execute_write(insert_rows)
    return root_id


def _display_page_progress_steps(db, session_id):
    callbacks = 0
    ...
    try:
        page = db.get_display_message_page(
            session_id, limit=120, latest=True, include_compacted=True
        )
    finally:
        db._conn.set_progress_handler(None, 0)
    assert len(page["messages"]) == 120
    return callbacks


def test_display_message_page_work_is_bounded_across_large_lineage(db):
    small = _seed_indexed_display_lineage(db, "small", 1_000)
    large = _seed_indexed_display_lineage(db, "large", 10_000)

    small_steps = _display_page_progress_steps(db, small)
    large_steps = _display_page_progress_steps(db, large)

    assert large_steps < small_steps * 3


def test_provenance_display_page_work_is_bounded_across_large_lineage(db):
    def seed(prefix, rows_per_segment):
        root_id = _seed_indexed_display_lineage(db, prefix, rows_per_segment)
        ...
        def add_provenance(conn):
            ...
            conn.execute(
                ...
                "(SELECT display_order FROM messages WHERE id = ?) WHERE id = ?",
                (source_id, clone_id),
            )
            conn.execute(
                "INSERT INTO message_clone_lineage (source_message_id, clone_message_id) "
                "VALUES (?, ?)",
                (source_id, clone_id),
            )

        db._execute_write(add_provenance)
        return root_id

    small = seed("provenance-small", 1_000)
    large = seed("provenance-large", 10_000)
...
def test_modern_lineage_after_legacy_ceiling_uses_bounded_indexed_page(db):
    db.create_session("legacy", "desktop")
    legacy_id = db.append_message("legacy", "user", "pre-provenance row")
    db._execute_write(
        lambda conn: conn.execute(
            "INSERT INTO state_meta (key, value) VALUES (?, ?) "
            "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
            (MESSAGE_CLONE_LINEAGE_LEGACY_CEILING_KEY, str(legacy_id)),
        )
    )
    small = _seed_indexed_display_lineage(db, "upgrade-small", 1_000)
    large = _seed_indexed_display_lineage(db, "upgrade-large", 10_000)

    with db._read_ctx() as conn:
        assert db._indexed_display_page_ready(
            conn, ["upgrade-large-root", "upgrade-large-child"]
        )
    small_steps = _display_page_progress_steps(db, small)
    large_steps = _display_page_progress_steps(db, large)
...
def test_indexed_display_page_large_offset_materializes_only_limit_rows(db, monkeypatch):
    root_id = _seed_indexed_display_lineage(db, "offset", 10_000)
    fetched_rows = 0
    original = db._get_indexed_display_segment_rows_from_conn
    ...
    monkeypatch.setattr(db, "_get_indexed_display_segment_rows_from_conn", tracked)
    page = db.get_display_message_page(
        root_id, limit=120, offset=15_000, latest=True, include_compacted=True
    )

    assert len(page["messages"]) == 120
    assert fetched_rows <= 120
