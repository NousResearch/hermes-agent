...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db, monkeypatch):
        ...
        @contextmanager
        def traced_read_ctx():
            # Trace the connection actually checked out for each query. Calling
            # _get_read_conn() directly opens an unpooled connection, so tracing
            # that object misses later pool checkouts after the bounded read-pool
            # refactor (and strands one pool permit for the fixture lifetime).
