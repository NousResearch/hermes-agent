...
class TestApplyWalWithFallback:
    ...
    def test_retries_busy_wal_setup_then_succeeds(self, tmp_path):
        ...
        with patch("hermes_state_wal.time.sleep") as sleep_mock:
            mode = apply_wal_with_fallback(conn)
    ...
    def test_reraises_busy_wal_setup_after_bounded_retries(self, tmp_path):
        ...
        assert attempts[0] == hermes_state_wal._WAL_SETUP_MAX_ATTEMPTS
        assert sleep_mock.call_count == hermes_state_wal._WAL_SETUP_MAX_ATTEMPTS - 1
        conn.close()
