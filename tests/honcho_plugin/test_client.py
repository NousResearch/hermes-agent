...
class TestGetHonchoClient:
    ...
    def test_local_url_resolves_apikey_via_host_block_legacy_fallback(self):
        """Regression for #37436: get_honcho_client's _is_local branch must
        use the _host_block() helper (which handles legacy dot-form host keys
        like hermes.<profile>) instead of a bare dict.get(config.host). Without
        the helper, a config with dot-form keys is invisible to the SDK bypass,
        effective_api_key falls back to 'local', and every Honcho call returns
        401 Invalid JWT."""
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="jwt-token-abc",
        ...
        passed_api_key = mock_honcho.call_args.kwargs.get("api_key")
        assert passed_api_key == "jwt-token-abc", (
            f"Expected 'jwt-token-abc', got {passed_api_key!r} — "
            f"SDK bypass did not resolve apiKey from legacy dot-form host block"
        )
