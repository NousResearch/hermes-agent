...
from hermes_memory.plugins.memory.honcho import HonchoMemoryProvider
...
class TestConcludeToolDispatch:
    def test_conclude_schema_has_no_anyof(self):
        ...
        from hermes_memory.plugins.memory.honcho import CONCLUDE_SCHEMA
...
class TestToolsModeInitBehavior:
    ...
    def _make_provider_with_config(self, recall_mode="tools", init_on_session_start=False,
        ...
        with patch("hermes_memory.plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             patch("hermes_memory.plugins.memory.honcho.client.get_honcho_client", return_value=MagicMock()), \
             patch("hermes_memory.plugins.memory.honcho.session.HonchoSessionManager", return_value=mock_manager) as mock_manager_cls, \
...
class TestPerSessionMigrateGuard:
    ...
    def _make_provider_with_strategy(self, strategy, init_on_session_start=True):
        ...
        with patch("hermes_memory.plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             patch("hermes_memory.plugins.memory.honcho.client.get_honcho_client", return_value=MagicMock()), \
             patch("hermes_memory.plugins.memory.honcho.session.HonchoSessionManager", return_value=mock_manager), \
             patch("hermes_constants.get_hermes_home", return_value=MagicMock()):
...
class TestDialecticLifecycleSmoke:
    ...
    def test_full_multi_turn_session(self):
        ...
        with patch("hermes_memory.plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             patch("hermes_memory.plugins.memory.honcho.client.get_honcho_client", return_value=MagicMock()), \
             patch("hermes_memory.plugins.memory.honcho.session.HonchoSessionManager", return_value=mgr), \
             patch("hermes_constants.get_hermes_home", return_value=MagicMock()):
