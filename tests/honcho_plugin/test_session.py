...
class TestReadOnlySessionHandles:
    def test_open_read_only_does_not_create_or_configure_remote_session(self):
        ...
        honcho = MagicMock(name="Honcho")
        read_only_honcho = MagicMock(name="read_only_honcho")
        honcho.model_copy.return_value = read_only_honcho
        honcho._workspace_ensured = False
        session_cls = MagicMock(name="Session")
        peer_cls = MagicMock(name="Peer")
        fake_honcho_module = SimpleNamespace(Session=session_cls, Peer=peer_cls)
        ...
        assert session.messages == []
        assert manager._sessions_cache["cron-test"] is session_cls.return_value
        assert peer is peer_cls.return_value
        session_cls.assert_called_once_with("cron-test", read_only_honcho)
        peer_cls.assert_called_once_with("user-cron-test", read_only_honcho)
        honcho.model_copy.assert_called_once_with(deep=False)
        honcho.session.assert_not_called()
        honcho.peer.assert_not_called()
        read_only_honcho.session.assert_not_called()
        read_only_honcho.peer.assert_not_called()
        ...
        assert manager._honcho is read_only_honcho
        assert honcho._workspace_ensured is False
        assert read_only_honcho._workspace_ensured is True
