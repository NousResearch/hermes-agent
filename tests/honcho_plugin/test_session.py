...
class TestHonchoUserOnlyIngestion:
    ...
    def test_tagged_a2a_reply_is_sent_while_plain_assistant_output_is_not(self):
        """The one exception: a bot-to-bot reply is tagged ``_a2a_reply`` (see
        ``sync_turn``) and goes out with the batch, while an untagged assistant
        entry in the same session is still withheld."""
        manager = HonchoSessionManager()
        session = HonchoSession(
            key="telegram:123",
            user_peer_id="user",
        ...
        session.add_message("user", "Message from coder: hi")
        session.add_message("assistant", "hello coder", _a2a_reply=True)
        session.add_message("assistant", "internal status narration")
        user_peer = MagicMock()
        assistant_peer = MagicMock()
        user_message = MagicMock()
        a2a_message = MagicMock()
        user_peer.message.return_value = user_message
        assistant_peer.message.return_value = a2a_message
        manager._get_or_create_peer = MagicMock(side_effect=[user_peer, assistant_peer])
        remote_session = MagicMock()
        manager._sessions_cache[session.honcho_session_id] = remote_session

        assert manager._flush_session(session) is True

        user_peer.message.assert_called_once_with("Message from coder: hi")
        assistant_peer.message.assert_called_once_with("hello coder")
        remote_session.add_messages.assert_called_once_with([user_message, a2a_message])
        assert all(message["_synced"] for message in session.messages)
