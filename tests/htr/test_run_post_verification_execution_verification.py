"""Tests for Task 15 — Manual Post-Verification Execution Verification Recording."""
...
def test_valid_run_post_verification_execution_verification_record_passes():
    record = _minimal_post_verification_execution_verification(
        verification_items=[_sample_post_verification_verification_item()],
        verification_status=contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_VERIFIED,
    )
    schemas.validate(record, "run_post_verification_execution_verification_record")
...
def test_invalid_run_id_fails_schema():
    record = _minimal_post_verification_execution_verification(run_id="bad")
...
def test_invalid_source_execution_result_fingerprint_fails():
    with pytest.raises(ValueError, match="source_execution_result_fingerprint"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="",
            source_execution_verification_fingerprint="fp-verification",
            source_post_verification_followup_plan_fingerprint="fp-pvfp",
...
def test_invalid_verifier_fails():
...
def test_verification_items_must_be_list():
    record = _minimal_post_verification_execution_verification()
    record["verification_items"] = {}
    with pytest.raises(ValueError, match="verification_items must be a list"):
        schemas.validate(record, "run_post_verification_execution_verification_record")


def test_verification_items_entries_must_be_dicts():
    record = _minimal_post_verification_execution_verification()
    record["verification_items"] = ["bad"]
    with pytest.raises(ValueError, match="verification_items entries must be dicts"):
        schemas.validate(record, "run_post_verification_execution_verification_record")


def test_missing_verification_item_id_fails():
    with pytest.raises(KeyError, match="verification_item_id"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
...
def test_invalid_verification_decision_after_result_fails():
    with pytest.raises(ValueError, match="verification_decision_after_result"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[
                _sample_post_verification_verification_item(verification_decision_after_result="draft")
            ],
        )


def test_source_result_item_id_must_be_string_or_none():
    with pytest.raises(ValueError, match="source_result_item_id"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            ...
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[
                _sample_post_verification_verification_item(source_result_item_id=123)
            ],
        )
...
def test_source_post_verification_followup_item_id_must_be_string_or_none():
    with pytest.raises(ValueError, match="source_post_verification_followup_item_id"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[
                _sample_post_verification_verification_item(
...
def test_source_execution_item_id_must_be_string_or_none():
    with pytest.raises(ValueError, match="source_execution_item_id"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(source_execution_item_id=123)],
        )
...
def test_source_followup_item_id_must_be_string_or_none():
    with pytest.raises(ValueError, match="source_followup_item_id"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(source_followup_item_id=123)],
        )
...
def test_request_kind_must_be_string_or_none():
    with pytest.raises(ValueError, match="request_kind"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(request_kind=123)],
        )
...
def test_execution_kind_must_be_string_or_none():
    with pytest.raises(ValueError, match="execution_kind"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(execution_kind=123)],
        )
...
def test_item_status_must_be_string_or_none():
    with pytest.raises(ValueError, match="item_status"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(item_status=123)],
        )
...
def test_verification_decision_must_be_string_or_none():
    with pytest.raises(ValueError, match="verification_decision"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(verification_decision=123)],
        )
...
def test_followup_kind_must_be_string_or_none():
    with pytest.raises(ValueError, match="followup_kind"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(followup_kind=123)],
        )


def test_result_item_status_must_be_string_or_none():
    with pytest.raises(ValueError, match="result_item_status"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[
                _sample_post_verification_verification_item(result_item_status=123)
            ],
        )
...
def test_reason_must_be_string_or_none():
    with pytest.raises(ValueError, match="reason"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(reason=123)],
        )
...
def test_evidence_must_be_dict():
    with pytest.raises(ValueError, match="evidence"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(evidence="bad")],
        )
...
def test_item_metadata_must_be_dict():
    with pytest.raises(ValueError, match="metadata"):
        contracts.make_run_post_verification_execution_verification_record(
            ...
            source_post_verification_execution_request_fingerprint="fp-pver",
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item(metadata="bad")],
        )
...
def test_empty_verification_status_requires_empty_verification_items():
    with pytest.raises(ValueError, match="empty verification_status requires verification_items"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            ...
            source_post_verification_execution_result_fingerprint="fp-pve-result",
            verification_items=[_sample_post_verification_verification_item()],
            verification_status=contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_EMPTY,
        )


def test_verified_verification_status_requires_verified_items():
    with pytest.raises(ValueError, match="verified verification_status requires"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            ...
            verification_items=[
                _sample_post_verification_verification_item(
                    verification_decision_after_result=(
                        contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_ITEM_DECISION_REJECTED
                    )
                )
            ],
            verification_status=contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_VERIFIED,
...
def test_rejected_verification_status_requires_rejected_items():
    with pytest.raises(ValueError, match="rejected verification_status requires"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            ...
            verification_status=contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_REJECTED,
        )


def test_needs_changes_verification_status_requires_mixed_or_needs_changes_items():
    with pytest.raises(ValueError, match="needs_changes verification_status requires"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            ...
            verification_status=contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_NEEDS_CHANGES,
        )


def test_all_not_applicable_verification_items_invalid():
    with pytest.raises(ValueError, match="verification_items cannot all be not_applicable"):
        contracts.make_run_post_verification_execution_verification_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            ...
            verification_items=[
                _sample_post_verification_verification_item(
                    verification_decision_after_result=(
                        contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_ITEM_DECISION_NOT_APPLICABLE
                    )
                )
            ],
        )
...
def test_make_run_post_verification_execution_verification_record_returns_valid_schema(
    tmp_path,
):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
...
def test_factory_metadata_defaults_to_empty_dict(tmp_path):
    ...
    record = contracts.make_run_post_verification_execution_verification_record(
        run_id=run_id,
        source_execution_result_fingerprint=contracts.run_execution_result_fingerprint(
            result
        ),
...
def test_factory_reason_remains_none(tmp_path):
    ...
    record = contracts.make_run_post_verification_execution_verification_record(
        ...
        verification_items=[
            _sample_post_verification_verification_item(reason=None)
        ],
    )
    assert record["verification_items"][0]["reason"] is None
...
def test_factory_normalizes_evidence_and_item_metadata(tmp_path):
    ...
    assert record["verification_items"][0]["evidence"] == {}
    assert record["verification_items"][0]["metadata"] == {}
...
def test_factory_does_not_write_files(tmp_path):
    ...
    _post_verification_execution_verification_record(
        run_id, result, verification, pvfp, pver, pve_result
    )
    assert not contracts.run_post_verification_execution_verification_record_json_path(
        run_id, tmp_path
    ).exists()
...
def test_fingerprint_validates_schema_first():
    bad = _minimal_post_verification_execution_verification()
    bad["metadata"] = "bad"
    with pytest.raises(ValueError, match="metadata must be a dict"):
        contracts.run_post_verification_execution_verification_fingerprint(bad)


def test_equivalent_records_same_fingerprint():
    first = _minimal_post_verification_execution_verification(
        verification_items=[_sample_post_verification_verification_item()],
        verification_status=contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_VERIFIED,
    )
    second = dict(first)
    assert contracts.run_post_verification_execution_verification_fingerprint(
        first
    ) == contracts.run_post_verification_execution_verification_fingerprint(second)


def test_changed_verification_items_changes_fingerprint():
    base = _minimal_post_verification_execution_verification(
        verification_items=[_sample_post_verification_verification_item()],
        verification_status=contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_VERIFIED,
    )
    changed = dict(base)
    changed["verification_items"] = [
        _sample_post_verification_verification_item(reason="different reason")
    ]
    assert contracts.run_post_verification_execution_verification_fingerprint(
        base
    ) != contracts.run_post_verification_execution_verification_fingerprint(changed)
...
def test_fingerprint_uses_canonical_json():
    record = _minimal_post_verification_execution_verification(metadata={"z": 1, "a": 2})
    expected = json.dumps(record, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
    assert contracts.run_post_verification_execution_verification_fingerprint(record) == expected


def test_fingerprint_does_not_read_files():
    record = _minimal_post_verification_execution_verification()
    with patch("htr.io.read_json") as mock_read:
        contracts.run_post_verification_execution_verification_fingerprint(record)
    mock_read.assert_not_called()


# --- D. Verification item correspondence ---


def test_global_manual_verification_items_with_null_source_result_item_id_allowed():
    execution_result = _minimal_post_verification_execution_result(
        result_items=[_sample_post_verification_result_item()]
    )
    verification_items = [
        _sample_post_verification_verification_item(source_result_item_id=None)
    ]
    contracts.validate_post_verification_execution_verification_items_correspond(
        verification_items, execution_result
    )


def test_result_linked_verification_item_must_reference_existing_source_result_item_id():
    execution_result = _minimal_post_verification_execution_result(
        result_items=[_sample_post_verification_result_item()]
    )
    verification_items = [
        _sample_post_verification_verification_item(source_result_item_id="missing")
    ]
    with pytest.raises(ValueError, match="unknown source_result_item_id"):
        contracts.validate_post_verification_execution_verification_items_correspond(
            verification_items, execution_result
        )


def test_result_linked_verification_item_preserves_source_request_item_id(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    contracts.validate_post_verification_execution_verification_items_correspond(
        [verification_item], pve_result
    )
    assert verification_item["source_request_item_id"] == pve_result["result_items"][0][
        "source_request_item_id"
    ]


def test_result_linked_verification_item_preserves_source_post_verification_followup_item_id(
    tmp_path,
):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    ...
    assert verification_item["source_post_verification_followup_item_id"] == pve_result[
        "result_items"
    ][0]["source_post_verification_followup_item_id"]


def test_result_linked_verification_item_preserves_source_execution_item_id(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["source_execution_item_id"] == pve_result["result_items"][0][
        "source_execution_item_id"
    ]


def test_result_linked_verification_item_preserves_source_followup_item_id(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["source_followup_item_id"] == pve_result["result_items"][0][
        "source_followup_item_id"
    ]


def test_result_linked_verification_item_preserves_request_kind(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["request_kind"] == pve_result["result_items"][0]["request_kind"]


def test_result_linked_verification_item_preserves_execution_kind(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["execution_kind"] == pve_result["result_items"][0][
        "execution_kind"
    ]


def test_result_linked_verification_item_preserves_item_status(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["item_status"] == pve_result["result_items"][0]["item_status"]


def test_result_linked_verification_item_preserves_verification_decision(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["verification_decision"] == pve_result["result_items"][0][
        "verification_decision"
    ]


def test_result_linked_verification_item_preserves_followup_kind(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["followup_kind"] == pve_result["result_items"][0]["followup_kind"]


def test_result_linked_verification_item_preserves_result_item_status(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_item = _verification_item_from_result(pve_result["result_items"][0])
    ...
    assert verification_item["result_item_status"] == pve_result["result_items"][0][
        "result_item_status"
    ]
...
@pytest.mark.parametrize(
    "field,value,match",
    [
        ("source_request_item_id", "wrong-request", "source_request_item_id"),
        ...
        ("result_item_status", "failed", "result_item_status"),
    ],
)
def test_mismatch_for_preserved_source_field_fails(field, value, match):
    ...
    verification_item = _verification_item_from_result(execution_result["result_items"][0])
    verification_item[field] = value
    with pytest.raises(ValueError, match=f"{match} does not match"):
        contracts.validate_post_verification_execution_verification_items_correspond(
            [verification_item], execution_result
        )
...
def test_record_post_verification_execution_verification_succeeds_after_full_chain(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
    ...
    returned = events.record_post_verification_execution_verification(
...
def test_result_linked_verification_items_correspond_to_execution_result(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
    ...
    contracts.validate_post_verification_execution_verification_items_correspond(
        record["verification_items"], pve_result
    )
    events.record_post_verification_execution_verification(
...
@pytest.mark.parametrize(
    "verification_status,item_decisions",
    [
        (contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_EMPTY, []),
        (
            contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_VERIFIED,
            [contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_ITEM_DECISION_VERIFIED],
        ),
        (
            contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_REJECTED,
            [contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_ITEM_DECISION_REJECTED],
        ),
        (
            contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_STATUS_NEEDS_CHANGES,
            [contracts.POST_VERIFICATION_EXECUTION_VERIFICATION_ITEM_DECISION_NEEDS_CHANGES],
        ),
    ],
)
def test_verification_status_writes_record_and_event(tmp_path, verification_status, item_decisions):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_items = []
    for idx, decision in enumerate(item_decisions):
        if idx < len(pve_result["result_items"]):
            verification_items.append(
                _verification_item_from_result(
                    pve_result["result_items"][idx],
                    decision=decision,
                    verification_item_id=f"pve-verify-{idx + 1}",
                )
            )
        else:
            verification_items.append(
    ...
    assert record["verification_status"] == verification_status


def test_record_post_verification_execution_verification_event_payload_fields(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
        run_id, result, verification, pvfp, pver, pve_result, verifier="assistant"
    )
    events.record_post_verification_execution_verification(
    ...
    assert payload["verifier"] == record["verifier"]
    assert payload["verification_status"] == record["verification_status"]
    ...
    assert payload["run_post_verification_execution_verification_fingerprint"] == (
        contracts.run_post_verification_execution_verification_fingerprint(record)
    )
    assert payload["run_post_verification_execution_verification_record_path"].endswith(
        "run_post_verification_execution_verification_record.json"
    )


def test_record_post_verification_execution_verification_returns_written_record(tmp_path):
    run_id, _, _, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
...
def test_invalid_run_id_fails_no_side_effects(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
        run_id, result, verification, pvfp, pver, pve_result
...
def test_run_id_mismatch_fails_no_side_effects(tmp_path):
    ...
    before = _snapshot(tmp_path, run_id, task_ids, attempt_ids)
    with pytest.raises(ValueError, match="verification record run_id does not match"):
        events.record_post_verification_execution_verification(
            tmp_path,
            run_id,
            _post_verification_execution_verification_record(
                new_run_id(), result, verification, pvfp, pver, pve_result
            ),
            actor="human",
        )
    assert _snapshot(tmp_path, run_id, task_ids, attempt_ids) == before
...
def test_missing_run_manifest_fails_no_side_effects(tmp_path):
    ...
    with pytest.raises(InvalidTransition):
        events.record_post_verification_execution_verification(
            tmp_path,
            run_id,
            _minimal_post_verification_execution_verification(run_id=run_id),
            actor="human",
        )
...
def test_unknown_source_result_item_id_fails_no_side_effects(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_items = [
        _verification_item_from_result(
            pve_result["result_items"][0],
            source_result_item_id="unknown-result",
        )
    ]
    record = _post_verification_execution_verification_record(
        run_id, result, verification, pvfp, pver, pve_result, verification_items=verification_items
    )
    before = _snapshot(tmp_path, run_id, task_ids, attempt_ids)
    with pytest.raises(InvalidTransition, match="unknown source_result_item_id"):
        events.record_post_verification_execution_verification(
            tmp_path, run_id, record, actor="human"
        )
    assert _snapshot(tmp_path, run_id, task_ids, attempt_ids) == before
...
def test_verification_item_field_mismatch_fails_no_side_effects(
    tmp_path, field, value, match
):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    verification_items = [_verification_item_from_result(pve_result["result_items"][0])]
    verification_items[0][field] = value
    record = _post_verification_execution_verification_record(
        run_id, result, verification, pvfp, pver, pve_result, verification_items=verification_items
    )
    before = _snapshot(tmp_path, run_id, task_ids, attempt_ids)
...
def test_write_order_record_before_event(tmp_path):
    ...
    def track_write(path, data):
        if str(path).endswith("run_post_verification_execution_verification_record.json"):
...
def test_existing_post_verification_execution_verification_event_id_none_raises(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
...
def test_existing_post_verification_execution_verification_missing_event_raises(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
    )
    record = _post_verification_execution_verification_record(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda e: {
                **e,
                "payload": {
                    **e["payload"],
                    "run_post_verification_execution_verification_fingerprint": "different",
                },
            },
            EventConflict,
        ),
        ...
        (
            lambda e: {**e, "payload": {**e["payload"], "verifier": "other"}},
            EventConflict,
        ),
    ],
)
def test_existing_post_verification_execution_verification_replay_semantic_mismatch(
...
def test_existing_post_verification_execution_verification_same_event_id_same_semantic_returns_existing(
    tmp_path,
):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification, pvfp, pver, pve_result = (
        _run_with_post_verification_execution_result(tmp_path)
...
def test_idempotent_same_event_id_same_semantic(tmp_path):
    ...
    event_id = new_event_id()
    first = events.record_post_verification_execution_verification(
        tmp_path, run_id, record, actor="human", event_id=event_id
    )
    second = events.record_post_verification_execution_verification(
        tmp_path, run_id, record, actor="human", event_id=event_id
    )
    assert second == first
...
def test_same_event_id_different_semantic_raises_conflict(tmp_path):
    ...
    event_id = new_event_id()
    first = _post_verification_execution_verification_record(
        run_id, result, verification, pvfp, pver, pve_result
    )
    events.record_post_verification_execution_verification(
        tmp_path, run_id, first, actor="human", event_id=event_id
    )
    second = _post_verification_execution_verification_record(
        run_id, result, verification, pvfp, pver, pve_result, notes="different semantic"
    )
...
def test_idempotency_requires_post_verification_execution_result(tmp_path):
    run_id = new_run_id()
    io.create_run_workspace(run_id, base_dir=tmp_path)
    manifest = io.read_json(paths.run_manifest_path(run_id, tmp_path))
    ...
    stub_pver = _minimal_post_verification_execution_request(run_id=run_id)
    io.atomic_write_json(
        contracts.run_post_verification_execution_request_record_json_path(
            run_id, tmp_path
        ),
    ...
    stub_verification_record = _minimal_post_verification_execution_verification(run_id=run_id)
    with pytest.raises(
        InvalidTransition,
        match="run_post_verification_execution_result_record.json is missing",
    ):
...
def test_task15_modules_do_not_import_forbidden_modules():
...
def test_package_exports_task15_apis():
    import htr
