"""Tests for Task 12 — Verification-Driven Follow-up Planning."""
...
def _minimal_post_verification_plan(**overrides):
    run_id = overrides.pop("run_id", new_run_id())
    followup_items = overrides.pop("followup_items", [])
    plan_status = overrides.pop("plan_status", None)
    if plan_status is None:
        plan_status = (
            contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_EMPTY
            if not followup_items
            else contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED
        )
    record = {
        "schema_version": 1,
        "run_id": run_id,
...
def test_valid_run_post_verification_followup_plan_record_passes():
    record = _minimal_post_verification_plan(
        followup_items=[_sample_post_verification_followup_item()],
        plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
    )
    schemas.validate(record, "run_post_verification_followup_plan_record")
...
def test_invalid_run_id_fails_schema():
    record = _minimal_post_verification_plan(run_id="bad")
...
def test_invalid_source_execution_result_fingerprint_fails():
    with pytest.raises(ValueError, match="source_execution_result_fingerprint"):
        contracts.make_run_post_verification_followup_plan_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="",
            source_execution_verification_fingerprint="fp-verification",
            followup_items=[],
...
def test_invalid_plan_status_fails():
    with pytest.raises(ValueError, match="plan_status must be one of"):
        contracts.make_run_post_verification_followup_plan_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
...
def test_missing_followup_item_id_fails():
    with pytest.raises(KeyError, match="followup_item_id"):
        contracts.make_run_post_verification_followup_plan_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
...
def test_invalid_followup_kind_fails():
    with pytest.raises(ValueError, match="followup_kind is invalid"):
        contracts.make_run_post_verification_followup_plan_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            followup_items=[
                _sample_post_verification_followup_item(followup_kind="auto_execute")
            ],
            plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
        )


def test_verification_decision_must_be_non_empty_string():
    with pytest.raises(ValueError, match="verification_decision"):
        contracts.make_run_post_verification_followup_plan_record(
            ...
            source_execution_verification_fingerprint="fp-verification",
            followup_items=[
                _sample_post_verification_followup_item(verification_decision="")
            ],
            plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
        )
...
def test_command_must_be_dict():
    with pytest.raises(ValueError, match="command must be a dict"):
        contracts.make_run_post_verification_followup_plan_record(
            ...
            source_execution_verification_fingerprint="fp-verification",
            followup_items=[
                _sample_post_verification_followup_item(command="bad")
            ],
            plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
        )
...
def test_item_metadata_must_be_dict():
    with pytest.raises(ValueError, match="metadata must be a dict"):
        contracts.make_run_post_verification_followup_plan_record(
            ...
            source_execution_verification_fingerprint="fp-verification",
            followup_items=[
                _sample_post_verification_followup_item(metadata="bad")
            ],
            plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
        )
...
def test_instructions_must_be_string_or_none():
    with pytest.raises(ValueError, match="instructions must be a string or null"):
        contracts.make_run_post_verification_followup_plan_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            followup_items=[
                _sample_post_verification_followup_item(instructions=123)
            ],
            plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
        )
...
def test_empty_plan_status_requires_empty_followup_items():
    with pytest.raises(ValueError, match="empty plan_status requires followup_items"):
        contracts.make_run_post_verification_followup_plan_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
            followup_items=[_sample_post_verification_followup_item()],
            plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_EMPTY,
        )


def test_planned_plan_status_requires_non_empty_followup_items():
    with pytest.raises(ValueError, match="planned plan_status requires non-empty"):
        contracts.make_run_post_verification_followup_plan_record(
            run_id=new_run_id(),
            source_execution_result_fingerprint="fp-result",
            source_execution_verification_fingerprint="fp-verification",
...
def test_make_run_post_verification_followup_plan_record_returns_valid_schema(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path
    )
    schemas.validate(
        _post_verification_plan_record(run_id, result, verification),
...
def test_factory_metadata_defaults_to_empty_dict(tmp_path):
    ...
    assert _post_verification_plan_record(run_id, result, verification)["metadata"] == {}
...
def test_factory_notes_remain_none(tmp_path):
    ...
    assert _post_verification_plan_record(run_id, result, verification)["notes"] is None
...
def test_factory_normalizes_command_and_item_metadata(tmp_path):
    ...
    record = _post_verification_plan_record(run_id, result, verification)
    item = record["followup_items"][0]
    assert item["command"] == {}
...
def test_factory_instructions_remain_none(tmp_path):
    ...
    record = _post_verification_plan_record(run_id, result, verification)
    assert record["followup_items"][0]["instructions"] is None
...
def test_derive_accepted_verification_produces_empty_plan(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path,
        verification_decision=contracts.EXECUTION_VERIFICATION_ACCEPTED,
    )
    items = contracts.derive_post_verification_followup_items(result, verification)
    assert items == []
    record = _post_verification_plan_record(run_id, result, verification)
    assert record["plan_status"] == contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_EMPTY
    assert record["followup_items"] == []


def test_derive_rejected_verification_produces_planned_items(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path,
        verification_decision=contracts.EXECUTION_VERIFICATION_REJECTED,
    )
    items = contracts.derive_post_verification_followup_items(result, verification)
    assert len(items) == 1
    assert items[0]["followup_kind"] == (
        contracts.POST_VERIFICATION_FOLLOWUP_KIND_REVIEW_REJECTED_ITEM
    )
    record = _post_verification_plan_record(run_id, result, verification)
    assert record["plan_status"] == contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED
    assert len(record["followup_items"]) == 1


def test_derive_needs_changes_verification_produces_planned_items(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path,
...
def test_fingerprint_validates_schema_first():
    ...
    record["metadata"] = []
    with pytest.raises(ValueError, match="metadata must be a dict"):
        contracts.run_post_verification_followup_plan_fingerprint(record)


def test_equivalent_records_same_fingerprint():
    run_id = new_run_id()
    a = _minimal_post_verification_plan(
        run_id=run_id, created_at="2026-07-18T08:00:00+00:00"
    )
    b = dict(a)
    assert contracts.run_post_verification_followup_plan_fingerprint(
        a
    ) == contracts.run_post_verification_followup_plan_fingerprint(b)


def test_changed_followup_items_changes_fingerprint():
    run_id = new_run_id()
    first = _minimal_post_verification_plan(
        run_id=run_id,
        followup_items=[_sample_post_verification_followup_item()],
        plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
    )
    second = _minimal_post_verification_plan(
        run_id=run_id,
        followup_items=[
            _sample_post_verification_followup_item(followup_item_id="pvfp-exec-2")
        ],
        plan_status=contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_PLANNED,
    )
    assert contracts.run_post_verification_followup_plan_fingerprint(
        first
    ) != contracts.run_post_verification_followup_plan_fingerprint(second)


def test_changed_metadata_changes_fingerprint():
    run_id = new_run_id()
    first = _minimal_post_verification_plan(run_id=run_id, metadata={"a": 1})
    second = _minimal_post_verification_plan(run_id=run_id, metadata={"a": 2})
    assert contracts.run_post_verification_followup_plan_fingerprint(
        first
    ) != contracts.run_post_verification_followup_plan_fingerprint(second)
...
def test_fingerprint_uses_canonical_json():
    run_id = new_run_id()
    record = _minimal_post_verification_plan(run_id=run_id, metadata={"b": 2, "a": 1})
    expected = json.dumps(record, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
    assert contracts.run_post_verification_followup_plan_fingerprint(record) == expected
...
def test_plan_post_verification_followup_succeeds_after_full_chain(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path
    )
    record = _post_verification_plan_record(run_id, result, verification)
    returned = events.plan_post_verification_followup(
        tmp_path, run_id, record, actor="planner"
    )
    assert returned == record


def test_plan_post_verification_followup_empty_plan_writes_record_and_event(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path,
        verification_decision=contracts.EXECUTION_VERIFICATION_ACCEPTED,
    )
    record = _post_verification_plan_record(run_id, result, verification)
    before = len(events.read_task_events(run_id, base_dir=tmp_path))
    events.plan_post_verification_followup(tmp_path, run_id, record, actor="planner")
    assert io.read_json(
        contracts.run_post_verification_followup_plan_record_json_path(run_id, tmp_path)
    ) == record
    rows = events.read_task_events(run_id, base_dir=tmp_path)
    assert len(rows) == before + 1
    event = rows[-1]
    assert event["event_type"] == events.EVENT_TYPE_RUN_POST_VERIFICATION_FOLLOWUP_PLANNED
    assert record["plan_status"] == contracts.POST_VERIFICATION_FOLLOWUP_PLAN_STATUS_EMPTY


def test_plan_post_verification_followup_planned_plan_writes_record_and_event(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path,
        verification_decision=contracts.EXECUTION_VERIFICATION_REJECTED,
    )
...
def test_plan_post_verification_followup_event_payload_fields(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path,
        verification_decision=contracts.EXECUTION_VERIFICATION_REJECTED,
    )
    record = _post_verification_plan_record(run_id, result, verification, planner="assistant")
    ...
    assert payload["plan_status"] == record["plan_status"]
    ...
    assert payload["run_post_verification_followup_plan_fingerprint"] == (
        contracts.run_post_verification_followup_plan_fingerprint(record)
    )
    assert payload["run_post_verification_followup_plan_record_path"].endswith(
        "run_post_verification_followup_plan_record.json"
    )


def test_item_linked_followup_items_correspond_to_execution_and_verification(tmp_path):
    run_id, _, _, _, _, _, _, result, verification = _run_with_verified_execution(
        tmp_path,
        verification_decision=contracts.EXECUTION_VERIFICATION_REJECTED,
    )
    record = _post_verification_plan_record(run_id, result, verification)
    contracts.validate_post_verification_followup_items_correspond(
        record["followup_items"], result, verification
    )
    events.plan_post_verification_followup(tmp_path, run_id, record, actor="human")
    assert contracts.run_post_verification_followup_plan_record_json_path(
        run_id, tmp_path
    ).exists()
...
def test_invalid_run_id_fails_no_side_effects(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification = (
        _run_with_verified_execution(tmp_path)
    )
    record = _post_verification_plan_record(run_id, result, verification)
...
def test_run_id_mismatch_fails_no_side_effects(tmp_path):
    ...
    with pytest.raises(ValueError, match="run_id does not match"):
        events.plan_post_verification_followup(
            tmp_path,
            run_id,
            _post_verification_plan_record(new_run_id(), result, verification),
            actor="human",
        )
    assert _snapshot(tmp_path, run_id, task_ids, attempt_ids) == before
...
def test_missing_run_manifest_fails_no_side_effects(tmp_path):
    ...
    with pytest.raises(InvalidTransition):
        events.plan_post_verification_followup(
            tmp_path,
            run_id,
            _minimal_post_verification_plan(run_id=run_id),
            actor="human",
        )
...
def test_missing_execution_result_record_fails_no_side_effects(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, request, result, verification = (
        _run_with_verified_execution(tmp_path)
    )
...
def test_source_execution_result_fingerprint_mismatch_fails_no_side_effects(tmp_path):
    ...
    with pytest.raises(InvalidTransition, match="source_execution_result_fingerprint"):
        events.plan_post_verification_followup(tmp_path, run_id, bad, actor="human")
    assert _snapshot(tmp_path, run_id, task_ids, attempt_ids) == before
...
def test_verification_record_stale_result_fingerprint_fails_no_side_effects(tmp_path):
    ...
    record = contracts.make_run_post_verification_followup_plan_record(
        run_id=run_id,
        source_execution_result_fingerprint=contracts.run_execution_result_fingerprint(
            result
        ),
...
def test_unknown_source_execution_item_id_fails_no_side_effects(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification = (
    ...
    bad_item = _sample_post_verification_followup_item(
        source_execution_item_id="unknown-exec"
    )
    record = _post_verification_plan_record(
        run_id, result, verification, followup_items=[bad_item]
    )
    before = _snapshot(tmp_path, run_id, task_ids, attempt_ids)
    with pytest.raises(InvalidTransition, match="unknown source_execution_item_id"):
        events.plan_post_verification_followup(tmp_path, run_id, record, actor="human")
    assert _snapshot(tmp_path, run_id, task_ids, attempt_ids) == before
...
def test_source_followup_item_id_mismatch_fails_no_side_effects(tmp_path):
    ...
    derived = contracts.derive_post_verification_followup_items(result, verification)
    derived[0]["source_followup_item_id"] = "wrong-followup"
    record = _post_verification_plan_record(
        run_id, result, verification, followup_items=derived
    )
    before = _snapshot(tmp_path, run_id, task_ids, attempt_ids)
...
def test_verification_decision_mismatch_fails_no_side_effects(tmp_path):
    ...
    before = _snapshot(tmp_path, run_id, task_ids, attempt_ids)
    with pytest.raises(
        InvalidTransition, match="verification_decision does not match verification"
    ):
        events.plan_post_verification_followup(tmp_path, run_id, record, actor="human")
    assert _snapshot(tmp_path, run_id, task_ids, attempt_ids) == before
...
def test_write_order_record_before_event(tmp_path):
    ...
    def track_write(path, data):
        if str(path).endswith("run_post_verification_followup_plan_record.json"):
...
def test_existing_post_verification_plan_event_id_none_raises(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification = (
        _run_with_verified_execution(tmp_path)
    )
    record = _post_verification_plan_record(run_id, result, verification)
...
def test_existing_post_verification_plan_missing_event_raises(tmp_path):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification = (
        _run_with_verified_execution(tmp_path)
    )
    record = _post_verification_plan_record(run_id, result, verification)
        ...
        (
            lambda e: {
                **e,
                "payload": {
                    **e["payload"],
                    "run_post_verification_followup_plan_fingerprint": "different",
                },
            },
            EventConflict,
        ),
...
def test_existing_post_verification_plan_replay_semantic_mismatch(
...
def test_existing_post_verification_plan_same_event_id_same_semantic_returns_existing(
    tmp_path,
):
    run_id, task_ids, attempt_ids, _, _, _, _, result, verification = (
        _run_with_verified_execution(tmp_path)
...
def test_idempotent_same_event_id_same_semantic(tmp_path):
    ...
    record = _post_verification_plan_record(run_id, result, verification)
    event_id = new_event_id()
    first = events.plan_post_verification_followup(
        tmp_path, run_id, record, actor="human", event_id=event_id
    )
    second = events.plan_post_verification_followup(
        tmp_path, run_id, record, actor="human", event_id=event_id
    )
    assert second == first
...
def test_same_event_id_different_semantic_raises_conflict(tmp_path):
    ...
    event_id = new_event_id()
    first = _post_verification_plan_record(run_id, result, verification)
    events.plan_post_verification_followup(
        tmp_path, run_id, first, actor="human", event_id=event_id
    )
    second = _post_verification_plan_record(
        run_id, result, verification, notes="different semantic"
    )
...
def test_idempotency_requires_execution_verification(tmp_path):
    run_id = new_run_id()
    io.create_run_workspace(run_id, base_dir=tmp_path)
    manifest = io.read_json(paths.run_manifest_path(run_id, tmp_path))
    ...
    stub_plan = _minimal_post_verification_plan(run_id=run_id)
    with pytest.raises(
        InvalidTransition, match="run_execution_verification_record.json is missing"
    ):
        events.plan_post_verification_followup(
...
def test_boundaries_preserved_on_success(tmp_path):
    ...
    assert after["post_verification_plan_exists"] is True
    assert len(list(paths.tasks_dir(run_id, tmp_path).iterdir())) == len(task_ids)
    task_id = task_ids[0]
...
def test_task12_modules_do_not_import_forbidden_modules():
...
def test_package_exports_task12_apis():
    import htr
