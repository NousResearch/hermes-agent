...
class TestPrefetchItems:
    ...
    @pytest.mark.parametrize(
        ("first", "second"),
        [("prefetch", "prefetch_items"), ("prefetch_items", "prefetch")],
    )
    def test_prefetch_consumes_warmed_recall_across_api_shapes(
        self, provider, first, second
    ):
        ...
        first_result = getattr(provider, first)(query)
        second_result = getattr(provider, second)(query)

        assert first_result
        assert second_result == ([] if second == "prefetch_items" else "")
...
class TestPrefetchItemsRecallIndicator:
    ...
    def test_manager_prefetch_all_drives_describe_recall(self, provider):
        ...
        from plugins.memory.hindsight.settings import _HINDSIGHT_GLYPH
