...
def test_openviking_desktop_setup_uses_selected_profile_env_instead_of_process_env(
    tmp_path,
    monkeypatch,
):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setenv("OPENVIKING_URL", "https://dashboard-profile.example")
    monkeypatch.setenv("OPENVIKING_ACTOR_PEER_ID", "dashboard-agent")
    monkeypatch.setattr(
        openviking_module,
        "_validate_openviking_setup_values",
        lambda values, *, require_api_key=False: (True, "", "user"),
    ...
    payload = openviking_module.get_desktop_openviking_setup(
        ...
        env_values={
            "OPENVIKING_URL": "https://selected-profile.example",
            "OPENVIKING_ACTOR_PEER_ID": "selected-agent",
            "OPENVIKING_ENDPOINT": "https://legacy-selected.example",
        },
    )

    assert payload["active"]["url"] == "https://selected-profile.example"
    assert payload["active"]["actor_peer_id"] == "selected-agent"
    assert payload["legacy_env_present"] == ["OPENVIKING_ENDPOINT"]
...
def test_openviking_desktop_setup_keeps_invalid_linked_profile_recoverable(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    openviking_home = tmp_path / ".openviking"
    openviking_home.mkdir()
    invalid_path = openviking_home / "ovcli.conf.invalid"
    invalid_path.write_text("{not-json", encoding="utf-8")
    invalid_fields_path = openviking_home / "ovcli.conf.invalid-fields"
    invalid_fields_path.write_text(
        json.dumps({
            "url": "https://invalid.example/openviking",
            "actor_peer_id": "hermes",
            "agent_id": "legacy-hermes",
        }),
        encoding="utf-8",
    )
    valid_path = openviking_home / "ovcli.conf.valid"
    valid_path.write_text(
        json.dumps({
            "url": "https://valid.example/openviking",
    ...
    monkeypatch.setattr(
        openviking_module,
        "_validate_openviking_setup_values",
        lambda values, *, require_api_key=False: pytest.fail("invalid profile should not be probed"),
    )

    payload = openviking_module.get_desktop_openviking_setup({
        "use_ovcli_config": True,
        "ovcli_config_path": str(invalid_path),
    })

    assert payload["active"]["source"] == "ovcli"
    assert payload["active"]["ovcli_config_path"] == str(invalid_path)
    assert payload["health"] == {
        "status": "unreachable",
        "label": "Profile invalid",
        "message": (
            "The linked OpenViking profile could not be loaded. "
            "Fix the profile file, refresh profiles, or choose another profile."
        ),
    }
    assert [(profile["display_name"], profile["path"]) for profile in payload["profiles"]] == [
        ("valid", str(valid_path)),
    ]


def test_openviking_rejects_actor_peer_id_with_path_separators(monkeypatch):
    _clear_openviking_env(monkeypatch)

    ok, message, role = openviking_module._validate_openviking_setup_values({
        "url": "http://127.0.0.1:1933",
        "actor_peer_id": "team/hermes",
    })

    assert ok is False
    assert message == "Agent ID cannot contain path separators."
    assert role is None


def test_openviking_rejects_profile_with_both_actor_peer_id_and_agent_id():
...
def test_openviking_desktop_setup_can_skip_live_health_probe(monkeypatch):
    _clear_openviking_env(monkeypatch)

    def unexpected_probe(*_args, **_kwargs):
        raise AssertionError("desktop setup snapshot should not perform network I/O")
    ...
    assert payload["health"] == {
        "status": "checking",
        "label": "Checking",
        "message": "Checking OpenViking connection status.",
    }
...
def test_openviking_desktop_save_secondary_profile_preserves_process_env(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    hermes_home = tmp_path / "worker"
    hermes_home.mkdir()
    env_path = hermes_home / ".env"
    env_path.write_text("OPENVIKING_URL=http://worker-old.local\n", encoding="utf-8")
    monkeypatch.setenv("OPENVIKING_URL", "http://dashboard.local")
    monkeypatch.setattr(openviking_module.Path, "home", staticmethod(lambda: tmp_path))

    openviking_module.save_desktop_openviking_setup(
        config={"memory": {"openviking": {}}},
        hermes_home=hermes_home,
        values={"url": "http://127.0.0.1:1933", "actor_peer_id": "worker-agent"},
        save_mode="profile",
        profile_name="worker",
    ...
    assert os.environ["OPENVIKING_URL"] == "http://dashboard.local"


def test_openviking_desktop_save_profile_rejects_reserved_active_name(tmp_path):
    with pytest.raises(ValueError, match="reserved"):
        openviking_module.save_desktop_openviking_setup(
            config={"memory": {"openviking": {}}},
            hermes_home=tmp_path / "hermes",
            values={"url": "http://localhost:1933"},
            save_mode="profile",
            profile_name="active",
        )
...
def test_openviking_provider_desktop_config_hook_uses_provider_config(monkeypatch, tmp_path):
    import hermes_cli.config as config_module

    captured = {}
    profile_env = {"OPENVIKING_URL": "https://worker.example"}
    monkeypatch.setattr(
        config_module,
        "load_config",
        lambda: {"memory": {"openviking": {"use_ovcli_config": True, "ovcli_config_path": "/tmp/profile"}}},
    )
    monkeypatch.setattr(config_module, "load_env", lambda: profile_env)

    def fake_snapshot(provider_config, *, env_values=None, probe_health=True):
        captured["provider_config"] = provider_config
        captured["env_values"] = env_values
        captured["probe_health"] = probe_health
        return {"active": {"url": "http://127.0.0.1:1933"}}

    monkeypatch.setattr(openviking_module, "get_desktop_openviking_setup", fake_snapshot)

    result = openviking_module.OpenVikingMemoryProvider().get_desktop_config(
        hermes_home=str(tmp_path)
    )

    assert result == {"active": {"url": "http://127.0.0.1:1933"}}
    assert captured == {
        "provider_config": {"use_ovcli_config": True, "ovcli_config_path": "/tmp/profile"},
        "env_values": profile_env,
        "probe_health": False,
    }


def test_openviking_provider_running_profile_uses_process_env(monkeypatch, tmp_path):
    import hermes_cli.config as config_module
    import hermes_constants

    captured = {}
    monkeypatch.setattr(hermes_constants, "get_process_hermes_home", lambda: tmp_path)
    monkeypatch.setenv("OPENVIKING_URL", "https://process.example")
    monkeypatch.setattr(
        config_module,
        "load_config",
        lambda: {"memory": {"openviking": {"use_ovcli_config": False}}},
    )
    monkeypatch.setattr(
        config_module,
        "load_env",
        lambda: {"OPENVIKING_URL": "https://profile-file.example"},
    )

    def fake_snapshot(provider_config, *, env_values=None, probe_health=True):
        captured["provider_config"] = provider_config
        captured["url"] = env_values.get("OPENVIKING_URL")
        captured["probe_health"] = probe_health
        return {"active": {"url": captured["url"]}}
    ...
    assert result == {"active": {"url": "https://process.example"}}
    assert captured == {
...
def test_openviking_provider_health_action_runs_live_probe(monkeypatch, tmp_path):
    provider = openviking_module.OpenVikingMemoryProvider()
    captured = {}
    ...
    monkeypatch.setattr(provider, "_desktop_setup_snapshot", fake_snapshot)
    ...
    assert captured == {"hermes_home": str(tmp_path), "probe_health": True}


def test_openviking_provider_desktop_actions_delegate_to_setup_helpers(monkeypatch, tmp_path):
    import hermes_cli.config as config_module
    import hermes_constants
    ...
    monkeypatch.setattr(
        openviking_module,
        "validate_desktop_openviking_setup",
        lambda values, **kwargs: {"ok": True, "values": values, **kwargs},
    )
    monkeypatch.setattr(
        openviking_module,
    ...
    validation = provider.handle_desktop_config_action(
        "validate",
        {
            "values": {"url": "https://openviking.example"},
            "profile_path": "/tmp/ovcli.conf.test",
            "require_api_key": True,
        },
        hermes_home=str(tmp_path),
    )
    started = provider.handle_desktop_config_action(
        "start-local",
        {"url": "http://127.0.0.1:1933"},
        hermes_home=str(tmp_path),
    )
    ...
    assert started == {"ok": True, "url": "http://127.0.0.1:1933"}
    ...
    monkeypatch.setattr(config_module, "reload_env", lambda: reloaded.append(True))
    monkeypatch.setattr(hermes_constants, "get_process_hermes_home", lambda: tmp_path)
    save_kwargs = {}

    def fake_save(**kwargs):
        save_kwargs.update(kwargs)
        return {"ok": True, "profile_path": kwargs["profile_path"]}

    monkeypatch.setattr(
        openviking_module,
        "save_desktop_openviking_setup",
        fake_save,
    )
    ...
    assert result == {"ok": True, "profile_path": "/tmp/ovcli.conf.test"}
    assert saved == [config]
    assert reloaded == [True]
    assert save_kwargs["update_process_env"] is True


def test_openviking_provider_secondary_profile_save_does_not_reload_process_env(
    monkeypatch,
    tmp_path,
):
    import hermes_cli.config as config_module
    ...
    process_home = tmp_path / "dashboard"
    selected_home = tmp_path / "worker"
    config = {"memory": {"openviking": {}}}
    reloaded = []
    save_kwargs = {}
    ...
    monkeypatch.setattr(config_module, "reload_env", lambda: reloaded.append(True))
    monkeypatch.setattr(hermes_constants, "get_process_hermes_home", lambda: process_home)

    def fake_save(**kwargs):
        save_kwargs.update(kwargs)
        return {"ok": True}

    monkeypatch.setattr(openviking_module, "save_desktop_openviking_setup", fake_save)

    result = openviking_module.OpenVikingMemoryProvider().handle_desktop_config_action(
        "save",
        {
            "profile_path": "/tmp/ovcli.conf.worker",
            "save_mode": "profile",
            "values": {},
        },
    ...
    assert save_kwargs["update_process_env"] is False
    assert reloaded == []


def test_openviking_provider_rejects_unknown_desktop_action(tmp_path):
    provider = openviking_module.OpenVikingMemoryProvider()

    with pytest.raises(NotImplementedError, match="Unknown OpenViking"):
...
def test_openviking_linked_profile_overrides_cli_config_env(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    ...
    assert settings["url"] == "http://saved.local"
    assert settings["api_key"] == "saved-key"


def test_openviking_cli_config_env_is_used_without_linked_profile(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    env_path = tmp_path / "ovcli.conf.env"
    env_path.write_text(
        json.dumps({"url": "http://env-profile.local", "api_key": "env-profile-key"}),
    ...
    settings = openviking_module._resolve_connection_settings({"use_ovcli_config": True})
...
def test_discover_ovcli_profiles_lists_saved_profiles_without_active_label(tmp_path, monkeypatch):
    ...
    assert openviking_module._profile_display_name(profiles[0]) == "custom-ovcli.conf"
...
def test_validate_openviking_setup_values_user_key_runs_status_and_classifies_role(monkeypatch):
    ...
    class FakeVikingClient:
        ...
        def health_payload(self):
            events.append("health")
            return {"healthy": True, "auth_mode": "api_key"}
...
def test_validate_openviking_setup_values_root_key_runs_admin_probe(monkeypatch):
    ...
    class FakeVikingClient:
        ...
        def health_payload(self):
            events.append("health")
            return {"healthy": True, "auth_mode": "trusted"}
...
def test_validate_openviking_setup_values_rejects_root_key_for_api_key_mode(monkeypatch):
    events = []
    ...
    assert ok is False
    assert message == "OpenViking root API keys require a trusted-mode server for tenant memory access."
    assert role == "root"
    assert events == ["health", "status", "admin"]
