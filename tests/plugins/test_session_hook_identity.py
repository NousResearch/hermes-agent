...
def test_identity_is_published_for_two_new_lazy_sessions_before_rpc_returns(tmp_path, monkeypatch):
    ...
    def create(home):
        ...
        assert payload["session_origin"] == "fresh"
...
def test_live_resume_publishes_identity_only_after_reattach_is_accepted(tmp_path, monkeypatch, persisted, settling):
    ...
    if settling:
        ...
        assert observed[0]["session_origin"] == "resume"
...
def test_resumed_sessions_keep_owner_through_tools_compression_and_finalization(tmp_path, monkeypatch, resume_mode):
    ...
    for name in ("a", "b"):
        ...
        assert identity[-1]["session_origin"] == "resume"
    ...
    try:
        for home, sid, key, record, agent in owners:
            ...
            assert receipts(home, "on_session_identity")[-1]["session_origin"] == "resume"
...
def test_supplied_history_never_publishes_fresh_permission_provenance(tmp_path, monkeypatch):
    ...
    try:
        assert agent_session_identity(agent)["session_origin"] == "fresh"
        monkeypatch.setattr(agent, "_build_system_prompt", lambda _: "Unmodified old private wording")
        _restore_or_build_system_prompt(agent, None, [{"role": "user", "content": "Historical input"}])
        assert receipts(home, "on_session_start")[-1]["session_origin"] == "resume"
        assert agent_session_identity(agent)["session_origin"] == "resume"
    finally:
        agent.close()
