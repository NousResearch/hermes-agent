...
def test_stage_runtime_sync_failure_does_not_blame_the_lockfile_for_other_errors(tmp_path, monkeypatch):
    """A fetch/build/tooling failure must not point at pm/uv.lock: it did not cause it (#123241 review)."""
    from pm.environment import PythonEnvironment
    from pm.package import InstallError
    from pm.runtime_stage import stage_runtime
    ...
    def fake_run(self, args, *, cwd, timeout):
        if args[0] == "sync":
            return subprocess.CompletedProcess(args, 1, "",
                ...
                "  Caused by: Network connectivity error")
        return subprocess.CompletedProcess(args, 0, "", "")

    monkeypatch.setattr(PythonEnvironment, "_run", fake_run)
    with pytest.raises(InstallError) as excinfo:
        stage_runtime(Path("uv"), Path(sys.executable), tmp_path / "dest", project=project)
    assert str(project / "uv.lock") not in str(excinfo.value)
