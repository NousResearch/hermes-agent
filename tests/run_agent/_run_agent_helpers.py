...
Extracted verbatim from the former monolithic ``test_run_agent.py`` when it
was split into per-theme files (it had outgrown the per-file CI timeout).
These are plain helper functions called directly by tests; the pytest
*fixtures* (``agent`` / ``agent_with_memory_tool``) live in ``conftest.py``.
"""
