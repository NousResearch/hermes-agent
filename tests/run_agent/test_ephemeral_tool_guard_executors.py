...
def test_single_registry_write_blocked_on_sequential_router(tool_name, args):
    ...
    with patch("model_tools.handle_function_call", side_effect=recorder):
...
def test_read_side_action_still_dispatches_in_ephemeral_chat(tool_name, args):
    ...
    content = _result_content(messages, "c1")
    assert dispatched in ([tool_name], ["cronjob_manage"] if tool_name == "cronjob" else [])
...
def test_writes_dispatch_normally_when_not_ephemeral():
    ...
    assert dispatched in (["cronjob"], ["cronjob_manage"])
...
def test_every_sessions_insert_site_consults_the_ephemeral_registry():
    ...
    for mod in modules:
        ...
        insert_lines_count += len(insert_lines)
        ...
        for lineno in insert_lines:
            ...
            if not enclosing:
                if any("is_session_ephemeral" in l for l in lines):
                    continue
                unguarded.append((mod.__name__, lineno, "<module level>"))
                continue
            ...
            if "is_session_ephemeral" not in chain_source:
                unguarded.append((mod.__name__, lineno, innermost.name))
    ...
    assert insert_lines_count >= 3, (
        f"expected the known INSERT INTO sessions sites, found {insert_lines_count} "
...
def test_trajectory_capture_still_works_for_normal_agents():
    ...
    with (
        ...
        patch("agent.session_persistence._save_trajectory_to_file") as save,
    ):
