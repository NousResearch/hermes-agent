...
def test_moa_volatile_context_fanout_runs_inside_provider_guard(
    monkeypatch, tmp_path
):
    """A stop during advisor setup discards the whole MoA attempt and guidance."""
...
""".strip(),
    ...
    def fake_call_llm(**kwargs):
        ...
        attempted.append((kwargs["task"], serialized))
        if current["value"] is not None:
            # Model a stop landing after MoA converted the advisor request but
            # immediately before its physical client call.
            current["value"] = None
        relay_llm.run_provider_call_guard()
        transmitted.append((kwargs["task"], serialized))
        return _response(
            "reference advice"
            if kwargs["task"] == "moa_reference"
            else "aggregator acted"
        )
    ...
    result = agent.run_conversation(
        "where am I?", ephemeral_user_context=current_context
    )

    assert result["final_response"] == "aggregator acted"
    assert volatile in attempted[0][1]
    assert [task for task, _payload in transmitted] == [
        "moa_reference",
        "moa_aggregator",
    ]
    assert all(volatile not in payload for _task, payload in transmitted)
