...
class TestSourceLinesAreClamped:
    ...
    def test_gateway_run_clamped(self):
        # The /usage stats handler lives in the usage leaf after the package split.
        src = self._read_file("gateway/slash_commands/usage.py")
        ...
        assert "min(100, _lpt / ctx.context_length" in src, (
            "gateway slash-command stats pct is not clamped with min(100, ...)"
        )
