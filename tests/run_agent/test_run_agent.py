...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_timeout_returns_finished_tools_without_hanging(
        self, agent, monkeypatch
    ):
        ...
        tc1 = _mock_tool_call(
            name="web_search", arguments='{"q": "fast"}', call_id="c1"
        )
        tc2 = _mock_tool_call(
            name="web_search", arguments='{"q": "slow"}', call_id="c2"
        )
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc1, tc2])
        messages = []
        flushed = []
        ...
        def record_flush(flush_messages, conversation_history=None):
            flushed.append([
                m.copy() for m in flush_messages if m.get("role") == "tool"
            ])
    ...
    def test_concurrent_timeout_prefers_late_real_result_over_timeout_message(
        self, agent, monkeypatch
    ):
        ...
        with (
            patch("agent.tool_executor.concurrent.futures.wait", side_effect=fake_wait),
            patch(
                "run_agent.handle_function_call",
                side_effect=lambda name, args, task_id, **k: f"real-{args.get('q')}",
            ),
        ):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
        ...
        assert "timed out after" not in joined, (
            "late-completing real results must not be discarded"
        )
...
class TestRunConversation:
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        As of the bounded-resume change, the exhaustion path routes through
        ``kanban_db._handle_budget_exhaustion``, which either cleanly requeues
        the task with a fresh budget (up to ``kanban.budget_resume_limit``) or
        — once the cap is reached — falls through to
        ``_record_task_failure(outcome="timed_out")`` so the
        ``consecutive_failures`` counter increments and the ``failure_limit``
        breaker eventually trips. This test asserts the worker signals the
        dispatcher exactly once via that single funnel.
        """
        self._setup_agent(agent)
        ...
        mock_handle_exhaustion = MagicMock(return_value="resumed")
        mock_connect = MagicMock(return_value=MagicMock())

        with (
            patch("run_agent.handle_function_call", return_value="ok"),
            patch(
                "hermes_cli.kanban_db._handle_budget_exhaustion", mock_handle_exhaustion
            ),
        ...
        # _handle_budget_exhaustion should have been called exactly once for
        # the exhaustion event, carrying the task id + budget context.
        assert mock_handle_exhaustion.call_count == 1, (
            f"Expected exactly 1 _handle_budget_exhaustion call, "
            f"got {mock_handle_exhaustion.call_count}. "
            f"Calls: {mock_handle_exhaustion.call_args_list}"
        )
        call = mock_handle_exhaustion.call_args_list[0]
        # Positional: (conn, task_id, ...)
        assert call.args[1] == "t_test_task_123"
        assert "Iteration budget exhausted" in call.kwargs.get("error", "")
        extra = call.kwargs.get("event_payload_extra", {})
        assert extra.get("budget_used") == 2
        assert extra.get("budget_max") == 2
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        ...
        with (
            patch("run_agent.handle_function_call", return_value="ok"),
            patch(
                "hermes_cli.kanban_db._handle_budget_exhaustion",
                mock_handle_exhaustion,
            ),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ...
        assert mock_handle_exhaustion.call_count == 0, (
            "_handle_budget_exhaustion should not be called outside kanban mode"
        )
