...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_preserves_canonical_plan_goal_and_reasoning(self, agent):
        """The integrated todo path keeps both authority rails intact."""
        from agent.adaptive_reasoning import (
            configure_adaptive_reasoning,
            effective_reasoning_config,
            reset_adaptive_reasoning_turn,
        ...
        payload = json.loads(result)
        assert payload["plan_approval"] == {"status": "granted", "revision": 7}
        assert payload["goal_outcome"] == goal_outcome
        assert payload["reasoning_control"]["status"] == "applied"
        assert effective_reasoning_config(agent) == {
            "enabled": True,
            "effort": "xhigh",
