...
def test_guardrail_halt_emits_final_response_through_stream_delta_callback():
    """Regression for #30770: guardrail halts must explain the stop to clients.

    When this branch did not emit a final text delta, SSE clients saw an empty
    finish chunk, which looked like a crash instead of a controlled stop.
    """
    agent = _make_agent("web_search", max_iterations=10, config=_hard_stop_config())
    same_args = {"query": "same"}
    responses = [
    ...
    # Mocked responses are SimpleNamespace objects, not streaming chunks; force
    # the non-streaming path so the guardrail-halt branch is reached directly.
