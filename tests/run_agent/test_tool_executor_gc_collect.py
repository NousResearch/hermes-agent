...
def test_maybe_collect_gc_triggers_for_large_content(monkeypatch):
    ...
    if helper is None:
        pytest.skip("_maybe_collect_gc_after_tool_result not present")
...
def test_sequential_tool_gc_collect_for_large_result(monkeypatch):
    """A large tool result appended by the sequential executor triggers gc.collect."""
    mock_collect = MagicMock()
    monkeypatch.setattr(gc, "collect", mock_collect)
