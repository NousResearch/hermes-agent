...
def test_publish_creates_once_then_edits_same_canonical_message(tmp_path):
    ...
    assert all(json.loads(request.data)["allowed_mentions"] == {"parse": []} for request in requests)
...
def test_only_https_snapshot_links_are_rendered():
    projection = pipeline.render_projection({
        "model": "card",
        "id": 444,
        "title": "Pipeline operacional",
        ...
        "last_transition": "#506 em execução",
        "links": {
            "github": "http://github.com/CerteiroDevTeam/hermes-agent/issues/506",
            "trello": "javascript:alert(1)",
            "thread": "https://discord.com/channels/1/2/3",
        },
    })

    assert "GitHub: evidência indisponível" in projection.content
    assert "Trello: evidência indisponível" in projection.content
    assert "[Demanda](<https://discord.com/channels/1/2/3>)" in projection.content


def test_rejects_oversized_content_without_changing_the_ledger(tmp_path):
    ledger = tmp_path / "projection.json"
    ledger.write_text(json.dumps({"epic:444": "9001"}))
    requests = []
    ...
    projection = pipeline.Projection("epic:444", "x" * 2001)
    with pytest.raises(pipeline.ProjectionError, match="2,000-character"):
        pipeline.publish("123", projection, ledger, "token", opener)
