...
def _patch_repo(tmp_path, monkeypatch, *, with_installer: bool = True):
    ...
    lock.write_text('{}\n', encoding="utf-8")
    monkeypatch.setattr(release, "PKG_LOCK_FILE", lock)
    uv_lock = repo / "uv.lock"
    uv_lock.write_text('name = "hermes-agent"\nversion = "0.0.1"\n', encoding="utf-8")
...
def test_update_version_files_stamps_bootstrap_installer(tmp_path, monkeypatch):
    ...
    updated = release.update_version_files("0.21.1", "2026.9.10")
    assert {str(paths[key]) for key in ("installer_pkg", "tauri_conf", "cargo_toml")} <= set(updated)
