"""Unit tests for the Anthropic thinking.display resolver (#48024).
...
def test_kwargs_display_invalid_falls_back():
    th = _thinking_for({"effort": "high", "display": "loud"})
    assert th and th["type"] == "adaptive"
    assert th["display"] == "summarized"


def test_kwargs_display_unchanged_when_reasoning_disabled():
    # enabled:False → no adaptive thinking kwarg (display moot)
    th = _thinking_for({"enabled": False, "effort": "high"})
    assert th is None or th.get("type") != "adaptive"
