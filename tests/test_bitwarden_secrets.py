...
def _apply_bitwarden(*, home_path, encrypted_cache_enabled=False,
                     encrypted_cache_max_stale_seconds=0, **config):
    """Exercise the current registry apply entrypoint with the original cache scenarios."""
    from agent.secret_sources import registry

    config["encrypted_cache"] = {
        "enabled": encrypted_cache_enabled,
        "max_stale_seconds": encrypted_cache_max_stale_seconds,
    }
    report = registry.apply_all({"bitwarden": config}, home_path)
    assert len(report.sources) == 1
    return report.sources[0].result
...
def test_encrypted_cache_migrates_v1_hkdf_payload(tmp_path):
    ...
    home.mkdir()
    access_token = "short-token"
    cache_key = (bw._token_fingerprint(access_token), "proj-1", "https://vault.bitwarden.com")
    serialized_key = bw._cache_key_str(cache_key)
    salt = bytes(range(16))
    nonce = bytes(range(12))
...
def test_encrypted_cache_incomplete_v1_cleanup_is_not_stale_fallback(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        bw.fetch_bitwarden_secrets(
            access_token=access_token,
            project_id="proj-1", server_url=cache_key[2],
            binary=fake_binary,
            cache_ttl_seconds=300,
            encrypted_cache_enabled=True,
            encrypted_cache_max_stale_seconds=300,
...
def test_apply_bitwarden_secrets_freezes_source_environment_once(
    ...
    """The registry apply path uses one endpoint snapshot through the child."""
    home = tmp_path / ".hermes"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "",  # A formerly inherited endpoint has now been unset.
    ],
)
...
def test_encrypted_stale_windows_are_finite_positive_and_bounded(
    ...
    monkeypatch.setattr(
        bw,
        "get_source_environment",
        lambda: {"BWS_ACCESS_TOKEN": access_token, "BWS_SERVER_URL": cache_key[2]},
    )
    monkeypatch.setattr(bw, "find_bws", lambda **kwargs: fake_binary)
    monkeypatch.setattr(
        bw,
...
@pytest.mark.parametrize("marker", ["{}", "[]", "null", '{"invalidated": false}'])
def test_existing_invalid_invalidation_marker_remains_a_durable_veto(tmp_path, marker):
    """Only successful marker removal clears a veto, including malformed JSON schemas."""
    home = tmp_path / "home"
    path = bw._encrypted_cache_invalidation_marker_path(home)
    path.parent.mkdir(parents=True)
    path.write_text(marker)
    bw._ENCRYPTED_CACHE_INVALIDATIONS.clear()  # a new process has no L1 veto

    assert bw._encrypted_cache_was_invalidated(("fingerprint", "project", ""), home)
