...
@pytest.mark.parametrize(
    "token,diagnostic,expect_fallback",
    [
        ("invalid_client", "invalid_client: connection refused", False),
        ("connection", "connection refused by remote endpoint", True),
    ],
)
def test_redaction_preserves_cache_failure_classification(
    monkeypatch, tmp_path, token, diagnostic, expect_fallback
):
    """Display masking must not change which failures permit stale secrets."""
    stale = bw._CachedFetch(secrets={"SAVED_KEY": "last-good"}, fetched_at=time.time() - 100)
    monkeypatch.setattr(bw._STORE, "lookup", lambda *a, **kw: None)
    stale_read = mock.Mock(return_value=stale)
    monkeypatch.setattr(bw._STORE.disk, "read", stale_read)
    monkeypatch.setattr(
        bw.subprocess, "run",
        lambda *a, **kw: mock.Mock(returncode=1, stdout="", stderr=diagnostic),
    )
    kwargs = dict(access_token=token, project_id="project", binary=tmp_path / "bws",
                  use_cache=True, cache_ttl_seconds=1, home_path=tmp_path)

    if expect_fallback:
        secrets, warnings = bw.fetch_bitwarden_secrets(**kwargs)
        assert secrets == {"SAVED_KEY": "last-good"}
        assert warnings and token not in warnings[0]
    else:
