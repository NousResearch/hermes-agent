"""EvidencePack v1 + EvidencePackEngine — hermetic canary implementation.

Self-contained module implementing the EvidencePack v1 contract from
~/.hermes/reports/HERMES_B1_KNOWLEDGE_DISCOVERY_CLOSURE_DESIGN_READONLY/evidence_pack_schema.md.

This module does **not** import from any production code path. It does **not**
touch GBrain, Obsidian, the real state.db, network, or subprocess. All side
effects are routed through an injected `in_memory_storage` and (optionally)
an injected `audit_sink` callable.

The engine is OFF by default (HERMES_EXECUTIVE_KNOWLEDGE_DISCOVERY_ENABLED=0).
The canary can run regardless because it instantiates the engine directly.
"""
