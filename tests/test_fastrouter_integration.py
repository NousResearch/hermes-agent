...
class TestFastRouterPricing:
    def test_billing_route_by_provider_name(self):
        ...
        assert route.provider == "fastrouter"
        assert route.billing_mode == "official_models_api"

    def test_billing_route_by_base_url(self):
        from agent.usage_pricing import resolve_billing_route
        route = resolve_billing_route(
            "anthropic/claude-opus-4.6",
            provider="custom",
    ...
    def test_pricing_entry_from_models_api(self, monkeypatch):
    ...
    def test_cost_note_mentions_fastrouter(self, monkeypatch):
        ...
        assert any("FastRouter" in n for n in result.notes)
