...
class _CheckpointFailConn:
    """Proxy connection whose execute() raises or returns for selected SQL fragments."""

    def __init__(self, real_conn, failures=None, results=None):
    ...
    def execute(self, sql, *args, **kwargs):
        ...
        for fragment, exc in self._failures.items():
            if fragment in sql:
                raise exc
        for fragment, row in self._results.items():
            if fragment in sql:
...
class TestWalCheckpointErrorHandling:
    ...
    def test_busy_checkpoint_exception_skipped_quietly(self, db, caplog):
        db._conn = _CheckpointFailConn(
            db._conn,
            failures={"wal_checkpoint": sqlite3.OperationalError("database is locked")},
        )
    ...
    def test_returned_busy_checkpoint_skipped_quietly(self, db, caplog):
        db._conn = _CheckpointFailConn(
            db._conn,
            results={"wal_checkpoint": (1, 12, 0)},
        )
        with caplog.at_level(logging.DEBUG, logger="hermes_state"):
            db._try_wal_checkpoint()

        assert any("WAL checkpoint skipped" in r.message for r in caplog.records)
        assert not any("quick_check" in sql for sql in db._conn.executed)
        assert not [r for r in caplog.records if r.levelno >= logging.WARNING]
    ...
    def test_vacuum_logs_checkpoint_failure_and_continues(self, db, caplog):
        db._conn = _CheckpointFailConn(
            db._conn,
            failures={"wal_checkpoint": sqlite3.OperationalError("checkpoint failed")},
        )
        with caplog.at_level(logging.DEBUG, logger="hermes_state"):
            db.vacuum()

        assert any(
            "WAL checkpoint before VACUUM failed" in r.message for r in caplog.records
        )
        assert any(sql == "VACUUM" for sql in db._conn.executed)
