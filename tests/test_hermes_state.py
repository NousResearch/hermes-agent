...
class TestDeleteAndExport:
    ...
    def test_import_sessions_rejects_timestamp_outside_utc_range(
        ...
        monkeypatch.setattr(
            hermes_state_portability, "datetime", LocalOnlyDatetime
        )
    ...
    def test_import_sessions_rejects_non_finite_timestamp_string(self, db):
        payload = {"id": "bad-time", "started_at": "Infinity", "messages": []}
        ...
        assert result["errors"] == [
            {
                "index": 0,
                "session_id": "bad-time",
                "error": "started_at must be a finite number",
            }
        ]
        assert db.get_session("bad-time") is None
