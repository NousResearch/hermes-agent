...
def test_setup_writes_marker_only_after_service_setup_succeeds():
    text = (REPO_ROOT / "hermes_cli" / "setup.py").read_text(encoding="utf-8")

    fn_idx = text.index("def setup_gateway(config: dict):")
    ensure_idx = text.index('if ensure_gateway_service(context="setup"):', fn_idx)
    marker_idx = text.index("_write_gateway_setup_marker()", ensure_idx)

    assert fn_idx < ensure_idx < marker_idx
