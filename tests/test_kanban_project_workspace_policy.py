...
def test_policy_project_local_worktree_is_allowed(tmp_path):
    policy = _policy(tmp_path)
    findings = validate_tasks([
        _task(workspace_path=str(policy.worktree_root / "issue-123"))
    ], policy=policy)
    assert "worktree_outside_policy_root" not in {f.code for f in findings}
    assert "shared_project_root_workspace" not in {f.code for f in findings}
