...
class TestRequestBuildClamping:
    def test_full_window_provider_default_is_clamped(self):
        """The regression: a full-window cap (user/profile default equal to the
        window) must shrink so the request fits, on EVERY build — no error
        round-trip required."""
        agent = _make_agent(max_tokens=65536)
    ...
    def test_clamp_repeats_on_subsequent_builds(self):
        ...
        assert first is not None and first < 65536
