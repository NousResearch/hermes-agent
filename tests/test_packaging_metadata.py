...
def test_transitive_security_floors_are_bounded_core_requirements(
    ...
    `<3` from `<=3`, so beyond the vulnerable/fixed/next-major probes the
    requirement must also structurally encode an explicit lower bound at or
    above the first fixed release and a strict upper bound below the next
    major release.
    """
    data = tomllib.loads((REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    requirements = {
    ...
    assert any(bound <= Version(next_major) for bound in strict_upper_bounds), (
        f"{package} requirement must encode a strict upper bound below the "
        f"next major release {next_major} (got {requirements[package]!s})"
    )
