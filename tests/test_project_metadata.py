...
def test_release_script_stages_mcp_registry_manifest(tmp_path):
    """The release commit must include every version file it mutates."""
    root = Path(__file__).resolve().parents[1]
    manifest = tmp_path / "server.json"
    manifest.write_text("{}\n", encoding="utf-8")
    ...
    bump_files = getattr(release_script, "_version_bump_files")()

    assert manifest in bump_files
