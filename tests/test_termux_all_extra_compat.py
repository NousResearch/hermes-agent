...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"ht-ai-agent[termux]"' in text
    assert '"ht-ai-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"ht-ai-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
