...
def test_complete_slash_tui_extra_exposes_stable_localization_key():
    ...
    compact = next(item for item in resp["result"]["items"] if item["text"] == "/compact")
    assert compact["meta"] == "Toggle compact display mode"
...
def test_commands_catalog_exposes_stable_presentation_ids():
    ...
    compact = next(item for item in tui["pairs"] if item[0] == "/compact")
    assert compact[1] == "Toggle compact display mode"
    assert tui["key"] == "tui"
    assert resp["result"]["description_keys"]["/compact"] == "compact"
