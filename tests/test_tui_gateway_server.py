...
def test_profile_home_isolated_default_resolves_to_launch_profile(
    monkeypatch, tmp_path
):
    """#88897: a profile-scoped process (isolated dashboard, -p testuser) must
    treat the desktop's profile=default as its OWN launch profile, never the
    machine root's default — otherwise agent sessions land in the machine DB."""
    ...
    monkeypatch.setattr(profiles_mod, "get_profile_dir", lambda name: str(machine_root))
    monkeypatch.setattr(
        server,
        "_process_is_profile_scoped",
        lambda: True,
    )

    assert server._profile_home("default") is None

    # A machine-root process keeps the historical resolution.
...
def test_process_is_profile_scoped_detects_scoped_home(monkeypatch, tmp_path):
    """The scoped detection compares the process home against the default root."""
    root = tmp_path / "root"
    root.mkdir()
    ...
    scoped = tmp_path / "root" / "profiles" / "coder"
