...
def test_prompt_submit_fails_open_inline_when_compute_host_dispatch_breaks(monkeypatch):
    ...
    monkeypatch.setattr(
        server,
        "_run_prompt_submit",
        lambda rid, sid, _session, text, chat_mode=False: inline_calls.append((rid, sid, text)),
    )
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # `learning` joined the core tool list (toolsets.py) and is recovered onto
    # a saved list the same way `kanban` is.
    from hermes_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS

    result = server._load_enabled_toolsets()
    assert result is not None
    assert {"kanban", "memory", "project", "learning"} <= set(result)
    assert set(result) - {"kanban", "memory", "project", "learning"} <= _RECENTLY_SHIPPED_TOOLSETS
