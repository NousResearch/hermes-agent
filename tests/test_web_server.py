...
def test_start_server_enables_ws_ping_for_half_open_detection(monkeypatch):
    ...
    GIL-pressure event-loop stalls (#48445/#50005): 30s interval / 60s timeout.
    This prevents a recoverable local event-loop stall from looking like a dead
    Desktop chat socket."""
...
def test_start_server_keeps_shorter_ws_ping_for_non_loopback(monkeypatch):
    ...
    heartbeat under Cloudflare-style idle windows while loopback Desktop gets
    the relaxed GIL-stall budget."""
    captured = _stub_uvicorn(monkeypatch)
