    ...
    async def test_web_extract(self, urls: List[str] = None):
        ...
        if urls:
            try:
                ...
                result = await web_extract_tool(test_urls, format="markdown", use_llm_processing=False)
    ...
    async def test_web_extract_with_llm(self, urls: List[str] = None):
        ...
        try:
            ...
            if content:
                ...
                if content_len > 0:
                    self.log_result("Extract (with LLM)", "passed", f"Content processed: {content_len} chars")
