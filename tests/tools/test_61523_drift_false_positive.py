...
def test_fuzz_random_whitespace_transforms_dont_trigger_drift(tmp_path):
    """Engine-run addition: 100-round randomized fuzz.

    For each round:
      1. Take a tool-canonical MEMORY.md body (entries joined by ENTRY_DELIMITER).
      2. Apply a random whitespace transform: random blank lines, mixed CRLF/LF,
         leading/trailing whitespace within an entry.
      3. Verify _detect_external_drift does NOT fire (the round-trip
         comparison should be invariant to these transforms).

    This fuzz validates the fix's invariant holds across the entire
    transform space, not just the two specific cases in the issue body.
    """
    from tools.memory_tool import MemoryStore, ENTRY_DELIMITER
    from unittest import mock
    import random
    ...
    store = MemoryStore(memory_char_limit=500, user_char_limit=300)
    rng = random.Random(0x61523)  # deterministic seed

    canonical_entries = [
        "Python 3.12 project.",
        "User prefers concise explanations.",
        "Default model: GPT-4o for chat, Claude 3.5 Sonnet for code.",
    ]
    canonical = ENTRY_DELIMITER.join(canonical_entries)
    ...
    for i in range(n_rounds):
        ...
        if rng.random() < 0.5:
            text = text.replace("\n", "\r\n")
        if rng.random() < 0.5:
            n_blank = rng.randint(1, 3)
            text = text.replace(ENTRY_DELIMITER, "\n" * n_blank + ENTRY_DELIMITER)
        if rng.random() < 0.5:
            entries = text.split(ENTRY_DELIMITER)
            entries = ["  " + e.strip() + "\n  " for e in entries]
            text = ENTRY_DELIMITER.join(entries)

        (tmp_path / "target_fuzz.md").write_text(text, encoding="utf-8")
        with mock.patch.object(MemoryStore, "_path_for", staticmethod(lambda t: tmp_path / f"{t}.md")):
            drift = store._detect_external_drift("target_fuzz")

        if drift is not None:
            n_failures.append((i, text[:200], drift))

    assert not n_failures, (
        f"Fuzz: {len(n_failures)}/{n_rounds} rounds false-positived on "
        f"cosmetic whitespace. First failure: {n_failures[0]}"
    )
