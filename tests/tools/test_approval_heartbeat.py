...
class TestApprovalHeartbeat:
    ...
    def test_heartbeat_import_failure_does_not_break_wait(self):
        ...
        # Wait until the worker has actually registered the approval request.
        # A fixed sleep races thread startup, especially in the simulated
        # import-failure path where the wait loop has no heartbeat side effect
    ...
    def test_gateway_timeout_uses_public_approval_timeout_and_guides_agent(self):
        """Expired gateway approvals should tell the model not to silently degrade."""
        from tools.approval import check_all_command_guards, register_gateway_notify
        ...
        with patch(
            ...
            result = check_all_command_guards(
                "rm -rf /tmp/nonexistent-timeout-target", "local"
            )

        assert result["approved"] is False
        assert result["approval_outcome"] == "timeout"
        assert "The command did not run" in result["message"]
        assert "stop and ask the user" in result["message"]
