...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_python_uuid(self, monkeypatch):
        """Temp names must not depend on $BASHPID, absent from macOS Bash 3.2."""
        monkeypatch.setattr(
            "tools.environments.base.uuid.uuid4",
            lambda: MagicMock(hex="writer123"),
        )
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
        assert ".tmp.writer123" in wrapped
        assert "$BASHPID" not in wrapped
        assert ".tmp.$$" not in wrapped

    def test_temp_path_is_fully_quoted(self, monkeypatch):
        monkeypatch.setattr(
            "tools.environments.base.uuid.uuid4",
            lambda: MagicMock(hex="writer123"),
        )
        ...
        wrapped = env._wrap_command("echo hi", "/tmp")
        assert "'/tmp/has space/hermes-snap-x.sh.tmp.writer123'" in wrapped
        assert "export -p > /tmp/has space" not in wrapped
    ...
    def test_init_session_bootstrap_also_atomic_and_unique(self, monkeypatch):
        """The initial shared snapshot must use the same atomic UUID staging."""
        monkeypatch.setattr(
            "tools.environments.base.uuid.uuid4",
            lambda: MagicMock(hex="bootstrap123"),
        )
        env = _TestableEnv()
        captured = {}
        ...
        assert ".tmp.bootstrap123" in boot and "mv -f " in boot, boot
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH. Each simulated command gets the same kind of
    Python-generated unique token used by ``_wrap_command``.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        w = " & ".join(writer(f"writer-{i}") for i in range(4))
        r = " & ".join([reader] * 4)
