...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_mktemp_not_shell_pid_variables(self):
        """The temp name must be atomically unique even on macOS bash 3.2.

        ``$BASHPID`` is unset there and ``$$`` stays the parent PID in
        concurrent subshells, so neither is a portable uniqueness source.
        """
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
    ...
    def test_temp_path_template_is_quoted(self):
        """The mktemp template stays one shell word when its path has spaces."""
        env = _TestableEnv()
        env._snapshot_ready = True
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH.  Crucially it uses a same-directory ``mktemp``;
    PID-derived names are not portable to macOS bash 3.2.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        writer = (
            ...
            f"__snap_tmp=$(mktemp {_snap_template}); "
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        script = (
            f"__snap_tmp=$(mktemp {bad_template}) && "
            f"{{ export -p > \"$__snap_tmp\" && mv -f \"$__snap_tmp\" {_q(snap)}; }} "
            "2>/dev/null || rm -f \"${__snap_tmp:-}\" 2>/dev/null || true"
        )
        self._run(script)
