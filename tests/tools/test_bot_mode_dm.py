...
def test_delivery_pins_the_hermes_entrypoint_beside_this_interpreter(tmp_path, monkeypatch):
    ...
    # ``--in`` is the TARGET profile's own home: a named profile must not share the default
    # profile's live Bot Chat session (see the scoping tests above).
    assert transport_argv[1:] == ["-p", "researcher", "chat", "--in",
                                  str(home / "profiles" / "researcher"), "-c", "Bot Chat",
                                  "--create-if-missing", "-Q"]
