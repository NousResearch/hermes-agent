...
def test_browser_command_context_cache_hit_reuses_base_env(browser_tool_for_context_cache, monkeypatch):
    ...
    def fake_build_browser_env():
        ...
        return {"PATH": "/base/bin", "BROWSER_USE_API_KEY": "key"}
    ...
    assert first["browser_env"]["PATH"] == "merged:/base/bin"
    assert first["browser_env"]["AGENT_BROWSER_SOCKET_DIR"] == first["task_socket_dir"]
    assert first["browser_env"]["AGENT_BROWSER_IDLE_TIMEOUT_MS"]


def test_browser_command_context_returns_defensive_copies(browser_tool_for_context_cache, monkeypatch):
    browser_tool = browser_tool_for_context_cache
    session_info = _install_session(browser_tool)
    monkeypatch.setattr(browser_tool, "_build_browser_env", lambda: {"PATH": "/base/bin"})
    ...
    first["browser_env"]["PATH"] = "mutated"
