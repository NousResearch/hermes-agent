...
class TestReapOrphanedBrowserSessions:
    ...
    def test_real_profile_attach_daemon_is_reaped_when_owner_is_dead(self, fake_tmpdir):
        ...
        def _pid_exists(pid):
            return pid == 4242 and not terminate_calls  # gone after the mocked signal
    ...
    def test_unfingerprintable_daemon_is_refused(self, fake_tmpdir):
        ...
        with patch("gateway.status._pid_exists", return_value=True), \
             ...
             patch("tools.browser_tool_lifecycle._verify_reapable_browser_daemon", return_value=None), \
             patch("tools.process_registry.ProcessRegistry._terminate_host_pid",
                   side_effect=lambda pid, expected_start=None: terminate_calls.append(pid)):
            _reap_orphaned_browser_sessions()
...
class TestOwnerPidCrossProcess:
    ...
    def test_write_owner_pid_publishes_by_atomic_replace(self, fake_tmpdir):
        ...
        with patch("os.replace", side_effect=observed_replace):
            bt_lifecycle._write_owner_pid(str(socket_dir), session_name)
    ...
    def test_concurrent_owner_pid_reader_never_observes_partial_content(
        ...
        try:
            ...
            for _ in range(100):
                bt_lifecycle._write_owner_pid(str(socket_dir), session_name)
            assert overlapping_read.wait(timeout=5)
        finally:
            stop.set()
...
class TestOrphanedChromiumReaper:
    ...
    def test_exact_dead_owner_session_is_reaped_with_start_guard(self, fake_tmpdir):
        from tools.browser_tool_lifecycle import _reap_session_chromium
    ...
    def test_symlinked_profile_is_rejected(self, fake_tmpdir):
        from tools.browser_tool_lifecycle import _managed_chrome_profile
    ...
    def test_unreadable_final_process_scan_preserves_socket(self, fake_tmpdir):
        ...
        with patch("psutil.process_iter", return_value=[_UnreadableProc()]):
            assert bt_lifecycle._remove_browser_socket_dir_if_safe(
    ...
    def test_global_scan_requires_dead_owner_metadata(self, fake_tmpdir):
        ...
        with patch("gateway.status._pid_exists", return_value=False), \
             patch.object(bt_lifecycle, "_reap_session_chromium",
                          side_effect=lambda *a, **k: calls.append((a, k)) or 0):
            bt_lifecycle._reap_orphaned_browser_chromes(min_age_seconds=120)
    ...
    def test_global_scan_preserves_live_owner(self, fake_tmpdir):
        ...
        with patch("gateway.status._pid_exists", return_value=True), \
             patch.object(bt_lifecycle, "_reap_session_chromium") as reap:
            bt_lifecycle._reap_orphaned_browser_chromes(min_age_seconds=120)
    ...
    def test_cleanup_thread_scans_chrome_before_daemon_with_timeout(self, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            bt_lifecycle, "_reap_orphaned_browser_chromes",
...
class TestLeakedDaemonWithLiveOwner:
    ...
    def test_idle_untracked_daemon_with_live_owner_is_reaped(self, fake_tmpdir):
        ...
        with patch("gateway.status._pid_exists", side_effect=[True, True, False]), \
             patch("tools.browser_tool_lifecycle._verify_reapable_browser_daemon", return_value=24680), \
             patch("tools.process_registry.ProcessRegistry._terminate_host_pid",
    ...
    def test_reaper_runs_on_every_interval_not_just_startup(self):
        ...
        try:
            with patch("tools.browser_tool_lifecycle._reap_orphaned_browser_sessions",
                 ...
                 patch("tools.browser_tool_lifecycle._reap_orphaned_browser_chromes",
