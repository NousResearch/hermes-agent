...
def test_discovery_view_requires_exact_read_only_hint_and_registration():
    ...
    read_only = {(None, READ_TOOL): "linear"}
...
def test_local_rpc_refuses_unexposed_raw_mcp_and_enforces_sub_budget():
    ...
    thread = threading.Thread(
        ...
        kwargs={
            ...
            "mcp_revalidate": lambda tool_name: tool_name == READ_TOOL,
        },
        daemon=True,
    )
    thread.start()
...
def test_persistent_kernel_revalidates_in_the_active_cells_context():
    from contextvars import ContextVar
    from types import SimpleNamespace

    from tools.code_kernel import CellAuthority, _rpc_forever

    profile = ContextVar("test_execute_code_mcp_profile", default="default")
    token = profile.set("profile-a")
    authority = CellAuthority("task")
    profile.reset(token)
    stop = threading.Event()
    ...
    def serve_once(*_args, **kwargs):
        observed.append(kwargs["mcp_revalidate"](READ_TOOL))
        stop.set()

    with patch("tools.code_execution_rpc._rpc_server_loop", side_effect=serve_once), patch(
        "tools.code_execution_tool._sandbox_mcp_tools",
        side_effect=lambda _tools: (
            frozenset({READ_TOOL}) if profile.get() == "profile-a" else frozenset()
        ),
    ):
        _rpc_forever(kernel, 50, frozenset({READ_TOOL}), frozenset({READ_TOOL}), 10)
