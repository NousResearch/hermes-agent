...
class TestCuaWindowTargeting:
    ...
    def test_metadata_exact_match_beats_weak_raw_prefix_match(self):
        backend = self._backend(
            on_screen_windows=[
                {
                    "app_name": "Browser Helper",
                    "pid": 100,
                    "window_id": 1,
                    "is_on_screen": True,
                    "z_index": 0,
    ...
    def test_bundle_id_prefix_does_not_select_arbitrary_sibling(self, monkeypatch):
        backend = self._backend(on_screen_windows=[
            {
                "app_name": "Safari",
        ...
        monkeypatch.setattr(backend, "_system_pids_for_bundle_id", lambda _bundle_id: [])

        assert backend._select_window("com.apple") is None

    def test_exact_bundle_id_outranks_app_name_collision(self):
        backend = self._backend(on_screen_windows=[
            {
        ...
        target = backend._select_window("com.apple.Safari")
    ...
    @pytest.mark.parametrize("operation", ["capture", "focus_app"])
    def test_explicit_selection_error_clears_all_previous_target_state(
        self, monkeypatch, operation,
    ):
        backend = self._backend(on_screen_windows=[])
        backend._active_pid = 111
        backend._active_window_id = 222
        backend._last_app = "StaleApp"
        backend._snapshot_tokens = {1: "stale-token"}

        def fail_enumeration(*_args, **_kwargs):
            raise RuntimeError("list_windows transport failed")

        monkeypatch.setattr(backend, "_list_windows", fail_enumeration)

        with pytest.raises(RuntimeError, match="list_windows transport failed"):
            if operation == "capture":
                backend.capture(mode="ax", app="Browser")
            else:
                backend.focus_app("Browser")
    ...
    def test_equal_app_matches_use_stable_frontmost_order(self):
        backend = self._backend(on_screen_windows=[
            {"app_name": "Browser", "pid": 37031, "window_id": 2, "title": "Docs", "is_on_screen": True, "z_index": 3},
            {"app_name": "Browser", "pid": 37031, "window_id": 1, "title": "Home", "is_on_screen": True, "z_index": 0},
        ])

        first = backend._select_window("Browser")
        second = backend._select_window("Browser")

        assert first is not None and second is not None
        assert first["window_id"] == second["window_id"] == 1

    def test_focus_app_and_capture_active_keep_exact_window_identity(self):
        backend = self._backend(on_screen_windows=[
            {"app_name": "Browser", "pid": 37031, "window_id": 1, "title": "", "is_on_screen": True, "z_index": 0},
            {"app_name": "Browser", "pid": 37031, "window_id": 2, "title": "Documentation", "is_on_screen": True, "z_index": 1},
        ])
        ...
        assert backend._last_app == "Browser"

        setattr(backend._session, "all_windows", [
            {"app_name": "Mail", "pid": 99, "window_id": 9, "title": "Inbox", "is_on_screen": True, "z_index": 0},
            {"app_name": "Browser", "pid": 37031, "window_id": 1, "title": "Home", "is_on_screen": True, "z_index": 1},
            {"app_name": "Browser", "pid": 37031, "window_id": 2, "title": "Documentation", "is_on_screen": False, "z_index": 9},
        ])

        cap = backend.capture_active(mode="ax")

        assert cap.app == "Browser"
        assert backend._active_pid == 37031
        assert backend._active_window_id == 2
