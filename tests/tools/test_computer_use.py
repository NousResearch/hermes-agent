...
class TestCuaDriverSessionReconnect:
    ...
    def test_cli_fallback_reads_screenshot_from_file(self, tmp_path, monkeypatch):
        ...
        class FakeProc:
            returncode = 0
            stderr = ""
            # Daemon returns a path, not inline base64. Serialise with
            # json.dumps: interpolating a Windows path into a JSON string
            # literal produces invalid escapes ("\U", "\b"), so the fixture
            # itself was unparseable there.
            stdout = json.dumps({
                "element_count": 7,
