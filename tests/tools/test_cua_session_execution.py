...
def test_registry_task_alias_uses_private_transport_without_moving_approvals(tmp_path, monkeypatch, session_id):
    ...
    with tempfile.TemporaryDirectory(prefix='hc-alias-') as runtime:
        ...
        token = set_current_session_key(approval_owner)
        try:
            ...
            denied = json.loads(registry.dispatch('computer_use', {'action': 'click', 'coordinate': [1, 1]},
                                                 session_id=session_id, task_id='transport-task'))
            assert denied['error'].startswith('BLOCKED'), denied
            assert log.read_text() == before, 'absent authorization must not send input'
            ...
            assert approval.is_approved(approval_owner, 'cua:click:background')
            assert not approval.is_approved(approval_owner, 'cua:click:foreground')
            assert not approval.is_approved('transport-owner', 'cua:click:background')
            assert not approval.is_approved('transport-task', 'cua:click:background')
            before = log.read_text()
            valid[0] = False
            ...
            approval.clear_session(approval_owner)
