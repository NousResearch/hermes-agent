...
from tools.async_delegation_formatter import (
    format_async_delegation as _format_async_delegation,
)
...
from tools.delegation_outcome import (
    ...
    delegation_schema_retry_allowed,
...
class TestExtractedOutcomePolicy(unittest.TestCase):
    def test_runtime_failure_cannot_authorize_schema_repair(self):
        self.assertFalse(
            delegation_schema_retry_allowed({
                "completed": True,
                "failed": True,
    ...
    def test_schema_verdict_is_separate_from_completed_lifecycle(self):
        classification = classify_delegation_result(
            {"completed": True, "final_response": "invalid"},
            aggregate_result={"completed": True},
            summary="invalid",
        ...
        self.assertEqual(classification.outcome, "failed")
        self.assertTrue(classification.schema_failed)
...
class TestRunSingleChildOutcome(unittest.TestCase):
    ...
    def test_budget_exhaustion_remains_partial_even_when_completed(self):
        entry = _shape({
            "final_response": "Budget ended after useful partial work.",
            "completed": True,
            "failed": False,
        ...
        self.assertEqual(entry["status"], "completed")
        self.assertEqual(entry["outcome"], "partial")
        self.assertEqual(entry["exit_reason"], "budget_exhausted")
    ...
    def test_unknown_turn_exit_reason_stays_unknown_with_usable_output(self):
        entry = _shape({
            "final_response": "Runtime ended without an authoritative reason.",
            "completed": False,
            "failed": False,
        ...
        self.assertEqual(entry["exit_reason"], "unknown")
...
class TestFormatterNeutralisation(unittest.TestCase):
    ...
    def test_legacy_schema_invalid_result_is_failed_with_evidence(self):
        result = {
            "task_index": 0,
            "status": "completed",
            "summary": "schema-invalid payload",
            "schema_valid": False,
        ...
        self.assertIn("schema_valid=false", out)
        self.assertIn("'city' is a required property", out)
