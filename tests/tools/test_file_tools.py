...
class TestWriteFileHandler:
    ...
    def test_writes_content(self, mock_get):
        ...
        from tools.file_tools import _resolve_path_for_task, write_file_tool
        result = json.loads(write_file_tool("/tmp/out.txt", "hello world!\n"))
        assert result["status"] == "ok"
        mock_ops.write_file.assert_called_once_with(
            str(_resolve_path_for_task("/tmp/out.txt")),
            "hello world!\n",
        )
...
class TestPatchHandler:
    ...
    def test_replace_mode_calls_patch_replace(self, mock_get):
        ...
        from tools.file_tools import _resolve_path_for_task, patch_tool
        ...
        mock_ops.patch_replace.assert_called_once_with(
            str(_resolve_path_for_task("/tmp/f.py")),
            "foo",
            "bar",
            False,
        )
