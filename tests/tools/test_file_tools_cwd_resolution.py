...
@pytest.mark.platforms("posix")  # symlink creation needs elevated privileges on Windows
...
@pytest.mark.platforms("windows")  # UNC path semantics are Windows-specific
...
def test_ssh_doubled_path_is_detected_in_the_remote_namespace(monkeypatch):
    root = PurePosixPath("/home/u/dev")
    monkeypatch.setattr(ftp, "_terminal_env_type_for_task", lambda task_id: "ssh")
    monkeypatch.setattr(
        ftp, "_resolve_ssh_path",
        lambda filepath, task_id: root if filepath == "." else root / filepath,
    )

    filepath = "home/u/dev/notes/x.md"
    resolved = ftp._resolve_path_for_task(filepath, task_id="default")
    warn = ftp._path_resolution_warning(filepath, resolved, task_id="default")

    assert warn is not None
    assert warn.startswith(ftp._DOUBLED_PATH_MARKER)
    assert "'/home/u/dev/notes/x.md'" in warn


def test_ssh_tilde_relative_root_is_not_checked_for_echo(monkeypatch):
    root = PurePosixPath("~/a/b")
    monkeypatch.setattr(ftp, "_terminal_env_type_for_task", lambda task_id: "ssh")
    monkeypatch.setattr(
        ftp, "_resolve_ssh_path",
        lambda filepath, task_id: root if filepath == "." else root / filepath,
    ...
    assert warn is None or not warn.startswith(ftp._DOUBLED_PATH_MARKER)
