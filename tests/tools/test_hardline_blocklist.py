...
_BARE_BRACE_REDIRECTION_OPERANDS = [
    "${x:-{}",
    '"${x:-{}"',
    "$(echo ${x:-{})",
    "${x:-${y:-{}}",
...
@pytest.mark.parametrize("operand", _BARE_BRACE_REDIRECTION_OPERANDS)
def test_bare_brace_redirection_reaches_dangerous_command(operand):
    assert detect_dangerous_command(f">{operand} python -c '1'")[0] is True


@pytest.mark.parametrize(
    "operand",
    [operand for operand in _BARE_BRACE_REDIRECTION_OPERANDS if "\\" not in operand],
)
def test_bare_brace_redirection_reaches_hardline_command(operand):
    assert detect_hardline_command(f">{operand} bash -c 'rm -rf /'")[0] is True


@pytest.mark.parametrize(
    ("command", "expected_end"),
    [
        (">${x:-{} true", 8),
        (">${x:-{a}b} true", 11),
    ],
)
def test_bare_brace_redirection_operand_boundaries(command, expected_end):
    assert approval_detection._scan_redirection_operand_end(command, 1) == (
        expected_end,
        "ok",
    )
