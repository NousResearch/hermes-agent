...
def test_complete_with_artifacts_lands_in_event_payload(worker_env, tmp_path):
    ...
    artifact_paths = [str(chart), str(report)]
    ...
    try:
        ...
        assert payload.get("artifacts") == artifact_paths
        # And the artifacts also live on metadata for downstream workers
        run = kb.latest_run(conn, worker_env)
        assert run is not None
        assert run.metadata.get("artifacts") == artifact_paths
    finally:
        conn.close()
...
def test_complete_rejects_missing_artifact_path_without_state_change(worker_env, tmp_path):
    """Claimed artifact paths must exist before completion mutates task state."""
    from hermes_cli import kanban_db as kb
    from tools import kanban_tools as kt

    missing = tmp_path / "missing-report.md"
    out = kt._handle_complete({
        "summary": "claimed a missing report",
...
def test_create_persists_model_override(worker_env):
    ...
    try:
        ...
        assert child.model_override == "gpt-5.4-mini"
    finally:
        conn.close()
...
def test_create_schema_exposes_model_override():
    ...
    assert "-m" in props["model_override"]["description"]
