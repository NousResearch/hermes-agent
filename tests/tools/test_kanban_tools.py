...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        _owner_kanban_claims.claim_task(conn, goal_task_id)
    finally:
        conn.close()
...
def test_create_explicit_scratch_ignores_ambient_board_project(
    ...
    _owner_kanban_boards.write_board_metadata("default", project_id=project_id)
    _owner_kanban_boards.create_board("target", name="Target", project_id=project_id if target_scoped else "")
...
def test_unblock_happy_path(monkeypatch, worker_env):
    ...
    try:
        ...
        _owner_kanban_db.block_task(conn, tid, reason="waiting")
    finally:
        conn.close()
...
def test_worker_unblock_rejects_foreign_task_id(worker_env):
    ...
    try:
        ...
        _owner_kanban_db.block_task(conn, other, reason="waiting")
    finally:
        conn.close()
...
def multi_board_env(monkeypatch, tmp_path):
    ...
    return {
        ...
        "alt_seed": seed_alt,
        "default_db": _owner_kanban_db.kanban_db_path(),
        "alt_db": _owner_kanban_db.kanban_db_path(board="alt"),
    }
...
def test_board_param_none_falls_back_to_env(worker_env):
    ...
    # forced.
    assert _owner_kanban_db.kanban_db_path() == _owner_kanban_db.kanban_db_path(board="default")
