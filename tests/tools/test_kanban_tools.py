...
def test_complete_zero_evidence_is_bounced_with_correction(worker_env, monkeypatch):
    """A worker run with no non-kanban tool evidence is refused with a
    correction naming what is missing; the task stays in-flight."""
    monkeypatch.setenv("HERMES_SESSION_ID", "sess-no-evidence")
    from tools import kanban_tools as kt
    monkeypatch.setattr(
        "tools.kanban_tools._run_tool_evidence", lambda _sid: 0,
    )
...
def test_complete_with_tool_evidence_is_accepted_unchanged(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setattr(
        "tools.kanban_tools._run_tool_evidence", lambda _sid: 7,
    )
    out = kt._handle_complete({
