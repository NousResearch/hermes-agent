...
def test_create_autosubscribes_current_gateway_source(worker_env):
    ...
    set_session_vars(
        ...
        user_id="12345",
        session_key="napcat:group:610066383",
    )
    try:
        out = kt._handle_create({
            "title": "qq child task",
    ...
    assert len(subs) == 1
    assert subs[0]["platform"] == "napcat"
    assert subs[0]["chat_id"] == "group:610066383"
    assert subs[0]["thread_id"] == ""
    assert subs[0]["user_id"] == "12345"
