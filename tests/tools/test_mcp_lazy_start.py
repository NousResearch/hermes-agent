...
class TestLazyMcpStatus:
    ...
    def test_lazy_server_reports_lazy_status_with_cached_tool_count(self):
        self._seed_lazy()
        with patch("tools.mcp_tool._load_mcp_config", return_value=dict(self._CONFIG)):
            [entry] = mcp.get_mcp_status()
...
class TestLazyDiscoverySummary:
    ...
    def _discover(self):
        with patch("tools.mcp_tool._load_mcp_config", return_value=dict(self._CONFIG)), \
             ...
             patch("tools.mcp_schema_cache.config_fingerprint", return_value="abc"), \
             patch("tools.mcp_schema_cache.get_cached_entry", return_value=_fake_cache_entry()), \
             patch("tools.mcp_tool._register_from_cache_sync", side_effect=self._fake_register_from_cache), \
             patch("tools.mcp_tool._discover_and_register_server", new_callable=AsyncMock) as mock_discover, \
             patch("tools.mcp_tool._ensure_mcp_loop"), patch("tools.mcp_tool._run_on_mcp_loop"):
