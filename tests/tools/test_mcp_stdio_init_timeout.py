...
class TestStdioStdoutNoise:
    def test_lazy_sdk_transport_detection_keeps_tolerant_stdio_identity(self):
        """The stdio seam composes with current HTTP/protocol SDK detection."""
        from mcp.client.stdio import stdio_client as sdk_stdio_client
        from tools import mcp_tool

        assert mcp_tool._ensure_mcp_sdk() is True
        assert mcp_tool._SDK_STDIO_CLIENT is sdk_stdio_client
        assert mcp_tool._MCP_HTTP_AVAILABLE is (
            mcp_tool._MCP_NEW_HTTP or mcp_tool._MCP_LEGACY_HTTP
        )
        assert mcp_tool.LATEST_HANDSHAKE_VERSION
