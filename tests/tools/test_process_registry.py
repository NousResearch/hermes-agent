...
class TestReaderLoopWindowsPeekBranch:
    """Pins the PeekNamedPipe loop's cadence with fake msvcrt, _winapi,
    and time on a Windows host."""
    ...
    def test_finish_request_stops_one_window_while_child_alive(
        self, registry, monkeypatch, tmp_path
    ):
        """Alive child, peek stuck at 0; the first fake sleep sets the finish request."""
        sleeps = self._patch_windows(monkeypatch, [(0, 0)], tmp_path)
        import tools.process_registry as pr_mod
        proc = MagicMock()
        proc.stdout = _FakeWinStdout([])
        proc.poll.return_value = None
        proc.wait.return_value = proc.returncode = 0
        s = _make_session(sid="proc_win_peek_finish")
        s.process, s.notify_on_complete = proc, True
        registry._running[s.id] = s
        proxy = pr_mod.time
        orig = proxy.sleep
        ...
        def _sleep(seconds):
            if not proxy._log:
                s._reader_finish_requested.set()
            orig(seconds)

        monkeypatch.setattr(proxy, "sleep", _sleep)
        done = threading.Event()
        threading.Thread(
        ...
        assert proc.stdout.read1_calls == 0
        assert sleeps == [0.2]
        got = registry.completion_queue.get_nowait()
        assert (
            got["type"] == "completion"
            and got["session_id"] == s.id
            and registry.completion_queue.empty()
        )
