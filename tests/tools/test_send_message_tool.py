...
class TestSendMessageTool:
    ...
    def test_media_tag_outside_allowed_roots_is_not_sent(self, tmp_path, monkeypatch):
        ...
        assert result["media_dropped"] == [{
            "path": str(secret),
            "reason": "denied by the delivery policy",
        }]
        send_mock.assert_awaited_once_with(
            Platform.TELEGRAM,
            telegram_cfg,
    ...
    def test_partial_media_drop_reports_incomplete_delivery(self, tmp_path, monkeypatch):
        ...
        send_mock.assert_awaited_once_with(
            ...
            "report",
            thread_id=None,
            media_files=[(str(report.resolve()), False)],
            force_document=False,
        )
