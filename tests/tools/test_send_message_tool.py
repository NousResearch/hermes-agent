...
class TestParseTargetRefSlack:
    ...
    def test_channel_with_thread_ts_is_parsed(self):
        """slack:CHANNEL_ID:THREAD_TS routes to the explicit channel in the given thread."""
        chat_id, thread_id, is_explicit = _parse_target_ref("slack", "C0B0GV4FKK7:1778181769.602679")
        assert chat_id == "C0B0GV4FKK7"
        assert thread_id == "1778181769.602679"
        assert is_explicit is True

    def test_bare_channel_still_works_after_thread_regex(self):
        """Bare channel ID (no thread_ts) still returns thread_id=None."""
        chat_id, thread_id, is_explicit = _parse_target_ref("slack", "C0B0GV4FKK7")
        assert chat_id == "C0B0GV4FKK7"
        assert thread_id is None
        assert is_explicit is True
