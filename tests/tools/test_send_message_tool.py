...
def test_schema_documents_napcat_cross_chat_targets():
    target_desc = SEND_MESSAGE_SCHEMA["parameters"]["properties"]["target"]["description"]

    assert "napcat:direct:<qq_id>" in target_desc
    assert "napcat:group:<group_id>" in target_desc


def test_qq_send_message_schema_is_direct_for_napcat_cross_chat_sends():
    assert QQ_SEND_MESSAGE_SCHEMA["name"] == "qq_send_message"
    desc = QQ_SEND_MESSAGE_SCHEMA["description"]

    assert "QQ/NapCat" in desc
    assert "cross-chat" in desc
    assert "must use this tool" in desc

    props = QQ_SEND_MESSAGE_SCHEMA["parameters"]["properties"]
    assert props["target_type"]["enum"] == ["group", "direct"]
    assert "QQ group id" in props["target_id"]["description"]
    assert "private QQ user id" in props["target_id"]["description"]
    assert "MEDIA:/" in props["message"]["description"]


def test_qq_send_message_tool_delegates_group_send_to_napcat_target(monkeypatch):
    calls = []

    def fake_send_message(args, **kw):
        calls.append(args)
        assert kw == {"prefer_standalone": True}
        return json.dumps({"success": True, "target": args["target"]})
    ...
    assert calls == [
        {
            "action": "send",
            "target": "napcat:group:610066383",
            "message": "早上好",
        }
    ]


def test_qq_send_message_tool_rejects_send_without_structured_target():
    result = json.loads(
        qq_send_message_tool(
            {
                "action": "send",
...
def test_qq_send_message_tool_allows_media_payloads_for_cross_chat(monkeypatch):
    calls = []
        ...
        qq_send_message_tool(
            {
                ...
                "target_type": "group",
                "target_id": "610066383",
                "message": "MEDIA:/tmp/hermes-cache/voice.ogg",
            }
        )
    )
...
class TestSendViaAdapterStandaloneFallback:
    ...
    @pytest.mark.asyncio
    async def test_live_adapter_media_send_runs_on_gateway_loop(self, monkeypatch):
        """Live plugin adapter sends must execute on the gateway loop.

        Agent tools run inside a worker thread with their own event loop; NapCat's
        websocket belongs to the gateway loop, so direct awaits from the tool loop
        can fail with "Future attached to a different loop".
        """
        ...
        assert adapter.voice_loop is gateway_loop
