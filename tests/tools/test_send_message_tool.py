...
class TestSendTelegramRichMessages:
    ...
    ``sendRichMessage`` when supported and fall back to legacy MarkdownV2
    on capability / permanent errors. This keeps cron and tool replies
    aligned with the in-gateway adapter's rendering.
    """
    ...
    def test_rich_message_includes_thread_id_when_set(self, monkeypatch):
        ...
        asyncio.run(
            _send_telegram("tok", "123", "hi", thread_id="17585")
        )
    ...
    def test_rich_message_omits_none_thread_id(self, monkeypatch):
        ...
        asyncio.run(
            _send_telegram("tok", "123", "hi", thread_id="1")
        )
    ...
    def test_html_payload_skips_rich_path(self, monkeypatch):
        ...
        asyncio.run(_send_telegram("tok", "123", "<b>bold</b>"))
    ...
    def test_media_payload_skips_rich_path(self, monkeypatch, tmp_path):
        ...
        asyncio.run(
            _send_telegram("tok", "123", "caption", media_files=[(str(img), False)])
        )
