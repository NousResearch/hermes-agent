...
# --- ROLLBACK-UNDER-LOCK ---------------------------------------------------
# Minimal additional witness for the security-scan rollback-under-lock
# contract surface. The donor focal suite covers the lock primitive
# exhaustively but does NOT exercise the rollback path: when
# ``_security_scan_skill`` blocks a skill, ``_create_skill`` must run
# ``shutil.rmtree(skill_dir)`` BEFORE releasing the canonical-name lock.
# A concurrent acquirer that fires during rollback must observe
# CONTENTION — proving the rollback ran while the lock was still held.
#
# This witness was added because the donor focal leaves the rollback-under-lock
# surface uncovered, and the gap is exactly the integration the A1G contract
# is supposed to close (no half-deleted state visible to a concurrent
# publisher). It does not modify the lock module or skill_manager_tool.

@pytest.mark.parametrize("contention_errno", [errno.EWOULDBLOCK, errno.EAGAIN])
def test_rollback_under_lock_holds_through_shutil_rmtree(
    monkeypatch, tmp_path, contention_errno,
):
    """The shutil.rmtree rollback after a blocked security scan MUST run while
    the canonical-name publication lock is still held.

    Mechanism:
      - monkeypatch ``_security_scan_skill`` on tools.skill_manager_tool to
        return a blocking error string (forces the rollback path);
      - monkeypatch ``tools.skill_manager_tool.shutil.rmtree`` to record
        that it was called AND to attempt a NON-BLOCKING acquire of the
        SAME canonical-name lock from inside the rollback. If the rollback
        ran under the lock, that inner acquire MUST fail with CONTENTION;
        if the rollback ran AFTER release, it would succeed.
      - call ``skill_manage(action="create", ...)``; the create must fail
        with the scan error, the rmtree must have been called, and the inner
        acquire must have observed CONTENTION.
    """
    import shutil as real_shutil
    from tools import skill_manager_tool as smt
    from tools.skill_publish_guard import (
        LOCK_KIND_CONTENTION, live_skill_publish_guard,
    )
    from tools.skill_manager_tool import skill_manage

    hermes_home = Path(os.environ.get("HERMES_HOME") or str(tmp_path))
    hermes_home.mkdir(parents=True, exist_ok=True)
    (hermes_home / "skills").mkdir(parents=True, exist_ok=True)

    rollback_observations = {}

    def _blocking_scan(skill_dir):
        return "Security scan blocked this skill (synthetic A1G witness)"

    original_rmtree = real_shutil.rmtree

    def _probing_rmtree(path, *args, **kwargs):
        # We're INSIDE the canonical-name lock window now. Try to acquire
        # the same canonical name non-blockingly; if the lock is held by
        # the outer create, this MUST fail with CONTENTION.
        with live_skill_publish_guard(_SKILL_NAME, target=hermes_home):
            pass  # never raises CONTENTION here — the inner acquire succeeded,
                  # which means the outer lock had already been released by
                  # the time rmtree ran. The rollback-under-lock contract
                  # is BROKEN.
        original_rmtree(path, *args, **kwargs)

    # Better approach: catch the CONTENTION via the explicit SkillPublishLockError
    def _strict_probing_rmtree(path, *args, **kwargs):
        observed = {"contention_observed": False, "rmtree_called": True}
        try:
            with live_skill_publish_guard(_SKILL_NAME, target=hermes_home):
                # If we got here, the outer lock had ALREADY been released.
                # Record the violation.
                observed["contention_observed"] = False
                observed["lock_was_released"] = True
        except Exception as exc:  # SkillPublishLockError is the success path
            from tools.skill_publish_guard import SkillPublishLockError
            if isinstance(exc, SkillPublishLockError) and exc.kind == LOCK_KIND_CONTENTION:
                observed["contention_observed"] = True
                observed["lock_was_released"] = False
            else:
                raise
        rollback_observations.update(observed)
        # Always run the real rmtree so the test cleans up.
        try:
            original_rmtree(path, *args, **kwargs)
        except Exception:
            pass

    monkeypatch.setattr(smt, "_security_scan_skill", _blocking_scan)
    monkeypatch.setattr(smt.shutil, "rmtree", _strict_probing_rmtree)

    result_json = skill_manage(
        action="create", name=_SKILL_NAME,
        content=_SKILL_CONTENT, category=None,
    )
    result = json.loads(result_json)

    assert result.get("success") is False, (
        f"rollback-under-lock witness: create must fail (scan blocked), got {result!r}"
    )
    assert "scan blocked" in result.get("error", "").lower(), (
        f"rollback-under-lock witness: failure must cite the scan, got {result!r}"
    )
    assert rollback_observations.get("rmtree_called") is True, (
        f"rollback-under-lock witness: shutil.rmtree was NOT called after scan block; "
        f"observations={rollback_observations!r}"
    )
    assert rollback_observations.get("contention_observed") is True, (
        f"rollback-under-lock witness: rmtree ran OUTSIDE the canonical-name lock; "
        f"a concurrent acquirer would see the half-deleted state. "
        f"observations={rollback_observations!r}"
    )


# --- EXTERNAL-ROOT CREATE AUTHORITY REGRESSION ------------------------------
# Focused regression witness for the T4-R3A finding:
# _validate_publish_target must NOT grant CREATE authority over roots that
# get_external_skills_dirs() returns. External skills are by policy READ-ONLY
# to the agent — only `_find_skill` may use them for duplicate lookup, never
# `_create_skill`. Pre-repair, the validator extended its accepted-roots set
# with get_external_skills_dirs() entries, allowing it to silently accept a
# publication target beneath an external root. Post-repair, the validator's
# accepted-roots set must equal the set of bases `_resolve_skill_dir` can
# return ({_skills_dir(), get_skill_create_dir()}), and this witness must
# PASS by refusing the external-root target with a publication-refused
# error.
#
# Mechanism:
#   - construct a tmp external root, distinct from _skills_dir() and
#     get_skill_create_dir() (which are None in the test sandbox);
#   - patch the SOURCE module agent.skill_utils (the validator lazily
#     imports get_external_skills_dirs from there, so the guards-module
#     binding is not authoritative);
#   - call the REAL _validate_publish_target with a target inside the
#     external root;
#   - assert the helper REFUSES (returns a non-None error string).

def test_external_root_rejected_for_create_publication(monkeypatch, tmp_path):
    """A target beneath a root returned by get_external_skills_dirs() MUST be
    refused by _validate_publish_target. External skills are read-only to the
    agent; the validator's accepted-root set must NOT silently authorize
    publishes into them."""
    from tools import skill_manager_guards as guards
    from tools.skill_manager_guards import _validate_publish_target
    from tools import skill_manager_tool as _smt
    ...
    # External root distinct from _skills_dir() and get_skill_create_dir().
    external_root = tmp_path / "external-skills-root"
    external_root.mkdir(parents=True, exist_ok=True)
    target_skill_dir = external_root / "external-only-skill"

    # The validator lazily imports get_external_skills_dirs from
    # agent.skill_utils INSIDE the function body (line 144), so patching the
    # guards-module binding is a no-op. Patch the SOURCE module directly.
    monkeypatch.setattr(su, "get_external_skills_dirs", lambda: [external_root])

    # Ensure get_skill_create_dir returns None so the validator does not
    # accept the target under the create_dir root (the discriminator).
    monkeypatch.setattr(su, "get_skill_create_dir", lambda: None, raising=False)
    # Clear cached config so the patched functions take effect.
    if hasattr(su, "_external_dirs_cache_clear"):
        su._external_dirs_cache_clear()
    if hasattr(su, "_raw_config_cache_clear"):
        su._raw_config_cache_clear()

    # Sanity guard: external root must be distinct from _skills_dir().
    assert external_root.resolve() != _smt._skills_dir().resolve(), (
        f"external root must be distinct from _skills_dir(); both resolved to "
        f"{external_root.resolve()}"
    )

    result = _validate_publish_target(target_skill_dir)

    assert result is not None, (
        f"external-root CREATE authority regression: _validate_publish_target "
        f"returned None for target beneath an external-skills root "
        f"({target_skill_dir}); the validator silently authorized a publish "
        f"into a read-only external skill tree. "
        f"external_root={external_root} _skills_dir()={_smt._skills_dir()}"
    )
    assert "outside" in str(result).lower() or "refus" in str(result).lower(), (
        f"external-root CREATE authority regression: refusal wording must be "
        f"diagnostic; got {result!r}"
    )
