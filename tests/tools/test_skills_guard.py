...
class TestFalsePositiveReductions:
    ...
    def test_python_secrets_read_in_documented_example_is_capped(self, tmp_path):
        # Review follow-up: py_read_secrets_file is the Python twin of js_read_secrets_file —
        # same local-read family — so a documented-example bullet quoting
        # `open('~/.aws/credentials')` caps one step like its shell/JS siblings.
        f = tmp_path / "antipatterns.md"
        f.write_text("- `open('~/.aws/credentials')` — never read real credentials\n", encoding="utf-8")
        findings = scan_file(f, "antipatterns.md")
        py_reads = [fi for fi in findings if fi.pattern_id == "py_read_secrets_file"]
        assert py_reads and all(fi.severity == "high" for fi in py_reads)
        assert all("documented example" in fi.description for fi in py_reads)

    def test_documented_example_cap_not_applied_inside_code_blocks(self, tmp_path):
        # A fenced block — plain or inside a blockquote/bullet container — is embedded code,
        # same as a .sh file: an anti-pattern bullet INSIDE it keeps full severity, so a
        # malicious skill cannot earn the cap by moving its instructions into a fenced list.
        fenced = tmp_path / "fenced.md"
        fenced.write_text(
            "```sh\n"
            "- `cat .env.example` — executable content despite the bullet shape\n"
            "```\n", encoding="utf-8")
        quoted_fence = tmp_path / "quoted-fence.md"
        quoted_fence.write_text(
            "> ```sh\n"
            "> - `cat .env.example` — blockquoted fence\n"
            "> ```\n", encoding="utf-8")
        for path in (fenced, quoted_fence):
            reads = [fi for fi in scan_file(path, path.name) if fi.pattern_id == "read_secrets_file"]
            assert reads and all(fi.severity == "critical" for fi in reads), path.name
