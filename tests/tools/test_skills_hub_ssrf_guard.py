...
def test_source_fetch_validates_redirect_with_real_url_policy(monkeypatch, tmp_path):
    from tools.skills_hub_models import _get_text
    ...
    monkeypatch.setattr("tools.url_safety.create_ssrf_safe_client", client)
    assert _get_text("https://93.184.216.34/skill.md") is None
    assert requests == ["https://93.184.216.34/skill.md"]
...
def test_guarded_http_get_blocks_redirect_to_metadata_ip():
    ...
    with patch("tools.skills_hub._ssrf_safe_http_get", side_effect=fake_get):
