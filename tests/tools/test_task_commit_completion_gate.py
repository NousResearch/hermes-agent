...
def test_create_persists_completion_gate():
    result = _call(
        ...
        completion_gate="verify-gate",
    )

    assert result["success"] is True
    assert result["goal"]["completion_gate"] == "verify-gate"
    assert GoalManager("s").state.completion_gate == "verify-gate"


def test_amend_preserves_gate_when_omitted_and_can_change_it():
    _call("create", objective="first", completion_gate="verify-gate")

    preserved = _call("amend")
    assert preserved["goal"]["completion_gate"] == "verify-gate"
