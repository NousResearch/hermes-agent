...
class TestNonFileWriteConsumersUntouched:
    ...
    def test_sibling_file_write_does_not_tar_the_interpreter_heredoc(self):
        # The heredoc belongs to `python3`; the `> a.txt` belongs to the earlier `cat`
        # command, which owns no heredoc. Attributing the heredoc from the start of the
        # command unit would sweep in `cat > a.txt` and -- since the interpreter regex is
        # anchored -- fail to recognise python as the consumer, wrongly blocking this.
        cmd = "cat > a.txt; python3 <<EOF" + NL + "print(1)" + NL + "EOF"
        assert detect_unquoted_heredoc_file_write(cmd) is None

    def test_sibling_redirect_then_real_file_write_is_still_blocked(self):
        # The mirror image: the heredoc really does feed `cat > out.ts`, so it must flag.
        cmd = "echo hi; cat > out.ts <<EOF" + NL + "title: ok" + NL + "EOF"
        result = detect_unquoted_heredoc_file_write(cmd)
        assert result is not None
        assert "write_file" in result
