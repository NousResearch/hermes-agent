...
class TestBridgeDispatch:
    ...
    def test_tool_search_bounds_match_descriptions(self):
        """tools.tool_search.max_description_chars caps each hit's description in the
        shared ``tools`` map (full text stays reachable through tool_describe)."""
        from tools.registry import registry
        from tools.tool_search import ToolSearchConfig, dispatch_tool_search

        name = "mcp_test_long_description"
        tool_def = _td(name, "long description " + "x" * 500)
        registry.register(
            name=name,
            handler=lambda args, **kwargs: "{}",
            schema=tool_def,
        ...
        result = json.loads(dispatch_tool_search(
            {"queries": ["long description"]},
            current_tool_defs=[tool_def],
            config=ToolSearchConfig.from_raw({"max_description_chars": 120}),
        ))
        [group] = result["results"]
        assert group["matches"] == [name]
        # 120 chars of text + the "…" marking the cut (upstream's _clip_description).
        assert result["tools"][name]["description"].endswith("…")
        assert len(result["tools"][name]["description"]) == 121
