...
class TestRemoteAudioUrl:
    def test_groq_remote_url_resolves_redirect_before_transcribing(self, monkeypatch):
        ...
        with patch("tools.transcription_tools._load_stt_config", return_value={"provider": "groq"}), \
             patch("tools.transcription_tools._HAS_OPENAI", True), \
             patch("tools.transcription_remote.is_safe_url", return_value=True), \
    ...
    def test_remote_audio_probe_blocks_private_initial_url(self):
        from tools.transcription_remote import _probe_remote_audio_url

        with patch("tools.transcription_remote.is_safe_url", return_value=False):
    ...
    def test_remote_audio_probe_blocks_redirect_to_private_url(self):
        ...
        with patch("tools.transcription_remote.is_safe_url", side_effect=fake_is_safe_url), \
             patch("requests.Session", return_value=fake_session):
            from tools.transcription_remote import _probe_remote_audio_url
    ...
    def test_remote_audio_limits_use_config_values(self):
        from tools.transcription_remote import MAX_FILE_SIZE, _remote_audio_limits
    ...
    def test_remote_audio_limits_ignore_env_overrides(self, monkeypatch):
        ...
        from tools.transcription_remote import (
    ...
    def test_ffprobe_duration_uses_subprocess_safeguards(self):
        ...
        with patch("tools.transcription_remote._find_ffprobe_binary", return_value="/usr/bin/ffprobe"), \
             patch("tools.transcription_remote.windows_hide_flags", return_value=0), \
             patch("tools.transcription_remote.subprocess.run", side_effect=fake_run):
            from tools.transcription_remote import _probe_audio_duration_for_chunk
    ...
    def test_ffmpeg_split_uses_subprocess_safeguards(self, tmp_path):
        from tools.transcription_remote import MAX_FILE_SIZE, _split_audio_file_for_stt
        ...
        with patch("tools.transcription_remote._find_ffmpeg_binary", return_value="/usr/bin/ffmpeg"), \
             patch("tools.transcription_remote._probe_audio_duration_for_chunk", return_value=(60.0, None)), \
             patch("tools.transcription_remote.windows_hide_flags", return_value=0), \
             patch("tools.transcription_remote.subprocess.run", side_effect=fake_run):
    ...
    def test_local_file_path_keeps_existing_file_upload_path(self, sample_ogg):
        with patch("tools.transcription_tools._load_stt_config", return_value={"provider": "groq"}), \
             ...
             patch("tools.transcription_remote._probe_remote_audio_url") as mock_probe, \
             patch(
    ...
    def test_large_remote_audio_uses_chunk_fallback(self, tmp_path):
        from tools.transcription_remote import MAX_FILE_SIZE
        ...
        with patch("tools.transcription_tools._load_stt_config", return_value={"provider": "groq"}), \
             patch("tools.transcription_tools._get_provider", return_value="groq"), \
             patch("tools.transcription_remote._probe_remote_audio_url", return_value=probe), \
             patch("tools.transcription_remote._download_remote_audio", return_value=(str(big_audio), None)), \
             patch("tools.transcription_remote._transcribe_groq_remote_url") as mock_direct_url, \
             patch(
                 "tools.transcription_remote._transcribe_audio_file_chunks",
                 return_value={
                     "success": True,
