...
class TestSharedGateKnownBlindModel:
    """The shared gate (`_accepts_tool_result_images`) is the one both native lanes use —
    the ``vision_analyze`` fast path and the ``computer_use`` capture route. The MODEL must
    be decisive there: a known-blind model routed through a tool-media-capable PROVIDER
    used to open the native route, and the image died upstream with
    ``404 No endpoints found that support image input``.
    """

    def test_known_blind_model_is_a_hard_stop(self):
        ...
        with patch("agent.image_routing._lookup_supports_vision", return_value=False), \
            ...
            assert _accepts_tool_result_images("openrouter", "deepseek/deepseek-chat", cfg) is False

    def test_known_blind_model_routes_computer_use_capture_to_aux(self):
        ...
        with patch("agent.image_routing._lookup_supports_vision", return_value=False), \
            ...
            assert vision_routing.should_route_capture_to_aux_vision(
                "openrouter", "deepseek/deepseek-chat", cfg) is True

    def test_catalog_vision_model_still_opens_the_gate(self):
        from tools.vision_tools import _accepts_tool_result_images
        ...
        with patch("agent.image_routing._lookup_supports_vision", return_value=True), \
             ...
             patch("tools.vision_tools._profile_rejects_tool_media", return_value=False):
            assert _accepts_tool_result_images("openrouter", "google/gemma-4-31b-it", cfg) is True
