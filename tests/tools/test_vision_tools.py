...
class TestVisionSafetyGuards:
    ...
    @pytest.mark.asyncio
    async def test_native_fast_path_local_env_file_blocked_via_read_guard(
        self, tmp_path
    ):
...
class TestMsysPathTranslation:
    ...
    async def test_msys_path_translated_to_windows_path(self, tmp_path):
        ...
        with (
            patch("tools.image_source.sys.platform", "win32"),
            patch(
    ...
    async def test_msys_path_skipped_on_posix(self, tmp_path):
        ...
        with (
            patch("tools.image_source.sys.platform", "linux"),
            patch(
                "tools.vision_tools._image_to_base64_data_url",
                return_value="data:image/png;base64,abc",
            ),
            ...
            result = await vision_analyze_tool(str(img), "describe this", "test/model")
            data = json.loads(result)
            assert data["success"] is True
            assert data["analysis"] == "POSIX local file"
