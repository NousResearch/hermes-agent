...
def test_marker_records_its_writer(tmp_path):
    record_turn_start(tmp_path, "abc", "fix the bug")

    marker = read_turn_marker(tmp_path, "abc")

    assert marker["writer_pid"] == os.getpid()
    assert isinstance(marker["writer_start_time"], float)


def test_marker_writer_state_reads_own_pid_as_alive(tmp_path):
    record_turn_start(tmp_path, "abc", "prompt")

    assert marker_writer_state(read_turn_marker(tmp_path, "abc")) == "alive"


def test_marker_writer_state_reads_an_exited_writer_as_dead():
    child = subprocess.Popen([sys.executable, "-c", "pass"])
    child.wait(timeout=10)

    assert marker_writer_state({"writer_pid": child.pid}) == "dead"
...
def test_marker_writer_state_without_a_writer_is_unknown():
    assert marker_writer_state({}) == "unknown"
    assert marker_writer_state({"writer_pid": None}) == "unknown"
    assert marker_writer_state({"writer_pid": "not-a-pid"}) == "unknown"
