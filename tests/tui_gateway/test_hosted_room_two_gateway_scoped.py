...
async def test_in_process_scoped_transport_contract_finishes_headlessly(
    ...
    agent = MagicMock()
    agent.run_conversation.return_value = {"final_response": "Scoped peer response."}
    ...
    with patch.object(target, "_create_agent", return_value=agent):
        ...
        manifest = [{key: stored[key] for key in ("attachment_id", "kind", "name", "size", "mime")}]
        home.send(
            room_id="room-1",
            event_id="user-1",
            payload={
        ...
        while asyncio.get_running_loop().time() < deadline:
            if any(event["kind"] == "message.member" for event in home._events("room-1")):
                break
    ...
    reply = next(event for event in home._events("room-1") if event["kind"] == "message.member")
