...
def _complete(tid: str, summary: str = "all done") -> None:
    conn = kbc.connect()
    try:
        _owner_kanban_completion.complete_task(conn, tid, summary=summary)
    finally:
        conn.close()
...
class TestCollectKanbanNotifications:
    def test_zero_sub_board_is_never_opened_writable(self):
        conn = kbc.connect()
        conn.close()
        _owner_kanban_boards.create_board("second-board")
    ...
    def test_done_reopen_notifies_once_per_event_until_archive(self):
        ...
        try:
            ...
            assert _owner_kanban_completion.complete_task(conn, tid, summary="review corrections")
        finally:
            conn.close()
        ...
        conn = kbc.connect()
        try:
            assert _owner_kanban_transitions.archive_task(conn, tid)
        finally:
            conn.close()
    ...
    def test_matching_tui_sub_delivers_and_advances_cursor(self):
        ...
        conn = kbc.connect()
        try:
            _owner_kanban_db.block_task(conn, tid, reason="waiting on review")
        finally:
            conn.close()
