...
def test_set_api_key_rejects_portable_only_name(hermes_root, monkeypatch):
    ...
    ``mcp_rpc_helpers.server_configs_with_sources()`` view (issue #87253) but
    must not be writable here — it has no native config.yaml entry to update,
    and unconditionally writing one would create a stale, credential-bearing
    shadow that permanently shadows the live portable config under
    native-wins precedence."""
    import hermes_cli.plugins as plugins_mod

    root = hermes_root
    portable_name = "supabase"
    ...
    monkeypatch.setattr(
        ...
        lambda: type(
            ...
            (),
            {
                ...
                "get_portable_mcp_server_plugins": lambda self: {portable_name: "supabase-plugin"},
            },
        )(),
    )
