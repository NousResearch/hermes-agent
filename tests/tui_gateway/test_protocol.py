...
def test_session_resume_returns_hydrated_messages(server, monkeypatch):
    ...
    monkeypatch.setattr(server, "_make_agent", lambda sid, key, session_id=None, session_db=None, **_kw: object())
...
def test_session_resume_live_payload_uses_current_history_with_ancestors(server, monkeypatch):
    ...
    monkeypatch.setattr(
        server,
        "_make_agent",
        lambda _sid, key, session_id=None, session_db=None, **_kw: types.SimpleNamespace(
