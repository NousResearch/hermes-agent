...
def test_model_save_key_refreshes_a_stale_setup_record(monkeypatch):
    """A key saved from the gated picker must move the boot record too, or the
    chat stays "need setup" (setup.status answers from the record) until the
    serve process restarts."""
    from hermes_cli import free_tier_bootstrap as fb

    fb.reset_for_tests()
    stale = fb.SetupRecord(
        provider_configured=False, inference_provider="", free_tier=False,
    ...
    retried = []
    monkeypatch.setattr(
        fb, "retry_bootstrap_mint", lambda **kw: retried.append(kw) or stale
    )
    monkeypatch.setattr(
        "hermes_cli.auth.PROVIDER_REGISTRY",
        {"test-provider": types.SimpleNamespace(
            name="Test Provider", auth_type="api_key",
            api_key_env_vars=("TEST_STALE_RECORD_KEY",))})
    monkeypatch.setattr("hermes_cli.config.is_managed", lambda: False)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "hermes_cli.inventory.build_models_payload",
        Mock(return_value={"providers": [{"slug": "test-provider", "models": [], "total_models": 0}]}))

    try:
        resp = server._methods["model.save_key"](
            104, {"slug": "test-provider", "api_key": "k-1"})

        assert "result" in resp, resp
        assert retried and retried[0].get("force") is True
    finally:
        fb.reset_for_tests()


def test_model_save_key_leaves_a_profile_scoped_record_alone(monkeypatch):
    """The record belongs to the launch profile; a profile-scoped save must not
    rewrite it with another home's inventory."""
    from hermes_cli import free_tier_bootstrap as fb

    fb.reset_for_tests()
    with fb._lock:
        fb._record = fb.SetupRecord(
            provider_configured=False, inference_provider="", free_tier=False,
            has_identity=False, other_providers=False)
        fb._started = True
        fb._done.set()
    retried = []
    monkeypatch.setattr(fb, "retry_bootstrap_mint", lambda **kw: retried.append(kw))
    monkeypatch.setattr(
        "hermes_cli.auth.PROVIDER_REGISTRY",
        {"test-provider": types.SimpleNamespace(
            name="Test Provider", auth_type="api_key",
            api_key_env_vars=("TEST_PROFILE_SCOPE_KEY",))})
    monkeypatch.setattr("hermes_cli.config.is_managed", lambda: False)
    monkeypatch.setattr(
        "hermes_cli.credential_lifecycle.save_provider_env_credential", Mock())
    monkeypatch.setattr(server, "_model_picker_context", Mock(return_value=object()))
    ...
    try:
        resp = server._methods["model.save_key"](
            105, {"slug": "test-provider", "api_key": "k-2", "profile": "other"})
