...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch, personal):
    ...
    # person.admission takes the private single-peer route (it must never ride a
    # session's fan-out to every attached viewer), so record that sink too.
    monkeypatch.setattr(
        server,
        "_emit_private_live",
        lambda event, sid, payload=None, peer=None: emitted.append((event, payload)),
    )
    monkeypatch.setitem(
        sys.modules,
        "tools.voice_mode",
