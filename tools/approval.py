...
_approval_listeners: list = []  # global listeners: [callable(session_key, approval_data), ...]


def register_approval_listener(cb) -> None:
    """Register a global listener called whenever an approval is queued.

    Signature: ``cb(session_key: str, approval_data: dict) -> None``.
    Called inside the approval lock — listeners must not block or acquire
    the lock themselves.  Runs in the agent thread.
    """
    _approval_listeners.append(cb)
...
def check_all_command_guards(
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            for _listener in _approval_listeners:
                try:
                    _listener(session_key, approval_data)
                except Exception:
                    pass
