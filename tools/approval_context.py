...
def _tirith_fail_open() -> bool:
    """``security.tirith_fail_open`` / ``TIRITH_FAIL_OPEN`` (default True; True when config is
    unreadable or tirith is disabled). False means the operator opted into fail-closed: an
    un-importable scanner must not silently grant access."""
