...
def detect_hardline_command(command: str) -> tuple:
    ...
    if _command_parser_limit_exceeded(command):
        # the separator-free size ceiling exists to stop one giant regex token from starving
        # every legacy pattern. A long payload whose QUOTED spans are inert data (a multi-line
        # python -c program, a long grep pattern) parses in bounded pieces; only the unquoted
        # skeleton can blow up the matchers. Measure the skeleton (quotes masked to single
        # spaces) instead of the raw length, so legitimate one-liners up to the overall
        # command ceiling are inspected rather than blocked.
        if not _unquoted_skeleton_within_limit(command):
            return (True, _PARSER_LIMIT_DESCRIPTION)
    normalized = _normalize_command_for_detection(command)
    _, malformed_grep = _grep_safe_detection_variant(normalized)
    if malformed_grep:
        # the malformed verdict is only trusted when the RAW command (author-written escapes
        # intact) is also unlexable. Normalization's global backslash-strip flips quote parity
        # on legal grep/sed patterns ([^\"]*, a\|b) and manufactured this verdict on 30+
        # read-only diagnostic commands (Aug 2026 incidents). Genuinely malformed authoring
        # still fails both probes and stays blocked.
...
# skeleton-size probe for the separator-free ceiling. A `python3 -c "<4KB program>"` with zero
# ;&| outside quotes is a legitimate one-liner, not a regex-starvation bomb — its QUOTED payload
# is inert data. Replace every quoted span with a single space and re-measure: only the unquoted
# skeleton counts against the ceiling.
...
# heredoc data-span locator. Stream-write rules (`tee`, `>`/`>>` redirection) use `.*` under
# re.DOTALL, which walks across heredoc bodies: `tee /safe/report.md << 'EOF'` … `report text
# mentioning config.yaml` … `EOF` matched the "overwrite project env/config via tee" pattern
# 131 chars past the actual tee target — the trigger lived in the heredoc DATA. This locator
# returns (start, end) spans of heredoc bodies so the detection can inspect the command with
# its data lines blanked.
...
def _shell_tokens_with_spans(segment: str, start: int):
    ...
    for kind, i, _, _ in _scan_shell(segment, start):
        ...
        if kind == "quote":
            ...
            value.append(ch)
    if quote:
        # an unterminated quote can be a genuine authoring error OR an artifact of
        # detection-side rewrites. The caller decides via _raw_command_lexes_cleanly();
        # this function keeps its original contract (None = malformed).
        return None
    if token_start is not None:
        flush(end_at)
    return tokens
...
# pure-lexer probe that ignores detection rewrites. _normalize_command_for_detection strips
# backslash-escapes GLOBALLY (\X -> X) to defeat r\m-style obfuscation, which flips quote parity
# in ordinary shell text: `grep "a\|b"` (a legal BRE alternation inside double quotes) becomes
# `grep "a|b"`, where the internal `\"` escapes of JSON-ish patterns like [^\"]* collapse and
# the trailing quote lands in the wrong parity class. The quote-aware lexers then see an
# unterminated quote that does not exist in the real command and fail closed at the HARDLINE
# floor ("command parser limit or malformed executable payload") — 30+ live incident blocks
# (read-only grep/sed/strings diagnostics across unattended agent sessions over five days of
# live use). This probe lexes the segment as the AUTHOR wrote it: a trailing backslash or a
# genuinely unterminated quote still fails; escapes the author actually wrote (\| \d \" inside
...
# scratch-space cleanup classifier. The `delete in root path` / `recursive delete` patterns
# fire on ANY absolute path, so `rm -f /tmp/a.md /tmp/b.md` (13+ incident blocks over five days
# of live use — scratch-dir cleanup after test runs) demanded interactive approval in headless
# worker sessions, where approval == permanent block. This classifier tokenizes the command and
# returns True only when EVERY rm operand (across every rm invocation in the command) resolves
# under a scratch root: the OS temp dir ($TMPDIR or /tmp) or a Hermes kanban workspace
# directory. Fail-closed by construction: any substitution ($VAR, ${...}, `...`), any
# path-traversal component (`..`), any operand outside the scratch roots, or an unparseable
# command returns False and the original approval flow applies unchanged.
...
def _iter_command_segments_for_scratch(command: str):
    ...
    for line in masked.splitlines() or [masked]:
        line = _scratch_strip_line_comment(line)
        if not line.strip():
            continue
        try:
...
def _scratch_strip_line_comment(line: str) -> str:
    """Remove a trailing ``# comment`` from one shell line, quote-aware.

    Same contract as :func:`tools.approval_smart._strip_line_comment`; kept
    local so this module stays stdlib-only (importing approval_smart here
    would pull the guardian-LLM stack into every detection call).
    """
...
def detect_dangerous_command(command: str) -> tuple:
    ...
    if _command_parser_limit_exceeded(command):
        # same skeleton probe as the hardline floor — quoted payload bulk is inert data,
        # only the unquoted skeleton counts against the separator-free ceiling.
        if not _unquoted_skeleton_within_limit(command):
            return (True, _PARSER_LIMIT_DESCRIPTION, _PARSER_LIMIT_DESCRIPTION)
    if _is_verification_artifact_cleanup(command):
        return (False, None, None)
    # scratch-space rm cleanup skips only the four rm-shape dangerous patterns (approval-tier).
    # The hardline floor above still blocks any rm touching protected roots regardless of this flag.
    scratch_cleanup = _is_scratch_cleanup(command)

    for command_variant in _command_detection_variants(command):
        # heredoc bodies are DATA. The stream-write rules (tee / >> into env/config) use
        # DOTALL `.*` that walked 100+ chars into report prose and matched a config.yaml
        # mention that was never a redirection target (heredoc report-prose incidents:
        # stream-write rules matched a config.yaml mention inside heredoc data). Mask
        # heredoc bodies for the pattern scan so those rules see only the real command
        # line. Other patterns are unaffected because the masking preserves every
        # non-heredoc byte.
        if not command_variant:
            continue
    ...
    for description, _ in _execution_flag_findings(normalized):
        # a malformed verdict manufactured by normalization's backslash-strip (quote-parity
        # flip) is not trusted when the raw command lexes cleanly. See _raw_command_lexes_cleanly.
