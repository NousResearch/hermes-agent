...
def _persist_completion(event: Dict[str, Any], _result: Dict[str, Any]) -> None:
    """Persist a redacted, replay-safe completion envelope only."""
    now = time.time()
    ...
    with _DB_LOCK, _transaction() as conn:
        conn.execute(
            """UPDATE async_delegations SET state=?, completed_at=?, updated_at=?,
               ...
               WHERE delegation_id=?""",
            (
                durable_event.get("status", "completed"),
                durable_event.get("completed_at", now),
                now,
                json.dumps(durable_event),
                json.dumps(durable_result),
                durable_event["delegation_id"],
            ),
        )
        ...
        for row in rows:
            ...
            conn.execute(
                """UPDATE async_delegations SET state='unknown', completed_at=?,
                   ...
                   WHERE delegation_id=?""",
                (now, now, json.dumps(durable_event), json.dumps(durable_result), delegation_id),
            )
            recovered += 1
    return recovered
