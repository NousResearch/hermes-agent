...
BROWSER_TOOL_SCHEMAS = [
    {
        ...
        "description": "Navigate to a URL in the browser. Initializes the session and loads the page. Must be called before other browser tools. If the user explicitly asks to use the browser/Chromium, inspect console errors, take a screenshot or snapshot, or open a local app/page (localhost, 127.0.0.1, file://), use browser_navigate directly instead of web_search/web_extract. For simple information retrieval from normal public pages, prefer web_search or web_extract (faster, cheaper). For plain-text endpoints — URLs ending in .md, .txt, .json, .yaml, .yml, .csv, .xml, raw.githubusercontent.com, or any documented API endpoint — prefer curl via the terminal tool or web_extract; the browser stack is overkill and much slower for these. Use browser tools when you need to interact with a page (click, fill forms, dynamic content). Returns a compact page snapshot with interactive elements and ref IDs — no need to call browser_snapshot separately after navigating.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
