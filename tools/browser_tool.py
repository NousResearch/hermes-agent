...
def browser_snapshot(
    ...
    if result.get("success"):
        ...
        # Keep snapshot protection aligned with the shared page-URL guard used
        # by console and input actions. The helper owns probing, fail-open
        # behavior, and the always-blocked metadata-address floor.
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    # Keep vision protection aligned with the shared page-URL guard used by
    # console and input actions. Check before capturing a screenshot.
    if _eval_ssrf_guard_active(effective_task_id):
        _blocked_url = _current_page_private_url(effective_task_id)
        if _blocked_url:
            return json.dumps({
