...
try:
    _aux_text_client, _DEFAULT_TEXT_MODEL = get_text_auxiliary_client(
        "web_extract")
except Exception as _init_err:
...
registry.register(
    ...
    handler=lambda args, **kw: browser_vision(question=args.get(
        "question", ""), annotate=args.get("annotate", False), task_id=kw.get("task_id")),
    check_fn=check_browser_requirements,
)
registry.register(
    name="browser_console",
