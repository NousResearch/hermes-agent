...
Captured-primitive hygiene (defect-4 fix): the raw C callables
(``_C_CONNECT``, ``_C_GETADDR``, …) live ONLY in the closure of the private
``_build_guard()`` factory and are never module attributes. After
``install()``, ``import browser_exec_egress_guard; g._C_CONNECT`` raises
AttributeError, so the one-line unguarded-connect bypass is gone; the
guarded callables keep the primitives via the closure for the process
lifetime.
...
def _build_guard():
    """Build the guarded socket interposer inside a private closure.

    Every captured C primitive (``_socket.socket.connect``,
    ``_sock.getaddrinfo``, …) and all mutable guard state live in THIS
    frame's closure. The module namespace below exposes only the exported
    callables — never a raw primitive — so model code in the CLI subprocess
    cannot ``import browser_exec_egress_guard as g; g._C_CONNECT(...)`` its
    way around the guard with a one-line import.
    """
    # ── Captured primitives (before ANY patching / model code) ─────────────
    _C_TYPE = _socket.socket                      # C type — closure-only
    _C_INIT = _socket.socket.__init__             # C-level tp_init
    _C_CONNECT = _socket.socket.connect
    _C_CONNECT_EX = _socket.socket.connect_ex
    _C_SENDTO = _socket.socket.sendto
    _C_SENDMSG = _socket.socket.sendmsg if hasattr(_socket.socket, "sendmsg") else None
    _C_CREATE = _sock.create_connection
    _C_GETADDR = _sock.getaddrinfo                # resolve-once primitive; NOT patched
    ...
    def _load_policy() -> None:
        ...
        nonlocal _POLICY, _ALWAYS_BLOCKED_IP_SET, _ALWAYS_BLOCKED_NET_LIST, _CGNAT_NETWORK
        nonlocal _ALLOWLIST, _ALLOWLIST_IPS
        ...
        for entry in allow_entries:
            ...
            # 10.0.0.9:8444.
            try:
                for _fam, _typ, _proto, _canon, sockaddr in _C_GETADDR(
    ...
    def _check_and_resolve(self_or_none, address):
        ...
        try:
            addr_info = _C_GETADDR(host, port, _sock.AF_UNSPEC, _sock.SOCK_STREAM)
        except Exception:
    ...
    def _g_init(self, family=-1, type=-1, proto=-1, fileno=None):
        if fileno is None:
            ...
            if proto == -1:
                proto = 0
        _C_INIT(self, family, type, proto, fileno)
        self._io_refs = 0
        self._closed = False
    ...
    def _g_connect(self, address):
        try:
            ...
            raise
        return _C_CONNECT(self, (ip, port))
    ...
    def _g_connect_ex(self, address):
        try:
            ...
            return errno.EACCES
        return _C_CONNECT_EX(self, (ip, port))
    ...
    def _g_sendto(self, data, *args):
        ...
        if address is not None:
            try:
                ...
                raise
        return _C_SENDTO(self, data, *args)
    ...
    def _g_sendmsg(self, buffers, ancdata=(), flags=0, address=None):
        if address is not None:
            try:
                ...
                raise
        if _C_SENDMSG is None:
            raise AttributeError("sendmsg not available on this platform")
        return _C_SENDMSG(self, buffers, ancdata, flags, address)
    ...
    def _g_create_connection(address, timeout=_sock._GLOBAL_DEFAULT_TIMEOUT,
        ...
        try:
            ...
            raise
        return _C_CREATE((ip, port), timeout, source_address, all_errors=all_errors)
    ...
    def _g_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        infos = _C_GETADDR(host, port, family, type, proto, flags)
        kept = []
        for info in infos:
            try:
    ...
    def install() -> None:
        ...
        nonlocal _MARKER_FD, _INSTALLED
        if _INSTALLED:
            return
        _INSTALLED = True
        try:
        ...
        if _C_SENDMSG is not None:
            _sock.socket.sendmsg = _g_sendmsg

        # STEP 2 — introduce the visible guarded class (base = patched wrapper).
    ...
    return install, _check_and_resolve, _assert_allowed_ip, _g_create_connection, _g_getaddrinfo


install, _check_and_resolve, _assert_allowed_ip, _g_create_connection, _g_getaddrinfo = _build_guard()
del _build_guard
