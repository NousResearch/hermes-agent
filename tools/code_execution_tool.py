...
def execute_code(
    ...
    try:
        ...
        # HERMES-WRITE-FILE-PATH-CORRUPTION-2026-08-19.md). _script_path is
        # generated by os.path.join(tmpdir, "script.py") where tmpdir is
        # from tempfile.mkdtemp() (safe by construction), but every
        # shell-out should pass through the fail-closed gate.
        try:
            from agent.file_safety import get_invalid_path_error
            invalid_err = get_invalid_path_error(_script_path)
            if invalid_err:
                raise RuntimeError(
                    f"_script_path failed validation: {invalid_err}"
                )
        except (ImportError, ValueError) as exc:
            # If the safety module is unavailable, fall through. The path
            # is still safe-by-construction (tempfile + literal filename).
            logger.debug("path validation unavailable: %s", exc)
