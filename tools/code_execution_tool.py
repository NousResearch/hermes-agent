...
_SECRET_SUBSTRINGS = ("KEY", "TOKEN", "SECRET", "PASSWORD", "CREDENTIAL",
                      ...
                      # without an underscore).  ``_PASS`` (leading
                      # underscore) catches DB_PASS / REDIS_PASS / HOST_PASS
                      # without false-positiveing on bare ``PASS`` in
                      # PASSENGER_HOST / BYPASS_CACHE / COMPASS_DIR.
                      "CREDS", "BEARER", "APIKEY", "_PASS")

# Connection-string regex matching credential-bearing values like
# ``postgresql://user:password@host/db`` or ``redis://:token@host`` (empty
# username).  Inlined here to avoid a circular import from agent/redact.py.
