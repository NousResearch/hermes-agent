...
Architecture (two transports):
  ...
  **Local backend (UDS / TCP):**
  1. Parent generates a `hermes_tools.py` stub module with RPC functions
  2. Parent opens a Unix domain socket (Linux/macOS) or TCP localhost socket
     (Windows) and starts an RPC listener thread
  3. Parent spawns a child process that runs the LLM's script
  4. Tool calls travel over UDS/TCP back to the parent for dispatch
...
# Transport constants
_TRANSPORT_UDS = "uds"     # Unix domain sockets (Linux / macOS)
_TRANSPORT_TCP = "tcp"     # TCP localhost fallback (Windows)

def _detect_transport() -> str:
    """Return the local RPC transport appropriate for this platform."""
    if _IS_WINDOWS:
...
# Sandbox is available on all platforms (UDS on POSIX, TCP on Windows)
...
def check_sandbox_requirements() -> bool:
    """Code execution sandbox is available on all platforms (UDS on POSIX, TCP on Windows)."""
...
def execute_code(
    ...
    try:
        ...
        if _RPC_TRANSPORT == _TRANSPORT_TCP:
