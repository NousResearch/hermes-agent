...
def _window_matches_app_filter(w: Dict[str, Any], app_lower: str) -> bool:
    """Return True when a normalized window matches a requested app."""
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        app_lower = app.lower()
        matched = [w for w in windows if _window_matches_app_filter(w, app_lower)]
        ...
        if target:
            self._active_pid = target["pid"]
            self._active_window_id = target["window_id"]
            self._last_app = target["app_name"] or app  # preserve for capture_after= follow-ups
            return ActionResult(
