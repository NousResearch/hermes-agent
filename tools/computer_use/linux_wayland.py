...
def configured_native_wayland(config: Optional[Mapping[str, Any]] = None) -> bool:
    """Return the single persistent native-Wayland opt-in.

    Accept either the full Hermes config or the already-unwrapped
    ``computer_use`` mapping used by cua_backend.
    """
    cfg = config or {}
    if not isinstance(cfg, Mapping):
        return False
    if "native_wayland" in cfg:
        cu = cfg
    else:
        cu = cfg.get("computer_use") or {}
    if not isinstance(cu, Mapping):
        return False
    return bool(cu.get("native_wayland", False))
...
def native_wayland_enabled(
    ...
    *,
    ...
    requested = configured_native_wayland(config) or e.get(WAYLAND_ENABLE_ENV) == "1"
    if not requested or session.kind not in {"wayland", "wayland-xwayland"}:
        return False
    return (features or probe_driver_features(driver_cmd, e)).wayland_native
...
def diagnose_linux_desktop(
    ...
    *,
    ...
    configured = configured_native_wayland(config)
    ...
    if session.kind in {"x11", "wayland-stale-xwayland"}:
        ...
        if not native_enabled:
            caps.hard_failures.append("Native Wayland is not opted in, the session is unsafe, or the installed driver did not advertise native Wayland support.")
        if not atspi_owner:
    ...
    return {
        ...
        "native_wayland_configured": configured,
