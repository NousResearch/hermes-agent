...
def _capture_view(cap: CaptureResult, max_elements: int) -> SimpleNamespace:
    ...
    # The driver's own AX walk may have stopped at ``computer_use.ax_max_elements``: then the spill file is NOT
    # the full tree, and the hint must not promise one.
    ax_bound = _ax_walk_bound()
    ax_capped = ax_bound > 0 and len(cap.elements) >= ax_bound
    return SimpleNamespace(cap=cap, visible=visible, total=len(cap.elements), width=width, height=height,
                           ...
                           ax_capped=ax_bound if ax_capped else 0,
...
def _ax_walk_bound() -> int:
    """``computer_use.ax_max_elements`` as the driver saw it (0 = unbounded / unknown)."""
    with contextlib.suppress(Exception):
        from tools.computer_use.cua_backend import _cua_configured_ax_max_elements
        return int(_cua_configured_ax_max_elements())
    return 0
...
def _capture_summary_lines(v: SimpleNamespace) -> List[str]:
    ...
    notes = (
        ...
        v.cap.note,
        v.elements_file and (f"{'' if getattr(v, 'ax_capped', 0) else 'full '}element tree with untruncated labels "
                             f"saved to {v.elements_file} — read_file/search_files it if you need dropped label "
                             "text or elements beyond the cap"),
        getattr(v, "ax_capped", 0) and (f"accessibility walk capped at {v.ax_capped} elements; pass app= to narrow"),
    )
    return [
