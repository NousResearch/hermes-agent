...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "deliver": {
                "type": "string",
                "description": "Omit this parameter to auto-deliver back to the current chat and topic (recommended). Auto-detection preserves thread/topic context. Only set explicitly when the user asks to deliver somewhere OTHER than the current conversation. Values: 'origin' (same as omitting), 'local' (no delivery, save only), 'all' (fan out to every connected home channel), or platform:chat_id:thread_id for a specific destination. Slack accepts legacy 'slack:<channel_id>' / 'slack:<channel_id>:<thread_ts>', canonical workspace-qualified 'slack:<T-team_id>:<C/G/D-channel_id>[:<thread_ts>]', and lowercase-friendly names such as 'slack:engineering', 'slack:team-2024', and 'slack:e2e-testing'. Malformed colon-qualified T/E-prefixed refs and bare ID-looking T/E values (9+ uppercase alphanumeric characters including a digit) fail closed; ordinary names remain legacy targets. Combine with comma: 'origin,all' delivers to the origin plus every other connected channel. Examples: 'telegram:-1001234567890:17585', 'discord:#engineering', 'slack:T0123ABCD:C0123ABCD', 'sms:+155****4567', 'all'. WARNING: 'platform:chat_id' without :thread_id loses topic targeting. 'all' resolves at fire time, so a job created before a channel was wired up will pick it up automatically once connected."
            },
            "skills": {
                "type": "array",
                "items": {"type": "string"},
