    ...
    def _adjust_thread_count(self) -> None:
        ...
        if num_threads < self._max_workers:
            ...
            if hasattr(self, "_create_worker_context"):
                # Python 3.14+: ``_worker(executor_reference, ctx, work_queue)`` — initializer/initargs
                # moved into a WorkerContext (shared with InterpreterPoolExecutor).
            ...
            t = threading.Thread(name=thread_name, target=_worker, daemon=True, args=worker_args)
            t.start()
            self._threads.add(t)
