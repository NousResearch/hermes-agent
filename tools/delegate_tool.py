...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    otherwise ``delegation.api_key_env`` can name the provider-specific env var
    to read. If that env var is configured but missing, raise instead of
    silently inheriting the parent agent's unrelated key. When neither key
    source is configured, ``api_key`` is returned as ``None`` so
    ...
    if configured_base_url:
        ...
        # is configured. Without the missing-env guard, a custom delegation
        # endpoint can receive the parent's unrelated key and fail with 401.
        api_key = configured_api_key
        if not api_key and configured_api_key_env:
            raw_api_key = os.getenv(configured_api_key_env)
            api_key = raw_api_key.strip() if raw_api_key else None
            if not api_key:
                raise ValueError(
                    f"delegation.api_key_env is set to {configured_api_key_env!r}, "
