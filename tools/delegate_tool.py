...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "BOTH MODES RUN IN THE BACKGROUND. delegate_task normally returns "
        "immediately with NO result — the completed result re-enters the "
        "conversation as a "
        "separate follow-up message when it finishes, and you can only report it "
        "then. A batch returns one handle, runs N subagents concurrently, and "
        "delivers one consolidated result after ALL of them finish. Do not wait "
        "or poll, and do not report, summarize, or invent a result before its "
        "follow-up arrives — do other independent work or end your turn.\n\n"
