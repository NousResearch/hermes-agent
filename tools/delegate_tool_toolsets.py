...
def _strip_blocked_tools(toolsets: List[str]) -> List[str]:
    ...
    composite toolsets children must never get (``delegation``, ``kanban``). The predicate runs on the RESOLVED
    surface: a composite declares ``tools: []`` and pulls its real tools in via ``includes``, so reading ``tools``
    alone made ``all([])`` classify it as entirely blocked and silently drop it (``safe``, ``hermes-gateway``)."""
