...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        #
        # The temp name MUST be unique per concurrent writer on every supported
        # bash.  macOS still ships bash 3.2, where ``$BASHPID`` is unset, while
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # truncated/half-written file.  Use ``mktemp`` instead of ``$BASHPID``:
