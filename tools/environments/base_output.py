...
def _drain_fd_windows(proc, fd: int, output: _BoundedOutputCollector, decoder, stop=None, msl=None) -> None:
    ...
    while True:
        ...
        if available.value:
            ...
            if not chunk:
                return
            output.append(msl.feed(decoder.decode(chunk)) if msl is not None else decoder.decode(chunk))
            idle_after_exit = 0
            continue
