...
def _sanitize_subprocess_env(
    ...
    *,
    ...
    boundary_active = enforce_profile_boundary or is_multiplex_active() or serves_routed_profile()
    if boundary_active:
        try:
    ...
    if not cross_profile:
        from tools.env_passthrough import scoped_passthrough_additions
        out.update(scoped_passthrough_additions(out))
...
def hermes_subprocess_env(
    *, inherit_credentials: bool = False, base_env: dict[str, str] | None = None,
    ...
    CLI, TUI Node host). ``base_env`` supplies an already curated environment.
    Terminal/execute_code use ``_sanitize_subprocess_env``."""
    env = dict(base_env) if base_env is not None else os.environ.copy()
    from agent.secret_scope import build_profile_env_boundary, serves_routed_profile
    boundary = profile_boundary
    if serves_routed_profile() or boundary is not None:
...
def served_profile_child_env(
    ...
    from agent.secret_scope import (
        ...
        is_multiplex_active, serves_routed_profile,
    )
    ...
    if inherit_credentials:
        if target:
            ...
            if secrets is None and (is_multiplex_active() or serves_routed_profile()):
