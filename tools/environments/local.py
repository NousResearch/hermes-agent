...
from tools.environments.local_env_policy import (  # noqa: F401 — _HERMES_PROVIDER_ENV_BLOCKLIST stays importable from here
    ...
    _is_routed_profile_sdk_credential, _is_terminal_first_party_env,
...
def hermes_subprocess_env(
    *, inherit_credentials: bool = False, base_env: dict[str, str] | None = None
    ...
    ``base_env`` supplies an authoritative snapshot instead of rereading live
    ``os.environ``.
    Terminal and execute_code spawns use the skill-aware sanitizer instead.
    """
    env = _scrub_credentials(
        dict(base_env) if base_env is not None else os.environ.copy(),
    ...
    return _finalize_child_env(
        env,
        allow_process_fallback=base_env is None,
    )
...
def served_profile_child_env(
    ...
    if target:
        ...
        launch_owner = (
            ...
            else get_routing_process_hermes_home()
        )
        if _is_routed_home(target, launch_home=launch_owner):
            strip_launch_profile_env(env, target, launch_home=launch_owner)
            _scrub_credentials(env, inherit_credentials=False)
            from agent.secret_scope import _is_global_env
            from hermes_cli.env_loader import (
                get_secret_source_owned_names,
                get_secret_source_values,
            )

            # A source-supplied name is part of the launch profile's credential
            # boundary even when preserve-existing left the process value in
            # place. Keep only names explicitly classified as process-global;
            # source-owned globals still scrub through the ownership set.
            source_credential_names = {
                name
                for name in get_secret_source_values(launch_owner)
                if not _is_global_env(name.upper())
            }
            credential_names_folded = frozenset(
                name.upper()
                for name in (
                    set(credential_env_names)
                    | set(get_secret_source_owned_names(launch_owner))
                    | source_credential_names
                )
            )
            ...
            aws_defaults = {
                ...
                "BOTO_CONFIG": str(Path(target) / ".boto"),
                "AWS_LOGIN_CACHE_DIRECTORY": str(aws_home / "login" / "cache"),
                "AWS_EC2_METADATA_DISABLED": "true",
            }
            for key in list(env):
                if (
                    _is_routed_profile_sdk_credential(key)
                    or key.upper() in credential_names_folded
                    or key.upper() in aws_defaults
                ):
                    env.pop(key, None)
...
def strip_launch_profile_env(
    ...
    *,
    ...
    residue_names = {
        ...
        set(load_env_file(launch_owner / ".env")) | set(TERMINAL_CONFIG_ENV_MAP.values())
