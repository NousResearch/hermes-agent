...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _is_likely_binary(self, path: str, content_sample: str = None) -> bool:
        ...
        if content_sample:
            ...
            # EXCEPT at the tail: the sample is `head -c 1000`, a BYTE cut that lands
            # mid-character whenever the file uses multi-byte UTF-8 (Vietnamese, CJK,
            # emoji, one accented word near the boundary). The orphaned prefix decodes to
            # a single trailing U+FFFD and would condemn a perfectly good text file. Skip
            # the last few chars so only the cut's own damage is ignored; a genuinely
            # binary sample carries U+FFFD throughout and is still caught. Slice from the
            # END, not [:997] — 1000 bytes of multi-byte text is far fewer than 1000 chars.
