...
class ShellFileOperations(FileOperations):
    ...
    def _read_image(self, path: str) -> ReadResult:
        ...
        if b64_result.exit_code != 0:
            return ReadResult(
                is_image=True, is_binary=True, file_size=file_size, error=f"Failed to read image: {b64_result.stdout}"
            )
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str, replace_all: bool = False) -> PatchResult:
        ...
        new_content, match_count, error = fuzzy_find_and_replace(content, old_string, new_string, replace_all)
        ...
        return PatchResult(
            success=True, diff=diff, files_modified=[path], lint=lint_result.to_dict() if lint_result else None
        )
    ...
    def _search_files(self, pattern: str, path: str, limit: int, offset: int) -> SearchResult:
        ...
        if not self._has_command("find"):
            return SearchResult(
                error="File search requires 'find' command. On Windows, use Git Bash, WSL, or install Unix tools."
            )
    ...
    def _search_content(
        self, pattern: str, path: str, file_glob: Optional[str], limit: int, offset: int, output_mode: str, context: int
    ) -> SearchResult:
    ...
    def _search_with_rg(
        ...
        if output_mode == "files_only":
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                if len(parts) >= 3:
                    try:
                        matches.append(SearchMatch(path=parts[0], line_number=int(parts[1]), content=parts[2][:500]))
                        continue
                    except ValueError:
                        pass
                ...
                if context > 0:
                    ...
                    if len(parts) >= 3:
                        try:
                            matches.append(
                                SearchMatch(path=parts[0], line_number=int(parts[1]), content=parts[2][:500])
                            )
                        except ValueError:
                            pass
