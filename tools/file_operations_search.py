...
class SearchMixin:
 ...
 lane cannot express (the multi-root ``cd`` prefix); ``shell_prefix`` is shell-only."""
    ...
    def _search_files_rg(self, pattern: str, path: str | List[str], limit: int, offset: int,
        ...
        # Wrap bare names so -g matches at any depth (equivalent to find -name). Both ends, not just
        # the leading one: a name anchored only on the left still has to END with the term, and real
        # filenames end with an extension, so `report` silently matched nothing while report-2026.md
        # sat right there. A trailing `*` the caller already wrote collapses into the one added here.
