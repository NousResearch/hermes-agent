...
def _kanban_partial_read_write_error(
    path: str,
    warning: str | None,
    *,
    partial_read: bool = False,
) -> str | None:
    if not _in_kanban_worker_context():
        return None
    if not partial_read and (
        not warning or _FILE_STATE_PARTIAL_READ_FRAGMENT not in warning
    ):
        return None
    return (
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    try:
        ...
        with file_state.lock_path(_resolved):
            ...
            partial_read = file_state.has_partial_read(task_id, _resolved)
            kanban_write_error = _kanban_partial_read_write_error(
                path,
                effective_warning,
