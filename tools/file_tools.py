...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    try:
        with file_state.lock_path(target.backend_path):
            ...
            blocker = _stale_overwrite_blocker(path, target.backend_path, task_id)
            if blocker:
                return json.dumps(
                    _stale_write_refusal(path, blocker, target.backend_path), ensure_ascii=False)
            ...
            # Always report the ABSOLUTE backend path written so a wrong-cwd mismatch
