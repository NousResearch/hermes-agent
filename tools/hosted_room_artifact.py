...
def _active_room_artifact_scope() -> RoomArtifactScope | None:
    # Canonical turns, including unsupported/expired copied contexts, never
    # fall back to a legacy hosted binding. Standalone hosted producers retain
    # their existing explicit scope; no classic export scope is consulted.
    from gateway.session_finite import finite_turn_required
    if finite_turn_required() is not None:
...
def _output_outbox(scope: RoomArtifactScope) -> RoomArtifactOutbox:
    binding = current_output_binding()
    if binding is not None and binding.scope == scope:
        return binding.outbox()
    if _active_room_artifact_scope() != scope:
        raise RoomArtifactError("Group Chat file sharing requires a live owner admission.")
    from hermes_constants import get_hermes_home
    return RoomArtifactOutbox(Path(get_hermes_home()) / "state.db")
...
def _store_backend_group_file(
    *,
    ...
    return _output_outbox(scope).put_bytes(
        scope=scope,
        data=data,
