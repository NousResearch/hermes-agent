...
def _handle_block(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        user_action = args.get("user_action")
        readiness_probe = args.get("readiness_probe")
        _check(user_action is None or isinstance(user_action, dict), "user_action must be an object")
        _check(readiness_probe is None or isinstance(readiness_probe, dict), "readiness_probe must be an object")
        ok = kb.block_task(
...
def _handle_create(args: dict, **kw) -> str:
    ...
    validate_profile_skills(str(assignee), skills)
    with _board(args.get("board")) as (kb, conn):
