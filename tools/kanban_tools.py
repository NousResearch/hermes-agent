...
def _ok_landed(kb, conn, tid: str, default_status: str, **extra: Any) -> str:
    ...
    landed = kanban_db.get_task(conn, tid)
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    try:
        ...
        with _board(None, quiet_close=True) as (kb, conn):
            ...
            ops = ((kanban_worker_identity.register_worker_pid, {"pid": os.getpid(), "expected_run_id": run_id}),
                   (_owner_kanban_claims.heartbeat_claim, {"claimer": os.environ.get("HERMES_KANBAN_CLAIM_LOCK"),
                                         "expected_run_id": run_id}),
                   (kbd.heartbeat_worker, {"note": None, "expected_run_id": run_id}))
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    try:
        with _board(None, quiet_close=True) as (kb, conn):
            rows = kanban_db.list_comments_after(conn, tid, after_id=seen or 0)
    except Exception:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        return json.dumps({
            ...
            "parents": kanban_db.parent_ids(conn, tid),
            "children": kanban_db.child_ids(conn, tid),
            "comments": [_fields(c, _COMMENT_FIELDS) for c in kanban_db.list_comments(conn, tid)],
            # Capped; full log via CLI.
            "events": [_fields(e, _EVENT_FIELDS) for e in kanban_db.list_events(conn, tid)[-50:]],
            "runs": [_fields(r, _RUN_FIELDS) for r in _owner_kanban_stats.list_runs(conn, tid)],
            # Same string build_worker_context hands the dispatcher at spawn time.
            "worker_context": _owner_kanban_context.build_worker_context(conn, tid)})
...
def _handle_complete(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        try:
            ok = _owner_kanban_completion.complete_task(
                conn, tid, result=result, summary=summary, metadata=metadata,
                created_cards=created_cards, expected_run_id=_worker_run_id(tid))
        except _owner_kanban_completion.ArtifactPreservationError as artifact_err:
            ...
            return tool_error(
                ...
                f"handoff.")
        except _owner_kanban_completion.HallucinatedCardsError as hall_err:
...
def _handle_block(args: dict, **kw) -> str:
    ...
    kind = args.get("kind")
    with _board(args.get("board")) as (kb, conn):
        _check(kind is None or kind in kanban_db.VALID_BLOCK_KINDS,
               f"kind must be one of {sorted(kanban_db.VALID_BLOCK_KINDS)} (or omit it)")
        ...
        ok = _owner_kanban_db.block_task(conn, tid, reason=reason, kind=kind, expected_run_id=_worker_run_id(tid))
...
def _handle_request_review(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        _goal_gate("kanban_request_review", kanban_db.get_task(conn, tid), tid, summary)
        ok, fail_reason = _owner_kanban_transitions.request_review(
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        run_id = _worker_run_id(tid)
        try:
            kanban_worker_identity.register_worker_pid(conn, tid, expected_run_id=run_id, pid=os.getpid())
        except Exception:
            logger.debug("kanban_heartbeat: worker pid registration failed", exc_info=True)
        _owner_kanban_claims.heartbeat_claim(conn, tid, claimer=os.environ.get("HERMES_KANBAN_CLAIM_LOCK"),
                           expected_run_id=run_id)
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        cid = kanban_db.add_comment(conn, tid, author=author, body=str(body))
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    try:
        data, fetched_ct = _download_url_with_cap(url, kanban_db.KANBAN_ATTACHMENT_MAX_BYTES)
...
def _handle_attachments(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        return json.dumps({
            "ok": True, "task_id": tid,
            "attachments": [
                _fields(a, _ATTACHMENT_FIELDS) for a in kanban_db.list_attachments(conn, tid)]})
...
def _handle_create(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        self_task = kanban_db.get_task(conn, self_tid) if self_tid else None
        ...
        landed = _fields(kanban_db.get_task(conn, new_tid), _CREATED_FIELDS)
...
def _handle_unblock(args: dict, **kw) -> str:
    ...
    _enforce_worker_task_ownership(tid)
    with _board(args.get("board")) as (kb, conn):
        _check(_owner_kanban_transitions.unblock_task(conn, tid), f"could not unblock {tid} (not blocked or unknown)")
        return _ok(task_id=tid, **_fields(kanban_db.get_task(conn, tid), ("status",)))
...
def _handle_link(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        kanban_db.link_tasks(conn, parent_id=parent_id, child_id=child_id)
