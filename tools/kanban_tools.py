...
def _handle_unlink(args: dict, **kw) -> str:
    """Remove a dependency edge so a worker can correct a reversed link."""
    _reject_delegated_child_mutation("kanban_unlink")
    parent_id = args.get("parent_id")
    child_id = args.get("child_id")
    _check(parent_id and child_id, "both parent_id and child_id are required")
    with _board(args.get("board")) as (kb, conn):
        _check(kb.unlink_tasks(conn, parent_id=parent_id, child_id=child_id),
               f"could not unlink {parent_id} → {child_id} (edge not found)")
