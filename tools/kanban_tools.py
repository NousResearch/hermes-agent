...
def _subscription_context_from_session(kb: Any) -> Any:
    """Translate a durable gateway or TUI delivery source for task creation."""
    ...
    if platform or chat_id:
        if not platform or not chat_id:
            # A partial gateway context is not a durable delivery target, but
            # it must still enter the canonical policy so parent inheritance
            # and configured defaults can apply.
            return kb.SubscriptionContext()
        return kb.SubscriptionContext(
            ambient_origin=kb.NotificationTarget(
                platform=platform,
                chat_id=chat_id,
    ...
    if not session_key:
        # Unattached workers have no ambient destination.  They still need an
        # empty context rather than ``None``: ``None`` bypasses the canonical
        # policy entirely, including actual-parent inheritance and defaults.
        return kb.SubscriptionContext()
    return kb.SubscriptionContext(
        ambient_origin=kb.NotificationTarget(
            platform="tui", chat_id=session_key, notifier_profile=profile or None,
        )
    )
    ...
    try:
        ...
        try:
            ...
            new_tid = kb.create_task(
                ...
                subscription_context=_subscription_context_from_session(kb),
            )
            new_task = kb.get_task(conn, new_tid)
            subscribed = bool(kb.list_notify_subs(conn, new_tid))
            return _ok(
                task_id=new_tid,
