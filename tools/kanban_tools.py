...
def _handle_create(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            _maybe_subscribe_current_gateway_source(kb, conn, new_tid)
...
def _maybe_subscribe_current_gateway_source(kb, conn, task_id: str) -> None:
    """Subscribe the originating gateway chat to child task terminal events.

    Slash `/kanban create` already does this in the gateway handler. Model
    tool calls need the same behavior so a task created from QQ/Weixin/etc.
    reports completion, block, crash, or timeout back to the chat that asked
    for it. CLI and non-gateway contexts leave HERMES_SESSION_* empty and are
    ...
    try:
        ...
        logger.warning(
            "kanban_create gateway-source auto-subscribe failed for %s",
            task_id,
            exc_info=True,
        )
