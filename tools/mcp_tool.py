...
# Read-only MCP tools eligible for execute_code, keyed by (registry scope,
# registry name). Profiles can register the same name with different servers
# or annotations, so this authority must never be process-global.
_mcp_tool_read_only: Dict[tuple[Optional[str], str], str] = {}
