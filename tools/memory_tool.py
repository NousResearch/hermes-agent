...
MEMORY_SCHEMA = {
    "name": "memory",
    "description": (
        "Save compact durable facts to persistent memory that survives across sessions. "
        "Memory is injected into future turns, so it must stay small and useful in nearly "
        "every future session.\n\n"
        "BEFORE SAVING: classify the destination first. Use memory only for:\n"
        "- User preferences, communication style, durable personal facts, and pet peeves\n"
        "- Stable environment facts, tool quirks, and conventions that affect future work\n"
        "- Short facts that prevent repeated corrections across unrelated sessions\n\n"
        "DO NOT put these in memory:\n"
        "- Procedures, checklists, templates, reusable workflow corrections, or operational rules; use skills or governing instructions\n"
        "- Project-scoped conventions; use project docs or AGENTS.md\n"
        "- Knowledge assets or long-form notes; use the user's knowledge base such as Obsidian / AI_Agent_Assets\n"
        "- Do NOT save task progress, session outcomes, completed-work logs, or temporary TODO state; use session_search to recall those from past transcripts\n\n"
        "If a user correction is about how to perform a class of work, prefer skill_manage or a documented rule. "
        "Use memory only for the compact user/environment fact, if one is still needed.\n\n"
        "TWO TARGETS:\n"
