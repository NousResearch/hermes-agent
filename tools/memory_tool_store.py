...
class MemoryStore:
    ...
    def load_from_disk(self):
        ...
        def _sanitize(entry, filename):
            # Scan every entry: a literal "[BLOCKED:" prefix on disk does not prove
            # it was generated by the sanitizer, and must not bypass inspection.
            findings = scan_for_threats(entry, scope="strict") if entry else None
            if not findings:
                return entry
