...
from tools.process_registry_models import MAX_OUTPUT_CHARS, ProcessSession
...
from tools import process_registry_scope as _scope
...
def systemd_user_bus_env(base_env: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    if not _scope._IS_LINUX:
        return env
    configured = env.get("XDG_RUNTIME_DIR")
    if configured and _scope._secure_user_runtime_dir(Path(configured)):
        runtime_dir = Path(configured)
    else:
        runtime_dir = _scope._default_user_runtime_dir()
        if not _scope._secure_user_runtime_dir(runtime_dir):
            return env
    ...
    try:
        ...
        return env
    if not stat.S_ISSOCK(bus_metadata.st_mode) or bus_metadata.st_uid != os.getuid():  # windows-footgun: ok — behind the _scope._IS_LINUX gate above
        return env
...
def restart_safe_gateway_child_argv(
    ...
    if not _scope._is_supervised_gateway_process() or not os.environ.get("INVOCATION_ID"):
        return GatewayChildDispatch("in_process", command)
    ...
    def _degrade(detail: str) -> GatewayChildDispatch:
        ...
        _scope._warn_scope_degraded_once(detail)
        return GatewayChildDispatch("degraded", command)

    if not _scope._systemd_run_user_scope_available():
        return _degrade(
            "systemd-run --user --scope is unavailable (usually no reachable user D-Bus session at "
            f"/run/user/{os.getuid()}/bus). On a system-level service install, run "  # windows-footgun: ok — behind the _scope._IS_LINUX return above
            "`sudo loginctl enable-linger <gateway-user>` and restart the gateway."
        )
    scoped = _scope._build_systemd_scope_argv(
        command,
        unit_suffix=unit_suffix,
        # Memory bounds are caller-resolved since the builder stopped
        # auto-filling them — same default as before.
        memory_max_bytes=_scope._worker_memory_max_bytes(),
    )
    if scoped == command:
...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _scope_argv(self, session: ProcessSession, safe_command: str, unit_suffix: str, label: str) -> List[str]:
        ...
        in_supervised_gateway = _scope._IS_LINUX and _scope._is_supervised_gateway_process()
        if in_supervised_gateway and _scope._systemd_run_user_scope_available():
            session.systemd_unit = f"hermes-worker-{unit_suffix}.scope"
            return _scope._build_systemd_scope_argv(
                argv, unit_suffix=unit_suffix, memory_max_bytes=_scope._worker_memory_max_bytes())
    ...
    def _spawn_local_pty(self, session: ProcessSession, safe_command: str, env_vars: dict) -> ProcessSession:
        ...
        if session.systemd_unit:
            pty_env = _launch_registry.systemd_user_bus_env(pty_env)
    ...
    def spawn_local(
        ...
        if use_pty:
            try:
                ...
                if session.systemd_unit:
                    ...
                    if not _scope._stop_systemd_unit(session.systemd_unit):
                        raise RuntimeError(
        ...
        if session.systemd_unit:
            spawn_env = _launch_registry.systemd_user_bus_env(spawn_env)
    ...
    def _reap_untracked(self, session: ProcessSession, proc: subprocess.Popen) -> None:
        ...
        with suppress(Exception):
            if session.systemd_unit:
                ...
                _scope._stop_systemd_unit(session.systemd_unit)
