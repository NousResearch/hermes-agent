    ...
    try:
        ...
        if run_state.trainer_process.poll() is not None:
            ...
            run_state.error_message = (
                f"Trainer exited with code {run_state.trainer_process.returncode}. Check {trainer_log}"
            )
...
async def rl_check_status(run_id: str) -> str:
    ...
    result = {
        ...
        "processes": {name: "running" if code is None else f"exited ({code})" for name, code in processes.items()},
...
async def rl_test_inference(
    ...
    for model_info in test_models:
        ...
        try:
            ...
            async def read_stream(stream, lines_list, prefix=""):
                ...
                while True:
                    ...
                    if any(
                        kw in decoded.lower() for kw in ["processing", "group", "step", "progress", "%", "completed"]
                    ):
        ...
        if model_results["steps_tested"] > 0:
            ...
            model_results["step_success_rate"] = round(steps_with_correct / model_results["steps_tested"], 3)
        else:
    ...
    results["summary"] = {
        ...
        "avg_accuracy": round(sum(m.get("accuracy", 0) for m in working_models) / len(working_models), 3)
        if working_models
        else 0,
...
RL_CHECK_STATUS_SCHEMA = {
    ...
    "parameters": {
        "type": "object",
        "properties": {"run_id": {"type": "string", "description": "The run ID from rl_start_training()"}},
        "required": ["run_id"],
    },
}
...
RL_GET_RESULTS_SCHEMA = {
    ...
    "parameters": {
        "type": "object",
        "properties": {"run_id": {"type": "string", "description": "The run ID to get results for"}},
        "required": ["run_id"],
    },
}
RL_LIST_RUNS_SCHEMA = {
...
registry.register(
    ...
    handler=lambda args, **kw: rl_test_inference(
        num_steps=args.get("num_steps", 3), group_size=args.get("group_size", 16), models=args.get("models")
    ),
    check_fn=check_rl_api_keys,
    requires_env=_rl_env,
    is_async=True,
