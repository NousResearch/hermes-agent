...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    try:
        ...
        if send_parse_mode == ParseMode.MARKDOWN_V2 and len(text_chunks) > 1:
            # truncate_message() appends a raw " (N/M)" suffix to each chunk; escape it now that
            # formatting has already run, or Telegram rejects every chunk's MarkdownV2 parse and
            # the whole multi-chunk delivery silently falls back to unstyled plain text.
