...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    1. **Bot API 10.1 rich message** (``sendRichMessage`` via ``do_api_request``)
       when supported. The raw agent markdown is passed through and Telegram
       renders tables, task lists, ``<details>``, math, underline, and custom
       emoji natively. This is the path the in-gateway adapter already uses;
       we mirror it here so MCP ``send_message`` and cron delivery to
       Telegram get the same rich rendering.
    2. **MarkdownV2** (``sendMessage`` with ``parse_mode=MARKDOWN_V2``) for
       clients that reject the rich payload (BadRequest on parser/limit
       errors) or older PTB that lacks ``do_api_request``.
    ...
    try:
        ...
        _can_try_rich = False
        if not _has_html and not (media_files or []):
            try:
