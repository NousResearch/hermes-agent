    ...
    try:
        ...
        for media_path, is_voice in media_files:
            ...
            try:
                ...
                # Validate response - last_msg must exist and have message_id
                if (
                    last_msg is None
                    or not hasattr(last_msg, "message_id")
                    or last_msg.message_id is None
                ):
                    error_msg = (
                        f"Telegram returned invalid response for media {media_path}"
                    )
                ...
                warnings.append(warning)
                # Mark failure so we don't report success when file delivery fails
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if len(attachment_paths) < len(valid_media):
                result["warnings"] = [
                    f"Some media files were skipped (not found on disk)"
                ]
            return result
    except Exception as e:
