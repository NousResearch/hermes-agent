...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target. Format: 'platform' (uses home channel), 'platform:#channel-name', 'platform:chat_id', or 'platform:chat_id:thread_id' for Telegram topics and Discord threads. Examples: 'telegram', 'telegram:-1001234567890:17585', 'discord:999888777:555444333', 'discord:#bot-home', 'slack:#engineering', 'signal:+155****4567', 'matrix:!roomid:server.org', 'matrix:@user:server.org', 'napcat:direct:<qq_id>' (QQ private chat), 'napcat:group:<group_id>' (QQ group chat), 'yuanbao:direct:<account_id>' (DM), 'yuanbao:group:<group_code>' (group chat)"
            },
            "message": {
                "type": "string",
...
QQ_SEND_MESSAGE_SCHEMA = {
    ...
    "description": (
        "QQ/NapCat cross-chat sender. When a QQ user asks you to send a message "
        "to a different QQ private chat or QQ group, you must use this tool; do "
        "not just promise that you will send it in the final response. Use "
        "action='list' first if the recipient is ambiguous, then action='send'. "
        "Use this only for an explicitly different chat; ordinary replies and "
        "same-chat media returns should be final responses in the current chat. "
        "Only say the message was sent after this tool returns success."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
                "type": "string",
                "enum": ["send", "list"],
                "description": "Use 'list' to inspect known QQ/NapCat targets, or 'send' to deliver a message.",
            },
            "target_type": {
                "type": "string",
                "enum": ["group", "direct"],
                "description": "Recipient kind for action='send': 'group' for a QQ group, 'direct' for a private QQ chat.",
            },
            "target_id": {
                "type": "string",
                "description": "Raw QQ group id or private QQ user id. Do not include 'napcat:' here.",
            },
            "message": {
                "type": "string",
                "description": "Message to send. For cross-chat media, include MEDIA:/absolute/path/to/file.",
            },
        },
        "required": ["action"],
    },
...
def send_message_tool(args, **kw):
    ...
    return _handle_send(args, prefer_standalone=bool(kw.get("prefer_standalone")))


def qq_send_message_tool(args, **kw):
    """QQ-specific wrapper around send_message for clearer model tool selection."""
    action = str(args.get("action", "send") or "send").strip().lower()
    if action == "list":
        return _handle_qq_list()
    if action != "send":
        return json.dumps(_error("Unsupported action for qq_send_message. Use 'list' or 'send'."))
    ...
    target_type = target_aliases.get(target_type, target_type)
    if target_type not in {"group", "direct"}:
        return json.dumps(_error("target_type is required and must be 'group' or 'direct'."))
    if not target_id:
        return json.dumps(_error("target_id is required for action='send'."))
    if not message:
        return json.dumps(_error("message is required for action='send'."))

    lowered_target_id = target_id.lower()
    if lowered_target_id.startswith(("group:", "direct:")):
...
def _handle_qq_list():
    """Return known QQ/NapCat targets in a schema shaped for qq_send_message."""
    try:
        from gateway.channel_directory import load_directory
        directory = load_directory()
        entries = directory.get("platforms", {}).get("napcat", [])
        targets = []
        for entry in entries:
            ...
            target_type = "group" if entry_type == "group" or raw_id.startswith("group:") else "direct"
            target_id = raw_id
            if raw_id.startswith(("group:", "direct:")):
                prefix, target_id = raw_id.split(":", 1)
                target_type = "group" if prefix == "group" else "direct"
            targets.append({
        ...
        return json.dumps({
            ...
            "platform": "napcat",
            "targets": targets,
            "usage": "Call qq_send_message(action='send', target_type=..., target_id=..., message=...).",
        }, ensure_ascii=False)
    except Exception as e:
        return json.dumps(_error(f"Failed to load QQ/NapCat targets: {e}"))
...
def _handle_send(args, *, prefer_standalone: bool = False):
    ...
    same_chat_skip = _maybe_skip_same_chat_send(platform_name, chat_id)
    if same_chat_skip:
        return json.dumps(same_chat_skip)
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                pass
            try:
                from gateway.xiaoxing_working_memory import record_event
                from gateway.session_context import get_session_env
                record_event(
                    direction="outbound",
                    ...
                    interlocutor_id=str(chat_id),
                    interlocutor_name=str(chat_id),
                    session_id=get_session_env("HERMES_SESSION_ID", ""),
                    session_key=get_session_env("HERMES_SESSION_KEY", ""),
                    text=mirror_text,
                    trigger=get_session_env("HERMES_SESSION_PLATFORM", "cli") or "cli",
                )
            except Exception:
                pass
...
def _parse_target_ref(platform_name: str, target_ref: str):
    ...
    if platform_name == "napcat":
        lowered = target_ref.strip().lower()
        if lowered.startswith(("group:", "g:", "private:", "direct:", "dm:", "user:")):
...
def _maybe_skip_same_chat_send(platform_name: str, chat_id: str) -> Optional[Dict[str, object]]:
    """Prevent tool-sending to the chat that is already receiving this turn's reply."""
    try:
        from gateway.session_context import get_session_env
        current_platform = (get_session_env("HERMES_SESSION_PLATFORM", "") or "").strip().lower()
        current_chat_id = str(get_session_env("HERMES_SESSION_CHAT_ID", "") or "").strip()
    except Exception:
        return None

    if not current_platform or not current_chat_id:
        return None
    if current_platform in {"cli", "cron", "local"}:
        return None
    if platform_name == current_platform and str(chat_id or "").strip() == current_chat_id:
        return {
            "error": (
                "You are already replying in this chat. Put the content in the final response "
                "instead of using send_message to the same channel."
            ),
            "same_chat": True,
        }
    return None
...
async def _send_via_adapter(
    ...
    *,
    ...
    if runner is not None and not prefer_standalone:
        try:
        ...
        if adapter is not None:
            try:
                ...
                async def _on_adapter_loop(coro):
                ...
                for media_path, is_voice in media_files or []:
                    ...
                    if force_document:
                        result = await _on_adapter_loop(
                            adapter.send_document(chat_id=chat_id, file_path=media_path, metadata=metadata)
                        )
                    elif is_voice or ext in _AUDIO_EXTS:
                        if hasattr(adapter, "send_voice"):
                            result = await _on_adapter_loop(
                                adapter.send_voice(chat_id=chat_id, audio_path=media_path, metadata=metadata)
                            )
                        else:
                            result = await _on_adapter_loop(
                                adapter.send_document(chat_id=chat_id, file_path=media_path, metadata=metadata)
                        ...
                        result = await _on_adapter_loop(
                            adapter.send_image_file(chat_id=chat_id, image_path=media_path, metadata=metadata)
                        )
                    elif ext in _VIDEO_EXTS and hasattr(adapter, "send_video"):
                        result = await _on_adapter_loop(
                            adapter.send_video(chat_id=chat_id, video_path=media_path, metadata=metadata)
                        )
                    else:
                        result = await _on_adapter_loop(
                            adapter.send_document(chat_id=chat_id, file_path=media_path, metadata=metadata)
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False, prefer_standalone=False):
    ...
    # --- Local plugin platforms with native media support via live adapter ---
    if getattr(platform, "value", "") in {"napcat", "codex-xiaoxing-channel"} and media_files:
        last_result = None
        for i, chunk in enumerate(chunks):
            is_last = (i == len(chunks) - 1)
    ...
    if media_files and not message.strip():
        return {
            "error": (
                f"send_message MEDIA delivery is currently only supported for telegram, discord, matrix, weixin, signal, yuanbao, feishu, napcat and codex-xiaoxing-channel; "
    ...
    if media_files:
        warning = (
            ...
            "native send_message media delivery is currently only supported for telegram, discord, matrix, weixin, signal, yuanbao, feishu, napcat and codex-xiaoxing-channel"
        )
...
registry.register(
    name="qq_send_message",
    toolset="messaging",
    schema=QQ_SEND_MESSAGE_SCHEMA,
    handler=qq_send_message_tool,
    check_fn=_check_send_message,
    emoji="💬",
)
