...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "direct source (URL, phone/contact, app/thread, file path, account, website, "
        "live system), inspect that original source before or instead of "
        "session_search when accessible. Use session_search as secondary context for "
        "what was previously said, not as primary proof of current source contents. "
        "If the original source is inaccessible, say so and why before falling back. "
        "Do not conclude 'not found' or 'no prior correspondence' from session_search "
        "alone when a direct source was provided.\n\n"
        "FOUR CALLING SHAPES\n\n"
        "  1) DISCOVERY — pass `query` (optional limit/sort). FTS5, lineage-deduped "
        "top-N sessions with snippet, ±5 messages around the hit, bookend_start/end, "
        "match_message_id, messages_before/after.\n"
        "  2) SCROLL — pass `session_id` + `around_message_id` (optional window). "
        "Window only, no FTS/bookends. To scroll FORWARD: pass messages[-1].id; to "
        "scroll BACKWARD: pass messages[0].id. Boundary message appears in both "
        "windows; messages_before/after < window means start/end.\n"
        "  3) READ — pass `session_id` only (optional profile). Full session dump "
        "(first 20 + last 10 when large). Resolve `@session:<profile>/<id>` by "
        "splitting on `/` into profile + id.\n"
        "  4) BROWSE — no args. Recent sessions chronologically (titles, previews, "
        "timestamps).\n\n"
        "FTS5 SYNTAX: AND is default; use OR, quoted phrases, NOT, or prefix "
        "wildcards (`deploy*`). Use for Hermes conversation-history questions only; "
        "external tools show current source/world state."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
                ...
                "description": (
                    ...
                    "expressions. Omit to browse recent sessions. Ignored for scroll "
            ...
            "sort": {
                ...
                "enum": ["newest", "oldest"],
                "description": (
                    "Discovery only. Temporal bias on FTS ranking. Omit for "
                    "relevance-only; 'newest' for recency; 'oldest' for origin. "
                    "Ignored in scroll/browse."
                ),
            },
            "session_id": {
                "type": "string",
                "description": (
                    "Session to read/scroll. From discovery results, or id segment of "
                    "`@session:<profile>/<id>`. With around_message_id = scroll; alone = read."
                ),
            },
            "around_message_id": {
                "type": "integer",
                "description": (
                    "Scroll anchor message id (use match_message_id from discovery, or "
                    "window edge ids to scroll forward/backward)."
                ),
            },
            "window": {
                "type": "integer",
                "description": (
                    "Scroll only. Messages on each side of the anchor (anchor included). "
            ...
            "role_filter": {
                ...
                "description": (
                    ...
                    "'user,assistant'. Pass 'user,assistant,tool' or 'tool' to include "
                    "tool output."
                ),
            },
            "profile": {
                "type": "string",
                "description": (
                    "Optional read-only other-profile DB. For `@session:<profile>/<id>`, "
                    "pass the profile segment with session_id as the id. Omit for current."
                ),
            },
        },
        "required": [],
