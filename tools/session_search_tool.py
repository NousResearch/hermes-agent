...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "This tool deliberately EXCLUDES the current session because the current "
        "transcript is already visible to you.\n\n"
        "Do NOT use this tool to answer what is happening in the current chat, "
        "current Telegram topic, current thread, or current session. For questions "
        "like 'what are we doing here?', 'what is this session about?', 'where did "
        "we stop in this topic?', answer from the current conversation history and "
        "the Current Session Context/Channel Topic. If the current history is "
        "unclear, say that and ask for the missing anchor instead of searching "
        "unrelated sessions.\n\n"
        "TWO MODES:\n"
        "1. Recent sessions (no query): Call with no arguments to see what was worked on recently. "
        "Returns titles, previews, and timestamps. Zero LLM cost, instant. "
        "Use this only when the user asks about previous/recent sessions globally, "
        "not the current one.\n"
