...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "twice.\n\n"
        "FTS5 SYNTAX\n\n"
        "  AND is the default — multi-word queries require all terms. When that "
        "strict pass finds nothing, plain queries are automatically retried with "
        "OR-joined terms, so naturally-phrased queries still recall. Use OR "
        "explicitly for broader recall up front (`alpha OR beta OR gamma`), quoted "
        "phrases for exact match (`\"docker networking\"`), boolean "
        "(`python NOT java`), or prefix wildcards (`deploy*`) — queries using any "
