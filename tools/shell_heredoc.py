...
def detect_unquoted_heredoc_file_write(command: str) -> str | None:
    ...
    while command_start < len(command):
        ...
        # Attribute the heredoc to the simple command that OWNS it, not to everything from
        # ``command_start``. ``_scan_heredoc_command_unit`` reports where the owner begins,
        # past any preceding list operators. Slicing from ``command_start`` would sweep in a
        # sibling command's redirect -- ``cat > a.txt; python3 <<EOF`` would look like a file
        # write even though the heredoc feeds python -- and, because
        # ``_HEREDOC_INTERPRETER_CONSUMER_RE`` is anchored, the interpreter test would then
        # fail to recognise the real consumer and wrongly block a legitimate command.
        masked_opener = _mask_simple_quotes(command[owner_start:command_end])
