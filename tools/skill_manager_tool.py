...
_pending_target_anchor: "_ctxvars.ContextVar[Optional[Dict[str, Any]]]" = (
    _ctxvars.ContextVar("pending_target_anchor", default=None)
)
...
def _skill_manage_batch(
    ...
    if any(isinstance(op, dict) and op.get("action") == "delete" for op in operations):
        ...
        with _pending_target_anchor_context(nm, op.get("category")):
    ...
    # A one-op array is the advertised modern call shape. Route it through the
    # existing descriptor-bound single-op path so approval staging records the
    # exact target pre-image and provenance instead of the weaker upstream
    # batch payload. Multi-op batches remain available when the approval gate
    # is off, but fail closed below when approval would need replay.
    if len(operations) == 1:
        op = operations[0]
        with _pending_target_anchor_context(names[0], op.get("category")):
            return _skill_manage_unlocked(
                action=op["action"],
                name=names[0],
    ...
    try:
        for i, op in enumerate(operations):
            with _pending_target_anchor_context(names[i], op.get("category")):
                raw = _skill_manage_unlocked(
                    action=op["action"],
                    name=names[i],
                    content=op.get("content"),
                    category=op.get("category"),
