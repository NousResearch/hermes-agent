...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "ACTIONS:\n"
        "- create     — Create new skill (full SKILL.md content required)\n"
        "- patch      — Find-and-replace on SKILL.md (preferred for updates)\n"
        "- edit       — Replace entire SKILL.md (major changes)\n"
        "- delete     — Remove skill entirely\n"
        "- write_file — Add supporting files (references/, templates/, scripts/, assets/)\n"
        "- remove_file\n\n"
        "SKILL FORMAT (for 'create' and 'edit'):\n"
        "Content MUST start with YAML frontmatter (required by validation):\n\n"
        "  ---\n"
        ...
        "  # Trigger conditions, procedure, pitfalls, verification...\n\n"
        "CATEGORY RULE (for 'create'):\n"
        "Must be a SINGLE directory name — no slashes allowed.\n"
        "Don't include name in category or vice versa.\n"
        "Examples: 'devops' ✓, 'mlops/training' ✗\n\n"
        "WHEN TO CREATE SKILLS:\n"
        "✓ Complex task succeeded (5+ tool calls)\n"
        "✓ Errors overcome that were not trivial\n"
        "✓ User corrected your approach, and was correct\n"
        "✓ Non-trivial workflow discovered\n\n"
        "WHEN TO UPDATE SKILLS:\n"
        "Instructions stale/wrong, OS-specific failures, missing steps or pitfalls found during use.\n"
        "If you used a skill and hit issues not covered by it, patch it immediately.\n\n"
        "Best practice: Use skill_view() to read an existing skill first to see the format.\n"
