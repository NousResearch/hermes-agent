...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "IMPORTANT — state file paths: Skills may run in environments where the "
        "skill directory is read-only (Docker, remote execution). Scripts that persist "
        "state MUST resolve writable paths via: "
        "Path(os.environ.get('HERMES_HOME', str(Path.home() / '.hermes'))) / 'state' / 'skills' / '<skill-name>'. "
        "NEVER write state to Path(__file__).parent or the skill directory itself.\n\n"
