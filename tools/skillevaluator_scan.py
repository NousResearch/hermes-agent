...
def run_tier1_scan(skill_dir: Path, timeout: int = SCAN_TIMEOUT_SECONDS) -> Tier1Report:
    ...
    with tempfile.TemporaryDirectory(prefix="se-tier1-") as outdir:
        ...
        # HERMES-WRITE-FILE-PATH-CORRUPTION-2026-08-19.md). outdir is
        # generated by tempfile.TemporaryDirectory() (safe by construction),
        # but every shell-out should pass through the fail-closed gate.
        try:
            from agent.file_safety import get_invalid_path_error
            invalid_err = get_invalid_path_error(outdir)
            if invalid_err:
                return Tier1Report(
                    available=False,
                    error=f"outdir failed validation: {invalid_err}",
                )
        except (ImportError, ValueError) as exc:
            # If the safety module is unavailable, fall through. The
            # outdir is still safe-by-construction (tempfile).
