...
class GitHubAuth:
    ...
    GitHub API authentication. Tries methods in priority order:
      1. auth.json credential_pool (PAT — SSOT)
      2. GITHUB_TOKEN / GH_TOKEN env var (PAT)
      3. `gh auth token` subprocess (if gh CLI is installed)
      4. GitHub App JWT + installation token (if app credentials configured)
      5. Unauthenticated (60 req/hr, public repos only)
    """
    ...
    def _get_auth_json_token(self) -> Optional[str]:
        """Read GitHub token from auth.json credential_pool via read_credential_pool API."""
        try:
            from hermes_cli.auth import read_credential_pool
            for entry in read_credential_pool("github"):
                token = entry.get("access_token", "") if isinstance(entry, dict) else ""
                if token and str(token).startswith("ghp_"):
                    return str(token)
        except Exception:
            pass
        return None
    ...
    def _resolve_token(self) -> Optional[str]:
        ...
        # 1. auth.json credential_pool (SSOT)
        token = self._get_auth_json_token()
        if token:
            self._cached_token = token
            self._cached_method = "auth-json"
            return token
