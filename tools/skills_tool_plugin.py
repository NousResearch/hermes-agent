...
def _available_skill_files(skill_dir: Path) -> Dict[str, List[str]]:
    ...
    for f in skill_dir.rglob("*"):
        if not f.is_file() or f.name == "SKILL.md":
            continue
        # POSIX form: the group key below is the first "/"-separated segment, so a native
        # ``references\api.md`` on Windows has no separator at all — every supporting file fell
        # through to "other", and one with a non-source suffix (assets/logo.png) was dropped
        # from the listing the caller is told to choose from.
