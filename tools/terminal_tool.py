...
def _terminal_env_snapshot(
    ...
    """Build terminal settings without mutating the process environment.

    A bound per-turn terminal scope is authoritative. Unscoped callers retain
    the historical config-to-environment bridge. Callers that supply a raw
    config get an isolated projection for cache identity calculations.
    """
