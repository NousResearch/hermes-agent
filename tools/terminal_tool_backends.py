...
# --- Environment builders. Signature: (*, image, cwd, timeout, cc, task_id, ssh_config, host_cwd, local_config)
...
def _build_local_env(*, cwd, timeout, local_config=None, **_):
    execution = (local_config or {}).get("execution_context")
    return _LocalEnvironment(cwd=cwd, timeout=timeout, **({"execution_context": execution} if execution else {}))
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    kwargs = dict(image=image, cwd=cwd, timeout=timeout, cc=container_config or {}, task_id=task_id,
                  ssh_config=ssh_config, host_cwd=host_cwd, local_config=local_config, probe_only=probe_only)
