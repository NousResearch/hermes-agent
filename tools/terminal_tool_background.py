...
_HOMEBREW_CI_POLLER_HINT = (
    ...
    'appear when piped. The green-ci-policy guidance is inlined here. Use one of these instead. '
    '(1) Exit-code-driven: `gh pr checks $PR >/dev/null; rc=$?` then branch on rc (0 = green, 8 = pending, '
    ...
    "(`tee -a $TMPDIR/ci.log`) and read THAT file with `process(action='log')` rather than "
