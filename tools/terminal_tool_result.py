...
def _sudo_annotations(command: str, output: str, env_type: str) -> tuple[str, bool, bool]:
    ...
    Hermes never pipes a sudo password into a command (see
    ``tools.terminal_tool._transform_sudo_command`` and Standing Exclusion 3
    in Update & Merge Policy.md), so there is no session-scoped credential
    cache here to invalidate — ``cache_cleared`` is always False and no
    re-prompt is ever offered. Only the fork's own NOPASSWD-sudoers-rule
    tip (``_handle_sudo_failure``) and the wrong-password output detector
    (``_sudo_wrong_password_failure``) apply.
    """
    from tools.terminal_tool import _handle_sudo_failure, _sudo_wrong_password_failure
    output = _handle_sudo_failure(output, env_type)
    auth_failed = _sudo_wrong_password_failure(output)
    return output, auth_failed, False
