...
def check_command_security(command: str) -> dict:
    ...
    if action == "warn" and findings and all(_is_app_tld_finding(f) for f in findings):
        return _verdict("allow")
    # Suppress the tirith dotfile_overwrite / mass_file_deletion findings for the two benign
    # operation shapes that dominate live false positives (automated-worker audit-log appends
    # and scratch-dir cleanup — 10+ incident blocks over five days of live use). Suppression is
    # shape-bounded and can only ever downgrade warn/block to allow for APPEND-only log writes
    # and scratch-dir deletion; the single-'>' shell-rc overwrite shape keeps its finding.
...
_RE_APPEND_LOG = re.compile(
    r">>{1,2}\s*[\"']?(?:~|\$HOME|\$\{HOME\}|/(?:home|Users)/[^/\s\"']+)?"  # >> target
    r"(?:[^\s\"';&|]*\/)?logs?\/[^\/\s\"';&|]*\.log\b"                       # .../logs/x.log
)
...
def _benign_shape_suppressible(command: str, findings: dict | list) -> bool:
    ...
    if "mass_file_deletion" in rule_ids:
        ...
        try:
            from tools.approval import _is_scratch_cleanup
