...
``tool_call`` — and surfaced on demand. Core Hermes tools never defer by
default; ``include_builtin`` extends deferral to them as an explicit opt-in.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._HERMES_CORE_TOOLS`` are *never* deferred
  by default. ``tools.tool_search.include_builtin: true`` relaxes this as an
  explicit opt-in for installs where builtin schemas dominate the per-turn
  overhead (hermes-agent#6839) — and even then the ``always_include`` set
  stays directly visible, with a hard floor (``ALWAYS_INCLUDE_FLOOR``) of
  agent-loop tools that user config can extend but never remove.
...
# Hard floor under ``always_include``: tools the agent loop intercepts and
# services itself (run_agent's TodoStore / MemoryStore / delegation — see
# model_tools._AGENT_LOOP_TOOLS) plus the clarification primitive. Deferring
# these breaks the loop, so user config can extend ``always_include`` but
# never remove these names from it.
ALWAYS_INCLUDE_FLOOR = frozenset({
    "todo", "memory", "session_search", "delegate_task", "clarify",
})

# Default ``always_include`` when ``include_builtin`` is enabled and the user
# does not set ``tools.tool_search.always_include``: the lean "hot set" from
# the hermes-agent#6839 measurements. Files, terminal, web, code execution
# and skills stay direct; browser / kanban / Home Assistant / media tools
# become deferrable.
DEFAULT_ALWAYS_INCLUDE = ALWAYS_INCLUDE_FLOOR | frozenset({
    "terminal", "process",
...
class ToolSearchConfig:
    ...
    # Opt-in: make builtin (core) tools deferrable too, except always_include.
    include_builtin: bool = False
    # Names that never defer. Only consulted as an *exclusion*: adding a name
    # here never makes an unavailable tool available. Always a superset of
    # ALWAYS_INCLUDE_FLOOR.
    always_include: frozenset = DEFAULT_ALWAYS_INCLUDE
    ...
    def from_raw(cls, raw: Any) -> "ToolSearchConfig":
        ...
        include_builtin = _safe_bool(raw.get("include_builtin"), False)
        always_raw = raw.get("always_include")
        if isinstance(always_raw, (list, tuple)):
            user_always = frozenset(
                str(x).strip() for x in always_raw if str(x).strip()
            )
            # User config extends the floor; it can never remove floor names.
            always_include = user_always | ALWAYS_INCLUDE_FLOOR
        else:
...
def is_deferrable_tool_name(name: str, config: Optional[ToolSearchConfig] = None) -> bool:
    ...
    by default, even when their toolset is technically plugin-provided
    (this protects against accidental shadowing); ``include_builtin`` makes
    them eligible too. Names in ``always_include`` never defer regardless
    of source — pinning applies to MCP/plugin tools as well.

    ``config`` defaults to the user config; callers iterating many names
    should resolve it once and pass it through (``classify_tools`` does).
    """
    if name in BRIDGE_TOOL_NAMES:
        return False
    if config is None:
        config = load_config()
    if name in config.always_include:
        return False
    if name in _core_tool_names():
        return config.include_builtin
...
def classify_tools(
    ...
    every always-include/core tool, plus any tool we can't classify.
...
def _classify_source(name: str) -> Tuple[str, str]:
    ...
    try:
        ...
        if is_core:
            # Only present in the catalog when include_builtin is enabled.
            return ("builtin", entry.toolset if entry is not None else "")
        if entry is None:
            return ("other", "")
        return ("plugin", entry.toolset)
    except Exception:
        return ("builtin", "") if is_core else ("other", "")
...
def assemble_tool_defs(
    ...
    *,
    ...
    plus builtin tools outside ``always_include`` when ``include_builtin``
    is enabled — are stripped from the visible list and replaced with the
    three bridge tools.
