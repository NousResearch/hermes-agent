...
class ToolSearchConfig:
    ...
    # Per-hit description cap for tool_search results; full metadata stays reachable via tool_describe.
    max_description_chars: int = 400
    # Cap on the top-level description tool_describe returns; the parameter schema is never truncated.
    max_schema_description_chars: int = 1200
    ...
    def from_raw(cls, raw: Any) -> "ToolSearchConfig":
        ...
        return cls(
            ...
            max_description_chars=_clamped_int(raw.get("max_description_chars"), 400, 80, 1000),
            max_schema_description_chars=_clamped_int(
                raw.get("max_schema_description_chars"), 1200, 200, 4000),
...
def _clip_description(text: str, cap: int = 400) -> str:
    ...
    The cap itself is configurable (tools.tool_search.max_description_chars)
    because chatty MCP servers blow the catalog budget on descriptions alone.
    """
    return text if len(text) <= cap else text[:cap] + "…"


def _shared_tool_record(entry: CatalogEntry, *, max_description_chars: int = 400) -> Dict[str, Any]:
    ...
    return {"source": entry.source, "source_name": entry.source_name,
            # cap chatty MCP descriptions (tools.tool_search.max_description_chars)
            "description": _clip_description(entry.description or "", max_description_chars),
...
def dispatch_tool_search(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    for position, query in enumerate(queries):
        ...
        for h in hits:
            tools_map.setdefault(
                h.name, _shared_tool_record(h, max_description_chars=config.max_description_chars))
...
def dispatch_tool_describe(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    for name in names:
        ...
        if fn is not None:
            # Cap only the top-level description (tools.tool_search.max_schema_description_chars);
            # the parameter schema is preserved so calls stay reliable.
            tools[name] = {"description": (fn.get("description", "") or "")[:config.max_schema_description_chars],
