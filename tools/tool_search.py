...
default, with an explicit opt-in for providers that reject large tool arrays.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._HERMES_CORE_TOOLS`` stay visible unless
  ``defer_core`` is explicitly enabled. ``always_visible`` pins the small
  bootstrap set needed to discover and invoke every deferred capability.
    ...
    def from_raw(cls, raw: Any) -> "ToolSearchConfig":
        ...
        if isinstance(always_visible_raw, (list, tuple, set, frozenset)):
            always_visible = frozenset(
                name
                for value in always_visible_raw
...
def _core_tool_names() -> frozenset[str]:
    """Return the set of built-in Hermes core tool names.
...
def is_deferrable_tool_name(
    ...
    OR it is a core tool and ``defer_core`` is explicitly enabled. Core tools
    and plugins named in ``always_visible`` remain model-visible. Unknown tools
    remain visible defensively so classification can never silently drop one.
    """
    if config is None:
...
def classify_tools(
    ...
    every pinned/default-visible core tool, plus any tool we can't classify.
...
def assemble_tool_defs(
    ...
    *,
    ...
    tools. Core tools are deferred only when ``defer_core`` is explicitly set;
    ``always_visible`` tools remain in the model-facing list.
