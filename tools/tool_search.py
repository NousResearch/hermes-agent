...
def dispatch_tool_describe(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    for name in names:
        ...
        if fn is not None:
            ...
            # A globally registered tool may be disabled for this profile or
            # session. Never tell the model to call it directly unless this
            # session actually exposed that direct tool.
    ...
    * the trajectory recorder.
    ...
    if not is_deferrable_tool_name(name, load_config_readonly().effective_defer_tools):
        if current_tool_defs is not None and _registry_entry(name) is not None:
            if name not in frozenset(_tool_def_names(current_tool_defs)):
                return None, {}, f"'{name}' is not available in this session. Use tool_search to find tools you can call."
