...
# Supported STT models for the OpenAI backend. This list is intentionally
# small; we still allow custom model names but emit a warning so
# misconfigurations are easier to spot.
...
def _validate_audio_file(file_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error("Failed to access audio file %s: %s", file_path, e, exc_info=True)
