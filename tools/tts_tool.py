...
async def _generate_edge_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # aaocubo V7.3 tts: pass through rate/pitch/volume from config
    _rate_override = edge_config.get("rate")
    if _rate_override:
        kwargs["rate"] = str(_rate_override)
    _pitch = edge_config.get("pitch")
    if _pitch:
        kwargs["pitch"] = str(_pitch)
    _volume = edge_config.get("volume")
    if _volume:
        kwargs["volume"] = str(_volume)
