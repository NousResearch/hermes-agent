...
# ===========================================================================
def _resolve_minimax_api_key(mm_config: Dict[str, Any]) -> str:
    api_key_env = str(mm_config.get("api_key_env") or "").strip()
    candidates = [api_key_env] if api_key_env else []
    candidates.extend(["MINIMAX_CN_API_KEY", "MINIMAX_API_KEY"])
    ...
    expected = api_key_env or "MINIMAX_CN_API_KEY or MINIMAX_API_KEY"
    raise ValueError(f"{expected} not set. Get one at https://platform.minimax.io/")


def _coerce_minimax_number(value: Any) -> Any:
...
def _generate_minimax_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    The default api.minimax.chat/v1/text_to_speech path uses a simple payload
    and may return raw audio bytes directly. CN token-plan custom voices can use
    api.minimaxi.com/v1/t2a_v2 with voice_setting/audio_setting and JSON
    hex-encoded audio.
    ...
    api_key = _resolve_minimax_api_key(mm_config)
    model = mm_config.get("model", DEFAULT_MINIMAX_MODEL)
    voice_id = mm_config.get("voice_id", DEFAULT_MINIMAX_VOICE_ID)
    base_url = mm_config.get("base_url", DEFAULT_MINIMAX_BASE_URL)

    use_voice_settings = (
        str(base_url).rstrip("/").endswith("/t2a_v2")
        or any(
            key in mm_config
            for key in (
                "speed",
    ...
    if use_voice_settings:
        voice_setting = {"voice_id": voice_id}
        speed = _coerce_minimax_number(mm_config.get("speed"))
        volume = _coerce_minimax_number(mm_config.get("vol", mm_config.get("volume")))
        pitch = _coerce_minimax_number(mm_config.get("pitch"))
        ...
        payload = {
            "model": model,
            "text": text,
            "stream": bool(mm_config.get("stream", False)),
            "voice_setting": voice_setting,
        }

        audio_setting = {}
        sample_rate = mm_config.get("sample_rate")
        bitrate = mm_config.get("bitrate")
        audio_format = mm_config.get("audio_format", mm_config.get("format"))
        channels = mm_config.get("channels", mm_config.get("channel"))
        if sample_rate:
            audio_setting["sample_rate"] = int(sample_rate)
        if bitrate:
            audio_setting["bitrate"] = int(bitrate)
        if audio_format:
            audio_setting["format"] = str(audio_format)
        if channels:
            audio_setting["channel"] = int(channels)
        if audio_setting:
            payload["audio_setting"] = audio_setting

        language_boost = mm_config.get("language_boost", mm_config.get("language"))
        if language_boost:
            payload["language_boost"] = str(language_boost)
    else:
        payload = {
            "model": model,
            "text": text,
...
def check_tts_requirements() -> bool:
    ...
    try:
        ...
        pass
    if get_env_value("MINIMAX_CN_API_KEY") or get_env_value("MINIMAX_API_KEY"):
        return True
...
if __name__ == "__main__":
    ...
    minimax_key_status = (
        "API key set"
        if get_env_value("MINIMAX_CN_API_KEY") or get_env_value("MINIMAX_API_KEY")
        else "not set (MINIMAX_CN_API_KEY or MINIMAX_API_KEY)"
    )
    print(f"  MiniMax:    {minimax_key_status}")
