...
class TermuxAudioRecorder:
    ...
    def start(self, on_silence_stop=None) -> None:
        ...
        try:
            subprocess.run(command, capture_output=True, text=True, timeout=15, check=True, stdin=subprocess.DEVNULL)  # windows-footgun: ok — Termux (Android) only, never runs on Windows
    ...
    def _stop_termux_recording(self) -> None:
        ...
        if not mic_cmd:
            return
        subprocess.run([mic_cmd, "-q"], capture_output=True, text=True, timeout=15, check=False, stdin=subprocess.DEVNULL)  # windows-footgun: ok — Termux (Android) only, never runs on Windows
