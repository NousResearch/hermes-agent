...
async def process_content_with_llm(
    ...
    try:
        ...
        if content_len > CHUNK_THRESHOLD:
            ...
            return await _process_large_content_chunked(content, context_str, model, CHUNK_SIZE, MAX_OUTPUT_SIZE)
        ...
        if processed_content:
            ...
            logger.info(
                "Content processed: %d -> %d chars (%.1f%%)", content_len, processed_length, compression_ratio * 100
            )
...
async def _call_summarizer_llm(
    content: str, context_str: str, model: str, max_tokens: int = 4000, is_chunk: bool = False, chunk_info: str = ""
) -> Optional[str]:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    "LLM API call failed (attempt %d/%d): %s", attempt + 1, max_retries, str(api_error)[:100]
                )
    ...
    async def summarize_chunk(chunk_idx: int, chunk_content: str) -> tuple[int, Optional[str]]:
        ...
        try:
            ...
            summary = await _call_summarizer_llm(
                chunk_content, context_str, model, max_tokens=2000, is_chunk=True, chunk_info=chunk_info
            )
            if summary:
                logger.info(
...
async def web_crawl_tool(
    ...
    try:
        ...
        if hasattr(crawl_result, "data"):
            ...
            if not data_list:
                logger.debug(
                    "CrawlJob attributes: %s", [attr for attr in dir(crawl_result) if not attr.startswith("_")]
                )
