...
_KNOWN_WEB_BACKENDS = frozenset(
    {"local-browser", "parallel", "firecrawl", "tavily", "exa", "searxng", "brave-free", "ddgs", "xai"}
)

# Built-in web backends whose availability is driven by hardcoded env-var /
# package / OAuth probes below. Any name outside this set is a candidate
# plugin-registered provider and is resolved through the web_search_registry.
# ``xai`` remains built-in because availability is probed via xAI credentials,
# not a registered WebSearchProvider.
...
def _get_backend(capability: str = "search") -> str:
    ...
    if configured in _KNOWN_WEB_BACKENDS or _registered_web_provider(configured) is not None:
        return configured
    ...
    # Final plugin fallback before the keyless Parallel terminal default: walk
    # registered custom providers so a plugin with no built-in credential probe
    # can still resolve. Built-in names are already covered above.
    ...
    # Keyless Parallel free MCP — always available, the intended no-key default
    # for both search and extract.
    return "parallel"
...
def _is_backend_available(backend: str) -> bool:
    ...
    provider's ``is_available()`` via the web_search_registry. Built-in
    backends keep their cheap hardcoded probes below.
    """
...
def check_web_api_key() -> bool:
    ...
    if any(
        ...
        return True
    # Any custom plugin-registered provider with its own availability gate.
    # Do not delegate to get_active_*_provider() here: the registry can return
    # the keyless built-in Parallel fallback, but check_fn metadata should only
    # light up from explicit credentials/configured local availability or a real
    # custom provider.
    for provider in _list_registered_web_providers():
        if provider.name in _LEGACY_WEB_BACKENDS:
            continue
