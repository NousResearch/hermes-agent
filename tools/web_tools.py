        ...
        if provider is None:
            ...
            try:
                ...
                if (
                    ...
                    # Mirror case: the call rode the keyless ring and every
                    # vendor throttled. One-shot keyed retry so a configured
                    # API key still answers when the free tier is dry.
                    _err = str(response_data.get("error", ""))
                    if _backstop_eligible(provider, _err):
                        response_data = _backstop_search(
                            provider, _err, query, limit
                        )
    ...
    try:
        ...
        if not safe_urls:
            ...
            try:
                ...
                if (
                ...
                elif results and all(r.get("error") for r in results):
                    # Mirror case: the batch rode the keyless ring and every
                    # vendor throttled — retry once on the keyed path. Policy
                    # blocks are intentional refusals and never backstopped.
                    ...
                    if not any(
                        ...
                        results = await asyncio.to_thread(
                            _backstop_extract, provider, safe_urls, results
                        )
