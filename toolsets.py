...
TOOLSETS = {
    ...
    "reference": _ts("Read-only local reference lookup", ["read_file", "search_files"]),
    ...
    # Jarvis: the API server is an embedded backend surface — keep it to read-only reference
    # lookup plus clarification; everything else (web, terminal, browser, HA, …) stays off.
    "hermes-api-server": _ts(
        "OpenAI-compatible API server with minimal read-only reference and clarification tools",
