...
def _compute_host_turn_frame(
    ...
    return {
        ...
        **({"display_kind": display_kind} if display_kind else {}), "history": history,
        ...
        "draft_image_paths": list(draft_image_paths or []),
...
def _submit_prompt_to_compute_host(
    ...
    try:
        draft_image_paths = _validate_draft_image_paths(session, draft_image_paths or [])
    except (ValueError, OSError) as exc:
        return _err(rid, 4017, str(exc))
    frame = _compute_host_turn_frame(rid, sid, session, text, image_paths=image_paths,
                                     queued_prompt_generation=queued_prompt_generation,
                                     display_kind=display_kind, draft_image_paths=draft_image_paths)
