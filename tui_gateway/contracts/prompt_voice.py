...
class PromptSubmitParams(SessionParams):
    ...
    # This turn's images (host paths, as ``image.attach`` takes), bound to this submit instead of the
    # session's shared ``image.attach`` staging slot. Advertised by ``gateway.ready.atomic_image_submit``.
