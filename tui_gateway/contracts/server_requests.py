...
class ClarifyFormQuestion(Params):
...
class ClarifyFormRequestParams(ServerRequestParams):
    """Every question of one clarify_form call, rendered as a single card the user submits once."""

    questions: list[ClarifyFormQuestion]


class ClarifyFormResult(Result):
    """``answers`` maps question id → answer; a response without it is a cancel."""

    answers: dict[str, str] | None = None


server_request("clarify_form", params=ClarifyFormRequestParams, result=ClarifyFormResult,
               doc="The clarify_form tool: one card carrying several questions, answered in one submit.")
