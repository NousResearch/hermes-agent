...
def _profile_ui_meta_fields(row: dict, profile_dir) -> None:
    ...
    # for the row's description/display name), on a listing the roster polls every 5s per
    # connection — so it is reused while the file has not moved (#117383). The ui_meta CAS writer
    # below keeps reading it raw and uncached: it writes the document back.
    from tui_gateway.profile_ui_meta_cache import cached_ui_meta_fields
