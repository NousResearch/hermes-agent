...
def _lock_in_submit_turn(
    ...
    fields = {}
    # The caller holds _session_turn_admission, including history_lock, continuously
    # from the idle check through this claim.
    ...
    # Keep later submits behind this acceptance decision even if a fast worker clears
    # running before this handler returns.
    ...
    while True:
        with _session_turn_admission(session) as admitted:
            ...
            if not session.get("running") and session.get("_submit_acceptance_gate") is None:
                if not admitted:
                    err, survivor_fields = (
                        _err(rid, 5035, "backend is retiring; reconnect to continue"), {})
                else:
