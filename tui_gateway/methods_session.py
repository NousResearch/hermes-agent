...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        with _session_resume_lock:
            ...
            # Use the session's profile-scoped DB, not the launch profile's shared
            # handle, so remote/profile resume reads agent_mode from the right db.
            with _session_db(session) as mode_db:
                if mode_db:
                    try:
                        db_session = mode_db.get_session(target)
                        if db_session:
