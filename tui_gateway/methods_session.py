...
def _(rid, params: dict) -> dict:
    ...
    # End-user id forwarded by the calling backend — kept on the session so every turn can re-bind
    # HERMES_SESSION_USER_ID via _set_session_context, mirroring how `source`/`cwd` are threaded
    # through. Not authenticated by Hermes itself; the caller is trusted to have verified it upstream.
    hermes_user_id = _str_param(params, "user_id")
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "hermes_user_id": hermes_user_id, "slash_worker": None, "tool_progress_mode": _load_tool_progress_mode(), "tool_started_at": {},
