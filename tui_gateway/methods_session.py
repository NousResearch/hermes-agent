...
def _session_list_by_title(rid, db, title_lookup: str, *, source: str = "") -> dict:
    ...
    row = (db.get_session_by_title(title_lookup, source=source)
           if source else db.get_session_by_title(title_lookup))
    canonical_room = (
        source == "bot_room" and title_lookup.startswith("Group: ")
        and row is not None and row.get("source") == "bot_room")
    if canonical_room:
        if row.get("archived"):
...
def _(rid, params: dict, db) -> dict:
    try:
        if title_lookup := _str_param(params, "title"):
            return _session_list_by_title(rid, db, title_lookup, source=_str_param(params, "source"))
