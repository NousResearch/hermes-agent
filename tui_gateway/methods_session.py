...
def _(rid, params: SessionSetHiddenParams) -> SessionSetHiddenResult | dict:
    ...
    session = srv._sessions.get(str(params.session_id or ""))
...
def _(rid, params: SpawnTreeLoadParams) -> SpawnTreeLoadResult | dict:
    ...
    if not raw_path:
        return srv._err(rid, 4000, "path required")
    try:
        (resolved := Path(raw_path).resolve()).relative_to(srv._spawn_trees_root().resolve())
    except (ValueError, OSError) as exc:
        return srv._err(rid, 4030, f"path outside spawn-trees root: {exc}")
    try:
        payload = json.loads(resolved.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return srv._err(rid, 5000, f"spawn_tree.load failed: {exc}")
    if not isinstance(payload, dict):
        return srv._err(rid, 5000, "spawn_tree.load failed: snapshot is not a JSON object")
