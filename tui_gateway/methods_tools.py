...
def _(rid, params) -> ReloadMcpResult | dict:
    ...
    def _do_full_reload() -> None:
        ...
        with srv._session_profile_runtime_scope({"profile_home": None}):
...
def _(rid, params, session) -> RollbackRestoreResult | dict:
    ...
    def go(mgr, cwd):
        if reason := srv._container_checkpoint_refusal(session, mgr, cwd):
...
def _(rid, params, session) -> RollbackDiffResult | dict:
    ...
    def go(mgr, cwd):
        ...
        if reason := srv._container_checkpoint_refusal(session, mgr, cwd):
            return srv._err(rid, 5022, reason)
        r = mgr.diff(cwd, srv._resolve_checkpoint_hash(mgr, cwd, target))
        raw = r.get("diff", "")[:4000]
        rendered = srv.render_diff(raw, session.get("cols", 80))
        return RollbackDiffResult(stat=r.get("stat", ""), diff=raw, rendered=rendered or None)
    return srv._with_checkpoints(session, go)
...
def _container_checkpoint_refusal(session, mgr, cwd) -> str | None:
    ...
    tokens = srv._set_session_context(task_id, cwd=cwd)
    try:
