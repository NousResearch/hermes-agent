"""Pet sprite payload + generation helpers (moved verbatim from server.py).

Wave-1 godfile extraction (shard s3, cluster c22): sprite-payload building,
frame-count probing, generation staging, and cooperative cancellation. Bodies
are byte-identical to their pre-split server.py form; all module state these
functions close over (payload cache + LRU trim, cancel-token set, reference
image limits) moved with them.
"""
