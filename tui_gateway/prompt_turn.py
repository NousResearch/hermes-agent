...
def _goal_followup_after_turn(
    ...
    try:
        ...
        if recovery_notice:
            ...
            render_notification(
                lambda: srv._emit("status.update", sid, StatusUpdatePayload(kind="goal", text=recovery_notice)),
...
def _invoke_agent(
    ...
    srv._adopt_submit_user_row(session, agent, run_kwargs["persist_user_message"], text)
...
def _run_prompt_submit(
    ...
    def run():
        ...
        if followup is not None:
            srv._run_post_turn_followups(rid, sid, session, *followup)
