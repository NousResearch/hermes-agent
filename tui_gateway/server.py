...
def _get_usage(agent) -> dict:
    ...
    # carry no api_duration, so turn_model_seconds stays unavailable).
    with contextlib.suppress(Exception):
        ...
        if _turn_model is not None:
            ...
            if _turn_model == _turn_model and 0 <= _turn_model < 1e6:
                usage["turn_model_seconds"] = round(_turn_model, 1)
        for _key in ("_turn_started_at", "_turn_first_token_at"):
