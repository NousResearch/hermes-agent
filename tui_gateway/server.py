...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_hermes_home).resolve():
        return None
    # A profile-scoped process (isolated dashboard / `-p <name>`, whose own
    # HERMES_HOME is NOT the machine root) must treat "default" as its OWN
    # launch profile — never the machine root's default. Otherwise the
    # desktop's profile-less session.create writes the agent's session into
    # the machine DB while the dashboard serves a named profile (#88897,
    # #78423). A machine-root process keeps the historical resolution.
...
def _process_is_profile_scoped() -> bool:
    """True when this process's own HERMES_HOME is not the machine root."""
    try:
