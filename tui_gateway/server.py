...
def _agent_cbs(sid: str) -> dict:
    ...
    # When enabled this overrides the interim-assistant emit above, routing
    # commentary to the Working lane instead. Leaving it off keeps the upstream
    # behaviour above, byte-identical. Upstream fires this via
    # _fire_streamed_codex_commentary only when the callback is set and
    # show_commentary is on; the per-turn _delivered_interim_texts dedup keeps
    # the same text out of the final. Live sessions are kept in sync on toggle
    # by _apply_commentary_lane_to_live_sessions.
