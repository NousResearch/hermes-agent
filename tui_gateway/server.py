...
def _normalize_todo_state(value: object) -> dict | None:
    ...
    if not todos and revision == 0:
        return None
    out: dict = {"todos": todos, "revision": revision}
    # KENSEI CUSTOM: durable-state generation (mutation authority for the
    # desktop controls) rides alongside revision when the store provides it.
...
def _session_todo_state(session: dict) -> dict | None:
    ...
    snapshot = getattr(store, "snapshot_state", None) or getattr(store, "snapshot", None)
...
def inject_external_message(
    ...
    *,
    ...
    ``target_session`` is the dashboard session id (or an exact
    ...
    with _sessions_lock:
        ...
        if session is None:
            ...
            for _sid, _s in _sessions.items():
                if (
                    str(_s.get("session_key") or "") == target_session
