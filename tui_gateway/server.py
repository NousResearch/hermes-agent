...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        hermes_user_id = ""
        # A close_on_disconnect session (dashboard/sidecar chat) is reaped the moment its WS drops, and
        # its async-completion drain re-enters results as a *separate* later turn. Left async, a
        # backgrounded delegate_task would surface subagent output straight to the UI while the
        # spawning turn is still running (and lose it entirely if the socket has closed). Declare
        # async delivery unsupported so delegate_task takes its synchronous path — children still run
        # in parallel, but join in-turn — and only the spawning agent's own reply reaches the user.
        async_delivery = True
        if sess is not None:
        ...
        return set_session_vars(
            ...
            ui_session_id=ui_session_id, cron_session="", user_id=hermes_user_id,
...
def _clarify_form_block(sid: str, questions) -> str:
    """Bridge the clarify_form tool callback onto a ``clarify_form`` server request: one card carrying
    every question, answered in a single submit. Returns the answers map as JSON (`""` when the user
    cancelled, the card timed out, or the client does not answer server requests)."""
    from tui_gateway import server_requests
    wire = [{"id": str(q.get("id") or ""), "question": str(q.get("question") or ""),
             "choices": q.get("choices"), "multiSelect": bool(q.get("multiSelect"))}
            for q in (questions or [])]
    result = server_requests.send("clarify_form", sid, {"questions": wire},
                                  timeout=_clarify_timeout_seconds())
    if not result or "answers" not in result:
        return ""
    return json.dumps(result.get("answers") or {}, ensure_ascii=False)
