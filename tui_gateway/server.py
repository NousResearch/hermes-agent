...
def _profile_scoped(handler):
    ...
    def wrapper(rid, params):
        profile = str(getattr(params, "profile", None) or "").strip()
        if profile:
            home = _profile_home(profile)
            profile_home = str(home) if home else None
        else:
            session = _sessions.get(str(getattr(params, "session_id", None) or ""))
...
_server_requests.bind_sinks(lambda frame: write_json(frame), lambda event, sid, payload: _emit(event, sid, payload),
                            lambda sid: _session_transports._session_client_answers_requests(sid))
