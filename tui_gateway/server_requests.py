...
def send(method: str, sid: str, params: Params, *, timeout: float | None,
    ...
    if timed_out:
        ...
        if req.qids is not None:
            ...
            return ClarifyAnswers(answers=dict(req.locked), timed_out=True)
        return None
...
def resolve_response(frame: dict) -> bool:
    ...
    if "error" in frame:
        ...
        try:
            result = TypeAdapter(contracts.SERVER_REQUESTS[req.method].result).validate_python(frame["result"])
