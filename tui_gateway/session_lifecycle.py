...
def _ensure_active_session_slot(sid: str, session: dict) -> str | None:
    ...
    lease, limit_message = srv._claim_active_session_slot(
        key, live_session_id=sid, surface=srv._session_source(session), profile_home=session.get("profile_home"))
...
def _detached_lease_holder(session: dict, key: str) -> tuple[str, dict] | None:
    ...
    for other_sid, other in srv._sessions.items():
        if (other is session or other.get("_finalized") or str(other.get("session_key") or "") != key
                or not srv._live_profile_matches(other, session.get("profile_home"))):
            continue
        lease = other.get("active_session_lease")
        if lease is not None and not lease.released and srv._transport_is_dead(other.get("transport")):
...
def _settle_isolated_turn_before_close(session: dict) -> None:
    ...
    if not session.get("_compute_host_turn_id") or not srv._session_uses_compute_host(session):
        return
    with contextlib.suppress(Exception):
        _interrupt_session_turn(_lifecycle_own_sid(session), session)
    deadline = time.monotonic() + srv._TURN_SETTLE_BEFORE_CLOSE_SECONDS
    ...
    logger.warning("isolated turn still live after %.1fs close grace; holding lease for %s until the child settles",
                   srv._TURN_SETTLE_BEFORE_CLOSE_SECONDS, session.get("session_key"))
