...
def _notif_poll_kanban_scoped(sid: str, session: dict) -> None:
    ...
    for text in texts:
        ...
        render_notification(lambda: _emit("status.update", sid, StatusUpdatePayload(kind="process", text=text)),
...
def _notif_handle_event(sid, session, evt, emitted, registry, fmt, deferred, completions=None, *, owned=False) -> bool:
    ...
    if dedup_key not in emitted:
        ...
        from agent.notification_presentation import diagnostic_process_event
        from gateway.warning_notifications import render_notification
        render_notification(lambda: _emit("status.update", sid, StatusUpdatePayload(kind="process", text=display_text)),
