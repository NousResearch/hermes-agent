...
def _notif_submit(rid: str, sid: str, session: dict, text: str, what: str, **kwargs) -> None:
    ...
    try:
        ...
        with srv._session_profile_runtime_scope(session):
            render_notification(lambda: srv._emit("message.start", sid), platform="tui",
...
def _kb_poll_board(_kb, slug: str, session_key: str) -> list:
    ...
    with contextlib.closing(conn):
        ...
        for sub in subs:
            ...
            for ev in events:
                text = srv._format_kanban_event_text(sub, task, ev, slug)
                if text:
...
def _notif_poll_kanban_scoped(sid: str, session: dict) -> None:
    ...
    for text in texts:
        ...
        render_notification(lambda: srv._emit("status.update", sid, StatusUpdatePayload(kind="process", text=text)),
...
def _notif_handle_event(sid, session, evt, emitted, registry, fmt, deferred, completions=None, *, owned=False) -> bool:
    ...
    if dedup_key not in emitted:
        ...
        from agent.notification_presentation import diagnostic_process_event
        from gateway.warning_notifications import render_notification
        render_notification(lambda: srv._emit("status.update", sid, StatusUpdatePayload(kind="process", text=display_text)),
