...
def _kb_board_key(_kb, board_meta) -> tuple[str, str]:
    ...
    slug = (board_meta or {}).get("slug") or _owner_kanban_boards.DEFAULT_BOARD
    db_path = (board_meta or {}).get("db_path")
    try:
        return slug, str(Path(db_path).expanduser().resolve() if db_path else _owner_kanban_db.kanban_db_path(slug).resolve())
    except Exception:
