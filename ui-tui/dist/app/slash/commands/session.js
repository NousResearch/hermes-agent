...
export const sessionCommands = [
    ...
    },
    {
        ...
        run: (arg, ctx) => {
            ...
            ctx.gateway.rpc('prompt.btw', { session_id: ctx.sid, text: arg }).then(ctx.guarded(() => {
    ...
    },
    {
        ...
        run: (arg, ctx) => {
            ...
            ctx.gateway.rpc('config.set', { key: 'model', session_id: ctx.sid, value: arg.trim() }).then(ctx.guarded(r => {
                if (!r.value) {
    ...
    },
    {
        ...
        run: (arg, ctx) => {
            ...
            ctx.gateway
                ...
                .then(ctx.guarded(r => r.value && ctx.transcript.sys(`reasoning: ${r.value}`)));
        }
    },
    {
