...
export function useComposerState({ gw, onClipboardPaste, submitRef }) {
    const [input, setInput] = useState('');
    const [inputBuf, setInputBuf] = useState([]);
    const [pasteSnips, setPasteSnips] = useState([]);
    const isBlocked = useStore($isBlocked);
    const { queueRef, queueEditRef, queuedDisplay, queueEditIdx, enqueue, dequeue, replaceQ, setQueueEdit, syncQueue } = useQueue();
