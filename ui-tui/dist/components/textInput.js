...
function cursorLayout(value, cursor, cols) {
...
function offsetFromPosition(value, row, col, cols) {
    if (!value.length) {
        return 0;
    }
...
export function TextInput({ columns = 80, value, onChange, onPaste, onSubmit, mask, placeholder = '', focus = true }) {
    ...
    const commit = (next, nextCur, track = true) => {
    ...
    return (_jsx(Box, { onClick: (e) => {
            ...
            setCur(next);
            curRef.current = next;
        }, ref: boxRef, children: _jsx(Text, { wrap: "wrap", children: rendered }) }));
}
