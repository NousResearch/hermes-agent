...
export function useVirtualHistory(scrollRef, items, { estimate = ESTIMATE, overscan = OVERSCAN, maxMounted = MAX_MOUNTED, coldStartCount = COLD_START } = {}) {
    ...
    useSyncExternalStore(useCallback((cb) => (hasScrollRef ? scrollRef.current?.subscribe(cb) : null) ?? (() => () => { }), [hasScrollRef, scrollRef]), () => {
    ...
    const offsets = useMemo(() => {
        void ver;
        const out = new Array(items.length + 1).fill(0);
    ...
    const measureRef = useCallback((key) => {
        ...
        if (!fn) {
            fn = (el) => (el ? nodes.current.set(key, el) : nodes.current.delete(key));
