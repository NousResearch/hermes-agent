...
const formatUnknown = (value: null | string | undefined): string => (value?.trim() ? value.trim() : UNKNOWN)
const formatDetected = (value: null | string | undefined): string => (value?.trim() ? value.trim() : NOT_DETECTED)
...
const summarizeTerminal = (signals: TerminalSignals, capabilities: TerminalCapabilities): string => {
  ...
  const bridge = signals.multiplexer.tmux
    ? 'tmux'
    : signals.multiplexer.screen
      ? 'screen'
      : signals.multiplexer.zellij
        ? 'zellij'
        : signals.multiplexer.cy
          ? 'cy'
          : ''
...
const envRow = (label: string, value: null | string | undefined): string => `${label}: ${formatDetected(value)}`

const redactIp = (value: string | undefined): string | undefined =>
  value
    ?.split(/(\s+)/)
    .map(part => (isIP(part) ? '<redacted>' : part))
    .join('')
...
const buildEnvironmentRows = (signals: TerminalSignals): string[] => {
  ...
  if (signals.ssh.hasSshConnection || signals.ssh.hasSshClient || signals.ssh.hasSshTty) {
    ...
    rows.push(envRow('SSH_CLIENT', redactIp(signals.ssh.client)))
...
export function formatTerminalDoctor(env: TerminalEnvironment): string {
  ...
  const lines: string[] = [
    '=== Terminal Doctor ===',
    `transport: ${summarizeTransport(env.capabilities)}`,
