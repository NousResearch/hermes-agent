...
const TERMINAL_REGISTRY_UNSORTED: TerminalProfile[] = [
...
export const TERMINAL_REGISTRY: TerminalProfile[] = [...TERMINAL_REGISTRY_UNSORTED].sort((a, b) => b.priority - a.priority)
...
/**
 * Find the first profile with any positive signal, highest priority first.
 *
 * Detection fields are OR-style positive signals. A present matching envVar,
 * TERM_PROGRAM, TERM pattern, or custom check can select the profile. A
 * set-but-mismatched TERM_PROGRAM or TERM pattern rejects that profile early,
 * so a stale generic TERM cannot override a more specific terminal signal.
 */
