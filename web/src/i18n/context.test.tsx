...
describe("I18nProvider initial locale", () => {
  it("uses a supported browser language", async () => {
  ...
  it("uses Traditional Chinese for zh-TW", async () => {
    setBrowserLanguage("zh-TW");

    expect(await renderLocale()).toBe("zh-hant");
  });

  it("uses Traditional Chinese for zh-Hant-TW", async () => {
    setBrowserLanguage("zh-Hant-TW");

    expect(await renderLocale()).toBe("zh-hant");
  });

  it("falls back to English for an unsupported browser language", async () => {
    setBrowserLanguage("xx-XX");

    expect(await renderLocale()).toBe("en");
  });
});
