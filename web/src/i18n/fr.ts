  ...
  skills: {
    wisdom: {
      notificationPreferences: {
        ...
        scope: "Vos notifications proactives sur les clients de cette organisation. La consultation et le partage manuels restent disponibles.",
        ...
        pending: "Votre choix est enregistré localement et attend la synchronisation.",
        failed: "Échec de la synchronisation. Actualisez les paramètres avant de choisir à nouveau.",
        conflict: "Votre préférence a changé sur un autre client. Actualisez pour la vérifier.",
        expired: "Ce choix a expiré. Actualisez les paramètres pour réessayer.",
      },
      tab: 'Collectif',
      browseHub: 'Parcourir le Hub',
      title: 'Savoir collectif',
      loading: 'Chargement du Savoir collectif',
      unavailable: 'Le Savoir collectif est indisponible.',
      setup: 'Le Savoir collectif n’est pas configuré pour ce profil.',
      setupDisclosure:
        'La qualification des candidats reste dans ce profil. Seuls le contenu du brouillon privé approuvé par son propriétaire, le texte de l’auteur, les métadonnées déclaratives du manifeste et l’état des installations gérées parviennent à la Gateway.',
      setupAction: 'J’ai compris — configurer ce profil',
      settingUp: 'Configuration…',
      scanLocal: 'Analyser les compétences locales',
      ...
      draftState: state => {
        const labels: Record<string, string> = {
          vetting: 'Vérification du serveur en cours',
          ready: 'Prêt pour votre examen',
          owner_approved: 'Approuvé par vous',
          publishing: 'Publication en cours',
          pending_moderation: 'En attente de l’approbation de l’administrateur du collectif',
          changes_requested: 'Modifications demandées'
        }
        return labels[state] || state.replaceAll('_', ' ')
      },
      serverScanPassed: 'analyse serveur réussie',
      noDescription: 'Aucune description rédigée par le propriétaire.',
      managedInstalls: 'installations gérées',
      close: 'Fermer',
      readEvery: 'Lisez chaque fichier brut. L’approbation est liée aux trois hachages exacts affichés ci-dessous.',
      editReview:
        'Modifiez ici la description, SKILL.md ou le manifeste déclaratif. L’enregistrement crée une nouvelle révision privée, relance les analyses et renvoie de nouveaux hachages ; il ne réécrit pas votre compétence locale source.',
      unsavedChanges: 'Ces modifications n’ont pas été analysées. Enregistrez et relancez l’analyse avant d’approuver.',
      saveAndRescan: 'Enregistrer et relancer l’analyse',
      savingRevision: 'Enregistrement et nouvelle analyse…',
      resetChanges: 'Abandonner les modifications',
      reviewedHashes: 'Hachages de la révision actuellement examinée par le serveur',
      prepareTitle: 'Examiner le paquet local avant l’envoi',
      prepareNotice:
        'Ces champs restent dans ce profil jusqu’à ce que vous les soumettiez explicitement à un examen serveur privé réservé au propriétaire. Les compteurs et motifs de qualification locaux ne sont jamais inclus.',
      ownerDescription: 'Description rédigée par le propriétaire',
      ...
      decline: 'Refuser',
      checkUpdates: count => `Rechercher des mises à jour${count ? ` (${count})` : ''}`,
      checking: 'Vérification…',
      refreshShared: 'Actualiser les compétences partagées',
      refreshingShared: 'Actualisation…',
      ...
      updateModeDefault: 'Utiliser la valeur par défaut de l’organisation',
      updateModeManual: 'Manuelles',
      updateModeAutomatic: 'Automatiques avec notification',
      updateModeRequired: 'Obligatoires',
      updateModeHelp:
        'Gateway applique la politique actuelle de votre organisation. Les changements sensibles restent soumis à votre approbation.',
      updateModePlan: mode => `Mises à jour futures : ${mode}`,
      activityReady: count => `${count} nouvelle${count === 1 ? ' notification' : 's notifications'}`,
      decisionPublished: skill => `${skill} a été approuvée et est maintenant partagée avec votre équipe.`,
      decisionChanges: skill => `${skill} doit être modifiée avant de pouvoir être partagée.`,
      ...
      takedownNotice: skill => `${skill} a été retirée du collectif.`,
      markSeen: 'Tout marquer comme lu',
      install: 'Installer…',
      uninstall: 'Désinstaller…',
      checkSkill: 'Vérifier cette compétence',
      updateAvailable: version => (version ? `Mise à jour v${version} disponible` : 'Mise à jour disponible'),
      reviewUpdate: 'Vérifier la mise à jour',
      installed: (version, mode) => `Installée v${version} · ${mode}`,
      confirmAction: action => `Confirmer ${action}`,
      acceptCompatibility: 'J’ai examiné et j’accepte les actions de compatibilité.',
