...
export const ko: Translations = {
  ...
  skills: {
    wisdom: {
      ...
      browseHub: 'Hub 둘러보기',
      title: '집단 지식',
      loading: '집단 지식 불러오는 중',
      unavailable: '집단 지식을 사용할 수 없습니다.',
      setup: '이 프로필에 집단 지식이 설정되지 않았습니다.',
      setupDisclosure:
        '후보 자격 평가는 이 프로필에만 남습니다. 소유자가 승인한 비공개 초안 내용, 작성자 문구, 선언형 매니페스트 메타데이터 및 관리형 설치 상태만 Gateway에 전달됩니다.',
      setupAction: '이해했습니다 — 이 프로필 설정',
      settingUp: '설정 중…',
      scanLocal: '로컬 스킬 검사',
      search: '공유 스킬 검색…',
      potential: '제안된 기여',
      ...
      draftState: state => {
        const labels: Record<string, string> = {
          vetting: '서버 검토 진행 중',
          ready: '검토 준비 완료',
          owner_approved: '내가 승인함',
          publishing: '게시 중',
          pending_moderation: '컬렉티브 관리자 승인 대기 중',
          changes_requested: '변경 요청됨'
        }
        return labels[state] || state.replaceAll('_', ' ')
      },
      serverScanPassed: '서버 검사 통과',
      noDescription: '소유자가 작성한 설명이 없습니다.',
      managedInstalls: '관리형 설치',
      close: '닫기',
      readEvery: '모든 원본 파일을 읽으세요. 승인은 아래에 표시된 정확한 세 해시에 결합됩니다.',
      editReview:
        '여기에서 설명, SKILL.md 또는 선언형 매니페스트를 수정하세요. 저장하면 새 비공개 리비전이 생성되고 검사가 다시 실행되며 새 해시가 반환됩니다. 로컬 원본 스킬은 변경되지 않습니다.',
      unsavedChanges: '이 변경 사항은 아직 검사되지 않았습니다. 승인하기 전에 저장하고 다시 검사하세요.',
      saveAndRescan: '변경 사항 저장 및 다시 검사',
      savingRevision: '저장하고 다시 검사하는 중…',
      resetChanges: '편집 내용 버리기',
      reviewedHashes: '현재 서버에서 검토한 리비전의 해시',
      prepareTitle: '업로드 전 로컬 패키지 검토',
      prepareNotice:
        '소유자 전용 서버 검토에 명시적으로 제출할 때까지 이 필드는 이 프로필에 남습니다. 로컬 자격 횟수와 사유는 절대 포함되지 않습니다.',
      ownerDescription: '소유자가 작성한 설명',
      ...
      decline: '거절',
      checkUpdates: count => `업데이트 확인${count ? ` (${count})` : ''}`,
      checking: '확인 중…',
      refreshShared: '공유 스킬 새로 고침',
      ...
      updateModeDefault: '조직 기본값 사용',
      updateModeManual: '수동',
      updateModeAutomatic: '알림 후 자동',
      updateModeRequired: '필수',
      updateModeHelp: 'Gateway는 조직의 현재 정책을 적용합니다. 보안에 민감한 변경은 계속 승인이 필요합니다.',
      updateModePlan: mode => `향후 업데이트: ${mode}`,
      activityReady: count => `새 알림 ${count}개`,
      decisionPublished: skill => `${skill}이(가) 승인되어 이제 팀과 공유됩니다.`,
      decisionChanges: skill => `${skill}을(를) 공유하려면 변경이 필요합니다.`,
      ...
      takedownNotice: skill => `${skill}이(가) 컬렉티브에서 제거되었습니다.`,
      markSeen: '모두 읽음으로 표시',
      install: '설치…',
      uninstall: '제거…',
      checkSkill: '이 스킬 확인',
      updateAvailable: version => (version ? `v${version} 업데이트 가능` : '업데이트 가능'),
      reviewUpdate: '업데이트 검토',
      installed: (version, mode) => `설치됨 v${version} · ${mode}`,
      confirmAction: action => `${action} 확인`,
      acceptCompatibility: '호환성 작업을 검토했으며 동의합니다.',
