...
const KANBAN_BUNDLE = fileURLToPath(new URL('../../../plugins/kanban/dashboard/dist/index.js', import.meta.url))
const ACHIEVEMENTS_BUNDLE = fileURLToPath(
  new URL('../../../plugins/hermes-achievements/dashboard/dist/index.js', import.meta.url)
)
const ACHIEVEMENTS_API = fileURLToPath(
  new URL('../../../plugins/hermes-achievements/dashboard/plugin_api.py', import.meta.url)
)
...
describe('bundled Dashboard plugin localization contract', () => {
  it.each([
    ...
    ['Achievements', ACHIEVEMENTS_BUNDLE, en.achievements, zh.achievements!]
  ])(
    ...
    (_name, bundle, englishCatalog, simplifiedCatalog) => {
      ...
      expect(keys.filter(key => typeof resolvePath(englishCatalog, key) !== 'string')).toEqual([])
      expect(keys.filter(key => typeof resolvePath(simplifiedCatalog, key) !== 'string')).toEqual([])
    }
  )
  ...
  it('keeps every built-in achievement and criterion metric in both complete catalogs', () => {
    ...
    const catalogSource = source.match(/ACHIEVEMENTS:[\s\S]*?(?=# ---- Durable state files ----)/)?.[0]

    expect(catalogSource).toBeTruthy()

    const ids = [...new Set([...catalogSource!.matchAll(/\b_ach\("([^"]+)"/g)].map(m => m[1]))].sort()
    const metrics = [
      ...new Set([
        ...[...catalogSource!.matchAll(/\bmetric="([^"]+)"/g)].map(m => m[1]),
        ...[...catalogSource!.matchAll(/\("([^"]+)",\s*\d+\)/g)].map(m => m[1])
      ])
    ].sort()
