...
export const zh: TranslationOverlay = {
  ...
  app: {
    ...
    memoryOomRestartBanner:
      "智能体意外重启，最可能是内存不足。长会话和大量并发任务会增加内存占用。",
    memoryCriticalBanner:
      "智能体内存即将耗尽，可能会重启。请考虑关闭空闲会话或增加可用内存。",
    memoryElevatedBanner: "智能体可用内存偏低。",
    diskCriticalBanner:
      "智能体磁盘空间即将耗尽，新消息、记忆和设置可能无法保存。",
    diskElevatedBanner:
      "智能体磁盘空间正在减少，请考虑清理旧会话或扩充存储空间。",
    dismiss: "关闭",
  },
  ...
  sharedGateway: {
    restartTitle: "重启共享网关？",
    restartDescription: "此设备上的所有机器人将重新连接：{profiles}",
    restartAll: "全部重启",
    restarted: "共享网关已重启（{count} 个机器人）",
    servedWith: "与 {profiles} 共用网关。",
    migrating: "正在迁移到单个共享网关",
    migrationFailed: "网关迁移失败：{error}",
    migrationDescription:
      "各配置档目前分别运行网关。迁移后，一个共享网关进程将为所有配置档提供服务。",
    migrate: "迁移到单个共享网关",
  ...
  sessions: {
    ...
    pruneSkippedOpen:
      "已跳过 {count} 个仍打开的会话；清理操作只会删除已结束的会话。",
    pruneFailed: "清理会话失败",
    pruneDescription:
  ...
  cron: {
    ...
    continuityDescription:
      "连续运行：每次运行都能看到上一次的输出（用于去重或从中断处继续）",
  ...
  pluginsPage: {
    ...
    catalogHeading: "插件目录",
    catalogHint:
      "经 Nous 审核并锁定到精确提交的插件。从此安装可使用供应链更安全的版本。",
  ...
  achievements: {
    ...
    definitions: {
      ...
      five_model_flight: {
        name: "五模型巡礼",
        description:
          "尝试至少五种不同的大语言模型，而不是只守着第一个会回答的模型。",
      },
      provider_polyglot: {
        name: "多服务商通才",
  ...
  kanban: {
    ...
    bulkFailedDetails:
      "批量操作失败：{total} 项中有 {failed} 项失败：{details}",
    ...
    confirmDoneMany:
      "将 {n} 个任务标记为完成？工作进程将被释放，依赖这些任务的子任务将变为就绪。",
    ...
    delete: "删除",
    doneNoResult:
      "未记录最终结果。请到运行历史、日志或子任务中查看工作进程输出。",
    doneParentNote:
      "此卡片是编排器或父任务，请在子任务结果中查看实际工作成果。",
    ...
    projectDirectoryHelp:
      "Git 项目使用保留的工作树；其他文件夹直接使用该目录。仅临时工作可留空。",
    ...
    cardHints: {
      ...
      created: "创建于 {time}",
      dependencies:
        "{parents} 个父任务，{children} 个子任务；父任务完成前子任务保持阻塞。",
      diagnostics:
        "有 {count} 个活动诊断（严重程度：{severity}）。打开任务可查看详情。",
      noProfile: "尚未分配配置档案。",
    ...
    orchestration: {
      ...
      manualModeHelp:
        "手动编排会保留待分类任务，直到你点击「⚗ 拆解」。点击切换为自动。",
      mode: "编排模式",
      noDescription: "⚠ 无描述",
      noProfiles: "未安装配置档案。",
      orchestratorHelp:
        "负责拆解后的根任务并评审完成状态。拆解模型请在 auxiliary.kanban_decomposer 下配置。",
      profile: "编排器配置档案",
      profileDescriptionPlaceholder: "此配置档案擅长什么？",
      profileDescriptions: "配置档案描述",
      profileDescriptionsHelp:
        "描述用于指导路由。点击 ⚗ 自动生成，或手动编辑并保存。",
      reload: "重新加载",
