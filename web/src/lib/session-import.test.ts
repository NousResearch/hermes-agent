...
describe('parseImportSessions', () => {
  it('accepts a single exported session', () => {
    expect(parseImportSessions('{"id":"session-1","messages":[]}')).toEqual([{ id: 'session-1', messages: [] }])
  })
  ...
  it('rejects empty files and non-object entries', () => {
    ...
    expect(() => parseImportSessions('[{"id":"one"},42]')).toThrow('Expected exported session JSON or JSONL')
  })
})
