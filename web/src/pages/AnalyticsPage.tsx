...
function TokenBarChart({ daily, t }: { daily: AnalyticsDailyEntry[]; t: (key: string, opts?: Record<string, unknown>) => string }) {
              ...
              >
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10 pointer-events-none">
                  <div className="rounded-md bg-card border border-border px-2.5 py-1.5 text-[10px] text-foreground shadow-lg whitespace-nowrap">
                    ...
                    <div>{t("analytics.total", { tokens: formatTokens(total) })}</div>
                  </div>
                </div>
                {/* Input bar */}
                <div
...
function DailyTable({ daily, t }: { daily: AnalyticsDailyEntry[]; t: (key: string) => string }) {
  if (daily.length === 0) return null;
...
function ModelTable({ models, t }: { models: AnalyticsModelEntry[]; t: (key: string) => string }) {
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <SummaryCard
              ...
              sub={t("analytics.tokensInOut", { input: formatTokens(data.totals.total_input), output: formatTokens(data.totals.total_output) })}
            />
            <SummaryCard
              icon={BarChart3}
              label={t("analytics.totalSessions")}
              value={String(data.totals.total_sessions)}
              sub={t("analytics.avgPerDay", { avg: (data.totals.total_sessions / days).toFixed(1) })}
            />
            <SummaryCard
              icon={TrendingUp}
