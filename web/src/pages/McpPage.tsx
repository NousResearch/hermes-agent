...
export default function McpPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : 'Invalid MCP server', 'error')
      return
    }
    ...
    try {
      ...
      showToast(transport === 'http' && httpAuth === 'oauth' ? 'Added — authenticate with OAuth' : 'Add ✓', 'success')
      setName('')
      setNetwork('auto')
              ...
              {transport === 'http' ? (
                <>
                  <McpNetworkFields
                    ...
                    onTransportChange={setHttpTransport}
                  />
                  <div className="grid gap-2">
                  ...
                  {httpAuth === 'oauth' && (
                    <p className="text-xs text-muted-foreground">
                      Add the server, then use Authenticate. Hermes opens the OAuth browser on the machine running the
                      Dashboard backend.
                    </p>
                  )}
                </>
      ...
      {installEntry && (
        ...
        >
          ...
          >
            ...
            <div className="p-5 grid gap-4">
              <p className="text-xs text-muted-foreground">This MCP requires the following values to be configured.</p>
              ...
              <CardContent className={cn('flex items-start gap-4 py-4', !server.enabled && 'opacity-60')}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {server.auth && (
                      <Badge tone="outline">auth: {server.auth === 'header' ? 'bearer' : server.auth}</Badge>
                    )}
              ...
              <CardContent className="flex items-start gap-4 py-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {entry.installed && !entry.enabled && <Badge tone="outline">disabled</Badge>}
                  </div>
                  {entry.description && <p className="text-xs text-muted-foreground">{entry.description}</p>}
                  {/* Connection detail: what the agent actually talks to. */}
                  {entry.transport === 'http' && entry.url && (
                    <p className="mt-1 text-xs text-muted-foreground">
                      <span className="font-medium">Endpoint:</span> <code className="font-mono">{entry.url}</code>
                    </p>
                  )}
