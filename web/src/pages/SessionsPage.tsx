...
const ROLE_STYLES: Record<string, { bg: string; text: string; labelKey: string }> = {
  user: { bg: "bg-primary/10", text: "text-primary", labelKey: "sessions.roleUser" },
  assistant: { bg: "bg-success/10", text: "text-success", labelKey: "sessions.roleAssistant" },
  system: { bg: "bg-muted", text: "text-muted-foreground", labelKey: "sessions.roleSystem" },
  tool: { bg: "bg-warning/10", text: "text-warning", labelKey: "sessions.roleTool" },
};
  ...
  return (
    <div className="mt-2 rounded-md border border-warning/20 bg-warning/5">
      <button
        ...
        aria-label={`${open ? t("sessions.collapse") : t("sessions.expand")} tool call ${toolCall.function.name}`}
      >
...
function MessageBubble({ msg, highlight }: { msg: SessionMessage; highlight?: string }) {
  ...
  const label = msg.tool_name ? t("sessions.toolPrefix", { name: msg.tool_name }) : t(style.labelKey);
...
function SessionRow({
  ...
  const hasTitle = session.title && session.title !== t("common.untitled");
          ...
          {!searchResults && total > PAGE_SIZE && (
            <div className="flex items-center justify-between pt-2">
              <span className="text-xs text-muted-foreground">
                {t("sessions.rangeOf", { start: page * PAGE_SIZE + 1, end: Math.min((page + 1) * PAGE_SIZE, total), total })}
              </span>
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-muted-foreground px-2">
                  {t("sessions.pageOf", { current: page + 1, total: Math.ceil(total / PAGE_SIZE) })}
                </span>
                <Button
                  variant="outline"
                  size="sm"
