...
// Compaction handoff handling moved to `@/lib/chat/compaction.ts`; the
// extracted `MessageBubble` (components/chat/MessageBubble.tsx) and the
// `ToolCallCard` (components/chat/ToolCallCard.tsx) re-use those helpers.
// See `web/src/lib/chat/compaction.ts` for the source of truth.
//
// Background, kept here as documentation only:
//
...
// session timeline (#29824), so the extracted `MessageBubble` downgrades
// them to a muted, clearly-labelled "Context handoff" row.
//
// Keep the prefix list and end marker in `lib/chat/compaction.ts` in sync
// with ``SUMMARY_PREFIX`` / ``LEGACY_SUMMARY_PREFIX`` and the
