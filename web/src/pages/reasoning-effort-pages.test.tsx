...
describe("ProfilesPage reasoning effort selector", () => {
  ...
  it("hides the profile reasoning selector when the model has no reasoning dial", async () => {
    mockedApi.getModelOptions.mockResolvedValue({
      providers: [
        {
          slug: "provider-a",
          name: "Provider A",
          models: ["model-a"],
          capabilities: { "model-a": { reasoning_levels: [] } },
        },
      ],
    });
    ...
    expect(container.querySelectorAll("button[role=combobox]")).toHaveLength(1);
  });
  ...
  it("ignores a stale model catalog after closing and reopening the same profile", async () => {
    const requests: Array<{
      resolve: (value: { providers: Array<{ slug: string; name: string; models: string[] }> }) => void;
    }> = [];
    mockedApi.getModelOptions.mockImplementation(
      () =>
        new Promise((resolve) => {
    ...
    await act(async () => {
      requests[0].resolve({
        providers: [{ slug: "old-provider", name: "Old Provider", models: ["old-model"] }],
      });
      await Promise.resolve();
    });
    expect(openOptions(container.querySelector<HTMLButtonElement>("button[role=combobox]")!)).not.toContain(
    ...
    expect(
      openOptions(container.querySelector<HTMLButtonElement>("button[role=combobox]" )!).map(
        (option) => option.textContent?.trim(),
      ),
    ).toContain("New Provider · new-model");
  });
  ...
  it("ignores a late fallback load after reopening the same profile editor", async () => {
    type FallbackResponse = Awaited<ReturnType<typeof api.getProfileFallbacks>>;
    let resolveFirstFallback!: (value: FallbackResponse) => void;
    let resolveSecondFallback!: (value: FallbackResponse) => void;
    const firstFallback = new Promise<FallbackResponse>((resolve) => {
      resolveFirstFallback = resolve;
    });
    const secondFallback = new Promise<FallbackResponse>((resolve) => {
      resolveSecondFallback = resolve;
    });
    mockedApi.getProfileFallbacks
      .mockImplementationOnce(() => firstFallback)
      .mockImplementationOnce(() => secondFallback);
    ...
    await act(async () => container.querySelector<HTMLButtonElement>("button[aria-label='Close']")!.click());
    await act(async () => actions!.click());
    const reopenedChangeModel = [...container.querySelectorAll<HTMLButtonElement>("[role=menuitem]")].find(
      (button) => button.textContent?.includes("Change model"),
    );
    await act(async () => reopenedChangeModel!.click());
    await settle();

    const freshEntry = {
      source_index: 0,
      source_provider: "provider-a",
      source_model: "fresh-model",
    ...
    await act(async () => {
      resolveSecondFallback({ fallbacks: [freshEntry] });
      await Promise.resolve();
    });
    await settle();
    const currentEntry = container.querySelector<HTMLElement>("[data-fallback-entry]");
    expect(currentEntry?.textContent).toContain("provider-a · fresh-model");

    const staleEntry = { ...freshEntry, model: "stale-model" };
    await act(async () => {
      resolveFirstFallback({ fallbacks: [staleEntry] });
      await Promise.resolve();
    });
    await settle();
    expect(container.querySelector<HTMLElement>("[data-fallback-entry]")?.textContent).toContain(
      "provider-a · fresh-model",
    );
    expect(container.querySelector<HTMLElement>("[data-fallback-entry]")?.textContent).not.toContain(
      "stale-model",
    );
  });
