...
compression:
  ...
  mode: standard             # Built-in ContextCompressor residual only (context.engine=compressor). Codex native compaction may bypass.
...
| `mode` | `standard` | `standard`, `catalog`, `hybrid` | Compaction residual for the built-in `ContextCompressor` only (`context.engine=compressor`). `standard` is the backward-compatible LLM summary. `catalog` is an extractive redacted catalog (no summarizer). `hybrid` is Standard plus one unique-handle index. Plugin engines are not passed this parameter. Provider-native Codex compaction may bypass it |
