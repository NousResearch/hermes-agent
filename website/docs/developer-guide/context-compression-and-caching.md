...
| `threshold_tokens` | `256000` | int or `null` | Absolute cap on the trigger: compaction fires at the lower of the ratio trigger and this count, so a 1M window compacts at 256K instead of 500K. A matching `model_thresholds` entry outranks the implicit default; a user-set cap remains absolute. `null` = ratio-only |
| `model_thresholds` | `{}` | map | Per-model overrides of `threshold`. Keys are substring-matched against the model name (longest match wins); `"<provider>:<substring>"` keys apply only on that provider. A matching override outranks the implicit default `threshold_tokens` cap, while an explicitly configured cap still wins. The small-context floor still applies on top (see below) |
