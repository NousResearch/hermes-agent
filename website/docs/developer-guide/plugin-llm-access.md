...
### Carrying the binding into a worker thread

The binding is context-local. A `threading.Thread` starts with a fresh
context and is not the main thread, so inside it
`ctx.llm.current_invocation()` is `None` and
`inherit_turn_invocation=True` refuses with `no_turn_invocation`. Run the
worker inside a copy of the turn's context instead:
...
An `asyncio` task created inside the turn already runs in a copy of the
turn's context and needs no extra step.
