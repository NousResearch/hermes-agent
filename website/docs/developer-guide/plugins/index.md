...
| `pre_api_request` | Before each raw provider API request (several per turn when the model calls tools) | `session_id: str, model: str, provider: str, base_url: str, api_mode: str, api_call_count: int, message_count: int, tool_count: int, approx_input_tokens: int, max_tokens: int, request: dict` | a dict may rewrite `reasoning_effort`, `max_tokens`, `temperature`, `top_p` on the outgoing request; non-dict results and other keys are ignored |
