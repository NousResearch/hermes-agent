...
| [`pre_llm_call`](../../user-guide/features/hooks.md#pre_llm_call) | Once per turn, before the tool-calling loop | `session_id: str, user_message: str, conversation_history: list, is_first_turn: bool, model: str, platform: str` | [ephemeral context injection](#pre_llm_call-context-injection) |
| [`pre_persist_user_message`](../../user-guide/features/hooks.md#pre_persist_user_message) | Once per turn, before the inbound user message is persisted | `session_id: str, turn_id: str, user_message: str, conversation_history: list, platform: str, sender_id: str` | [durable context injection](../../user-guide/features/hooks.md#pre_persist_user_message) |
...
Most hooks are fire-and-forget observers — their return values are ignored. The exceptions are `pre_llm_call`, which injects context into the prompt without changing the stored user-message `content`; `pre_persist_user_message`, which injects **durable** context that is written to the persisted user-message row (see the [Event Hooks reference](../../user-guide/features/hooks.md#pre_persist_user_message)); and `pre_tool_call`, which can return a block/approve directive.
...
When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Hermes injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context. The injection does not change the stored user-message `content`: Hermes may keep the exact API-bound bytes in the row's `api_content` sidecar for replay and prompt-cache parity, but search, recall and compaction see the clean message. For context that must become part of the durable user message itself, use [`pre_persist_user_message`](../../user-guide/features/hooks.md#pre_persist_user_message) instead.
