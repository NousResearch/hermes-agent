...
| [`post_agent_result`](/user-guide/features/hooks#post_agent_result) | Every root or native-subagent `run_conversation` result or raised error (excluding persistence-isolated internal forks) | `event: dict` (allowlisted and bounded; one isolated async queue per listener) | ignored |
