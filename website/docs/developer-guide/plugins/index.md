...
| [`gateway_platform_event`](../../user-guide/features/hooks.md#gateway_platform_event) | An authorized platform-native event is projected from decoded SDK values. Telegram reactions include update/actor identity, decoded old/new snapshots, and chat type; see the contract limitations below and the linked per-event contracts. | `platform: str, event_type: str, payload: dict` | ignored |
...
**Telegram reaction contract:** the additive `gateway_platform_event` envelope projects decoded SDK values for well-formed Bot API events and trusts Telegram's required-fields contract. Guards validate only properties still observable after decoding; they do **not** validate raw wire JSON. PTB 22.8 can collapse missing, `null`, `{}`, or `""` old/new snapshots to the same empty tuple as valid `[]`, so an empty decoded snapshot is **not proof of raw presence or validity**, and unknown reaction state cannot be inferred. For well-formed events, the empty new snapshot represents removal. This observer is not a durable, trustworthy reaction ledger; consumers needing raw-state validity/provenance require an independent boundary. Authorization remains fail closed. Paid and aggregate reactions are unsupported; inspected paid/unknown typed entries drop the event. Reaction `thread_id=None` means **unknown**, not General/root; no topic cache, raw SDK callback surface, or message-to-topic lookup is added. See [reaction details and bounds](../../user-guide/features/hooks.md#gateway_platform_event).
