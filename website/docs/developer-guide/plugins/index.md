...
`append_runtime_footer` is an append-only display hook for standalone observability plugins. It fires only when the user has enabled `display.runtime_footer`; it receives no config object, credentials, adapter handle, or transcript. Plugins should read precomputed local state rather than making network requests while rendering a reply. Empty or failed fragments are omitted, so a missing provider metric cannot suppress the normal footer or the response.
