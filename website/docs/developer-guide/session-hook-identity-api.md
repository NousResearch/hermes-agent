...
| `session_origin` | Optional trusted lifecycle provenance: `fresh` for an unseeded native create or an agent ID generated by core, `resume` for native resume/reattach/compression or supplied conversation history, and `None` when the caller cannot establish freshness. This is not model-provided permission or an ID-shape inference. |

`session_origin` describes the observed lifecycle boundary, not a durable permission
receipt. An existing owner/alias must retain its authority across a fresh runtime.
Caller-supplied standalone IDs and explicit agent resets remain unknown unless
an owning lifecycle publisher supplies provenance. Never infer freshness from
an absent plugin row or empty session history alone. A consumer should treat unknown
provenance conservatively and offer native review.

The classic CLI captures provenance when it generates its initial ID or a new ID
for `/new`, and passes that captured identity to a newly built or reused agent.
A resume or supplied conversation history is not fresh; the shared `/resume` and
`/branch` switch stamps continuation provenance before lazy construction or a
reused-agent reset. An unrecognized replacement ID cannot inherit the previous
creation receipt. This does not add a pre-agent
`on_session_identity` publisher to the classic CLI or infer freshness for gateway,
ACP, or arbitrary library-supplied IDs.
