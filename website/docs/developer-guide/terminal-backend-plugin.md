...
The definition is frozen. Hermes keeps a host-owned copy and rejects built-in names or cross-plugin name collisions.
...
`TerminalBackendRequest.settings` is an independent, read-only snapshot of `plugins.entries.<plugin-id>.terminal_backend`.
