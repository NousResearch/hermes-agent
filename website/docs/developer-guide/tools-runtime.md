...
`tools/process_registry.py` retains the shared restart-safe launch and user-bus
entrypoints. `tools/process_registry_scope.py` owns their systemd scope helpers,
cgroup inspection, and cancellable verified stops for terminal, Kanban, and cron
workers. Scope quiescence includes nested cgroups: cgroup v2 uses
`cgroup.events` population; legacy hierarchies require a bounded recursive walk.
Unreadable or malformed evidence stays unknown and cannot authorize a retry.
Unsupported-host launch policy remains with the caller.
