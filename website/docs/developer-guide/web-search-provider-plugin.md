...
- **`plugins/web/deepseek/`** — LLM-backed search via DeepSeek's server-side `web_search` tool on the Responses API. Closest sibling to `web/xai`, but reuses the same `DEEPSEEK_API_KEY` as the DeepSeek model provider and keeps all parsing in the plugin (no core tools touched).
