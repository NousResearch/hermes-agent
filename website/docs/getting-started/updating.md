...
When the parked branch has **uncommitted changes** (dirty tree), Hermes does **not switch branches**. The code update is marked **SKIPPED** with a loud warning naming the branch, how far behind `origin/main` it is, and the exact commands to resolve. One explicit exception is `update_in_place` on a branch with unmerged local commits: the updater stays on that branch and uses its normal autostash flow. Desktop retains the stash with `--keep-stash`; CLI normally restores it. The completion line always shows the actual branch and HEAD (`✓ Update complete! [main @ 30fcf9580]`) so drift is visible at a glance. Set `updates.auto_switch_parked_branch: false` in `config.yaml` to disable the auto-switch entirely (the skip warning still fires).
