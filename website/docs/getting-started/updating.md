...
2. **Git pull** — pulls the latest code from `main` (or the selected `--branch` target) and updates submodules. On a clean local branch with commits still unique by patch ID, Hermes saves its old tip under `hermes-update-snapshot/`, rebases it onto the updated target, and switches back. If comparison fails, the update stops before switching; if rebase conflicts, Hermes verifies the abort, preserves the original branch and snapshot, and prints recovery commands.
...
- **Branch fully merged** (every commit already covered in `origin/<target>`): the update switches to the selected target and stays there.
- **Branch has local-only commits** and the tree is clean: the update saves a snapshot branch, updates the selected target, rebases the original branch, and returns to it. A failed comparison stops before checkout. A rebase conflict stops the update with the original commits and snapshot preserved.

Set `updates.parked_branch_strategy: update_in_place` to merge the selected target into a maintained custom branch instead of rebasing it. `hermes update --switch-branch` explicitly leaves local commits on their existing branch and runs on the selected target for one update.
