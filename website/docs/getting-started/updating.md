...
6. **Desktop install (prebuilt, then source)** — if the Hermes Desktop app was built from this checkout, update first looks for a prebuilt artifact matching the new commit SHA, then the nearest tagged release within `desktop.prebuilt_artifacts.compatibility_window` (default 64 commits). The artifact's checksum is verified before anything is unpacked; install uses the same stage-and-swap as a source pack (`apps/desktop/.staging-*` → `release/`). When no artifact exists (dev checkouts, forks, unsupported arch) or the checksum/extract fails, update falls back to a local `hermes desktop --build-only` rebuild. A failed install leaves the previous app untouched and launchable; the update reports `⚠ Update partially complete` and `hermes desktop` retries the rebuild. Set `desktop.prebuilt_artifacts.enabled: false` to skip the prebuilt rungs.
