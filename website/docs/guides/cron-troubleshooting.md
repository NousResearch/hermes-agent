...
hermes cron run <job_id>            # Execute the job immediately (for testing)
...
1. Run the job with `hermes cron run <job_id>` (executes immediately, even without a running gateway) and watch for errors in the chat output
