...
- **Dependencies are explicit and fail closed**: set a unique `id` on every task and use `depends_on` for prerequisites. Hermes rejects missing ids, unknown references, and cycles before spawning. Disconnected components can deliver independently; that split automatically consolidates when the session or async capacity cannot support it.
