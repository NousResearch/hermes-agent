...
| Working directory | `agents.defaults.workspace` | `config.yaml` → `terminal.cwd` | The legacy `MESSAGING_CWD` / `TERMINAL_CWD` `.env` entries are deprecated; the loader warns when it sees them. |
