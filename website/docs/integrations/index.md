...
- **[Memory Providers](/user-guide/features/memory-providers)** — Plug in external memory backends and routers for deeper personalization. Nine providers are supported: Layered (taxonomy/routing), Honcho (dialectic reasoning), OpenViking (tiered retrieval), Mem0 (cloud extraction), Hindsight (knowledge graphs), Holographic (local SQLite), RetainDB (hybrid search), ByteRover (CLI-based), and Supermemory.
