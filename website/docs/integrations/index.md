...
| **Alibaba DashScope** | Good | Paid | `DASHSCOPE_API_KEY` |
| **NeuTTS** | Good | Free | None needed |

Speech-to-text supports local faster-whisper (free, runs on-device), local command wrappers, Groq, OpenAI Whisper API, Mistral, xAI, ElevenLabs Scribe, DeepInfra, and Alibaba DashScope Qwen ASR. Voice message transcription works across Telegram, Discord, WhatsApp, and other messaging platforms. See [Voice & TTS](/user-guide/features/tts) and [Voice Mode](/user-guide/features/voice-mode) for details.
