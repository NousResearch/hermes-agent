...
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.hermes/.env` (provider: `gemini`). Legacy `google-gemini-cli` OAuth has been removed. |
| **LM Studio** | `hermes model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `hermes model` → choose "Custom endpoint" (saved in `config.yaml`) |

:::note Migration
If you used `google-gemini-cli` before, switch to `gemini`, set `GOOGLE_API_KEY` or `GEMINI_API_KEY`, and remove any stale `~/.hermes/auth/google_oauth.json` file.
:::
