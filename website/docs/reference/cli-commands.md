...
- **Local branch reconciliation.** On a clean local branch, `git cherry` compares its commits with `origin/<target>` (`main` unless `--branch` selects another target). Upstream-equivalent patches stay on the target. Local-only patches are rebased onto the updated target after a `hermes-update-snapshot/...` branch saves the old tip. Comparison or rebase failure stops with recovery guidance. `--switch-branch` leaves the local branch untouched and stays on the target.
