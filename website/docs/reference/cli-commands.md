...
| `-k`, `--keep <N>` | After a full backup, keep at most N default-named `hermes-backup-*.zip` files total; complete archives are retained first and incomplete ones fill spare slots (default 3; `0` keeps everything). Custom-named zips are never touched. |
