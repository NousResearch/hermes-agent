...
**This is the human / scripting surface.** Ordinary dispatcher workers use the lifecycle-only `kanban-worker` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board); profiles listed in `kanban.routing_profiles` receive the full routing surface. Workers have `HERMES_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
