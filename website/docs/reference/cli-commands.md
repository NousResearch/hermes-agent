...
| `hermes update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--branch <name>` targets a branch; `--version <tag>` targets a release tag; `--backup` takes a pre-pull `HERMES_HOME` snapshot. |
