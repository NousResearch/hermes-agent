...
| `reopen-done <id> [reason...]` | Operator recovery: retract a verified-bad `done`/`archived` task back to `ready` (or `todo` while parents are open) and retract its descendants. Flags: `--ids` (bulk), `--json`. Denied to delegated children. |
| `schedule <id> "<reason>"` | Park time-delay/follow-up work in `scheduled` so it is not shown as a human blocker. Parks from todo/ready/running/blocked/review and resumes into the parked phase. Flag: `--wake-at <ISO8601-or-epoch>` (dispatcher auto-resumes when due; omit to wait for `unblock`). |
