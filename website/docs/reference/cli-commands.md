...
| `specify <id>` / `specify --all` | Flesh out a triage-column task into a concrete spec (title + body with goal, approach, acceptance criteria) via the auxiliary LLM, then promote it to `todo`. If `auxiliary.triage_router` is configured, `specify` first runs a fast/cheap pre-check there and auto-promotes with a minimal spec when it comes back trivial, skipping the full LLM call — see [Configuration](/user-guide/configuration#full-auxiliary-config-reference) and [Kanban](/user-guide/features/kanban). Flags: `--tenant` (scope `--all` to one tenant), `--author`, `--json`. Configure the model under `auxiliary.triage_specifier` in `config.yaml`. |
