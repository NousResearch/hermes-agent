...
| `HERMES_GATEWAY_REPLACE_FORCE` | Power-user escape hatch for the launchd/systemd supervisor guard on `hermes gateway run --replace`. By default, when an unsupervised caller (e.g. `hermes-web-ui`'s startup hook) invokes `--replace` against a service-managed gateway, the call exits cleanly without sending SIGTERM — preventing supervisor restart loops. Set to `1` to bypass the guard and force a takeover anyway. |
