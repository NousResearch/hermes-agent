...
| `MATTERMOST_ALLOWED_CHANNELS` | Comma-separated channel IDs where the bot is allowed to respond or observe context |
| `MATTERMOST_OBSERVE_UNMENTIONED_CHANNEL_MESSAGES` | Store unmentioned allowed-channel/thread posts as context-only transcript rows without dispatching the agent |
