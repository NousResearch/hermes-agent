...
| `HERMES_TUI_INLINE` | Force inline rendering on or off when set to `1`/`0`. When on, the TUI renders into the host terminal's primary buffer so prior output stays in scrollback; when off, the TUI uses the alternate screen (no scrollback clutter after quitting). Defaults to on under Termux and off everywhere else. |
| `HERMES_TUI_MOUSE_TRACKING` | Explicit mouse-tracking override when set to `1`/`0`; takes precedence over all defaults and the legacy `HERMES_TUI_DISABLE_MOUSE` toggle. When unset, mouse tracking defaults to off under Termux and to on elsewhere unless the legacy `HERMES_TUI_DISABLE_MOUSE` is truthy. Also surfaced at runtime via the `/mouse` slash command and the `display.mouse_tracking` `config.yaml` key. |
