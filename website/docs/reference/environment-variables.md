...
| `DISCORD_COMMAND_SYNC_POLICY` | Discord slash-command startup sync policy: `safe` (diff and reconcile while preserving command IDs when possible), `bulk` (legacy `tree.sync()`), or `off` |
| `DISCORD_COMMAND_SYNC_ALLOW_RECREATE` | Opt in to delete/recreate for Discord slash commands whose metadata cannot be patched (`true`/`false`, default `false`). Recreating commands can briefly invalidate Discord client command caches after restart. |
