...
| `HERMES_DESKTOP_REMOTE_URL` | (Desktop side) Base URL of the remote backend, e.g. `http://host:9119`. When set, overrides the in-app Gateway URL. Without `HERMES_DESKTOP_REMOTE_TOKEN`, Desktop probes the gateway's advertised session provider and lets you sign in from Gateway settings (OAuth redirect or username/password). |
| `HERMES_DESKTOP_REMOTE_TOKEN` | Static token paired with `HERMES_DESKTOP_REMOTE_URL`. It is optional when the gateway advertises session authentication; otherwise it remains required. When set, Desktop uses legacy token authentication instead of the session sign-in flow. |
