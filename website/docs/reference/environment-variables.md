...
| `HERMES_REPO_MIRROR_URL` | Windows installer only (`scripts/install.ps1`): `;`-separated list of `https://` mirror URLs for restricted networks. Each entry is tried, in order, only after the official `HERMES_REPO_URL` clone ladder is fully exhausted, with the same retry ladder per URL. A clone that succeeds via a mirror leaves `origin` at that mirror, so reruns keep updating from it; the installer names the source on every run and warns while `origin` differs, and pointing `HERMES_REPO_URL` back at the official repository for one rerun restores it. Mirror content is operator-trusted: pin `-Commit` when the mirror cannot be fully trusted, since git's content addressing is what bounds what a mirror can serve for a pinned SHA. |
