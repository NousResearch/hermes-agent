...
| `HERMES_DESKTOP_IGNORE_EXISTING` | Set to `1` to make Desktop ignore every discovered local runtime (the managed install, a `hermes` on `PATH`, and a system-Python `hermes_cli`) during backend resolution, so no local backend starts and Desktop falls through to connect or onboarding. Explicit `HERMES_DESKTOP_HERMES_ROOT`, an unpackaged source checkout, and `HERMES_DESKTOP_HERMES` still win. A runtime this launch just installed is still used. Equivalent to `hermes desktop --ignore-existing`. |
