...
| `HERMES_GH_BIN` | Explicit GitHub CLI/wrapper executable path for Hermes GitHub subprocesses. The value is used as one literal argv element; when unset Hermes uses `shutil.which("gh")`. Example for a managed technical identity: `/opt/data/bin/gh-hermes`. |
