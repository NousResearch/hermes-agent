...
| `GITHUB_TOKEN` | GitHub token for Skills Hub (higher API rate limits, skill publish) and the update check on every surface — the desktop app, the CLI banner and the dashboard's update endpoint (`GH_TOKEN` also honoured; without either, the update check falls back to the `gh` CLI login, then anonymous) |
