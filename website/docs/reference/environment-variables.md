...
| `HERMES_VISION_EMBED_TARGET_BYTES` | Byte budget for an image embedded natively into a `vision_analyze` tool result, which is re-sent on every later turn (override for `auxiliary.vision.embed_target_bytes`; default: `262144` = 256 KB). Images over budget are re-encoded as JPEG at falling quality and then halved in size until they fit. Raise it (e.g. `2097152`) when the images are document pages rather than screenshots: at the default a 150 DPI A4 page ends up ~620 px wide and table digits blur. Values below 16 KB are ignored; capped at the 20 MB hard ceiling. |
| `HERMES_VISION_EMBED_MAX_DIMENSION` | Long-edge pixel cap for the same native embed (override for `auxiliary.vision.embed_max_dimension`; default: `1568`). Values below 64 are ignored; capped at 8000. |
