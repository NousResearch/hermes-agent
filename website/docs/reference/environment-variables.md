...
| `HERMES_LOCAL_STT_LANGUAGE` | Fallback STT language hint, consulted only when both the provider-specific and global `stt.language` settings are empty. Command backends can apply additional defaults when no hint is resolved. See [STT language resolution](../user-guide/configuration.md#speech-to-text-stt). |
