...
| `HERMES_STDERR_LOG_LEVEL` | Override the gateway's stderr threshold with a standard Python logging level name (for example, `INFO` or `WARNING`) or a numeric level. By default, foreground TTY runs follow `-v`/`-q`; macOS non-TTY runs use `CRITICAL` to avoid duplicating routine records into launchd's unbounded `gateway.error.log`. Rotating file logs are unaffected. |
