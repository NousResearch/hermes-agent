...
| `TERMINAL_CWD` | **Deprecated** when set in `.env`. Configure `terminal.cwd` in `~/.hermes/config.yaml` instead; the gateway bridges that value to `TERMINAL_CWD` at runtime. CLI runs always use the directory you launched `hermes` from. |
...
| `MESSAGING_CWD` | **Deprecated.** Use `terminal.cwd` in `~/.hermes/config.yaml` to set the gateway / cron working directory. The loader prints a one-shot migration warning when it sees this var in `.env`. |
