...
Hermes also syncs bundled skills on `hermes update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hermes/skills/` tree, it is still shipped with Hermes; restore it with `hermes skills reset <profile> --restore`.
...
| Skill | Description | Path |
|-------|-------------|------|
| [`computer-use-agents`](/docs/user-guide/skills/bundled/computer-use/computer-use-computer-use-agents) | Build AI agents that interact with computers like humans — screenshots, mouse, keyboard, file operations, code execution. Uses the agentic computer-use library for the orchestration loop. | `computer-use/computer-use-agents` |
| [`orca-cli`](/docs/user-guide/skills/bundled/computer-use/computer-use-orca-cli) | Use the public `orca` CLI to operate Orca-managed worktrees: create, list, export, capture, revert, and get help. | `computer-use/orca-cli` |
...
| [`ideation`](/docs/user-guide/skills/bundled/creative/creative-ideation) | Generate project ideas via creative constraints. | `creative/ideation` |
...
| [`muller-brockmann-grid-systems`](/docs/user-guide/skills/bundled/creative/creative-muller-brockmann-grid-systems) | Implement genuine Swiss-design modular grids on the web — with precise 8-column, 12-column, 24-column grid templates, modular calculus, and strict typographic rhythm (leading, measure, baseline). | `creative/muller-brockmann-grid-systems` |
| [`nyt-data-viz`](/docs/user-guide/skills/bundled/creative/creative-nyt-data-viz) | NYT-discipline data visualization — Observable Plot, D3, scrollytelling, map projections, and annotated charts. | `creative/nyt-data-viz` |
...
| [`veo-hyperframes`](/docs/user-guide/skills/bundled/creative/creative-veo-hyperframes) | AI video production pipeline — text-zone composition, scene blocks, camera choreography, shot grading, voiceover, and audio scoring. Uses HyperFrames (React/Drei/Postprocessing) for composable production. | `creative/veo-hyperframes` |
| [`vignelli-canon-design-system`](/docs/user-guide/skills/bundled/creative/creative-vignelli-canon-design-system) | Massimo Vignelli's complete design discipline — intangible philosophy, tangible production specs, and a rigorous multi-phase methodology for creating authentic Vignellian communication artifacts. | `creative/vignelli-canon-design-system` |
| [`web-clone`](/docs/user-guide/skills/bundled/creative/creative-web-clone) | Website cloning methodology — capture, analyze, rebuild, and deploy. | `creative/web-clone` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`audio-source-separation`](/docs/user-guide/skills/bundled/media/media-audio-source-separation) | Vocal & instrumental separation via Demucs (Meta) — speech denoising, music stem extraction, video dubbing prep. | `media/audio-source-separation` |
...
| [`media-download`](/docs/user-guide/skills/bundled/media/media-media-download) | Download video/audio from streaming platforms — YouTube, Bilibili, Douyin, Twitter/X, Instagram, and more. Uses yt-dlp with per-platform config and cookie management. | `media/media-download` |
| [`omnivoice-studio-windows`](/docs/user-guide/skills/bundled/media/media-omnivoice-studio-windows) | Diagnose, configure, and troubleshoot OmniVoice Studio on Windows — ASR, TTS, voice cloning, Higgs Audio, and system-level issues (GPU, CUDA, proxy). | `media/omnivoice-studio-windows` |
| [`seedance2-prompt-en`](/docs/user-guide/skills/bundled/media/media-seedance2-prompt-en) | Expert prompt engineering guide for ByteDance's Jimeng Seedance 2.0 multimodal AI video generation model. Covers @ target syntax, camera language, shot types, action choreography, and advanced technique layering. | `media/seedance2-prompt-en` |
| [`seedance2-prompt-zh`](/docs/user-guide/skills/bundled/media/media-seedance2-prompt-zh) | 字节跳动即梦 Seedance 2.0 多模态 AI 视频生成模型的提示词撰写指南。涵盖 @ 引用语法、运镜语言、景别分类、动作编排技巧及高级技法叠加。 | `media/seedance2-prompt-zh` |
...
| [`subtitle-edit`](/docs/user-guide/skills/bundled/media/media-subtitle-edit) | Subtitle Edit CLI — batch subtitle format conversion, time shifting, encoding, frame-rate correction. | `media/subtitle-edit` |
| [`video-subtitle-translate`](/docs/user-guide/skills/bundled/media/media-video-subtitle-translate) | Video subtitle transcription + translation workflow using transub (Whisper + LLM). | `media/video-subtitle-translate` |
...
| [`obsidian-diagrams`](/docs/user-guide/skills/bundled/note-taking/note-taking-obsidian-diagrams) | Create, troubleshoot, and choose diagram formats in Obsidian. | `note-taking/obsidian-diagrams` |
...
| [`handoff`](/docs/user-guide/skills/bundled/productivity/productivity-handoff) | HANDOFF.md — cheapest cross-session memory for AI agents. Write before closing a session, read at the start of the next. | `productivity/handoff` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`do-task`](/docs/user-guide/skills/bundled/software-development/software-development-do-task) | One-click task execution: plan → subagent execute → verify → handoff. Chains write-plan, delegate, verify, and report in a single command. | `software-development/do-task` |
...
| [`python-source-install`](/docs/user-guide/skills/bundled/software-development/software-development-python-source-install) | Install Python CLI tools from GitHub source when standard package managers fail or are outdated. | `software-development/python-source-install` |
