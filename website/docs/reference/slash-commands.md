...
| `/pause [reason\|off]` | Emergency stop — pause new work globally, across every platform (not just one adapter, unlike `/platform pause`). `/pause off` resumes. |
...
- `/sethome`, `/restart`, `/approve`, `/deny`, `/topic`, `/platform`, `/pause`, and `/commands` are **messaging-only** commands.
