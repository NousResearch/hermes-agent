...
| `/branch [name]` or `/branch --here [name]` (alias: `/fork`) | Branch the current session. On Discord, Telegram, and Slack, the default creates a new thread while `--here` keeps the branch in the current conversation. The CLI and other platforms branch in place. |
