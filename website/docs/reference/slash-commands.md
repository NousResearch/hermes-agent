...
| `/btw <question>` | Ask a quick side question **about the current conversation** without interrupting it. A one-shot auxiliary LLM call answers from a read-only snapshot of the transcript — the live session's history and prompt cache are untouched, and the current turn keeps running. On the **desktop app** this opens a floating side chat window beside the app — the answer and any follow-up questions stay in that window instead of the transcript. For independent work with a fresh context, use `/bg`. |
