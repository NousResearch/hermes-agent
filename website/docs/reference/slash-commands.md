...
| `/background [prompt]` | With a prompt, run a new isolated background session. On Telegram, send bare `/background` (or `/bg`) while an agent is running to detach that exact turn and open a fresh foreground session. Results are delivered back to the same chat. See [Messaging Background Sessions](/user-guide/messaging/#background-sessions). |
| `/back <session-id>` | **Telegram only.** Move the live agent with this `/agents` session ID into the background and open a fresh foreground session. |
| `/front <session-id>` | **Telegram only.** Move the live agent with this `/agents` session ID into the foreground. The previously foregrounded agent keeps running in the background. |
