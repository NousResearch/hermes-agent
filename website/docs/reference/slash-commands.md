...
| `/browser [connect\|disconnect\|status]` | Manage a local Chromium-family CDP connection. `connect` only attaches browser tools to an already-running Chrome, Brave, Chromium, or Edge instance (default: `http://127.0.0.1:9222`). If the endpoint is unreachable, it fails closed and prints manual launch instructions without replacing the current connection. `disconnect` detaches. `status` shows current connection. |
