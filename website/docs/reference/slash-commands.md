...
| `/branch [name]` | Branch the current session in place (explore a different path) |
| `/fork [name]` | **Gateway (Discord only):** when invoked from an existing Discord forum thread, copy the current conversation into a new visible forum thread while leaving the original unchanged. **CLI:** branch the current session in place. Deployments that restrict slash commands must allow `fork` explicitly; `branch` allowlists and `command:branch` hooks do not apply to this distinct command. |
