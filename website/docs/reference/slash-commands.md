...
| `/resume [name]` | Resume a previously-named session. A session that is currently open in another surface (TUI, desktop, gateway) is **not** taken over: Hermes shows its transcript read-only with the owner named, leaves that surface's lease intact, and you retry once it is closed there. An unowned session moves this CLI's active-session lease with it, so no second writer can attach mid-chat. |
