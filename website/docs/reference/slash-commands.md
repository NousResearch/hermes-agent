...
| `/queue [prompt]` (alias: `/q`) | A non-empty prompt is queued as one FIFO turn without interrupting the current agent, and its first word is always prompt text rather than an operation. Without a prompt, Hermes opens a short-lived numbered view of your manageable queued turns; use `/dequeue <N\|all>` (alias: `/dq`) to remove only positions from that latest view. On Discord, a native prompt-less slash interaction renders the same view as an ephemeral caller-only manager. In shared chats, text views are delivered privately when the platform supports it; otherwise Hermes returns only a safe retry hint. |
