...
| `/queue [prompt]` (alias: `/q`) | With a non-empty prompt, queue it as one FIFO turn without interrupting the current response; its first word is always prompt text, never an operation. With no prompt, show a short-lived numbered view of the current session's manageable user turns. Use `/dequeue <N\|all>` (alias: `/dq`) to remove positions from your latest view only. In Discord's queue manager, select a queued text prompt and choose **Steer selected** while the current Gateway run owns that local turn to inject it into the current run after the next tool call instead of waiting for a new turn. |
...
| `/queue [prompt]` (alias: `/q`) | A non-empty prompt is queued as one FIFO turn without interrupting the current agent, and its first word is always prompt text rather than an operation. Without a prompt, Hermes posts a short-lived numbered view of the current session's manageable user turns; use `/dequeue <N\|all>` (alias: `/dq`) to remove only positions from your latest view. Every authorized participant routed to that same session sees the same queue. Discord renders a public channel/thread card, while its buttons remain bound to the member who opened that snapshot; another member can run `/queue` to get their own snapshot. Select a queued text prompt and choose **Steer selected** while the current Gateway run owns that local turn to inject it after the next tool call instead of waiting for a new turn. Internal continuation work stays hidden. |
