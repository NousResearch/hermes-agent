...
| `/goal_prompt [path]` | Load a project's `docs/runbooks/GOAL_PROMPT.md` or an explicit prompt file, set it as the active `/goal`, and run one continuation-oriented project slice without copy/pasting the prompt. |
| `/goal_prompt_oneshot [path]` | High-autonomy project-loop variant of `/goal_prompt`: uses a larger budget, requires deterministic `CONTINUE` / `STOP_FOR_OPERATOR` / `COMPLETE` final blocks, shows both the parsed judge verdict and exact next action between loops, and can restart through `/new` after repeated compactions. |
