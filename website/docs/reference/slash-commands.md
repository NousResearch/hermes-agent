...
| `/mute [duration\|off\|status]` (alias: `/unmute`) | Silence Hermes in this chat — while muted, the gateway drops incoming messages deterministically (no agent turn, no tokens, no reply). Slash commands still work, so `/unmute` always lifts it. `/mute` alone mutes indefinitely; `/mute 30m`, `/mute 2h`, `/mute 1d` set a duration (bare numbers are minutes). The mute is chat-scoped and survives gateway restarts. |
...
- `/sethome`, `/restart`, `/approve`, `/deny`, `/topic`, `/platform`, `/mute`, and `/commands` are **messaging-only** commands.
