...
| `/reload-permissions` (alias: `/reload_permissions`) | Admin-only. Reload gateway permission state without restarting the gateway; model, tools, system prompt, and sessions stay unchanged |
...
| `/reload-permissions` (alias: `/reload_permissions`) | Admin-only. Reload gateway permissions without restarting the process. Invalid permission config is rejected and previous permissions stay active. |
| `/yolo` | Toggle YOLO mode — skip all dangerous command approval prompts. |
...
- `/sethome`, `/update`, `/restart`, `/reload-permissions`, `/approve`, `/deny`, `/topic`, and `/commands` are **messaging-only** commands.
