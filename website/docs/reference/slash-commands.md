...
| `/memory [pending\|approve\|reject\|approval\|show]` | Review pending memory writes staged by the write-approval gate (`memory.write_approval`) and toggle the gate. `/memory show [memory\|user]` prints the current persistent-memory contents (MEMORY.md + USER.md) with per-store character usage. See [Controlling memory writes](/user-guide/features/memory#controlling-memory-writes-write_approval). |
...
| `/memory [pending\|approve\|reject\|approval\|show]` | Review pending memory writes staged by the write-approval gate (`memory.write_approval`) — approve or reject them right in chat — and toggle the gate with `/memory approval on\|off`. `/memory show [memory\|user]` prints the current persistent-memory contents (MEMORY.md + USER.md) with per-store character usage. See [Controlling memory writes](/user-guide/features/memory#controlling-memory-writes-write_approval). |
