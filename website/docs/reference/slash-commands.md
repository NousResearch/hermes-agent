...
| `/fast [normal\|fast\|auto\|cold\|status]` | Fast mode — OpenAI Priority Processing / Anthropic Fast Mode. `fast` = every request; `auto` = only requests in the first `agent.fast_auto_seconds` (default 60s) of each turn; `cold` = that same window on the first turn of a session only. Default `normal` (off). `status` reports the effective service tier: `fast` (priority), `flex` (OpenRouter flex), or `normal`. See [Fast mode](../user-guide/configuration.md#fast-mode). |
...
| `/fast [normal\|fast\|auto\|cold\|status]` | Fast mode — OpenAI Priority Processing / Anthropic Fast Mode. `auto`/`cold` open a bounded fast window per turn / per session. `status` reports the effective service tier: `fast` (priority), `flex` (OpenRouter flex), or `normal`. |
