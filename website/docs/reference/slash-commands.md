...
| `/queue <prompt>` (alias: `/q`) | Queue any non-empty argument as one complete FIFO prompt without interrupting the current agent response. There are no text subcommands, and a bare CLI invocation shows usage. |
...
| `/queue <prompt>` (alias: `/q`) | Queue any non-empty argument as one complete FIFO prompt without interrupting the current agent. The first word is always prompt text, never an operation. On Discord, only a prompt-less native slash interaction opens the caller's ephemeral private manager; a bare command sent as ordinary Gateway text still shows usage. |
