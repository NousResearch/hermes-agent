...
| `/handoff <platform>` | **CLI and Desktop chat.** Hand the current session off to a messaging platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix). The gateway creates a fresh thread where supported, binds it to the same session id, and runs a confirmation turn. The CLI exits on success with a `/resume` hint; Desktop confirms the transfer and keeps the session in its list. Refused mid-turn. Requires a running gateway and a home channel configured for the target (`/sethome` from the destination chat). To return to Desktop, reopen the session from its list; this does not detach the messaging thread. See [Cross-Platform Handoff](../user-guide/sessions.md#cross-platform-handoff). |
...
- `/skin`, `/snapshot`, `/export`, `/import`, `/reload`, `/tools`, `/toolsets`, `/browser`, `/config`, `/cron`, `/platforms`, `/paste`, `/image`, `/statusbar`, `/battery`, `/focus`, `/plugins`, `/indicator`, `/wake`, `/journey`, `/redraw`, `/clear`, `/history`, `/save`, `/copy`, `/prompt`, `/pet`, `/hatch`, `/timestamps`, `/subscription`, and `/quit` are **CLI-only** commands. `/handoff` also works in Desktop chat, but not in messaging chat.
