...
| `/resume [name]` | Resume a saved or closed session. In the TUI picker, resumable sessions are ordered by most recently closed first and show how long ago they closed. |
| `/sessions` (TUI alias: `/switch`) | Classic CLI: browse and resume previous sessions in an interactive picker. TUI: open the live session switcher for currently open TUI sessions. Use `/sessions new` in the TUI to start another live session immediately, or `/resume` for saved/closed transcripts. |
