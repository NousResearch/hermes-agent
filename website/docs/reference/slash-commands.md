...
| `/fast [normal\|fast\|auto\|cold\|status]` | Set provider speed policy. `auto` opens a bounded fast window each turn; `cold` only on the first logical-session turn. |
...
| `/fast [normal\|fast\|auto\|cold\|status]` | Set normal, always-fast, bounded per-turn auto, or first-turn cold speed. |
