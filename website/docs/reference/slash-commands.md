...
| `/kanban <action>` | Drive the multi-profile, multi-project collaboration board from chat with the same arguments as the CLI. `/kanban create …` uses the originating chat as an automatic notification target when `kanban.auto_subscribe_on_create` is enabled. `--notify` supplies explicit targets, and `--no-subscribe` suppresses all targets. See [Kanban slash command](/user-guide/features/kanban#kanban-slash-command). |
