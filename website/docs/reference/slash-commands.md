...
| `/background <prompt>` (alias: `/bg`, `/btw`) | Run a prompt in a separate background session. In the Ink TUI only, bare `/background` detaches the exact turn already running and immediately opens a fresh chat; the original transcript keeps the turn and its result is also reported in the fresh chat. Prompted forms keep their isolated-new-agent behavior. See [CLI Background Sessions](/user-guide/cli#background-sessions). |
| `/detach` | **Ink TUI only.** Detach the exact currently running turn without cancelling, restarting, or duplicating it, then continue immediately in a fresh live session. Equivalent to bare `/background`; reports a clear message when idle. |
