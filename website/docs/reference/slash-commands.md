...
| `/steer <prompt>` | Inject a mid-run note that arrives at the agent **before the next tool call** — no interrupt, no new user turn. The text is appended to the last tool result's content once the current tool completes, so the agent sees it before deciding its next step. Use this to nudge direction mid-task (e.g. "focus on the auth module" while the agent is running tests). |
...
| `/steer <prompt>` | Inject a message before the next tool call without interrupting — the model picks it up on its next iteration rather than as a new turn. |
