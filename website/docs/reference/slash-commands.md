...
| `/handoff <platform[:chat_id]>` | **CLI only.** Hand the current session off to a messaging platform's home channel, or to an explicit chat/channel ID. The gateway creates a fresh thread where supported, re-binds the destination to your CLI session id so the full role-aware transcript replays, and forges a synthetic user turn so the agent confirms it's working in the new place. Use `hermes sessions handoff <session> --to <platform[:chat_id]>` for non-interactive automation. The CLI exits cleanly on success with a `/resume` hint; resume locally any time with `/resume <title>`. Bare-platform targets require a home channel configured with `/sethome`; explicit targets do not. See [Cross-Platform Handoff](/user-guide/sessions#cross-platform-handoff). |
