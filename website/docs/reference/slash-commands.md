...
| `/plan [task]` | Write a markdown implementation plan to `.hermes/plans/` in the active workspace — planning only, no execution. Empty argument infers the task from the conversation. After saving, Hermes presents the complete plan for review and asks you to approve it for a later turn, request changes, or save it without execution; interactive surfaces use a choice card when available. Plan approval does not authorize later privileged or production actions. (Formerly the bundled `plan` skill; now built-in so it survives the Telegram/Discord command-menu caps.) |
...
| `/plan [task]` | Write a markdown implementation plan to `.hermes/plans/`; Hermes presents the complete plan and asks whether to approve it for a later turn, revise it, or save it without execution. |
