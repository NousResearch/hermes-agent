...
| `/retitle` | Generate a fresh session title from recent real conversation turns and persist it. In a Telegram topic or Discord thread, the current topic/thread is renamed to match. Title generation uses the configured `auxiliary.title_generation` route. |
