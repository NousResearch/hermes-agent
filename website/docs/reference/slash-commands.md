...
| `/hatch <description> [options]` (alias: `/generate-pet`) | Generate a brand-new petdex pet with any configured reference-capable image backend. Use `/hatch --help` for provider/model/style/seed/reference and quality controls. See [Pets](/user-guide/features/pets). |
