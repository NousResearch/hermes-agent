...
| `/native-compact` (alias: `/compact-native`) | **CLI only.** Immediately call OpenAI's standalone Responses compaction endpoint for an eligible ChatGPT Codex OAuth session. Requires the same gpt-5.6, `compression.enabled`, and `compression.codex_responses_native` gates as automatic native compaction. It creates a replayable provider checkpoint without changing the readable Hermes transcript. |
