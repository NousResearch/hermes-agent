...
| `/resume [name\|number]` | Resume by title, session ID, or position in the numbered recent-sessions list shown by bare `/resume` (including unnamed sessions). `/resume 1` selects the first listed session, `/resume 2` the second. |
...
| `/resume [name\|number]` | Resume by title, session ID, or position in the numbered list shown by bare `/resume`. The gateway list contains named sessions visible to the caller; `/resume 1` selects its first entry. |
