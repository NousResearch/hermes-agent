...
In the Ink TUI, `/handoff` uses the gateway's native handoff RPCs rather than the general slash-command worker. The TUI detaches the source session while waiting, so further prompts cannot race the transfer. While pending, slash commands (including `/new`) and session-picker activation, resume, close, and new-session actions are blocked until the request settles or polling times out. A confirmed preflight rejection restores the source session. After completion, failure, or an uncertain outcome (such as a polling timeout), the TUI stays open but detached: check the destination before explicitly resuming the old session, or use `/new` for an independent conversation. A client-side timeout does not cancel a gateway handoff that may already have delivered.
