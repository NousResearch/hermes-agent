...
| `/fast [normal\|fast\|auto\|status]` | Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode. `auto` accelerates requests that start during the first 60 seconds of each user turn (configurable with `agent.fast_auto_on_seconds`). |
...
| `/fast [normal\|fast\|auto\|status]` | Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode; `auto` applies fast mode only during the opening window of each user turn. |
