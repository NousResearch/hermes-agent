...
| `/moa <prompt>` | Run a single prompt through the default [Mixture of Agents](/user-guide/features/mixture-of-agents) preset, then restore your current model. One-shot — does not change your session model. Use `/moa aggregate <prompt>` (also `summarize`, `summary`, `solo`, `direct`) to run the preset aggregator only without reference-model fan-out. |
