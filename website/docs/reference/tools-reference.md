...
| `kanban_heartbeat` | Tell humans the worker is still there during a long-running operation. Renews liveness only — the separate progress lease moves on real tool calls and board transitions, never on a heartbeat note. | `HERMES_KANBAN_TASK` or `kanban` toolset |
