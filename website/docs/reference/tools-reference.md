...
| `kanban_withdraw` | Retract a card this run filed by mistake — a duplicate, a superseded brief, a card filed twice when the first create looked like it had failed. The card lands in the terminal `archived` state, so no dispatcher tick can promote or claim it, and a worker already spawned on it is stopped. Only the run that filed the card may withdraw it: the card's create-time `creator_task_id` must equal the caller's own `HERMES_KANBAN_TASK`. Refused for the caller's own card (`kanban_complete` / `kanban_block` close that one) and for a card already `done`/`archived`. | `HERMES_KANBAN_TASK` |
