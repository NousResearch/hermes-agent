...
| `kanban_archive` | Archive one finished task — the model-facing counterpart of `hermes kanban archive`. Soft archive over `kanban_db.archive_task`: comments/attachments/history/links are preserved, the active run closes, and dependent children are released by the post-archive recompute. Single task per call; deterministic refusal for unknown/already-archived. Orchestrator-only; hidden from dispatcher-spawned task workers. | profile with `kanban` toolset |
