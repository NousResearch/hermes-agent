...
| `kanban_attach` | Attach a file to a task. Prefer `path` (absolute path on the agent's host; read server-side, byte-exact). `content_base64` is a fallback for content with no file; the model re-emits base64 token by token and alters long payloads. Stored as a real attachment under the task's attachments dir, capped at 25 MB. | `HERMES_KANBAN_TASK` or `kanban` toolset |
