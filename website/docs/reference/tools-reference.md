...
| `kanban_attach` | Attach a file to a task by passing its bytes inline (base64). Stored as a real attachment under the task's attachments dir, capped at 25 MB. Requires `expected_sha256` (hash the original bytes before encoding) and returns the verified SHA-256; rejects mismatches. | `HERMES_KANBAN_TASK` or `kanban` toolset |
