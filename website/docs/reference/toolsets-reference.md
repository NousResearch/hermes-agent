...
|---------|-------|---------|
| `browser` | `browser_back`, `browser_cdp`, `browser_click`, `browser_console`, `browser_dialog`, `browser_get_images`, `browser_navigate`, `browser_press`, `browser_scroll`, `browser_snapshot`, `browser_type`, `browser_vision`, `web_search` | Core browser automation. Includes `web_search` as a fallback for quick lookups. `browser_cdp` is direct read-only CDP inspection, not a browser-controller capability (including Developer Mode); arbitrary evaluation is retired. `browser_cdp` and `browser_dialog` register only with an explicit CDP override at session start (via `/browser connect` or `browser.cdp_url`), which may be cloud-hosted. Provider-managed per-session CDP is not automatically surfaced to these tools. `browser_snapshot` may add `pending_dialogs` and `frame_tree` whenever its supervisor has an attached CDP session. |
