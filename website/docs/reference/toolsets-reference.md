...
| `messaging` | `send_message` | Outbound cross-platform messages and reactions. **Default-off on every platform**; explicitly enable it with `hermes tools` for the platform that should expose it. The gateway/session runtime gate still applies. |
