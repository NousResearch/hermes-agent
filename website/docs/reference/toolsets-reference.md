...
| `kanban-worker` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_heartbeat`, `kanban_show` | Lifecycle-only surface automatically assigned to ordinary dispatcher workers. Calls are scoped to `HERMES_KANBAN_TASK`; no board routing or fan-out. |
| `kanban` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | Full coordination surface. Dispatched workers receive it only when their profile is listed in `kanban.routing_profiles`; normal sessions may explicitly enable it by name. The `all`/`*` wildcard does **not** enable it. |
