...
| `profiles` | `profile_manage` | Profile (Bot) creation and configuration from inside a session — `list`, `create`, `configure`. Registered only for profiles that explicitly list the `profiles` toolset by name (the `all`/`*` wildcard does **not** enable it), so agents cannot mint profiles by default. Never exposed to `delegate_task` children: a subagent given a narrow goal should not create persistent profiles as a side effect. Profile **deletion is deliberately excluded** — it stays behind the CLI and desktop destructive-confirmation flows. |
