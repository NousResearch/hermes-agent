...
:::note
**`message_agent` (and the protocol) on headless installs.** "Bot-Mode-managed" means at least one profile's `profile.yaml` carries a `ui_meta` block — a marker written only by the desktop app when it configures a Bot, with no CLI surface today. Both the messaging protocol section and the `message_agent` tool are gated on that marker (plus the canonical `Bot Chat` session title), so on an install where the desktop app has never configured a Bot — e.g. a headless box used only through a messaging platform — neither is injected, even if a session titled `Bot Chat` exists. `agent.bot_mode_protocol` toggles the feature; it does not create the marker.
:::
