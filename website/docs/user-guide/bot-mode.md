...
- **Browse every conversation with that Bot** — select the Bot, then click the history button in the Bots-pane header (or right-click the Bot → **Conversation history**). The list comes from that Bot profile's durable session store, includes archived conversations, and follows compression continuations. Opening one restores it as a normal Bot-workspace tab. Closing a tab or restarting Desktop does not remove the conversation from this list; the canonical Bot Chat, group-room plumbing, and routine runs stay in their own dedicated surfaces.
