...
- **Rosters propagate on their own.** While the Desktop runs, it periodically tells each connected gateway which agents live on the *other* connections. Every Bot Chat's teammate roster then lists them ("Teammates on OTHER connected machines"), with names, roles, and which machine they're on — and the roster refreshes when agents appear, disappear, or get renamed (capability epoch). A teammate on another machine answers to the same target forms as a local one — profile name, friendly name, or the `@`-tag — and to `<name>@<connection>` when two machines share a name.
