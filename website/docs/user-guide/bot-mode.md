...
- The room header's **gear** button opens **Group settings**, where you can **rename** the group, set, replace, or remove its picture, and adjust its per-send limits. A rename carries everything with it — the room log, each member's room session, memberships, the limits, and the picture — so no history is lost. Renaming to a name that's already taken is rejected rather than silently suffixed.
...
- The defaults are 10 Bot replies, 3 rounds, and 2 follow-on mention rounds per send. Open **Group settings** to adjust those three safety ceilings for an individual room.
