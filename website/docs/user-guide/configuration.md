...
### Task-bound execution contracts

Set `approvals.require_task_execution_contract: true` only for a dedicated Kanban worker profile. It keeps ordinary commands unchanged, but refuses a mutating ArgoCD command unless the worker's assigned card has an immutable matching `execution_contract`. The initial contract permits only direct, single-application `sync`, `rollback`, and `terminate-op`; selectors, multi-app forms, unknown subcommands, shell composition, `--prune`, and `execute_code` are always refused.

```yaml
approvals:
  require_task_execution_contract: true
```
