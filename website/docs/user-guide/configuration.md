...
| `cost` | Session cumulative estimated cost so far (opt-in; skipped while unmeasured) | `≈$0.018` |
| `cwd` | Home-relative working directory | `~` |

The default field set is `["model", "context_pct", "cwd"]`. `latency`, `served_model`, and `cost` are opt-in — add them to `fields` to use them. `cost` shows the session's cumulative estimated spend in USD and is skipped while the session has no measured cost yet, so it never renders as `$?`. `served_model` renders nothing when the served model is the configured one (or when the proxy sends no such header), so behind a routing proxy or an active fallback it is the field that makes the switch visible. Fields whose data is unavailable are skipped silently rather than rendering an empty slot.
