...
display:
  ...
  language: en            # UI language for static messages (approval prompts, some gateway replies). en | zh | ja | de | es | uk
```
...
Supported values: `en` (default), `zh` (Simplified Chinese), `ja` (Japanese), `de` (German), `es` (Spanish), `uk` (Ukrainian). Unknown values fall back to English.
