...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Hermes normally inherits the parent agent's key. For credential-managed provider endpoints such as Nous (`nous`, `nous-portal`, `nousresearch`), setting `delegation.provider` with `delegation.base_url` but without `delegation.api_key` keeps the configured endpoint and resolves the key through that provider; an explicit `delegation.api_key` always wins.
