...
compression:
  ...
  prepare_ahead: false                              # Opt-in: prepare the next summary in the background — see below
...
`prepare_ahead` (default `false`) writes the next compaction summary in the background once the session is close to the trigger, while tools run and between turns. When automatic compaction fires and the summarized messages are unchanged, the prepared summary is used instead of a summary call at that moment; messages newer than the prepared summary stay outside it and are handled as the usual uncompressed tail. If no prepared summary is ready or it no longer matches, compaction runs exactly as it does without the option. It spends auxiliary summary calls that may go unused, and it does not apply to manual `/compress`, plugin context engines, or native/app-server compaction.
