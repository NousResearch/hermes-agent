...
compression:
  ...
  tool_result_projection: auto                      # Wire-only replacement of stale tool results by stubs ("off" disables; see below)
  tool_result_projection_min_tokens: 0              # Stale tool payload (tokens) needed before a pass; 0 = derive from window + caching
  tool_result_projection_min_result_chars: 4000     # Only tool results larger than this (chars) are eligible
  tool_result_projection_min_reclaim_tokens: 8192   # A pass only commits when it reclaims at least this many tokens
  tool_result_projection_tail_ratio: 0.025          # Protected verbatim tail as a fraction of the window
  tool_result_projection_tail_min_tokens: 12000     # Protected tail floor
  tool_result_projection_tail_max_tokens: 32000     # Protected tail cap
  tool_result_projection_tail_messages: 8           # Protected tail message-count floor
  tool_result_projection_tail_max_messages: 60      # Protected tail message-count cap
...
`tool_result_projection` (default `auto`) is the wire-only counterpart of that prune, and the one that applies to your session's context window without rewriting it. Old, large tool results are replaced **in the request only** by a stub carrying the tool name, its arguments, the byte count, a content hash and the spillover path — the canonical transcript, session resume, the UI timeline and summarization keep every byte, so nothing is lost and a `/compress` or `session_search` still sees the full result. Before a result is replaced, its full bytes are written to `$HERMES_HOME/cache/spillover/`; if that write fails, the result is left intact (a stub pointing at nothing would be worse than the bytes it replaced). The newest messages keep their full results — the protected verbatim tail is `tool_result_projection_tail_ratio` of the window clamped between `tool_result_projection_tail_min_tokens` and `tool_result_projection_tail_max_tokens`, with a message-count floor and cap (`tool_result_projection_tail_messages`, `tool_result_projection_tail_max_messages`). Small results (`tool_result_projection_min_result_chars`, default `4000`), errors, multimodal results and any structured result declaring `projection_safe: false` are never projected, and a result already projected stays projected so the wire prefix is byte-stable between passes.

A pass runs only when it is worth its cache break: the newly stale payload (the rows that became eligible since the last pass — a pile already projected is not charged again) must exceed `tool_result_projection_min_tokens` (`0` = derived from the route — 16K when it has no prompt caching, 32K when it does, capped at a quarter of the window), the reclaim must clear `tool_result_projection_min_reclaim_tokens`, and on a cached route it must also cover the region the rewrite invalidates. So each break has to be paid for by stale bytes that arrived since the previous one, rather than by the session simply getting longer. Set `tool_result_projection: "off"` to disable it entirely.
