...
`"split"` and `"verbose"` also show fast tool-less turns that `"total"` would suppress. Turns that report no model time (e.g. Codex app-server turns carry no `api_duration`) omit the model segment rather than printing a misleading `0.0s`. The same per-turn fields ride the `usage` object on `message.complete`, so TUI and desktop clients can render them from one insertion point.
