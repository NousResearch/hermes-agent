...
- `secret_name_pattern` — optional regex that marks an environment-variable NAME as a secret in the exact-value redaction pass. Any `.env` variable whose name matches this pattern (case-insensitive) has its exact value masked in every redacted surface, even when the value has no recognizable vendor prefix. Defaults to a built-in pattern (`TOKEN`, `PASSWORD`, `SECRET`, `KEY`, `CREDENTIAL`, `AUTH`, and variants). An invalid pattern, a non-string value, or an empty string falls back to the built-in default with a warning. The pass masks the whole value only: a truncated fragment or a deliberate slice is not the full value and passes through. It guards against accidental leakage, not deliberate exfiltration by an agent that already holds the secret. Values shorter than 12 characters are never loaded. For example, to treat only `PASSWORD`, `KEY` and `TOKEN` names as secrets, set `secret_name_pattern: "(?:^|_)(?:PASSWORD|KEY|TOKEN)(?:_|$)"`.
