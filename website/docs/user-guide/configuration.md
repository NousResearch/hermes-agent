...
### Skill index descriptions

The system prompt carries an index of every visible skill with its one-line description. On small local models that index can cost thousands of tokens per session, so it can be reduced to names only:
