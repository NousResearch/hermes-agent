...
`compression.threshold` also sets the net's **floor**: hygiene never fires *before* the agent's own compressor, so raising the agent threshold (e.g. `0.95` on a measured 1M window) raises the net with it. The net's own ratio is otherwise fixed at `0.85` and is deliberately higher than the agent's shipped `0.50`, because at `0.50` it was doing the agent compressor's job with rougher token estimates (see [#1161](https://github.com/NousResearch/hermes-agent/pull/1161)); that ordering only holds while the agent's ratio stays below the net's, so a widened agent threshold now moves the net instead of being silently capped by it.
