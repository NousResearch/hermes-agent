...
display:
  ...
  language: en            # UI language for static messages (approval prompts, some gateway replies). en | zh | ja | de | es | pt | pt_BR
```
...
Supported values: `en` (default), `zh` (Simplified Chinese), `ja` (Japanese), `de` (German), `es` (Spanish), `pt` (Portuguese), `pt_BR` (Brazilian Portuguese). Unknown values fall back to English.
