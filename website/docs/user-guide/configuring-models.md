...
### Agent-selected model routes

Hermes can expose a config-gated `select_model` tool when an operator wants the
agent to choose among a small, pre-approved set of aliases. The tool changes
only the active session; it does not rewrite the global default. Every
successful selection reports the model, task reason, reasoning effort, and
reason for that effort through the active chat surface.

```yaml
model:
  aliases:
    luna: openai/gpt-fast
    terra: openai/gpt-engineering
    sol: openai/gpt-deep
  ...
  model_selection:
    ...
    routes:
      luna:
        ...
        description: Bounded extraction, formatting, summarization, or classification.
      terra:
        target: terra
        default_reasoning: medium
        allowed_reasoning: [medium, high]
        description: Ordinary engineering work.
      sol:
        target: sol
        default_reasoning: high
        allowed_reasoning: [high, xhigh]
        description: High-consequence or ambiguous work.
```

`target` must resolve through the existing model alias/provider machinery.
`default_reasoning` is used when the tool omits an effort; an explicit effort
must appear in `allowed_reasoning` or the switch fails without changing the
session. The selected route and reasoning effort survive Desktop/TUI session
reloads and messaging-gateway restarts. Tool calls emitted after
`select_model` in the same model response are skipped and regenerated by the
selected model. Delegated child agents cannot change the parent session route,
and surfaces without a durable selection-report callback fail closed.

Keep the route set small and policy-based. Mid-session changes still incur the
prompt-cache cost described above.
