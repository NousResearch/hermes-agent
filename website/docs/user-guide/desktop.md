...
- **Session tags** — right-click a session (or open its actions menu), choose **Tags**, then check existing tags or use **Create and assign** for a new one, such as `urgent` or `long-process`. A session can have multiple tags. Names are case-sensitive, trimmed, and limited to 64 characters; removing a tag from a session keeps it available for reuse.
- **Filter by tags** — open the session-list menu and choose **Filters → Tags**. Selecting several tags matches any of them, combined with the other active filters. Tags also filter pinned sessions, project views, and search results, without changing pin state or order. **Reset to defaults** clears the filter, not the assignments. Available tag names are shared across all live profiles in the same Hermes installation, including profiles with no loaded sessions. Assignments remain in the owning profile's session database and survive context compression; identical session IDs in different profiles stay independent. Selecting a tag does not change the profile filter: choose **All profiles** to search across profiles. Separate servers do not synchronize catalogues. Both Desktop and the backend must support tags.
