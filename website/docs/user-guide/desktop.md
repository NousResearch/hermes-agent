...
- **Favorite models.** Open a model row's options and choose **Add to favorites** to pin it to a **Favorites** section at the top of the picker — in the composer, in session tiles, and anywhere else a model is picked, because it is one stored preference. Stars persist per device, keep the order you added them, and ignore the **Edit models** shortlist (a star IS an explicit "always show me this one"); a star for a provider that is not connected is kept and reappears when it reconnects. Searching is unaffected — a query lists every match in its provider's place.
