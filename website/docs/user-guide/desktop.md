...
- **Keep computer awake** — **Settings → Advanced → Keep computer awake** stops the machine from sleeping while an agent run is in progress, so long or overnight runs keep going (the display can still dim). The hold is released as soon as the last run finishes, fails, or is cancelled, so an idle Hermes lets the computer sleep on its normal schedule. This is a per-computer setting.
