...
- **Copy keeps formatting** — the **Copy** action under a reply puts both the reply's Markdown and its rendered HTML on the clipboard. Paste into mail, docs or chat apps and headings, lists, links, tables and code blocks arrive formatted; paste into a code editor or terminal and you still get the plain Markdown. Reasoning, tool cards and status rows are never part of the copy.
