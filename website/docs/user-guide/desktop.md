...
- **Customizable items** — right-click the status bar (**Show in status bar**) to choose what appears: the context meter, cache hit rate, tokens per second, the **Tokens** item (session-wide total; click it for a per-model breakdown — one row per model/provider route the session used, with calls, tokens and cost), workspace, model, approvals, turn/session timers, terminal, Command Center, backend version, and more — or hide the bar entirely (**Cmd/Ctrl+Shift+S** toggles it).
