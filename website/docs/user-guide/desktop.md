...
- **Lists in the composer** — on a Markdown list line (`1.`, `1)`, `-`, `*`, `+`, or a `- [ ]` task box), **Shift+Enter** starts the next item and **Tab** / **Shift+Tab** nest it under the item above or bring it back out. Shift+Enter on an empty item ends the list, and plain Enter still sends. Letters, Roman numerals, outline numbers, `(1)` parentheses and symbol bullets are off by default; turn them on under **Settings → Appearance → Lists in the message box**.
