...
- **Continue a chat on messaging** — enter `/handoff telegram` (or another configured platform) in the Desktop composer after the current turn finishes. The messaging gateway must be running, and `/sethome` must have been set in the destination chat. Open the same session from the Desktop session list when you return; this does not detach the messaging thread. See [Cross-Platform Handoff](./sessions.md#cross-platform-handoff).
