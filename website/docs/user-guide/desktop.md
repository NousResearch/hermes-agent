...
- **Enter While Running** — **Settings → Chat → Enter While Running** (`display.busy_input_mode` in `config.yaml`) decides what pressing Enter does while the agent is still working, the same key the CLI and TUI read. **Interrupt** (the default) redirects the live turn with your text, keeping the reasoning and tool work already shown. **Steer** adds your text to the model's next tool result without stopping the turn. **Queue** holds the text and sends it once the turn finishes. A message with attachments, or one typed while the turn is compacting or parked on an approval prompt, always queues, and a submission the gateway refuses falls back to the queue rather than being dropped. Slash commands run immediately in every mode. The change takes effect on the next Enter.
