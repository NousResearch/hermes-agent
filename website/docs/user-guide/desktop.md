...
- **Filter by tags** — open the session-list menu and choose **Filters → Tags**. Selecting several tags matches any of them, combined with the other active filters. Tags also filter pinned sessions, project views, and search results, without changing pin state or order. **Reset to defaults** clears the filter, not the assignments. Tags live in the owning profile's session database and survive context compression; Desktop clients connected to the same backend share them. Both Desktop and the backend must support tags.
