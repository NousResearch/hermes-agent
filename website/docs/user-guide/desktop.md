...
- **Inspect images while the agent works** — image-bearing tool calls stay reachable rather than disappearing into the activity ticker. Click **Open image** to inspect a screenshot or visual input, click the image to enlarge it, or choose **Open preview** to keep it beside the chat while typing a correction. Native image results show the actual supplied pixels, including crops; local image paths are read from the tool's originating session and gateway. Images load on demand, with a retry action for unavailable files. Output previews arrive when the tool returns its image, not only when the overall turn finishes; this is not a continuous video feed inside a long-running tool. Text-only corrections use the existing live steering path; image attachments still queue for the next turn. Multiple images use a numbered gallery with five thumbnails per page. Use the previous/next buttons, arrow keys, or Home/End while the gallery or enlarged image has focus; these keys do not intercept typing in the composer. Only the current thumbnail page is loaded, with at most three local-file reads in flight across tool galleries. Each selected image can be kept in its own transient side-preview tab without persisting image bytes. Reopening retries a failed load; the Retry button targets the selected failed image.
