...
sidebar_position: 3
title: "Desktop App"
description: "The native Nautilus desktop app — a polished experience for chatting with Nautilus, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Nautilus desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Nautilus Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `nautilus` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Nautilus has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`nautilus`) and **[TUI](./tui.md)** (`nautilus --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`nautilus dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Nautilus Desktop](../getting-started/installation.md).

If you already have Nautilus installed, simply run
...
- **The same conversation history** as every other Nautilus surface — sessions started here resume in the CLI/TUI and vice versa.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `nautilus desktop --cwd <path>` (or the `NAUTILUS_DESKTOP_CWD` environment variable).

### Voice

Talk to Nautilus and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `nautilus model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Nautilus management surface so you don't have to drop to a terminal:
...
- **Profiles** — switch between [Nautilus profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Nautilus agent, your config, and your chats stay. (Same as `nautilus uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `nautilus uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `nautilus uninstall --full`.)
...
You can do the same from the terminal — `nautilus uninstall --gui` for the GUI alone, or `nautilus uninstall` / `nautilus uninstall --full` for the agent too.

:::note
Running `nautilus uninstall --gui` from a **source checkout** (a `nautilus desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `nautilus desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `nautilus desktop`

To launch via the CLI, simply run `nautilus desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `nautilus update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `NAUTILUS_DESKTOP_CWD`)           |
| `--hermes-root PATH` | Override the Nautilus source root the app uses (sets `NAUTILUS_DESKTOP_HERMES_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `nautilus` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships only the Electron shell. On first launch it installs the Nautilus Agent runtime into `NAUTILUS_HOME` (`~/.nautilus`, or `%LOCALAPPDATA%\nautilus` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. The React renderer talks to a `nautilus dashboard` backend over the standard gateway APIs and reuses the agent rather than reimplementing it. Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Nautilus backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `nautilus dashboard` process
"Remote backend" means a **`nautilus dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `nautilus dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Keep that `nautilus dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `NAUTILUS_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `NAUTILUS_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **Signed out on every restart** — set `NAUTILUS_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Nautilus Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Nautilus Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).

## Troubleshooting

Boot logs land in `NAUTILUS_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
tccutil reset Microphone com.nousresearch.nautilus
```
...
```bash
NAUTILUS_DESKTOP_HERMES_ROOT=/path/to/clone npm run dev
NAUTILUS_HOME=/tmp/throwaway npm run dev
