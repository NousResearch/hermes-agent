...
- **Rootful Linux:** the docker bridge gateway (`172.17.0.1:<tunnel_port>` by default). Containers reach the proxy via `host.docker.internal`, which `--add-host=host.docker.internal:host-gateway` resolves to exactly this bridge gateway IP — a loopback-only bind would be unreachable from inside sandboxes. The bridge IP is an address on the host's `docker0` interface, so it is not exposed to the LAN; it IS reachable by other containers on the default bridge network, but requests still require a minted proxy token and an allowlisted upstream. If Docker is not installed or running, the bind falls back to loopback with a warning.
- **Rootless Linux:** setup, start, and Docker sandbox creation fail clearly. The reported bridge gateway exists only inside RootlessKit and cannot be bound by the host-side iron-proxy process; Docker also disables access to host loopback by default. Hermes therefore refuses to report a host-only listener as a healthy sandbox egress endpoint. Use a rootful Docker daemon when `proxy.enforce_on_docker` is required, or explicitly disable the egress proxy for that backend.
