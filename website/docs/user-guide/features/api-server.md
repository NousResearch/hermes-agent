...
**Reasoning in streams**: Reasoning-capable providers keep thinking separate from assistant output. Chat Completions emits `choices[0].delta.reasoning_content`; Responses emits one `reasoning` output item per reasoning burst, in the order the agent produced them, with `response.reasoning_text.delta` events; a burst closes when the next tool call starts or the assistant message opens. The non-streaming Responses payload places each assistant message's reasoning in `output[]` right before that message's `function_call` items. Reasoning streams and output items on these endpoints are gated by `display.platforms.api_server.show_reasoning` (falling back to `display.show_reasoning`), default off. With the gate off no reasoning callback is installed and no reasoning is emitted.
