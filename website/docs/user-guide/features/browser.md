...
#### One-call page snapshot (workspace helper)

Before every `browser_exec` call, Hermes keeps two files in the session's workspace:

```
$BH_AGENT_WORKSPACE/
  hermes_browser_snapshot.py   generated by Hermes; rewritten when the helper version changes
  agent_helpers.py             created if absent; a marker block imports the module above
```

Browser-harness auto-imports `agent_helpers.py` on each call and injects its public names into the exec namespace, so the agent can write:

```python
snap = snapshot()                 # ONE round trip for the whole page
...
`snapshot()` returns every visible interactive control — role, name, value, geometry in viewport CSS px, and occlusion (`covered_by`) — which is what lets one call replace the accessibility tree plus a per-node `DOM.getBoxModel` loop. Default cap is 120 (`truncated` is true when more exist). Limits match the upstream payload: open shadow roots, iframes, canvas, and nested scrolling are not covered; file inputs still appear as buttons but choosing a file is out of scope; use `cdp()` for the rest.

The install is additive and reversible: a workspace that already has an agent-written `agent_helpers.py` keeps its contents and gains one marker block (an upgrade rewrites only that block), and anything unexpected is debug-logged rather than failing the call. `tools/browser_exec_snapshot.py` owns the payload and its version.
