...
- **You want to attach to a Chrome, Brave, Chromium, or Edge window you can watch.** Run `/browser connect` with nothing after it. It is a CLI slash command: type it in `hermes` or `hermes chat` in a terminal, not in a Discord, Telegram, or WebUI chat. It auto-launches or attaches to a local browser at `http://127.0.0.1:9222`. Pass a `ws://host:port` endpoint only when the browser is already running with remote debugging somewhere else, such as on another port or another machine. See [Local Chromium-family browser via CDP](#local-chromium-family-browser-via-cdp-browser-connect).
