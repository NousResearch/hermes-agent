...
- **Hermes auth and codex auth are separate sessions.** You need both `codex login` AND `hermes auth add openai-codex` for the cleanest UX (the runtime uses codex's session for the LLM call). Hermes deliberately does not import or share OAuth state with codex CLI because refresh tokens are single-use.
