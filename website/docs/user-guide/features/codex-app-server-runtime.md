...
- **Stateful Hermes tools remain available** — active `memory`, `session_search`, `todo`, and `delegate_task` tools are exposed through Codex's dynamic-tool protocol and execute through the live AIAgent rather than a stateless subprocess.
