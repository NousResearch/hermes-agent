...
- **`kanban_show`** — read-only board query for the worker to check its own context.
- **`kanban_create` / `kanban_link`** — worker-visible follow-up/dependency tools. A dispatcher-spawned worker can spawn follow-up tasks (`kanban_create`) and declare dependency edges (`kanban_link`) for work it discovers mid-task; both gate on `HERMES_KANBAN_TASK` like the other worker handoff tools.
- **`kanban_list` / `kanban_unblock` / `kanban_archive`** — orchestrator-only board-routing tools, intentionally hidden from task workers. Available for orchestrator agents running on the codex runtime that route work via Kanban (board-wide listing, unblocking, and archiving tasks).
