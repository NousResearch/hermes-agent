...
- **`browser_navigate` / `browser_click` / `browser_type` / `browser_press` / `browser_snapshot` / `browser_scroll` / `browser_back` / `browser_get_images` / `browser_console` / `browser_vision`** — full browser automation via Camofox, Browserbase, Browser Use, Browserless, or Firecrawl.
...
| Browser automation (Camofox/Browserbase/Browser Use/Browserless/Firecrawl) | yes | yes (via MCP callback) |
