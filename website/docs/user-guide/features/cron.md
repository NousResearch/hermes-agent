...
If a hand edit leaves `jobs.json` malformed, the scheduler repairs it on the next load instead of stopping: entries in the `jobs` list that are not JSON objects are dropped, and a `repeat.completed` that is not a non-negative integer is normalized to one (0 when it can't be read). Each repair is logged as a warning (value types only, never contents).
