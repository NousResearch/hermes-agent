...
delegation:
  quality_gate:
    command: ["/usr/local/bin/my-judge"]          # argv list — NEVER a shell string (refused)
    ...
    # env_passthrough: ["MY_JUDGE_PROFILE"]       # extra env vars the judge may see (secrets are scrubbed)
```
...
delegation:
  ...
  #   command: ["/usr/local/bin/my-judge"]
