...
2. Call `/api/dashboard/plugins/rescan` with the dashboard session token after installing or editing a backend. This works for both `hermes dashboard` and Desktop's `hermes serve`: lifecycle-free API routers can be mounted or replaced without disconnecting clients. Dashboard install/update/enable/disable/remove actions also reconcile the routes.
   - A failed import retains the previous route/module references and logs the failure; it cannot roll back arbitrary import side effects (including mutations of imported helpers). In-flight requests retain their selected handlers across route replacement/removal; this is not process-wide transactional isolation.
   - Removed or disabled plugins lose their routes on rescan, but already-started lifecycle services stop only at backend shutdown. If the enable/disable policy readers raise (for example a malformed list entry in `plugins.enabled`/`plugins.disabled`), reconciliation logs the error and removes all tracked plugin API routes without importing plugin code; an unreadable `config.yaml` still falls back to defaults as before. Other server routes remain available; rescan reports `ok: false, policy_error: true`. Fix the policy and rescan again.
   ...
   - Routers with startup/shutdown callbacks or a custom/composed lifespan require a backend restart for **any API-file change**, even a handler-only edit. Rescan retains their boot generation without reimporting it and lists them in `restart_required`. Newly discovered routers with lifecycle hooks, or updates adding hooks, are not published live; any previous routes remain. Their source must be imported to inspect the router, so import side effects are still possible. Services are never started again during rescan. REST mutation responses include this reconciliation report under `api_routes`; `count` is the discovered-plugin count, not the successfully mounted count.
   - Only the declared API file is reloaded for lifecycle-free routers. Imported helper modules remain cached and need a backend restart. Desktop's file watcher and **Reload desktop plugins** reload only its JavaScript half; call the backend rescan separately.
