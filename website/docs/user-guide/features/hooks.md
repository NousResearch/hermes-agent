...
| `transform_llm_output` | Transform | Before `post_llm_call` and final delivery; first non-empty string replaces the response. | `response_text`, `session_id`, `model`, `platform`, `tool_activity` | Full final assistant text. |
...
| `tool_activity` | `list[str]` | Tool names called THIS turn only, in chronological order (same turn-boundary rule as the reasoning-extraction helper — never crosses into a prior turn). Empty list when no tools ran. Lets a plugin judge "was this claim looked up?" (e.g. flag a response containing numbers/versions/paths when `tool_activity` is empty) without maintaining its own parallel ledger of the turn's tool calls. |
