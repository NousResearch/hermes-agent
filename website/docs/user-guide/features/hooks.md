...
| `pre_tts_synthesis` | Directive/control | Before each synthesis request: once per `text_to_speech_tool` invocation (including gateway auto-TTS), or per cleaned sentence/piece for chunked PCM streaming (gateway, CLI speaker and dashboard WebSocket). Return `None`, `{"action": "block", "message": "..."}`, or `{"text": "replacement spoken script"}`. Malformed directives, callback errors and timeouts fail closed. Rewritten scripts are checked by all hooks again; a further rewrite that changes the script is rejected. To require a policy plugin even if its import/register fails or it registers no hook, set `tts.pre_synthesis.required_plugins` to a list of plugin keys (for example `['my-policy']`) in that profile's `config.yaml`, and enable/install each plugin separately. If any required owner is missing, server synthesis is refused before the provider; desktop client-direct TTS relays to the guarded server. Without this opt-in Hermes retains its usual no-hook behavior. Removing the key (or setting `[]`) restores that default; keep the requirement during plugin repair to avoid fail-open. Previously issued client-direct credentials cannot be revoked by this setting. | `text`, `provider` | The script is user content; no credentials, voice or model identifiers are passed. Live provider-native voice sessions (for example GPT-Live) do not use this TTS pipeline. |
