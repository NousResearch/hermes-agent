...
Fires **once per turn**, before the tool-calling loop begins. All valid callback returns are aggregated in plugin order and injected into the current turn's user message. The stored user-message `content` stays clean (see below for the `api_content` replay sidecar and the multimodal case) — contrast [`pre_persist_user_message`](#pre_persist_user_message), whose returns become part of the stored `content` itself.
...
Fires **once per turn**, on the agent path, just before the inbound user message is appended and persisted to the session store. Where [`pre_llm_call`](#pre_llm_call) context stays out of the stored `content` (kept, at most, in the row's `api_content` replay sidecar), `pre_persist_user_message` returns are folded into the user message's `content` itself — the injected block is part of the **canonical transcript** that search, compaction, exports and later turns read, and it survives a restart.
