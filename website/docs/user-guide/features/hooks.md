...
| `pre_verify` | Directive/control | At the bounded edited-code verify gate (also on no-edit turns when `agent.pre_verify_on_no_edit_turns` is enabled); first valid continue/block-stop directive keeps the turn going. | `session_id`, `platform`, `model`, `coding`, `attempt`, `final_response`, `changed_paths` | Draft response and changed paths. |
...
| `changed_paths` | `list` | Files the agent edited this turn (sorted). Non-empty by default; `[]` only when `agent.pre_verify_on_no_edit_turns` is enabled |
...
Set `agent.pre_verify_on_no_edit_turns: true` (default `false`) to fire the same gate on turns that edited **no** files, with `changed_paths=[]`. Same payload, same `has_hook` guard, same `agent.max_verify_nudges` bound — it exists for policy hooks whose subject is not the diff (backlog/board/review-state reconciliation), which otherwise have no supported way to continue a no-edit turn. With the default `false`, no registered hook sees a call it did not see before.
