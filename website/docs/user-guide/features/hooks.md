...
| `pre_tts_synthesis` | Directive/control | Before each synthesis request: once per `text_to_speech_tool` invocation (including gateway auto-TTS), or per cleaned sentence/piece for chunked PCM streaming (gateway, CLI speaker and dashboard WebSocket). Return `None`, `{"action": "block", "message": "..."}`, or `{"text": "replacement spoken script"}`. Malformed directives, callback errors and timeouts fail closed. Rewritten scripts are checked by all hooks again; a further rewrite that changes the script is rejected. With a policy hook active, desktop client-direct TTS resolves to server relay; already-issued client-direct credentials cannot be revoked by this hook. | `text`, `provider` | The script is user content; no credentials, voice or model identifiers are passed. Live provider-native voice sessions (for example GPT-Live) do not use this TTS pipeline. |
