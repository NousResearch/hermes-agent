...
| `gateway_ready` | Observer | Once gateway adapter maps and delivery routing are ready; async callbacks only, awaited in registration order with a 10-second per-callback timeout. | `gateway`, `adapters`, `profile_adapters` | In-process gateway and adapter handles; callback failures/timeouts are isolated. |
| `gateway_stopping` | Observer | Once during shutdown, after completion-batch cancellation and before adapter teardown; async callbacks only, awaited in registration order with a 10-second per-callback timeout. | `gateway`, `adapters`, `profile_adapters` | In-process gateway and adapter handles; callback failures/timeouts are isolated. |
