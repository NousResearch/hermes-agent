...
| `on_session_pre_switch` | Observer | CLI-only: immediately before `new_session()` finalizes the old session or mutates the session identity. Return values are ignored; callback/dispatch failures are fail-open. | `old_session_id`, `cli` | Session identifier plus the privileged in-process `HermesCLI` object. |
| `on_session_post_switch` | Observer | CLI-only: after `new_session()` completes successfully, including durable-row creation, title handling, memory-provider handoff, and `on_session_reset`; it does not fire when the switch raises. Return ignored. | `old_session_id`, `new_session_id`, `cli` | Session identifiers plus the privileged in-process `HermesCLI` object. |
