...
Outbound webhooks POST a signed JSON payload to HTTP endpoints you configure whenever a matching
lifecycle event fires — the push-side mirror of the [inbound webhook platform](../messaging/webhooks.md).
Configuration, wire format, signature verification, replay protection, and delivery semantics are
documented in **[Outbound Webhooks → ./outbound-webhooks.md](./outbound-webhooks.md)**.
