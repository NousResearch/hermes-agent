...
| `pre_delivery` | Directive/control | Before final persistence and delivery; may pass, request a bounded retry, or withhold the candidate. | `session_id`, `task_id`, `turn_id`, `platform`, `model`, `attempt`, `original_prompt`, `final_response`, `conversation_history` | Full prompt, response, and history. |
...
`pre_delivery` is a retry-capable final-response gate for policy and fidelity sidecars.
Return `{"action": "RESET", "message": "..."}` to keep the original turn alive and
request a corrected response. Hermes permits at most three automatic retries. Return
`EXHAUSTED` to replace and withhold the candidate; `PASS`, `AMBIGUOUS`, and `NUDGE` allow
normal delivery. The gate runs before the candidate is persisted or emitted.
