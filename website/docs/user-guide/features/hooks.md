...
| `transform_persisted_row` | Transform | Once per session-db row as it is flushed; first dict replaces the row (the callback gets a copy). | `agent`, `message`, `row`, `msg_idx` | Full transcript row content. |
| `transform_mcp_servers` | Transform | Once per `mcp_servers` load, before `${VAR}` interpolation and the suspicious-server filter; first dict replaces the server map (may add, drop or rewrite servers). | `servers` | Server commands and `env` blocks may carry credentials. |
| `transform_mcp_child_env` | Transform | Once per stdio MCP child spawn, after the server's own `env`, before delegated-child scrubbing; first dict replaces the child env. | `server_name`, `env` | The env may carry credentials from the server config or a secret source. |
