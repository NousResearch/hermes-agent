...
| `pre_api_request` | Observer + request mutations | Per provider attempt, immediately before the request. A returned dict may rewrite `reasoning_effort`, `max_tokens`, `temperature`, `top_p` (other keys ignored); the prompt is not mutable. | `task_id`, `turn_id`, `api_request_id`, `session_id`, `user_message`, `conversation_history`, `platform`, `model`, `provider`, `base_url`, `api_mode`, `api_call_count`, `retry_count`, `request_messages`, `message_count`, `tool_count`, `approx_input_tokens`, `request_char_count`, `max_tokens`, `started_at`, `middleware_trace`, `request` | High sensitivity: legacy `user_message`, `conversation_history`, and `request_messages` are intentionally raw; prefer sanitized `request`. |
...
Fires for each provider attempt immediately before sending it. The legacy `user_message`, `conversation_history`, and `request_messages` fields are raw and intentionally unsanitized for compatibility; new consumers should prefer the sanitized `request` envelope. Returning a dict rewrites `reasoning_effort`, `max_tokens`, `temperature`, or `top_p` on the outgoing request (last writer wins); every other key, and any non-dict result, is ignored — so a plugin that returns correlation data behaves exactly as before. The prompt itself (`messages`, `system`, `tools`) is deliberately not mutable here: rewriting it mid-conversation would invalidate the provider's cached prefix.
