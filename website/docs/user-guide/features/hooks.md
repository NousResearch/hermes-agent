...
- Callback exceptions are logged and later callbacks continue. They are otherwise skipped, except on `pre_gateway_dispatch`, where each exception synthesizes a `skip` denial so a broken admission callback fails closed. A callback that fails the same way on every call (typically a signature naming a field the hook does not send, e.g. `tool_data` instead of `tool_name`/`args`) is reported **once** at WARNING — the message lists the fields the hook provides — and identical repeats go to DEBUG, so a mis-declared plugin cannot flood the log.
