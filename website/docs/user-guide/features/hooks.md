...
- **Not delivered on isolated compute-host turns** ([#122297](https://github.com/NousResearch/hermes-agent/issues/122297)). With dashboard process isolation enabled, the compute-host frame carries no voice context, so hooks see `{}` even on a spoken turn. Detect voice by its presence, and treat absence as "unknown", not as "definitely typed".
- **Desktop coverage is partial today** ([#122296](https://github.com/NousResearch/hermes-agent/issues/122296)). Only the GPT-Live full-duplex mode tags its turns. The default `chained` voice mode (mic → STT → ordinary turn → TTS) submits nothing to distinguish itself from typing, so a spoken turn there arrives as `{}`.
