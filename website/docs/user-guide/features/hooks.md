...
| `pre_kanban_dispatch` | Directive/control | At the dispatch respawn-guard chokepoint, inside the dispatch lock; return `{"action": "hold", "reason": "..."}` to hold a task. Keep subscribers fast. Invalid, missing-reason, or raising callbacks fall through. Shell hooks cannot use directive-only hooks. | `task_id`, `profile_name`, `board`, `assignee`, `lane`, `task` | Task and board metadata. |
| `pre_kanban_task_create` | Directive/control | Before the create write transaction; return `{"action": "suppress", "reason": "...", "existing_task_id": "..."?}` to suppress creation. An empty returned ID means no row was created; invalid, missing-reason, or raising callbacks fall through. Shell hooks cannot use directive-only hooks. | `title`, `body`, `board`, `assignee`, `session_id`, `idempotency_key` | Task content may contain project/user data. |
