...
| `pre_kanban_dispatch` | Directive/control | At the dispatch respawn-guard chokepoint; return `{"action": "hold", "reason": "..."}` to hold a task. Invalid, missing-reason, or raising callbacks fall through. Shell hooks cannot use directive-only hooks. | `task_id`, `profile_name`, `board`, `assignee`, `lane`, `task` | Task and board metadata. |
