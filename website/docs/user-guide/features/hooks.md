...
| `on_session_reset` | Observer | CLI/TUI session boundary and gateway after the replacement session exists; return ignored. | CLI: `session_id`, `platform`, `reason`; TUI: `session_id`, `platform`; gateway: those plus `reason`, `old_session_id`, `new_session_id`, `session_key`, `source` | Session and routing identifiers. |
...
| `pre_gateway_dispatch` | Directive/control | Incoming non-internal message before auth/pairing/dispatch; first valid `skip`, `rewrite`, or `allow` controls flow. | `event`, `gateway`, `session_store`, `session_key`, `source` | Extremely privileged in-process objects expose inbound user/routing data and host handles. |
...
| `session_key` | `str`, optional | Gateway-only stable messaging route; it does not rotate with `session_id`. |
| `source` | `dict`, optional | Gateway-only copy of the current platform-neutral `SessionSource`. |
...
| `session_key` | `str` | Stable messaging route for this chat or thread. |
| `source` | `dict` | Copy of the platform-neutral source metadata. |
