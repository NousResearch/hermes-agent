...
| `hermes:gateway_agent_lifecycle` | Observer event stream | After gateway `agent:start`, `agent:step`, or `agent:end`; subscribe with `ctx.subscribe`, never emit. | `lifecycle_schema_version`, `event_type`, `platform`, `user_id`, `chat_id`, `thread_id`, `chat_type`, `profile`, `session_id`, `message`; `agent:step` adds `iteration`, `tool_names`; `agent:end` adds `response`, `model`, `provider`, `failed` | Plain bounded data only; no gateway, adapter, SDK, or client handles. Delivery is queued and cannot block the turn. |
