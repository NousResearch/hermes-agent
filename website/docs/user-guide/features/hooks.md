...
| `card_action_response` | Directive/control | Feishu interactive-card tap, on the SDK callback thread, after built-in approval / update-prompt buttons are handled. First dict with `header` or `elements` is returned as the in-place card (`P2CardActionTriggerResponse`); `None` falls through to the synthetic `/card` command. Python plugins only; must stay synchronous and fast (no HTTP). | `action_value` (button `value` dict), `chat_id`, `open_id`, `token` | `action_value` is plugin-defined JSON; `token` is the Feishu card-action callback token. |
