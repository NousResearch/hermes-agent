...
| `post_api_request` | Observer | After normalized provider success; return ignored. | `task_id`, `turn_id`, `api_request_id`, `session_id`, `platform`, `model`, `provider`, `base_url`, `api_mode`, `api_call_count`, `api_duration`, `started_at`, `ended_at`, `finish_reason`, `message_count`, `response_model`, `response`, `usage`, `cost`, `assistant_message`, `assistant_content_chars`, `assistant_tool_call_count` | Sanitized `response` is available, but raw normalized `assistant_message` may contain model/user content; `usage` and `cost` are accounting data. |
...
`cost` is this call's estimate from the same pricing path that feeds the session totals — `{"estimated_cost_usd": float | None, "cost_status": "actual" | "estimated" | "included" | "unknown", "cost_source": str}` — or `None` when the provider reported no usage. The `codex_app_server` runtime fires this hook once per turn for its single provider call; timing, `api_request_id` and `response` are not visible across the app-server boundary and are `None` there. The turn result carries the same records as `api_call_records`, one per provider call (`model`, `provider`, the token buckets, and the three cost fields), so a caller can attribute spend to one turn without subscribing.
