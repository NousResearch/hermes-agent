...
- **Lane priority inversion**: `kanban.max_in_progress_per_profile` does not preempt. Once a lane is full, a higher-priority `ready` card waits for as long as the cards holding its slots keep running. That holds even when a holder has lower priority. The dispatcher never stops a healthy worker to free a slot, because that would throw away real work. Instead, a `lane_priority_inversion` error appears on the card after `kanban.diagnostics.lane_inversion_threshold_seconds` (default 5 min). It shows up in `hermes kanban diagnostics`, `hermes kanban show` and on the dashboard. It fires only when the card's lane is at the cap and at least one holder has strictly lower priority. The detail names the lower-priority holders. It also lists every fully idle lane, meaning a claimable profile with no running, ready or review card, and suggests `hermes kanban reassign <task> <idle-lane>`. `delegate_task` child contexts still cannot reassign: the fence in `kanban_db` is the trust boundary for every board write. From inside that fence, the way to re-route is to create a replacement card on the idle lane.
