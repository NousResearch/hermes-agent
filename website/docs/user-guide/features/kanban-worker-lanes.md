...
- **Per-task progress timeout** — optional `task.progress_timeout_seconds` (set via `hermes kanban create --progress-timeout`). **Semantic progress** comes from a `kanban_heartbeat` with a non-empty `note` (model-authored milestone narration). **Automatic progress** is recorded internally when qualifying tool calls complete — verified `write_file` / `patch`, or successful terminal runs classified as tests, build, typecheck, lint, compile, `git commit`, or `graphify update` — using sanitized category labels only (never command text, paths, or output). Automatic runtime heartbeats (`note=None`) and ordinary reads/searches do **not** advance `last_progress_at`. When the silence budget expires, the dispatcher emits `progress_stalled`, terminates the host-local worker, and re-queues — counting toward the same failure circuit breaker as `timed_out`.
