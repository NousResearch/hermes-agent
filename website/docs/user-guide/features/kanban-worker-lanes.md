...
- **Stale claim TTL** — a worker that claims and then never heartbeats / completes / blocks gets reclaimed after `DEFAULT_CLAIM_TTL_SECONDS` (15 min default) — but only if the worker process has actually died. A live worker (slow model spending 20+ min in one tool-free LLM call) gets the claim *extended* instead of killed; only a dead PID is reclaimed. A PID the dispatcher cannot verify (claim from another host or PID namespace, or one with no recorded namespace) is neither extended nor signalled — the claim is released at the TTL.
- **Crashed worker** — a worker whose PID has vanished from the dispatcher's own host *and* PID namespace is detected by `detect_crashed_workers` and reaped; the task increments `consecutive_failures` and may auto-block when the breaker trips.
