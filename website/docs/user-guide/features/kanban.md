...
| `kanban.worker_isolation` | `auto` | How each dispatched worker is isolated from the gateway. `auto` (default): when `systemd-run --user --scope` works on the host (Linux with a user systemd session), spawn each worker inside its own transient scope `hermes-kanban-<task_id>.scope` with its own memory ceiling; otherwise fall back to the classic unisolated spawn (macOS, containers). `systemd-scope`: same, but log a warning instead of silently falling back. `none`: never isolate — the pre-isolation behaviour; also the rollback setting. Scoped workers survive gateway restarts (the scope is an independent unit, not a child of the gateway service), are re-adopted by the new gateway when their PID is verified alive and their heartbeat is fresh, and are stopped via the whole scope (`systemctl --user stop`, SIGTERM→SIGKILL) on timeout / reclaim / crash cleanup, so dev servers and browsers a worker spawned die with it. `hermes kanban show`/`runs` print the scope unit while a run is live in one. |
| `kanban.worker_memory_max_mb` | unset (computed) | Per-worker cgroup memory ceiling (MiB) when isolation is active — set as the scope's `MemoryMax`, with `MemorySwapMax` at the same value. Default: the tighter of the gateway's own cgroup limit and half the host RAM, floored at 64 MiB and capped at 4 GiB. Explicit values are clamped to those bounds. |
