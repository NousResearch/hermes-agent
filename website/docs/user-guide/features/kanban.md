...
3. Call `kanban_heartbeat(note="...")` every few minutes during long operations. Heartbeats report liveness, while each previously unseen tool-and-arguments signature reports progress. The dispatcher reclaims a task after `kanban.dispatch_stale_timeout_seconds` without distinct progress (default 7 minutes), even if a looping worker continues to emit heartbeats. A reclaim is benign (the task goes back to `ready` for re-dispatch without a failure-counter tick) but you lose your current run's progress.
...
| `stale` | `{elapsed_seconds, last_heartbeat_at, last_progress_at, progress_age_seconds, progress_timeout_seconds, pid, terminated}` | Task produced no previously unseen tool-and-arguments signature for `kanban.dispatch_stale_timeout_seconds` (default 7 minutes), even if liveness heartbeats stayed fresh. Dispatcher SIGTERM'd the host-local worker (if any) and reset the task to `ready` for re-dispatch. Does NOT tick the failure counter (stale is dispatcher-side progress detection, not a worker fault). |
