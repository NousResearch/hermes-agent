...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready or review task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (ready lane: a completed run happened in the last hour — wait for review before re-running), `active_pr` (ready lane: a GitHub PR URL appears in a recent comment — a prior worker already opened a PR), `self_review` (review lane: the row's `assignee` still equals the implementer recorded on its `review_requested` event — spawning would hand the card back to its own author; name an explicit reviewer to clear it). The task stays in `ready` (or `review`); the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
