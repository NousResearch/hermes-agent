...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree under `.worktrees/<id>/` for coding tasks. Use `worktree:<path>` to pin the exact target path. The dispatcher creates it before spawning the worker, using `--branch` when provided, and runs the worker with that directory as its cwd. Clean task-id-keyed worktrees are removed after completion, cancellation (archive), or spawn failure; their branches remain available. Dirty worktrees and explicit custom paths are preserved to prevent data loss.
