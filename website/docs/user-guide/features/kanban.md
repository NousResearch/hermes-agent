...
booked as a protocol violation — and `78` (`EX_CONFIG`) when the worker hit
something a retry cannot fix. Two failures take that route: the **provider**
rejected the profile's credential (401/403, revoked or invalid key), the model
(404 / model not found) or the TLS chain; or the **card itself is misdefined** —
every `--skills` name it was created with fails to resolve
(`Unknown skill(s): …`), so the agent raises during startup, before it ever
reads the card, and a re-spawn would reproduce the error byte for byte.
That **terminal error** trips the circuit breaker on the first
...
it) with the worker's own words in `last_failure_error`, instead of
...
after one attempt*, or as *Card names skills this profile does not have —
blocked after one attempt* when the card's skill list is the problem; fix the
assignee profile (`hermes -p <profile> auth` / `setup`, or install the missing
skills), then `hermes kanban unblock <id>`. The worker also writes its exit code as the
...
| `gave_up` | `{failures, effective_limit, limit_source, error, terminal_provider?}` | Circuit breaker fired after N consecutive non-successful attempts. Task auto-blocks with the last error. The effective limit resolves as task `max_retries`, then dispatcher `failure_limit` / `kanban.failure_limit`, then the built-in default. `terminal_provider: true` means the worker exited `78` on an error a retry cannot fix — the provider's credential/model (revoked, gone), or the card's own definition (none of its `--skills` names resolve) — and the breaker fired on that first attempt, sticky, regardless of the limit. |
