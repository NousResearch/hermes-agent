...
| `notify_artifacts` | `true` | Upload completed-task artifacts as native attachments with the notification. Set to `false` to keep the text notification while suppressing uploads. |
