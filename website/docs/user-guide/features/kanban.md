...
|------------|---------|--------------|
| `kanban.max_in_progress` | unset (unlimited) | Caps simultaneously running tasks across **all active boards** in a gateway dispatch tick (not independently per board). Useful for slow workers (local LLMs, resource-constrained hosts) so aggregate workers cannot exceed the configured ceiling. Single-board CLI `hermes kanban dispatch` applies the same cap to that board. Invalid or below-1 values log a warning and behave as unlimited. |
