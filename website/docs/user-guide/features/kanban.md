...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree under `.worktrees/<id>/` for coding tasks. It must be anchored at creation by an absolute `worktree:<repository-path>`, a linked project's primary repository, or the board's absolute `default_workdir`; an unanchored task is rejected before it enters the queue. Worker-side `git worktree add` creates it, using `--branch` when provided. **Preserved on completion.**
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `hermes skills list` to see what's available); there's no runtime install. Creation rejects a missing assignee or reports every unavailable forced skill before writing the card. Reassignment, unblock and review return re-check the stored list, the central claim boundary refuses a card whose forced skills no longer resolve — including after a reclaim or automatic requeue re-exposes it — and the dispatcher blocks legacy invalid cards before spawning them, in every case without consuming retry budget. A refused `kanban claim` names the missing skill.
