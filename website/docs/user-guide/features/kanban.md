...
| `kanban_block` | Stop work and route by why: `kind=dependency` waits in `todo` and auto-resumes only when an unresolved parent edge exists; otherwise it routes to `triage` so the missing edge can be added. `needs_input`/`capability`/`transient` surface to a human. Repeated same-kind re-blocks auto-escalate to `triage`. | `reason` |
...
| `dependency_wait` | `{reason, kind}` | Worker blocked with `kind=dependency` and at least one unresolved parent edge, so the task routes to `todo` (parent-gated, auto-promoted) instead of `blocked`. No human is needed while the encoded parent remains unresolved. |
| `dependency_wait_unresolved` | `{reason, kind}` | Worker blocked with `kind=dependency`, but no unresolved parent edge exists. The task routes to `triage` instead of entering an immediate promote/re-block loop. Add/link the missing parent or choose the correct non-dependency block kind. |
