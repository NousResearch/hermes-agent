...
`running`, it stopped without reporting: the dispatcher emits an
`unreported_exit` event and books the run under the `unreported` outcome — never
`crashed` ("the run ended" is not "the work failed") and never `completed`.
...
**Dispatcher-side recovery:** If the nudges are exhausted or the worker dies
before reaching the nudge, the dispatcher reads what *that run* left behind —
checkpoints (comments or attachments written while the run was open) and
heartbeats recorded against its run id, both keyed by the task — and when there
is anything at all it **parks the card** in the terminal state the worker
skipped: `blocked` with `block_kind=transient`, the last recorded state in the
block reason, a `dispatcher` comment carrying it, and a `blocked` event so
subscribers see the park. A parked card is never `done` and is not re-run from
scratch: unblock it to resume from the state the run recorded. A run that
recorded *nothing* observable is the one case with nothing to salvage, and keeps
the bounded retry (up to `_PROTOCOL_VIOLATION_FAILURE_LIMIT` consecutive
violations, default 3) before auto-blocking; its budget counts only
*consecutive* empty-exit violations — interleaved rate-limited requeues are
neutral, a parked run breaks the streak, and a per-task `max_retries` overrides
the bound. Either way the common cause is the model writing a plain-text answer
and exiting without using the Kanban tool surface.
...
| `unreported_exit` | `{pid, claimer, exit_code, protocol_violation, unreported_exit, parked, salvaged?, state?, reason?, kind?, recurrences?, source_status?}` | Worker exited successfully while the task was still `running` — it stopped without calling `kanban_complete` or `kanban_block`. The run is booked under the `unreported` outcome (`protocol_violation: true` in the run metadata is the durable marker), and the event says whether it was parked. **Parked** (`parked: true`, `salvaged: true`): the run left observable state behind (`state.checkpoints` — comments/attachments from its own window — and/or `state.heartbeats`), so the dispatcher parks the card (`blocked`, `block_kind=transient`, last recorded state in `last_failure_error` and in a `dispatcher` comment) instead of re-running it from scratch; `blocked` (or `block_loop_detected` at the recurrence limit) is emitted alongside. **Retried** (`parked: false`): the run recorded nothing observable, so it keeps the violation-only retry budget (up to `_PROTOCOL_VIOLATION_FAILURE_LIMIT`, default 3, *consecutive*, per-task `max_retries` overriding) and the task returns to `ready`; at the bound the dispatcher emits `gave_up` and auto-blocks. Runs recorded before this behaviour emitted the event kind `protocol_violation`. |
