...
**4. Verification-only isolated dispatch.** The dispatch that consumes the bypass does **not** spawn an ordinary worker. The verified contract flows atomically from the guard decision into the spawn, which runs a verification-only worker with only terminal/file tools and `HERMES_KANBAN_VERIFY_ONLY=1`. The child receives a single result channel plus a contract containing PR URL, approved head, approved base, task id, run id, claim lock, contract id, and a one-shot nonce. It does not receive the real kanban board env (`HERMES_KANBAN_DB`, `HERMES_KANBAN_TASK`, board slug, profile, hooks, or worker toolsets).

**5. Result protocol.** The verifier must write exactly one newline-delimited JSON object to the inherited result channel. Required fields are `version: 1`, `task_id`, `run_id`, `claim_lock`, `contract_id`, `pr_url`, `nonce`, and `action`. `action` is either `complete` or `re-block`; optional `summary` and `reason` are size-capped. Unknown fields, duplicate JSON keys, partial frames, multiple frames, invalid UTF-8, oversized payloads, binding mismatches, stale runs, and late results are rejected fail-closed. `complete` can only complete the still-current run; `re-block` maps to `kind=needs_input`.

**6. Authorization state machine.** Before spawn, the kernel creates a durable authorization row bound to the task/run/claim/contract/PR/nonce digest. Normal result application moves `authorized → reserved → started → applied` in one DB transaction with the task/run terminal transition; terminal failure states are `failed` and `reconciled`. The nonce itself and raw result envelope are never stored in the DB or audit log. Restart reconciliation is idempotent: pre-start `reserved` rows are restored to `authorized` only when the supervisor owner is missing/dead and the durable task/run/claim binding still matches; post-start rows whose owner is missing/dead are reconciled once and the task is re-blocked fail-closed. A live supervisor keeps ownership. Timeout/exit/protocol failure marks the authorization terminal and re-blocks the task in one transaction, so a late valid-looking result cannot win the race or leave a mixed task/auth state.
