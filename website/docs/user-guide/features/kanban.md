...
|------------|---------|--------------|
| `kanban.max_in_progress` | unset (memory-derived default on hosts where total RAM is readable; otherwise unlimited) | Caps simultaneously running tasks across **all active boards** (host-level / gateway-wide, not independently per board). Useful for slow workers (local LLMs, resource-constrained hosts) so aggregate workers cannot exceed the configured ceiling. Single-board CLI `hermes kanban dispatch` applies the same host-wide cap. Invalid or below-1 values log a warning and fall through to the memory-derived default (or unlimited when memory cannot be read). |
| `kanban.max_in_progress_per_profile` | unset (unlimited) | Per-profile variant of `max_in_progress` — caps how many tasks any single assignee profile may run concurrently on a board. Useful when one profile is slow or rate-limited but others should keep flowing. Applies alongside gateway-wide `max_in_progress`; both must allow a spawn for it to proceed. |
