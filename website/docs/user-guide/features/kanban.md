...
| `default_reviewer` | `""` | Fallback reviewer for a first `kanban_request_review` call that names none explicitly, used only when that would otherwise leave the review row self-assigned (assignee == implementer). Must name a real, installed profile or it's treated as unset. Empty = no fallback: `kanban_request_review` **refuses the transition** with a reason instead of writing a self-assigned row; see [Respawn guard](#respawn-guard)'s `skipped_self_review` for the dispatch-side backstop. |
...
The **review** lane gets its own guard instead: `skipped_self_review` fires when a review row's `assignee` still equals the implementer recorded on its `review_requested` event (or when that provenance is missing entirely — fail closed) — spawning would hand the card back to its own author to review, or to an unprovable reviewer. `kanban_request_review` (and `hermes kanban request-review`) prevent this at the source: a first review that names no explicit `reviewer` falls back to `kanban.default_reviewer` when that's configured to a real, live profile; when it's also unset (the default) or resolves to no installed profile, the call **refuses outright** — no row is written self-assigned in the first place, and the caller gets an immediate, actionable error. The dispatch guard above is the backstop for a row that reaches `review` self-assigned some other way (a hand reassign, a direct DB write, or a row from a pre-upgrade Hermes version), not the primary defense. Clear either case by naming an explicit reviewer — `hermes kanban assign <id> <reviewer-profile>` or reissuing with `hermes kanban request-review <id> --reviewer <profile>`.
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready or review task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (ready lane: a completed run happened in the last hour — wait for review before re-running), `active_pr` (ready lane: a GitHub PR URL appears in a recent comment — a prior worker already opened a PR). The task stays in `ready` (or `review`); the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. Self-review exclusion is a separate bucket — see `skipped_self_review` below. |
| `skipped_self_review` | `{reason}` | Review-lane row withheld because its `assignee` must not review its own work: `assignee_is_implementer` (the row's `assignee` equals the implementer recorded on its `review_requested` event — name an explicit, distinct reviewer to clear it) or `implementer_unknown` (no usable `review_requested` provenance at all, so a distinct reviewer can't be proven — fails CLOSED rather than trusting an unmarked row). Checked before any claim attempt, ahead of `respawn_guarded`, at the same operator-actionable level as `skipped_unassigned`. |
