...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a GitHub PR (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up. The guard still evaluates every dispatch tick; identical decisions persist the first `respawn_guarded` event and then only a heartbeat after a 1-hour cooldown (or immediately when the reason or PR URL set changes). See the `respawn_guarded` row in the [event reference](#event-reference).
...
| `respawn_guarded` | `{reason, pr_urls?, heartbeat?, quiet_seconds?}` | Dispatcher refused to re-spawn this ready task. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (a GitHub PR URL appears in a recent comment — a prior worker already opened a PR). The guard still runs every tick, but identical decisions are coalesced: the first event is persisted, repeats are suppressed, and a `heartbeat: true` summary with `quiet_seconds` is written after a 1-hour cooldown. A changed reason or PR URL set is recorded immediately. `active_pr` events include `pr_urls`. The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
