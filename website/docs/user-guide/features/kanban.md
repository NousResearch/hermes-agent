...
| `timed_out` | `{pid, elapsed_seconds, limit_seconds, sigkill}` **or** `{error, failures, retry_status}` | Two producers share this kind. Dispatcher wall-clock cap: `max_runtime_seconds` exceeded — SIGTERM, then SIGKILL after 5 s grace, and `limit_seconds` carries the cap. Worker iteration budget: `agent.max_turns` ran out, recorded by `agent/turn_finalizer.py` as `Iteration budget exhausted (used/max)` with **no** `limit_seconds` (the task has no cap). Both re-queue; the ping names the cause the payload actually recorded, never a limit the task does not have. |
