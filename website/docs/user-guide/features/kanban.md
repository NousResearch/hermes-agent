...
- **Dashboard** — `PATCH /api/plugins/kanban/tasks/:id` with `status="blocked"`, `block_kind="needs_input"`, and a structured `block_evidence` object. The PATCH contract is strict and whole-request atomic: unknown top-level fields are rejected outright (HTTP 400), and any rejected request — static validation, an invalid status transition, or parent gating (HTTP 400/409) — rolls back *every* change riding in the same request (assignee, status, priority, title/body), leaving no partial task or audit mutation behind.
...
**2. Human verifies, then unblocks with a typed intent.** Unblock the single blocked task with a structured `intent` containing `kind: "protected_merge_verifier"`, the immutable `pr_url`, `approved_head`, and `approved_base`, plus bounded `readiness_evidence` whose `ready` field is exactly `true`. Fields outside that vocabulary are rejected, never silently dropped — the same default-deny as block evidence, enforced by the one shared parser behind the CLI, dashboard, and `kanban_unblock` tool surfaces alike. The kernel grants a one-shot `active_pr` bypass token only when those values exactly match the evidence recorded for the prior `needs_input` block with a terminal blocked run. Malformed, unknown-kind, unknown-field, incomplete, stale, oversized, not-ready, or mismatched intents are rejected before the unblock mutates the task, on every surface.
...
**4. Verification-only isolated dispatch.** The dispatch that consumes the bypass does **not** spawn an ordinary worker and does **not** expose terminal tools, file tools, code execution, a normal profile, hooks, or a writable workspace cwd. The verified contract flows from the guard decision into an isolated Hermes chat process whose only enabled toolset is `kanban_verifier`; that toolset exposes only the one-shot `kanban_verifier_result` emitter under `HERMES_KANBAN_VERIFY_ONLY=1`. The child receives a single inherited result channel plus a contract containing PR URL, approved head, approved base, task id, run id, claim lock, contract id, canonical contract hash, and a one-shot nonce. The contract is re-canonicalized at the spawn boundary regardless of how it was stored, so the child's prompt and env carry only the readiness `ready` bit and its digest — raw readiness evidence never reaches any child-process, durable, or audit surface, and re-canonicalizing never changes the contract hash. It does not receive the real kanban board env (`HERMES_KANBAN_DB`, `HERMES_KANBAN_TASK`, board slug, profile, hooks, worker toolsets, or unrelated ambient credentials).

**5. Result protocol.** The verifier must write exactly one newline-delimited JSON object to the inherited result channel with no trailing bytes. Required fields are `version: 1`, `task_id`, positive integer `run_id`, `claim_lock`, `contract_id`, `contract_hash`, `pr_url`, `approved_head`, `approved_base`, `nonce`, and `action`. `action` is strictly either `complete` or `re-block`; optional `summary` and `reason` are size-capped. Unknown fields, duplicate JSON keys, partial frames, trailing bytes, multiple frames, invalid UTF-8, oversized payloads, binding mismatches, stale runs, and late results are rejected fail-closed. A frame also only counts once the supervisor observes end-of-file on the channel within the deadline — a verifier that writes a complete-looking frame but holds the channel open is rejected fail-closed with reason `result_no_eof`, never treated as a finished result. `complete` can only complete the still-current run; `re-block` maps to `kind=needs_input`.

**6. Authorization state machine.** Before spawn, the kernel creates a durable authorization row bound to the task/run/claim/contract id/canonical contract hash/PR/head/base/nonce digest. Normal result application moves `authorized → reserved → started → applied` in one DB transaction with the task/run terminal transition; terminal failure states are `failed` and `reconciled`. The nonce itself and raw result envelope are never stored in the DB or audit log, and nonce digest checks use constant-time comparison. The started row is owned by the durable verifier process identity (`hostname:pid`). Restart reconciliation is idempotent: pre-start `reserved` rows are restored to `authorized` only when the supervisor owner is missing/dead and the durable task/run/claim binding still matches; post-start rows whose owner is missing/dead are reconciled once and the task is re-blocked fail-closed. A live verifier process keeps ownership. Timeout/exit/protocol failure marks the authorization terminal and re-blocks the task in one transaction, so a late valid-looking result cannot win the race or leave a mixed task/auth state.

**7. Temporary verifier roots and cleanup.** Each verifier run gets a fresh `0700` root under the board's `verifier-roots/` directory and the child sees that root as `HERMES_HOME`, `HOME`, and its temp directory. The root allocator refuses a symlinked parent and verifies containment before launch. Normal and failure exits try to remove the run root. Cleanup failures are audited as redacted events and retried by reconciliation; orphaned `run-*` entries under `verifier-roots/` can be removed later, with symlinks unlinked rather than followed.

**8. Failure rollback.** If the consuming dispatch never actually starts a worker — claim race, workspace mount failure, spawn failure — the exact consumed token is restored verbatim (audited as `guard_bypass_restored`), so a verified recovery isn't lost to a transient failure. The restored token is still one-shot, and the restore never clobbers a newer grant.

**Threat model and platform limits.** This verifier protects the kanban respawn path from stale claims, duplicate PR retries, forged result frames, ambient board access, and cleanup path traversal. It does not prove GitHub state by itself, merge code, deploy, or protect against a local user with the same OS account tampering with local files. POSIX uses an inherited one-shot file descriptor read behind `select`; Windows uses an explicit inherited handle and never issues a blocking pipe read — it polls a non-blocking peek, and after the process exits it drains the channel under a short bounded EOF grace window rather than the full result deadline. Filesystems that cannot enforce `0700` still get isolated roots and containment checks, but permissions are best-effort.

**Migration, compatibility, rollout, and rollback.** No config keys, deployment steps, or restarts are involved — the feature is live as soon as the code ships. Existing boards migrate additively; older binaries ignore the verifier authorization table and ordinary kanban workers keep their existing env, profile, hooks, tools, and board pins. Stop submitting typed evidence/intents to roll back operationally; pending tasks continue under the normal `active_pr` guard. Code rollback does not require destructive schema rollback.

**Observability.** Watch `guard_bypass_granted`, `guard_bypass_consumed`, `guard_bypass_restored`, `verifier_authorization`, `verifier_authorization_reconciled`, `verifier_root_cleanup_failed`, `verifier_root_cleanup_retried`, and `respawn_guarded` in the [event reference](#event-reference). Events use authorization ids, states, and reason codes only; they do not include nonce values or raw result envelopes. Repeated identical guard events are coalesced to avoid tick-level audit spam.
...
| `verifier_authorization` | `{authorization_id, state}` | Protected-merge verifier authorization moved through a durable state. Payloads intentionally omit the nonce and raw result envelope. |
| `verifier_authorization_reconciled` | `{authorization_id, from_state, to_state, reason}` | Restart/timeout/crash reconciliation restored a pre-start authorization or fail-closed a post-start one. |
| `verifier_root_cleanup_failed` | `{authorization_id, reason}` | The per-run verifier temp root could not be deleted. The stored reason is redacted to an error class/message; retry reconciliation will try again. |
| `verifier_root_cleanup_retried` | `{authorization_id, result}` | A later cleanup reconciliation removed a previously failed verifier root and cleared its cleanup error. |
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (a GitHub PR URL appears in a recent comment — a prior worker already opened a PR). The task stays in `ready`; repeated identical reasons are coalesced in the audit log. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
