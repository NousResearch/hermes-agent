...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree under `HERMES_KANBAN_WORKTREE_ROOT/<board>/<task-id>/` (default `~/worktrees/<board>/<task-id>/`) for coding tasks when no explicit path is supplied. Worker-side `git worktree add` creates/registers it before implementation. For repo boards, prefer explicit hierarchy paths under `/root/worktrees/<repo>/<branch>/` so all related parent/child cards share one registered worktree.
