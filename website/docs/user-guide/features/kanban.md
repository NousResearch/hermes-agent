...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree under `.worktrees/<id>/` for coding tasks. Use `worktree:<path>` to select the target checkout. When that path is already a linked checkout on a different branch, Hermes treats it as a repository anchor and creates a per-task nested worktree instead of reusing the wrong branch. Worker-side `git worktree add` creates the task checkout, using `--branch` when provided. **Preserved on completion.**
