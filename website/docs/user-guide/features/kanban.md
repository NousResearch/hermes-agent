...
The **review** lane gets its own guard instead: `self_review` fires when a review row's `assignee` still equals the implementer recorded on its `review_requested` event — spawning would hand the card back to its own author to review. `kanban_request_review` (and `hermes kanban request-review`) never block on this: a first review that names no explicit `reviewer` best-effort falls back to `kanban.default_reviewer` when that's configured to a real, live profile; only when it's also unset (the default) or resolves to no installed profile does the row land here self-assigned and held, rather than spawning a worker to review its own work. Clear it by naming an explicit reviewer — `hermes kanban assign <id> <reviewer-profile>` or reissuing with `hermes kanban request-review <id> --reviewer <profile>`.
