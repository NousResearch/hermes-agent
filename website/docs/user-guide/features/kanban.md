...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), is still inside the cooldown after a rate-limited run (`rate_limit_cooldown`), or completed a run successfully within the guard window (`recent_success`). This prevents repeat worker storms on the same bug or task while a human catches up. See the `respawn_guarded` row in the [event reference](#event-reference).
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset or auth to be fixed), `rate_limit_cooldown` (latest run ended rate-limited and the cooldown has not elapsed — wait before probing again), `recent_success` (a completed run happened in the last hour — wait for review before re-running). The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
