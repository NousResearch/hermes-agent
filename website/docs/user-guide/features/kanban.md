...
The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `hermes profile create --description "..."`, `hermes profile describe <name> --text "..."`, `hermes profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer never lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child is stored with `assignee` set to `kanban.default_assignee` (or the active default profile if that's unset). Note that even manually-created tasks without `--assignee` are left unassigned in the DB. `kanban.default_assignee` is applied by the dispatcher on the next tick, not at creation time.
...
| `default_assignee` | `""` | Fallback assignee applied by the dispatcher at dispatch time, not at task creation. Used in two places: (1) when the decomposer picks an unknown profile for a child task; (2) when the dispatcher encounters any unassigned ready task, including ones created manually without `--assignee`. Tasks are stored without an assignee until dispatched. Changing this value re-routes tasks already in the queue on the next tick. |
