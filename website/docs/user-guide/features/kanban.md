...
| `kanban_withdraw` | Retract a card **this run filed** and should not be worked — a duplicate, a brief superseded before it started, a card filed twice because the first create looked like it had failed. It leaves dispatch on the spot: the card lands in the terminal `archived` state (no dispatcher tick can promote or claim it; a worker already spawned on it is stopped), and the `withdrawn` event records the actor + reason. Only the filer may withdraw: the card's create-time `creator_task_id` must equal the caller's own `HERMES_KANBAN_TASK`, so a `task_id` naming someone else's card is refused. Withdrawing the card you are *working* is refused too — `kanban_complete` / `kanban_block` close that one — as is a card already `done`/`archived`. | `task_id` |
