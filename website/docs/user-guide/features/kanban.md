...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree at `~/.hermes/worktrees/<task_id>/` for coding tasks. Use `worktree:<path>` to pin the exact target path. The dispatcher provisions the worktree before spawning the worker (`git worktree add` against the live checkout at `HERMES_KANBAN_LIVE_CHECKOUT` — default `~/.hermes/hermes-agent`), creating branch `kanban/<task_id>` off `myfork/main` (override base ref via `HERMES_KANBAN_WORKTREE_BASE_REF`). On respawn the existing worktree is reused so committed work survives crashes. When the kind is omitted, tasks whose title/body mentions hermes-agent source paths (`hermes_cli/`, `tools/`, `gateway/`, `tests/`, `agent/`) or git/PR verbs (`git rebase`, `gh pr`) auto-default to `worktree`; everything else stays `scratch`. Worktrees for `done`/`archived` tasks are GC'd 24h after completion (worktree removed, `kanban/<task_id>` branch deleted). **Preserved on completion.**
