...
| `unblocked` | `{status, resume_status, by}` | `blocked → ready` (or `todo` if parents are still open). `by` attributes the surface: `cli:<profile>`, `tool:<profile>:<session>`, `dashboard`, `reassign` (a cross-profile reassignment clearing a breaker trip), or `dispatcher:auto-gave-up-recovery` (stale gave_up cooldown elapsed; also carries `auto: true` and `stale_hours`). Resets the dispatcher's `consecutive_failures` but deliberately preserves `block_recurrences` so the loop breaker keeps its memory. `run_id` is `NULL`. |
...
| `gave_up` | `{failures, effective_limit, limit_source, error}` | Circuit breaker fired after N consecutive non-successful attempts. Task auto-blocks with the last error. The effective limit resolves as task `max_retries`, then dispatcher `failure_limit` / `kanban.failure_limit`, then the built-in default. A `gave_up` block is not terminal: once the task has sat blocked longer than `kanban.gave_up_stale_hours` (default 1.0h; set to `-1` to disable auto-recovery entirely), `recompute_ready` auto-recovers it (counter reset, `unblocked` event with `{auto: true, by: "dispatcher:auto-gave-up-recovery", stale_hours}`) so a transient infra failure can't park a task forever. Explicit worker/operator blocks (`kanban_block`) stay sticky and never age out. Reassigning a breaker-parked task to a different profile clears the trip immediately (audited as `by: "reassign"`) — an explicit human recovery, no cooldown wait. |
