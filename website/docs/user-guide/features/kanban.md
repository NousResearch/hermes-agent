...
| `block_loop_detected` | `{reason, kind, recurrences, limit, parked}` | A task was unblocked and re-blocked for the same reason `BLOCK_RECURRENCE_LIMIT` times (default 2). Instead of landing in `blocked` again — where a cron would keep unblocking it — it parks in `triage` (`parked: true`) for a human decision. Auto-specify/decompose will not promote a parked card; `hermes kanban unblock` admits exactly one run (`block_loop_admitted`). |
| `block_loop_admitted` | `{status, kind, recurrences, once}` | Operator recovery for a parked block-loop card. Promotes to `ready`/`todo` without rewriting title/body or clearing `block_kind`/`block_recurrences`. One-shot: a later same-kind block parks the card again. |
