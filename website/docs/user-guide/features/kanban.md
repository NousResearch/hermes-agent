...
**Live-claim guard on complete.** A `running` task whose claim is still live is only completed by the claim's owner (`kanban_complete` from inside the run, or by passing `expected_run_id`) or by an explicit operator override: `hermes kanban complete <id> --force` and the dashboard's "mark done" action. A claim-less `hermes kanban complete <id>` or an orchestrator session's `kanban_complete` is refused with a pointer to `--force` / `hermes kanban reclaim`, so a second session can no longer close a live claim underneath it. "Live" means two things: a claim whose spawned worker process still exists (PID + start-time fingerprint), and — since `hermes kanban claim` never spawns a worker — a claim from another session whose `claim_expires` TTL has not lapsed. The claimer holding that lock may close its own claim (`claim` → work → `complete` needs no override), and once the TTL passes, or with `--force` / `expected_run_id`, the card completes as it always did. Completing `ready`, `blocked` or `review` cards without a claim is unchanged. `request_review` applies the same fence.
