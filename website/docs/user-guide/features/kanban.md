...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a GitHub PR (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up. An explicit `hermes kanban unblock <id>` permits one claim that would otherwise be stopped by the `active_pr` guard. The dispatcher consumes this one-shot override when it claims the task, so a later retry is guarded again even if spawning that claim fails. The same one-shot override is minted when the dispatcher auto-promotes a task back to `ready` — the only re-queue path a dependency wait (`block` with `kind=dependency`) ever sees, since `unblock` only reaches `blocked`/`scheduled` tasks. The automatic path mints at most one override per PR-comment evidence, so a task that keeps re-blocking with the same cause cannot turn the guard into a spawn-per-tick loop; once that override is consumed the task waits for a newer PR comment or an explicit operator cycle (`block` + `unblock`). With either carve-out, duplicate-PR avoidance shifts to the worker: check `gh pr list --head <branch> --state all` before `gh pr create` and continue the existing PR rather than opening a second one. See the `respawn_guarded` row in the [event reference](#event-reference).
...
| `promoted` | `{respawn_overrides?}` | `todo → ready` because all parents hit `done`. When the task has a recent PR-comment guard active and no override for that evidence was consumed already, the event carries a one-shot `active_pr` override (`respawn_overrides`, same shape as on `unblocked`) that permits one otherwise-guarded claim; the claim consumes it. `run_id` is `NULL`. |
...
| `unblocked` | `{status?, respawn_overrides?}` | `blocked → ready` (or `todo` if parents are still open), either manually or via `/unblock`. Resets the dispatcher's `consecutive_failures` but deliberately preserves `block_recurrences` so the loop breaker keeps its memory. An explicit `hermes kanban unblock` records a one-shot override for the current `active_pr` guard, permitting one otherwise-guarded claim; that claim consumes the override, even if its subsequent spawn fails. `run_id` is `NULL`. |
