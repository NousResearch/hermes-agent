...
**2. Human verifies, then unblocks with a typed intent.** Unblock the single blocked task with a structured `intent` containing `kind: "protected_merge_verifier"`, the immutable `pr_url`, `approved_head`, and `approved_base`, plus bounded `readiness_evidence` whose `ready` field is exactly `true`. The kernel grants a one-shot `active_pr` bypass token only when those values exactly match the evidence recorded for the prior `needs_input` block with a terminal blocked run. Malformed, unknown, incomplete, stale, oversized, not-ready, or mismatched intents are rejected before the unblock mutates the task, on the CLI, dashboard, and `kanban_unblock` tool surfaces alike.
