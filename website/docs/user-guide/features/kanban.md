...
- **Agents drive the board through a role-scoped `kanban_*` toolset.** Dispatcher workers receive task lifecycle actions (`kanban_complete`, review handoffs, block, heartbeat) plus shared show/comment/attachment/fan-out tools. Profiles that explicitly enable `kanban` receive board-routing actions (`kanban_list`, `kanban_unblock`) plus those shared tools, without worker-only lifecycle actions. The model reads and routes tasks by calling tools directly, *not* by shelling out to `hermes kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
