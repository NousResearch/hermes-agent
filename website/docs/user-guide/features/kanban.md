...
| `block_loop_detected` | `{reason, kind, recurrences, limit}` | A task was unblocked and re-blocked for the same reason `kanban.block_recurrence_limit` times (default 2; configurable in `config.yaml`). Instead of landing in `blocked` again — where a cron would keep unblocking it — it routes to `triage` for a human decision, breaking the unblock↔re-block loop. Raise the limit for pipelines that legitimately block on more than two stages. |
