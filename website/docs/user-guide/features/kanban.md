...
| `kanban.max_in_progress_per_model` | unset (unlimited) | Caps active tasks for each exact explicit `provider_override` + `model_override` pair across boards. Cards without both overrides are unaffected. Useful for local backends that serialize requests to one model. |
