...
| `dependency_wait` | `{reason, kind, dependency_state}` | Worker blocked with `kind=dependency` — the task is only waiting on another task, so it routes to `todo` (parent-gated, auto-promoted) instead of `blocked`. `dependency_state` captures the parent ids/statuses seen at park time so the dispatcher only wakes the task after real graph truth changes. |
| `dependency_wait_without_unmet_parent` | `{reason, kind, dependency_state}` | Worker tried to park on `kind=dependency` but there was no unmet formal parent at exit time. Hermes still keeps the task dormant instead of spinning fresh runs, and emits this explicit defect signal so operators know the real blocker must be encoded as a formal dependency edge rather than left in prose/comments. |
