...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `HERMES_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc. Auto-blocked cards receive one fresh attempt after `kanban.failure_retry_seconds` (default: 24 hours), so a transient outage cannot abandon them permanently.
...
| `kanban.failure_retry_seconds` | `86400` (24 hours) | Cooldown before the dispatcher gives an auto-blocked `gave_up` card one fresh retry. If the retry fails, the normal failure budget trips again and starts another cooldown. Explicit worker/operator blocks are never auto-retried. Set to `0` to require manual unblocking. |
...
| `gave_up` | `{failures, effective_limit, limit_source, error}` | Circuit breaker fired after N consecutive non-successful attempts. Task auto-blocks with the last error. The effective limit resolves as task `max_retries`, then dispatcher `failure_limit` / `kanban.failure_limit`, then the built-in default. After `kanban.failure_retry_seconds`, the dispatcher arms the breaker for one probe and gives the card one fresh attempt. |
| `breaker_retry` | `{previous_failures, armed_failures, effective_limit, cooldown_seconds, gave_up_at}` | An auto-blocked card's cooldown elapsed. Its failure streak is armed one below the effective limit and it is eligible for promotion to its saved ready/review phase, so one failed probe immediately trips the breaker again. Explicit blocks do not emit this event. |
