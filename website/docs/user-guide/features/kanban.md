...
| `kanban.worker_isolation` | `auto` | How each dispatched worker is isolated from the gateway. `auto` (default): when `systemd-run --user --scope` works on the host (Linux with a user systemd session), spawn each worker inside its own transient scope `hermes-kanban-<task>-r<run_id>.scope` (the run suffix guarantees a respawn never collides with a lingering scope from the previous attempt) with its own memory ceiling; otherwise fall back to the classic unisolated spawn (macOS, containers) — a refused systemd-run launch also falls back per-run, with a warning. `systemd-scope`: same, but a refused launch fails the spawn loudly (`spawn_failed` with the systemd-run stderr) instead of falling back. `none`: never isolate — the pre-isolation behaviour; also the rollback setting. Scoped workers survive gateway restarts (the scope is an independent unit, not a child of the gateway service) and are re-adopted by the new gateway. Liveness is judged by worker self-registration: the worker writes its own PID + start-time fingerprint on first activity (the PID recorded at spawn is the systemd-run launcher's, which dies with the gateway), re-adoption verifies that registered PID (scope-cgroup truth first), and a scoped run that never registers within its 120 s launch grace is recorded as a spawn failure, not a crash. Every terminal transition — complete, block, review, timeout, reclaim, crash cleanup, archive, schedule, ancestor-reopen invalidation — stops the whole scope (`systemctl --user stop`, SIGTERM→SIGKILL, death confirmed from the unit's `cgroup.procs` — `unknown`/`deactivating` states are NOT treated as dead), so dev servers and browsers a worker spawned die with it; a stop that cannot be confirmed never releases the task's claim — the run is marked `stopping` and the next dispatcher tick re-checks, so no duplicate worker is ever spawned beside a wedged unit — and an audit sweep reaps any scope left behind. `hermes kanban show`/`runs` print the scope unit while a run is live in one. |
