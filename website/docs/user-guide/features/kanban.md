...
| `telegram_topic_rollups` | `false` | For Telegram topic subscriptions, mirror major lifecycle summaries to the forum's General topic. Replies to those summaries reopen the exact task topic. |
| `specialist_status_updates` | `false` | Let the assigned profile's connected Telegram bot announce major lifecycle steps inside the task topic. Group bots should use mention-only routing and the bot-loop guard. |
| `telegram_forum` | unset | Optional control-room projection. With `enabled`, `chat_id`, `board`, and `profile`, the notifier creates one forum topic per active card, subscribes it, posts a task card, and pins that card. `profile_mentions` maps assignees to Telegram `@handles`. The supergroup itself must be created by a human and the managing bot must be an administrator. |
