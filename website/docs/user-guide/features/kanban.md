...
| `kanban.default_max_runtime_seconds` | unset (no cap) | Dispatcher-wide runtime cap for running tasks that have no per-task `max_runtime_seconds`. Without it such a worker is bounded only by `dispatch_stale_timeout_seconds`, so one that keeps heartbeating while making no progress can run for hours. Enforced exactly like a per-task cap (SIGTERM, grace, SIGKILL, `timed_out` event with `limit_source: dispatcher_default`). A per-task `max_runtime_seconds` always wins, larger or smaller. |
