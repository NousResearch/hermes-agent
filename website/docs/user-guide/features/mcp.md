...
Each named profile keeps its own token cache under `~/.hermes/profiles/<name>/mcp-tokens/`. If several profiles should present the *same* identity to one server, the default profile can export that server's token pool with `oauth.share_with_profiles: true`; a profile whose own entry for the server is identical then reads and refreshes the root's tokens instead of holding a second grant. Nothing else is inherited, and a profile that edits its entry simply leaves the pool. See [Sharing one OAuth grant with named profiles](../../reference/mcp-config-reference.md#sharing-one-oauth-grant-with-named-profiles).
