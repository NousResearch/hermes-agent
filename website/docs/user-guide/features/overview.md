...
- **[Memory Providers](memory-providers.md)** — Plug in external memory backends, including bundled providers and standalone options such as Memori and Scope Recall, for cross-session user modeling and personalization beyond the built-in memory system.
