...
- **[Subagent Delegation](delegation.md)** — The `delegate_task` tool spawns child agent instances with isolated context, restricted toolsets, and their own terminal sessions. Run 3 concurrent subagents by default (configurable) for parallel workstreams. In production dispatcher setups, this primitive is often wrapped in a stable role router so orchestration stays policy-driven instead of ad hoc.
- **[Kanban (Multi-Agent Board)](kanban.md)** — Durable orchestration for long-running, human-in-the-loop, or cross-role work. Use it when work must survive restarts, move across named workers, or preserve a full audit trail.
