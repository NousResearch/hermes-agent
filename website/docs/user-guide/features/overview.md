...
- **[Memory Providers](memory-providers.md)** — Plug in external memory routers/backends (Layered, Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, Supermemory) for memory taxonomy, cross-session user modeling, and personalization beyond the built-in memory system.
