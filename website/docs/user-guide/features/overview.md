...
- **[Voice & TTS](tts.md)** — Text-to-speech output and voice message transcription across all messaging platforms, with native providers such as Edge TTS (free), ElevenLabs, OpenAI TTS, MiniMax, Mistral Voxtral, Google Gemini, xAI, NeuTTS, KittenTTS, and Piper; bundled local Japanese providers Irodori TTS and VOICEVOX; plus custom command providers for any local TTS CLI.
