...
- **[Voice & TTS](tts.md)** — Text-to-speech output and voice message transcription across all messaging platforms, with eleven native provider options: Edge TTS (free), ElevenLabs, OpenAI TTS, MiniMax, Mistral Voxtral, Google Gemini, Murf, xAI, NeuTTS, KittenTTS, and Piper — plus custom command providers for any local TTS CLI.
