...
| Register a realtime speech-to-speech voice provider | `ctx.register_realtime_voice_provider(provider)` — see [Register a realtime voice provider](/developer-guide/plugins#register-a-realtime-voice-provider); driven by [`hermes realtime`](/user-guide/features/realtime-voice) |
...
| A **realtime speech-to-speech provider** (Gemini Live, xAI, a duplex model of your own) | Python provider plugin — `ctx.register_realtime_voice_provider(provider)`. The plugin owns transport and audio; Hermes owns tools, approvals, and the `hermes realtime` session. | [Register a realtime voice provider](/developer-guide/plugins#register-a-realtime-voice-provider) · [Realtime Voice](/user-guide/features/realtime-voice) |
