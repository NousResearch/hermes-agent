...
| [`pre_model_route`](/user-guide/features/hooks#pre_model_route) | Once per turn after the user message and copied history are ready, but before system-prompt restore/build and the first provider request. Return `{"model": "...", "provider": "...", "reason": "..."}` (`new_model`/`target_provider` are aliases). The first valid proposal wins and is resolved by Hermes. Malformed Python-plugin proposal dictionaries, unresolved routes, and callback exceptions are logged; malformed shell-hook output is discarded during parsing. Payload fields are `session_id`, `user_message`, copied `conversation_history`/`messages`, `is_first_turn`, `model`, `provider`, `platform`, `sender_id`, `chat_id`, `chat_name`, `chat_type`, `thread_id`, `gateway_session_key`, and `has_images`. Hermes does not add configured credential fields, but conversation content may itself contain sensitive data; enable only trusted routing plugins. Plugins can feature-detect this contract with `hermes_cli.plugins.PRE_MODEL_ROUTE_HOOK_VERSION` (currently `1`). |
