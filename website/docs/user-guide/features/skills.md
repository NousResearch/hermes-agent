...
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins. When `skill_view` loads it by bare name, copies whose `SKILL.md` is byte-identical resolve to the local one; copies whose content differs are refused as ambiguous instead of silently picking one.
