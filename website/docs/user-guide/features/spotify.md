...
- **Cron jobs skip memory by default** (`cron.skip_memory: true`) so they don't touch your memory store. Set `cron.skip_memory: false` to allow external memory providers only; local MEMORY.md/USER.md injection stays off for cron either way.
