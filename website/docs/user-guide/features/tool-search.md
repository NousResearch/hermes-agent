...
tools:
  tool_search:
    ...
    never_defer: []     # names that stay eager, plugin/MCP tools included
    defer:              # replace the curated default; [] keeps every core tool eager
      - computer_use
      - session_search
      - image_generate
      - todo_list
...
| `defer` | Curated list | Tool names replaced by the bridge by default. The list may include cold built-in tools as well as MCP/plugin tools; an explicit list replaces it, and `[]` keeps every core tool eager. MCP and plugin tools defer whatever this list says; name them in `never_defer` to keep them eager. |
| `never_defer` | `[]` | Tool names that never defer, whatever their toolset — the only way to keep a plugin or MCP tool eager. Bridge tool names are ignored. |
