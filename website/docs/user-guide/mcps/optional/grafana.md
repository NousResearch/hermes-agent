...
<!-- This page is auto-generated from optional-mcps/grafana/manifest.yaml by website/scripts/generate-mcp-catalog-docs.py. Edit the manifest, not this page. -->
...
**Source:** [https://grafana.com/docs/grafana-cloud/ai-tools/mcp-servers/cloud-mcp/](https://grafana.com/docs/grafana-cloud/ai-tools/mcp-servers/cloud-mcp/)

Install this catalog entry with:

```bash
hermes mcp install grafana
```

or pick it interactively with `hermes mcp`. Uninstall with `hermes mcp uninstall grafana` (the server's config block is removed; any credentials in `~/.hermes/.env` are preserved).
...
The manifest below is the source of truth. It lives at `optional-mcps/grafana/manifest.yaml` in the hermes-agent repo.
