...
| `DISCORD_COMMAND_SYNC_POLICY` | No | `"safe"` | Controls native slash-command startup sync. `"safe"` diffs existing global commands and only applies patchable updates, preserving command IDs when metadata cannot be patched. `"bulk"` preserves the old `tree.sync()` behavior. `"off"` skips startup sync entirely. |
| `DISCORD_COMMAND_SYNC_ALLOW_RECREATE` | No | `false` | Allows safe sync to delete/recreate commands for unpatchable metadata drift. Keep this disabled in normal operation because recreating global commands can briefly invalidate Discord client command caches after a gateway restart. |
