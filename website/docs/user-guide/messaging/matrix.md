...
| **Thinking and tool activity** | Matrix `tool_progress` defaults to **all** and accumulates into one editable `m.replace` event (same transport as interim commentary). Opt out with `display.platforms.matrix.tool_progress: off`. `tool_progress_grouping: separate` is ignored on Matrix so tools cannot spawn extra roots. Thinking panes stay off by default. |
