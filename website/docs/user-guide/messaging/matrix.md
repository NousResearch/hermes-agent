...
| **Thinking and tool activity** | Matrix `tool_progress` defaults to **off** so tool streams do not permanently clutter the room timeline. When you opt in (`display.platforms.matrix.tool_progress: all` or `new`), Hermes keeps **one** sticky Tool activity message and updates it with `m.replace` (plain counter body + always-visible HTML tool list). Thinking panes stay off by default. |
