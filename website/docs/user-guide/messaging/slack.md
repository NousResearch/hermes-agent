...
The `:eyes:` → `:white_check_mark:` lifecycle reactions follow the same gate: a channel the bot answers without a mention gets the acknowledgement a mention would have earned, so a long turn is not silent. `require_mention_channels` pins a channel back to mention-only for both.
:::
