...
| `deliver` | No | Where to send the response: `github_comment`, `telegram`, `discord`, `slack`, `signal`, `whatsapp`, `matrix`, `mattermost`, `homeassistant`, `email`, `sms`, `dingtalk`, `feishu`, `wecom`, `wecom_callback`, `weixin`, or `log` (default). |
