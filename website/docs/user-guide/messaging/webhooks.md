...
|----------|----------|-------------|
| `events` | No | List of event types to accept (e.g. `["pull_request"]`); a single name or a comma-separated string (`push, pull_request`) also works and matches whole names only. If empty, all events are accepted. Event type is read from `X-GitHub-Event`, `X-GitLab-Event`, or `event_type` in the payload. |
...
| `skills` | No | List of skill names to load for the agent run; a single name or a comma-separated string also works. |
