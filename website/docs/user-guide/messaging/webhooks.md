...
| `script_failure_policy` | No | `ignore` by default preserves filter vetoes. Set `retry` for durable ingestion scripts: missing interpreter/script, timeout, launch failure or nonzero exit returns HTTP 503 with `Retry-After: 15`. Successful `[SILENT]` output remains quiet HTTP 200. |
