...
```json
{
  "_hermes": {
    ...
    "handoff_depth": 1,
    "delivery_id": "relay-task-2026-08-22T20:30:00Z"
  },
...
- `_hermes` accepts only `target_profile`, `handoff_depth`, and `delivery_id`; a request-side `toolsets` field fails closed. Toolsets come exclusively from `allowed_target_toolsets`.
- `_hermes.delivery_id` must be a unique 1–128 character identifier generated by the trusted relay. Because it is inside the signed body, unsigned headers such as `X-Request-ID` and `X-GitHub-Delivery` cannot change privileged execution identity.
...
- The gateway atomically claims each signed handoff delivery ID in `state.db` before dispatch. Claims survive gateway restarts and are not automatically expired, so retrying the same authenticated ID cannot repeat terminal/file work. A replay-store failure rejects the handoff instead of dispatching without protection. Depth and in-flight limits separately bound recursive or concurrent dispatch.
...
For generic senders, use the timestamp-bound V2 HMAC signature so the selector, delivery ID, and task body are integrity-protected together and captured requests expire. GitHub, Linear, generic V1/V2, and Svix signatures bind the body; GitLab's plain `X-Gitlab-Token` does not, so that authentication mode is not eligible for trusted profile handoff. Ordinary webhook routes remain on the constrained webhook-safe defaults.
...
For ordinary webhook routes, delivery IDs (from `X-GitHub-Delivery`, `X-Request-ID`, or a timestamp fallback) are cached in memory for **1 hour**. Duplicate deliveries (e.g. webhook retries) are silently skipped with a `200` response.

Trusted profile handoffs use the signed `_hermes.delivery_id` instead. The gateway claims that ID atomically and durably in `state.db` before dispatch, so changing unsigned transport headers or restarting the gateway does not admit the same privileged execution again.
