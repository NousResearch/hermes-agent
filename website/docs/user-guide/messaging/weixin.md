...
| `text_batch_delay_seconds` | `0.3` | Quiet period (seconds) before a buffered burst of rapid text messages is flushed as one combined request. iLink delivers messages individually, so this debounce avoids one agent invocation per fragment. Set `0` to dispatch each message immediately. |
| `text_batch_split_delay_seconds` | `1.0` | Extended flush delay used when the latest fragment is near the split threshold (long messages iLink may have chunked). |
