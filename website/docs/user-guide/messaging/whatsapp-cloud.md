...
| `WHATSAPP_CLOUD_SEND_READ_RECEIPTS` | `true` | Set to `false` to stop marking inbound messages read (no blue checkmarks). Disabling also turns off the typing indicator — Meta only exposes it as part of the mark-as-read request. Same key as `send_read_receipts` under `platforms.whatsapp_cloud.extra` in `config.yaml` (env wins when both are set). |
...
To opt out, set `WHATSAPP_CLOUD_SEND_READ_RECEIPTS=false` in `~/.hermes/.env` (or `send_read_receipts: false` under `platforms.whatsapp_cloud.extra` in `config.yaml`). Disabling read receipts also disables the typing indicator — Meta's Cloud API only exposes the indicator as part of the mark-as-read request, so there is no way to show "typing…" without also marking the message read.
...
| Read receipts | Opt-in (`send_read_receipts`, default off) | On by default (disable with `WHATSAPP_CLOUD_SEND_READ_RECEIPTS=false`) |
