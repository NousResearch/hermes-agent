...
Because a profile is its own Hermes home, a session in that profile loads that profile's `SOUL.md`. See [Context Files](./features/context-files.md) for which files are read from the profile home and which from the working directory.
