...
A v1 record with an empty endpoint cannot distinguish the default US service from a previously inherited `BWS_SERVER_URL`. Hermes re-encrypts that record into quarantine and requires a successful live fetch, even when the endpoint is now unset. Offline migration remains available for a matching, explicitly bound v1 endpoint.
