...
`<device>` above means a raw block device under any of its names: Linux `/dev/sd*`, `hd*`, `vd*`, `xvd*`, `nvme*`, `mmcblk*`, `md*`, `/dev/md/*`, `dm-*`, `/dev/mapper/*`, `loop*`, `nbd*`; macOS `/dev/disk0`, `/dev/disk0s2`, `/dev/rdisk0`; and the persistent-name symlinks `/dev/disk/by-{id,uuid,path,label,partuuid,partlabel}/*`. A path is read the way the kernel resolves it, so `../dev/sda`, `/tmp/../dev/sda`, `/dev//sda` and `/dev/./sda` are `/dev/sda`, while a file under some other directory called `dev` (`~/dev/sdk/token.json`, `../dev/sda-notes`) is not. Non-device `/dev` entries (`/dev/null`, `/dev/zero` as a source, `/dev/urandom`, `/dev/stdout`, `/dev/tty`, `/dev/shm/*`, ...) are untouched, and so are the read-only runs of these tools: `wipefs` without `-a`, `sgdisk -p`, `blkdiscard --help`, `diskutil list|info|mount|unmount`, `shred secret.txt`, and `dd if=/dev/sda of=backup.img` (reading a disk into a file).
