...
- Package runners (`npx`, `pnpx`, `bunx`, `pnpm dlx`, `yarn dlx`, `uvx`, `pipx run`) name a *package*, so a rule written for the package also meets its pinned and flagged spellings: `npx sketchy-tool *` matches `npx -y sketchy-tool@1.2.3 run`, and `uvx evil-pkg *` matches `uvx evil-pkg==1.0 run`. Only the package operand and the runner's own package options (`-p`, `--from`, `--with`, `--spec`) are folded; the package's own arguments stay literal.
