...
- The review policy is part of an approval's identity, and changing it revokes. The config is live-reloaded: the first command that matches the rule under its new `review` discards every session and permanent grant the pattern earned under the previous policy, so tightening `smart` → `human` asks a person again, and switching back to `smart` afterwards starts from zero (the old grant does not revive, even across a restart).
