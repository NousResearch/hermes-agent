...
| **AGENTS.override.md** / **AGENTS.md** / **agents.md** | Project instructions, conventions, architecture — commands, ports, paths, repo-specific workflows | You (or whoever authors the project) | One file per directory loads from Git root through the startup working directory; deeper copies are discovered progressively | Git root through the working directory + deeper subdirectories |
...
Only **one** project context type is loaded per session, first non-empty type wins: `.hermes.md` → the AGENTS directory chain → `CLAUDE.md` → `.cursorrules`. `SOUL.md` is loaded independently as the agent identity. See [Context Files](./features/context-files.md) for the full precedence and override rules.
:::
