...
| **AGENTS.override.md** | Personal, per-directory override of `AGENTS.md` (typically gitignored) | You | Loaded in place of `AGENTS.md` for that directory; nested copies are discovered progressively | Project working directory + subdirectories |
| **AGENTS.md** | Project instructions, conventions, architecture — commands, ports, paths, repo-specific workflows | You (or whoever authors the project) | Loaded into the system prompt at startup from the git root to the working directory; nested copies are discovered progressively | Project working directory + subdirectories |
...
Only **one** project context type is loaded per session, first match wins: `.hermes.md` → `AGENTS.override.md` / `AGENTS.md` chain → `CLAUDE.md` → `.cursorrules`. Within the AGENTS chain, the first matching file in each directory wins, so `AGENTS.override.md` shadows `AGENTS.md` in that directory. `SOUL.md` is always loaded independently as the agent identity — it is not part of that priority chain. See [Context Files](/user-guide/features/context-files) for the full list, including `CLAUDE.md` and `.cursorrules` compatibility.
:::
