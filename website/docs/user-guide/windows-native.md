...
- **Runtime** then *bypasses* recognized npm shims: the browser tool reads the located `.cmd`, finds the entrypoint it dispatches to — the `.js` for npm's Node shim layouts, or the native `.exe` that agent-browser's own postinstall writes into its optimized global shim — and launches that target directly, taking `cmd.exe` out of the spawn chain. That's what lets a URL containing `&` (or other cmd metacharacters like `| < > ^`) reach the browser child as one unmodified argument instead of being re-parsed by `cmd.exe` into broken "commands". A shim that isn't an exactly-recognized shape (or carries extra dispatch arguments) falls back to the `.cmd` unchanged, and the shortfall is logged.
