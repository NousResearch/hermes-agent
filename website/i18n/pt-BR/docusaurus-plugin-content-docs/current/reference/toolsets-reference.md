...
| `desktop_ui` | `close_terminal`, `focus_pane`, `open_preview`, `react_to_message`, `read_preview`, `read_terminal`, `read_window_below` | Ações que atuam no próprio app desktop do Hermes — ler/fechar o painel de terminal embutido, abrir e ler o browser in-app, identificar a janela do SO atrás do app, revelar um painel, reagir a uma mensagem. Ativado em sessões cuja origem é o app desktop, em qualquer backend ao qual ele esteja conectado (local, SSH, URL ou Hermes Cloud). Nunca presente em CLI, TUI, mensageria ou cron. |
