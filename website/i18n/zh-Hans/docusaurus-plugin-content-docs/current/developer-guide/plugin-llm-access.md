...
绑定是上下文局部的。`threading.Thread` 以全新上下文启动，且不在主线程上，因此其中的 `ctx.llm.current_invocation()` 返回 `None`，`inherit_turn_invocation=True` 会以 `no_turn_invocation` 拒绝。让 worker 在回合上下文的副本里运行：
...
回合内创建的 `asyncio` 任务本身就运行在上下文的副本里，无需额外处理。
