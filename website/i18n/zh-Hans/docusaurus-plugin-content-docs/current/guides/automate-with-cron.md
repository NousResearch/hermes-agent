...
**调度表达式。** 支持的格式：相对延迟（`30m`）、间隔（`every 2h`）、标准 cron 表达式（`0 9 * * *`）、ISO 时间戳（`2025-06-15T09:00:00`），以及 `daily at 9am`、`weekdays at 9am`、`every monday 9am` 这类自然的日期/时间短语（与 cron 表达式一样需要 `croniter` 包）。
