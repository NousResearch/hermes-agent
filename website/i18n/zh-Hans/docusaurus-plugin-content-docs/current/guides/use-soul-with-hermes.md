...
一个简单的判断原则：
- 如果某项内容应在所有地方生效，放入全局 `SOUL.md`
- 如果它是目录级 agent 身份，放入 `.hermes/soul.md`
- 如果它是项目指令而不是身份，放入 `AGENTS.md`

## 文件位置

Hermes 使用以下全局 SOUL 文件作为当前实例的回退：
...
可信项目上下文运行可以通过 CWD 中的 `.hermes/soul.md`、`.hermes/SOUL.md`、`soul.md` 或 `SOUL.md` 覆盖该回退。位于 git 仓库内时，Hermes 还会检查父目录直到 git 根目录；不在 git 仓库内时只检查 CWD。跳过上下文文件的隔离模式不会加载本地 soul。
...
Hermes 启动会话时，会选择可信的本地 SOUL 候选文件或 `HERMES_HOME` 回退，扫描其中的提示词注入（prompt-injection）模式，必要时进行截断，并将其作为 **Agent 身份标识**——系统提示词中的第 1 个槽位。这意味着选中的 `SOUL.md` 会完全替换内置的默认身份文本。
...
检查：
- 你编辑的是预期的全局回退或受支持的本地候选文件
- 已启用本地发现，且本地文件位于 CWD，或在 git 仓库内位于 CWD 与 git 根目录之间
- 文件不为空
- 编辑后已重启会话
