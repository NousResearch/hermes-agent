...
| `create <name> [--clone] [--clone-all] [--clone-from <source>] [--no-alias]` | 创建新 profile。`--clone` 从活跃 profile 复制 `config.yaml`、`.env` 凭据、`SOUL.md`、整理后的 `MEMORY.md`/`USER.md` 和 skills，同时保留全新的会话历史。`--clone-all` 复制除会话历史、备份、快照和检查点外的所有 profile 状态。`--clone-from` 指定源 profile，除非与 `--clone-all` 配合使用，否则会隐含 `--clone`。 |
