...
| `--script` | 位于 `~/.hermes/scripts/` 下的过滤/转换脚本。webhook payload 以 JSON 形式通过 stdin 传入；JSON stdout 会替换 payload。空 stdout、`[SILENT]` 或明确的忽略结果会有意忽略该 webhook；执行失败会返回 HTTP 500，以便提供方重试。参见[脚本过滤与转换](../user-guide/messaging/webhooks.md#script-filters-and-transforms)。 |
