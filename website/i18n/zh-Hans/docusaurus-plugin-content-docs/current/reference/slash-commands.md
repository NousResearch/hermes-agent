...
| `/stop` | 终止所有正在运行的后台进程 |
| `/queue [prompt]`（别名：`/q`） | 提供非空 prompt 时，将其作为一条 FIFO 轮次排队，不中断当前响应；首词始终是 prompt 文本，不会被解析为操作。不提供 prompt 时，显示当前可管理排队轮次的短期编号视图。使用 `/dequeue <N\|all>`（别名：`/dq`）只能删除该最新视图中的位置。 |
...
| `/queue [prompt]`（别名：`/q`） | 非空 prompt 会作为一条 FIFO 轮次排队，不中断当前 agent；首词始终是 prompt 文本，不会被解析为操作。不提供 prompt 时，Hermes 会打开当前可管理排队轮次的短期编号视图；使用 `/dequeue <N\|all>`（别名：`/dq`）只能删除该最新视图中的位置。Discord 的原生无 prompt slash interaction 会将同一视图渲染为仅调用者可见的 ephemeral 管理器。共享聊天中，文本视图会在平台支持时私密投递；否则 Hermes 只返回安全重试提示。 |
