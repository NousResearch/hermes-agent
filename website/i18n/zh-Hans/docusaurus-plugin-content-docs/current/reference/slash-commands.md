...
| `/stop` | 终止所有正在运行的后台进程 |
| `/queue <prompt>`（别名：`/q`） | 将任何非空参数作为一个完整 prompt 按 FIFO 顺序排队，不中断当前 agent 响应。它没有文字子命令；在 CLI 中不带 prompt 调用只会显示用法。 |
...
| `/queue <prompt>`（别名：`/q`） | 将任何非空参数作为一个完整 prompt 按 FIFO 顺序排队，不中断当前 agent；首词始终是 prompt 文本，不会被解析为操作。在 Discord 中，只有不带 prompt 的原生 slash interaction 会打开调用者的 ephemeral 私有管理器；通过普通 Gateway 文本发送的无参数命令仍只显示用法。 |
