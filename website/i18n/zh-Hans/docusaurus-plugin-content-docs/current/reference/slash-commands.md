...
| `/toolsets` | 列出可用工具集 |
| `/browser [connect\|disconnect\|status]` | 管理本地 Chromium 系浏览器的 CDP 连接。`connect` 只会将浏览器工具附加到已运行的 Chrome、Brave、Chromium 或 Edge 实例（默认：`http://127.0.0.1:9222`）。若端点不可达，命令会以 fail-closed 方式退出并输出手动启动指引，不会替换当前连接。`disconnect` 断开连接。`status` 显示当前连接状态。 |
