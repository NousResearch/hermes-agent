...
| `/stop` | 终止所有正在运行的后台进程 |
| `/queue [prompt]`（别名：`/q`） | 提供非空 prompt 时，将其作为一条 FIFO 轮次排队，不中断当前响应；首词始终是 prompt 文本，不会被解析为操作。不提供 prompt 时，显示当前 session 中可管理用户轮次的短期编号视图。使用 `/dequeue <N\|all>`（别名：`/dq`）只能删除自己最新视图中的位置。 |
...
| `/queue [prompt]`（别名：`/q`） | 非空 prompt 会作为一条 FIFO 轮次排队，不中断当前 agent；首词始终是 prompt 文本，不会被解析为操作。不提供 prompt 时，Hermes 会在当前聊天中发布该 session 中可管理用户轮次的短期编号视图；使用 `/dequeue <N\|all>`（别名：`/dq`）只能删除自己最新视图中的位置。所有实际路由到同一 session、且具备命令权限的参与者都能看到和管理同一队列。Discord 会渲染公开频道/线程卡片，但按钮仍绑定打开该快照的成员；其他成员可重新运行 `/queue` 获取自己的快照。内部 continuation 保持隐藏。 |
