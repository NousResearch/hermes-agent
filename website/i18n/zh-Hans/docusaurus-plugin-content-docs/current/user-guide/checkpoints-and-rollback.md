...
checkpoints:
  ...
  max_snapshots: 20           # 每个项目的最大检查点数（重写引用；延后 GC）
  max_total_size_mb: 500      # 由启动/手动维护执行的软上限
...
- **存储总大小上限** — `max_total_size_mb`（默认 500 MB）是软上限。交互式快照只记录待维护标记；标记至少存在一小时后可提前突破常规 24 小时启动维护间隔，也可由 `hermes checkpoints prune` 立即处理。维护过程会有界裁剪旧历史、执行 GC 并重新测量。每个项目始终保留最新 checkpoint，因此存储不一定能降到上限以下。
- **延后回收** — `max_snapshots` 只在交互路径重写当前项目引用；不可达对象由启动/手动维护回收，普通文件修改不会等待共享存储 GC。
