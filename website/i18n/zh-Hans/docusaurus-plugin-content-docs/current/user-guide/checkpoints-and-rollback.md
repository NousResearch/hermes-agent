...
- **真实剪枝** — `max_snapshots` 通过重写每项目引用并随后运行 `git gc --prune=2.hours.ago` 来强制执行，避免松散对象积累。两小时宽限期可保护并发会话刚写入的对象。如果 Git 报告实时 GC 已占用存储，Hermes 会安全跳过本次 GC；下次运行时由 Git 自行验证过期标记。
