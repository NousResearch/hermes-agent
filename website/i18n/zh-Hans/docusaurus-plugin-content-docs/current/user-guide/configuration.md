...
不设置这些值将保持旧版默认值（`HERMES_API_TIMEOUT=1800`s、`HERMES_API_CALL_STALE_TIMEOUT=90`s、原生 Anthropic 900s）。隐式的非流式 stale 检测会在本地端点上自动禁用，并且会在超大上下文下自动放宽。AWS Bedrock 的每条传输路径都使用相同的键：Converse（boto3 `read_timeout`）、AnthropicBedrock（Fable/Claude）以及 Mantle OpenAI Responses（Terra / GPT-5.6）。SDK 级重试保持为零，由 Hermes 外层重试循环负责退避；`compression.context_timeout_seconds` 不会延长这些套接字超时。请参阅 [`cli-config.yaml.example`](https://github.com/NousResearch/hermes-agent/blob/main/cli-config.yaml.example) 中的注释示例。
