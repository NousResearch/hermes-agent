...
**流中的工具进度：**
- **Chat Completions**：Hermes 发出 `event: hermes.tool.progress` 以提供工具启动可见性，同时不污染持久化的 assistant 文本。无法处理具名 SSE 事件的严格 OpenAI 客户端可设置 `gateway.platforms.api_server.tool_progress_events: false`（默认 `true`）关闭这些帧；同一开关也会关闭 provider 重试或恢复期间发出的 `event: hermes.status` 帧。内容块不受影响。该开关仅作用于 Chat Completions——`/v1/runs/{id}/events` 始终发出工具事件，`/v1/capabilities` 中的 `tool_progress_events` 功能描述的正是这一点。
