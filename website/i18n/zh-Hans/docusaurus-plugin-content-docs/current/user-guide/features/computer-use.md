...
1. `computer_use(action="capture", mode="some", app="Mail")` — 获取 Mail 的截图，其中每个侧边栏项目、工具栏按钮和邮件行均已编号。
...
| 提供商 | 支持视觉？ | 可用？ | 备注 |
|---|---|---|---|
| Anthropic（Claude Sonnet/Opus 3+） | ✅ | ✅ | 综合表现最佳；支持 SOME 与原始坐标。 |
...
**元素索引已过期** — SOME 索引仅在下次 `capture` 之前有效。任何改变状态的操作后请重新截图。
