...
会话启动时，Hermes 将 `AGENTS.override.md` / `AGENTS.md` 目录链（git 根目录 → 工作目录）加载到系统 prompt（提示词）中。在会话期间，当 agent 通过 `read_file`、`terminal`、`search_files` 等工具导航进入子目录时，它会**渐进式发现**这些目录中的上下文文件，并在其变得相关的时刻将其注入对话。
...
Hermes 兼容 Cursor IDE 的 `.cursorrules` 文件和 `.cursor/rules/*.mdc` 规则模块。若这些文件存在于项目根目录，且未找到更高优先级的上下文文件（`.hermes.md`、`AGENTS.override.md`、`AGENTS.md` 或 `CLAUDE.md`），则将其作为项目上下文加载。
...
3. **提示加载** — 若发现 `AGENTS.override.md`、`AGENTS.md`、`CLAUDE.md` 或 `.cursorrules`，则加载（每个目录先匹配先生效；同目录 override 优先）
