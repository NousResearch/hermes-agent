...
sidebar_position: 8
title: "上下文文件"
description: "项目上下文文件 — .hermes.md、AGENTS.md、CLAUDE.md、SOUL.md 以及 .cursorrules — 自动注入每次对话"
---

# 上下文文件

Hermes Agent 会自动发现并加载上下文文件，以塑造其行为方式。部分文件属于项目本地文件，从工作目录中发现。`SOUL.md` 是主要身份文件：Hermes 默认使用全局 `$HERMES_HOME/SOUL.md` 回退，并且只在可信项目上下文边界内使用 CWD 本地 soul。
...
| **SOUL.md** | 当前 Hermes 实例的主要身份与个性；可在可信项目上下文内使用 CWD 本地文件 | `$HERMES_HOME/SOUL.md` 回退；仅在启用项目上下文发现时检查 CWD/父目录 |
...
- `$HERMES_HOME/SOUL.md` 全局回退
- 启用项目上下文发现时，按 `.hermes/soul.md`、`.hermes/SOUL.md`、`soul.md`、`SOUL.md` 的顺序检查可信的 CWD 本地候选文件；仅在 git 仓库内向上搜索父目录，并在 git 根目录停止

重要说明：

- 若 `SOUL.md` 尚不存在，Hermes 会自动生成一个默认文件
- 禁用上下文文件时，即使仍启用 SOUL 身份，也会跳过 CWD 本地 `SOUL.md`
- `/status` 会以安全的逻辑标签显示当前使用的 SOUL.md 来源，不暴露绝对主机路径
