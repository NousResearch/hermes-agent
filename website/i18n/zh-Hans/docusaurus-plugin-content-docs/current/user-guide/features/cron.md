...
#### 代理撰写的频道摘要（`channel_summary`）

优先话题投递时，频道根消息通常是一个固定标签（`Hermes handoff — <任务名>`）。
对于繁忙频道中的高输出任务，可以让这一行变成 **agent 撰写的本次运行摘要**：
在 `attach_to_session` 之外，再通过 `cronjob` 工具为任务设置
`channel_summary: true`。

任务的 agent 会被指示在最终回复的开头写上
`[SUMMARY] <一到两行短句>`，接一个空行，然后是完整报告。投递时，摘要成为
频道根部的短消息，完整报告则落在其话题内。

频道根消息的替换只发生在专用话题锚定于一条已发布种子消息的平台上：Slack
（原生与 relay 前置均可）。在没有种子消息的话题平台上——Telegram 话题、
Discord 直接创建的话题——不存在可替换的频道根消息；摘要只是作为话题内简报的
开头行出现。

摘要提取刻意做到无损：摘要仍保留为完整报告的开头行，只有 `[SUMMARY]` 标记
本身被剥除。因此在任何不会新建专用话题的路径上——平铺投递、扇出目标、固定在
既有话题的任务、gateway 离线——完整文本照旧投递，只是以摘要开头（当 `cron.wrap_response` 开启时仍位于标准
`Cronjob Response:` 包装之内——搭配 `wrap_response: false` 可获得最整洁的输出）。摘要缺失或
超长（超过 2 行 / 400 字符）时回退为固定标签。
