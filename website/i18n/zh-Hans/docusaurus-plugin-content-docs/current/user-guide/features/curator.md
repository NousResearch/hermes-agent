...
默认情况下（`prune_builtins: false`），捆绑内置技能不会成为 Curator 候选项。操作员只有显式设置 `curator.prune_builtins: true` 后，Curator 才可在它们超过 `archive_after_days` 未使用时进行归档。通过 [agentskills.io](https://agentskills.io) 安装的 hub 技能始终不受影响。Curator 也**绝不自动删除**——最坏的结果是归档到 `~/.hermes/skills/.archive/`，这是可恢复的。
...
curator:
  ...
  prune_builtins: false        # 显式启用后才归档未使用的捆绑内置技能
```
