...
- **速率限制 / 计费**（HTTP 429、配额或额度耗尽）——立即触发；在同一提供商上重试无法清除配额
- **提供商过载 / 超时**（HTTP 503/529、连接超时）——快速重试一次后触发，因为瞬时抖动通常会自行恢复
- **服务器错误**（HTTP 500、502）——耗尽重试次数后
- **认证失败**（HTTP 401、403）——凭据刷新尝试失败后
- **未找到**（HTTP 404）——立即触发
...
:::info 按轮次，而非按会话
备用机制的**作用域为单次轮次**：每条新用户消息都从主模型重新开始（除非上一轮次的速率限制冷却仍在生效——此时会话会继续停留在备用上，直至冷却结束）。若主模型在某轮次中途失败，Hermes 会在该轮次内**按顺序遍历** `fallback_providers` 链：每次失败都会前进到下一个条目——同一轮次内不会回到更早的条目——不可用、配置错误或解析到刚刚失败的同一后端的条目会被跳过。只有整条链耗尽后才进入常规错误处理流程（重试，然后返回错误消息），随后一个短暂的冷却会阻止下一轮次立即重放整条链。恢复后，下一条消息会再次尝试主模型。
:::
