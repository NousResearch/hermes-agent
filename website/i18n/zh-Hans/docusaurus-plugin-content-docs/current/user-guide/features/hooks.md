...
| `{"action": "authorize"}` | 信任插件解析后的 `event.source`，跳过平台/全局用户认证与配对检查并继续分发。这是仅供受信任进程内身份/访问插件使用的安全敏感型绕过。 |
| `{"action": "allow"}` / `None` | 正常分发——运行完整的认证/配对/agent 循环链。 |

**使用场景：** 只听不回的群聊（仅在被 @ 时响应；将环境消息缓冲为上下文）；人工接管（所有者手动处理聊天时静默摄入客户消息）；跨平台身份解析与授权；按 profile 速率限制；策略驱动的路由。

插件必须只在已认证并授权解析后的身份时返回 `authorize`。Hook 异常、未知 action、`allow` 和 `None` 都会保留正常的失败关闭认证链。
