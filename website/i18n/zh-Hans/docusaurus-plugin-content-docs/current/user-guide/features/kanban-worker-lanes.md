...
在 production dispatcher 体系中，这通常会映射到稳定的角色路由，而不是自由形式委派。一个常见分工是：

- `architect` — 分解、风险和 acceptance criteria
- `coder` — 实现与调试
- `infra` — 部署与运行时健康
- `logic` — workflow JSON 与自动化映射

无论你最终采用什么命名，都要保持 routing contract 稳定，并向每条 lane 传递完整的 context packet。
