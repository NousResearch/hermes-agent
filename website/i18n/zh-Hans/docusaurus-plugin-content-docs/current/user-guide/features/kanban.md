...
**自动（默认）** —— `kanban.auto_decompose: true`。Gateway 内嵌调度器在每个 tick 运行**分解器**，受 `kanban.auto_decompose_per_tick`（默认每 tick 3 个任务）限制，以防批量加载分诊任务时突发消耗辅助 LLM。分解器读取粗略想法，查看你安装的配置文件及其描述，并要求 LLM 生成 JSON 任务图：要启动哪些任务、分配给谁，以及哪些依赖哪些。Hermes 先持久化生成任务之间的同级依赖边，再把每个生成任务链接为原始分诊任务的父级。因此，原始任务是生成任务图的依赖子级，绝不是其父级；它会保持存活直到所有生成任务完成，然后推进回 `ready`，让其受让人（`kanban.orchestrator_profile`，未设置时为活动默认配置文件）判断完成情况，并在工作未完成时添加更多任务。这是"丢一行描述，走开"的流程。
...
| `auto_subscribe_on_create` | `true` | 自动来源、父任务继承和默认目标订阅的唯一全局开关。即使设为 `false`，每次创建时提供的显式目标仍然有效。 |
| `notify_default_targets` | `[]` | 自动策略未找到来源或继承目标时使用的回退目标。 |
| `notify_inherit_depth` | `1` | 要检查的父任务层数：`0` 禁用继承，正整数限制层数，`null` 或 `"unlimited"` 遍历所有可达祖先。 |
...
### 创建时订阅策略 {#create-time-subscription-policy}

所有生产环境中的任务创建入口都把订阅决策交给同一个事务内策略。策略按以下优先级解析：

1. 每次创建的 `no_subscribe` 会禁止所有来源
2. 显式通知目标会取代自动目标
3. `kanban.auto_subscribe_on_create: false` 会停止自动解析
4. 持久的 gateway 或 TUI 来源会提供其投递目标
5. 任务根据 `notify_inherit_depth` 从实际父任务图继承订阅
6. 只有没有来源目标或继承目标时，才使用配置的默认目标

自动来源目标和继承目标可以同时存在。默认列表只是回退，不是附加广播列表。父图遍历可安全处理环。分解生成的子任务只检查已持久化的同级父任务图，不会把编排根任务当作隐藏父任务。

每个目标都需要 `platform` 和 `chat_id`。可选的 `thread_id`、`user_id` 和 `notifier_profile` 用于保留用户与投递所有权。Hermes 会去除首尾空白、规范化平台名称，并按 `platform`、`chat_id` 和 `thread_id` 去重。如果重复目标带有不同所有权元数据，则保留第一个规范化目标。

不同创建入口遵循以下范围规则：

- Gateway 和 worker 创建可以提供自动会话来源
- 直接 CLI 创建可以使用 `--notify` 或 `--no-subscribe`；配置的默认目标仍属于自动来源
- Dashboard 的 `POST /tasks` 只接受显式目标和显式退订；浏览器不会被视为自动通知目标
- Swarm 创建只把通知上下文应用到已完成的根卡片，不应用到 worker、验证者或综合者

幂等创建重试会在写入创建时订阅或任务事件之前返回现有任务。因此，重复目标和重试创建都会为每个目标身份保留一行订阅，并保留最初的一组事件。
...
| `POST` | `/tasks` | 创建（封装 `kanban_db.create_task`；接受 `triage`、`parents`、`notification_targets` 和 `no_subscribe`）。Dashboard 不会推断自动来源目标，也不会应用自动默认目标。 |
...
Dashboard 任务创建可以在请求正文中提供显式通知意图：
...
每个目标都需要 `platform` 和 `chat_id`，其他目标字段可选。将 `no_subscribe` 设为 `true` 可禁止提供的目标。省略目标列表或提供空列表时，即使存在自动默认目标，也不会创建 Dashboard 订阅。
...
启用 `kanban.auto_subscribe_on_create` 后，当你从 gateway 使用 `/kanban create "…"` 创建任务时，发起聊天会成为自动目标。其身份包含平台、聊天 id 和线程 id。如果来源提供用户 id 和通知 profile 所有权，目标也会保留这些字段。

以下流程为示意。完成通知的前缀会根据看板、受派人和通知界面而变化。
...
`--json` 输出标志不会改变订阅行为。使用 `--no-subscribe` 创建不通知的任务，或使用 `--notify` 指定显式目标。即使自动订阅已禁用，显式目标仍然有效。
...
启用自动订阅时，gateway 中的 `/kanban create …` 会订阅发起聊天。Gateway 通知器会把终态事件（`completed`、`blocked`、`gave_up`、`crashed` 和 `timed_out`）投递给每个订阅目标。完成事件包含 worker 结果的第一行。
