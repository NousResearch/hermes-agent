...
- **Agent 通过专用 `kanban_*` 工具集驱动看板** —— `kanban_show`、`kanban_list`、`kanban_complete`、`kanban_block`、`kanban_owner_action`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link`、`kanban_unblock`。调度器在 schema 中已内置这些工具来启动每个 worker；编排器（orchestrator）配置文件也可以通过 `kanban` 工具集显式启用。模型通过直接调用工具来读取和路由任务，*而不是*通过 shell 执行 `hermes kanban`。详见下方[Worker 如何与看板交互](#how-workers-interact-with-the-board)。
...
仅当被阻塞任务确实需要人类所有者作出决定或执行外部操作时，才使用结构化 owner action。它不同于 FYI 更新、受让 agent 可自行完成的工作、依赖等待、瞬时故障、能力诊断，以及不再需要个人回复的外部等待。

活动契约记录一个类别（`approval_required`、`decision_required`、`input_required` 或 `external_action_required`），以及确切操作、建议、为何现在重要、截止时间或紧迫性、等待后果、可选的 HTTP(S) 审查链接和简洁回复格式。所有字段均经过验证并有长度上限。

在阻塞任务时创建契约：

```bash
hermes kanban block <task-id> "需要批准" --kind needs_input \
  --owner-action-json '{"category":"approval_required","action":"批准已测试的候选版本。","recommendation":"建议批准。","why_now":"验证已通过。","urgency":"在下一次发布截止前。","consequence":"发布将继续暂停。","review_url":"https://example.com/reviews/123","reply_format":"回复 approve 或 reject。"}'
```

使用 `hermes kanban owner-action --list`、`hermes kanban owner-action <task-id> --json` 或 `hermes kanban owner-action <task-id> --resolve accepted|rejected|delegated|externally_waiting` 查询或解决个人操作队列。解决操作会被审计，并从活动队列移除该项，但不会推断其他任务状态转换；适当时请另行解除阻塞或重新路由任务。

订阅只会为新建或实质变更的契约发送一条简洁通知。重复相同契约不会产生事件，并会抑制对应的通用 blocked 通知。持久订阅游标可防止 watcher 重启后重放。解除阻塞、完成、归档、计划等待，或以其他方式让任务离开人工操作状态时，契约会自动停用，同时保留事件历史。普通阻塞及其生命周期通知保持不变。
