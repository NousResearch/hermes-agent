...
3. 在长时间操作期间每隔几分钟调用一次 `kanban_heartbeat(note="...")`。心跳表示存活状态；此前未见过的工具与参数签名表示进度。如果任务超过 `kanban.dispatch_stale_timeout_seconds`（默认 7 分钟）没有新的不同进度，调度器会将其回收，即使循环 worker 仍持续发送心跳。回收是无害的（任务返回 `ready` 重新调度，不增加失败计数器），但你会失去当前运行的进度。
...
| `stale` | `{elapsed_seconds, last_heartbeat_at, last_progress_at, progress_age_seconds, progress_timeout_seconds, pid, terminated}` | 任务在 `kanban.dispatch_stale_timeout_seconds`（默认 7 分钟）内没有产生此前未见过的工具与参数签名，即使存活心跳仍然新鲜也会被判定为停滞。调度器向本地 worker（如有）发送 SIGTERM，并将任务重置为 `ready` 重新调度。**不**增加失败计数器（stale 是调度器端的进度检测，不是 worker 故障）。 |
