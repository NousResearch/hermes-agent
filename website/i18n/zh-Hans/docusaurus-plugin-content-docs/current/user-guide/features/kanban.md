...
当 Kanban 被放进 dispatcher 栈中时，一个稳妥的默认 workflow 是：

1. 对传入请求做 triage
2. 组装 context packet：精确路径、日志、约束、acceptance criteria 与 verification 要求
3. 路由到正确的 specialist lane / role
4. 在标记完成前做 verification
5. 同步文档并沉淀可复用经验

对于 infra 类卡片，以 health check 和 rollback note 收尾。对于调试类卡片，坚持 investigate before fix，而不是盲补丁。
