...
- **Workspace（工作区）** —— worker 操作的目录。三种类型：
  ...
  - `worktree` —— 用于编码任务的 git worktree，位于 `.worktrees/<id>/` 下。使用 `worktree:<path>` 固定确切的目标路径。调度器在启动 worker 前创建 worktree（提供 `--branch` 时使用该分支），并将其设为 worker 的工作目录。任务完成、归档取消或启动失败后，会删除干净且以任务 ID 命名的 worktree，但保留对应分支；为防止数据丢失，脏 worktree 和显式自定义路径会保留。
