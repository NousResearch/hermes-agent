...
- **Agent 通过专用 `kanban_*` 工具集驱动看板** —— `kanban_show`、`kanban_list`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link`、`kanban_unblock`、`kanban_set_priority`。调度器在 schema 中已内置任务范围内的工具来启动每个 worker；编排器（orchestrator）配置文件也可以通过 `kanban` 工具集显式启用设置优先级等看板路由操作。模型通过直接调用工具来读取和路由任务，*而不是*通过 shell 执行 `hermes kanban`。详见下方[Worker 如何与看板交互](#how-workers-interact-with-the-board)。
...
| `kanban_set_priority` | （编排器）更新现有任务的非负整数调度优先级，而不改变任务状态。 | `task_id`、`priority` |
