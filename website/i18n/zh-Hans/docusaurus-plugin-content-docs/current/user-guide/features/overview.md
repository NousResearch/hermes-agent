...
- **[子 Agent 委派](delegation.md)** — `delegate_task` 工具可生成具有独立上下文、受限工具集和独立终端会话的子 Agent 实例。默认并发运行 3 个子 Agent（可配置），支持并行工作流。在生产型 dispatcher 体系里，这个 primitive 往往会被稳定的角色路由层封装起来，从而让编排保持 policy-driven，而不是临时拼接。
- **[Kanban（多 Agent 看板）](kanban.md)** — 面向长生命周期、可人工介入或跨角色流转任务的 durable orchestration 层。适用于任务需要跨重启存活、在具名 worker 之间移动，或需要完整审计轨迹的场景。
