...
| [`transform_tool_result`](/user-guide/features/hooks#transform_tool_result) | 任意工具返回后，结果进入上下文之前。返回字符串可替换结果。 |
| [`transform_terminal_output`](/user-guide/features/hooks#transform_terminal_output) | 终端输出捕获后，加入上下文之前。返回字符串可替换该片段。 |
| [`transform_llm_output`](/user-guide/features/hooks#transform_llm_output) | 助手消息追加到对话之前。返回字符串可替换文本。 |
