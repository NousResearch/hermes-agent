...
# Discord + Telegram 消息（包含已修复依赖版本的 Discord 语音组件）
...
:::info
Hermes 会与 `discord.py` 一起明确固定 **PyNaCl** 和 **davey** 的安全版本。这样既保留 Discord 语音支持，也避免 `discord.py` 2.7.1 语音扩展元数据中过时的 PyNaCl 版本上限。
:::
