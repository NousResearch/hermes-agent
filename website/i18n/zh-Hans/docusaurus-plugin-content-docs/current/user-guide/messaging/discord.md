...
Discord 只提供两个队列命令名：`/queue` 及其别名 `/q`。可选的 `prompt` 字段决定具体行为：

- **提供非空 prompt 时：** Hermes 将整个参数作为一条显式队列项，并按 FIFO 顺序处理这些显式队列项；它不会中断当前 agent。命令没有文字子命令，首词也绝不会被解析为队列操作。包含媒体的排队消息继续沿用原有的媒体队列行为。
- **在 Discord 原生 slash interaction 中不提供 prompt 时：** Hermes 打开仅调用者可见的 ephemeral 私有管理器。通过普通 Gateway 文本发送无参数命令仍只显示用法；CLI 中无参数调用也一样。

管理器只能查看并选择删除调用者在当前 session 中尚未消费的显式队列项；清空这些队列项前必须再进行一次确认。普通的繁忙期后续消息（busy follow-up）、内部续跑（continuation），以及其他用户排入的队列项都不会显示，也无法删除。查看或修改队列不会中断正在运行的 agent。

队列 ID 是不透明标识，仅在当前进程和 session 内有效，而且不会持久化。队列项被消费、执行 `/new` 或 `/reset`、或 gateway 重启后，已打开的管理器及其中的 ID 都可能失效；请重新打开管理器以刷新。删除原始 Discord 消息不会取消其内存队列项。

为保护隐私，管理器可以标记队列项包含媒体，但不会公开本地媒体路径，也不会显示该排队 prompt 所回复消息的内容。
