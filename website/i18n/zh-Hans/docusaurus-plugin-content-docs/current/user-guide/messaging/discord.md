...
Discord 提供 `/queue` 及其别名 `/q`，并通过 `/dequeue`（别名 `/dq`）从已显示的视图中删除队列项。

- **提供非空 prompt 时：** Hermes 将整个参数作为一条显式队列项，并按 FIFO 顺序处理这些显式队列项；它不会中断当前 agent。首词始终是 prompt 文本，不会被解析为操作。包含媒体的排队消息继续沿用原有的媒体队列行为。
- **不提供 prompt 时：** Hermes 会在来源频道或线程中发布当前 session 可管理用户队列的短期编号视图。Discord 原生 slash interaction 会显示公开卡片；普通 Gateway 文本会收到等价的公开文本视图。用 `/dequeue N` 或 `/dq N` 删除一个显示项，用 `/dequeue all` 删除该视图冻结的全部项。

队列属于实际路由到的 session，而不属于某个提交者。所有路由到同一 session、且具备命令权限的参与者，都能查看并选择删除其中尚未消费的用户轮次，包括显式 `/queue` 提交，以及 agent 忙碌时排队的普通用户后续消息。线程默认共享（`thread_sessions_per_user: false`）；设置为 `true` 后按参与者隔离。非线程群组/频道的默认值相反：`group_sessions_per_user: true` 会按参与者隔离，设置为 `false` 后才会共享房间及其队列。内部续跑（continuation）始终不会显示或删除。查看或修改队列不会中断正在运行的 agent。

显示的位置是短期快照，不是实时队列下标。每个参与者针对当前 session 持有独立的快照能力。该参与者打开新视图、成功删除、队列项被消费、执行 `/new` 或 `/reset`、切换会话或 gateway 重启，都可能使自己的旧视图失效；请重新运行 `/queue` 后再试。公开 Discord 卡片的按钮仍绑定打开它的成员；其他有权限的成员应重新运行 `/queue` 创建自己的快照。成功变更后会收到替换快照。prompt 内容公开可见，但队列 ID 与快照 token 都是不透明的进程内标识，不会被渲染。Discord 卡片清空前必须确认，且只删除确认时冻结的 ID，因此后来到达的队列项会保留。

管理器可以标记队列项包含媒体，但不会公开本地媒体路径，也不会显示该排队 prompt 所回复消息的内容。删除原始 Discord 消息不会取消其内存队列项。
