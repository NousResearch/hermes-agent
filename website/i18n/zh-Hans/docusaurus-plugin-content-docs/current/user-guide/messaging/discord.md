...
按钮在做出选择后会自动禁用，防止重复点击导致重复解析提示。通过 `~/.hermes/config.yaml` 中的 `agent.clarify_timeout` 配置 agent 侧等待时间（默认 `3600` 秒）。Discord 选项按钮还有单独的组件有效期：如果需要按钮本身保持更久可点击，请设置 `approvals.discord_prompt_timeout`（默认 `300` 秒，范围限制为 30-900 秒）。如果你在 agent 侧超时内没有响应，agent 会以一条哨兵消息解除阻塞并自行调整，而不是一直挂起。
