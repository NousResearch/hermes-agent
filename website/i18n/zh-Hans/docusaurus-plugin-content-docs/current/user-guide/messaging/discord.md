...
- **不提供 prompt 时：** Hermes 打开调用者可管理排队轮次的短期编号视图。在 Discord 原生 slash interaction 中，它会显示为仅调用者可见的 ephemeral 卡片；普通 Gateway 文本会收到等价的文本视图。用 `/dequeue N` 或 `/dq N` 删除一个显示项，用 `/dequeue all` 删除该视图中的全部项。

管理器只能查看并选择删除调用者在来源会话中尚未消费的用户队列项，包括显式 `/queue` 提交，以及 agent 忙碌时排队的普通用户后续消息；Discord 卡片清空前必须再进行一次确认。内部续跑（continuation）以及其他用户排入的队列项都不会显示，也无法删除。查看或修改队列不会中断正在运行的 agent。

显示的位置是短期快照，不是实时队列下标。打开新视图、成功删除、队列项被消费、执行 `/new` 或 `/reset`、切换会话或 gateway 重启，都可能使旧视图失效；请重新运行 `/queue` 后再试。Discord 卡片操作携带私有快照 token，成功变更后会收到替换快照。队列 ID 与这些 token 都是不透明的进程内标识，文本视图不会渲染它们。删除原始 Discord 消息不会取消其内存队列项。

在共享聊天中，文本队列详情会在平台支持时通过私密通知发送；若 Hermes 无法完成私密投递，只会返回安全的重试提示，并撤销未送达视图的快照，不会泄露队列详情或留下隐藏的删除能力。管理器可以标记队列项包含媒体，但不会公开本地媒体路径，也不会显示该排队 prompt 所回复消息的内容。
