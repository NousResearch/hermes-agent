...
当飞书发送 URL 验证挑战（`type: url_verification`）时，webhook 会自动响应，以便你在飞书开发者控制台完成订阅配置。URL 验证要求配置 `FEISHU_VERIFICATION_TOKEN`，并对挑战请求进行 token 校验——token 缺失或不匹配的挑战请求将被拒绝，防止未经认证的远端通过回显攻击者控制的挑战数据来证明端点控制权。
...
JSON 解析前另设认证前限额：每个 IP 在 60 秒固定窗口内最多 600 次请求，最多追踪 4096 个来源。这限制未经认证的解析工作；入口仍需配置请求和并发限制。
