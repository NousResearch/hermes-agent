...
Hermes 可将助手的最终回复渲染为飞书/Lark Card JSON v2。该功能默认不启用：
默认值仍是原有的 legacy 消息格式，因此升级不会自动改变现有机器人的发送行为。

```yaml
platforms:
  feishu:
    extra:
      final_response_format: auto  # legacy | auto | card（默认：legacy）
      markdown_tables: table       # table | markdown（默认：table）
```

| 设置 | 行为 |
|------|------|
| `legacy` | 始终使用现有文本/post 与原生附件投递路径。 |
| `auto` | 对安全的助手最终回复使用 Card v2；包含原生 `MEDIA:` 文件、音频或不支持附件的回复继续走 legacy 路径。 |
| `card` | 优先将助手最终回复渲染为 Card v2，并尽可能合并受支持的图片；不支持的附件仍降级为原生投递。 |

`markdown_tables: table` 会把受支持的 Markdown 表格渲染为 Card v2 原生
table 元素；设为 `markdown` 时，表格会作为卡片内的 Markdown/代码样式内容保留。
非法值会回退到安全默认值（`legacy` 和 `table`）。

该设置仅影响具有语义标记的**助手最终回复**。审批卡片、工具消息、系统通知和
非最终流式更新保持原有行为。线程范围的最终回复如果没有可用的回复锚点，会继续走
legacy 线程路径，而不会在父聊天中创建交互式卡片。超长回复会拆分为满足飞书请求体
和元素限制的多张卡片。如果前一张卡片已经成功投递而后续卡片失败，Hermes 会报告
部分投递，不会重新发送整段回复，从而避免产生重复可见内容。
