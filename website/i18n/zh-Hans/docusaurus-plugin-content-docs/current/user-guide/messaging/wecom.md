...
`/model` 不带参数时渲染模型选择卡片：先提供商页，再进该提供商的模型页。选中模型立即切换；卡片翻转为"正在切换…"提示，结果以补充的 markdown 消息送达。

两页都用**下拉选择器**（`button_selection`）而不是一排名称按钮。企微把卡片宽度固定、按钮文字按每行 3 个排版，所以落在 3 个一行的名称只能渲染约 6 个 ASCII 字符，其余被客户端截断（`Alibaba Token Plan (China)` → `Aliba…`）；下拉选项拿到的是整行宽度，名称因此能看清。

一个下拉最多 10 个选项（协议上限），更长的列表用「上页 / 下页」翻页。选项文案裁剪到约 20 个字符（官方"建议不超过10个字"按中文字宽计），并先去掉尾部日期戳（`deepseek-v4-flash-0731` → `deepseek-v4-flash`）。模型页会预选当前模型，所以点一次「切换」即可应用下拉里的选择。想选不在列表里的模型，直接用 `/model <名称>` 输入。
