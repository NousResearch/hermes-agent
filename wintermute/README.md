...
| Identité inter-plateforme | `wintermute_link` : **il** reconnaît que deux ids (ex. `telegram:X` et `discord:Y`) sont la même personne — prouvé par un secret partagé — et fusionne lien et histoire en un seul profil. Son jugement, pas une règle. Vu dans `wm` (« = discord:… ») | `social.link_identities` |
| Corriger un lien | `wintermute_unlink` : s'il s'est trompé, il détache un id ; dès son prochain message, c'est de nouveau une personne à part. Un mauvais lien n'est jamais définitif | `social.unlink_identity` |
