...
CONTRACT = {
    ...
    "recipes": "Verified graphs are captured automatically and matched by exact fingerprint, scope, target and preconditions. recipe_key remains supported. operation_fingerprint + recipe_scope can select a verified graph without a key. STALE recipes need corrected graph and new canary; no similarity reuse.",
    "adaptive": "Repeatability prose is only a hint. Compilation gates the third distinct equivalent operation, never unrelated actions. Resource authority, approvals and uncertainty still apply.",
    "reasoning_handoff": "NEEDS_REASONING returns completed_until, compact state_ref and safe_to_resume. After diagnosis resume confirmed checkpoints; unresolved dispatched mutations require reconciliation, never blind retry.",
